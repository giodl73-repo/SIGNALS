---
name: discover-brainstorm
description: "Generate a pool of ideas for any topic -- more than you need, so you can rank and filter. Each idea gets: name, one-line"
...
so you can rank and filter. Runs in 3 phases: Phase 1 (pool), Phase 2 (peer-comparison),
Phase 3 (finalize).
...
### 1a. Inertia Baseline (required first)

Before generating any ideas, state the Do Nothing option:
- Current trajectory: what is happening to {{topic}} with no intervention?
- Accumulated cost: what builds up if nothing changes for 6-12 months?
- Displacement threshold: a candidate beats Do Nothing only if it [specific condition].

This paragraph must appear as the first element in the final artifact.

### 1b. Idea Pool

Generate **19-39 additional candidates** for {{topic}} (20-40 total including Do Nothing).
Organize into labeled sections by dimension:
...
- **Status Quo** -- contains exactly the Do Nothing entry from 1a

For each idea:
```
### [Idea Name]
**Pitch**: One sentence.
**Rationale**: Why this serves {{topic}} specifically. How does it change the trajectory
named in 1a -- what does it displace or prevent?
```

Do not mark top-5 yet. No `**` markers on any idea during Phase 1.
