...
### 1. Create the Role System (`.roles/{domain}/`)

Every domain gets a **parent role** and **sub-roles**. Create these files:

#### Parent role: `.roles/{domain}/{domain}.md`
...
#### Sub-roles: `.roles/{domain}/{sub-role}.md`
...
### 2. Create the Pipeline (`.roles/pipelines/{domain}.yaml`)

```yaml
name: "{Domain} Pipeline"
description: "{One-line summary of the pipeline}"
docs: ".roles/pipelines/{domain}/"
...
### 3. Create Stage Documentation (`.roles/pipelines/{domain}/`)
...
Pipeline:  .roles/pipelines/{domain}.yaml
           .roles/pipelines/{domain}/
           ├── 01-{stage-1}.md
           └── 02-{stage-2}.md
