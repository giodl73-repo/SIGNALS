"""Write org-rob-rubric-v11-2026-03-16.md"""
import pathlib

OUT = pathlib.Path("C:/src/sim/simulations/quest/rubrics/org-rob-rubric-v11-2026-03-16.md")

rubric = r"""# org-rob Rubric v11

## Changelog

### v11 (2026-03-16) -- C-28 through C-31 from R11

All five R11 variations score 128/128 under v10 -- R11 is the convergence round for
v10. Four new criteria extracted from the four structural dimensions that differentiate
R11 variations from one another and from V-03 R10.

C-28 (Source-LID Risk Traceability) from V-01 R11: risk register entries carry a
Source-LIDs field citing LID documents that triggered each risk, making risk provenance
directly traceable without prose assertion.

C-29 (Prerequisite Completion Registry) from V-02 R11: a shared prerequisite registry
is established before Stage 1, and each stage marks itself COMPLETE upon exit, citing
a LID. Distinct from C-11 phase gate contracts (per-stage entry/exit conditions); the
registry is a cross-stage audit trail for prerequisite satisfaction.

C-30 (Evidence-Grounded Finding Schema) from V-03 R11: finding schema adds Evidence:
as the third column (after Via: in position 2, before Severity), grounding each finding
in a specific source artifact. Extends C-19's schema-enforcement principle to evidence
provenance; a finding schema that satisfies C-19 but omits the Evidence: column
satisfies C-30=o.

C-31 (Deep Enforcement Mesh) from V-05 R11: the enforcement taxonomy contains 7 or more
VIOLATION-NN entries with index >= 9, each targeting a distinct structural element. V-05
achieves this with VIOLATION-09 through VIOLATION-15: the three C-27 baseline loops
(C-21, C-25, C-11) plus four additional loops targeting Source-LIDs (C-28), prereq
completion (C-29), evidence tier (C-30), and a fourth structural element. The "four
loops beyond the C-27 minimum" is the explicit architectural threshold. V-04 R11 reaches
6 total loops (VIOLATION-09 through -14) -- one short of the C-31 threshold (C-31=o).
...
print(f"Written: {OUT}")
print(f"Size: {len(rubric)} chars, {rubric.count(chr(10))} lines")
