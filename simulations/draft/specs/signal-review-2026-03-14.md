...
`/review:design` and `/review:code` benefit most from custom expert roles. If your domain has specific experts not covered by auto-selection (e.g., a medical informaticist for healthcare data models, a NERC CIP specialist for energy compliance), create them as role files in `.craft/roles/{name}/ROLE.md` with expertise description, evaluation criteria, and typical findings. Custom roles participate in the same lifecycle as stock roles.
