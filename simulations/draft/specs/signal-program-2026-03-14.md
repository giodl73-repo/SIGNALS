...
/program:plan does not benefit from custom roles directly. However, each skill it orchestrates may use custom roles. If your initiative requires custom experts, personas, or stakeholders, create them in `.craft/roles/{name}/ROLE.md` before running the program — all skills in the program will have access to them.
...
**The dogfood program**:
The simulate plugin itself is being developed using /program:plan. The topic prefix is `sim`. The stages are: scout (validate skill catalog via Q01), prove (validate lifecycle via Q02), draft (write namespace specs — this document), review (persona + customer + design review of specs), build (implement the plugin), validate (code review + trace), listen (adoption monitoring). Every artifact in `C:\src\simulate\simulations\` uses the `sim-` prefix. This specification is artifact `program/plan/sim-program-2026-03-14.md` — the program describing itself.
