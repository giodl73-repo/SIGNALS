...
**Two new aspirational criteria extracted from R11 excellence signals:**

| ID | Criterion | Source Axis | Independence from Parent |
|----|-----------|-------------|--------------------------|
| C-38 | **Inertia-first narrative coherence** | V-05 excellence signal 2 | C-33 requires the Phase 0 causal chain contract to declare the formula, source attribution, and routing rule. C-38 requires that declaration to be preceded by a narrative framing block in Phase 0 that earns the formula by establishing the inertia premise and crossover argument before the formula is stated. The formula then appears as the formalization of a stated argument, not as an unexplained initialization item. Satisfying C-33 (the contract exists with all required fields) does not satisfy C-38 — C-38 requires the Phase 0 framing to make the formula semantically earned rather than structurally imposed. A Phase 0 block where the causal contract is the first item without preceding framing passes C-33 but fails C-38. |
| C-39 | **Frontline T-GUARD x dual-contract gate closed enforcement loop** | V-05 excellence signal 3 | C-25 requires T-GUARD to appear first in the trigger table. C-37 requires both Phase 0 contract binaries to co-occur in the gate. C-39 requires BOTH conditions to be simultaneously present AND the COMPLETION STATUS terminal declaration (C-26) to explicitly name both T-GUARD enforcement and the dual-contract gate as jointly active closure mechanisms. Satisfying C-25 and C-37 independently does not satisfy C-39 -- C-39 requires their co-occurrence confirmed by a COMPLETION STATUS declaration that names both mechanisms as jointly responsible for the no-escape-path enforcement outcome. |

**Denominator:** `/30` -> `/32`. Each aspirational criterion = `0.3125` points (10/32).

**Score projections under v12:**
| Variation | Aspirational pass | Composite |
|-----------|-------------------|-----------|
| R11 V-05 motivated (C-28 through C-39 all pass) | 32/32 | 100.00 |
| R11 V-03 split-gate (C-37 fail -> C-39 cascade fail) | 30/32 | 99.38 |
| R11 V-04 conversational (C-25 fail -> C-39 cascade fail) | 30/32 | 99.38 |
| R11 V-01 architect-first (C-34, C-36, C-37 fail -> C-39 cascade fail) | 28/32 | 98.75 |
| R11 V-02 compact PM-first (C-33, C-35, C-37 fail -> C-39 cascade fail) | 28/32 | 98.75 |

**C-39 cascade invariant:** Any variation failing C-37 also fails C-39 (C-37 is a prerequisite for C-39's dual-contract gate component). Any variation failing C-25 also fails C-39 (C-25 is a prerequisite for C-39's frontline T-GUARD component). V-03 (C-37 fail) and V-04 (C-25 fail) each carry one cascade failure into C-39.

**C-38 independence confirmed:** C-38 tests for a Phase 0 framing narrative preceding the causal contract declaration. A variation can satisfy C-33 (contract exists) without satisfying C-38 (framing earned the formula). R11 V-05 is the first variation to demonstrate the framing-first pattern; no single-axis R11 variation that omits Phase 0 narrative investment can achieve C-38.

**No new criterion warranted** from the split-gate fragmentation finding (V-03) or the cascade invariant. C-37's "same gate block" language already covers the V-03 failure mode; the cascade is a deterministic consequence of existing criteria, not a new structural requirement. The split-gate failure mode is added as a named entry in Common Failure Modes.
...
- C-32 is structurally testable: a reviewer reads the option anatomy template and confirms whether `PM FRAMING:` and `ARCHITECT VALIDATION:` appear as the first two typed slots in the template structure for every option entry. No phase scan required -- the template either carries both slots at the head of option structure, or it does not.
...
- C-35 is structurally testable: a reviewer reads the PREREQUISITE GATE block and confirms whether a binary item for Phase 0 causal chain contract verification is present. No Phase 0 scroll required -- the gate item either exists or it does not. Combined with C-33, a reviewer confirms Phase 0 causal contract presence and gate-item verification from two single-block reads.
- C-36 is structurally testable: a reviewer reads the PREREQUISITE GATE block and confirms whether a binary item for Phase 0 option anatomy contract verification is present. No Phase 0 scroll required -- the gate item either exists or it does not. Combined with C-34, a reviewer confirms Phase 0 anatomy contract presence and gate-item verification from two single-block reads. C-35 and C-36 are independently testable: each covers a different Phase 0 contract; a gate with one but not the other passes the present criterion and fails the absent one.
- C-37 is structurally testable: a reviewer reads the PREREQUISITE GATE block and confirms whether BOTH a causal chain contract binary (C-35) AND an option anatomy contract binary (C-36) are present in the same gate block. The test is a single conjunctive read: either both binaries exist in the gate, or they do not. A gate with only one Phase 0 contract binary fails C-37 even if that binary is fully correct for its own criterion.
- C-38 is structurally testable: a reviewer reads the Phase 0 block and confirms whether a narrative framing statement appears before the CAUSAL CHAIN CONTRACT declaration. The framing statement must name (1) the inertia premise -- the cost of not deciding accumulates per sprint -- and (2) the crossover argument -- INERTIA OFFSET sprint must be compared against TEMPORAL ANCHOR to determine whether action is still viable. A Phase 0 block where the causal contract appears as the first item, with no preceding framing narrative, fails C-38 regardless of C-33 status.
- C-39 is structurally testable via three sequential reads: (1) position 1 in the trigger table confirms T-GUARD first (C-25 component); (2) the PREREQUISITE GATE block confirms both contract binaries present (C-37 component); (3) the COMPLETION STATUS terminal value confirms both mechanisms are explicitly named as jointly active. A document where any of the three reads fails -- T-GUARD not first, only one contract binary in the gate, or COMPLETION STATUS not naming both T-GUARD enforcement and the dual-contract gate -- fails C-39. The three reads together confirm the closed enforcement loop was active during document authoring.

The R11 discriminator pattern confirmed C-38 and C-39 structural independence: V-05 provided Phase 0 inertia framing before the causal contract declaration (C-38 PASS) and a COMPLETION STATUS terminal value naming both T-GUARD and the dual-contract gate as joint enforcement closure mechanisms (C-39 PASS). The R11 cascade invariant was confirmed: any variation that fails C-37 automatically fails C-39 (gate component absent); any variation that fails C-25 automatically fails C-39 (frontline component absent). V-03's split-gate failure (C-37 FAIL from sub-section separation) cascades to C-39 FAIL even though V-03 provides both contract binaries -- the gate component of C-39 requires C-37 to pass, and C-37 requires co-presence in the same undivided block.
...
| C-35 | **PREREQUISITE GATE extension for Phase 0 causal chain contract verification** | depth | The PREREQUISITE GATE block (C-20) is extended with a binary check item confirming that the Phase 0 causal chain contract (C-33) is present and complete: specifically that (1) the formula `TEMPORAL ANCHOR - INERTIA OFFSET = DECISION LEAD TIME` appears as a typed field in Phase 0, (2) source phase attribution is declared for both input terms, and (3) a routing rule (T-GUARD or named T-NN trigger) is stated at initialization. A reviewer confirms Phase 0 causal contract completeness by reading a single gate item at Phase 11, without scrolling to Phase 0. Satisfying C-33 (the contract exists at Phase 0 with formula, source attribution, and routing rule) does not satisfy C-35 -- C-33 requires the contract to exist; C-35 requires the PREREQUISITE GATE to include a binary verification item that makes Phase 0 contract presence machine-checkable at Phase 11 independently of the Phase 0 block. A gate that confirms register ordering (C-20), inertia-axis completeness (C-27), and lead-time completeness (C-31) but omits a Phase 0 causal contract binary item fails C-35 regardless of whether C-33 passes. The structural pattern: C-20 makes C-17 single-block auditable; C-27 extends the gate for inertia; C-31 extends for lead time; C-35 extends for Phase 0 causal contract. Each new depth axis adds one binary to the PREREQUISITE GATE, preserving the gate's single-block auditability property. |
| C-36 | **PREREQUISITE GATE extension for Phase 0 option anatomy contract verification** | depth | The PREREQUISITE GATE block (C-20) is extended with a binary check item confirming that the Phase 0 option anatomy contract (C-34) is present and complete: specifically that (1) `PM FRAMING:` is declared as the first required field, (2) `ARCHITECT VALIDATION:` is declared as the second required field, and (3) a named T-NN trigger is stated for role-slot violations. A reviewer confirms Phase 0 option anatomy contract completeness by reading a single gate item at Phase 11, without scrolling to Phase 0. Satisfying C-34 (the anatomy contract exists at Phase 0 with field declarations and a named trigger) does not satisfy C-36 -- C-34 requires the contract to exist; C-36 requires the PREREQUISITE GATE to include a binary verification item that makes Phase 0 anatomy contract presence machine-checkable at Phase 11 independently of the Phase 0 block. A gate that includes a causal chain contract binary (C-35) but omits an anatomy contract binary fails C-36 regardless of whether C-34 passes. Mirrors C-35 for the orthogonal Phase 0 contract: C-35 extends the gate for the causal chain contract (C-33 axis); C-36 extends the gate for the option anatomy contract (C-34 axis). Two gate binaries for two orthogonal Phase 0 contracts. |
| C-37 | **PREREQUISITE GATE dual-contract coverage** | depth | The PREREQUISITE GATE block contains BOTH a binary item for Phase 0 causal chain contract verification (C-35 axis) AND a binary item for Phase 0 option anatomy contract verification (C-36 axis) in the same gate block. A gate with only one Phase 0 contract binary creates asymmetric auditability: a reviewer can confirm one Phase 0 contract from Phase 11 but must scroll to Phase 0 for the other. When both binaries are present together, a reviewer confirms both orthogonal Phase 0 contracts from a single Phase 11 block without any Phase 0 scroll. Satisfying C-35 (causal chain contract binary in gate) does not satisfy C-37. Satisfying C-36 (anatomy contract binary in gate) does not satisfy C-37. C-37 requires co-presence: both binaries must exist in the same gate block. A single-axis variation that addresses only one Phase 0 contract will supply at most one binary and therefore cannot satisfy C-37. Only a variation that combines the C-34 and C-33 axes simultaneously -- and extends the gate for both -- can reach C-37. The structural pattern: C-20 establishes the gate; C-27/C-31/C-35/C-36 each extend it by one binary for one depth axis; C-37 requires the two Phase 0 contract binaries to co-occur, making the gate comprehensively audit both Phase 0 initialization contracts in a single block. |
| C-38 | **Inertia-first narrative coherence** | behavior | The Phase 0 block includes a narrative framing statement appearing before the CAUSAL CHAIN CONTRACT declaration (C-33) that establishes: (1) the inertia premise -- the cost of not deciding accumulates per sprint at a computable rate, and (2) the crossover argument -- the INERTIA OFFSET sprint at which cumulative implementation cost equals cumulative inertia cost must be compared against the named TEMPORAL ANCHOR to determine whether action remains viable. The causal chain formula (`TEMPORAL ANCHOR - INERTIA OFFSET = DECISION LEAD TIME`) then appears as the formalization of this stated argument, not as an initialization artifact whose rationale is implied. Satisfying C-33 (the contract exists with formula, source attribution, and routing rule) does not satisfy C-38 -- C-38 requires the Phase 0 framing to make the formula semantically earned rather than structurally imposed. A Phase 0 block that declares the causal contract as its first item without preceding framing passes C-33 but fails C-38. The distinction: C-33 tests what the contract contains; C-38 tests what precedes the contract and whether the framing makes the formula an inevitable consequence of the argument rather than an administrative initialization item. |
| C-39 | **Frontline T-GUARD x dual-contract gate closed enforcement loop** | behavior | The document simultaneously satisfies C-25 (T-GUARD appears first in the trigger table at Phase 0 initialization) and C-37 (PREREQUISITE GATE contains both Phase 0 contract binaries in the same block), AND the COMPLETION STATUS terminal declaration (C-26) explicitly names both T-GUARD enforcement and the dual-contract gate as jointly active enforcement mechanisms. The two together create a closed enforcement loop: T-GUARD catches any structural gap during document authoring (frontline, pre-gate); the dual-contract gate confirms both Phase 0 initialization contracts before recommendation sign-off (terminal audit). Between them, no structural absence can proceed silently to the recommendation phase. Satisfying C-25 (T-GUARD first) does not satisfy C-39. Satisfying C-37 (both gate binaries) does not satisfy C-39. Satisfying C-26 (COMPLETION STATUS declared) does not satisfy C-39. C-39 requires all three to co-occur AND the terminal COMPLETION STATUS value to explicitly name both T-GUARD enforcement and the dual-contract gate as jointly responsible for the enforcement closure. A COMPLETION STATUS that reads "T-GUARD enforced all requirements successfully -- no violations detected" without naming the dual-contract gate, or that confirms the dual-contract gate without referencing T-GUARD's frontline role, passes C-26 but fails C-39. |
...
- Option anatomy template places a non-role typed field (e.g., DESCRIPTION:) before `PM FRAMING:` -- PM FRAMING: is not the first typed field overall; the role-sequence is satisfied for role fields in isolation but violated in the full template ordering; C-32 requires PM FRAMING: as the first typed field unconditionally; a Phase 0 OPTION ANATOMY CONTRACT (C-34) that declares PM FRAMING: first is the reliable mechanism for preventing this drift (fails C-32; C-34 absence allows option template to deviate from intended ordering)
...
- Phase 0 causal chain contract is present (C-33 PASS) but the PREREQUISITE GATE block contains no binary item for Phase 0 causal chain contract verification -- reviewer must scroll to Phase 0 to confirm contract presence; the gate's single-block auditability property does not extend to Phase 0 contract completeness; C-33 passes, C-35 fails (no gate binary for Phase 0 causal contract)
- Phase 0 option anatomy contract is present (C-34 PASS) but the PREREQUISITE GATE block contains no binary item for Phase 0 option anatomy contract verification -- reviewer must scroll to Phase 0 to confirm anatomy contract presence; the gate's single-block auditability property does not extend to Phase 0 anatomy contract completeness; C-34 passes, C-36 fails (no gate binary for Phase 0 anatomy contract)
- PREREQUISITE GATE contains a binary item for one Phase 0 contract (causal chain or anatomy) but not both -- one Phase 0 contract is machine-checkable from Phase 11; the other requires a Phase 0 scroll; asymmetric auditability; C-35 or C-36 passes (whichever gate binary is present), C-37 fails (both binaries must co-occur in the same gate block for dual-contract coverage)
- PREREQUISITE GATE contains binary items for both Phase 0 contracts (C-35 PASS, C-36 PASS) but they appear in separate sub-sections within the gate block -- sub-section separation violates C-37's co-presence requirement even though both binaries are structurally present; C-35 and C-36 pass; C-37 fails (split-gate fragmentation; R11 V-03 failure mode)
- Phase 0 CAUSAL CHAIN CONTRACT (C-33) declared as the first item in the Phase 0 block with no preceding framing narrative -- the formula appears as an initialization artifact whose rationale is assumed; reviewer cannot confirm whether the inertia premise and crossover argument were considered before the formula was stated (fails C-38; satisfies C-33 if formula, source attribution, and routing rule are all present)
- T-GUARD appears first in trigger table (C-25 PASS) and both Phase 0 contract binaries are present in the gate (C-37 PASS) but the COMPLETION STATUS terminal declaration names T-GUARD enforcement only, or the dual-contract gate only, or uses a generic completion statement without naming either mechanism explicitly -- the co-operative enforcement closure is not confirmed; reviewer cannot determine from the terminal state whether both mechanisms were jointly active (fails C-39; satisfies C-25, C-37, and C-26 individually)
...
Round 9 produced scorecards for two complete variations against the v9 rubric (/26
denominator). V-01 and V-02 both reached co-ceiling at 99.23. Both predictions of 100.0
were wrong -- the two new v9 criteria decompose into two orthogonal axes with no
...
**V-01/V-02 independence confirmed.** V-01: C-34 PASS, C-33 FAIL, C-35 FAIL. V-02:
C-33 PASS, C-35 PASS, C-32 FAIL, C-34 FAIL. Each variation fails the two criteria the
other passes.
...
## Round 10 Discriminator Notes

Round 10 produced scorecards for two complete variations against the v10 rubric (/28
denominator). R10 V-01 scored 99.64 (27/28 aspirational pass, C-35 only failure). R10
V-02 scored 99.64 under author prediction or 99.29 under strict C-32 reading. Both R10
targets of 100.0 were wrong.

**C-36: PREREQUISITE GATE extension for Phase 0 option anatomy contract verification
extends the gate's machine-checkable coverage to the anatomy contract domain.** R10 V-01's
excellence signal was a gate item confirming Phase 0 OPTION ANATOMY CONTRACT presence and
completeness.

**C-37: Dual-contract gate coverage requires co-presence of both Phase 0 contract binaries
in the same gate block.** R10 V-01 supplies C-36 but not C-35. R10 V-02 supplies C-35
but not C-36. Neither can satisfy C-37. A motivated R11 variation must combine C-33,
C-34, C-35, and C-36 simultaneously.

**C-32 structural asymmetry finding.** R10 V-02's DESCRIPTION-first template ordering
violates C-32's "first typed field" condition. V-01's Phase 0 OPTION ANATOMY CONTRACT
prevents this drift by declaring PM FRAMING: first at Phase 0. C-32's pass condition is
clarified in v11: PM FRAMING: must be first among ALL typed fields, not only among role
fields.

**Variation ranking by structural value beyond rubric (R10):**

| Rank | Variation | Why |
|------|-----------|-----|
| 1 | V-01 | Phase 0 anatomy contract gate binary: seeds C-36; gate's auditability extended to anatomy contract domain |
| 2 | V-02 | Exposes C-32 strict-field-ordering failure; seeds C-37's dual-contract requirement via its own C-36 absence |

---

## Round 11 Discriminator Notes

Round 11 produced five complete variation scorecards against the v11 rubric (/30
denominator). V-05 (Inertia + motivated) scored 100.0 (30/30). V-03 (Split-gate
lifecycle) and V-04 (Conversational register) scored 99.67 (29/30, single designed fail
each: C-37 and C-25 respectively). V-01 (Architect-first) and V-02 (Compact tables
PM-first) scored 99.00 (27/30, three-failure designs). All five Golden.

**Prediction table correction.** The round summary predicted 99.33 for V-01 and V-02.
Per-variation hypotheses are authoritative: both have 3-failure designs (27/30). Actual
score is 99.00 for each.

**C-37 confirmed as R11 discriminator.** C-37 appears in 3 of 5 failure sets. The cascade
invariant holds deterministically: 1 absent Phase 0 contract causes exactly 3 aspirational
failures (contract criterion + gate binary criterion + C-37). V-03 demonstrates the
structurally interesting case: both contracts and both gate binaries present, C-37 fails
purely from sub-section separation -- adding structure can violate a conjunction requirement
without any content missing.

**C-37's "same gate block" language covers the split-gate fragmentation failure (V-03).** No
new v12 criterion added. The V-03 failure mode is added as a named entry in Common Failure Modes.

**C-38: Inertia-first narrative coherence operationalizes the "earned formula" distinction
for C-33.** V-05's Phase 0 block established the inertia premise and crossover argument as
a framing statement before declaring the CAUSAL CHAIN CONTRACT. The formula thus appears
as the formalization of an argument the document has already made. Any motivated variation
can declare the causal contract at Phase 0 and pass C-33; only a variation that earns the
formula through preceding framing passes C-38.

**C-39: Frontline T-GUARD x dual-contract gate closed enforcement loop captures the
combination effect of C-25 and C-37 as confirmed by COMPLETION STATUS.** V-05
demonstrated that when T-GUARD fires first for any structural gap (C-25) AND the gate
covers both Phase 0 contracts (C-37), there is no escape path for structural absence.
C-39 requires the COMPLETION STATUS terminal declaration to name both mechanisms jointly --
the terminal state must confirm the enforcement loop was closed, not merely that individual
criteria were satisfied.

**C-38 independence from C-33.** C-33 tests what the contract contains; C-38 tests what
precedes the contract. A Phase 0 block can satisfy all C-33 requirements while failing
C-38 if the contract appears as the first Phase 0 item without inertia framing.

**C-39 independence from C-25, C-37, C-26.** Satisfying all three individually does not
satisfy C-39. The COMPLETION STATUS must name both T-GUARD enforcement and the
dual-contract gate as jointly active mechanisms. A generic completion statement naming
only one mechanism satisfies C-26 but fails C-39.

**C-39 cascade invariant.** Any variation failing C-37 automatically fails C-39 (gate
component absent). Any variation failing C-25 automatically fails C-39 (frontline
component absent). V-03 (C-37 fail) and V-04 (C-25 fail) each carry one cascade failure
into C-39 under v12.

**Variation ranking by structural value beyond rubric (R11):**

| Rank | Variation | Why |
|------|-----------|-----|
| 1 | V-05 | Inertia-first narrative framing earns the causal formula; T-GUARD x dual-contract gate named as joint closure in COMPLETION STATUS: seeds C-38 and C-39 |
| 2 | V-03 | Both contracts present, both gate binaries present, split-gate fragmentation: confirms C-37 co-presence as structural constraint; lower bound on C-38/C-39 |
| 3 | V-04 | Register orthogonality for conversational register confirmed again; C-25 single designed fail |
| 4 | V-01 | Cascade invariant confirmed at 3 deterministic failures from 1 absent contract |
| 5 | V-02 | Cascade invariant confirmed; DESCRIPTION-first drift when anatomy contract absent |
...
- **C-35**: Not demonstrated in trace -- no PREREQUISITE GATE block present (C-20 fails); no Phase 0 causal chain contract present (C-33 fails); a gate binary for Phase 0 causal contract verification cannot exist without both a gate block and a contract to verify; C-35 would fail independently even if C-20 and C-33 passed, because the gate binary is a distinct gate item not derivable from C-20's register-ordering binaries or C-33's contract content
- **C-36**: Not demonstrated in trace -- no PREREQUISITE GATE block present (C-20 fails); no Phase 0 option anatomy contract present (C-34 fails); a gate binary for Phase 0 anatomy contract verification cannot exist without both a gate block and an anatomy contract to verify; C-36 would fail independently even if C-20 and C-34 passed
- **C-37**: Not demonstrated in trace -- no PREREQUISITE GATE block present (C-20 fails); neither Phase 0 contract is present (C-33 and C-34 both fail); dual-contract gate coverage cannot exist without a gate block containing both contract binaries; C-37 would fail independently even if C-35 and C-36 both passed individually, because C-37 requires their co-presence in the same gate block
- **C-38**: Not demonstrated in trace -- no Phase 0 initialization block present (C-33 fails); an inertia-framing narrative cannot precede a contract that does not exist; C-38 would fail independently even if C-33 passed, because the narrative framing block is a distinct Phase 0 artifact from the contract declaration itself -- C-38 requires the framing to appear before the contract, not merely alongside it
- **C-39**: Not demonstrated in trace -- no PREREQUISITE GATE block present (C-20 fails), no T-GUARD trigger table present (C-25 fails), and no COMPLETION STATUS field present (C-26 fails); the closed enforcement loop cannot exist without all three prerequisite artifacts; C-39 would fail independently even if C-25, C-37, and C-26 all passed, because the COMPLETION STATUS terminal declaration must explicitly name both T-GUARD and the dual-contract gate as jointly active mechanisms -- a generic completion statement or one naming only one mechanism satisfies C-26 but fails C-39
