Written to `simulations/quest/rubrics/corps-scan-rubric-v5-2026-03-17.md`.

---

**Three new criteria extracted from R4:**

**C-20 -- Amend phase names an explicit anti-pattern for non-actionable offers** (V-01, V-02)
V-01 C-08 evidence: `"'Let me know if you want changes' does not satisfy this phase" explicit`. V-02 C-08 evidence: identical pattern. C-08 requires actionable amend options; C-20 additionally requires the anti-pattern named by example inside the amend directive. This is C-18's VIOLATION discipline applied to the amend phase boundary. Requires C-08.

**C-21 -- Each gate labeled with a category name identifying its enforcement type** (V-01, V-02)
V-01 C-19 evidence: `"GATE 1 (Structural Ordering) + GATE 2 (Semantic Traceability) explicitly labeled as independent"`. V-02 C-19 evidence: same labels present. C-19 requires structural independence; C-21 requires the category label in parentheses adjacent to each gate header so the failure mode is self-describing without reading the logic. Requires C-19.

**C-22 -- Every structural constraint in every phase expressed as a separate IVR triple** (V-01, V-02)
V-01 C-18 evidence: `"four separate IVR triples in Phase 3"`. V-02 C-18 evidence: `"All SPECs use full IVR"`. C-18 requires IVR on at minimum two constraints; C-22 requires the discipline applied to every structural constraint in every phase. Requires C-18.

---

**Score change:** 19 criteria → 22 criteria, 190 pts max → **220 pts max**. Golden threshold (all 5 essentials + composite ≥ 80 pts) unchanged.
ural constraint in every phase has its own dedicated IVR triple. A prompt that
applies IVR to only the two required constraints while leaving other structural constraints as
prose directives does not pass. Evidence: V-01 "four separate IVR triples in Phase 3"; V-02
"All SPECs use full IVR" across all phases. Requires C-18 to pass.

**Score change:** 19 criteria --> 22 criteria, 190 pts max --> **220 pts max**. Golden threshold
...
V-02 (R3) invented this criterion. The prompt directs the model to output a YES/NO Completion
Test block before advancing phases. Distinct from C-15: C-15 makes the boundary auditable to a
reader of the prompt; C-17 makes it auditable to a reader of the model's response. A predicate
in the prompt with no output directive fails.

**C-18 -- Constraints expressed as INVARIANT / VIOLATION / REPAIR triples**
V-03 (R3) introduced this pattern. Every structural constraint has three named blocks: what must
hold, a concrete anti-pattern example, and the repair steps. C-16 required a repair instruction;
C-18 additionally requires the VIOLATION block naming the anti-pattern by example. An INVARIANT
+ REPAIR without VIOLATION fails -- the VIOLATION is what makes the constraint unambiguous.

**C-19 -- Dual gate architecture: inventory gate and traceability gate structurally independent**
V-04 (R3) added a second gate independent of the C-13/C-14 inventory gate. C-13 requires one
gate; C-19 requires two named, structurally independent gates. A single block checking both
conditions does not pass. Requires C-13 and C-16 to both pass.
...
| Aspirational | C-09 -- C-22 | 140 pts |
| **Total** | 22 criteria | **220 pts** |
...
Fourteen criteria. Each worth 10 pts (140 pts total if all pass).

C-09 and C-10 carry forward from v1. C-11, C-12, C-13 added in v2. C-14, C-15, C-16 added in
v3. C-17, C-18, C-19 added in v4. C-20, C-21, C-22 new in v5.
...
- Pass condition: Each of the two independent gates (C-19) carries a category label in
  ...
  Ordering)" and "GATE 2 (Semantic Traceability)". The category label makes the gate's failure
...
| Aspirational | C-09 through C-22 (14 criteria) | 140 pts |
| **Total** | | **220 pts** |
...
|-------|-----------|--------|
| 220 / 220 | 5E + 3R + 14A all PASS | Perfect |
| 130 / 220 | 5E + 3R + 5A PASS | Golden |
| 80 / 220 | 5E + 3R + 0A PASS | Golden (threshold) |
| 70 / 220 | 5E + 2R + 0A PASS | Below threshold -- not golden |
