Written to `simulations/quest/rubrics/corps-scan-rubric-v7-2026-03-17.md`.

---

**Three new criteria from R6 evidence:**

**C-28 -- META-RULE enumerates all IVR triples by label with a count instruction**
Beyond C-26 (meta-rule declares non-binding). The meta-rule becomes a verifiable constraint manifest: lists every IVR label by identifier, states the total count. Discrepancy detection becomes mechanical. Requires C-25 + C-26.

**C-29 -- Explicit ENTRY contracts at phase start pairing with EXIT contracts**
Beyond C-27 (conditional advance in exit). Each phase opens with a named ENTRY block declaring what must already hold before entry. Pairs with EXIT contracts to form a complete phase contract. Requires C-27.

**C-30 -- Completion infrastructure covers every phase without exception**
Beyond C-15/C-17/C-27 (minimum two phases). Full suite — predicate + directed test block + conditional advance — required on every phase. No phase left undirected. Requires C-15 + C-17 + C-27.

**Score change:** 27 → 30 criteria, 270 → **300 pts max**. Golden threshold unchanged.
 are binding"
without enumerating them or stating a count does not pass. Requires C-25 and C-26.

**C-29 -- Explicit ENTRY contracts at phase start pairing with EXIT contracts** (V-02)
V-02 C-09 evidence: "ENTRY-P3 requires Phase 2 EXIT all YES." C-27 requires each phase's
completion test to include a conditional advance instruction (governing exit from the phase);
C-29 additionally requires each phase to open with a formal ENTRY contract block that declares
the preconditions that must already hold before that phase begins -- e.g., "ENTRY: Phase 2 EXIT
all YES." Pairing ENTRY and EXIT contracts creates a complete phase contract: entry declares
what was verified before entering; exit declares what must be verified before leaving. A prompt
with only EXIT conditions (no corresponding ENTRY declarations at phase start) does not pass.
The ENTRY block must be a named structural element, not a prose advisory. Requires C-27.

**C-30 -- Completion infrastructure covers every phase without exception** (V-02)
V-02 C-17 evidence: "Completion test blocks directed for all phases (P1 through P4)." V-02 C-27
evidence: "All 4 phases." C-15 requires incompleteness predicates for at minimum the inventory
phase and YAML authoring phase; C-17 requires directed completion test output for at least those
two phases; C-27 requires conditional advance instructions in each test block. C-30 requires the
full suite -- incompleteness predicate, directed visible test block, and conditional advance
instruction -- to be present for every phase in the prompt, with no phase left undirected. A
prompt that satisfies the C-15/C-17/C-27 minimums but leaves any phase without a complete
completion suite does not pass. Requires C-15, C-17, and C-27 to all pass.

---

**Score change:** 27 criteria --> 30 criteria, 270 pts max --> **300 pts max**. Golden threshold
...
| Aspirational | C-09 -- C-30 | 220 pts |
| **Total** | 30 criteria | **300 pts** |
...
Twenty-two criteria. Each worth 10 pts (220 pts total if all pass).
...
added in v6. C-28, C-29, C-30 new in v7.
...
| Aspirational | C-09 through C-30 (22 criteria) | 220 pts |
| **Total** | | **300 pts** |
...
|-------|-----------|--------|
| 300 / 300 | 5E + 3R + 22A all PASS | Perfect |
| 130 / 300 | 5E + 3R + 5A PASS | Golden |
| 80 / 300 | 5E + 3R + 0A PASS | Golden (threshold) |
| 70 / 300 | 5E + 2R + 0A PASS | Below threshold -- not golden |
