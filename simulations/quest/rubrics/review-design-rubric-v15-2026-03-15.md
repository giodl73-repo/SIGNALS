# review-design Rubric — v15

**Skill**: `review-design`
**Rubric version**: v15 (C-01 through C-46, denominator 39)
**Extracted from**: R15 scorecard (2026-03-15)
...
Note: aspirational denominator changed from 37 to 39 in v15 (added C-45, C-46). Each
aspirational criterion contributes ~0.256 points at full pass.
...
| Named halt corrective action uses bare verb without referent | C-37 | "Populate. Halt." fails; anaphoric "Populate it. Halt." passes only when "it" is unambiguously resolved by the trigger clause |
| Named halt names field but not required value class | C-37 | "Replace the Type value" fails when AGREE/SPLIT is the required closed enumeration; the value class must be named explicitly |
...
| No dedicated BLOCK 2.7 P1 Section registry before BLOCK 3 | C-43 | C-42/F-28 satisfied by live BLOCK 2 lookup or inline BLOCK 5 pre-check; upstream committed registry absent; anchor-then-verify chain incomplete at the section domain |
| BLOCK 2.7 position constraint not negatively named | C-43 | Pass condition names inline BLOCK 5 pre-check as non-conformant; omitting this explicit negative naming leaves the enforcement boundary ambiguous |
| Cross-block mismatch halt names only one resolution path | C-44 | F-28 (or equivalent) corrective action states "correct the downstream value" without also naming the upstream addition path; C-37 passes (corrective action present), C-44 fails (bidirectional coverage absent) |
| Bidirectional labeling applied to F-28 only, not all mismatch halts | C-44 | F-03, F-10, F-15, F-16, F-17 remain single-path; class-level uniformity required; partially applying C-44's pattern to one halt fails the criterion |
| BLOCK 3 produces no typed Elevation Record | C-45 | BLOCK 5 orders amendments by priority (C-39) but classification was not produced as a named typed artifact in BLOCK 3; BLOCK 5 re-computes what BLOCK 3 should have determined |
| BLOCK 5 CONSENSUS ELEVATION RULE does not reference BLOCK 3 | C-45 | Priority ordering present in BLOCK 5 but no named rule citing BLOCK 3 Elevation Record as source; intermediate-artifact consumption not declared |
| Lifecycle block closes without SEALED attestation | C-46 | Block ends with heading transition or blank line; verified-progression statement absent; success-path contract incomplete for that block |
| SEALED statement names no verification condition or next block | C-46 | Nominal SEALED marker present but does not attest what was verified or what proceeds; structural form without semantic content fails |
...
| v14 | 2026-03-15 | Added C-43 (BLOCK 2.7 P1 Section registry for upstream amendment target lock) and C-44 (cross-block mismatch halts specify both upstream and downstream resolution paths) from R14 scorecard excellence signals; updated aspirational denominator from 35 to 37; updated C-37 with R14 signal note clarifying the anaphoric resolution boundary (unambiguous referent in trigger clause passes; bare verb without referent fails); updated C-42 pass condition to note F-28 may resolve against BLOCK 2.7 registry; added R14 signal note to C-42 documenting three-strategy placement experiment |
| v15 | 2026-03-15 | Added C-45 (BLOCK 3 Elevation Record as typed intermediate artifact) and C-46 (BLOCK N SEALED lifecycle gate statements) from R15 scorecard excellence signals; updated aspirational denominator from 37 to 39; updated C-37 with R15 signal note clarifying that naming a field without naming the required value class fails when the class is a closed enumeration; updated C-43 pass condition with POSITION CONSTRAINT negative naming note; updated C-44 pass condition to enumerate all 6 cross-block mismatch halts (F-03, F-10, F-15, F-16, F-17, F-28) and added R15 signal note documenting V-05 as first full-class PASS with Downstream fix / Upstream fix labels |
