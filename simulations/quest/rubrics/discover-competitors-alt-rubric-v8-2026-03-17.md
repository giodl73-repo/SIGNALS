Rubric v8 written to `simulations/quest/rubrics/discover-competitors-alt-rubric-v8-2026-03-17.md`.

---

**3 new aspirational criteria (C-34 -- C-36):**

| ID | Pattern | Source | Pass Condition Summary |
|----|---------|--------|------------------------|
| C-34 | Inter-slot dependency column | R8 V-01, V-04 | Output contract includes a "Required by" column naming upstream slot dependencies per slot — blast radius of any gate failure is readable at contract time. Independent of C-27 (neither implies the other). |
| C-35 | Syntactic format template for cross-dimensional proof slot | R8 V-02 | Slot 5 has a parse-ready template (e.g., pipe-delimited `REDUCTION-1 NO: [...] \| REDUCTION-2 NO: [...] \| THEREFORE: [...]`), not a structural description. Strictly stronger than C-33. |
| C-36 | Cross-dimensional proof encoded as structural PASS/FAIL table | R8 V-03, V-04 | Phase 4 is a 4-row gate table (SOURCE / REDUCTION-1 / REDUCTION-2 / THEREFORE), each row with required value format and named failure state. SOURCE position enforced by table structure, not prose. Strictly stronger than C-15. |

**Scoring changes:**
- Aspirational tier: 25 → 28 criteria, max 125 → 140
- Max composite: 215 → 230
- Formula denominator: `aspirational_pass / 28 × 140`
- Grade bands: Exceptional ≥ 216, Strong ≥ 194, Passing ≥ 90
yntactically checkable template does not satisfy. C-35 is a strictly stronger form of C-33: C-35 satisfaction implies C-33; the converse is not true. |
| C-36 | Cross-dimensional proof encoded as structural PASS/FAIL table | R8 V-03, V-04 | The cross-dimensional proof phase (Phase 4 or equivalent) is structured as a named PASS/FAIL table with at least four rows -- SOURCE, REDUCTION-1, REDUCTION-2, and THEREFORE (or equivalent labels) -- each with a required value format and a named failure state. A prose-instructed proof phase with code-block output format does not satisfy; the proof structure must itself be a gate table where each row is an independently reviewable checkpoint. C-36 is a strictly stronger form of C-15: SOURCE row position is enforced by table structure (first row), not by prose instruction; C-36 satisfaction implies C-15; the converse is not true. |

**Scoring changes:**
- Aspirational tier: 25 -> 28 criteria, max 125 -> 140
- Max composite: 215 -> 230
- Formula denominator: `aspirational_pass / 28 x 140`
- Grade bands rescaled: Exceptional >= 216, Strong >= 194, Passing >= 90 (golden threshold unchanged)

**Dependency chain:**
- C-35 -> C-33 (strictly stronger; C-35 satisfaction implies C-33)
- C-36 -> C-15 (strictly stronger; C-36 satisfaction implies C-15)
- C-34 independent of C-27 (orthogonal contract columns; neither implies the other)
...
|----|-----------|----------|----------------|
| C-01 | Inertia assessed first | behavior | The skill instruction assesses the status quo competitor (C0) before any external competitor is evaluated. The inertia entry for C0 -- including at minimum a threat level and a stickiness mechanism -- is completed as a prerequisite to Phase 2 competitive enumeration. A skill that defers inertia to a later phase does not satisfy even if C0 appears in the final competitor table. |
| C-02 | Focus woven, not appended | behavior | The skill instruction directs the model to apply the user-supplied focus dimension as a column or attribute across all competitor rows (including C0), not as a post-hoc narrative layer added after the table is complete. A skill that generates a competitive table then produces a separate "focus analysis" block does not satisfy. |
| C-03 | Threat level per competitor | format | The competitor output includes a "Threat" column (or equivalent named field) populated for every competitor row, including C0. A skill that names competitors without a distinct threat assessment field does not satisfy. |
| C-04 | Whitespace identified | behavior | The skill instruction explicitly directs the model to identify at least one whitespace gap -- an area not strongly owned by any evaluated competitor -- and output it as a named finding. A skill that produces competitive analysis without a whitespace or gap section does not satisfy. |
| C-05 | Auto-detect without prompting | behavior | The skill instruction directs the model to infer TOPIC and FOCUS from the repository context (README, CLAUDE.md, package.json, or equivalent) when neither is supplied by the user. A skill that requires explicit user input for both fields does not satisfy; the detection step must be specified in the instruction. |
...
## Aspirational Criteria (weight = 140 points total, 5 points each)
...
Grade bands rescaled proportionally from v7 (v7 max = 215; v8 max = 230).
