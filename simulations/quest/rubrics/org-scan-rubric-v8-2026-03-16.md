Scanning the Round 7 scorecard evidence against all C-01 through C-28 definitions to find what's present but not yet formalized:

**V-02 C-25 PASS+** — Evidence: "**Inverting the order is a schema violation.**" C-25 requires column-order as a structural constraint; this adds a *named violation label* for the inverted case, making violations identifiable by output inspection rather than requiring prose interpretation. Not captured.

**V-02 C-27 PASS+** — Evidence: "This schema governs every table in this skill. Status applies to every column." C-27 requires per-column Status annotation; this adds a single *governing statement* at the schema header that asserts universal applicability across all tables, rather than relying on per-table annotations alone. Not captured.

**V-03 C-12 PASS+** — Evidence: "PHASE 1 / PHASE 2 BOUNDARY: GATE EVALUATION:" C-12 requires a hard sequencing gate; this introduces a *named phase boundary header* that makes the gate a structurally identifiable named element, not just an enforced transition. Not captured.

**V-03 C-14 PASS+** — Evidence: "evaluate each item in order; do not skip" C-14 requires a numbered checklist + confirmation sentence; this adds a *sequential evaluation enforcement instruction* making checklist execution order itself a stated constraint. Not captured.

Four new criteria → C-29 through C-32.

---

# Scoring Rubric: org-scan (v8)

## Changes from v7

Four new aspirational criteria added from R7 excellence signals. Aspirational band expands to 24 criteria.

**Scoring note**: With 24 aspirational criteria at 2 pts each the aspirational ceiling is 48 pts. Cap on aspirational contribution remains 10. A variation scoring all 24 still earns 10.

| New Criterion | Source Signal |
|---|---|
| C-29 | Schema violation label: the column-order constraint includes an explicit named violation label (e.g., "Inverting the order is a schema violation"), making constraint violations identifiable by output inspection rather than requiring prose interpretation |
...
| C-31 | Named phase boundary header: the gate is introduced with a named phase boundary label (e.g., "PHASE 1 / PHASE 2 BOUNDARY: GATE EVALUATION:") that makes the boundary a structurally identifiable named element rather than just a hard sequencing transition |
