The complete v9 rubric is written. Here's what changed:

**Two new aspirational criteria** (denominator 12 → 14):

**C-21** — *Inertia gap skeleton pre-seeded in classification table* (from V-02)
The ROLE 1 table must include a pre-filled namespace-level skeleton column (`"Without this signal, {topic}'s story would be missing: ___"` with the blank filled at the namespace level). ROLE 2 completes the blank with topic-specific content, making topic specificity additive rather than originating. Satisfying C-20 (ROLE 2 free-form inertia question) is necessary but not sufficient for C-21.

**C-22** — *Role identity violation mechanism is ontological, not behavioral* (from V-03)
The violation mechanism must be self-contradiction ("You ARE X — doing Y makes you NOT X"), not prohibition ("Doing Y is prohibited"). Prohibition-forward framing earns PARTIAL. The diagnostic test: can the model conceive of itself as still being the CLASSIFIER while violating the instruction? Prohibition says no (rule broken); identity says no (you have ceased to be who you are). C-22 is a higher bar than C-18 on the same dimension — a skill can pass C-18 while failing C-22.

**One annotation update:**

**C-18** — R8 confirmation (V-01) documents the minimum sufficient form: single preamble establishing the identity-violation mechanism + per-phase role-name headers. Per-role "You ARE" repetition is redundant amplification, not the load-bearing element.
d
criterion rather than a further annotation to C-18.

Denominator updated from 12 to 14.
...
### Aspirational (raise the bar — 10% weight; denominator = 14)
...
| Aspirational | 10% | C-09 – C-22 (denominator = 14) |
...
be expressed as self-contradiction, not prohibition. C-18 and C-22 test the same dimension at
different specificity levels; a skill can pass C-18 while failing C-22.
