Rubric v10 written to `simulations/quest/rubrics/discover-competitors-alt-rubric-v10-2026-03-17.md`.

---

**3 new aspirational criteria (C-40 -- C-42):**

| ID | Pattern | Source | Pass Condition Summary |
|----|---------|--------|------------------------|
| C-40 | FOCUS GATE as named PASS/FAIL gate in PREFLIGHT | R10 V-01, V-04, V-05 | FOCUS validation is a named gate subsection within PREFLIGHT (not prose before it). Closes the last distributed-gate surface C-26 alone doesn't force inside PREFLIGHT. Implies C-26 for the FOCUS gate. |
| C-41 | Phase 3 vs-INERTIA-REF row uses slot 6 ternary-token format | R10 V-02, V-04, V-05 | The vs-INERTIA-REF row in the Phase 3 PASS/FAIL table uses `vs. INERTIA-REF -- {reinforces\|challenges\|contextualizes}: {phrase}`. Prose comparison does not satisfy. Independent of C-37. |
| C-42 | EXECUTION DEPENDENCY table includes explicit Prerequisite steps column | R10 V-03, V-05 | The EXECUTION DEPENDENCY table has a dedicated "Prerequisite steps" column making DAG edges queryable by column, not inferable from row sequence. GATE 4 row shows `-- (root)` by declaration. Strictly stronger than C-39. |

**Scoring changes:**
- Aspirational tier: 31 → 34 criteria, max 155 → 170
- Max composite: 245 → 260
- Formula denominator: `aspirational_pass / 34 × 170`
- Grade bands rescaled: Exceptional ≥ 245, Strong ≥ 219, Passing ≥ 90 (golden threshold unchanged)

**Dependency chain:**
- C-40 implies C-26 for the FOCUS gate; C-40 is independent of C-37, C-38, C-39
- C-41 is independent of C-37 (table structure vs. row-value format; neither implies the other)
- C-42 is a strictly stronger form of C-39; C-42 satisfaction implies C-39
opic domain is inferred from repo context (README, CLAUDE.md, package.json, etc.). Output does not ask the user to supply the domain or competitor names. |
...
## Aspirational Criteria (weight = 170 points total, 5 points each)
...
Grade bands rescaled proportionally from v9 (v9 max = 245; v10 max = 260).
