Written to `simulations/quest/rubrics/topic-roadmap-rubric-v12-2026-03-17.md`.
...
|--------|--------|
| Version bump | 11 -> 12 |
| Formula denominator | `/36 * 10` -> `/40 * 10` (20 aspirational × 2 = 40 max) |
| Aspirational range | C-09--C-26 -> C-09--C-28 |
| **C-27 added** | Schema column header and exit criterion field name are lexically identical |
| **C-28 added** | Dual-site blocking enforcement: preamble before table + exit criterion at phase boundary |

**C-27 in brief:** The R11 V-01 failure isolates a specific gap: the table column was renamed but the exit criterion was not updated. C-26 passes when the exit criterion names the field; C-27 passes when the column header and exit criterion field name are the same string. They can drift apart independently, producing a verifiability failure even when both are present.

**C-28 in brief:** R11 V-02 through V-05 achieve C-12 full and C-26 full simultaneously by placing the blocking rule at two orthogonal sites -- a preamble before the defeat assessment table (guards row entry) and the exit criterion (guards phase departure). Each site must be self-contained. One site present without the other is a structural gap.

**Why two, not one:** C-27 and C-28 are orthogonal. A variation can fail C-27 (both sites present, different terms) while passing C-28, or fail C-28 (one site, identical term at that site) while passing C-27. The V-01 pattern fails both; the V-02--V-05 pattern passes both.
ectness
...
## Aspirational Criteria (scored 0 / 1 / 2 per criterion; max = 40)
...
| Change | Detail |
|--------|--------|
| Version bump | version: 11 -> version: 12 |
| Formula denominator | / 36 -> / 40 (20 aspirational criteria x 2 max each) |
| Aspirational range | C-09--C-26 -> C-09--C-28 |
| C-27 added | Schema column header and exit criterion field name are lexically identical -- from R11 V-01 failure: table column renamed to "Consequence if unchanged" but exit criterion not updated, producing C-26=0; C-27 tests that the two changes are made as a matched pair with identical field name strings at both sites |
| C-28 added | Dual-site blocking enforcement (preamble before table + exit criterion at phase boundary) -- from R11 high-scorer pattern: V-02 through V-05 achieve C-12 full and C-26 full simultaneously via a preamble asserting the blocking rule before table evaluation and an exit criterion naming the field at the phase boundary; the two sites are orthogonal and each must be self-contained |

**Why two new criteria rather than one:** Round 11 produced two distinct orthogonal
patterns from the same underlying gap. C-27 captures the naming-consistency
requirement (column header and exit criterion must use lexically identical terms --
a single-character difference produces a verifiability failure). C-28 captures the
dual-site placement requirement (blocking rule must be stated independently at
preamble and exit criterion -- one site present and the other absent is a structural
gap). A variation can fail C-27 while passing C-28 (both sites present, different
terms used), or fail C-28 while passing C-27 (identical terms at one site, other
site absent). V-01 R11 fails both: column renamed, exit criterion not updated,
no preamble site either. V-02 through V-05 R11 pass both.

**Orthogonality matrix:**

- C-27 fail, C-28 fail: column present but exit criterion absent or uses different
  term; no preamble site (V-01 R11 pattern).
- C-27 pass, C-28 partial: column and exit criterion use identical term, but only
  one blocking-rule site is present.
- C-27 partial, C-28 pass: both sites present and self-contained, but column
  header and exit criterion use different terms for the same field.
- C-27 pass, C-28 pass: identical field name at column header and exit criterion;
  preamble blocking rule + exit criterion rule both present and self-contained
  (V-02 through V-05 R11 pattern).
