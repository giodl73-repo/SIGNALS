**What changed v17 -> v18:** Two new criteria extracted from R18 above-ceiling signals:
...
**Tier:** Aspirational 44 / 220 pts -> 46 / 230 pts. Total ceiling **310 -> 320**.

---

**Signal logic:**

- **C-53** (tightens C-20, C-24): C-20 requires a summary table pre-committing all controlled-vocabulary values before prose card bodies; C-24 requires a collective-distribution audit checking phase, category, volume, and phase-character after the summary table. Neither criterion constrains the role dimension: a model satisfying C-24 can distribute tickets unevenly across roles -- all eight tickets assigned to the Support persona, zero to SRE -- without any structural gate making that distribution implausible before body generation. V-01 added a ROLE ALLOCATION TABLE at Step 3A declaring the intended per-role ticket count before role activation (e.g., "Support / 2, SRE / 2, PM / 1, ..."). Step 5B gains Constraint 1B, which fires before body generation and verifies that the Persona column in the summary table matches the declared allocation per role, producing a MATCH/MISMATCH per row. Pass 2 Part B item 4 re-checks the post-generation count against the declared allocation. The three-gate architecture -- pre-declaration (Step 3A), pre-body audit (Constraint 1B), post-generation re-check (Part B item 4) -- is structurally parallel to the phase-distribution architecture introduced by C-45/C-48. C-20 and C-24 pass with any per-row correct vocabulary and distribution audit; C-53 passes only when Step 3A carries a ROLE ALLOCATION TABLE pre-declaring per-role counts, Constraint 1B in Step 5B verifies Persona column compliance before body generation, and Pass 2 Part B item 4 re-checks post-generation.

- **C-54** (tightens C-17): C-17 requires the coverage table to have at least three columns (Ticket ID / STATUS QUO element / Gap traced to) and a no-orphan-gap check confirming every named gap maps to at least one table row. The failure mode C-17 leaves open: the table confirms ticket-to-STATUS-QUO traces and gap coverage, but the Phase column is absent. A scorer can confirm that every gap has a ticket row and every ticket has a STATUS QUO element, but cannot identify phase concentration in the coverage table without cross-referencing Phase columns in the card bodies. V-02 added Phase as the 2nd column, producing a 4-column table: Ticket ID / Phase / STATUS QUO element referenced / Gap traced to. A third post-table confirmation explicitly counts all three phases in the Phase column and confirms three-phase representation -- "All three phases represented in Phase column: Phase 1: [n] rows, Phase 2: [n] rows, Phase 3: [n] rows. Three-phase coverage: YES." Phase concentration (all tickets in Phase 1) becomes a cell-value pattern readable in a single column scan without row-by-row body reading. C-17 passes with a 3-column gap-bridged table; C-54 passes only when the table gains a Phase column as column 2 (4-column format: Ticket ID / Phase / STATUS QUO element / Gap traced to) and the output includes an explicit post-table confirmation counting all three phases in the Phase column.
...
## Aspirational Criteria (230 pts total -- raises the bar above 90)
...
### C-53 -- Role Allocation Pre-Commitment Table
- **Weight**: aspirational
- **Points**: 5
- **Category**: depth
- **Text**: Before tickets are generated, the output produces a ROLE ALLOCATION TABLE at Step 3A that pre-declares the intended per-role ticket count for each stock role (e.g., "Support / 2, SRE / 2, PM / 1, UX / 1, C-07 / 1, C-11 / 1"). Step 5B gains Constraint 1B, which fires before body generation and verifies that the Persona column counts in the summary table match the declared allocation per role, producing a MATCH/MISMATCH verdict per role. Pass 2 Part B item 4 re-checks the post-generation per-role count against the declared allocation. C-20 pre-commits controlled-vocabulary values (including persona) in a summary table; C-24 audits collective distribution across phase, category, and volume. Neither criterion pre-commits the role dimension structurally. The failure mode C-20 and C-24 leave open: the summary table may carry a valid persona value per row while the aggregate role distribution is skewed -- all eight tickets assigned to Support, zero to SRE -- without any structural gate detecting role saturation before body generation. The ROLE ALLOCATION TABLE closes this gap: a pre-committed per-role count makes saturation detectable at Constraint 1B before a single card body is written. The three-gate architecture mirrors the phase-distribution pattern: pre-declaration (Step 3A), pre-body audit (Constraint 1B), post-generation re-check (Part B item 4). C-20 and C-24 pass when per-row vocabulary and collective distribution are audited; C-53 passes only when Step 3A carries a ROLE ALLOCATION TABLE pre-declaring per-role counts, Constraint 1B in Step 5B verifies Persona column compliance before body generation, and Pass 2 Part B item 4 re-checks post-generation per-role counts.
- **Pass condition**: (1) A ROLE ALLOCATION TABLE appears at Step 3A (or equivalent pre-generation step) with at least two columns (Role / Tickets allocated) and one row per participating role; AND (2) Step 5B (or equivalent pre-body-generation step) includes a Constraint 1B (or equivalent) that verifies the Persona column counts in the summary table match the declared allocation per role, with a MATCH/MISMATCH verdict per role; AND (3) Pass 2 Part B (or equivalent post-generation verification) includes an item that re-checks per-role counts against the Step 3A declaration. A role distribution note embedded in prose does not pass. A post-generation role check without a pre-declaration does not pass. An output without C-20 cannot satisfy C-53.
- **Round 18 evidence**: V-01 (Step 3A adds ROLE ALLOCATION TABLE with Role / Tickets allocated columns, 6 rows; Constraint 1B in Step 5B fires before body generation with per-role MATCH/MISMATCH; Part B item 4 re-checks post-generation per-role state) satisfies C-53. V-04 (V-01+V-02 combined; V-01 axis inherited; all three gates present) satisfies C-53. V-05 (full synthesis; final verification explicitly states "Role allocation compliance (Step 3A vs Step 5): all roles MATCH. C-53 probe (role allocation pre-commitment table): complete.") satisfies C-53. V-02 (Phase-column axis only; no Step 3A, no Constraint 1B, no Part B item 4) and V-03 (Phase-2-template axis only; no role allocation mechanism) fail C-53. The three-gate role architecture closes the role-saturation failure mode that per-row persona vocabulary and collective distribution audits leave open.

---

### C-54 -- Phase Column in Coverage Table
- **Weight**: aspirational
- **Points**: 5
- **Category**: depth
- **Text**: The Pass 1 coverage table required by C-17 gains Phase as its 2nd column, producing a 4-column format: Ticket ID / Phase / STATUS QUO element referenced / Gap traced to. A third post-table confirmation explicitly counts all three phases in the Phase column and confirms three-phase representation. C-17 requires a 3-column gap-bridged coverage table (Ticket ID / STATUS QUO element / Gap traced to) and a no-orphan-gap check confirming every gap maps to at least one row. The failure mode C-17 leaves open: the table confirms ticket-to-STATUS-QUO traces and gap coverage, but the Phase column is absent -- a scorer cannot identify phase concentration in the coverage pass without cross-referencing card body phase labels row by row. A skewed phase distribution (six Phase 1 tickets, zero Phase 3) is not detectable from the 3-column table alone. Adding Phase as the 2nd column makes phase concentration a cell-value pattern readable in a single column scan: a scorer reads down column 2 and any phase with zero rows is structurally visible without parsing card prose. The third post-table confirmation ("All three phases represented in Phase column -- Phase 1: [n] rows, Phase 2: [n] rows, Phase 3: [n] rows. Three-phase coverage: YES") converts phase coverage from an implied property of the ticket set into an explicitly audited structural property of the coverage table. C-17 passes with a 3-column gap-bridged table; C-54 passes only when the table gains Phase as the 2nd column (4-column: Ticket ID / Phase / STATUS QUO element / Gap traced to) and the output includes an explicit post-table confirmation enumerating per-phase row counts and confirming three-phase coverage.
- **Pass condition**: The Pass 1 coverage table has at least four columns in the order Ticket ID / Phase / STATUS QUO element referenced / Gap traced to (or equivalent), AND the output includes an explicit post-table confirmation that counts the rows for each of the three phases in the Phase column and states whether all three phases are represented ("Three-phase coverage: YES/NO" or equivalent). A 3-column table with a separate prose note about phase coverage does not pass. A post-table confirmation that confirms phase coverage without enumerating per-phase row counts does not pass. An output without C-17 cannot satisfy C-54.
- **Round 18 evidence**: V-02 (Phase column inserted as column 2 -- table header: `| Ticket ID | Phase | STATUS QUO element referenced | Gap traced to |`; Confirmation 3: "All three phases represented in Phase column -- Phase 1: [n] rows, Phase 2: [n] rows, Phase 3: [n] rows. Three-phase coverage: YES."; final summary: "Phase column in coverage table: all three phases represented -- confirmed. C-54 probe (Phase column in coverage table): complete.") satisfies C-54. V-04 (V-01+V-02 combined; V-02 Phase-column axis inherited; 4-column table present; Confirmation 3 present) satisfies C-54. V-05 (full synthesis; Phase column in coverage table inherited from V-02 axis) satisfies C-54. V-01 (role-allocation axis only; Pass 1 coverage table retains 3-column V-05 R17 structure; no Phase column) and V-03 (Phase-2-template axis only; no Phase column in coverage table) fail C-54. The Phase column closes the phase-concentration blind spot that the 3-column gap-bridged table leaves open; the post-table confirmation converts phase coverage into an explicitly audited structural property.
...
| C-53 | Role allocation pre-commitment table           | aspirational | 5      | depth       |
| C-54 | Phase column in coverage table                 | aspirational | 5      | depth       |

**Essential ceiling**: 60 | **Recommended ceiling**: 90 | **Aspirational ceiling**: 320
...
| v18 | 2026-03-15 | Added C-53 (role allocation pre-commitment table) and C-54 (phase column in coverage table) from Round 18 excellence signals; aspirational tier grows from 220 to 230 pts, total ceiling 320 |
