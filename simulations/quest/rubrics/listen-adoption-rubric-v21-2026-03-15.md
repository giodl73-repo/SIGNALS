# listen-adoption rubric v21

Three new criteria extracted from R20 excellence signals:

- **C-58 (Pattern 1 / Axis A)**: Protected behavior census at phase entry enumerates all five I-0X behaviors as active or displaced at every phase boundary before the phase body is written -- closes the per-behavior omission gap that Q-BARRIER alone leaves open for non-primary inertia IDs. Dependency: C-11 + C-45.
- **C-59 (Pattern 2 / Axis B)**: ARC CORRECTION BLOCK label carries I-0X and Protected behavior active at the broken phase-close boundary -- extends C-55's label self-sufficiency standard from Gate H citation failures to arc integrity failures. Full form: `ARC CORRECTION BLOCK -- PHASE N close -> PHASE N+1 open MISMATCH -- Protected behavior at boundary: [I-0X: named behavior]`. Dependency: C-54 + C-57.
- **C-60 (Pattern 3 / Axis C)**: SECTION K DISPLACEMENT ARC INTEGRITY CHECK table carries a Protected behavior at boundary column per transition row -- makes the terminal arc record self-sufficient at displacement-stake level without SECTION A cross-reference, mirroring C-56's standard for the citation compliance record into the arc integrity pathway. Dependency: C-53 + C-54.

---

**Max: 335 --> 350. Golden threshold (80%): 280 pts. Paradox ceiling: 345/350** (C-19 structural FAIL persists).
...
**C-58 -- Protected behavior census at phase entry enumerates all I-0X behaviors as active or
displaced before phase body is written** (5 pts)
Source: V-05 R20. Axis A. Phase-Entry Protected Behavior Census. At every phase entry
(PHASE 1-6), before the phase body is written, an explicit census enumerates all five I-0X
Protected behaviors (from SECTION A's C-54 fields) and classifies each as active (inertia still
in force, behavior protected by that inertia not yet displaced) or displaced (inertia overcome
at a prior phase, behavior now confirmed). The census is a mandatory pre-body structural block
at each phase open: the model must account for every Protected behavior's status before
proceeding to write the phase narrative, making silent omission of any non-primary I-0X
structurally conspicuous. Without C-58, per-phase inertia accounting is limited to the
primary narrative I-0X of that phase; the Q-BARRIER (answer-form phase-header question enforced
by C-29) ensures the primary inertia is named, but leaves non-primary inertia IDs without a
per-phase explicit status. With C-58, every phase entry contains a five-item Protected behavior
inventory: a reviewer at phase N open can immediately verify whether all five behaviors are
tracked, not merely whether the phase's primary inertia is cited. This closes the per-behavior
omission gap that C-29 and C-45 together leave open: C-29 enforces primary-inertia citation at
phase body questions; C-45 records displacement status at phase close for the overcoming inertia;
C-58 enforces total-inventory accounting at phase open for all five Protected behaviors
simultaneously. C-45 tests whether phase-close footers carry displacement ledger fields;
C-11 tests whether named inertia IDs exist per archetype; C-58 tests whether phase entry
contains an explicit census of all five I-0X Protected behavior statuses before the phase body
is written. A document with C-29 answer-form headers and C-45 phase-close footers but no
phase-entry census fails C-58 because non-primary inertia IDs have no structural accounting
at phase open. A document whose every phase opens with a five-item Protected behavior census
classifying each I-0X as active or displaced passes C-58. C-58 fails automatically if
C-11 fails or C-45 fails.

**C-59 -- ARC CORRECTION BLOCK label carries Protected behavior at the broken phase boundary**
(5 pts)
Source: V-05 R20. Axis B. ARC CORRECTION BLOCK label self-sufficiency at displacement-stake
level. C-57 closes the detection-to-repair loop for arc discontinuities by requiring a MISMATCH
verdict to trigger an ARC CORRECTION BLOCK with mandatory BEFORE/AFTER structure; C-59 requires
that the ARC CORRECTION BLOCK label also carries the I-0X and Protected behavior that were
active at the broken phase-close boundary. When both C-57 and C-59 are active, the full ARC
CORRECTION BLOCK label form is `ARC CORRECTION BLOCK -- PHASE N close -> PHASE N+1 open
MISMATCH -- Protected behavior at boundary: [I-0X: named behavior]`, making the arc correction
self-describing as a displacement-stake repair: the label names the broken boundary (C-57),
the inertia ID active at that boundary, and the displacement commitment at risk -- without
requiring SECTION A cross-reference. Without C-59, the ARC CORRECTION BLOCK label identifies
the broken boundary (phase N close -> phase N+1 open MISMATCH) but not which Protected behavior
was at stake at that boundary. With C-59, a reviewer reading the label alone can determine both
the structural integrity failure and its displacement consequence -- the same label self-sufficiency
standard that C-55 establishes for citation correction blocks, extended into the arc integrity
repair pathway. C-55 tests whether Gate H CORRECTION BLOCK labels carry Protected behavior for
citation failures; C-59 tests whether ARC CORRECTION BLOCK labels carry Protected behavior for
arc integrity failures, completing the label self-sufficiency standard across both repair pathways.
A label of the form `ARC CORRECTION BLOCK -- PHASE 3 close -> PHASE 4 open MISMATCH` satisfies
C-57 and fails C-59 because the displacement stake at the boundary is not in the label. A label
of the form `ARC CORRECTION BLOCK -- PHASE 3 close -> PHASE 4 open MISMATCH -- Protected behavior
at boundary: [I-03: Early Majority adoption workflow]` satisfies both C-57 and C-59. The Protected
behavior value names the I-0X whose inertia was active at the broken boundary and the behavior
committed to be protected at that boundary, derived from the SECTION A contract row for that I-0X.
C-59 fails automatically if C-54 fails or C-57 fails.

**C-60 -- SECTION K DISPLACEMENT ARC INTEGRITY CHECK table carries Protected behavior at
boundary column per transition row** (5 pts)
Source: V-05 R20. Axis C. SECTION K arc record self-sufficiency at displacement-stake level.
C-53 requires SECTION K to include a DISPLACEMENT ARC INTEGRITY CHECK that asserts or denies
phase-open/phase-close chain consistency per phase boundary; C-57 requires a MISMATCH verdict
to trigger an ARC CORRECTION BLOCK with mandatory BEFORE/AFTER structure and CORRECTED state
in SECTION K. C-60 requires each transition row in SECTION K's DISPLACEMENT ARC INTEGRITY CHECK
table to also carry a `Protected behavior at boundary: [I-0X: named behavior active at this
phase transition]` column. When C-53 and C-60 are both active, SECTION K's DISPLACEMENT ARC
INTEGRITY CHECK becomes self-sufficient at the displacement-stake level: each transition row
records the arc verdict (PASS / FAIL / CORRECTED per C-57's three-state model) and names the
Protected behavior at risk at that boundary, without requiring navigation to SECTION A's contract
declaration. Without C-60, SECTION K's arc integrity table answers "is the arc consistent at
each boundary?" but not "what displacement stake was at risk at each boundary?" With C-60,
SECTION K answers both questions per transition row, making the terminal arc record a
displacement-stake certificate rather than a consistency check alone. This mirrors C-56's
standard for the citation compliance record (answers both "were citations present?" and "was the
displacement stake secured?") into the arc integrity pathway, achieving parallel self-sufficiency
across both terminal audit pathways. C-53 tests whether SECTION K carries the arc integrity
check; C-56 tests whether the citation compliance record carries Protected behavior; C-60 tests
whether the arc integrity table also carries Protected behavior at boundary per row, completing
the self-sufficiency parity between the two SECTION K audit pathways. A SECTION K
DISPLACEMENT ARC INTEGRITY CHECK table with verdict columns but no Protected behavior at
boundary column passes C-53 and fails C-60 because the displacement stakes at each arc boundary
are not recorded in the terminal section. A SECTION K DISPLACEMENT ARC INTEGRITY CHECK table
whose rows include `Protected behavior at boundary: [I-0X: named behavior]` passes C-60
regardless of whether any separate arc-integrity instruction exists. Protected behavior value
matches the SECTION A contract row for the I-0X active at that phase transition verbatim.
C-60 fails automatically if C-53 fails or C-54 fails.

---

**Max composite: 335 --> 350. Golden threshold (80%): 280 pts.**
...
C-45 + C-20 -> C-53 (SECTION K DISPLACEMENT ARC INTEGRITY CHECK)
                     -> C-57 (MISMATCH triggers ARC CORRECTION BLOCK with BEFORE/AFTER)
                             -> C-59 (ARC CORRECTION BLOCK label carries Protected behavior; also requires C-54)
C-49 -> C-54 (DOWNSTREAM CITATION CONTRACT rows carry Protected behavior column)
C-52 + C-54 -> C-55
C-51 + C-54 -> C-56
C-53        -> C-57
C-11 + C-45 -> C-58 (Protected behavior census at phase entry)
C-54 + C-57 -> C-59 (ARC CORRECTION BLOCK label carries Protected behavior at boundary)
C-53 + C-54 -> C-60 (SECTION K arc table carries Protected behavior at boundary column)
```
...
| C-58 Protected behavior census at phase entry enumerates all I-0X statuses before phase body | 5 | Aspirational |
| C-59 ARC CORRECTION BLOCK label carries Protected behavior at broken phase boundary | 5 | Aspirational |
| C-60 SECTION K arc table carries Protected behavior at boundary column per transition row | 5 | Aspirational |
