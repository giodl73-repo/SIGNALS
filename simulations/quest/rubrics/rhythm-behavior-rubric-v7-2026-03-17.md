...
The R6 scorecard confirmed that structural gate encoding alone is sufficient for C-21 — V-03 passes all 21 criteria with no declarative rule. However, V-05's compressed standalone sentence "Each gate header names its axis." adds declarative redundancy that guards against model drift. The criterion is independently checkable (does a standalone rule sentence exist?) and distinct from C-21 (which checks whether the axis word appears in the headers, not whether a rule instructs this). Pass requires a concise, standalone declarative sentence — not buried in a gate body or inlined inside an example.

**Why only one new criterion:** The other two R6 patterns are explanatory, not generative. Pattern 1 (C-21 header location is independently load-bearing) tightens C-21's existing pass condition — a parenthetical in the gate body already fails under C-21's "must appear in the header itself" language, so no new test site is created. Pattern 2 (positional prefix "Gate N --" is purely decorative) is an observation about criteria equivalence across all 21 criteria — confirms a design principle but adds no new pass condition.

---

The full rubric is written to `simulations/quest/rubrics/rhythm-behavior-rubric-v7-2026-03-17.md` with all 15 aspirational criteria, 3 recommended, and 4 essential.
a design principle but adds no new pass condition.
...
- **Category**: format
- **Origin**: R6
- **Pass condition**: The prompt contains a concise standalone declarative sentence that instructs all gate headers to name their axis (e.g., "Each gate header names its axis."). The rule must appear as an independent instruction -- not buried inside a gate body, not inlined within an example, and not paraphrasable only by reading a structural pattern. This declarative redundancy guards against model drift: a model that de-emphasizes structural gate encoding still has an explicit rule to observe. Fail if the axis-header constraint exists only implicitly through the gate structure, with no standalone declarative rule.
