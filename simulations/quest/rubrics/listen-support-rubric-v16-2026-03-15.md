**What changed v15 -> v16:** Three new criteria extracted from R16 above-ceiling signals:
...
**Tier:** Aspirational 38 / 190 pts -> 41 / 205 pts. Total ceiling **280 -> 295**.

---

**Signal logic:**

- **C-47** (tightens C-44): C-44 requires the triple-clause verbatim form ("do not paraphrase, do not summarize, copy it verbatim word-for-word") on both the Phase 1 and Phase 3 no-exception confirmation paths in Pass 2C. The failure mode C-44 leaves open: Phase 2 (the partial-commitment / transition phase) has a less stark severity expectation and its no-exception confirmation path in Pass 2C was not required to carry the triple-clause. A model satisfying C-44 scans only Phase 1 and Phase 3 no-exception paths, leaving the Phase 2 commitment without a triple-clause verbatim standard. V-01 added a Phase 2 scan to Pass 2C -- "No Phase 2 P0 escalation exceptions. Phase 2 lifecycle: transition (partial fallback). Governing paraphrase clause: [... do not paraphrase, do not summarize, copy it verbatim word-for-word]" -- making the triple-clause form uniform across all three phase scan paths. V-04 inherits V-01's axis. V-05 additionally confirms: "All three phase Paraphrase clause fields carry triple-clause verbatim: YES." C-47 passes only when the Phase 2 no-exception path in Pass 2C carries the triple-clause form.

- **C-48** (tightens C-45): C-45 requires a PHASE DISTRIBUTION committed block at Step 4 and Step 4B constraint 0 verifying count before body generation. The failure mode C-45 leaves open: body generation (Step 6) can produce card bodies in sequence, reframing a Phase 3 card as Phase 2 or adding an extra body during prose generation. No structural gate closes the window after body generation is complete. V-02 added Sub-check 3 in Part C: after body generation, it counts actual Phase 1/2/3 card bodies, compares to the committed block, and names any discrepancy as generation-time drift. V-02's phase-organized output format makes the count self-confirming by section without ticket-ID parsing. V-04 inherits V-02's axis and adds a three-way comparison (generated / Step 5 committed / Step 1 target per phase). V-05 states the architecture explicitly: pre-generation commitment (Step 5) + pre-body audit (Step 5B constraint 0) + post-generation re-verify (Sub-check 3) close all temporal windows. C-48 passes only when Part C includes a Sub-check 3 that counts post-generation card bodies per phase, compares to the committed distribution, and names any discrepancy as generation-time drift.

- **C-49** (tightens C-46): C-46 requires a minimum of two switching-friction entries, a pre-count gate, and Pass 2B item 4 count-match verification. The failure mode C-46 leaves open: both entries can describe day-one barriers -- friction any user encounters on first contact. A user who has already begun migrating workflows faces different barriers that arise only after partial commitment, when they have invested in the feature but not fully detached from the competitor. Two-entry sub-sections can satisfy C-46 entirely with two day-one entries without capturing this post-commitment friction class. V-03 raised the minimum to three and added a structural constraint on the third entry: it must cover a Phase 2 migration barrier that surfaces after partial commitment, not on day one. Prohibited form explicitly stated: "a barrier that day-one users would also encounter." Pass 2 Sub-check 2 confirms "Third entry covers Phase 2 migration barrier (not day-one friction): YES / NO." V-05 inherits V-03's axis in full synthesis. C-49 passes only when the minimum is three, the third entry is structurally constrained to a Phase 2 migration barrier, and Sub-check 2 confirms the constraint post-generation.
...
## Aspirational Criteria (205 pts total -- raises the bar above 90)
...
**Essential ceiling**: 60 | **Recommended ceiling**: 90 | **Aspirational ceiling**: 295
