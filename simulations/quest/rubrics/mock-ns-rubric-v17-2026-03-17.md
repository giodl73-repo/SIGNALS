...
**Two new aspirational criteria (C-44, C-45) extracted from R16 excellence signals. Max rises from 150 to 154.**

### C-44 -- CONSTRAINT per-step-ID operation-type co-location (R16 V-01)
> R16 V-01 canonical failure: "Do not perform S-1, S-2, S-3.1, S-3.2, or S-3.3 until this step's emit is written." -- step IDs present, no per-ID operation-type annotations.

C-15/C-21/C-24 test operation-type count (3+/4+/5+ types present anywhere in the CONSTRAINT block). C-44 tests structural co-location: each step ID named in the CONSTRAINT must be paired inline with its operation-type annotation as an adjacent parenthetical or equivalent co-located form. The golden CONSTRAINT form is "Do not perform S-1 (skill selection), S-2 (category lookup), S-3.1 (carry), S-3.2 (compliance detection), or S-3.3 (flag computation) until this step's emit is written." -- five step IDs, each with a parenthetical annotation. A CONSTRAINT that names operation types in a trailing group or separate clause rather than paired per-step-ID fails C-44 even when the type count satisfies C-15/C-21/C-24. Step-IDs-only CONSTRAINT (V-01 form, 0 per-ID annotations) fails both C-15/C-21/C-24 (count = 0) and C-44 (no per-ID pairing).

**Failure chain: C-15, C-21, C-24, C-44** -- all four fire for step-IDs-only CONSTRAINT; C-44 also fires independently when types are named as a group but not per-ID.

### C-45 -- CONSTRAINT S-3 sub-step granularity (R16 golden)
> Golden form names S-3.1 (carry), S-3.2 (compliance detection), S-3.3 (flag computation) as individually annotated entries -- not collapsed into a single "S-3" reference.

When the CONSTRAINT's prohibited step set includes S-3 sub-operations, each sub-step (S-3.1, S-3.2, S-3.3) must appear as a separately enumerated entry rather than collapsed into a parent "S-3" entry or similar grouping. C-44 tests per-ID annotation co-location; C-45 tests sub-step enumeration depth independently. A CONSTRAINT that names all sub-steps by ID but without per-sub-step annotations fails C-44 (co-location absent) but passes C-45 (enumeration present). A CONSTRAINT that collapses sub-steps into "S-3" fails C-45 regardless of whether annotations are present. R16 V-01 canonical: names S-3.1, S-3.2, S-3.3 without annotations -- C-44 FAIL, C-45 PASS. Golden canonical: names S-3.1 (carry), S-3.2 (compliance detection), S-3.3 (flag computation) -- C-44 PASS, C-45 PASS.

**Failure chain for S-3 collapse: C-45 alone** (-2 pts). **Combined failure when sub-steps absent AND unannotated: C-44, C-45** (-4 pts).

### Discriminator notes updated

| Criterion | Update |
|-----------|--------|
| C-15 | R16: Step-IDs-only CONSTRAINT confirmed canonical failure. "Do not perform S-1, S-2, S-3.1, S-3.2, or S-3.3" names zero operation types. R16 V-01 canonical. C-44 additionally fires for any form lacking per-ID annotation co-location. |
| C-21 | R16: Step-IDs-only CONSTRAINT names zero operation types; 0 < 4. Co-fires with C-15, C-24, C-44 in R16 V-01. |
| C-24 | R16: Step-IDs-only CONSTRAINT names zero operation types; 0 < 5. Co-fires with C-15, C-21, C-44 in R16 V-01. |
...
**R16 under v17:**
- V-01 (step-IDs-only CONSTRAINT): fails C-15, C-21, C-24, C-44 (-8 pts) -- **142/154**
- V-02 (golden: per-ID-annotated CONSTRAINT + lifecycle supplementation): all 45 criteria pass -- **154/154**
...
*Raise the bar once essential and recommended are stable. Each criterion is worth 2 pts (37 x 2 = 74; max score = 154).*
...
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: Each step ID named in the CONSTRAINT block is paired inline with its operation-type annotation as an adjacent parenthetical or equivalent co-located form (e.g., "S-1 (skill selection)", "S-2 (category lookup)"). The pairing must be per-step-ID -- each entry in the CONSTRAINT's step list must carry its own annotation. A CONSTRAINT that names operation types in a trailing group (e.g., "S-1, S-2, S-3 -- which involve skill selection, category lookup, and carry"), in a separate clause, or in a footnote fails C-44 even when the type count satisfies C-15/C-21/C-24. A step-IDs-only CONSTRAINT (zero per-ID annotations) fails both the count-based criteria and C-44. **Relationship to C-15/C-21/C-24:** Those criteria test total operation-type count; C-44 tests per-ID co-location. The two tests are orthogonal: a CONSTRAINT with 5 types named as a group passes C-15/C-21/C-24 but fails C-44; a CONSTRAINT with 5 types individually annotated passes all four. **Canonical R16 V-01 failure:** "Do not perform S-1, S-2, S-3.1, S-3.2, or S-3.3 until this step's emit is written." -- step IDs present, zero per-ID annotations -- C-15/C-21/C-24/C-44 all fail. **Canonical golden pass:** "Do not perform S-1 (skill selection), S-2 (category lookup), S-3.1 (carry), S-3.2 (compliance detection), or S-3.3 (flag computation) until this step's emit is written." -- five step IDs, each with parenthetical annotation -- C-15/C-21/C-24/C-44 all pass. Failure chain for step-IDs-only: C-15, C-21, C-24, C-44 (-8 pts). Source: R16 V-01; golden CONSTRAINT canonical form confirmed.

### C-45
- **Text**: CONSTRAINT enumerates S-3 sub-steps (S-3.1, S-3.2, S-3.3) as separately listed entries.
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: When the CONSTRAINT's prohibited step set includes S-3 sub-operations, each sub-step (S-3.1, S-3.2, S-3.3) must appear as a separately enumerated entry in the CONSTRAINT list, not collapsed into a parent "S-3" reference or similar grouping. C-44 tests per-ID annotation co-location; C-45 tests sub-step enumeration depth independently. A CONSTRAINT that lists all three sub-steps by ID but without per-sub-step annotations passes C-45 (sub-step granularity present) but fails C-44 (annotation absent). A CONSTRAINT that collapses sub-steps into "S-3" fails C-45 regardless of whether annotations are present. A CONSTRAINT that names "S-3.1, S-3.2, S-3.3" without annotations (V-01 form) passes C-45 and fails C-44. The golden CONSTRAINT names S-3.1 (carry), S-3.2 (compliance detection), S-3.3 (flag computation) as three separately annotated sub-step entries -- passes both C-44 and C-45. **Canonical failure form for C-45:** "Do not perform S-1, S-2, S-3 until this step's emit is written." -- S-3 as collapsed parent reference -> C-45 FAIL. **Canonical R16 V-01 boundary:** "Do not perform S-1, S-2, S-3.1, S-3.2, or S-3.3" -- sub-steps enumerated, no annotations -> C-45 PASS, C-44 FAIL. **Distinction from C-44:** C-45 tests whether the sub-step granularity is preserved at all; C-44 tests whether each named step (at whatever granularity) is annotated. Both can fail independently. Failure chain for S-3 collapse: C-45 alone (-2 pts). Source: R16 golden CONSTRAINT; sub-step granularity boundary confirmed.
