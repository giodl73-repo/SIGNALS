...
**Two new aspirational criteria extracted from R18 excellence signals:**

| ID | Criterion | Source Axis | Independence from Parent |
|----|-----------|-------------|--------------------------|
| C-46 | **Amendment table T-42 trigger entry includes correct-format prescription alongside the inline failure exemplar** | R18 Signal 1: "Specification phrasing register fidelity: LLM output abstraction level mirrors spec abstraction level — explicit labeled directives produce explicit outputs; category-only language produces category-only outputs, with no inference upgrade" — V-03 confirmed that where T-40 used explicit Part 1/Part 2 labeled structure the output faithfully reproduced all three parts, while T-42's category-only spec produced a category-only output that failed C-45. C-46 applies the same two-part completeness standard to T-42's CONDITION field that C-43 applied to T-40 and C-41 applied to T-38 | C-45 requires the T-42 CONDITION field to carry an inline exemplar showing what a deficient T-40 CONDITION entry looks like (Part 1). C-46 requires the T-42 CONDITION field to also carry an explicit correct-format prescription (Part 2) stating what a passing T-40 CONDITION entry looks like — the minimum form that satisfies C-42: a quoted or paraphrased exemplar of an abstract-only T-38 CONDITION form. The two-part T-42 CONDITION is fully self-contained: a reviewer can determine both what fires T-42 and what would satisfy C-42 without consulting the rubric or T-40's CONDITION cell. C-46 is independently testable: a reviewer reads the T-42 CONDITION cell and confirms whether both Part 1 and Part 2 are present. C-45 FAIL → C-46 FAIL (cascade: no exemplar means no prescription is possible). C-45 PASS does not guarantee C-46 PASS — the exemplar can be present without the correct-format prescription, which is the C-46-independent-fail state. |
| C-47 | **T-42 CONDITION exemplar includes explanation of why the quoted deficient T-40 form is abstract** | R18 Signal 2: "Triple-layer T-40 exemplar structure (quote + explain abstractness + prescribe correct form) is the minimum spec content for cascade immunity across C-42/C-43/C-44 simultaneously" — V-03 demonstrated that C-42 PASS + C-44 PASS + C-43 PASS are achievable together only when all three directives appear explicitly in the spec, establishing that the same triple-layer content standard applies at the T-42 tier | C-45 requires the T-42 CONDITION field to carry an inline exemplar of a deficient T-40 CONDITION form (Part 1). C-47 adds a content requirement on that same exemplar: it must additionally state WHY the quoted T-40 form is abstract — naming that it states only a condition category without showing what a deficient T-38 CONDITION entry looks like. An exemplar quoting "fires if T-40 CONDITION lacks an inline failure exemplar" passes C-45 (deficient form quoted) but fails C-47 unless the quoted form is accompanied by an explanation of its abstractness (e.g., "— abstract: names the condition without quoting what a deficient T-38 CONDITION looks like"). C-47 is independently testable: a reviewer reads the T-42 CONDITION cell's Part 1 exemplar construct and confirms whether an explanation-of-abstractness phrase is present alongside the quoted form. C-45 FAIL → C-47 FAIL (cascade: no exemplar means no explanation-of-abstractness content). C-45 PASS does not guarantee C-47 PASS — the exemplar can be present without the explanation, which is the C-47-independent-fail state. C-46 and C-47 are mutually independent: a T-42 CONDITION can carry an exemplar with explanation-of-abstractness but no correct-format prescription (C-47 PASS, C-46 FAIL) or an exemplar with correct-format prescription but no explanation of abstractness (C-47 FAIL, C-46 PASS). |

**Denominator:** `/38` → `/40`. Each aspirational criterion = `10/40` = `0.25` points.

**Score projections under v19 for R18 golden V-03** (denominator /40; V-03 was 37/38 = 99.74 under v18):

| Scenario | Fails | Composite |
|----------|-------|-----------|
| V-03 under v19: C-45 FAIL → C-46 FAIL → C-47 FAIL (cascade) | 3 (C-45, C-46, C-47) | **99.25** |
| V-03 with T-42 spec made explicit (all three parts): C-45 PASS, C-46 PASS, C-47 PASS | 0 | **100.00** |

Note: Under v19, V-03's single C-45 FAIL cascades to C-46 FAIL and C-47 FAIL because C-45 is the Part 1 prerequisite for both. The cascade reduces V-03 from 99.74 to 99.25. A rewritten T-42 spec with explicit Part 1 (exemplar + explanation-of-abstractness) and Part 2 (correct-format prescription) would achieve 40/40 = 100.00.

**Cascade invariants (updated for v19):**

- **C-46 cascade from C-45**: A T-42 CONDITION that carries no inline exemplar of a deficient T-40 form (C-45 FAIL) also fails C-46 (no exemplar → no correct-format prescription content available). C-45 FAIL → C-46 FAIL is deterministic.
- **C-46 independent fail**: A T-42 CONDITION carrying a quoted deficient T-40 form (C-45 PASS) but no correct-format prescription stating what a passing T-40 CONDITION looks like passes C-45 and fails C-46 independently. C-45 PASS does not guarantee C-46 PASS.
- **C-47 cascade from C-45**: A T-42 CONDITION that carries no inline exemplar (C-45 FAIL) also fails C-47 (no exemplar → no explanation-of-abstractness content). C-45 FAIL → C-47 FAIL is deterministic.
- **C-47 independent fail**: A T-42 CONDITION carrying a quoted deficient T-40 form (C-45 PASS) but no explanation of why that form is abstract passes C-45 and fails C-47 independently. C-45 PASS does not guarantee C-47 PASS.
- **C-46 and C-47 mutually independent**: Exemplar with explanation but no prescription (C-47 PASS, C-46 FAIL); exemplar with prescription but no explanation (C-47 FAIL, C-46 PASS). Both require C-45 PASS as their prerequisite.
- **C-44 cascade from C-42 (unchanged)**: C-42 FAIL → C-44 FAIL is deterministic.
- **C-45 cascade from C-42 (unchanged)**: Independent; no cascade from C-42.
- **C-24 version-bump cascade**: All R18 variations had 45-row amendment tables (correct under v18). Under v19, all R18 variations fail C-24 (47 rows required: T-46 and T-47 absent).

**Key changes to existing criteria:**
- **C-24**: updated from "exactly 45 trigger rows" → "exactly 47 trigger rows (one per C-01..C-47)"
- **Scoring formula denominator**: `/38` → `/40`
...
- **C-24** is structurally testable at PRE-DOCUMENT: a reviewer counts the amendment table rows at initialization. For v19, expected count = 47 (one row per C-01..C-47). A count of 45 (v18 table carried forward without adding T-46, T-47) fires C-24 at PRE-DOCUMENT.
...
## Aspirational Criteria (10% of composite, denominator /40)

### C-01 through C-07 — Mandatory C-tier (not counted in /40 denominator)
...
### C-08 through C-47 — Aspirational Criteria (denominator /40)
...
| C-24 | **Amendment table row count matches v19 criterion count** | behavior | Amendment table contains exactly 47 trigger rows at initialization (one per C-01..C-47 criterion). A table built from v18's criterion list (45 rows) without adding T-46, T-47 fires C-24 at PRE-DOCUMENT. A table with any count other than 47 fails C-24. |
...
| C-45 | **Amendment table T-42 trigger entry includes inline failure-condition exemplar** | behavior | The T-42 row in the amendment table carries an inline exemplar note within the CONDITION field — a quoted or paraphrased construct showing what an abstract-only T-40 CONDITION entry looks like (the exact form that fires T-42). An abstract-only T-40 CONDITION is one that names the condition category without showing what a deficient T-38 CONDITION entry looks like. A T-42 CONDITION entry stating only "fires if T-40 CONDITION lacks an inline failure exemplar" names the condition without exemplifying a deficient T-40 entry and fails C-45. The inline exemplar in T-42's CONDITION cell makes T-42 self-documenting: a reviewer assessing a T-40 CONDITION entry can determine whether T-42 fires without consulting C-42 or the rubric. Satisfying C-42 (T-40 CONDITION carries its own failure exemplar) does not satisfy C-45 — the exemplar in T-42's CONDITION is independently required and concerns T-42's trigger condition, not T-40's. This follows the same structural derivation as C-40 (T-38 CONDITION exemplar from R15 three-tier chain) and C-42 (T-40 CONDITION exemplar from R16 chain confirmation): each chain-tier's trigger entry must be independently self-documenting at the tier above it. |
| C-46 | **Amendment table T-42 trigger entry includes correct-format prescription alongside the inline failure exemplar** | behavior | The T-42 CONDITION field carries both the inline failure exemplar (Part 1, required by C-45) AND an explicit correct-format prescription (Part 2). Part 2 states what a passing T-40 CONDITION entry looks like — the minimum form that satisfies C-42: a quoted or paraphrased exemplar of an abstract-only T-38 CONDITION form, accompanied by an explanation of its abstractness (C-44) and a correct-format prescription (C-43). The two-part T-42 CONDITION makes it fully self-contained: a reviewer can determine both what fires T-42 and what would satisfy C-42 without consulting the rubric or T-40's CONDITION cell. A T-42 CONDITION carrying only Part 1 (the abstract-T-40-form exemplar) without a correct-format prescription passes C-45 but fails C-46. Satisfying C-43 (T-40 CONDITION carries its own correct-format prescription) does not satisfy C-46 — the correct-format prescription in T-42's CONDITION is independently required and concerns what satisfies C-42, not what satisfies C-40. C-45 FAIL → C-46 FAIL (cascade: no exemplar means no correct-format prescription is possible in the same cell). C-45 PASS does not guarantee C-46 PASS — the C-46-independent-fail state is: exemplar present, prescription absent. |
| C-47 | **T-42 CONDITION exemplar includes explanation of why the quoted deficient T-40 form is abstract** | behavior | The T-42 CONDITION field's Part 1 exemplar (required by C-45) must additionally include an explicit explanation of why the quoted or paraphrased T-40 CONDITION form is abstract — stating that it names the trigger condition category without showing what a deficient T-38 CONDITION entry looks like. A T-42 CONDITION that quotes the deficient T-40 form ("fires if T-40 CONDITION lacks an inline failure exemplar — this names the condition without exemplifying what a deficient T-38 entry looks like") passes C-47. A T-42 CONDITION that quotes the deficient T-40 form without any such explanation passes C-45 but fails C-47 — the quoted form alone identifies the deficient instance without explaining the nature of its deficiency. The explanation-of-abstractness is required content, not optional annotation: it makes the T-42 CONDITION fully self-contained such that a reviewer can determine not only what form fires T-42 but why that form is insufficient at the T-40 level. The explanation must appear within the T-42 CONDITION cell's exemplar construct, not in a separate annotation or rubric reference. C-45 FAIL → C-47 FAIL (cascade: an absent exemplar contains no explanation-of-abstractness). C-45 PASS does not guarantee C-47 PASS — the C-47-independent-fail state is: exemplar present but no explanation of why it is abstract. C-46 and C-47 are mutually independent: a T-42 CONDITION can carry an exemplar with explanation but no correct-format prescription (C-47 PASS, C-46 FAIL) or an exemplar with prescription but no explanation (C-47 FAIL, C-46 PASS). |
...
```
composite = 90 + (aspirational_pass / 40 × 10)
```
...
- Amendment table built from v17 criterion list (43 rows) without adding T-44 and T-45 — fires C-24 at PRE-DOCUMENT under v18; also cascades to C-44 FAIL (no T-44 entry to carry the explanation-of-abstractness requirement) and C-45 FAIL (no T-45 entry to carry an inline exemplar for what an abstract T-40 CONDITION looks like); T-42 and T-43 rows in the v17-era table do not supply the required T-44 and T-45 content
- Amendment table built from v18 criterion list (45 rows) without adding T-46 and T-47 — fires C-24 at PRE-DOCUMENT under v19; also cascades to C-46 FAIL (no T-46 entry to carry the correct-format prescription for T-42) and C-47 FAIL (no T-47 entry to carry the explanation-of-abstractness requirement for T-42's exemplar); T-44 and T-45 rows in the v18-era table do not supply the required T-46 and T-47 content
...
- Amendment table T-40 entry carries an inline abstract-T-38-form exemplar (C-42 PASS) with a correct-format prescription (C-43 PASS) but the exemplar does not include an explanation of why the quoted T-38 form is abstract — fails C-44 independently; C-42 PASS and C-43 PASS do not supply the explanation-of-abstractness required by C-44 in the T-40 CONDITION's exemplar construct
- Amendment table T-42 entry states trigger condition abstractly ("fires if T-40 CONDITION lacks an inline failure exemplar") without a quoted or paraphrased sample of what an abstract T-40 CONDITION looks like — fails C-45; also fails C-46 and C-47 by cascade (abstract T-42 CONDITION contains neither Part 1 exemplar nor prescription nor explanation-of-abstractness)
- Amendment table T-42 entry carries an inline abstract-T-40-form exemplar (C-45 PASS, Part 1 present) but stops there — no correct-format prescription stating what a passing T-40 CONDITION entry looks like — fails C-46; satisfies C-45 (structurally analogous to V-01 R15 ceiling for T-37)
- Amendment table T-42 entry carries an inline abstract-T-40-form exemplar (C-45 PASS) with a correct-format prescription (C-46 PASS) but the exemplar does not include an explanation of why the quoted T-40 form is abstract — fails C-47 independently; C-45 PASS and C-46 PASS do not supply the explanation-of-abstractness required by C-47 in the T-42 CONDITION's exemplar construct
...
For early-round discriminator notes (R1–R11), see the v12 rubric file. The v19 rubric inherits all prior discriminator observations unchanged.
...
**V-03 single-point calibration proof**: V-03's design — one deliberately abstract T-42 spec, everything else explicit — confirms the rubric is correctly calibrated at the 37-criterion level: all 37 other criteria are genuinely satisfiable without interdependence, and C-45 is a clean single-point failure with no cascade surface into other non-T-42 criteria. The cascade under v19 (C-45 FAIL → C-46 FAIL → C-47 FAIL) is inert for V-03 because T-46 and T-47 did not exist under v18 — V-03's phrasing register observation establishes that a T-42 spec rewritten with explicit labeled directives would satisfy C-45, C-46, and C-47 simultaneously (same structural guarantee as V-03's T-40 triple-layer pass).
...
- C-46 independent fail boundary: A T-42 CONDITION carrying a quoted deficient T-40 form (C-45 PASS) but no correct-format prescription stating what a passing T-40 CONDITION looks like passes C-45 and fails C-46. Not yet demonstrated live; structurally established by criterion definition and C-46's testability note.
...
- C-47 independent fail boundary: A T-42 CONDITION carrying a quoted deficient T-40 form (C-45 PASS) but no explanation of why it is abstract passes C-45 and fails C-47. Not yet demonstrated live; structurally established by criterion definition and C-47's testability note.
- C-46 and C-47 mutual independence: Confirmed structurally — exemplar with explanation but no prescription (C-47 PASS, C-46 FAIL) and exemplar with prescription but no explanation (C-47 FAIL, C-46 PASS) are distinct states. No live demonstration yet.
