Written to `simulations/quest/rubrics/topic-roadmap-rubric-v9-2026-03-17.md` (432 lines).

---

**Three new criteria extracted from Round 8 excellence signals:**

| Criterion | Source signal | What it tests | Orthogonal to |
|-----------|--------------|---------------|---------------|
| **C-21** Dual-site INERTIA-GATE enforcement | V-03/V-05: gate labeled at Phase 5 entry AND Phase 6 entry | Both structural transition points carry independent enforcement | C-11 (tests gate exists; not that it's at both sites) |
| **C-22** Restart isolation clause in Phase 1 read-barrier | V-05: "restart-without-context clause" in C-09 basis | Pre-read barrier covers mid-run restart, not just first-run | C-09 (tests first-run isolation only) |
| **C-23** Dedicated verdict vocabulary block before Phase 5 | V-05: "Verdict semantics block visible in text" as standalone block | Semantics encapsulated in a self-contained block, independently verifiable | C-13 (distributed framing across phases passes C-13, fails C-23) |

**Formula change:** `/ 24 * 10` → `/ 30 * 10` (15 aspirational criteria × 2 = 30 max).

The orthogonality holds: V-05 achieves all three; V-03 achieves C-21 only; V-02/V-04 achieve none of the three in V-05 form.
ne block rather than embedded across phase instructions.
