...
## Aspirational Criteria (teaching quality — 140 pts)
...
| Aspirational | 165 pts | C-08 through C-40 (33 criteria) | 5 | 165 |

```
composite = (essential_pass/4 * 60)
          + (recommended_pass/3 * 30)
          + (aspirational_pass/33 * 165)
```
...
Aspirational: C-08 [ ]  C-09 [ ]  C-10 [ ]  C-11 [ ]
              ...
              C-36 [ ]  C-37 [ ]  C-38 [ ]  C-39 [ ]
              C-40 [ ]
              Pass count: ___ / 33  ->  ___ / 33 * 165 = ___ pts  (of 165)
...
- **C-31 is aspirational** because making the BAD PLAN block a complete criterion index requires
  deliberate tagging discipline across all 7 criteria, not just the essential 4. C-16 requires
  at least one tag; C-31 requires a complete index -- every criterion must be illustratable in
  the single block. Without this, a model reading the BAD PLAN block gains a complete picture of
  essential violations but must rely on other artifacts (correction table, criterion notes) for
  recommended-tier error shapes. Complete in-block coverage makes the BAD PLAN a standalone
  teaching artifact that requires no cross-document consultation.
- **C-32 is aspirational** because routing the correction bridge to the production gate field
  specifically requires recognizing that C-28's three-field structure creates a new authoring
  target: the `gate:` sibling. Before C-28, `gate:` was the only gate field and C-21 would
  naturally bridge it. After C-28, two teaching fields (`gate_fail:`, `gate_pass:`) surround
  the production field, and the bridge must be placed at the production field explicitly rather
  than assumed to be inherited from the contrast fields. C-32 closes the co-location gap: the
  production slot carries its own lookup pointer, making the three-field teaching unit complete
  at the field level.
- **C-33 is aspirational** because maximal zone teaching density requires the prompt author to
  recognize that four independent annotation obligations exist in every dependent zone and to
  satisfy all four without allowing any to displace or substitute for another. Each prior
  criterion (C-26, C-27, C-28, C-30, C-32) is independently satisfiable; C-33 tests whether
  they can coexist in the same zone without annotation crowding or substitution. The R8
  evidence shows that high-scoring variates typically satisfy three of the four mechanisms --
  C-33 identifies the zone-level capstone where all four coexist, yielding a fully equipped
  authoring unit for every dependent zone.
- **C-34 is aspirational** because compound annotation at the `gate_fail:` field requires the
  prompt author to recognize that two independently satisfiable obligations (criterion tag via
  C-26, rationale via C-25) must be deliberately co-located at the structural field rather than
  split across field-inline and header positions. The R9 evidence (V-02) shows that building
  full gate production machinery (C-24+C-28+C-32) does not automatically produce the compound
  teaching annotation at `gate_fail:` -- the criterion tag and rationale each require their own
  deliberate placement at the field. C-34 closes the co-location gap: the `gate_fail:` field
  itself becomes the complete teaching unit, readable in isolation without surrounding context.
- **C-35 is aspirational** because dual error-format recommended coverage requires the prompt
  author to recognize that the BAD PLAN block (illustration format) and the correction table
  (lookup format) independently serve different model consultation patterns and must each carry
  full recommended-tier coverage to be effective as standalone reference artifacts. C-31 closes
  the BAD PLAN gap; C-29 closes the correction table gap; C-35 closes the cross-format gap by
  requiring both to independently carry recommended-tier error shapes. The R9 evidence (V-01)
  shows that achieving C-31 does not automatically produce C-29 -- the correction table
  recommended-tier rows require separate deliberate construction. A prompt satisfying C-35
  leaves no recommended-tier gap in either primary error reference format.
- **C-36 is aspirational** because BAD PLAN header label accuracy requires the prompt author to
  maintain coherence between the block label and the block body -- a discipline that becomes
  relevant only after C-31 is achieved. Before C-31, the block genuinely covers only essential
  criteria, making an "essential violations" label accurate; after C-31, the block covers all 7
  criteria and the label must be updated to reflect that. The R10 evidence (V-01) shows that
  C-31 can be achieved while retaining a label from an earlier essential-only version of the
  same prompt -- the label becomes stale rather than deliberately misleading, but the effect is
  the same: a model reading the header misestimates the block's teaching coverage. C-36 closes
  the label-currency gap, ensuring the header is updated alongside the body when coverage expands.
- **C-37 is aspirational** because field-level stage name annotation requires the prompt author
  to apply criterion-referenced tagging at a position distinct from the gate fields where C-26
  establishes the same discipline. C-26 requires `# WRONG C-04` at structural `gate_fail:`
  fields; C-37 requires `# WRONG C-06` at violating `name:` fields in the BAD PLAN block.
  The parallel is intentional: just as C-26 closes the gap between criterion tags in BAD blocks
  (C-16) and criterion tags at structural template fields (C-24), C-37 closes the gap between
  C-06 coverage anywhere in the BAD PLAN block (C-31) and C-06 coverage at the `name:` field
  specifically. The R10 evidence (V-03) shows that C-06 violations in BAD PLAN stage names can
  be left entirely unannotated even when C-22 and C-31 are targeted -- the field-level position
  is a distinct authoring decision. A prompt satisfying C-37 makes the wrong stage name pattern
  self-diagnosing at the field position itself, identical to how C-26 makes the wrong gate
  pattern self-diagnosing at `gate_fail:`.
- **C-38 is aspirational** because affirmative header labeling requires the prompt author to
  move from passive accuracy (not restricting scope, C-36) to active communication (asserting
  full scope). The incremental cost is one header revision; the benefit is that a model reading
  the BAD PLAN header before scanning the block is primed to look for all 7 criterion tags
  rather than scanning open-endedly or assuming partial coverage. C-36 closes the false-
  restriction failure; C-38 closes the neutral-silence failure. Together they ensure the header
  is both accurate and informative.
- **C-39 is aspirational** because field-level skills annotation completes the co-location
  discipline across all three primary YAML field types. C-26 established co-location for
  `gate_fail:` fields (C-04); C-37 established it for `name:` fields (C-06); C-39 extends it
  to `skills:` entries (C-03), making the BAD PLAN block fully self-annotated — every field
  that carries a criterion violation names the criterion at that field, with no criterion left
  to contextual inference. The discipline is uniform: wrong shape + criterion tag at the
  violating field, regardless of field type.
- **C-40 is aspirational** because the conjunction of C-34 (YAML-template compound annotation)
  and C-29 (correction-table recommended-tier rows) closes the gap between the two
  generation-time teaching formats. C-35 pairs example-format (BAD PLAN block) with lookup-
  format (correction table) for recommended-tier coverage; C-40 pairs template-zone-format
  (compound gate_fail: annotation) with lookup-format (correction table). The two conjunction
  criteria are orthogonal -- a prompt may satisfy C-35 without C-40 (if YAML-template compound
  annotations are absent) and C-40 without C-35 (if the BAD PLAN block lacks recommended tags).
  Together, C-35 and C-40 ensure all three error-teaching formats independently carry
  recommended-tier coverage.
...
- **Scoring total is 255** (not 240) because the aspirational tier expanded from 30 to 33
...
| v9 | 2026-03-16 | Added C-31/C-32/C-33 from R8 excellence signals; aspirational 23->26 criteria, 115->130 pts; total 220 pts |
| v10 | 2026-03-16 | Added C-34/C-35 from R9 excellence signals; aspirational 26->28 criteria, 130->140 pts; total 230 pts |
| v11 | 2026-03-16 | Added C-36/C-37 from R10 excellence signals; aspirational 28->30 criteria, 140->150 pts; total 240 pts |
| v12 | 2026-03-16 | Added C-38/C-39/C-40 from R11 excellence signals; aspirational 30->33 criteria, 150->165 pts; total 255 pts |
