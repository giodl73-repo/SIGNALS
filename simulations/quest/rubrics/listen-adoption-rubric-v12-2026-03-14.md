`listen-adoption-rubric-v12-2026-03-14.md` written. Here's what was extracted and why:

---

**Three new criteria from R11 excellence signals. Max 215 → 230.**

**C-34 — Correction mechanism anchor double-anchored at gate level and verification boundary** (5 pts)
V-05 was the only R11 variation to implement C-32 at *both* structural locations simultaneously: per-gate footers inside H/I/J AND a pre-verification declaration before `## VERIFICATION MODE`. The distinction from C-32 is that a single-location anchor proves availability at one point; double-anchoring proves it at two independent routes, so neither level is load-bearing alone. V-01/V-04 (boundary only), V-02 (gate only), V-03 (dedicated section only) all pass C-32 and fail C-34. Dependency: C-32.

**C-35 — STRUCTURAL CONTRACT explicitly names answer-form and mechanism anchor as mandatory co-present requirements** (5 pts)
V-01/V-02/V-03 satisfied C-33 via *implicit* co-presence — both mechanisms happened to be in the document. V-04/V-05 introduced a STRUCTURAL CONTRACT block that *declares* both mechanisms as mandatory, making their co-presence architecturally required rather than emergent. The observable distinguisher: no trade-off between the two mechanisms is possible without violating the contract itself. Dependency: C-33.

**C-36 — TABLE 3 row labels typed as structural slot keys in the table header** (5 pts)
C-31 enforces per-transition coverage by slot *count* (four named rows). V-05's TABLE 3 header went further — explicitly naming the rows as *typed structural slot keys*, signaling the slot-type contract to the downstream model at the header level. Same elevation class as C-27/C-28 (label-encodes-constraint pattern), now applied at the table-header level. Dependency: C-31.
...
C-33 -> C-35
```
ade-off between the two mechanisms is architecturally possible. C-35 tests
whether their co-presence is explicitly mandated at the structural contract level rather than
emergent from separate implementations. A document that passes C-29 and C-32 without a STRUCTURAL
CONTRACT passes C-33 and fails C-35; a document with a STRUCTURAL CONTRACT naming both mechanisms
as mandatory co-present requirements passes C-35. Fails automatically if C-33 fails.

**C-36 — TABLE 3 row labels typed as structural slot keys in the table header** (5 pts)
From: V-05 R11 C-31 enhancement. C-31 tests whether the four named transition-pair rows exist as
structural slots, enforcing per-transition coverage by slot count. V-05's TABLE 3 header
explicitly names the four rows as typed structural slot keys — signaling a slot-type contract to
the downstream model beyond row-count enforcement. Same elevation class as C-27 (field label
encodes concrete-action constraint) and C-28 (spread mechanism label encodes specificity
constraint): C-36 is the table-header analogue, encoding the row-as-typed-slot-key contract so
the model cannot satisfy the header's own type contract without providing a mechanism entry that
fits the slot type for each named row. A table with four named rows but a generic header passes
C-31 and fails C-36; a table whose header explicitly names the four rows as typed structural slot
keys passes C-36 regardless of whether any separate per-transition instruction exists. Fails
automatically if C-31 fails.
...
**Max composite: 215 → 230. Golden threshold (80%) unchanged.**
