v10 is written to `simulations/quest/rubrics/corps-scan-rubric-v10-2026-03-16.md`. Here is the complete updated rubric:
...
Both V-01 (Hypothesis Officer as Role 1) and V-02 (Phase 1 GATE/Case File) independently placed a testable structural prediction as the gate's primary output before any repo scanning began. The hypothesis-first gate transforms the scan from open-ended evidence collection into a structured experiment. Distinct from C-20 (compliance gate) -- C-38 requires a falsifiable predictive claim about the repo, not just constraint acknowledgment.

**C-39 -- DARK SIGNALS entries carry per-entry hypothesis relation** (depth)
Extends C-36: each DARK SIGNALS entry includes a "Hypothesis Relation" field (CONFIRMED / OVERTURNED / neutral). Makes the negative evidence section dual-purpose: pivot disqualification evidence (C-36) AND hypothesis testing instrument (C-39). Source: V-01 R10.

**C-40 -- Amend options carry hypothesis confirmation/overturn status** (behavior)
Extends C-37: at least one amend option carries `hypothesis [CONFIRMED/OVERTURNED]` alongside the DARK SIGNALS label citation, closing the hypothesis lifecycle at the amend layer. The full chain is now auditable from three fixed artifacts: gate (C-38) → DARK SIGNALS entry (C-39) → amend option (C-40). Source: V-01 R10.

**Scoring: 235 → 250 pts** (32 aspirationals × 5 = 160 pts + 60E + 30R). Golden threshold unchanged at 80 pts.

The trilogy follows the same layering pattern as C-36/C-37: C-38 is the precondition (declare the hypothesis), C-39 is the data layer (connect each negative finding to it), C-40 is the action layer (carry the conclusion into the amend decision).
NALS basis: [LABEL] -- rules out [mode]; hypothesis
...
strong outputs from excellent ones. 32 criteria x 5 pts = 160 pts available.
