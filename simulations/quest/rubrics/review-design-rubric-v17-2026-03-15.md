# review-design Rubric — v17

**Skill**: `review-design`
**Rubric version**: v17 (C-01 through C-51, denominator 44)
**Extracted from**: R17 scorecard (2026-03-15)

---

**Summary of changes from v16 to v17:**

**New criteria (C-50, C-51):**

- **C-50 | BLOCK 2.7 Registry Inbound Integrity Halt** — A named halt (F-29) fires when any BLOCK 2.7 P1 Section registry row does not correspond to a BLOCK 2 P1 finding. This closes the inbound direction of the BLOCK 2.7 <-> BLOCK 2 bidirectional contract: F-28 (C-42) guards the outbound direction (BLOCK 5 must only use sections from the registry); F-29 guards the inbound direction (registry may only contain sections sourced from BLOCK 2 P1 findings). Source: V-02 R17, BLOCK 2.7 SEALED: "F-29 (no spurious entries: every registry row maps to a BLOCK 2 P1 finding)."

- **C-51 | BLOCK 2.7 Registry Structural Position Named Halt** — A named halt (F-30) fires when BLOCK 2.7 is not present in its conformant position — after BLOCK 2.5 and before BLOCK 3. F-30 is the first F-code to fire on a block's lifecycle position rather than on the content or identity of values within the block, introducing the position-integrity enforcement class. Source: V-02 R17, BLOCK 2.7 SEALED: "F-30 (registry structurally present in conformant position after BLOCK 2.5 and before BLOCK 3)."

**R17 signals added to existing criteria:**

- **C-47** — V-01 achieves PASS: canonical form confirmed — F-31 present in BLOCK 3 body with closed-enumeration corrective action and named in BLOCK 3 SEALED enumeration.
- **C-48** — V-01 FAIL gap identified: BLOCK 2.7 SEALED names only F-28; F-29 and F-30 absent. V-02 PASS: BLOCK 2.7 SEALED enumerates all three — F-29, F-30, F-28 with role descriptions. BLOCK 2.7 has three F-IDs to enumerate: F-28, F-29, F-30.
- **C-49** — V-01 and V-02 both FAIL: prose rule present with tie-prohibition ("No two rows SHALL share the same Priority Rank") but without numbered tier steps, rank-range declaration (1..ELEVATED_count), or non-conformance prohibition.

**Formula:** denominator 44 → 44. Each aspirational criterion ~0.227 pts. Ceiling 100 preserved.
...
Note: aspirational denominator changed from 42 to 44 in v17 (added C-50, C-51). Each
aspirational criterion contributes ~0.227 points at full pass.
