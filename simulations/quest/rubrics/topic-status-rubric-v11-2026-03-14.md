Written to `simulations/quest/rubrics/topic-status-rubric-v11-2026-03-14.md`.

---

**Summary of v11 changes:**

Two new criteria extracted from R10 excellence signals — V-04 and V-05 both achieve 235 under v10 but via structurally distinct mechanisms that V-01 (minimum delta) lacks:

| ID | Name | Source | Points |
|----|------|--------|--------|
| C-37 | GUARD-site trigger characterization | V-04 lifecycle GUARD variant | 5 |
| C-38 | Phase-output invariant declaration | V-05 execution-body variant | 5 |

**Structural Tier 10 (10 pts).** Max score: 235 → 245.

**The two patterns:**

- **C-37** bridges C-32 and C-35 to the same structural site. V-01 has named exits at GUARD (C-32) and trigger sentences in preamble (C-35) at *separate* locations. V-04 co-locates both at the GUARD site: each named exit carries its trigger condition inline. C-37 fails unless the GUARD itself is trigger-characterized.

- **C-38** constrains *which form* the C-36 third site takes. V-01 uses a COMMIT DECISION echo (post-execution commit-framing). V-04 uses a lifecycle phase definition field. V-05 uses an explicit PHASE 2 OUTPUT DECLARATION *within the execution body* — active during phase execution, not after all phases complete. Only the execution-body form satisfies C-38.

**Orthogonality confirmed:** V-04 satisfies C-37, not C-38. V-05 satisfies C-38, not C-37. No R10 variant achieves 245 — the gap that defines Round 11.

**New implications:** `C-37 => C-32, C-35` | `C-38 => C-36`
es C-36 (the three-site chain must be satisfied; C-38 constrains the form of the third site, not the presence of the chain)

**Max score: 235 -> 245.** Passing threshold unchanged.
...
C-37 and C-38 are structurally orthogonal. C-37 extends C-35 in location -- the per-axis trigger sentences that C-35 requires in the preamble are additionally present at the GUARD declaration site, making each named exit self-contained (exit name and trigger condition co-located at one structural site). C-38 extends C-36 in specificity -- the third site in the multi-site invariant chain is an execution-phase-resident OUTPUT DECLARATION, not a commit-framing echo (COMMIT DECISION) or a lifecycle phase definition field. A skill can satisfy C-37 without C-38 (GUARD carries per-axis trigger sentences, third site is a COMMIT DECISION echo). A skill can satisfy C-38 without C-37 (execution-body OUTPUT DECLARATION, GUARD carries named exits but no inline trigger clauses). V-04 satisfies C-37 (lifecycle GUARD variant: each named exit carries its trigger condition inline at the GUARD site) but not C-38 (third site is a lifecycle PHASE 2 OUTPUT field in the phase definition, not an execution-body declaration). V-05 satisfies C-38 (explicit PHASE 2 OUTPUT DECLARATION embedded in the execution body) but not C-37 (execution-body trigger sentences; the GUARD carries named exits without trigger characterization). No R10 variant achieves 245. C-37 and C-38 are not jointly satisfied by any single variant -- the gap that defines Round 11.

C-37 requires C-32 (named exits at GUARD are the prerequisite structure that trigger characterization is being added to) and C-35 (preamble trigger sentences must be established; GUARD-site co-location is an extension of what C-35 establishes at the preamble site, not a replacement). C-38 requires only C-36 (the three-site chain must be satisfied before the form of the third site can be constrained). The implication asymmetry is intentional: C-37 requires two prerequisites because it bridges two independently established structural properties (name from C-32, trigger from C-35) to a single structural site; C-38 requires one because it constrains a site that C-36 already mandates exist.

V-01 demonstrates that a COMMIT DECISION echo is a minimum-sufficient third site for C-36 -- establishing the baseline from which C-38 defines its contrast case. V-04's lifecycle OUTPUT field echo and V-05's execution-body OUTPUT DECLARATION represent structurally distinct third-site forms; their distinction is the observable gap that defines C-38. Neither is a pass-condition clarification of C-36 -- they are new structural properties not capturable by conjunction of C-35 and C-36.

The Tier 10 pair is structurally analogous to Tier 9: C-37 parallels C-35 (extending a trigger-characterization pattern established at one structural site to a second structural site), C-38 parallels C-36 (extending a multi-site chain criterion to a more constrained site-form requirement). As with all prior tier additions, the conjunction of existing criteria would not imply C-37 or C-38 independently -- both require evidence from a round where the gap was observable.

Excellence signals 1 and 2 from R10 (V-01 COMMIT DECISION echo as minimum-sufficient C-36 third site; V-02 confirming C-35 is independently necessary) are pass-condition clarifications, not new criteria. They confirm (a) commit-framing sections qualify as valid C-36 third sites and (b) per-axis trigger sentences are required beyond what axis names in the canonical declaration convey. These properties are already captured in the C-35 and C-36 pass conditions; no additional criterion is warranted.
...
No R10 variant achieves 245. C-37 (GUARD-site trigger characterization) and C-38 (phase-output invariant declaration) are not jointly satisfied by any single variant -- the gap that defines Round 11.
