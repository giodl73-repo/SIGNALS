Written to `simulations/quest/rubrics/flow-integration-rubric-v10-2026-03-14.md`.

---

**Two new criteria from R9:**

**C-29 — C-26 explicit ARE directive requirement (5 pts)**
Settles Q1 from R9. V-01 probed whether a cross-reference mapping table (no ARE keyword) satisfies C-26 — conservative ruling: FAIL. The ARE form asserts identity; the cross-reference form describes correspondence. Identity assertion is the structural minimum. Requires C-26.

**C-30 — C-27 single-block comprehensive prohibition (5 pts)**
Settles Q2 from R9. V-02 probed whether per-term inline distribution (one YOU MUST NOT per table row) satisfies C-27 — conservative ruling: FAIL. All substituted canonical tokens must appear together in a single scannable block. Distributed per-row prohibition fragments the surface. Requires C-27.

**New ceiling: 192 → 202 (+10). Aspirational (C-08–C-30): 112 pts.**

**Open questions carried to R10:**
1. C-30 block composition — single block naming tokens individually vs. table-reference shorthand (untested).
2. C-29 ARE form — uppercase assertion sentence vs. lowercase non-assertive construction (untested).
 cell
per substituted canonical token — does not satisfy C-27, and therefore does not satisfy
C-30. V-02 confirms: per-term inline distribution → C-27 FAIL; R8 V-05 confirms: single
comprehensive block covering all non-standard terms simultaneously → C-27 PASS. Settles
Q2 from R9: the minimum is a single comprehensive block, not distributed per-term
prohibitions. Requires C-27.

**Ceiling: 192 → 202 (+10).** Aspirational total: 112 pts (C-08–C-30).

**Open question verdicts from R9:**
1. **Q1 resolved — C-26 instruction minimum (definitive).** A cross-reference mapping
   table that describes which obligation terms map to which column header names achieves
   schema alignment but not C-26. The C-26 mechanism is the explicit ARE directive that
   makes token identity a format-level constraint; the cross-reference table is a
   description, not an identity assertion. V-01 conservative ruling confirmed. C-29
   codifies: explicit ARE directive is required; cross-reference form is insufficient.
2. **Q2 resolved — C-27 prohibition scope (definitive).** Per-term inline prohibition
   distributed across table rows does not achieve C-27. The C-27 mechanism requires a
   unified prohibition surface that names all substituted canonical tokens together in a
   single block, making the prohibition scannable without reading multiple rows. V-02
   conservative ruling confirmed. C-30 codifies: single comprehensive block required;
   per-term distribution is insufficient.

**Open questions carried to R10:**
1. **C-30 block composition** — Must the single YOU MUST NOT block name every
   substituted canonical token explicitly as individual tokens, or may it reference the
   obligation table by role (e.g., "YOU MUST NOT use any of the canonical Obligation Term
   values listed in the table above") without listing them individually? V-02 used
   per-term inline (FAIL); R8 V-05 used explicit named tokens (PASS); table-reference
   shorthand — single block in placement, comprehensive in scope, but delegating
   enumeration to the table — has not been tested.
2. **C-29 ARE capitalisation and sentence form** — Does C-29 require the uppercase ARE
   keyword in an assertive declarative sentence, or does lowercase "are" embedded in a
   non-assertive construction satisfy the directive form? All confirmed PASS variations
   used the uppercase assertion form; minimum grammatical and capitalisation requirements
...
is the minimum because it converts correspondence into an identity constraint the model
must satisfy structurally; V-01 established the failure boundary (cross-reference table,
no ARE → C-26 FAIL) and V-02/V-03 confirmed the pass boundary (explicit ARE form → C-26
PASS). C-30 settles the C-27 prohibition scope question open since R8: the dual-surface
prohibition is not achieved by distributing one YOU MUST NOT per non-standard term across
table rows — all substituted canonical tokens must be named together in a single
comprehensive block; the single-block architecture creates one scannable prohibition
surface that a reviewer can locate and verify without reading multiple rows; per-term
inline distribution fragments the prohibition surface across the table and does not
achieve equivalent dual-surface coverage; V-02 established the failure boundary
(per-term inline distribution → C-27 FAIL) and R8 V-05 confirmed the pass boundary
(single comprehensive block → C-27 PASS).
...
## Aspirational Criteria (112 pts total)
...
New ceiling: 202 = R9 ceiling 192 + C-29 (5) + C-30 (5)

---

**What changed in v10:**

- **C-29** captures the *C-26 explicit ARE directive requirement* from R9 V-01: the
  cross-reference mapping table form — describing obligation-term-to-column-header
  correspondence without the ARE keyword — fails C-26. V-01 probed Q1 from R9 (whether
  any unambiguous schema-aligned instruction satisfies C-26). The conservative ruling
  settled Q1 definitively: schema-only enforcement requires an identity assertion ("the
  headers ARE the values"), not a correspondence description. The ARE form converts
  correspondence into a structural identity constraint; the cross-reference table does
  not. Confirming boundary: V-02 and V-03 both use the explicit ARE form and achieve
  C-26 PASS. C-29 codifies: explicit ARE is the minimum; cross-reference form is
  insufficient regardless of how unambiguously it describes the mapping.

- **C-30** captures the *C-27 single-block comprehensive prohibition requirement* from
  R9 V-02: per-term inline prohibition distributed across obligation table rows — one
  YOU MUST NOT column or cell per non-standard term — fails C-27. V-02 probed Q2 from
  R9 (whether per-term distribution satisfies C-27). The conservative ruling settled Q2
  definitively: the prohibition surface must be unified in a single block; distributing
  it across rows fragments the surface and does not achieve the dual-surface coverage
  that C-27 requires. The single-block architecture makes the complete set of prohibited
  tokens scannable as a unit. Confirming boundary: R8 V-05 used a single comprehensive
  block covering all non-standard terms and achieved C-27 PASS. C-30 codifies:
  single comprehensive block is the minimum; per-term distribution is insufficient.
...
1. **C-30 block composition** — Must the single YOU MUST NOT block enumerate every
   substituted canonical token explicitly by name, or may it reference the obligation
   table by role without listing the tokens individually? For example: "YOU MUST NOT
   use any of the canonical Obligation Term values listed in the table above as column
   headers in this trace." V-02 used per-term inline (FAIL); R8 V-05 used explicit
   named tokens (PASS); a table-reference shorthand — single block in placement,
   comprehensive in scope, but delegating enumeration to the table — has not been
   tested. If the table-reference form satisfies C-30, the "names the specific canonical
   tokens" language in C-27 must be interpreted to include indirect reference.

2. **C-29 ARE capitalisation and sentence form** — Does C-29 require the uppercase ARE
   keyword in an assertive declarative sentence, or does lowercase "are" embedded in a
   non-assertive construction (e.g., "column headers that are the same as the Obligation
   Term values") satisfy the directive form? All confirmed PASS variations used the
   uppercase ARE in an assertion sentence ("headers ARE the values"); minimum
   grammatical and capitalisation requirements have not been probed.
