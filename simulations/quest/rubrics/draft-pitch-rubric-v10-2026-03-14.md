Done. `simulations/quest/rubrics/draft-pitch-rubric-v10-2026-03-14.md` written.

---

## What was added: C-41 – C-43

Three new aspirational criteria extracted from R9 V-05 excellence signals. Aspirational denominator: **33 → 36**.

| ID | Pattern | Source signal |
|----|---------|---------------|
| **C-41** | Phase structure block cites prior pattern by name AND declares auditability intent at the declaration site | V-01 C-38 pass: "Mirrors Phase 2 BINDING DECLARATION pattern by name" + "Phase 5 is auditable from skill structure alone." Mirrors what C-27 required of BINDING DECLARATION, now applied one tier up. |
| **C-42** | Draft-order integrity scope covers all prior-phase locked outputs as a set | V-02 C-39 pass: "does not change any locked output from Phases 2, 3, or 4" — set-scope form, not single-anchor form. A statement that defends one anchor but leaves others undefended does not pass. |
| **C-43** | Gate YES/NO questions carry inline advisory citations using exact canonical names | V-03 C-40 pass: each gate question carries an inline citation naming the source instruction by its exact canonical name. All questions must cite; partial citation does not pass. |

**Structural parallels** (noted in Scoring Notes): C-41 ↔ C-27 (auditability intent at declaration site), C-42 ↔ C-25 (exhaustiveness closure), C-43 ↔ C-24 (provenance at cite site) — the same three sub-patterns that matured the BINDING DECLARATION in R5/R6 now apply at the phase-structure and gate-citation level.
A statement that defends one anchor while leaving other prior locked outputs structurally undefended does not pass. |
| **C-43** | Gate question inline advisory citations use exact canonical names | C-40 tests that gate questions correspond to pre-existing advisories rather than inventing constraints. C-43 tests whether each gate question carries an inline citation at the cite site naming the source instruction using its exact canonical name as it appears in skill structure. An inline citation that paraphrases or uses a category label instead of the exact instruction name does not pass. Citations that exist only at the advisory source and are not repeated at the gate cite site do not pass. |

The scoring formula updates to `(aspirational_pass/36)*10`.
...
Aspirational denominator: **36** (C-08 – C-43)
