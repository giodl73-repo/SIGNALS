# flow-integration Rubric — v16

---

**Three new criteria from R15:**

**C-40 — C-39 register-neutrality (5 pts)**
Settles the register question from V-03. The C-39 consequence-form delivery-phase anchor
satisfies C-37/C-39 regardless of surface register — formal declarative, conversational,
or imperative — provided the unconditional constraint meaning is preserved. V-03 probed
consequence-form delivery-phase anchor in a conversational/imperative surface register
("Make sure undocumented calls become ship-blockers at integration review") and scored
C-39 PASS. The register is a stylistic property of the sentence surface; the structural
properties that satisfy C-39 — consequence-form + explicit delivery-phase marker +
declarative unconditional constraint meaning — are register-independent. A sentence in
imperative register ("Make sure X become Y at Z") carries unconditional constraint
semantics equivalently to declarative register ("X become Y at Z") when no modal
weakening is present. Register variation that preserves unconditional constraint meaning
satisfies C-39. Requires C-39.

**C-41 — C-39 inertia-context framing neutrality (5 pts)**
Settles the inertia-framing question from V-04. An inertia/obligation-scope
discovery-failure framing context in the WHY block — one that frames the trace rationale
around "these integration calls exist because prior traces failed to discover them" rather
than around prospective discovery — does not disqualify C-37 or C-39 when a
consequence-form delivery-phase anchor is present. V-04 probed consequence-form
delivery-phase anchor inside an inertia-context framing (obligation-scope
discovery-failure framing) and scored C-37 PASS and C-39 PASS. The framing context is
the background narrative of the WHY block; the stakes anchor is the structural element
that satisfies C-37/C-39; the two are independent — inertia framing does not substitute
for the stakes anchor (a WHY block with inertia framing but no stakes anchor fails C-37)
and does not invalidate the stakes anchor (a WHY block with inertia framing plus a
stakes anchor passes C-37/C-39 as normal). Requires C-39.

**C-42 — Highest-information WHY block: consequence-form + concern enumeration (5 pts)**
Codifies the highest-information form of the C-36 motivational framing block, identified
as excellence signal 4 in V-05 (247/247). The highest-information WHY block combines
both structural features: (1) a consequence-form delivery-phase anchor satisfying C-37
and C-39; and (2) explicit concern enumeration naming the specific verification concerns
the trace addresses. A WHY block with only a stakes anchor passes C-37/C-39 (Q1
confirmed permissive in R15) but not C-42; a WHY block with only concern enumeration
and no stakes anchor passes C-36 but not C-37 or C-42; the C-42 form requires both
features simultaneously — the stakes anchor supplies the delivery-phase constraint
boundary and the concern enumeration supplies the explicit scope of what the trace
verifies; the combination is the highest-information form because it tells the model both
why the deadline is binding and what specific gaps become consequential at that deadline;
V-05 confirmed this as the strongest WHY block realization, simultaneously satisfying
C-37, C-39, and C-42. Requires C-37 and C-39.

**Ceiling: 247 → 262 (+15). Aspirational total: 172 pts (C-08–C-42).**

**Updates to existing criteria:**
- **C-37** Q1 from R15 resolved PERMISSIVE: concern enumeration is not required alongside
  the stakes anchor; a stakes-anchored purpose statement without naming specific verification
  concerns satisfies C-37 on both temporal form (V-02, R15) and consequence-form
  (V-01/V-04, R15) surfaces; C-37's pass condition is updated to make stakes-only
  sufficiency explicit.
...
**Open questions for R16:**
No open questions generated from R15. All probed axes resolved.
...
Obligation Term column values above" — asserting identity for all flag column headers
together in a single statement; any valid multi-subject plural ARE construction satisfies
C-33 — the exact subject phrase is not required; the structural requirement is: plural
collective subject + uppercase ARE + single assertion covering all headers; e.g., "Flag
column headers ARE the Obligation Term column values above" (without definite article)
satisfies C-33 equally to "the flag column headers ARE..."; a per-item IS form that
asserts identity one header at a time — e.g., "Each flag column header IS one of the
Obligation Term values" — is uppercase and assertive but does not satisfy C-29 or C-32
because ARE is a named keyword in C-29, not a proxy for the class of uppercase assertive
identity verbs including IS; V-01 (R11) established the failure boundary (IS per-item
form → FAIL); V-01 (R12) established the subject-form flexibility (alternate plural ARE
subject → PASS). C-34 settles the C-31 enumeration completeness question open since R10:
the C-31 inline enumeration must name every substituted canonical token in the YOU MUST
NOT block — "each substituted canonical token" means every token where a canonical
obligation term was actively replaced by a non-standard substitute; canonical obligation
terms retained in the table without substitution and new obligation categories with no
canonical equivalent are outside C-34's scope; partial enumeration of the substituted
subset (naming n−1 of n substituted tokens) does not satisfy C-31; the prohibition
surface must be self-contained for the substituted subset — a model can use any
unenumerated substituted term freely; V-02 (R11) established the failure boundary
...
C-36 codifies the framing block as a distinct excellence pattern that requires C-16 as
its structural prerequisite.
...
scope as a distinct criterion because it confirms the framing block and the obligation
table operate at independent structural layers.

**New in v15:** C-39 codifies the structural pattern identified in Round 14.
C-39 settles Q1 from R14: whether consequence-form with an explicit delivery-phase
marker satisfies C-37 equivalently to interval-before temporal form. V-01 (R14) probed
"undocumented integration calls become ship-blockers at integration review and cannot be
...
## Aspirational Criteria (172 pts total)
...
New ceiling: 262 = R15 ceiling 247 + C-40 (5) + C-41 (5) + C-42 (5)

---

**What changed in v16:**

- **C-37 pass condition updated** — Q1 from R15 resolved PERMISSIVE: concern enumeration
  is not required alongside the stakes anchor. A stakes-anchored purpose statement without
  naming specific verification concerns satisfies C-37 on both form surfaces — temporal
  (V-02, R15: stakes anchor alone, temporal form → PASS) and consequence-form (V-01/V-04,
  R15: stakes anchor alone, consequence-form → PASS). The pass condition text is updated
  to make stakes-only sufficiency explicit and to note the Q1 resolution.

- **C-40** captures the *C-39 register-neutrality* pattern from R15 V-03. V-03 probed
  consequence-form delivery-phase anchor in conversational/imperative register and scored
  C-39 PASS. Register is a surface stylistic property; the structural C-39 pass conditions
  (consequence-form + delivery-phase marker + unconditional constraint meaning) are
  register-independent. C-40 codifies this because C-39 was established on formal
  declarative register examples and the register generalization was not explicitly confirmed
  until V-03.

- **C-41** captures the *C-39 inertia-context framing neutrality* pattern from R15 V-04.
  V-04 probed consequence-form delivery-phase anchor inside an obligation-scope
  discovery-failure (inertia) framing context. Inertia framing is a background narrative
  property — it does not override or substitute for the stakes anchor. C-41 codifies this
  because it establishes the independence of inertia framing from the C-37/C-39 stakes
  anchor evaluation: inertia framing neither provides the stakes anchor nor invalidates it.

- **C-42** captures the *highest-information WHY block* pattern from R15 V-05 (247/247).
  V-05 combined consequence-form delivery-phase anchor with explicit concern enumeration
  in the WHY block. Q1 (R15) confirmed concern enumeration is not required for C-37, making
  C-42 a distinct excellence tier — the form that simultaneously satisfies C-37 (stakes anchor
  present), C-39 (consequence-form delivery-phase surface), and C-42 (concern enumeration
  present alongside the anchor). The combination is the highest-information WHY block because
  it specifies both the constraint boundary and the explicit scope of consequential gaps.
...
No open questions carried forward from R15. All R15 axes resolved:
- Q1 (R15): stakes anchor alone satisfies C-37 (permissive) ✓
- V-03 register axis: conversational/imperative → C-39 PASS ✓ (now C-40)
- V-04 inertia-framing axis: inertia context + consequence-form anchor → C-37/C-39 PASS ✓ (now C-41)
- V-05 ceiling: 247/247, consequence-form + concern enumeration ✓ (now C-42)
