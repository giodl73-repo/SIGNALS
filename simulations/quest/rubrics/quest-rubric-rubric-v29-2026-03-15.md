## quest-rubric rubric v12 -- complete

### Changes from v11

Three new aspirational criteria (C-35, C-36, C-37) extracted from Round 11 excellence
signals. Denominator /26 -> /29. Round 11 ceiling 100 (unchanged max; prior ceiling 98.966
without new criteria).

| # | Name | Signal it formalizes |
|---|------|---------------------|
| **C-35** | divergence-check-operationalized | C-13 universal PARTIAL -- all five Round 11 variations tier criteria by failure severity (producing organic category diversity by design) but none run an explicit divergence check that counts category assignments per tier and outputs the per-tier distribution as a verification artifact; C-13 requires both the tier-grounded design and the divergence check; C-35 requires the check to be a named step with structured output, making the verification independently falsifiable rather than a property of design intent |
| **C-36** | competitor-at-construction-step | V-05 PARTIAL on C-11 -- V-05 places the Unbounded-Aspirational competitor inline at Builder A (the exact step where aspirational count is committed); C-27 requires the competitor to constitute the gap specification regardless of placement; C-28 requires a derivation instruction; neither requires the competitor to appear at the specific construction step where the wrong implementation would most naturally be produced; V-01--V-04 group competitors in preamble sections (or have none); V-05 embeds each competitor inline at its construction step, creating a just-in-time prevention gate |
| **C-37** | competitor-criterion-coverage-complete | V-05 PASS on C-27/C-28 via four named competitors, each mapped to exactly one aspirational gap (Unbounded-Aspirational -> C-31, Category-Menu -> C-32, Formula-Only Scorer -> C-33, Declared-Independent Map -> C-34); C-27 requires competitors to exist and to constitute gap specifications; C-37 requires the coverage to be complete -- every aspirational criterion that introduces a novel construction requirement has exactly one corresponding competitor, and the competitor count equals the novel-criterion count, making the gap structure enumerable by inspection without reading the criteria themselves |

**Denominator:** `/26` -> `/29`

**New ceiling math:** A rubric passing all 26 Round 11 criteria (the Round 11 ceiling,
twenty-six of twenty-nine) scores 26/29 x 10 = 8.966 on aspirational; with full
essential+recommended that yields 98.966 -- still golden, but the new ceiling requires all
three new criteria to reach 100.

**Distinction notes:**

- C-35 != C-13: C-13 requires the tier structure to be grounded in distinct failure dimensions
  and requires the rubric builder to run a predictability or divergence check; it is universally
  PARTIAL in Round 11 because all variations satisfy the grounding requirement but none produce
  the explicit check as a named step with structured output. C-35 requires the check to be
  operationalized -- a named step, a structured output format, and a STOP condition -- making
  the verification independently falsifiable by an evaluator who examines the construction
  record rather than re-deriving the distribution from the criteria themselves. A rubric whose
  tier design organically produces category diversity (satisfying C-13 partially) fails C-35
  if no named step captures and records the distribution; design intent and verification
  artifact are distinct requirements.

- C-36 != C-27: C-27 requires the competitor to be named before the positive definition and
  to constitute the gap specification -- the gap between the competitor and the required
  implementation is the criterion the mechanism closes. C-36 requires the competitor block to
  appear inline at the specific construction step where the wrong implementation would most
  naturally be produced, not in a preamble, operating-principles section, or shared block
  preceding all mechanism definitions. A rubric can satisfy C-27 (competitor correctly
  constitutes the gap spec, placed in a preamble section) while failing C-36 (the competitor
  is not at the construction step itself). V-05 demonstrates the pattern: the Unbounded-
  Aspirational competitor appears at Builder A, the Category-Menu competitor at the category-
  verification step, the Formula-Only Scorer competitor at the scoring-section step, and the
  Declared-Independent Map competitor at the Mechanism Definer step -- each competitor is the
  first thing the builder encounters before committing to the design decision that would produce
  the wrong implementation.

- C-36 != C-11: C-11 requires a structured Reference / Passes / Fails anchor using a prior-
  version reference before the aspirational section is written; the three-field template names
  a reference that passes essential+recommended but falls short on a specific aspirational
  dimension. C-36 requires each mechanism's competitor to be placed inline at the construction
  step for that mechanism specifically -- the temporal anchor is per-mechanism and per-step,
  not per-section-opening. V-05's PARTIAL on C-11 traces precisely to the distinction: V-05's
  competitor at Builder A names the failure dimension but not the prior-version Reference /
  Passes / Fails format required by C-11; V-05 satisfies C-36 (construction-step placement)
  while failing C-11 (three-field prior-version format absent).

- C-37 != C-27: C-27 requires competitors to exist and to constitute gap specifications for
  the mechanisms they accompany; it is satisfied by naming any number of competitors as long
  as each is a genuine gap specification. C-37 requires the coverage to be complete with
  respect to novel-requirement aspirational criteria: for every aspirational criterion that
  introduces a construction requirement not present in the essential or recommended tiers,
  there is exactly one corresponding competitor, and no novel criterion lacks a competitor.
  A rubric with two well-formed competitors for two criteria (C-27 PASS) but two additional
  novel criteria with no competitors fails C-37. V-01--V-04 fail both C-27 and C-37 because
  no competitors are present; V-05 PASS on both because its four competitors cover the four
  novel aspirational criteria (C-31--C-34) exactly.

- C-37 != C-25: C-25 requires one-to-one assignment between mechanisms and criteria at the
  architecture level -- each mechanism closes exactly one criterion and each criterion is closed
  by exactly one mechanism. C-37 requires one-to-one correspondence between competitors and
  novel aspirational criteria in the gap-specification layer -- the specification layer and the
  criterion layer are in bijection. The two criteria operate at different abstraction levels:
  C-25 governs the mechanism architecture (what the output contains); C-37 governs the
  negative-space specification (what the builder reads before constructing). A rubric can
  satisfy C-25 (independent mechanism assignment) while failing C-37 (some novel criteria lack
  competitor gap specifications); a rubric can satisfy C-37 (one competitor per novel criterion)
  while failing C-25 (mechanisms are coupled).

- C-37 != C-36: C-36 requires each competitor to be placed at the correct construction step;
  C-37 requires the competitor set to be complete with respect to novel criteria. A rubric can
  satisfy C-36 (all present competitors are at their correct steps) while failing C-37 (some
  novel criteria have no competitor), and can satisfy C-37 (all novel criteria have a
  competitor) while failing C-36 (some competitors are in a preamble rather than at their
  construction steps).
...
## Aspirational Criteria (C-09--C-37) -- 10 points

Denominator: **/29**. Each criterion scores 1 (full pass), 0.5 (partial), or 0 (fail).
Score = (sum) / 29 x 10.
...
Aspirational: (C-09 through C-37) / 29 x 10                        max 10  (partials = 0.5)

Composite = Essential + Recommended + Aspirational                  max 100
Golden threshold: >= 90
```

**Scoring note:** The aspirational denominator shifts from `/26` to `/29` to reflect the
addition of C-35, C-36, and C-37.

*Round 11 ceiling recalibration.* A rubric that passes C-09--C-34 (the Round 11 ceiling,
twenty-six of twenty-nine) scores 26/29 x 10 = 8.966 on aspirational; with full
essential+recommended that yields 98.966 -- still golden, but the new ceiling requires C-35,
C-36, and C-37 to reach 100.

*V-05 aspirational recalibration under v12.* In Round 11, V-05 scored 25.0/26 x 10 = 9.615
on aspirational (C-09--C-10, C-12--C-26, C-29--C-34 all PASS = 24; C-11 PARTIAL + C-13
PARTIAL = 1.0 extra = 25.0). Under v12 scoring with /29 denominator: V-05 FAIL C-35 (no
divergence-check step), PASS C-36 (each competitor at its construction step), PASS C-37
(four competitors covering four novel criteria exactly). New V-05 aspirational: 27.0/29 x 10
= 9.31. The addition of C-35 as a universal FAIL preserves the ceiling gap while C-36 and
C-37 remain V-05-only PASSes, confirming V-05's ceiling leadership is structural.

*C-35 closes the operationalization gap in C-13.* C-13 requires tier-grounded design and an
explicit divergence check; all Round 11 variations satisfy the grounding component but none
operationalize the check as a named step with structured output. The universal PARTIAL on
C-13 across Round 11 confirms the pattern is systematic: organic diversity by design intent
is a necessary but not sufficient condition for C-13. C-35 isolates the missing half -- the
step, its output format, and its STOP condition -- making the verification independently
testable. A rubric satisfying C-35 cannot also fail the C-13 divergence-check requirement;
the two criteria are complementary, not redundant: C-13 is PARTIAL in Round 11 because its
divergence-check condition is unmet; C-35 captures that condition as a standalone testable
requirement with a structured output and STOP gate.

*C-36 captures the temporal dimension of competitor placement.* C-27 and C-28 establish that
competitors should exist and carry derivation instructions; neither captures when during the
construction workflow the competitor is encountered. C-36 requires the competitor to arrive
at the construction moment -- the builder's first encounter with the design decision is the
wrong-implementation competitor, not a preamble read minutes earlier. Temporal proximity is
the mechanism: a competitor in a preamble requires retention across multiple subsequent steps;
a competitor inline at the step requires no retention. V-05's partial satisfaction of C-11
and full satisfaction of C-36 distinguishes these dimensions precisely: construction-step
temporal placement is present (C-36 PASS); the three-field prior-version format is absent
(C-11 PARTIAL).

*C-37 completes the competitor-coverage triad with C-27 and C-36.* C-27 requires existence;
C-36 requires correct placement; C-37 requires complete coverage. A rubric satisfying all
three has a complete, correctly-placed competitor for every novel construction decision in
the aspirational tier, making the negative-space specification exhaustive and temporally
situated. V-05 demonstrates all three: four competitors exist (C-27), each at its construction
step (C-36), covering the four novel Round 10 aspirational criteria exactly (C-37). The
addition of C-37 closes the coverage requirement: future variations cannot satisfy C-27 and
C-36 with partial competitor sets and claim the full negative-space specification pattern.
