Looking at the R6 scorecard, three structural patterns emerge from the C-23 excellence gradient (variations passing at different fidelity levels) that go beyond what C-23 already captures:

1. **V-02**: Binding declaration includes an exhaustiveness scope-closure statement — "No other DEFAULTS fields are directly bound to BELIEF MAP columns." Declares the binding set is complete, not just what's in it.

2. **V-04 / V-05**: Binding declaration includes an explicit downstream access prohibition — names Phase 5 as the consuming step and explicitly forbids direct DEFAULTS access, closing the bypass path rather than only stating the correct path.

3. **V-05**: Binding declaration includes a meta-purpose statement — "This declaration makes the full DEFAULTS→BELIEF MAP data flow auditable from skill structure alone." Makes auditability intent declarative at the site of the declaration.

These become C-25, C-26, C-27. Denominator moves from 17 to 20.

---

# Rubric: draft-pitch — v7

**Change from v6**: Added C-25, C-26, C-27 from R6 excellence signals. Aspirational denominator updated from 17 to 20.
...
```
Score = (essential_pass/4)*60 + (recommended_pass/3)*30 + (aspirational_pass/20)*10
```
