Reading the two new patterns from the R3 scorecard:

1. **Mandatory fill-in opener** — C-15 is only structurally enforced when the stakeholder-risk section is an *active output production step* (fill-in table), not a static paragraph; role differentiation then derives from the model's own first output rather than from a late checklist
2. **Schema separation as constraint registry** — splitting constraints into FIELD SCHEMA and COVERAGE SCHEMA with Consequence columns creates a two-tier structure where every constraint has exactly one structural home and pre-write gate checkboxes cite schema rows by reference, making C-14 + C-16 co-satisfiable in one compact structure

These become C-17 and C-18.
...
**Version**: v4
**Date**: 2026-03-14
**Promoted from**: v3 + R3 excellence signals
...
- **10 Aspirational (C-09–C-18):** Explicit commit gate, artifact naming convention, checkbox-gate before transition, operational-consequence framing for invalid vocabulary, dedicated sections per aspiration, pervasive consequence framing across all constraints, stakeholder-risk-first opener, structural enforcement over prose elaboration, stakeholder-risk section as active fill-in output, schema separation as constraint registry
...
### Two changes from v2 → v3 (carried forward)

- **C-14 (Pervasive consequence framing)**: Generalizes C-12 — every enforced constraint carries its downstream failure mode, not just priority vocabulary
- **C-15 (Stakeholder-risk-first opener)**: Opening with role/risk enumeration produces organically differentiated assignments and closes the loop to the rationale template
- **C-16 (Structural enforcement over prose)**: Length-independence of R1 patterns implies the underlying mechanism is structural enforcement, not prose elaboration

### Two changes from v3 → v4

- **C-17 (Stakeholder-risk as active fill-in)**: Sharpens C-15 — a static WHO IS AT RISK paragraph is necessary but not sufficient. The section must be an active output production step: the model fills in a stakeholder-risk table as its first output, and all owner roles in the signal plan must trace back to rows in that table. R3 diagnosis: V-03 passed C-15 with a fill-in opener but failed C-16 because its consequence framing was in prose blocks rather than schema columns. V-05 resolved both by combining the fill-in opener with schema-column consequences.
- **C-18 (Schema separation as constraint registry)**: Identifies the compact two-tier structure that makes C-14 and C-16 simultaneously satisfiable — FIELD SCHEMA and COVERAGE SCHEMA, each with a Consequence column, give every constraint exactly one structural home; pre-write gate checkboxes cite schema rows by reference rather than restating rules. This eliminates the tension between pervasive consequence framing (C-14) and prose bloat: consequences live in table cells, not inline prose.

### Score ceiling effect

C-16 remains a dependent criterion: it passes if and only if every other criterion (C-01 through C-15, C-17, C-18) is enforced by a structural mechanism. C-17 and C-18 together describe the mechanism by which C-15 and C-14 become structurally co-satisfiable. A prompt that satisfies C-17 and C-18 will satisfy C-14, C-15, and C-16 as a structural consequence.
