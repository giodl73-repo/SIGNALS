Reading the four scorecards for patterns that are present in at least one variation's PASS evidence but not yet captured in C-01 through C-16:

**New patterns found:**

1. **Two-tier sentinel vocabulary** (V-01 + V-04) — `??` (unknown/open obligation) vs `--` (deliberate absence/closed decision) are distinct tokens; the distinction makes gap *type* machine-distinguishable, not just gap *presence*

2. **Pre-signal defense register** (V-04) — Before reading any artifact, commit a table of change dimensions alongside the current strategy's active defense arguments; each proposal must then name the defense it defeated; inertia linkage becomes evidence-based, not post-hoc

3. **Integer-enforcement gate language** (V-01 + V-03) — Beyond requiring the templated delta sentence (C-15), an explicit gate-failure rule names the banned forms: *"Writing 'a few', 'several', 'some', or any non-integer value is a gate failure"*

4. **Sequential phase-linked stop gates** (V-03) — Stop gates reference the next phase number explicitly (*"Do not advance to Phase N without passing this gate"*), creating a linked chain rather than isolated local checks

---

## Rubric — topic:plan (v4)

**5 essential / 3 recommended / 12 aspirational — 20 criteria total**
