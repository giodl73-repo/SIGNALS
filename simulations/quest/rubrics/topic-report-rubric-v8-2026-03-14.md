`topic-report-rubric-v8-2026-03-14.md` written.

**What changed from v7:**

| ID | Criterion | Excellence signal source |
|----|-----------|------------------------|
| **C-26** | Dual-chain co-presence at governed annotation slot | V-04 and V-05 showed C-23 + C-24 co-present at the same `[RULE]` position without conflict -- two independent recovery paths (register via G-9, rubric via C-21) at one slot is a composite property not implied by either criterion alone |
| **C-27** | Dual-branch three-layer closure | V-05 demonstrated that C-22 + C-25 together eliminate all cross-branch lookup across both execution paths -- the conjunctive system property is not implied by either header criterion in isolation |

**Formula:** `aspirational_pass/19 * 10` (was `/17`)

**R7 golden re-scores under v8:**

| Variation | v7 asp | C-26 | C-27 | v8 asp | v8 score |
|-----------|--------|------|------|--------|---------|
| V-05 Ceiling | 17/17 | YES | YES | 19/19 | **100.0** |
| V-04 C-23+C-24 | 16/17 | YES | NO | 17/19 | **98.9** |
| V-03 C-25 only | 15/17 | NO | YES | 16/19 | **98.4** |
| V-01 C-23 only | 15/17 | NO | NO | 15/19 | **97.9** |
| V-02 C-24 only | 15/17 | NO | NO | 15/19 | **97.9** |

The ordering is now fully resolved -- V-04 > V-03 > V-01 = V-02. The three-way tie at 98.8 in v7 is broken: dual-chain co-presence (C-26, annotation-level) outranks single-branch closure (C-27 without C-26), which outranks either annotation chain alone.
 and V-02 drop below V-03 under v8 -- dual-branch
closure (C-27) outweighs either annotation-level chain alone. All four prior criteria
(C-23, C-24, C-25 from v7; C-26, C-27 from v8) are live R8 discriminators. No R7
variation other than V-05 achieves 19/19.

**Design note on C-26:** C-23 and C-24 address different failure modes at the same
structural position. C-23 embeds the rubric criterion ID in the violation text so a model
tracing from bad output reaches the rubric invariant definition. C-24 links the annotation
label to the contract register so a model tracing from the annotation reaches the register
obligation. When both are co-present -- annotation label is `[RULE G-9 INERTIA]` (C-24
path) and violation text reads `(no stall cost -- C-21 violation)` (C-23 path) -- the
same slot serves two independent recovery chains. Round 7 finding: V-04 and V-05
demonstrate co-presence without structural conflict; V-01 (C-23 only) and V-02 (C-24
only) isolate each chain independently, confirming they are separable but jointly
stronger.

**Design note on C-27:** C-22 creates a named recovery point for the default execution
path; C-25 creates an equivalent named recovery point for the `--format teams` execution
path. When both are present, the spec has no execution path that requires cross-branch or
cross-spec recovery -- both paths are locally self-contained. The property is conjunctive,
not additive: either header alone leaves one path exposed. Round 7 finding: V-05 is the
only R7 design achieving dual closure; V-03 satisfies C-25 and passes C-22 via the
neutral base, making it the minimal R7 design to achieve C-27 without also achieving
C-26.
...
aspirational_pass = count of C-09..C-27 that pass   (max 19)
...
- Under v8 the R7 goldens score differently from v7: V-05 retains 19/19 = 100.0; V-04
  rises to 17/19 = 98.9 (gains C-26 from C-23 + C-24 co-presence); V-03 rises to 16/19 =
  98.4 (gains C-27 from C-22 + C-25 dual closure); V-01 and V-02 drop to 15/19 = 97.9
  (gain neither composite criterion). C-26 and C-27 are live discriminators for Round 8.

---

## R8 Discriminator Candidates

| ID | Design question | V-isolation needed |
|----|-----------------|-------------------|
| C-26 vs C-23 + C-24 individually | Does dual-chain co-presence at SLOT 4 reduce live-run criterion confusion below the rate achievable by either chain alone? | V-A: C-23 only (rubric path); V-B: C-24 only (register path); V-C: C-23 + C-24 co-present (C-26) |
| C-27 vs C-22 + C-25 individually | Does dual-branch closure reduce Teams card layer-omission rate below the rate achievable by either header alone? | V-D: C-22 main header only; V-E: C-25 Branch B header only; V-F: C-22 + C-25 both present (C-27) |
| Recovery directive in Branch B header | Does an explicit recovery note within the Branch B header -- e.g., "re-read this header to reconstruct the layer sequence without referencing Branch A" -- reduce cross-branch attention loss beyond C-25 structural self-containment alone? | V-G: C-25 without recovery directive; V-H: C-25 with explicit recovery directive |
| Register-to-rubric trace link in violation | Does naming both the register label and the rubric criterion ID in the violation text -- e.g., `(no stall cost -- G-9 INERTIA / C-21 violation)` -- reduce ambiguity beyond C-26, which requires co-presence at the annotation but does not require the violation to carry both identifiers simultaneously? | V-I: C-26 with single identifier in violation; V-J: violation carries both register label and rubric ID |
