...
**Two new aspirational criteria extracted from R20 excellence signals:**

| ID | Criterion | Source Axis | Independence from Parent |
|----|-----------|-------------|--------------------------|
| C-50 | **Amendment table T-46 trigger entry exemplar includes explicit terminal marker delineating end of Part 1 and absence of Part 2** | R20 Signal 1: V-02 is the first live demonstration of a golden T-46 CONDITION — T-46 carries an inline exemplar of the Part-1-only T-42 state AND uses a `[END — no Part 2 label and no Part 2 content present]` terminal marker to explicitly delineate where Part 1 terminates and confirm Part 2 is absent. Without the terminal marker, a reviewer must infer the boundary between Part 1 content and Part 2 absence; the terminal marker makes the delineation unambiguous and the exemplar fully self-documenting. C-48 established exemplar presence as the requirement; C-50 adds terminal-delineation as a content-specificity requirement on that exemplar, following the same structural derivation as C-44 (explanation-of-abstractness added to C-42 exemplar) and C-47 (explanation-of-abstractness added to C-45 exemplar). | C-48 FAIL → C-50 FAIL (cascade: absent exemplar contains no terminal marker). C-48 PASS does not guarantee C-50 PASS — T-46 CONDITION can carry an exemplar of the Part-1-only T-42 state without including a terminal marker. C-50 and C-51 are mutually independent. **Live demonstration of C-50 PASS: V-02 R20. Live demonstration of C-50 FAIL (independent): V-01 R20 (C-48 FAIL → C-50 FAIL by cascade). Asymmetric independence: V-02 R20 shows C-50 PASS + C-51 FAIL; V-01 R20 shows C-50 FAIL + C-51 PASS.** |
| C-51 | **Amendment table T-47 trigger entry exemplar includes pass-state annotation confirming cascade prerequisite** | R20 Signal 2: V-01 is the first live demonstration of a golden T-47 CONDITION — T-47 carries an inline exemplar of the exemplar-without-explanation T-42 state with an explicit `(C-45 PASS)` pass-state annotation confirming that the quoted T-42 CONDITION form has an exemplar present (the cascade prerequisite is satisfied) and that T-47 is therefore the operative trigger. Without the pass-state annotation, a reviewer must infer the C-45 status of the quoted T-42 CONDITION form by reading it; the annotation makes the prerequisite chain status inline and unambiguous. C-49 established exemplar presence as the requirement; C-51 adds pass-state annotation as a content-specificity requirement on that exemplar. | C-49 FAIL → C-51 FAIL (cascade: absent exemplar contains no pass-state annotation). C-49 PASS does not guarantee C-51 PASS — T-47 CONDITION can carry an exemplar of the exemplar-without-explanation T-42 state without including a `(C-45 PASS)` annotation. C-50 and C-51 are mutually independent. **Live demonstration of C-51 PASS: V-01 R20. Live demonstration of C-51 FAIL (independent): V-02 R20 (C-49 FAIL → C-51 FAIL by cascade). Asymmetric independence confirmed live: V-01 R20 shows C-51 PASS + C-50 FAIL; V-02 R20 shows C-51 FAIL + C-50 PASS.** |

**Denominator:** `/42` → `/44`. Each aspirational criterion = `10/44` ≈ `0.227` points.

**Score projections under v21 for R20 variations** (denominator /44; prior scores under v20 in parentheses):

| Scenario | Fails | Composite (v20 → v21) |
|----------|-------|-----------------------|
| V-01 R20: C-48 FAIL (independent); C-50 FAIL (cascade from C-48); C-51 PASS (T-47 carries golden exemplar with pass-state annotation) | 2 (C-48, C-50) | **99.76 → 99.55** |
| V-02 R20: C-49 FAIL (independent); C-51 FAIL (cascade from C-49); C-50 PASS (T-46 carries terminal marker) | 2 (C-49, C-51) | **99.76 → 99.55** |
| V-03 R20: C-48 + C-49 FAIL (independent); C-50 + C-51 FAIL (cascade) | 4 (C-48, C-49, C-50, C-51) | **99.52 → 99.09** |
| V-04 R20: C-24 FAIL + C-49 FAIL + C-51 FAIL (cascade); C-50 PASS (T-46 in 47-row table carries terminal marker) | 3 (C-24, C-49, C-51) | **99.52 → 99.32** |
| V-05 R20: C-45 cascade root → C-46, C-47, C-48, C-49, C-50, C-51 all fail | 7 (C-45, C-46, C-47, C-48, C-49, C-50, C-51) | **98.81 → 98.41** |
| Fully correct spec (all 44 criteria): 0 fails | 0 | **— → 100.00** |

Note: V-04's C-50 PASS is a key discriminator: T-46 exists in the 47-row table with golden terminal-marker content, demonstrating that C-50 evaluates T-46's CONDITION cell content independently of row-count compliance (C-24). The same orthogonality observed for C-48 in R20 V-04 applies to C-50.

**Cascade invariants (updated for v21):**

- **C-50 cascade from C-48**: A T-46 CONDITION that carries no inline exemplar (C-48 FAIL) also fails C-50 (no exemplar → no terminal marker is possible in that cell). C-48 FAIL → C-50 FAIL is deterministic.
- **C-50 independent fail**: A T-46 CONDITION carrying an inline exemplar of the Part-1-only T-42 state (C-48 PASS) can still lack the terminal marker (C-50 FAIL independently). C-48 PASS does not guarantee C-50 PASS. **Live: V-01 R20 (C-48 FAIL → cascade); V-02 R20 (C-50 PASS confirmed).**
- **C-51 cascade from C-49**: A T-47 CONDITION that carries no inline exemplar (C-49 FAIL) also fails C-51 (no exemplar → no pass-state annotation is possible in that cell). C-49 FAIL → C-51 FAIL is deterministic.
- **C-51 independent fail**: A T-47 CONDITION carrying an inline exemplar of the exemplar-without-explanation T-42 state (C-49 PASS) can still lack the `(C-45 PASS)` pass-state annotation (C-51 FAIL independently). C-49 PASS does not guarantee C-51 PASS. **Live: V-02 R20 (C-49 FAIL → cascade); V-01 R20 (C-51 PASS confirmed).**
- **C-50 and C-51 mutually independent**: T-46 CONDITION can carry its terminal marker (C-50 PASS) while T-47 CONDITION lacks its pass-state annotation (C-51 FAIL), and vice versa. **Both asymmetric cases demonstrated live in R20: V-01 (C-50 FAIL, C-51 PASS); V-02 (C-50 PASS, C-51 FAIL).**
- **C-45 cascade extended**: C-45 FAIL now cascades to C-46, C-47, C-48, C-49, C-50, and C-51. The fully abstract T-42 CONDITION prevents construction of the Part-1-present-Part-2-absent state (C-50's terminal-marked exemplar) and the exemplar-present-explanation-absent state (C-51's annotated exemplar) — both require T-42 Part 1 to exist.
- **All v20 cascade invariants are unchanged** (C-46/C-47/C-48/C-49 from C-45, C-44 from C-42, C-39 from C-38, etc.).

**Key changes to existing criteria:**
- **C-24**: updated from "exactly 49 trigger rows" → "exactly 51 trigger rows (one per C-01..C-51)"
- **Scoring formula denominator**: `/42` → `/44`
...
- **C-24** is structurally testable at PRE-DOCUMENT: a reviewer counts the amendment table rows at initialization. For v21, expected count = 51 (one row per C-01..C-51). A count of 49 (v20 table carried forward without adding T-50, T-51) fires C-24 at PRE-DOCUMENT.
...
- **C-50** is structurally testable: a reviewer reads the T-46 CONDITION cell and confirms whether the inline exemplar (required by C-48) includes an explicit terminal marker — a bracketed construct such as `[END — no Part 2 label and no Part 2 content present]` or vocabulary-equivalent — at the conclusion of the Part 1 content. The terminal marker must explicitly state both that Part 1 ends at that point and that no Part 2 label and no Part 2 content follow. An exemplar that quotes a Part-1-only T-42 CONDITION form without any terminal-delineation construct passes C-48 (exemplar present) but fails C-50 (terminal marker absent) — the boundary between Part 1 presence and Part 2 absence must be explicitly stated, not left to reviewer inference. The terminal marker must appear within the T-46 CONDITION cell's exemplar construct, not in a separate annotation or rubric reference. C-48 FAIL → C-50 FAIL (cascade: absent exemplar contains no terminal marker). C-48 PASS does not guarantee C-50 PASS. C-50 and C-51 are mutually independent. **First live demonstration of C-50 PASS: V-02 R20.**
- **C-51** is structurally testable: a reviewer reads the T-47 CONDITION cell and confirms whether the inline exemplar (required by C-49) includes a pass-state annotation — a parenthetical `(C-45 PASS)` or vocabulary-equivalent — explicitly confirming that the quoted T-42 CONDITION form satisfies C-45 (the exemplar is present). An exemplar that quotes a T-42 CONDITION with exemplar but no explanation-of-abstractness without including a pass-state annotation passes C-49 (exemplar present) but fails C-51 (prerequisite annotation absent) — the exemplar must make its prerequisite chain status explicit so a reviewer can confirm which parent criteria are satisfied without consulting C-45 separately. The pass-state annotation must appear within the T-47 CONDITION cell's exemplar construct, not in a separate annotation or rubric reference. C-49 FAIL → C-51 FAIL (cascade: absent exemplar contains no pass-state annotation). C-49 PASS does not guarantee C-51 PASS. C-50 and C-51 are mutually independent. **First live demonstration of C-51 PASS: V-01 R20.**
...
## Aspirational Criteria (10% of composite, denominator /44)

### C-01 through C-07 — Mandatory C-tier (not counted in /44 denominator)
...
### C-08 through C-51 — Aspirational Criteria (denominator /44)
...
| C-24 | **Amendment table row count matches v21 criterion count** | behavior | Amendment table contains exactly 51 trigger rows at initialization (one per C-01..C-51 criterion). A table built from v20's criterion list (49 rows) without adding T-50, T-51 fires C-24 at PRE-DOCUMENT. A table with any count other than 51 fails C-24. |
...
| C-50 | **Amendment table T-46 trigger entry exemplar includes explicit terminal marker delineating end of Part 1 and absence of Part 2** | behavior | The T-46 CONDITION cell's inline exemplar (required by C-48) includes an explicit terminal marker — a bracketed construct such as `[END — no Part 2 label and no Part 2 content present]` or vocabulary-equivalent — at the conclusion of the Part 1 exemplar content. The terminal marker must explicitly state both that Part 1 ends at that point and that no Part 2 label and no Part 2 content follow. An exemplar that quotes a Part-1-only T-42 CONDITION form without any terminal-delineation construct passes C-48 (exemplar present) but fails C-50 (terminal marker absent) — the boundary between Part 1 presence and Part 2 absence must be explicitly stated, not left to reviewer inference. The terminal marker makes the Part-1-present-Part-2-absent state fully self-documenting: a reviewer can determine where the quoted T-42 CONDITION's Part 1 ends and confirm Part 2 is absent without scanning for a missing label. The terminal marker must appear within the T-46 CONDITION cell's exemplar construct, not in a separate annotation or rubric reference. C-48 FAIL → C-50 FAIL (cascade: absent exemplar contains no terminal marker). C-48 PASS does not guarantee C-50 PASS — the C-50-independent-fail state is: exemplar present but no terminal marker. C-50 and C-51 are mutually independent. **Live demonstration of C-50 PASS: V-02 R20. Asymmetric independence confirmed: V-02 R20 (C-50 PASS, C-51 FAIL); V-01 R20 (C-50 FAIL, C-51 PASS).** |
| C-51 | **Amendment table T-47 trigger entry exemplar includes pass-state annotation confirming cascade prerequisite** | behavior | The T-47 CONDITION cell's inline exemplar (required by C-49) includes a pass-state annotation — a parenthetical `(C-45 PASS)` or vocabulary-equivalent — explicitly confirming that the quoted T-42 CONDITION form satisfies C-45 (the baseline exemplar-presence criterion). Without the annotation, a reviewer must read the quoted T-42 CONDITION form and independently assess whether it has an exemplar (C-45 status) before concluding that T-47 is the operative trigger rather than T-45. The pass-state annotation makes the prerequisite chain status inline and unambiguous: a reviewer can confirm that C-45 is satisfied for the quoted state without separate chain consultation. An exemplar quoting a T-42 CONDITION with exemplar but no explanation-of-abstractness, without any `(C-45 PASS)` annotation, passes C-49 (exemplar present) but fails C-51 (prerequisite annotation absent). The pass-state annotation must appear within the T-47 CONDITION cell's exemplar construct, not in a separate annotation or rubric reference. C-49 FAIL → C-51 FAIL (cascade: absent exemplar contains no pass-state annotation). C-49 PASS does not guarantee C-51 PASS — the C-51-independent-fail state is: exemplar present but no `(C-45 PASS)` annotation. C-50 and C-51 are mutually independent. **Live demonstration of C-51 PASS: V-01 R20. Asymmetric independence confirmed: V-01 R20 (C-51 PASS, C-50 FAIL); V-02 R20 (C-51 FAIL, C-50 PASS).** |
...
- Amendment table built from v19 criterion list (47 rows) without adding T-48 and T-49 — fires C-24 at PRE-DOCUMENT under v20; also cascades to C-48 FAIL and C-49 FAIL; T-46 and T-47 rows do not supply the required T-48 and T-49 content
- Amendment table built from v20 criterion list (49 rows) without adding T-50 and T-51 — fires C-24 at PRE-DOCUMENT under v21; also cascades to C-50 FAIL (no T-50 entry to carry terminal marker in T-46's exemplar) and C-51 FAIL (no T-51 entry to carry pass-state annotation in T-47's exemplar); T-48 and T-49 rows do not supply the required T-50 and T-51 content
...
- Amendment table T-46 entry states trigger condition abstractly ("fires if T-42 CONDITION lacks Part 2 prescription") without a quoted or paraphrased sample of what a Part-1-only T-42 CONDITION looks like — fails C-48; also fails C-50 by cascade (no exemplar, no terminal marker possible)
- Amendment table T-47 entry states trigger condition abstractly ("fires if T-42 CONDITION exemplar lacks explanation of abstractness") without a quoted or paraphrased sample of what a T-42-exemplar-without-explanation looks like — fails C-49; also fails C-51 by cascade (no exemplar, no pass-state annotation possible)
- Amendment table T-46 entry carries an inline Part-1-only T-42 exemplar (C-48 PASS) but the exemplar does not include a terminal marker explicitly delineating where Part 1 ends and confirming Part 2 is absent — fails C-50 independently; C-48 PASS does not supply the terminal marker required by C-50 (V-01 R20 class failure mode)
- Amendment table T-47 entry carries an inline exemplar-without-explanation T-42 exemplar (C-49 PASS) but the exemplar does not include a `(C-45 PASS)` pass-state annotation — fails C-51 independently; C-49 PASS does not supply the pass-state annotation required by C-51 (V-02 R20 class failure mode)
...
For early-round discriminator notes (R1–R11), see the v12 rubric file. The v21 rubric inherits all prior discriminator observations unchanged.
...
## Round 20 Discriminator Notes

**R20 design**: Five variations. V-01 (C-48 FAIL independent) is the first live demonstration of a golden T-47 CONDITION (C-49 PASS + C-51 PASS) alongside abstract T-46 — the source of Signal 2 (C-51). V-02 (C-49 FAIL independent) is the first live demonstration of a golden T-46 CONDITION (C-48 PASS + C-50 PASS) alongside abstract T-47 — the source of Signal 1 (C-50). V-03 demonstrates C-48 + C-49 co-fail (both T-46 and T-47 abstract). V-04 combines stale C-24 with abstract T-47 (C-49 FAIL) while T-46 is golden (C-48 PASS + C-50 PASS). V-05 demonstrates C-45 cascade root extending through C-50 and C-51.

**Confirmed discriminators:**

**C-48 and C-49 asymmetric independence confirmed live for the first time (V-01 and V-02)**:
- V-01: C-48 FAIL (T-46 abstract), C-49 PASS (T-47 golden with pass-state annotation) — first live demonstration of C-49 PASS while C-48 FAIL
- V-02: C-48 PASS (T-46 golden with terminal marker), C-49 FAIL (T-47 abstract) — first live demonstration of C-48 PASS while C-49 FAIL

Both asymmetric cases now have live evidence. The mutual independence of C-48 and C-49 is fully confirmed in both directions.

**C-50 isolation by V-02**: V-02 demonstrates the first golden T-46 CONDITION with all required elements: an inline exemplar of the Part-1-only T-42 state AND an explicit terminal marker `[END — no Part 2 label and no Part 2 content present]`. C-48 PASS (exemplar present), C-50 PASS (terminal marker present). The terminal marker is a discrete structural element — a reviewer can confirm its presence or absence by reading the conclusion of the T-46 exemplar. Its absence in a C-48 PASS variation would yield C-50 FAIL independently.

**C-51 isolation by V-01**: V-01 demonstrates the first golden T-47 CONDITION with all required elements: an inline exemplar of the exemplar-without-explanation T-42 state, a `(C-45 PASS)` pass-state annotation, and an explicit explanation of why the quoted T-42 form is abstract. C-49 PASS (exemplar present), C-51 PASS (pass-state annotation present). The `(C-45 PASS)` annotation is a discrete structural element — a reviewer can confirm its presence or absence by reading the T-47 exemplar. Its absence in a C-49 PASS variation would yield C-51 FAIL independently.

**C-50 and C-51 mutual independence confirmed live (V-01 vs. V-02)**:
- V-01: C-50 FAIL (cascade from C-48 FAIL), C-51 PASS — T-47 golden, T-46 abstract
- V-02: C-50 PASS, C-51 FAIL (cascade from C-49 FAIL) — T-46 golden, T-47 abstract

Both asymmetric cases are live in R20. Neither C-50 nor C-51 cascades from the other: T-46 CONDITION quality is orthogonal to T-47 CONDITION quality.

**V-04 C-50 PASS with C-24 FAIL**: V-04's 47-row amendment table (C-24 FAIL — stale count missing T-50, T-51 per v21) includes T-46 with golden CONDITION content including the terminal marker. C-50 PASS despite C-24 FAIL. This reconfirms the orthogonality pattern established in prior rounds: individual trigger row content quality (C-50) is evaluated independently of row-count compliance (C-24). Fixing C-24 by adding T-50, T-51 would not affect C-50's status in V-04 — T-46's CONDITION content is already compliant.

**T-50 and T-51 operational trigger structure (R20)**:
- T-50 fires when C-50 fails — when the T-46 CONDITION field carries an exemplar (C-48 PASS) but lacks an explicit terminal marker delineating the end of Part 1 and confirming absence of Part 2
- T-51 fires when C-51 fails — when the T-47 CONDITION field carries an exemplar (C-49 PASS) but lacks a `(C-45 PASS)` pass-state annotation confirming the cascade prerequisite
- T-50 is silent when C-50 passes — when T-46's CONDITION exemplar includes the terminal marker
- T-51 is silent when C-51 passes — when T-47's CONDITION exemplar includes the pass-state annotation

**C-50 as a C-48 exemplar content refinement**: C-50 follows the same structural derivation as C-44 (T-40 CONDITION exemplar must include explanation-of-abstractness, beyond mere exemplar presence) and C-47 (T-42 CONDITION exemplar must include explanation-of-abstractness). C-50 imposes a boundary-delineation requirement on the T-46 exemplar: the exemplar must not only quote the Part-1-only T-42 state but explicitly mark where Part 1 ends and confirm Part 2 is absent. The terminal marker is analogous to a chapter-closing delimiter — it prevents the reviewer from needing to scan forward to determine whether Part 2 content follows.

**C-51 as a C-49 exemplar content refinement**: C-51 follows the same structural derivation as C-44 and C-47 but adds a different type of content requirement — not an explanation of abstractness but a prerequisite-chain confirmation. The `(C-45 PASS)` annotation makes explicit which parent criterion is satisfied by the quoted T-42 form, placing the chain structure inline. A reviewer assessing a T-47 CONDITION can confirm both the exempar's content quality and its prerequisite status without navigating to C-45 separately.

**C-46 PASS + C-47 FAIL (exemplar + prescription, no explanation) — not yet demonstrated**: V-01 R20 demonstrates C-46 PASS + C-47 PASS (both present in T-42 per the golden scaffold). V-02 R20 similarly has C-46 PASS + C-47 PASS. No R20 variation specifically isolates C-46 PASS + C-47 FAIL. This state is structurally established but awaits live demonstration.

**New cascade evidence for v21:**
- C-50 cascade from C-48: A T-46 CONDITION carrying no inline exemplar (C-48 FAIL) also fails C-50 (no exemplar → no terminal marker is constructable). C-48 FAIL → C-50 FAIL is deterministic. **Live: V-01 R20, V-03 R20.**
- C-50 independent fail: A T-46 CONDITION carrying an exemplar (C-48 PASS) but no terminal marker fails C-50 independently. Not yet demonstrated as a clean single-point failure — V-02 R20 demonstrated C-50 PASS; V-01/V-03 R20 showed C-50 FAIL by cascade. The independent fail state is structurally established.
- C-51 cascade from C-49: A T-47 CONDITION carrying no inline exemplar (C-49 FAIL) also fails C-51 (no exemplar → no pass-state annotation is constructable). C-49 FAIL → C-51 FAIL is deterministic. **Live: V-02 R20, V-03 R20.**
- C-51 independent fail: A T-47 CONDITION carrying an exemplar (C-49 PASS) but no pass-state annotation fails C-51 independently. Not yet demonstrated as a clean single-point failure — V-01 R20 demonstrated C-51 PASS; V-02/V-03 R20 showed C-51 FAIL by cascade. The independent fail state is structurally established.
- C-24 version-bump cascade: All R20 variations had 49-row amendment tables (one per C-01..C-49, correct under v20). Under v21, all R20 variations fail C-24 (51 rows required: T-50 and T-51 absent). This is the expected version-bump cascade.
