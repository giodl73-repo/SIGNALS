Reading the R5 scorecard patterns, two new criteria emerge:

**Pattern D (C-19 V-05 vs V-01/V-04 PARTIAL):** V-05 wins by declaring GATE-0 with both conditions: the unconditional phase is labeled AND GATE-1 is explicitly named as the first conditional operation. V-01/V-04 named the unconditional phase but omitted the successor — PARTIAL. The operative gap: *unconditional label alone vs. unconditional label + named conditional successor*.

**Pattern E (C-20 V-04 PARTIAL):** V-04 uses `ROOT verdict` as the stability component header. A naive reader who doesn't know ROOT = inertia anchor cannot identify it as the stability verdict. C-20 requires four components by name; this pattern requires those names to be domain-neutral (rubric-standard) — domain-specific renamings of required components fail the naive-reader consultability test.

New criteria: **C-21** (pre-phase names both its unconditional nature and its conditional successor), **C-22** (AMEND schema component names are domain-neutral, passing naive-reader consultability without domain knowledge).

Aspirational tier: 12 → 14 criteria, 10/14 = 0.714 pts each.

---

```markdown
# discover-competitors-alt Rubric — v6
...
## Aspirational Criteria (weight = 10 points total, 0.714 points each)
...
| Aspirational | C-09 -- C-22 | 0.714 | 10 |
| **Max composite** | | | **100** |
...
- **C-21** closes the gap C-19 exposed in R5: labeling the unconditional phase is necessary but not sufficient — the conditional successor must also be named. Both conditions together constitute an architecturally declared boundary.
- **C-22** closes the gap C-20 exposed in R5: four-component enumeration is necessary but not sufficient — the names themselves must be domain-neutral so any reader can match them to their rubric-standard roles without domain fluency. Dependency chain: C-22 requires C-20, C-21 requires C-19.
