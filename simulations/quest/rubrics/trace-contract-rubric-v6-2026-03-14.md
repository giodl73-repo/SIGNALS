`trace-contract-rubric-v6-2026-03-14.md` written.

Summary of what changed:

**Formula:** `A/16 × 10` → `A/20 × 10`. Per-criterion weight ~0.625 → ~0.5. Ceiling unchanged.

**Four new aspirational criteria:**

| ID | Name | Source | Key distinction |
|----|------|---------|-----------------|
| C-25 | Phase 0 scope enumeration creates verifiable prior commitment | V-01/V-04/V-05 R5 | C-06 tests completeness declared within Phase 1; C-25 tests completeness verifiable against a prior named artifact. GATE 0 fires first; C-19 must extend to `gate0_scope_confirmed`. |
| C-26 | Pre-populated skeleton separates coverage commitment from value derivation | V-02/V-04/V-05 R5 | Step 1 builds index, Step 2 fills values. Omission = blank cell in a committed row, not a silently absent row. Independent of C-25 — each can hold without the other. |
| C-27 | Challenge log converts GATE 1 isolation attestation to two-reader record | V-03/V-05 R5 | C-14 tests that the attestation is confirmable; C-27 tests that a second reader's challenge log backs it. Self-certified confirmable checkboxes pass C-14 but fail C-27. |
| C-28 | CONTRACT DELTA includes required amendment type taxonomy column | V-05 R5 | C-22 adds a severity-axis priority column; C-28 adds an orthogonal change-kind axis (add-field / remove-field / change-type / change-enum / change-error-shape / change-auth-flow). Both columns required; blank cells are gate failures. |

The distinction ladder in Design Notes gains three new rungs: `C-12/C-14/C-23 → C-27`, `C-10 → C-18 → C-22 → C-28`, and `C-06 → C-25 → C-26`.
red sub-field** | essential | depth | Each finding includes a named root cause hypothesis as a required sub-field in the finding scaffold. A finding that states a mismatch without a root cause hypothesis does not pass. "Unknown" alone does not pass; the hypothesis must name a candidate mechanism. |
