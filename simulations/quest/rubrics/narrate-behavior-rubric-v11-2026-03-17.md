...
- Aspirational: cap 10 pts (any 14 of 28 criteria earn full cap; each criterion +1 pass toward threshold)
...
## Aspirational Criteria (10 points total, capped; any 14 of 28 earns the cap)
...
- **Source pattern**: risk-density-matrix (R9 SIGNAL-2; V-42); confirmed R10 (V-01, V-04, V-05)
...
- **Source pattern**: lifecycle-phase-structure (R9 SIGNAL-3; V-44); confirmed R10 (V-02, V-04, V-05)
...
- **Source pattern**: boundary-risk-density-score (R9 SIGNAL-4; V-45); confirmed R10 (V-03, V-04, V-05)

### C-35 -- Per-Form Discovery-Ratio Statement Text in DEFINITIONS
- **Weight**: aspirational
- **Category**: depth
- **Text**: DEFINITIONS contains three separately labeled ratio-form blocks -- one per
  discovery-pathway-ratio direction -- each spelling out the complete required meta-finding
  statement text in full prose. The three blocks are: (a) inertia-boundary > contract-
  boundary: full statement text declaring what this ratio signals about implicit-assumption
  surface breadth; (b) contract-boundary > inertia-boundary: full statement text declaring
  what this ratio signals about declared-contract coverage dominance; (c) equal counts: full
  statement text declaring what balance signals about neither surface dominating. Each block
  is independently readable without decoding external code references. This is the positive
  form of the C-23 condensed-DEFINITIONS failure pattern confirmed by V-43: rather than
  referencing (a)/(b)/(c) as shorthand codes, the variation spells out each form's statement
  so that an executor reading DEFINITIONS alone can emit the correct meta-finding without
  consulting any external definition. C-35 strengthens C-23: a variation can satisfy C-23
  (unconditional REQUIRE present) while failing C-35 (statement text condensed to codes).
  A variation satisfying C-35 automatically satisfies the DEFINITIONS portion of C-23.
- **Pass condition**: DEFINITIONS contains three separately labeled blocks for the three
  ratio forms, each including the complete required meta-finding statement text in full
  prose (not a code reference). All three forms must be present and each must state the
  meta-finding text that an executor should emit. Fail if any form is absent from
  DEFINITIONS. Fail if any form's required statement text is condensed to a code reference
  (e.g., "emit (a)" without the statement text). Partial pass (half credit) if all three
  forms are present and labeled but one form's statement text is abbreviated rather than
  fully spelled out.
- **Source pattern**: per-form-discovery-ratio-statements (R10 SIGNAL-1; V-05)
...
| v11     | C-01 -- C-35  | C-01--C-04 | C-05--C-07  | C-08--C-35 (cap 10; any 14 of 28)   | +C-35 from R10 excellence signal; C-32/C-33/C-34 confirmed by all 5 R10 variations (V-01--V-05); per-form-discovery-ratio-statements codified as positive form of C-23 condensed-DEFINITIONS failure; V-05 first variation to satisfy all 27 prior aspirational criteria (27/27 golden) |
