...
`.craft/roles/{area}/{role}.md` files with five required fields per role. The rubric tests both
...
| C-02 | **All five role fields present in every .craft/roles/ file** | correctness | Every `.craft/roles/{area}/{role}.md` file contains all five fields: `orientation`, `lens`, `expertise`, `scope`, `collaborates_with`. Any role file missing one or more fields fails this criterion. |
| C-03 | **inertia-advocate role always present** | correctness | A role file whose name or title contains `inertia` or `advocate` (case-insensitive) exists under `.craft/roles/`. The inertia-advocate is the devil's-advocate role whose job is to challenge every proposal with why the status quo is sufficient. Absence fails unconditionally. |
...
| ID   | Text | Category | Pass Condition |
|------|------|----------|----------------|
| C-06 | **Role files grouped by area in subdirectories** | format | `.craft/roles/` contains at minimum two area subdirectories (e.g., `.craft/roles/engineering/`, `.craft/roles/operations/`). All roles dumped flat into `.craft/roles/` with no area grouping fails. |
...
[ ] C-02  every .craft/roles/ file has all 5 fields
