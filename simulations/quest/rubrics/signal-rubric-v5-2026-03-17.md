Done. `signal-rubric-v5-2026-03-17.md` is written. Here's a summary of what changed:
...
**Retroactive R4 leaderboard under v5:** V-05 still 100; V-04 drops to 99.09 (misses C-17); V-01/V-02/V-03 each drop to 98.18 (each satisfies only one of the three new axes). The differentiation structure is clean -- R5's target is a single variation that combines all three.

**Evaluation notes added** for C-17 (prompt-design, mark N/A if prompt unavailable), C-18 (C-14 fail auto-fails C-18; can infer from 61-line count), C-19 (prompt-design, refinement of C-05).
4 quest + 3 mock + 4 org = 61). If not -- a
  sub-skill is missing or duplicated. Restart." Format-only gates catch structural violations
  but allow silent skill omission. Count gate closes that gap.
- Pass condition: Bare compliance gate includes both (1) format check and (2) exact 61-line
  count with per-namespace breakdown. Format-only gate with restart = C-14 pass, C-18
  partial. No gate at all = C-14 fail (auto C-18 fail).

**C-19 -- PARSE MODE enumerates all 12 canonical namespace names** (aspirational)
- Source: R4 V-03 only. PARSE MODE lists all 12 canonical names explicitly so unknown-
  namespace arguments fall back to FULL by explicit rule, not by model inference. Standard
  `anything else -> FULL` routing works but requires the model to infer what "anything else"
  means for a near-miss (e.g. `/signal flows`). Explicit enumeration removes that inference.
- Pass condition: PARSE MODE enumerates all 12 canonical namespace names. Unknown-namespace
  argument falls back to FULL deterministically. Partial: routes unknown to FULL via
  `anything else` with no enumeration. Fail: no explicit fallback rule.

**Formula change:** aspirational denominator bumps from `/8` to `/11`. Max composite stays 100.
...
three axes simultaneously; each axis was isolated in a single-axis variation (V-01, V-02,
V-03 respectively). R3 V-05 satisfies none of them.
...
V-05 uniquely satisfies all three axes. R5 target: a single variation that sources all three
structural patterns simultaneously and establishes dual convergence under v5.
...
- **Source**: Round 1 excellence signal. V-04 scored full on C-07; V-03 scored partial
  because footers used a generic `<domain>` placeholder that the model had to infer.
- **Pass condition**: Each dispatch footer substitutes the generic slot with a noun
  ...
  "flow scenario" for flow). A footer retaining a literal `<domain>` or `<X>` token
  fails. Footers using the same noun across all 12 namespaces are also a soft fail.
  At least 10 of 12 namespaces must have distinct, namespace-appropriate nouns to pass.
...
- **Source**: Round 3 excellence signal. V-05 added an "example pad" column to the C-15
  ...
  required (e.g. "/scout-naming (13) needs 7 spaces"). Model copies a pattern, not
  arithmetic. V-04 has the table plus a single worked example in a separate section --
  C-15 pass, C-16 fail.
...
- **Source**: Round 4 excellence signal. R4 V-02 upgraded the C-14 restart gate to
  include: "Does your output contain exactly 61 lines? (8 scout + 4 draft + 4 review +
  7 flow + 7 trace + 9 prove + 3 listen + 2 program + 6 topic + 4 quest + 3 mock + 4
  org = 61). If not -- a sub-skill is missing or duplicated. Restart." Format-only gates
  catch structural violations but allow silent skill omission. Count gate closes that gap.
...
- **Source**: Round 4 excellence signal. R4 V-03 listed all 12 canonical names in PARSE
  ...
  correctly classifying a near-miss as "anything else." Explicit enumeration eliminates
  that inference step and makes fallback deterministic.
...
- C-12, C-15, and C-16 apply to `->` list output format only. If a variation uses a
  Markdown table, mark all three as N/A and adjust aspirational denominator to 8.
