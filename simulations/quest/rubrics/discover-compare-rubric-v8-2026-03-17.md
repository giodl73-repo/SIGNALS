v8 rubric written. Here's what changed:

**R9 patterns resolved as follows:**

**register-gated-routing-valid → C-25 (new aspirational criterion)**
A unified conditional routing block that issues ordering instructions for *both* register values (exec→reco-first, engineering→matrix-first) in a single construct before output sections. Pass requires both branches explicit; one branch only = partial; no routing construct = fail (falls back to C-23 scoring). Denominator bumps /17 → /18.

**c24-partial-path-agnostic → scoring clarification in C-24 (no new criterion)**
The insight is a scoring boundary, not a new output excellence pattern — there is no better-than-partial output structure to reward here, only a clarification that partial is flat. Incorporated into C-24's pass condition text: "PARTIAL is a flat discrete state: asymmetric depth... and uniform breadth... score identically within the PARTIAL band -- coverage distribution does not create sub-grades." Two new watch list rows added for both patterns.
n prose)
  = partial. No register-gated routing construct present = fail (C-23 scoring applies
  independently). **Denominator bumps from /17 to /18.**

**One scoring clarification applied to C-24:**

- **c24-partial-path-agnostic** — C-24 PARTIAL is a flat discrete state regardless of how
  partial coverage is distributed across AMEND paths. Asymmetric depth (one path carries full
  mini-ledger + HALT; remaining paths bare) and uniform breadth (all paths carry bare HALT with
  no enumeration) score identically within the PARTIAL band. C-24 has no sub-grades within
  PARTIAL — breadth vs. depth distribution does not move the score. The only path to C-24 PASS
  is full enumeration at every AMEND path. This clarification is incorporated into C-24's pass
  condition text below; no new criterion generated; no denominator change from this pattern.

**Two new patterns added to watch list:**

- **register-gated-routing-valid** — a single conditional routing block (exec->reco-first,
  engineering->matrix-first) before output sections satisfies C-25 and C-09 simultaneously;
  the routing instruction is orthogonal to all other criteria including C-24 gate structure
  (C-25 pass; confirmed V-04)
- **c24-partial-path-agnostic** — C-24 PARTIAL is a flat state; asymmetric depth (one path
  full, others absent) and uniform breadth (all paths bare-halt) score identically; no
  sub-grades within the PARTIAL band; full PASS requires enumeration at every path
  (confirmed V-02 = V-03)
...
Aspirational passed: ___ / 18  =>  ___ * 10 / 18 = ___
