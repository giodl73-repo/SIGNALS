Looking at the R3 scorecard, I can identify two new excellence patterns not yet captured in v3:

**C-17 — Phase-as-severity-key declaration**: V-04 introduced Phase as an *explicit semantic key* for severity lookup — not merely a metadata field or a separate mechanical model. The card template definition or Phase/Severity rule block states "Phase field is the lookup key for severity assignment" and "No override path exists" as an explicit constraint. Distinct from C-14 (coexistence as named fields) and C-15 (mechanical day-window table): C-17 captures the *semantic coupling* being declared structurally, making the Phase→Severity relationship machine-checkable from the template definition alone.

**C-18 — Gate minimum correct at ≥7**: C-05 was PARTIAL across all four R3 variants — every variant gates at ≥6 instead of ≥7. The structural gate is present (C-05 PARTIAL, not FAIL), but the threshold is wrong. C-18 is the aspirational target: TABLE CHECK or equivalent gate enforces exactly ≥7. Achieves full C-05 intent. All four R3 variants failed this, making it a clear synthesis target for R4.

---

```markdown
# listen-support Quest Rubric — v4
...
| Aspirational (×10) | 5 | 2 | 0 |

**Max composite:** 140 pts  
...
## Aspirational Criteria (5 pts each, max 50)
