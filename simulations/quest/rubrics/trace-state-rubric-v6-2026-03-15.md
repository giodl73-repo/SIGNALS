Scanning the R7 scorecard for structural patterns not yet mapped to a criterion slot.

**New signals from Round 7:**

1. **V-01 C-10 "Silent omissions invalidate this section"** — the reachability section declares its own section-scoped invalidation consequence, distinct from C-19's artifact-level consequence; a section can carry its own enforcement weight independently of the whole-artifact rule.

2. **V-01 C-15 "do not include in output" guard** — the example row carries an explicit anti-literal-copy guard preventing the template from being reproduced verbatim; C-15 requires a filled example row but says nothing about sanitizing it from output pollution.

3. **V-02 C-13 "Consolidated Invariant Register … `Derived From (row refs)` column with example `Pass 1 R2, Pass 2 R1`"** — invariants from multiple domain passes are aggregated into a unified register with explicit cross-pass derivation linkage; C-13 covers single-trace derivation direction but not multi-domain synthesis into a named aggregation structure.

Mapped to: **C-23**, **C-24**, **C-25**. Aspirational pool grows from 14 to 17; cap and any-5 threshold unchanged.

---

## trace-state rubric (v22) — outer quest rubric v6

**5 essential + 3 recommended + 17 aspirational (cap 5) = 25 criteria**
...
### Aspirational (10 pts — 2 pts each, any 5 of 17)
...
- **Aspirational cap**: score min(earned × 2, 10). A perfect output scoring all 17 aspirational criteria still earns 10 aspirational points.
