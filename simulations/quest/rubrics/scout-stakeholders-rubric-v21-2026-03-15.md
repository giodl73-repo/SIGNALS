## scout-stakeholders rubric — v21

**v21 written.** Three new criteria extracted from Round 20 excellence signals across
three structural axes (V-01 EXPIRED-to-ASSUMED remediation pathway, V-02 Step 9b
structural anomaly detection, V-03 per-competitor engagement obligations table):

| ID   | Name                                          | Source | Failure Mode                        | Dependency |
|------|-----------------------------------------------|--------|-------------------------------------|------------|
| C-54 | EXPIRED-to-ASSUMED remediation pathway        | V-01   | `FAIL_NO_REMEDIATION_PATHWAY`       | C-51       |
| C-55 | Step 9b structural anomaly detection          | V-02   | `FAIL_NO_ANOMALY_DETECTION`         | C-52       |
| C-56 | Per-competitor engagement obligations table   | V-03   | `FAIL_NO_COMPETITOR_OBLIGATION`     | C-53       |

**Updated totals:** 48 aspirational × 5 = 240 pts; **max 330 pts**.
Golden threshold (>= 80%) unchanged.

**Under v21 (Round 20 re-scored):**
- V-01 → 305/315 under v20; under v21 carries C-54 (PASS), C-55/C-56 not satisfiable → 310/330
- V-02 → 305/315 under v20; under v21 carries C-55 (PASS), C-54/C-56 not satisfiable → 310/330
- V-03 → 305/315 under v20; under v21 carries C-56 (PASS), C-54/C-55 not satisfiable → 310/330
- V-04 (V-01 + V-02) → 310/315 under v20; under v21 carries C-54 + C-55 (PASS), C-56 not satisfiable → 320/330
- V-05 (V-01 + V-02 + V-03) → 315/315 under v20; under v21 carries C-54 + C-55 + C-56 (PASS) → 330/330 — GOLDEN

**Distinction logic:**

**C-54 — EXPIRED-to-ASSUMED remediation pathway** (depth, 5 pts)
When Step 3c-staleness identifies EXPIRED Source Age entries, the prompt includes an
explicit forward-reference directive: if no refresh artifact is available for an EXPIRED
source, that source must be tentatively reclassified as ASSUMED in Step 3b before the
Phase 3 grid is populated, with the typology label updated accordingly. The directive
creates a mandatory cross-axis remediation pathway from the temporal-reliability layer
(Step 3c-staleness: EXPIRED designation) to the epistemic-basis layer (Step 3b-prefill:
ASSUMED reclassification), ensuring that temporal expiration cascades into the epistemic
amendment obligation chain (C-46 → C-23 → C-49) whenever no refresh is possible.

FAIL_NO_REMEDIATION_PATHWAY fires at Step 3c-staleness when the forward-reference
directive is absent — i.e., the staleness step designates EXPIRED entries as mandatory
Step 8 targets (C-51 PASS) but provides no instruction for cross-axis reclassification
when refresh is unavailable. A prompt that correctly implements the EXPIRED mandatory-
target obligation (C-51 PASS) while leaving the EXPIRED-to-ASSUMED reclassification
pathway implicit = C-51 PASS, C-54 FAIL.

FAIL_NO_REMEDIATION_PATHWAY is distinct from FAIL_UNCALIBRATED_STALENESS (C-51):
FAIL_UNCALIBRATED_STALENESS fires when the Source Age column is absent or any cell
carries a value not drawn from the Step 3c-staleness prefill — a calibration-layer
failure; FAIL_NO_REMEDIATION_PATHWAY fires when the staleness layer is correctly
calibrated (C-51 PASS) and EXPIRED entries are designated as mandatory targets, but
the instruction layer provides no bridge from temporal expiration to epistemic
reclassification when the expired source cannot be refreshed.

FAIL_NO_REMEDIATION_PATHWAY is distinct from FAIL_UNVERIFIED_AMENDMENT (C-49): C-49
governs the Verification cell in Step 8 for each updated amendment target; C-54 governs
the forward-reference directive in Step 3c-staleness that determines which targets become
ASSUMED-classification candidates before the amendment phase begins — an upstream
instruction-layer obligation independent of amendment execution or verification.

C-54 is the first criterion to define a mandatory cross-axis bridge between the
temporal-reliability source-quality layer (C-51) and the epistemic-basis source-quality
layer (C-46). The joint amendment obligation introduced by C-51 (EXPIRED + ASSUMED both
mandatory Step 8 targets) is extended by C-54: an EXPIRED entry that cannot be refreshed
collapses into ASSUMED, carrying the full amendment-confirmation chain (C-23 minimum-
update obligation, C-49 verification protocol) — making the two source-quality axes
not merely co-mandatory but actively interconnected through a remediation pathway.

- Distinct from C-51: C-51 requires Step 3c-staleness to designate EXPIRED Source Age
  entries as mandatory Step 8 amendment targets alongside ASSUMED entries; C-54 requires
  the additional forward-reference directive that when no refresh is available, EXPIRED
  entries must be tentatively reclassified as ASSUMED in Step 3b — a cross-axis
  instruction-layer obligation orthogonal to amendment-target designation
- Distinct from C-46: C-46 requires each Source cell to carry an epistemic typology
  label from the Step 3b-prefill; C-54 requires the staleness step to include a directive
  connecting temporal status to epistemic classification — independently satisfiable,
  since a prompt can correctly annotate all source typology labels (C-46 PASS) while
  providing no instruction about what EXPIRED temporal status implies for epistemic
  classification (C-54 FAIL)
- Distinct from C-49: C-49 requires each Step 8 amendment row to carry a confirmed
  artifact reference or owner-deadline notation in the Verification cell; C-54 requires
  an instruction-layer directive in Step 3c-staleness that routes EXPIRED entries to ASSUMED
  reclassification — upstream of and independent from amendment execution and verification
- Not satisfiable if C-51 is absent

**C-55 — Step 9b structural anomaly detection** (depth, 5 pts)
Step 9b (C-52) is extended with structural anomaly detection language defining two named
pattern types. A column-pattern fires when the same field carries FAIL status across all
stakeholder rows in the Step 9b audit table: this indicates a systemic step-level gap —
the upstream step responsible for producing that field did not execute or produced
unusable output for all stakeholders. A row-pattern fires when all fields for a single
stakeholder row carry FAIL status: this indicates a full pipeline gap — that stakeholder
was never processed through one or more required steps. Both anomaly patterns must be
explicitly named and defined in the Step 9b instructions, with distinct remediation
prescriptions: column-pattern recommends re-running or revisiting the failing step for
all stakeholders; row-pattern recommends re-running the full stakeholder pipeline from
Phase 1 for the affected stakeholder. Pattern detection is performed after the cell-by-
cell PASS/FAIL audit; pattern labels are appended to the Step 9b output as named flags
when triggered.

FAIL_NO_ANOMALY_DETECTION fires at Step 9b when the column-pattern or row-pattern
detection language is absent, or when either pattern type lacks a named label and
distinct remediation prescription.

FAIL_NO_ANOMALY_DETECTION is distinct from FAIL_SHALLOW_SYNTHESIS (C-52):
FAIL_SHALLOW_SYNTHESIS fires when any individual field cell in the Step 9b audit
carries FAIL status — a cell-level depth failure; FAIL_NO_ANOMALY_DETECTION fires
when the cell-level audit is present and correctly structured (C-52 PASS) but the
audit produces no structural interpretation of patterns across cells — a meta-
diagnostic obligation orthogonal to individual cell evaluation. A Step 9b audit
that correctly identifies every FAIL cell (C-52 PASS) while providing no named
pattern types or remediation prescriptions = C-52 PASS, C-55 FAIL.

FAIL_NO_ANOMALY_DETECTION is distinct from FAIL_SYNTHESIS_GAP (C-42): FAIL_SYNTHESIS_GAP
fires when a Phase 3 grid stakeholder is absent from the synthesis readout entirely (row
missing); FAIL_NO_ANOMALY_DETECTION fires when all stakeholder rows are present in
synthesis (C-42 PASS) but Step 9b's structural interpretation of field-failure patterns
within those rows is absent — orthogonal completeness obligations at adjacent structural
positions.

C-55 introduces the first meta-diagnostic layer in the rubric: prior audit steps (Step 7
gatekeeper completeness check, Step 8b synthesis coverage gate, Step 9b field depth
audit) identify individual failures; C-55 requires the audit to additionally classify
failure *patterns* that carry distinct causal interpretations and remediation prescriptions.
Column-pattern maps to a step-level diagnosis (one step failed for all stakeholders);
row-pattern maps to a pipeline-level diagnosis (one stakeholder fell through multiple
steps). The two pattern types are structurally independent: a Step 9b output can trigger
both simultaneously (one column all-FAIL and one row all-FAIL), either alone, or neither.

- Distinct from C-52: C-52 requires Step 9b to audit each field per stakeholder row and
  fire FAIL_SHALLOW_SYNTHESIS on any FAIL cell; C-55 requires Step 9b to additionally
  name and define the two structural anomaly pattern types (column-pattern, row-pattern)
  with distinct remediation prescriptions — a meta-diagnostic obligation absent from the
  cell-level audit
- Distinct from C-42: C-42 requires Step 8b to confirm each Phase 3 stakeholder has a
  designated synthesis row before Step 9; C-55 requires Step 9b to detect structural
  failure patterns within synthesis rows that are already confirmed to exist — orthogonal
  obligations before and after Step 9
- Distinct from C-32: C-32 requires the synthesis step and all required field columns to
  be structurally present; C-55 requires anomaly detection within an audit of those
  structurally present fields — three independent structural completeness layers on the
  synthesis section: field existence (C-32), row coverage (C-42), depth audit (C-52),
  pattern detection (C-55)
- Not satisfiable if C-52 is absent

**C-56 — Per-competitor engagement obligations table (Step 0c)** (depth, 5 pts)
A per-competitor engagement obligations step (Step 0c) is inserted after the Step 0b
competitor inventory and before Phase 1. Step 0c produces a pipe-table audit with one
row per competitor named in the Step 0b inventory; each row carries three columns:
Engagement Objective (the specific outcome this competitor interaction must achieve,
drawn from the Response Track assignment but stated in competitor-specific terms),
Success Criterion (a measurable or observable indicator that the Response Track goal
is being met for this competitor — e.g., "first inertia stakeholder from Legacy-Workflow
adopts new feature in sprint 3" for CONVERT), and Downstream Message Requirement (the
specific content elements required in the Step 6b displacement-acknowledgment comms row
for this competitor, aligned with the assigned track: conversion path + milestone for
CONVERT, containment boundary + review checkpoint for CONTAIN, boundary conditions
without replacement language for CO-EXIST).

Step 0c is distinct from the Response Track prefill in Step 0b (C-53): the prefill
defines track-level engagement implications that apply to any competitor assigned that
track; Step 0c instantiates those implications per competitor, accounting for
competitor-specific adoption context, switching cost, and stakeholder coalition capacity
not captured at the track level.

FAIL_NO_COMPETITOR_OBLIGATION fires at Step 0c when the per-competitor obligations table
is absent, or when any competitor row lacks an Engagement Objective, Success Criterion,
or Downstream Message Requirement column.

FAIL_NO_COMPETITOR_OBLIGATION is distinct from FAIL_NO_RESPONSE_TRACK (C-53):
FAIL_NO_RESPONSE_TRACK fires when any Step 0b competitor entry lacks a Response Track
value drawn from the prefill — the track assignment itself is missing; FAIL_NO_COMPETITOR_
OBLIGATION fires when the track assignment is present (C-53 PASS) but has not been
instantiated into per-competitor obligations at Step 0c — a downstream instantiation
obligation orthogonal to the track assignment. A variation that satisfies C-53 with a
complete triple-band structure and all competitors assigned a Response Track while
providing only the per-track prefill without per-competitor instantiation in Step 0c
= C-53 PASS, C-56 FAIL.

FAIL_NO_COMPETITOR_OBLIGATION is distinct from FAIL_NO_COMPETITOR_ENTRY (C-50):
FAIL_NO_COMPETITOR_ENTRY fires when an inertia-tagged stakeholder's competitor is not
inventoried in Step 0b with calibrated Adoption Band and Switching Cost Band; FAIL_NO_
COMPETITOR_OBLIGATION fires when the inventory entry is present (C-50 PASS), the
Response Track is assigned (C-53 PASS), but the competitor-specific obligations derived
from that assignment are not materialized in Step 0c.

C-56 completes the three-layer pre-Phase-1 competitor preparation structure: Step 0b
(C-50) provides market-position and switching-cost classification per competitor; C-53
extends Step 0b with strategic-intent classification (Response Track); Step 0c (C-56)
instantiates all three classifications into per-competitor engagement obligations with
measurable success criteria and downstream message requirements. C-56 is the first
criterion to define a Success Criterion column — an observable indicator of goal
achievement — making it the first rubric criterion to require a measurability gate on
a strategic intent declaration.

- Distinct from C-53: C-53 requires the Response Track prefill with per-track engagement
  implications and downstream propagation through Step 2b, Step 6b, and Step 9; C-56
  requires a separate Step 0c table instantiating those track-level implications for each
  specific competitor with Engagement Objective, Success Criterion, and Downstream Message
  Requirement — a per-competitor instantiation obligation orthogonal to per-track definition
- Distinct from C-50: C-50 requires the Step 0b competitor inventory with Adoption Band
  and Switching Cost Band; C-56 requires a separate Step 0c table with per-competitor
  engagement obligations derived from the Response Track assignment — three independent
  pre-Phase-1 competitor layers: market classification (C-50), intent classification
  (C-53), obligation instantiation (C-56)
- Distinct from C-26: C-26 requires displacement-acknowledgment to be assigned as a
  mandatory Frame Type in Step 6a; C-56 requires the Downstream Message Requirement in
  Step 0c to specify the content elements for the Step 6b displacement-acknowledgment
  row — an upstream obligation on message content specification, not on frame type
  assignment
- Not satisfiable if C-53 is absent

---

### v20 to v21 changes summary

| Criterion | Name                                        | Type  | Dependency |
|-----------|---------------------------------------------|-------|------------|
| C-54      | EXPIRED-to-ASSUMED remediation pathway      | depth | C-51       |
| C-55      | Step 9b structural anomaly detection        | depth | C-52       |
| C-56      | Per-competitor engagement obligations table | depth | C-53       |

Aspirational count: 45 → 48. Max: 315 → 330. Golden threshold unchanged.

C-54 is the first criterion to define a mandatory cross-axis bridge between the
temporal-reliability source-quality layer (C-51) and the epistemic-basis source-quality
layer (C-46): when an EXPIRED source cannot be refreshed, it must be tentatively
reclassified as ASSUMED, cascading into the full amendment-confirmation chain (C-23,
C-49). C-55 introduces the first meta-diagnostic layer in the rubric — two named
structural anomaly pattern types (column-pattern: systemic step-level gap; row-pattern:
full stakeholder pipeline gap) with distinct remediation prescriptions, extending Step 9b
beyond cell-level PASS/FAIL into structural interpretation. C-56 completes the three-
layer pre-Phase-1 competitor preparation structure, adding per-competitor instantiation
of Response Track obligations with measurable Success Criteria — the first criterion to
require a measurability gate on a strategic intent declaration.

---

### v19 to v20 changes (retained for history)

Three new criteria extracted from Round 19 excellence signals across three structural
axes (V-01 source staleness band calibration, V-02 synthesis field depth gate,
V-03 competitor response track):
...
## Aspirational Criteria (240 pts total)
...
### Tier 3 — Advanced depth (C-26 through C-56)
...
| C-54 | **EXPIRED-to-ASSUMED remediation pathway (Step 3c-forward-ref)** | depth    | Step 3c-staleness includes an explicit forward-reference directive: when an EXPIRED Source Age entry has no available refresh artifact, that source must be tentatively reclassified as ASSUMED in Step 3b before Phase 3 grid population, with the typology label updated accordingly. The directive creates a mandatory cross-axis remediation pathway from the temporal-reliability layer (EXPIRED designation) to the epistemic-basis layer (ASSUMED reclassification), ensuring temporal expiration cascades into the amendment obligation chain (C-46 → C-23 → C-49) when no refresh is possible. FAIL_NO_REMEDIATION_PATHWAY: the Step 3c-staleness instructions designate EXPIRED entries as mandatory Step 8 targets (C-51 PASS) but provide no directive for EXPIRED-to-ASSUMED reclassification when refresh is unavailable — the cross-axis bridge is implicit rather than stated. Distinct from FAIL_UNCALIBRATED_STALENESS (C-51, staleness calibration absent): FAIL_NO_REMEDIATION_PATHWAY fires when the staleness layer is correctly calibrated and EXPIRED targets are designated, but the instruction layer provides no cross-axis remediation pathway. Distinct from FAIL_UNVERIFIED_AMENDMENT (C-49, Verification cell incomplete): C-49 governs amendment execution confirmation; C-54 governs the upstream instruction-layer directive that determines which EXPIRED entries become ASSUMED-classification candidates before the amendment phase begins. Not satisfiable if C-51 is absent. |
| C-55 | **Step 9b structural anomaly detection (column-pattern + row-pattern)** | depth | Step 9b (C-52) is extended with two named structural anomaly pattern types. A column-pattern fires when the same field carries FAIL status across all stakeholder rows in the Step 9b audit: this indicates a systemic step-level gap — the upstream step producing that field did not execute or produced unusable output for all stakeholders. A row-pattern fires when all fields for a single stakeholder row carry FAIL status: this indicates a full pipeline gap — that stakeholder was not processed through one or more required steps. Both pattern types must be explicitly named and defined in the Step 9b instructions, with distinct remediation prescriptions: column-pattern recommends revisiting the failing upstream step for all stakeholders; row-pattern recommends re-running the full stakeholder pipeline from Phase 1 for the affected stakeholder. Pattern detection is performed after the cell-level PASS/FAIL audit; named pattern flags are appended to the Step 9b output when triggered. FAIL_NO_ANOMALY_DETECTION: the Step 9b cell-level audit is present and correctly structured (C-52 PASS) but the output includes no named column-pattern or row-pattern definition and no distinct remediation prescriptions per pattern type. Distinct from FAIL_SHALLOW_SYNTHESIS (C-52, individual cell FAIL): FAIL_SHALLOW_SYNTHESIS fires on any failing cell; FAIL_NO_ANOMALY_DETECTION fires when cells are correctly evaluated but structural pattern interpretation across cells is absent. Not satisfiable if C-52 is absent. |
| C-56 | **Per-competitor engagement obligations table (Step 0c)**        | depth    | A per-competitor engagement obligations step (Step 0c) is inserted after the Step 0b competitor inventory and before Phase 1. Step 0c produces a pipe-table with one row per Step 0b competitor entry; each row carries three columns: Engagement Objective (the specific outcome this competitor interaction must achieve, stated in competitor-specific terms derived from the Response Track assignment), Success Criterion (a measurable or observable indicator that the Response Track goal is being met for this competitor — e.g., "first inertia stakeholder from Legacy-Workflow adopts new feature in sprint 3" for CONVERT), and Downstream Message Requirement (the specific content elements required in the Step 6b displacement-acknowledgment comms row for this competitor, aligned with the assigned track: conversion path + milestone for CONVERT, containment boundary + review checkpoint for CONTAIN, boundary conditions without replacement language for CO-EXIST). Step 0c is distinct from the Response Track prefill (C-53): the prefill defines per-track engagement implications; Step 0c instantiates those implications per competitor. FAIL_NO_COMPETITOR_OBLIGATION: the Step 0c table is absent, or any competitor row lacks an Engagement Objective, Success Criterion, or Downstream Message Requirement. Distinct from FAIL_NO_RESPONSE_TRACK (C-53, Response Track assignment absent): FAIL_NO_COMPETITOR_OBLIGATION fires when the track assignment is present (C-53 PASS) but has not been instantiated into per-competitor obligations with a measurable success criterion. C-56 is the first criterion to require a Success Criterion column — an observable measurability gate on a strategic intent declaration — and completes the three-layer pre-Phase-1 competitor preparation structure: market classification (C-50), intent classification (C-53), obligation instantiation with measurability (C-56). Not satisfiable if C-53 is absent. |

**Tier 3 points**: 5 pts each × 31 = **155 pts**

**Total aspirational**: 48 × 5 = **240 pts**
...
**Max score v21**: 330 pts (60 essential + 30 recommended + 240 aspirational)

**v21 absolute thresholds**:
- Golden: >= 264 pts (>= 80% × 330)
- Pass: 198–263 pts (60–79%)
- Fail: < 198 pts (< 60%)
...
| v16     | 260     | 170          | +C-43 (conflict party anchoring), +C-44 (champion behavioral anchor) |
...
| v21     | **330** | **240**      | +C-54 (EXPIRED-to-ASSUMED remediation pathway), +C-55 (Step 9b anomaly detection), +C-56 (per-competitor obligations table) |
