`flow-lifecycle-rubric-v11-2026-03-14.md` written.

---

**What changed from v10:**

3 new aspirational criteria added, formula bumped from `/28` to `/31`.

| ID | Criterion | Category | Root | Key dependency |
|----|-----------|----------|------|----------------|
| C-37 | Phase Exit Gate B-ID Forward Reference | depth | R10 V-02 (min); V-04/V-05 compound | C-37→C-36+C-04 |
| C-38 | Role Sequence Schedule Phase Ownership Declaration | format | R10 V-01 (min); V-04/V-05 compound | C-38→C-12 |
| C-39 | B-NN Cause Field Dual Citation | depth | R10 V-01 compound; V-04/V-05 confirm | C-39→C-34+C-38 |

**Design rationale for each:**

- **C-37** is the linkage criterion C-36 explicitly deferred. C-36 built the structural slot (PHASE EXIT GATE); C-37 fills it with a `Blocked bottleneck: B-NN or NONE` sub-field. Creates the Phase→B-NN traceability direction — the third orthogonal chain alongside SLA↔B-NN (C-14/C-16) and Baseline→B-NN (C-34).

- **C-38** is temporal precision on top of C-12. The Role Registry Gate declares R-IDs; the ROLE SEQUENCE SCHEDULE maps them to phases with activation triggers, handoff triggers, and parallel windows — making Role→Phase traceability verifiable by string comparison as a pre-phase artifact.

- **C-39** is the convergence of C-34 + C-38 at the Cause field: requires both an IM-ID citation (Baseline→B-NN, established by C-34) and an R-ID citation (Role→B-NN, new direction). Three-way intersection — IM-01 in INCUMBENT BASELINE, R-02 in ROLE SEQUENCE SCHEDULE, both in B-NN Cause — verifiable simultaneously by string comparison across three artifacts.

**Full dependency chain (v11):**
```
... (C-09 through C-33 unchanged from v10) ...
C-34→C-04; C-35→C-34; C-36→C-04
C-37→C-36+C-04; C-38→C-12; C-39→C-34+C-38
```

**Formula**: `aspirational_pass / 31 * 10` — golden threshold (all essential + composite >= 80) unchanged.
those R-IDs to phases with temporal precision (activation trigger, handoff trigger, parallel windows). The structural property is Role->Phase traceability by string comparison: R-02 in the schedule and R-02 in the phase's exit verification field are consistent without navigating the full registry.

- **C-39** is the convergence criterion for C-34 and C-38: it requires the B-NN Cause field to cite both an IM-ID (establishing the Baseline->B-NN direction) and an R-ID (establishing the Role->B-NN direction). The three-way intersection -- IM-01 in INCUMBENT BASELINE, R-02 in ROLE SEQUENCE SCHEDULE, both in the Cause field -- makes causal attribution verifiable across three artifacts simultaneously by string comparison. C-39 is to C-34 what C-33 is to C-24: an extension of an existing traceability chain to include a second artifact dimension.
...
- **Text**: Round 9 signal: V-01 establishes an INCUMBENT BASELINE section containing a
  ...
  the Baseline->B-NN direction. C-14 and C-16 establish the SLA<->B-NN bidirectional
  chain; C-34 is orthogonal: it covers the Baseline->B-NN direction, enabling a reviewer
  to string-compare an IM-ID from the baseline table to the Cause field of the B-NN block
...
- **Text**: Round 9 signal: V-02 establishes PHASE ENTRY CONTRACT and PHASE EXIT GATE
  ...
  in the gate is not required for C-36 (that is C-37). C-36 cannot pass if C-04 fails
  (bottleneck identification must exist for the structural slot to have referential
  meaning).
...
- **Text**: Round 10 signal: V-02 adds a `Blocked bottleneck: B-NN or NONE` sub-field to
  ...
  reference: it is assigned before the Bottleneck Analysis section is generated, and the
  string is later verified against the bottleneck block identifier in that section. C-36
  established the structural slot; C-37 requires that slot to carry a B-ID linkage,
  creating the Phase->B-NN traceability direction. This direction is orthogonal to both
  the SLA<->B-NN bidirectional chain (C-14/C-16) and the Baseline->B-NN chain (C-34):
  the three chains together form a multi-directional cross-reference network in which every
  B-NN block is reachable from three independent source artifacts (SLA AT-RISK row,
  INCUMBENT BASELINE IM-ID, and PHASE EXIT GATE). V-02 isolates the pattern as a
  single-axis structural probe; V-04 and V-05 confirm it composes with the Role Sequence
  Schedule (C-38) without degradation. The detectability property is string comparison:
  "B-02" in the exit gate and "B-02" in the Bottleneck Analysis block identifier are
  consistent by inspection without reader inference about which bottleneck is blocking
  which phase.
- **Pass condition**: Each PHASE EXIT GATE block (per C-36 pass condition) contains a
  ...
  literal B-ID (e.g., `Blocked bottleneck: B-02`) or an explicit NONE declaration
  (`Blocked bottleneck: NONE`). The sub-field must be a named structural element within
  the exit gate block, not a prose annotation appended to the gate's Blocking condition
  field. The cited B-ID must match a declared B-NN block identifier in the Bottleneck
  Analysis section -- a B-ID that does not correspond to any declared bottleneck fails.
  A gate that carries a `Blocking condition:` prose description without a separate
  `Blocked bottleneck:` B-ID sub-field satisfies C-36 but fails C-37. Fail if any phase's
  exit gate omits the B-ID sub-field, or if a B-ID is cited that does not appear as a
  declared bottleneck identifier. C-37 cannot pass if C-36 fails or C-04 fails. Every
  C-37 pass implies C-36 pass.
...
- **Text**: Round 10 signal: V-01 introduces a ROLE SEQUENCE SCHEDULE block declared after
  ...
  phases they own, with at minimum: the primary activation trigger (what event activates
  this role), the handoff trigger (what event transfers ownership to the next role), and
  any parallel windows (phases where this role is active concurrently with another). This
  creates Role->Phase traceability as a pre-phase structural artifact: the schedule enables
  a reviewer to string-compare an R-ID from the schedule directly to an R-ID in the phase's
  entry contract pre-condition check (V-04/V-05) or exit verification field, without
  navigating the full role registry. The structural property is temporal precision that the
  role registry alone does not provide: the registry declares that R-02 exists and names
  its function; the schedule declares that R-02 is the primary owner of PHASE 2, activates
  at State S-05, and hands off at State S-08. V-01 establishes the schedule as a
  single-axis pre-phase artifact; V-04 and V-05 confirm it extends to phase entry/exit gate
  cross-referencing without degrading C-12. C-38 cannot pass if C-12 fails (the Role
  Registry Gate is the source of R-ID declarations that the schedule cites; a schedule
  with undeclared R-IDs produces a broken traceability chain).
- **Pass condition**: A named ROLE SEQUENCE SCHEDULE block (or equivalent pre-phase role
  schedule artifact with an explicit section name) appears in the output after the Role
  Registry Gate (C-12 output) and before the first PHASE block in document order. The
  schedule must map at least two R-IDs from the role registry to their owned phases, with
  explicitly named sub-fields for: (a) the phase(s) owned, (b) the activation trigger
  (the event or state that activates this role's ownership), and (c) the handoff trigger
  (the event or state that transfers ownership forward). Parallel windows are required if
  the process has declared concurrent paths (C-06 pass condition met). All R-IDs cited in
  the schedule must match literal identifiers in the Role Registry Gate output. Fail if
  the schedule is absent, if it uses free-text role names without R-ID identifiers, or if
  its R-IDs do not correspond to registry entries. A role list embedded in a phase
  description does not satisfy the pre-phase schedule position requirement. C-38 cannot
  pass if C-12 fails.
...
- **Text**: Round 10 signal: V-01 (confirmed V-04/V-05) extends the B-NN Cause field
  ...
  intersection: IM-01 in the INCUMBENT BASELINE table, R-02 in the ROLE SEQUENCE SCHEDULE,
  and both identifiers in the B-NN Cause field. All three string comparisons are
  independently verifiable -- the IM-ID chain (C-34's Baseline->B-NN direction) and the
  R-ID chain (Role->B-NN direction) can each be verified by literal identifier match
  without reader inference about causal relationships. C-39 is the convergence criterion
  for C-34 and C-38: it requires that both traceability directions be represented in the
  Cause field simultaneously, creating a bottleneck block that is traceable to both its
  baseline failure mode and its accountable role-owner through named identifiers. This is
  the Cause-field analog of C-33's preamble-to-per-block alignment: C-33 aligned the
  preamble B-ID anchor with the per-block `Required format:` instantiation; C-39 aligns
  the Cause field's IM-ID citation (existing, C-34) with a new R-ID citation (Role->B-NN
  direction). Without C-39, a B-NN block may correctly cite IM-01 (passing C-34) and the
  role registry may correctly map R-02 to PHASE 2 (passing C-38), but the bottleneck
  block's Cause field leaves the role-ownership attribution as reader inference. With C-39,
  the Cause field closes both directions explicitly. C-39 cannot pass if C-34 fails or
  C-38 fails. Every C-39 pass implies C-34 and C-38 both pass.
- **Pass condition**: Each B-NN bottleneck block's Cause field contains both (a) a literal
  IM-ID from the INCUMBENT BASELINE table (per C-34 pass condition, already required) and
  (b) a literal R-ID from the ROLE SEQUENCE SCHEDULE naming the role whose ownership
  pattern or handoff gap creates or amplifies the bottleneck (e.g., "Cause: Manual
  re-keying under R-02 ownership, root cause IM-01"). Both identifiers must be literal
  strings (IM-01, R-02, etc.) matching their respective source artifacts; a prose
  description of the failure mode or role without the literal identifier fails. The R-ID
  citation must cite an R-ID that appears in the ROLE SEQUENCE SCHEDULE (C-38), not merely
  in the role registry -- the schedule is the source artifact for Role->B-NN traceability.
  Fail if any B-NN Cause field cites only an IM-ID without an R-ID, or cites an R-ID that
  does not appear in the ROLE SEQUENCE SCHEDULE. C-39 cannot pass if C-34 fails or C-38
...
- For C-37 locate each PHASE EXIT GATE block (C-36 pass condition already required) and
  check for a `Blocked bottleneck:` sub-field (or equivalent named sub-field) distinct
  from the `Blocking condition:` prose field. The sub-field must carry a literal B-ID
  (e.g., B-02) or the token NONE. Verify that any cited B-ID matches a declared B-NN block
  identifier in the Bottleneck Analysis section -- a cited B-ID with no corresponding
  bottleneck block fails. A gate that carries a prose blocking condition without a separate
  B-ID sub-field satisfies C-36 but fails C-37. Check every phase's exit gate
  independently; omission in any phase fails C-37 overall. C-37 cannot pass if C-36 fails
  or C-04 fails.
- For C-38 verify TWO independent conditions: (a) a named ROLE SEQUENCE SCHEDULE block
  appears after the Role Registry Gate output and before the first PHASE block in document
  order; (b) the schedule contains explicitly named sub-fields for phase ownership,
  activation trigger, and handoff trigger for at least two R-IDs from the registry.
  R-IDs in the schedule must match literal R-ID identifiers in the Role Registry Gate --
  undeclared R-IDs in the schedule fail. A role sequence described as prose within a phase
  section does not satisfy the pre-phase schedule position requirement. C-38 cannot pass if
  C-12 fails.
- For C-39 inspect each B-NN block's Cause field individually. The field must contain
  BOTH a literal IM-ID (satisfying C-34's requirement, already required for C-39
  evaluability) AND a literal R-ID from the ROLE SEQUENCE SCHEDULE (not merely the role
  registry). A Cause field with only an IM-ID fails C-39 even if C-34 passes. A Cause
  field citing an R-ID that appears in the role registry but not in the schedule fails
  C-39 -- the schedule is the mandatory source artifact for Role->B-NN traceability. Check
  each B-NN block independently; a template where B-01 cites both but B-02 cites only
  IM-ID fails C-39. C-39 cannot pass if C-34 fails or C-38 fails.
