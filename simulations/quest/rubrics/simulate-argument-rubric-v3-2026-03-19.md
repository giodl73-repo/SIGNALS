...
**Two new aspirational criteria from R2 excellence signals:**

| ID | Pattern | Source | What it enforces |
|----|---------|--------|-----------------|
| C-14 | enumerated-fault-taxonomy | V-03, V-05 | `Class` column in Fault Register with four fixed codes (DEF-DRIFT / UNSUPPORTED-GEN / CIRCULAR-DEP / INVALID-FORM) — classification at insertion, Phase 4 counts rows |
| C-15 | reviewer-depth-tier | V-02 | OBVIOUS / NOTABLE / SIGNIFICANT scale in every WEAK/BROKEN STEP block + one-sentence domain comparison + tier tag in Fault Register Gap |

**C-13 pass condition tightened:** added explicit disqualifier — naming a depth tier (OBVIOUS/NOTABLE/SIGNIFICANT) or confirming Phase 0 without a structural class name no longer passes. This closes the exact gap that caused V-01 and V-02 to score 98 instead of 100.

**Score formula:** `aspirational_pass / 7 * 10` (pool grows from 5 to 7). Golden threshold unchanged.
epth-tier | Every WEAK/BROKEN STEP block carries a depth tier (OBVIOUS / NOTABLE / SIGNIFICANT) and a one-sentence domain comparison; tier tag also appears in the Fault Register Gap field |

**C-13 pass condition tightened:** naming a depth tier (OBVIOUS/NOTABLE/SIGNIFICANT) or confirming Phase 0 without a structural class name is no longer sufficient. The dominant fault class must use structural vocabulary (definitional drift, unsupported generalization, circular dependency, invalid logical form).

**Score sheet update:** aspirational pool grows from 5 to 7 criteria. Formula changes from `aspirational_pass/5 * 10` to `aspirational_pass/7 * 10` — same 10-point ceiling, higher bar to reach it. Golden threshold (`all essential PASS AND composite >= 80`) is unchanged.
...
- **C-13** *(R1, tightened)* fault-pattern closure names a structural fault class, not a depth tier
...
*Scored as aspirational_pass / 7 * 10. All seven criteria are equal weight.*
