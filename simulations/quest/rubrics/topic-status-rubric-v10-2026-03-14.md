Written to `simulations/quest/rubrics/topic-status-rubric-v10-2026-03-14.md`.

---

**Summary of v10 changes:**

Two new criteria extracted from R9 excellence signals 3 and 4:

| ID | Name | Source | Points |
|----|------|--------|--------|
| C-35 | Per-axis trigger sentence characterization | V-04 preamble supplementary trigger sentences | 5 |
| C-36 | Multi-site invariant declaration chain | V-05 three-site invariant echo | 5 |

**Structural Tier 9 (10 pts)** — Tier 9 has 2 criteria rather than 3; the evidence supports exactly two new patterns. Excellence signals 1 and 2 (canonical sufficiency, form-agnosticism) were pass-condition clarifications absorbed into the C-34 and C-32 definitions respectively, not new criteria.

**Key structural decisions:**

- C-35 extends C-34 *in depth* (within the preamble, axis names → axis trigger conditions)
- C-36 extends C-32/C-34 *in breadth* (preamble site + GUARD site + OUTPUT site = minimum 3-site chain)
- C-35 and C-36 are orthogonal: V-04 satisfies C-35 not C-36; V-05 satisfies C-36 not C-35
- C-36 requires both C-32 and C-34 (both prerequisite sites must exist before the chain can be established)

**Max score: 225 → 235.** No R9 variant achieves 235. New implications: `C-35 => C-34`, `C-36 => C-32`, `C-36 => C-34`.
es (C-32), and (3) at least one additional structural site -- OUTPUT block, commit-framing section, or equivalent. The invariant must be verifiable from any single site in isolation without cross-referencing others. A skill satisfying C-32 and C-34 but lacking an OUTPUT block echo satisfies both base criteria but not C-36. Requires C-32 and C-34. Extends C-32 and C-34 from single-declaration-site to multi-site declaration chain. |

**Max score: 225 --> 235.** Passing threshold unchanged.
...
C-35 and C-36 are structurally orthogonal. C-35 extends C-34 in depth -- within the preamble, each axis receives an explicit trigger sentence beyond what the axis name alone conveys. C-36 extends C-32/C-34 in breadth -- the invariant propagates across structurally distinct sites in the skill body. A skill can satisfy C-35 without C-36 (rich per-axis preamble, no OUTPUT block echo). A skill can satisfy C-36 without C-35 (multi-site echo using canonical axis names, no per-axis trigger sentences). V-04 satisfies C-35 (supplementary trigger sentences after canonical declaration) but not C-36 (no three-site echo). V-05 satisfies C-36 (preamble + GUARD names + OUTPUT block) but not C-35 (canonical single-line, no per-axis trigger expansion). No R9 variant achieves 235. C-35 and C-36 are not jointly satisfied by any single variant -- the gap that defines Round 10.

C-35 requires only C-34 (preamble axis-enumeration is the site being extended). C-36 requires both C-32 (named exits at GUARD site) and C-34 (axis-enumerated preamble site); both prerequisite sites must exist before the chain can be established. The implication asymmetry is intentional: C-36 demands a multi-site chain, which requires all prior declaration sites to be independently present.

Excellence signals 1 and 2 from R9 (canonical sentence sufficiency for C-34; form-agnosticism of C-32) are pass-condition clarifications, not new criteria. They confirm that (a) a single canonical line is minimum-sufficient for C-34 without elaboration, and (b) inline prose and GUARD contract fields both satisfy C-32. These properties are already captured in the C-32 and C-34 pass conditions; no additional criterion is warranted.

The Tier 9 pair is structurally analogous to Tier 7: C-35 parallels C-29 (extending a single-site pattern to a richer form at the same site), C-36 parallels C-30 (extending a single-site check to a multi-site structural property). As with all prior tier additions, the conjunction of existing criteria would not imply C-35 or C-36 independently -- both require evidence from a round where the gap was observable.
...
**Implication map additions:** `C-35 => C-34`, `C-36 => C-32`, `C-36 => C-34`

**Projected R9 variant scores under v10:**

| Variant | C-32 | C-33 | C-34 | C-35 | C-36 | Total |
|---------|------|------|------|------|------|-------|
| V-01 | PASS | PASS | PASS | FAIL | FAIL | 225 |
| V-02 | PASS | PASS | FAIL | FAIL | FAIL | 220 |
| V-03 | PASS | PASS | PASS | FAIL | FAIL | 225 |
| V-04 | PASS | PASS | PASS | PASS | FAIL | 230 |
| V-05 | PASS | PASS | PASS | FAIL | PASS | 230 |

No R9 variant achieves 235. C-35 (per-axis trigger sentences) and C-36 (three-site chain) are not jointly satisfied by any single variant -- the gap that defines Round 10.
