...
| Variation | Axis | Single/Combined | Hypothesis summary |
|---|---|---|---|
| V-01 | Inertia framing (Common-skip labeling) | Single | Skip labels at each boundary increase failure surface completeness |
| V-02 | Phrasing register (Formal/declarative) | Single | Declarative register increases C-37/C-38 structural compliance |
| V-03 | Output format (Pre-declared trace contract) | Single | Pre-declaration reduces boundary omissions |
| V-04 | Inertia framing + Machine-first output | Combined | Skip labels + machine-first maximize completeness + token density |
| V-05 | Formal register + Storage-first + Equal phase budget | Combined | Three-axis combination maximizes overall composite score |

**Excellence signal target for R17:** Across V-01 through V-05, identify whether pre-declaration (V-03) or inertia framing (V-01/V-04) produces a qualitatively distinct HALT-RULE dual-surface encoding that would justify a new C-39 criterion — specifically: whether the pre-declared HALT boundary in V-03 causes the HALT-RULE to encode the expected contradiction surface name as a structural token (e.g., HALT-EXPECTED-BOUNDARY: "PreOperation elevated FLS bypass") alongside the dual-surface predicate, making the predicted halt target machine-greppable before the trace executes.

---

# trace-request Rubric — v14 (2026-03-15)

**Scale:** 245 (Essential 60 + Recommended 30 + Aspirational 155)
**Golden threshold:** all 4 essential PASS AND composite >= 80% (196/245)

---

## One new criterion from R15 excellence signal
...
**Scale:** 240 → **245** (aspirational tier: 30 → 31 criteria, 150 → 155 pts). Golden threshold stays at >= 80%.
...
**Signal not promoted — C-39 design surface:** Live automated checker: a runnable tool or schema validator that (1) parses the Step 8 Header VT-N schema via `^VT-[0-9]+: ".*"$` and CHECKER ALGORITHM block MATCH-RULE/HALT-RULE/OUTPUT-RULE tokens, (2) executes the declared algorithm against Step 8c data using the HALT-RULE dual-surface trigger (Step 8b prose coherence state AND Step 8c Match? = N), (3) emits Row-Verdict per row and CHECK RESULT summary — producing binary pass/fail without human judgment at any step. Requires C-38 to produce consistent multi-round evidence that the HALT-RULE dual-surface encoding is stable across variation axes before a tool implementation contract can be reliably frozen.
...
**Scale change from v13:** 235 → 240 (aspirational tier: 29 → 30 criteria, 145 → 150 pts).
...
**Signal promoted in v14:** The C-38 design surface identified in v13 (live automated checker) was superseded by the R15 excellence signal (HALT-RULE dual-surface architecture encoding). The live automated checker concept shifts to C-39 design surface.
...
## Full aspirational tier — C-21 through C-38
...
**Aspirational subtotal (C-21–C-38):** 18 criteria shown above × 5 = 90 pts of the 155 pt aspirational tier. Criteria C-08 through C-20 (13 criteria × 5 = 65 pts) are stable and inherited from v12 without modification.
...
| Aspirational | C-08–C-38 | 155 |
| **Total** | **38 criteria** | **245** |

**Golden threshold:** all 4 essential PASS AND composite >= 196/245 (80%).

---

## Dependency chain — checker contract stack (C-28 → C-38)
...
C-38 closes the chain: the dual-surface architecture first established structurally at C-30 (two required surfaces, mismatch detectable) is now required to be encoded semantically within the HALT-RULE token itself — the machine-greppable algorithm declaration must name the dual-surface event as the logical basis for halt, not merely point to its output token.
