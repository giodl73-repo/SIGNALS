Written to `simulations/quest/rubrics/prove-interview-rubric-v6-2026-03-14.md`.

---

**What changed from v5 -> v6:**

Two new aspirational criteria extracted from R5 excellence patterns:

---

**C-20 -- Compliance blocks enumerate partial-compliance failure variants** (5 pts)
Sourced from **V-05**: the 6th DISPOSITION REQUIREMENT failure condition (`"SKEPTIC-first ordering present but Arc Signal uses symmetric framing: FAILS C-18"`) is qualitatively different from full-absence conditions -- it covers the case where structure is present but the complementary condition is absent. C-19 covers full absence; C-20 covers partial presence. Required: DISPOSITION REQUIREMENT must include the SKEPTIC-first-without-framing failure condition; COLUMN POLICY must include equivalent column-coexistence partial-compliance conditions (e.g., `"Quote present but Rationale absent: FAILS C-10, FAILS C-16"`). Both blocks required for PASS; one block covering partial-compliance without the other is PARTIAL.

**C-21 -- Compliance blocks use three-column tabular structure** (5 pts)
Sourced from **V-04**: tabular format is explicitly non-material to C-19 (prose bullets score identically), so it is promotable as its own structural ceiling. Required: both COLUMN POLICY and DISPOSITION REQUIREMENT blocks formatted as three-column tables (`Condition / What is absent / Criterion(s) violated`). Tabular enforces one-condition-to-one-criterion-row isolation that prose cannot guarantee. A C-19-compliant prose-bullet block fails C-21. Both blocks required for PASS; one tabular without the other is PARTIAL.

---

**Point accounting:** Aspirational 55 -> 65 pts, ceiling 145 -> 155. Golden threshold stays at 80.

**New ladder entries:**
- `C-19 -> C-20`: full-absence failure statements (C-19 floor) -> partial-presence failure variants (C-20 ceiling)
- `C-19 -> C-21`: any-format criterion-ID citation (C-19 floor) -> three-column tabular structure (C-21 ceiling)

**Note on independence:** C-20 and C-21 are independent of each other (prose with partial-compliance variants passes C-20, fails C-21; tabular without partial-compliance variants passes C-21, fails C-20). Both are C-19-dependent floors.
rose-bullet block that satisfies C-19 fails C-21. Both blocks must
be tabular for C-21 PASS; one tabular and one prose is PARTIAL.
...
**New ladder entries:**
- `C-19 -> C-20`: C-19 requires named compliance blocks with criterion-ID failure statements for
  full-absence conditions; C-20 requires those same blocks to additionally enumerate the
  partial-structural-compliance failure variants (one condition present, complementary condition
  absent).
- `C-19 -> C-21`: C-19 accepts any format (prose or tabular) as long as criterion IDs are cited;
  C-21 requires both blocks use three-column tabular structure (Condition / What is absent /
  Criterion(s) violated).
...
Note: composite ceiling is 155 in v6 (65 aspirational pts). Golden threshold is still 80 --
