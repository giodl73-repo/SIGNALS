...
date: 2026-03-15
version: 7
prior_version: quest-rubric-variate-R6-2026-03-15.md
---

# Rubric: quest-variate (v7)
...
**v7 changes**: Added C-32 through C-34 from Round 6 excellence signals. No essential or
recommended criteria changed. Aspirational denominator updated from /23 to /26.

---

## What changed v6 -> v7

3 new aspirational criteria (C-32 through C-34), each traceable to a specific R6 excellence signal:

| ID | Criterion | Source signal | Distinction from prior criteria |
|----|-----------|---------------|---------------------------------|
| C-32 | **Named verdict token in inline consistency gates** | R6 V-03, V-04, V-05: `CONSISTENT / AXIS DIVERGENCE` emitted as a labeled output line | C-31 requires the cross-artifact consistency check to fire and halt on divergence. C-32 requires the check to also emit a named binary verdict token as a labeled output line -- making the gate outcome mechanically readable without parsing conditional prose. A C-31-compliant gate that uses only conditional instructions ("if mismatch, stop") without producing a named verdict token does not satisfy C-32. |
| C-33 | **Per-phase declared responsibility as a structurally labeled key** | R6 V-02, V-03, V-04, V-05 PASS C-23; V-01 PARTIAL because Phase 3 lacks the `Declared responsibility:` label | C-23 requires each phase to have its declared responsibility stated in the prompt. C-33 requires that responsibility to appear as an explicitly labeled key (e.g., `Declared responsibility: [text]`) -- not as a prose sentence embedded in the phase description or implied from the phase title. Makes C-23 compliance structurally detectable by key presence rather than prose parsing. |
| C-34 | **Round-qualified single-axis comparison denominator** | R6 V-04 cites "V-01 R6 alone (C-25 single-axis baseline)"; V-05 cites "V-02 R6 alone (C-30 single-axis baseline)" | C-18 requires combination failure conditions to name a specific single-axis variation as the comparison baseline. C-34 requires that citation to include an explicit round qualifier (e.g., "V-01 R6" rather than "V-01"), making the reference unambiguous across experimental iterations where the same variation label can recur in different rounds with different bodies. A C-18-compliant failure condition that names a variation ID without a round tag does not satisfy C-34. Only evaluated for combination passes with a named single-axis comparison denominator. |

**Scoring formula**: aspirational denominator bumped from `/23` to `/26`. All bands and weights unchanged.
