...
**New criteria: C-41, C-42, C-43** (5 pts each, aspirational)

Extracted from the R16 candidate patterns confirmed across the five R16 variants. All five R16 variants scored 250/250 under v14 (perfect); the three new criteria differentiate them.

| Criterion | Pattern | Variants that PASS |
|-----------|---------|-------------------|
| C-41 | Lifecycle Section NH Dimension Score Emission | V-03, V-04, V-05 |
| C-42 | Finding Identifier as Formal Named Field (F-ID) | V-02, V-04 |
| C-43 | CH-ID Challenge Grounding Column (§0 origin) | V-01, V-05 |

**C-41 chain position:** C-40 requires DOMAIN sections to emit NH Dimension Score sub-tables. C-41
extends that requirement to the LIFECYCLE archetype. The analogy to C-21 (extends C-18 gate ledger
from some archetypes to all archetypes) is exact:

| Extension | Origin requirement | Extended by |
|-----------|-------------------|-------------|
| Gate ledger | C-18 (DOMAIN + BRACKET local rows) | C-21 (all archetypes including LIFECYCLE) |
| NH score emission | C-40 (DOMAIN sections only) | **C-41** (LIFECYCLE archetype) |

Without C-41: C-39/C-40 are fully mechanical for DOMAIN sections, but LIFECYCLE reviewer NH
dimension scores are extracted from prose. With C-41: LIFECYCLE section also emits a labeled
sub-table; C-39 formula extends to avg(DOMAIN B + LIFECYCLE B); no editorial extraction step
remains across any non-bracket reviewer archetype.

**C-42 chain position:** C-05 requires action items to trace to findings (by reference). C-42 makes
that reference a named, stable field (F-ID) rather than a positional row number (#). A named F-ID
is insertion-stable: a new finding inserted at row 2 does not invalidate `F-04`; it does
invalidate `Finding #4`. Downstream citation tables (action register, CH-ID response tracking,
lens coverage) use named values, not counted positions.

**C-43 chain position:** C-04 requires CH-ID challenge tracking (forward saturation: was the
challenge answered?). C-43 requires backward grounding: what preamble commitment authorized this
challenge? The §0 grounding column is populated at challenge registration time, before any reviewer
section executes. Every CH-ID becomes bi-directionally traceable: backward to preamble origin
(C-43), forward to reviewer responses (C-04/C-27).

**Score changes:**
- Aspirational: 32 -> 35
- Max: 250 -> 265
- Gold threshold: 215 -> 230

**R16 ranking under v15:**

| Rank | Variant | Score |
|------|---------|-------|
| 1 (tie) | V-04 R16 (v1-FL) | **260/265** |
| 1 (tie) | V-05 R16 (v1-NL) | **260/265** |
| 3 (tie) | V-01 R16 (v1-NH) | **255/265** |
| 3 (tie) | V-02 R16 (v1-FID) | **255/265** |
| 3 (tie) | V-03 R16 (v1-LC) | **255/265** |

**R16 score computation (baseline = all R16 variants at 250/250 under v14):**

| Variant | C-41 | C-42 | C-43 | Delta | v15 Score |
|---------|------|------|------|-------|-----------|
| **V-04** | PASS (+5) | PASS (+5) | FAIL (0) | +10 | **260/265** |
| **V-05** | PASS (+5) | FAIL (0) | PASS (+5) | +10 | **260/265** |
| **V-01** | FAIL (0) | FAIL (0) | PASS (+5) | +5 | **255/265** |
| **V-02** | FAIL (0) | PASS (+5) | FAIL (0) | +5 | **255/265** |
| **V-03** | PASS (+5) | FAIL (0) | FAIL (0) | +5 | **255/265** |

**C-41 PASS (V-03, V-04, V-05):** LIFECYCLE NH SCORE EMISSION REQUIREMENT pre-committed in §17
alongside C-40 DOMAIN NH SCORE EMISSION REQUIREMENT. LIFECYCLE reviewer section emits labeled NH
Dimension Score sub-table with per-dimension B and C scores. C-39 aggregation formula extended to
avg(DOMAIN B + LIFECYCLE B) for column B and avg(DOMAIN C + LIFECYCLE C) for column C. No prose
extraction from lifecycle section in the NH aggregation chain.

**C-41 FAIL (V-01, V-02):** C-40 PASS (DOMAIN sections emit sub-tables), but no LIFECYCLE NH SCORE
EMISSION REQUIREMENT in preamble and no sub-table in the LIFECYCLE reviewer section. C-39
aggregation formula covers DOMAIN scores only.

**C-42 PASS (V-02, V-04):** Finding table column renamed from positional `#` to explicitly labeled
`F-ID`. Action register entries, CH-ID response tracking, and lens coverage table citations
reference F-ID values. Finding identity is named, not positional.

**C-42 FAIL (V-01, V-03, V-05):** Finding table uses positional `#` column. Downstream citations
reference row positions. Finding references are positionally fragile.

**C-43 PASS (V-01, V-05):** CH-ID tracing table includes explicit `§0` grounding column populated
at challenge registration time. Each CH-ID row records the specific preamble contract clause or
commitment that authorizes it. Challenges are bi-directionally traceable.

**C-43 FAIL (V-02, V-03, V-04):** CH-ID tracing table tracks forward saturation (per-section
response columns) but carries no §0 origin citation column. Challenge authority cannot be verified
against preamble without reading narrative.

No retroactive score changes on R1--R15. All R15 v14 scores carry forward unchanged.
...
## Aspirational Criteria -- 175 pts (structural integrity, auditability, automation)
...
the LIFECYCLE reviewer archetype. When the LIFECYCLE reviewer section produces its own evaluation
of the proposed-build and best-non-build-alt alternatives, that evaluation is emitted as labeled,
structured data rather than embedded in narrative prose. The LIFECYCLE reviewer section emits an
NH Dimension Scores sub-table (or equivalent labeled structure) reporting the reviewer's
per-dimension score or rating for both the Proposed-Build (B) and the Best-Non-Build-Alt (C)
dimensions, using the same dimension rows as the C-35 NH dimension comparison table. The C-39
aggregation formula is correspondingly extended to include lifecycle scores: Column B =
avg(all DOMAIN B scores + LIFECYCLE B score); Column C = avg(all DOMAIN C scores + LIFECYCLE C
score).

The analogy to C-21 (extending C-18 gate ledger from some archetypes to all archetypes) is exact:

| Extension | Origin requirement | Extended by |
|-----------|-------------------|-------------|
| Gate ledger | C-18 (DOMAIN + BRACKET local rows) | C-21 (LIFECYCLE archetype) |
| NH score emission | C-40 (DOMAIN sections only) | C-41 (LIFECYCLE archetype) |

Without C-41: C-40 PASS -- domain sections emit sub-tables and bracket closing averages them
mechanically -- but the LIFECYCLE reviewer's NH dimension scores are extracted from prose. If the
LIFECYCLE reviewer has relevant dimension expertise, excluding it from the structured aggregation
creates an asymmetry: DOMAIN reviewers are fully mechanical; the LIFECYCLE reviewer reintroduces
an editorial extraction step.

With C-41: DOMAIN and LIFECYCLE sections both emit labeled NH Dimension Score sub-tables.
Bracket closing reads all sub-tables, applies the extended C-39 formula, and re-populates columns
B and C mechanically across all non-bracket reviewer archetypes. No prose extraction step
remains anywhere in the NH aggregation chain.

**Distinction from C-40**: C-40 requires DOMAIN reviewer sections to emit NH Dimension Score
sub-tables. C-41 requires the LIFECYCLE reviewer section to emit an NH Dimension Score sub-table.
C-40 can pass when only DOMAIN sections emit sub-tables; C-41 additionally requires LIFECYCLE
section emission and extension of the C-39 aggregation formula to include lifecycle scores. C-41
extends the upstream data emission requirement to the LIFECYCLE archetype.

**Distinction from C-21**: C-21 requires the inline gate ledger (Gate Verdict + Class) from ALL
reviewer archetypes including LIFECYCLE. C-41 requires the NH Dimension Score sub-table from the
LIFECYCLE archetype. Both are universal-coverage extensions, but C-21 operates on the gate ledger
chain while C-41 operates on the NH table aggregation chain. Independent requirements on
independent data structures.

**Distinction from C-22**: C-22 requires the bracket closing to receive and reference the
lifecycle gate verdict (g_lifecycle) during null hypothesis reassessment. C-41 requires the
lifecycle reviewer section to emit structured NH dimension scores that the bracket closing reads
mechanically. C-22 addresses verdict propagation to the bracket closing; C-41 addresses score
data emission from the lifecycle section. Both operate on the lifecycle reviewer's contribution,
but at different structural layers.

**Pass condition**: the preamble (or equivalent contract block) pre-commits a LIFECYCLE NH SCORE
EMISSION REQUIREMENT (or equivalent) alongside the C-40 DOMAIN NH SCORE EMISSION REQUIREMENT.
The LIFECYCLE reviewer section contains a labeled NH Dimension Scores sub-table before its gate
ledger row. The sub-table covers the same dimension rows as the C-35 NH dimension comparison
table and reports the lifecycle reviewer's per-dimension score or rating for column B
(Proposed-Build) and column C (Best-Non-Build-Alt). The C-39 aggregation formula pre-committed
in the preamble references lifecycle scores as included in the per-column average (i.e., the
formula is extended to avg(DOMAIN B scores + LIFECYCLE B score) for column B and equivalently
for column C). The bracket closing reads the LIFECYCLE sub-table alongside DOMAIN sub-tables
when computing column B and C averages.
...
A review with no lifecycle reviewer archetype makes C-41 vacuous regardless of C-40 status.
...
include lifecycle scores; or lifecycle dimension scores appear only in reviewer prose without a
labeled structured emission; or the bracket closing reads domain sub-tables mechanically but
extracts lifecycle scores from prose. Partial if the lifecycle NH sub-table is emitted but the
C-39 formula is not extended to include lifecycle scores; or the sub-table covers only column B
without column C scores; or the pre-commitment is absent from the preamble (sub-table emitted
but not required).
...
Without a named F-ID: citations like "Finding #3" are positionally fragile -- if a new finding
is inserted before row 3, all subsequent references are invalidated without the reference text
changing. An auditor cannot verify citation consistency without counting rows across sections.

With a named F-ID: each finding has a stable, addressable label (e.g., F-03). Citations remain
valid regardless of row insertion order. An auditor can verify citation consistency by matching
F-ID values across sections without counting positions.

The upgrade is minimal in structure but significant in auditability: renaming a column from `#`
to `F-ID` signals a contract change -- the identifier is now a named entity, not a positional
slot. All downstream structural references treat it as such.

**Distinction from C-02**: C-02 requires severity labels to carry commit-gate semantics. C-42
requires each finding to carry a named, stable identifier. Different fields, independent
requirements.

**Distinction from C-05**: C-05 requires action items to trace to findings (by role + finding
reference). C-42 requires that trace to use a named F-ID field rather than a positional reference.
C-05 can pass when findings are referenced by row position; C-42 additionally requires named
identification. C-42 strengthens the traceability contract established by C-05.

**Distinction from C-18 / C-14**: C-18 and C-14 govern the gate verdict chain-of-custody. C-42
governs finding identity and cross-reference stability. Both operate on the finding row but serve
different auditability purposes: C-18/C-14 track what the verdict was; C-42 tracks which finding
is being referenced.
...
**Vacuous case**: C-02 not active -- this criterion scores 0 (not partial, not fail). Without a
structured finding format (C-02), the named identifier has no structured row to anchor to.

**Fails if** (C-02 active): finding rows carry only a positional row number (`#`) without a
named identifier field; or a named identifier field is declared but downstream references use row
positions rather than named values; or F-ID values are not unique within the review. Partial if
F-ID fields are present in finding rows but some downstream references (action register, CH-ID
table, lens table) cite positional row numbers rather than named F-ID values; or the preamble
...
tracks which sections have responded. Without the §0 column, a CH-ID is a floating challenge
whose authority cannot be verified against the preamble without reading reviewer narrative.

The analogy to C-12 (action item class derives from gate verdict, not editorial re-assessment)
is structural: C-12 requires action item authority to trace back to a gate verdict; C-43 requires
challenge authority to trace back to a preamble commitment. Both eliminate floating authority
citations at different structural layers.

The analogy to C-20 (verbatim assembly prohibition protects gate ledger chain-of-custody) also
applies: C-20 ensures that gate verdicts are not re-derived at assembly; C-43 ensures that
challenge authority is not assumed implicitly but is declared at origin. Both are chain-of-custody
integrity requirements.

Without C-43: the CH-ID tracing table records which reviewer sections responded to each challenge
(forward saturation, C-04/C-27) but does not record what authorized the challenge in the first
place. An auditor validating whether a challenge is legitimate must trace back through reviewer
narrative to find the preamble commitment that licensed it.

With C-43: every CH-ID row records its §0 grounding citation. The preamble commitment that
authorized each challenge is visible at challenge registration time, before any reviewer section
executes. Challenges are bi-directionally auditable: backward to preamble origin, forward to
reviewer responses.
...
reference (backward: what authorized this challenge?). C-04 can pass when CH-IDs are listed and
tracked without preamble citations; C-43 additionally requires the origin column.

**Distinction from C-27**: C-27 requires a cross-section saturation requirement (SATURATED /
FULLY SATURATED tiers, blocking PASS on UNSATURATED). C-43 requires challenge grounding at
origin (§0 citation column). Both operate on the CH-ID table but serve orthogonal purposes:
C-27 enforces forward coverage completeness; C-43 enforces backward authority traceability.

**Distinction from C-19**: C-19 pre-commits the gate ledger protocol in the preamble. C-43
pre-commits challenge grounding citations in the CH-ID table. Both are preamble-origin
traceability requirements, but on independent data structures (gate ledger vs. challenge table).
...
registration time -- in the preamble or at the top of the CH-ID table before any reviewer section
executes -- not retrospectively. Each CH-ID can be verified against its stated preamble origin
without reading reviewer narrative.

**Vacuous case**: C-04 not active -- this criterion scores 0 (not partial, not fail). Without a
CH-ID tracking structure (C-04), the grounding column has no table to anchor to.

**Fails if** (C-04 active): the CH-ID tracing table contains no §0 or preamble-citation column;
or a §0 column is present but values are blank or populated retrospectively after reviewer
sections execute; or the origin citation is embedded in challenge description prose rather than
a dedicated column. Partial if a §0 column is present for some CH-IDs but absent for others; or
the preamble declares grounding is required but the table structure does not include a dedicated
column; or grounding values are generic (e.g., "Preamble") rather than citing specific contract
clauses or section identifiers.
...
aspirational_pts =  aspirational_pass    *  5   # 5 pts each, max 175
composite = essential_pts + recommended_pts + aspirational_pts
```

**Maximum**: 60 + 30 + 175 = **265 pts**

### Score Bands

| Band | Condition | Composite |
|------|-----------|-----------|
| Gold | all 5 essential pass | >= 230 |
| Strong | all 5 essential pass | 90-229 |
| Partial | <= 4 essential pass | any |
| Minimal | <= 3 essential pass | any |

### Paths to Gold (230 pts)

| Path | Essential | Rec | Asp passed | Composite |
|------|-----------|-----|------------|-----------|
| Full rec + 28 asp | 5/5 | 3/3 | 28/35 | 60+30+140 = 230 |
| 2 rec + 30 asp | 5/5 | 2/3 | 30/35 | 60+20+150 = 230 |
| 1 rec + 32 asp | 5/5 | 1/3 | 32/35 | 60+10+160 = 230 |
| 0 rec + 34 asp | 5/5 | 0/3 | 34/35 | 60+0+170 = 230 |

### Round 11 Calibration (retroactive, v10 -> v15)

All R11 variants: C-33/C-34/C-35 absent -> C-36 through C-43 all vacuous or FAIL.
No retroactive score change; all R11 v11 scores carry forward unchanged.

### Round 12 Calibration (retroactive, v11 -> v15)

C-36/C-37/C-38/C-39 status same as v13 assessment. C-40: C-39 vacuous or FAIL in all R12
variants -> C-40 vacuous -> C-41 vacuous. C-42/C-43: no R12 variant adopted F-ID or §0
grounding column. No retroactive score change.

### Round 13 Calibration (v11 -> v15)

| Variant | v11 | C-36 | C-37 | C-38 | C-39 | C-40 | C-41 | C-42 | C-43 | v15 |
|---------|-----|------|------|------|------|------|------|------|------|-----|
| **V-05** | 220 | PASS (+5) | PASS (+5) | FAIL | vac | vac | vac | FAIL | FAIL | **230/265** |
| **V-03** | 220 | PASS (+5) | vac | vac | vac | vac | vac | FAIL | FAIL | **225/265** |
| V-04 | 215 | FAIL | PASS (+5) | FAIL | vac | vac | vac | FAIL | FAIL | **220/265** |
| V-02 | 215 | FAIL | vac | vac | vac | vac | vac | FAIL | FAIL | **215/265** |
| V-01 | 210 | FAIL | PASS (+5) | FAIL | vac | vac | vac | FAIL | FAIL | **215/265** |

C-39/C-40/C-41 vacuous for all R13 variants: C-38 FAIL in every variant.
C-42/C-43: not present in any R13 variant.

### Round 14 Calibration (v13 -> v15)

Baseline = V-05 R13 under v13 = 245. C-40/C-41/C-42/C-43 are the open criteria for R14.

| Variant | v13 | C-40 | C-41 | C-42 | C-43 | v15 |
|---------|-----|------|------|------|------|-----|
| **V-05** | 245 | FAIL (0) | vac | FAIL | FAIL | **245/265** |
| **V-01** | 240 | FAIL (0) | vac | FAIL | FAIL | **240/265** |
| **V-04** | 240 | FAIL (0) | vac | FAIL | FAIL | **240/265** |
| **V-02** | 235 | vac (0) | vac | FAIL | FAIL | **235/265** |
| **V-03** | 230 | vac (0) | vac | FAIL | FAIL | **230/265** |

C-41 vacuous for all R14 variants: C-40 FAIL or vacuous -> C-41 vacuous.
C-42/C-43: not present in any R14 variant.

### Round 15 Calibration (v14 -> v15)

Baseline = all R15 variants at 250/250 under v14. C-41/C-42/C-43 are the open criteria.

| Variant | v14 | C-41 | C-42 | C-43 | v15 |
|---------|-----|------|------|------|-----|
| **V-01** | 250 | FAIL (0) | FAIL (0) | FAIL (0) | **250/265** |
| **V-02** | 250 | FAIL (0) | FAIL (0) | FAIL (0) | **250/265** |
| **V-03** | 250 | FAIL (0) | FAIL (0) | FAIL (0) | **250/265** |
| **V-04** | 250 | FAIL (0) | FAIL (0) | FAIL (0) | **250/265** |
| **V-05** | 250 | FAIL (0) | FAIL (0) | FAIL (0) | **250/265** |

C-41/C-42/C-43 not present in any R15 variant. No retroactive score change from v14.

### Round 16 Calibration (v14 -> v15)

Baseline = all R16 variants at 250/250 under v14. C-41/C-42/C-43 differentiate R16 variants.

| Variant | v14 | C-41 | C-42 | C-43 | Delta | v15 |
|---------|-----|------|------|------|-------|-----|
| **V-04** (v1-FL) | 250 | PASS (+5) | PASS (+5) | FAIL (0) | +10 | **260/265** |
| **V-05** (v1-NL) | 250 | PASS (+5) | FAIL (0) | PASS (+5) | +10 | **260/265** |
| **V-01** (v1-NH) | 250 | FAIL (0) | FAIL (0) | PASS (+5) | +5 | **255/265** |
| **V-02** (v1-FID) | 250 | FAIL (0) | PASS (+5) | FAIL (0) | +5 | **255/265** |
| **V-03** (v1-LC) | 250 | PASS (+5) | FAIL (0) | FAIL (0) | +5 | **255/265** |

**C-41 PASS (V-03, V-04, V-05):** LIFECYCLE NH SCORE EMISSION REQUIREMENT pre-committed in §17
alongside C-40. LIFECYCLE reviewer section emits labeled NH Dimension Score sub-table. C-39
aggregation formula extended to avg(DOMAIN B + LIFECYCLE B) for column B, avg(DOMAIN C +
LIFECYCLE C) for column C. No prose extraction from lifecycle section in NH aggregation chain.

**C-41 FAIL (V-01, V-02):** C-40 PASS (DOMAIN sub-tables present), but LIFECYCLE reviewer section
does not emit an NH Dimension Score sub-table and C-39 formula is not extended to include
lifecycle scores.

**C-42 PASS (V-02, V-04):** Finding table column labeled `F-ID` (named identifier). Action
register, CH-ID response tracking, and lens coverage table citations reference F-ID values.
Finding identity is named and stable, not positional.

**C-42 FAIL (V-01, V-03, V-05):** Finding table uses positional `#` column. Downstream citations
reference row positions. Finding references are positionally fragile.

**C-43 PASS (V-01, V-05):** CH-ID tracing table includes explicit §0 grounding column populated
at challenge registration time. Each CH-ID row records the specific preamble contract clause
that authorizes it. Challenges are bi-directionally traceable.

**C-43 FAIL (V-02, V-03, V-04):** CH-ID table tracks forward saturation (response columns
present) but carries no §0 origin citation column. Challenge authority requires prose inference.
...
C-41 vacuous (C-40 inactive OR no lifecycle reviewer) = 0, not partial.
C-42 vacuous (C-02 inactive) = 0, not partial.
C-43 vacuous (C-04 inactive) = 0, not partial.
