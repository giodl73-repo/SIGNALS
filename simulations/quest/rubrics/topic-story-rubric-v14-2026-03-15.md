Reading the V-02 scorecard carefully for excellence signals not yet captured.

**V-02's distinctive pattern** is the role sequence itself — "Analyst then Author" as an explicit structural constraint. V-01 makes blocks *visible* as output sections (C-37). V-02 goes further: it makes blocks *complete artifacts from a prior role* before the author writes a single narrative word. This is a different and stronger enforcement mechanism than visibility alone.

No other V-02 signals are novel — C-36/C-37/C-38 are already in v13, and the PARTIALs are truncation artifacts, not design choices.

**New criterion for v14:**

**C-39 — Analyst/Author role sequence** — Pre-composition work (BLOCK D, BLOCK P, BLOCK B, LEDGER) is assigned to an explicitly named *analyst* role that completes before any narrative authoring begins. The two roles are sequenced, not interleaved. A prompt that directs the model to build blocks and write narrative in a single pass fails. A prompt where the analyst's output is the input to the author — not a parallel activity — passes.

---

```markdown
# Quest Rubric — `topic-story` v14

**Version:** v14
**Source rounds:** R1–R14
...
| Aspirational count | 31 | 32 |
| Total criteria | 38 | 39 |
| Composite formula denominator | 31 | 32 |

**New aspirational criterion from R14 (V-02 excellence signal):**

| C | Name | Axis | Category |
|---|------|------|----------|
| C-39 | Analyst/Author role sequence | V-02 | depth |
...
| Aspirational count | 31 | 32 |
| Total criteria | 38 | 39 |
| Composite denominator | 31 | 32 |

C-39 captures the structural enforcement mechanism that distinguishes V-02 from V-01: blocks-as-visible-sections (C-37) is necessary but not sufficient — role sequencing is what prevents the model from interleaving analysis and writing and arriving at blocks that rationalize a story already composed.
