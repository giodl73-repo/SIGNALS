Written to `simulations/quest/rubrics/topic-echo-rubric-v14-2026-03-15.md`.

---

## What changed -- v13 -> v14

Four new aspirational criteria from R13 V-05 excellence signals:

**C-46 -- Assembly step declares structural dependency on detection verdict**
Closes the gap between C-44 (temporal ordering) and architectural blocking. A detection step that precedes assembly can still be bypassed if the assembly step doesn't reference the verdict. C-46 requires the assembly step header to name the detection verdict token by name and NODE number as a required input -- the assembly step is blocked, not merely sequenced.

**C-47 -- Token identity contract names satisfied criteria and explicit non-identity**
Extends C-37 (NODE assignment) and C-34 (forward-reader rationale) with two additional requirements: an enumeration of which criteria the token satisfies (e.g., "satisfies C-37/C-41/C-43/C-45") and an explicit non-identity clause ("not a structural marker embedded in C-40's node declaration template"). Full audit footprint reconstructible without re-reading the prompt.

**C-48 -- Authority block cites consumed detection verdict with node reference**
Extends C-43 (non-vacuity count) by requiring the authority block to open with a citation of the detection verdict by name and NODE number: "BACK-FILL-VERDICT (NODE N): confirmed PASS prior to this step (C-44 satisfied)." Operationally links C-43 and C-44 in a single block -- both preconditions verifiable without consulting separate steps.

**C-49 -- Version-aligned closing tag on terminal enumeration**
Extends C-39 (self-referential guard) by requiring the enumeration close to carry a version-scoped inventory tag: `[C-42 precondition typed; C-43 count; C-44 pre-assembly ordering; C-45 dedicated token]`. Machine-checkable against the "What changed" section without traversing the enumeration body.

**Scoring:** 60 + 21 + 123 = **204** (up from 192). Aspirational band 37 -> 41 criteria (max 111 -> 123 pts).
nce signal 3. C-43 requires the authority block to embed a non-vacuity
count. C-48 requires the authority block to open by citing the consumed detection verdict by
name and NODE number with its PASS status before stating the non-vacuity count: "BACK-FILL-
VERDICT (NODE N): confirmed PASS prior to this step (C-44 satisfied)." The citation
operationally links C-43 and C-44 in a single block -- a model executing the authority step
can verify both the structural ordering (C-44) and the non-vacuity (C-43) without consulting
separate steps. Distinct from C-43 (which requires the count) and C-44 (which requires
pre-ordering); this requires the authority block to produce the operational linkage
explicitly by citing the verdict with its NODE reference.

**C-49 -- Version-aligned closing tag on terminal enumeration**
Source: V-05 excellence signal 4. The terminal dependency enumeration block (C-33) closes
with a version-aligned closing tag naming all criteria introduced in the current rubric
version: e.g., `[C-42 precondition typed; C-43 count; C-44 pre-assembly ordering; C-45
dedicated token]`. The tag is machine-checkable against the rubric's version history: a
reviewer can verify the tag lists all new criteria by comparing it to the "What changed"
section without traversing the enumeration body. Distinct from C-39 (which requires a
self-referential guard against missing loops); this requires the enumeration close to carry
a version-scoped inventory of new criteria as a machine-checkable design-intent audit target.

**Scoring:** 60 + 21 + 123 = **204** (up from 192). Aspirational band 37 -> 41 criteria
(max 111 -> 123 pts).
