...
|----|-----------|-------------|--------------------------|
| C-36 | **PREREQUISITE GATE extension for Phase 0 option anatomy contract verification** | V-01 gate item 9 | C-34 requires the Phase 0 OPTION ANATOMY CONTRACT to exist with PM FRAMING: first, ARCHITECT VALIDATION: second, and a named T-NN trigger. C-36 requires the PREREQUISITE GATE to include a binary item confirming Phase 0 option anatomy contract presence and completeness. Makes C-34 compliance single-block auditable at Phase 11 without Phase 0 scroll. Mirrors C-35's upgrade over C-33: C-35 extends the gate for the causal chain contract; C-36 extends the gate for the option anatomy contract. Two orthogonal gate binaries for two orthogonal Phase 0 contracts. |
| C-37 | **PREREQUISITE GATE dual-contract coverage** | R10 discriminator finding | C-35 requires a gate binary for the Phase 0 causal chain contract. C-36 requires a gate binary for the Phase 0 option anatomy contract. C-37 requires BOTH binaries to be present in the same gate block. A gate with only one Phase 0 contract binary creates asymmetric auditability: a reviewer can confirm one contract from Phase 11 but must scroll to Phase 0 for the other. When both binaries are present, a reviewer confirms BOTH orthogonal Phase 0 contracts from a single Phase 11 block. Satisfying C-35 and C-36 independently does not satisfy C-37 — C-37 requires co-presence of both binaries in the same gate block, which no single-axis variation can supply. |

**Denominator:** `/28` → `/30`. Each aspirational criterion is worth `0.333` points (10/30).

**Ceiling under v11:** A V-06 combining all eight v8–v10 axes (C-28 through C-35) scores 99.33. The motivated R11 variation combining all ten axes (C-28 through C-37) reaches 100.0.

**Two new failure modes** added to Common Failure Modes. **Round 10 Discriminator Notes** section added. **Structural Testability Notes** and **Trace Alignment** extended through C-37.
...
The R10 discriminator pattern confirmed C-36 and C-37 structural independence: V-01 provided the Phase 0 OPTION ANATOMY CONTRACT (C-34 PASS) with a gate binary verifying anatomy contract compliance (C-36 PASS) but no gate binary for the causal chain contract (C-35 FAIL, C-37 FAIL); V-02 provided the Phase 0 CAUSAL CHAIN CONTRACT (C-33 PASS) with a gate binary for causal contract completeness (C-35 PASS) but no Phase 0 option anatomy contract (C-34 FAIL) and no anatomy-contract gate binary (C-36 FAIL, C-37 FAIL). Each variation fails the two criteria the other supplies; neither supplies C-37 because C-37 requires both gate binaries simultaneously.
...
**Ceiling under v11 (/30 denominator).** A V-06 combining all eight v8/v9 axes (C-28
through C-35) scores 99.33 (28/30 aspirational pass). The motivated R11 variation
combining all ten axes (C-28 through C-37) reaches 100.0.
...
**Ceiling under v11 (/30 denominator).** Both R9 V-01 and V-02 score 98.67 under v11 (25
of 30 aspirational criteria pass -- each fails C-30, plus either C-33/C-35/C-36/C-37 or
C-32/C-34/C-36/C-37). A motivated R11 variation combining all ten axes (C-28 through
C-37) reaches 100.0.
...
V-02 scored 99.64 under author prediction (C-34 absent) or 99.29 under strict C-32 reading
(C-32 and C-34 both absent due to DESCRIPTION-first template ordering). Both R10 targets
of 100.0 were wrong -- the two new v10 criteria decompose into two orthogonal axes, and
neither variation supplied both.

**Two new excellence patterns identified in R10: one from V-01's anatomy gate axis (C-36)
and one from the discriminator finding that no single-axis variation can reach C-37.**
...
excellence signal was a gate item (item 9) confirming Phase 0 OPTION ANATOMY CONTRACT
presence and completeness -- PM FRAMING: declared first, ARCHITECT VALIDATION: declared
second, named T-NN trigger stated. This makes C-34 compliance single-block auditable at
Phase 11 without Phase 0 scroll. Mirrors C-35's pattern: C-35 extends the gate for the
causal chain contract (C-33 axis); C-36 extends the gate for the option anatomy contract
(C-34 axis). The gate now has two Phase 0 contract binaries -- one for each orthogonal
Phase 0 initialization contract.

**C-37: Dual-contract gate coverage requires co-presence of both Phase 0 contract binaries
in the same gate block.** The R10 discriminator finding: V-01 supplies the anatomy-contract
gate binary (C-36) but not the causal-chain gate binary (C-35 FAIL). V-02 supplies the
causal-chain gate binary (C-35) but not the anatomy-contract gate binary (C-36 FAIL by
design -- no anatomy contract to verify). Neither variation can satisfy C-37 because C-37
requires both binaries to co-occur. A motivated R11 variation must combine C-33, C-34,
C-35, and C-36 simultaneously -- the Phase 0 block declares both contracts; the gate block
includes a binary for each. C-37 captures the qualitative advance of a gate that is
comprehensively auditable over all Phase 0 initialization contracts, not just one.

**C-32 structural asymmetry finding.** R10 V-02's Phase 4 option anatomy template places
DESCRIPTION: before PM FRAMING:, violating C-32's "PM FRAMING: as the first typed field"
pass condition. V-01's Phase 0 OPTION ANATOMY CONTRACT explicitly declares PM FRAMING:
first, propagating that ordering to every option entry and preventing DESCRIPTION-first
drift. The finding confirms that C-32's "first typed field" condition means first among
ALL typed fields (including non-role fields like DESCRIPTION), and that C-34's Phase 0
anatomy contract is the reliable mechanism for preventing such drift. C-32's pass condition
is clarified in v11 accordingly. The asymmetry also confirms C-34's independence from
C-32: C-32 requires the slots to exist in the template; C-34 requires a Phase 0 declaration
that enforces their ordering before any option-contributing phase executes.

**C-36 independence from C-35.** R10 V-01 PASS C-36 / FAIL C-35. The distinguishing test:
a reviewer reads the PREREQUISITE GATE block and confirms whether a binary item for Phase 0
OPTION ANATOMY CONTRACT verification is present (PM FRAMING: first, ARCHITECT VALIDATION:
second, named trigger). The Phase 0 CAUSAL CHAIN CONTRACT binary (C-35) is orthogonal --
its presence or absence does not determine C-36.

**C-37 independence from C-35 and C-36.** Neither R10 variation reaches C-37. The
distinguishing test: a reviewer reads the PREREQUISITE GATE block and confirms whether
BOTH a causal chain contract binary AND an anatomy contract binary are present. A gate with
only one Phase 0 contract binary fails C-37 regardless of which contract it covers.

**Ceiling under v11 (/30 denominator).** R10 V-01 scores 98.67 under v11 (27/30
aspirational pass -- C-35, C-37 fail; C-36 passes). R10 V-02 scores 98.33 strict (26/30
-- C-32, C-34, C-36, C-37 fail; C-35 passes) or 98.67 under author prediction (27/30 --
C-34, C-36, C-37 fail; C-32, C-35 pass). A motivated R11 variation combining all ten
axes (C-28 through C-37) reaches 100.0.
...
| 2 | V-02 | Exposes C-32 strict-field-ordering failure: confirms DESCRIPTION-first drift when anatomy contract absent; seeds C-37's dual-contract requirement via its own C-36 absence |
...
- **C-35**: Not demonstrated in trace -- no PREREQUISITE GATE block present (C-20 fails); no Phase 0 causal chain contract present (C-33 fails); a gate binary for Phase 0 causal contract verification cannot exist without both a gate block and a contract to verify; C-35 would fail independently even if C-20 and C-33 passed, because the gate binary for the causal contract is a distinct gate item not derivable from C-20's register-ordering binaries or C-33's contract content
- **C-36**: Not demonstrated in trace -- no PREREQUISITE GATE block present (C-20 fails); no Phase 0 option anatomy contract present (C-34 fails); a gate binary for Phase 0 anatomy contract verification cannot exist without both a gate block and an anatomy contract to verify; C-36 would fail independently even if C-20 and C-34 passed, because the anatomy-contract gate binary is a distinct gate item not derivable from C-20's register-ordering binaries, C-35's causal-contract binary, or C-34's contract content
- **C-37**: Not demonstrated in trace -- no PREREQUISITE GATE block present (C-20 fails); neither Phase 0 contract is present (C-33 and C-34 both fail); dual-contract gate coverage cannot exist without a gate block containing both contract binaries; C-37 would fail independently even if C-35 and C-36 both passed individually, because C-37 requires their co-presence in the same gate block, which is a conjunctive requirement not derivable from either criterion alone
