Written to `simulations/quest/rubrics/listen-support-rubric-v10-2026-03-14.md`.

---

**What changed v9 -> v10:**

Two new criteria extracted from R9 above-ceiling signals:

| ID | Name | Tightens | Who satisfies |
|----|------|----------|---------------|
| C-30 | Inference Rule Paraphrase-Before-Proceed Gate | C-28 | V-05 |
| C-31 | Inference Audit in Dual-Pass Part C | C-23, C-28, C-29 | V-05 |

**Tier:** Aspirational 21 / 105 pts -> 23 / 115 pts. Total ceiling **195 -> 205**.

---

**Signal extraction rationale:**

**C-30** comes from V-05's paraphrase mechanism. C-28 requires the inference rule as an imperative constraint; V-04 satisfies that with an isolated named step (STEP 2C) the model reads and applies. V-05 adds a paraphrase gate — the model must restate the rule in its own words before proceeding. The structural difference: a read-and-apply instruction is satisfied by verbatim copy; a paraphrase gate requires demonstrated encoding. If the model later assigns a Phase 1 P0 ticket, the scorer can compare it to the paraphrase the model committed to in its own words — that comparison is structurally self-contradicting in a way that a rule-violation-against-prose-instruction is not.

**C-31** comes from V-05's Pass 2 Part C (INFERENCE AUDIT). C-23 covers two verification surfaces: coverage traces (Pass 1) and frontmatter drift (Pass 2). Neither catches inference rule violations or switching-friction field completeness after the fact. V-05 adds Part C to Pass 2 with two named sub-checks: (1) scan every ticket for C-28 directional compliance with an exception acknowledgment gate, and (2) confirm the C-29 sub-section exists and all `Migrating from:` fields are populated. This converts C-28 and C-29 from structural assumptions into verified output properties — which is exactly why V-05 ranks above V-04 despite an identical numeric score.
ns and requires explicit exception acknowledgment. C-28 becomes a verified output property." C-23 requires two structurally independent verification passes -- Pass 1 covers STATUS QUO traces and gap orphans; Pass 2 covers frontmatter vocabulary drift. V-05 extends Pass 2 with a third part (Part C) that verifies two additional properties: (1) the Phase-Architecture Severity Inference Rule is not violated -- every Phase 1 ticket is P2/P3 and every Phase 3 ticket is P0/P1, with any exception acknowledged explicitly; (2) the dedicated switching-friction sub-section from C-29 is present and every entry has the structured competitor field populated. Without Part C, C-28 and C-29 are generation-time structural constraints -- satisfied or not at the point of generation, but never verified post-generation. Pass 2 covers vocabulary drift; it cannot catch inference rule violations or sub-section field completeness. Part C closes this gap: it makes C-28 directional compliance and C-29 field completeness verified output properties rather than assumed structural properties.
