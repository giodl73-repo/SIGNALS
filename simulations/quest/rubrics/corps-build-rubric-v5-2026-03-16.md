Written to `simulations/quest/rubrics/corps-build-rubric-v5-2026-03-16.md`.

---

**What changed in v5:**

Two new aspirational criteria extracted from R4 scorecard:

| ID | Pattern | Source |
|----|---------|--------|
| C-19 | TRANSCRIPTION-RECEIPT with per-artifact verdict and auto-correction | V-02 Signal on C-18 |
| C-20 | BUILD-VERIFY as single-purpose phase with binary per-team verdict | V-01 Signal on C-17 |

**Extraction rationale:**

**C-19** comes from V-02 (99 pts), the highest-scoring R4 variation. V-02 passes C-18 via CONTRACT-SEAL + TRANSCRIPTION-RECEIPT with `VERBATIM/REVISED` per artifact and a rewrite triggered on `REVISED`. C-18 only requires that transcription occurred; C-19 requires the phase be self-correcting — it cannot close with any artifact in a `REVISED` state.

**C-20** comes from V-01 (97 pts). V-01 passes C-17 via a dedicated `BUILD-VERIFY` phase described as "single purpose; VERBATIM/DRIFT binary per team." C-17 only requires the check exists (and is satisfied by a CROSS-REF subitem as V-02 demonstrates); C-20 requires the check to be the sole responsibility of its phase with one binary verdict per team and no other outputs.

**Pair logic:**
- C-17 + C-20: check-existence → check-isolation
- C-18 + C-19: transcription-fidelity → transcription-self-correction

**Formula:** `aspirational_pass / 12 * 10` (pool grows from 10 to 12 criteria, max points unchanged at 10).
ty
