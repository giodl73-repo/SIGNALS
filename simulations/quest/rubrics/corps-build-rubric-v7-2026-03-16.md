Written to `simulations/quest/rubrics/corps-build-rubric-v7-2026-03-16.md` (457 lines).
...
| ID | Pattern | Source |
|----|---------|--------|
| C-22 | Named binary path structure | V-02 differentiating signal on C-21 |
| C-23 | Declarative block-shape correctness rules | V-03 differentiating signal across C-17, C-20, C-21 |

**Extraction logic:**

**C-22** — V-01 and V-02 both score 100, but V-02 goes beyond C-21's re-audit gate requirement by labeling the two conditional execution branches (PATH-A / PATH-B). The label appears in both the branch condition definition and the phase output, making which path fired legible without re-reading the condition logic. Exempt path and obligation path are named as peer structures, not as a main case and an unnamed else. C-21 captures what the obligation path must do; C-22 captures that both paths must be named.

**C-23** — V-03 shows a consistent pattern across every aspirational evidence cell: each structural block is declared with an explicit correctness rule in output-forward terms ("a correct X names all three artifacts") and a named structural invalidity condition ("a BUILD-VERIFY block with non-verdict content is structurally invalid"). The PARTIAL on C-08 confirms the pattern by its absence — V-03 names the AMEND section but gives no block-shape rule, so that block is underdeclared.

**Formula:** `/13 -> /15`. Max aspirational points unchanged at 10.
he one that was rewritten. A TRANSCRIPTION-CLEAR that names fewer than three artifacts is structurally incomplete." Each block has an explicit correctness rule in declarative terms ("a correct X is...") rather than procedural terms ("the phase should emit..."). The PARTIAL on C-08 confirms the pattern by its absence: V-03 names the AMEND section but does not declare the three concrete option shapes with value placeholders -- a missing correctness rule for that block. C-23 extracts the pattern that makes this distinction scoreable.
