Written to `simulations/quest/rubrics/topic-retro-rubric-v10-2026-03-17.md`.

---

**Two new criteria from Round 11:**

**C-30 -- Phase 8 SEAL Copy-Verified Label and Source Named** (`phase-8-seal-copy-verified-label-and-source`)
- Source: V-05 C-23/C-29 excellence signal (Round 11)
- V-05's Phase 8 SEAL adds a `COPY-VERIFIED` label *and* explicitly names "Phase 6 verdict" as the copy source. C-29 requires the copy instruction exist; C-30 requires the SEAL encode both the verified state and the origin as named fields -- elevating the copy guard from an instruction to an auditable contract. Copy instruction present but SEAL lacks label or source = PARTIAL.

**C-31 -- Design Guarantees Summary Explicit** (`design-guarantees-summary-explicit`)
- Source: V-05 C-28 excellence signal (Round 11)
- V-05 carries a design-guarantees summary that explicitly names structural decisions (slot template, SEAL slot check). Guarantees buried across phase specs require reading every phase to audit; an explicit summary section lets a reviewer confirm key guarantees without reconstruction. Guarantees in phase specs but no dedicated summary = PARTIAL.
...
| Aspirational | C-09--C-31 | 2 | **46** |
| **Total** | | | **136** |

Denominators: non-AMEND = 132, AMEND/no-exclusions = 134, AMEND/exclusions = 136.
ted the decision. A gap listed without decision impact fails. An empty
...
Effective denominator in non-AMEND runs = 132. In AMEND runs with no per-phase
exclusions = 134. In AMEND runs with per-phase exclusions = 136.
