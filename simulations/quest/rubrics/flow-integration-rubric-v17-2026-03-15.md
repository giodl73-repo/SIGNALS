# flow-integration Rubric — v17

---

**Two new criteria from R16:**

**C-43 — RFC-modal MUST/SHALL unconditional obligation class (5 pts)**
Settles the RFC-modal register question from V-02. RFC 2119 MUST and SHALL constitute
a specific named class of unconditional obligation register within C-40's scope. The
critical finding is not merely that MUST/SHALL pass C-40 (register-neutral) but that
they do so because they are *stronger-obligation* modals than indicative form — RFC
2119 defines MUST as "absolute requirement" and SHALL as "absolute obligation"; neither
introduces epistemic weakening; both carry higher obligation force than declarative
indicative. The concern that normative RFC modals might weaken the consequence statement
is resolved NEGATIVE: they strengthen it. By contrast, RFC-weakening modals — SHOULD
("recommended but not required"), MAY ("optional"), NEED NOT ("not required") — DO
introduce conditional epistemic weakening and do NOT satisfy C-40 because they convert
...
C-40 PASS, 232/232, explicit verdict: "RFC-modal MUST/SHALL register is register-neutral.
No degradation." Requires C-40.

**C-44 — C-41 inertia-dominant saturation: three-sentence inertia prefix with C-42 anchor close (5 pts)**
Settles the inertia saturation boundary from V-03. A WHY block with three inertia
sentences — covering (1) implicit SDK call undercounting, (2) delegation chain
documentation failure, (3) obligation-scope never explicitly defined — followed by a
single closing sentence that supplies scope definition, explicit concern enumeration, and
a consequence-form delivery-phase anchor, constitutes an inertia-dominant WHY block (3:1
inertia-to-anchor ratio by sentence count). C-41 holds at this saturation level —
inertia framing quantity does not affect C-37/C-39 evaluation; only the presence or
absence of the consequence anchor in the WHY block determines the outcome. When the
anchor close sentence also contains concern enumeration, the block simultaneously
satisfies C-37, C-39, C-41, and C-42 — the inertia-dominant variant of the
highest-information form. The three-sentence inertia prefix adds discovery-context
saturation: three independent scope-failure root causes establish why the obligation
boundary must be made explicit, providing maximum justificatory framing before the
anchor close. V-03 (R16) probed this form with WHY block: three inertia sentences + one
scope + anchor + enumeration closing sentence. Requires C-41 and C-42.

**Ceiling: 262 → 272 (+10). Aspirational total: 182 pts (C-08–C-44).**
...
- Q1 (R17): Do RFC-weakening modals (SHOULD/MAY) fail C-40? V-02 asserts they do on logical grounds but no probe has confirmed the SHOULD/MAY failure boundary.
- Q2 (R17): Does V-03 (three-sentence inertia dominant) score C-44 at ceiling? V-03 table was not completed in R16.
...
## Aspirational Criteria (182 pts total)
...
New ceiling: 272 = R16 ceiling 262 + C-43 (5) + C-44 (5)

---

**What changed in v17:**

- **C-43** captures the *RFC-modal MUST/SHALL unconditional obligation class* pattern from
  R16 V-02. V-02 probed RFC 2119 normative modal vocabulary (MUST/SHALL) in a consequence-
  form delivery-phase WHY block and scored 232/232 with explicit C-40 verdict: PASS. The
  key finding is directional: MUST/SHALL are stronger-obligation than indicative form, not
  merely equivalent. This distinguishes C-43 from C-40's general register-neutrality claim —
  C-43 names the specific RFC modal class and closes the RFC-weakening modal failure boundary
  (SHOULD/MAY → fail C-40 because they introduce conditional epistemic weakening). C-40 covers
  all register variations; C-43 names the specific RFC normative vocabulary as a confirmed
  unconditional-obligation subclass with stronger-than-indicative obligation semantics.

- **C-44** captures the *inertia-dominant saturation form* pattern from R16 V-03. V-03 probed
  a three-sentence inertia prefix (3:1 inertia-to-anchor ratio) before a single C-42 anchor-
  close sentence. The structural finding is inertia-sentence-count neutrality: C-41 is not
  bounded by inertia sentence count at any saturation level — only the presence of the
  consequence anchor matters. When the anchor-close sentence also supplies concern enumeration,
  the block simultaneously satisfies C-37, C-39, C-41, and C-42 — the inertia-dominant variant
  of the highest-information form. C-44 extends C-41's framing neutrality claim from "inertia
  framing is neutral" (any amount) to the saturation-specific named form: three independent
  root-cause inertia sentences + C-42 close. The three-sentence structure adds maximum
  justificatory framing by naming three independent scope-failure mechanisms before the anchor.
...
- Q1 (R17): Do RFC-weakening modals (SHOULD/MAY) fail C-40? V-02 establishes this on logical
  grounds (SHOULD = "recommended but not required" → conditional epistemic weakening → C-40
  FAIL) but no probe has confirmed the failure boundary empirically. Candidate probe: WHY block
  using "Undocumented integration calls SHOULD become ship-blockers at integration review" —
  expected C-37 FAIL (conditional modal), C-40 FAIL (weakening modal).

- Q2 (R17): Does V-03 (three-sentence inertia dominant) score C-44 at ceiling? V-03 table was
  not completed in R16. Full criterion scoring of V-03's WHY block against all applicable
  criteria is needed to confirm C-44 is achievable at the expected ceiling.
