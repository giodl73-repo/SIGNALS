Written to `simulations/quest/rubrics/quest-score-rubric-v5-2026-03-16.md`.

---

**What changed v4 -> v5:**

Two new Aspirational criteria. N_aspirational: 8 -> 10.

---

**C-17 -- Inertia departure labeling** (behavior)
Source: R4 Signal 4 (V-05 Axis M). V-05's synthesis note names this explicitly: "V-05 would be the expected champion if a C-17 were added requiring explicit inertia departure documentation." Each structural enforcement mechanism must be accompanied by a labeled statement of the failure mode it prevents -- an "Inertia:" label, a "without this mechanism:" clause, or a concrete negative example. Distinct from C-16 (phase distribution): C-16 measures where mechanisms are, C-17 measures whether they explain themselves.

**C-18 -- Bilateral symmetry audit sweep** (correctness)
Source: R4 Signal 3 (V-03 Axis L -- SYMMETRY AUDIT SWEEP). V-03 extends the binary-declaration pattern from C-12 (arithmetic: YES|NO) to symmetry verification: a per-comparison Symmetric: YES/NO verdict at a phase boundary. Distinct from C-15 (both sides present): C-15 measures the result, C-18 measures whether a sweep mechanism exists that would catch the failure independently of prevention. This is the symmetry analogue of C-12.
...
```
vents -- either a named "Inertia:" label, a "without this
mechanism:" clause, or a concrete example of the bare-scorecard output that would result from its
absence. PARTIAL if at least one mechanism (but not all) includes a labeled departure explanation.
FAIL if no mechanism labels its departure from the bare-scorecard baseline. N/A rule: if the output
contains no structural enforcement mechanisms beyond verdict labels (implies C-16 FAIL), write "No
mechanisms present -- inertia labeling not applicable" -> PARTIAL (not FAIL).

---

**C-18 -- Bilateral symmetry audit sweep** (correctness)

Source: R4 Signal 3 (V-03 Axis L -- SYMMETRY AUDIT SWEEP). V-03 introduces a post-write bilateral
audit at Phase 1 exit and Phase 3 exit: a table or checklist checking each comparison for presence
of both prior and current state values, with a binary per-comparison verdict (Symmetric: YES/NO).
This extends the binary-declaration pattern from C-12 (arithmetic: "Matches stated score: YES|NO")
to symmetry verification. Like C-12 makes arithmetic errors structurally visible regardless of
whether the scorer is looking for them, C-18 makes asymmetric comparisons structurally visible -- a
Symmetric: NO cell is detectable by scanning the sweep table without reading the underlying
comparison.

Distinction from C-15: C-15 asks "were both sides of each required comparison present?" C-18 asks
"did the output include a bilateral audit sweep producing a binary per-comparison verdict at a phase
boundary?" An output can pass C-15 (both sides present through structural prevention or author
memory) while failing C-18 (no sweep -- no catch mechanism independent of prevention). An output
with a C-18 sweep that records Symmetric: NO without remediation would score FAIL on C-15 and PASS
on C-18: the criterion measures the sweep mechanism's presence, not the underlying symmetry.

Distinction from C-12: C-12 applies the binary-declaration pattern to arithmetic verification.
C-18 applies the same pattern to symmetry verification. Both are instances of the same technique
in different domains; neither implies the other.

Pass condition: The output includes a bilateral audit sweep at at least one phase boundary (Phase 1
exit or Phase 3 exit) that checks each comparison in scope for presence of both prior and current
state values, producing a per-comparison binary verdict. PARTIAL if a bilateral audit exists but
covers only a subset of the comparisons in scope -- e.g., formula delta swept but regression and
arithmetic discrepancy comparisons not included. FAIL if no bilateral symmetry audit sweep exists.
N/A rule: if no delta comparisons are in scope for this scoring run, write "No comparisons in scope
-- bilateral audit not applicable" -> PARTIAL (not FAIL).
...
| C-13 | **Formula version delta declaration** -- when the applied rubric version introduces formula parameter changes from a known prior version, the scoring output explicitly names what changed before any score is computed | correctness | A delta notice appears before or within the Phase 1 LOAD block that names: (a) the prior rubric version or formula parameter value, and (b) the current value -- e.g., "N_aspirational=10 (was 8 in v4)" or "Aspirational denominator changed: 8 -> 10". The notice must appear in the scoring output, not only in the prompt instructions. PARTIAL if the current formula is correctly loaded (C-01 PASS) but no prior-vs-current comparison is made and a version change is in scope. FAIL if a formula version change is in scope and no delta is declared, or if an outdated denominator appears in any computation. When no rubric version change is in scope, write "No prior version delta -- scoring under v[N] as the baseline version" -> PARTIAL (not FAIL). |
...
| Aspirational | C-09, C-10, C-11, C-12, C-13, C-14, C-15, C-16, C-17, C-18 | 10 | 10 |
...
final score is given (e.g., "E=4/5, R=2/3, A=7/10"). A final number alone is PARTIAL. Note the
aspirational denominator is now 10; any computation using /8 is a formula version error.
...
current value only -- "N_aspirational=10" without "was 8 in v4". This is sufficient for C-01
