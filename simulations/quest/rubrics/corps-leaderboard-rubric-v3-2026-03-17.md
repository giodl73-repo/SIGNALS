Here is the complete v3 rubric. Summary of what changed:

---

**What changed in v3**

**C-13 added** — Named Completion Gate in Role Sequence. C-11 tests *whether* inertia precedes compilation in the output. C-13 tests *whether the instruction text names the gate mechanism* that enforces that ordering structurally. V-01's "Role 1 = Inertia Diagnostician with completion gate before Role 3" and V-04's "fires before Compiler" both pass. V-03 (coach voice) and V-05 (lifecycle phases) fail — they produced correct ordering by convention but gave no structural gate, making them vulnerable to generation reordering.

**C-14 added** — Per-Action Field Template with Explicit Failure-Mode Prohibition. C-12 tests whether every action names the pattern. C-14 tests whether the instruction text itself forecloses the omission path by naming the prohibited escape hatch. V-01's `"No action may omit field 3. N/A is not valid."` is the differentiating construct. V-04 achieves C-12 through section design ("Anti-Inertia Next Actions") but never states the prohibition — making C-14 the criterion that separates the two previously tied 100-point variates.

**Scoring updated** — Aspirational tier now contains 6 criteria at 10/6 pts each (~1.67). Total remains 100. V-01 scores 100 against v3. V-04 scores 98.3 (C-14 FAIL).
gory**: correctness
...
*Six criteria sharing 10 pts total (~1.67 pts each). A perfect aspirational score requires all six.*
...
| Aspirational | C-09, C-10, C-11, C-12, C-13, C-14 | 6 | 10 pts |

**Aspirational per-criterion weight**: 10/6 pts (~1.67). Scores are reported to one decimal place.

**Golden threshold**: All 5 essential pass + composite >= 80.

**Minimum passing composite** (all essential, no recommended, no aspirational):
`5/5 * 60 + 0/3 * 30 + 0/6 * 10 = 60` -- below golden threshold.

**Typical floor for golden** (all essential + 2/3 recommended):
`60 + 20 + 0 = 80` -- exactly at golden threshold.

**V-01 score against v3 rubric**:
`5/5 * 60 + 3/3 * 30 + 6/6 * 10 = 100` -- ceiling intact.

**V-04 score against v3 rubric**:
`5/5 * 60 + 3/3 * 30 + 5/6 * 10 = 98.3` -- C-14 FAIL (no explicit N/A prohibition in instruction text).
