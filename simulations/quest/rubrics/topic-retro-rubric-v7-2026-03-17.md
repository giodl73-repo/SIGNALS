Written to `simulations/quest/rubrics/topic-retro-rubric-v7-2026-03-17.md`.

---

**Two new criteria promoted from Round 8:**

**C-24 — Echo Why-Unexpected Explained** (`echo-why-unexpected-named`)
- Source: V-02 excellence — Echo sections label findings as unexpected but never require naming the prior belief that was contradicted.
- The pattern: "unexpected" is a label; the belief-correction is the learning. `Why unexpected: [prior model contradicted]` = PASS. Label alone = PARTIAL.
- Distinct from C-09 (systemic pattern) and C-18 (pattern named): those ask *what pattern* the Echo belongs to. C-24 asks *what belief the author held* that the Echo overturned.

**C-25 — Recommendation Outcome Measurable** (`recommendation-outcome-measurable`)
- Source: V-02 C-07 PASS — Phase 5 requires three slots: gap/echo addressed + namespace/process step + measurable outcome.
- The pattern: C-07 requires gap linkage + practice change. C-17 requires a bracket-slot template. Neither requires measurability. The full contract is: "address [gap/echo] by [practice change] so that [measurable outcome]."
- Gap + change without measurable outcome = PARTIAL.
...
| Aspirational | C-09--C-25 | 2 | 34 |
| **Total** | | | **124** |

120 → 124. Denominator notes updated: non-AMEND = 120, AMEND/no-exclusions = 122, AMEND/exclusions = 124.
ement of a wrong
...
## Aspirational Criteria (34 points total)
...
- **Pass condition**: The Echo section carries an explicit field or statement naming *why* the
  ...
  or equivalent labeled field = PASS. Unexpected label present but no explanation of the
  prior belief = PARTIAL. (2 pts full / 1 pt partial)
...
- **Pass condition**: The improvement recommendation includes an explicit measurable outcome
  -- a statement of what would be observably different if the practice change succeeded. This
  is a third required slot beyond gap/echo linkage (C-07) and practice change specification
  (C-07/C-17): "address [gap/echo] by [practice change] so that [measurable outcome]." A
  recommendation that names the gap and change without specifying a measurable outcome = PARTIAL.
  No recommendation present = FAIL (C-07's domain). (2 pts full / 1 pt partial)
...
Effective denominator in non-AMEND runs = 120. In AMEND runs with no per-phase
exclusions = 122. In AMEND runs with per-phase exclusions = 124.
