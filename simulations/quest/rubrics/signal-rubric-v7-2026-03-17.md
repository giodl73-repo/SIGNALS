Done. Here's a summary of what's in v7:

---

## signal-rubric-v7-2026-03-17.md
...
**Structural theme for R6:** *achieve symmetric verification* -- every emission path explicitly verified, no mode left to model judgment.

**Retroactive R6 leaderboard under v7:**

| Rank | V | C-23 | C-24 | Composite |
|------|---|------|------|-----------|
| 1 | **V-05** | PASS | PASS | **100.00** |
| 2 | V-03 | PASS | PARTIAL | **98.75** |
| 3 | V-04 | FAIL | PARTIAL | **98.13** |
| 4 (tie) | V-01 | FAIL | PARTIAL | **97.50** |
| 4 (tie) | V-02 | FAIL | FAIL | **97.50** |

V-03 climbs to 2nd because it has FULL+BARE mode coverage (C-22+C-18) plus the catalog-as-output labeling (C-23). V-04 drops a slot: it has BARE+FILTER coverage (C-18+C-20) but no C-23. V-02 is last: only BARE mode covered, C-24 fails outright.
ffects. The R6 score
is exactly additive: each axis contributes a clean +0.71 composite in isolation, and
combining all three yields 100.00 with no cross-axis interference. Recorded as an
evaluation note; not a new criterion.
...
V-03 climbs to 2nd under v7: it has C-22+C-23 (FULL mode activated and labeled), giving
it C-24 partial at FULL+BARE coverage. V-04 holds 3rd with BARE+FILTER coverage (C-18+C-20)
but no C-23, no FULL-mode verification. V-02 is last: only BARE mode verified (C-18 only),
no C-23, C-24 fails outright.
...
For criteria with PARTIAL credit (C-14, C-15, C-24), partial counts as 0 in the
aspirational_pass count. The partial designation is noted in scorecards and informs
next-round variation design, but does not fractionally contribute to the formula.
...
- C-12, C-15, and C-16 apply to `->` list output format only. If a variation uses a
  Markdown table, mark all three as N/A and adjust aspirational denominator to 13.
...
- **Additive independence (R6 empirical finding)**: D+E+F axes combine with zero
  ...
  together yields 100.00. This independence has been confirmed across R5 single-axis probes
  and the R6 convergence run. Prompt designers can add axes D, E, and F in any order
  without unexpected scoring side-effects.
