Three new patterns from R13 V-05: dual-token Phase 2 gate (C-38), named sub-phase between numbered phases (C-39), and gate token invalidity cross-referencing a co-located structural rule (C-40). Let me write the complete v13 rubric.
...
version: v13
date: 2026-03-15
source_rounds: R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11, R12, R13
---

# Rubric — `draft-spec` — v13
...
```
composite = (essential_pass/5 * 60) + (recommended_pass/3 * 30) + (aspirational_pass/32 * 85)
```
...
| Aspirational count | 29 | 32 |
| Total criteria | 37 | 40 |
| Composite formula denominator | 29 | 32 |

**Three new aspirational criteria from R13:**

---

### C-38 — Multi-token Phase 2 gate with dual named dependencies (governance)

Phase 2 declares REQUIRES for two named gate tokens from two distinct prior phases — specifically [PM-CONTRACT-SEAL] from Phase 1 AND a second named gate token (e.g., [STRATEGY-SCOPE-SEAL]) from a sub-phase or additional phase that precedes Phase 2. Both tokens must be named explicitly in Phase 2's REQUIRES header or preamble — "Phase 2 begins after [PM-CONTRACT-SEAL] is present AND [STRATEGY-SCOPE-SEAL] is present" or the compressed equivalent ("→ BLOCKED: Phase 2 requires [PM-CONTRACT-SEAL] AND [STRATEGY-SCOPE-SEAL]"). A Phase 2 REQUIRES declaration naming only one token does not satisfy; a second token named only in surrounding prose or in the second token's own definition without a corresponding Phase 2 REQUIRES does not satisfy. The pattern captures the case where a Strategy scoping step (or equivalent role insertion) is computationally enforced at the Phase 2 gate level — not merely noted as a prerequisite. Applicable only to templates that introduce a pre-Phase-2 role emitting its own named gate token; a template with no second pre-Phase-2 seal cannot be evaluated against this criterion. Requires C-17 and C-28.

Source signal: V-05 — Phase 2 REQUIRES: "[PM-CONTRACT-SEAL] from Phase 1 AND [STRATEGY-SCOPE-SEAL] from Phase 1.5" — two named tokens from two distinct prior phases declared jointly in Phase 2's gate dependency.

---

### C-39 — Named fractional sub-phase between numbered phases with role and seal (structure)

The template includes at least one named fractional sub-phase (e.g., Phase 1.5) inserted between two consecutive numbered phases. Three conditions must be satisfied simultaneously: (1) all five required numbered phases from C-01 remain present in prescribed order and are not merged, replaced, or reordered by the sub-phase; (2) the sub-phase carries a descriptive header that names both its ordinal position and its role scope (e.g., "Phase 1.5 — STRATEGY INERTIA SCOPING"); (3) the sub-phase emits at minimum one named gate token. A new role inserted via a renamed required phase, an unnumbered section between phases, or a sub-phase with a descriptive header but no emitted gate token does not satisfy. The numbered phase ordering contract of C-01 is orthogonal to fractional sub-phases — C-01 passes when five required phases are in order regardless of sub-phases between them; C-39 passes when a fractional sub-phase exists with the prescribed anatomy. Requires C-01.

Source signal: V-05 — Phase 1.5 (STRATEGY INERTIA SCOPING) inserted between Phase 1 and Phase 2; header names both ordinal (1.5) and role scope (STRATEGY INERTIA SCOPING); sub-phase emits [STRATEGY-SCOPE-SEAL]; all five C-01 phases pass in prescribed order unaffected.

---

### C-40 — Gate token invalidity rule cross-references a co-located structural rule by name (depth)

A named gate token's INVALID IF rule explicitly names a co-located structural rule — defined within another block in the same template — as a required presence condition: the token is invalid not only if the target block is absent, but if the named structural rule is absent from within that block. The cross-reference must identify both the target block and the specific rule it must contain (e.g., "[STRATEGY-SCOPE-SEAL] INVALID IF the [STATUS-QUO-SNAPSHOT] structural fail rule is absent from the [STATUS-QUO-SNAPSHOT] block definition"). A token that names only block presence without referencing the rule within the block does not satisfy — C-36 covers block presence in Condition 1; C-40 is the deeper dependency where a seal's validity depends on a specific co-located enforcement rule existing within another block. This pattern formalizes cross-block structural rule dependencies at the token-definition level: a seal that requires a structural rule to be in force can only be validly emitted if that rule is verifiably present in the template. Applicable only to templates where a gate token's invalidity logic names a co-located structural rule in another block as a precondition. Requires C-37.

Source signal: V-05 — [STRATEGY-SCOPE-SEAL] INVALID IF rule requires [STATUS-QUO-SNAPSHOT] presence AND the co-located structural fail rule co-located with the [STATUS-QUO-SNAPSHOT] row definition — the seal's invalidity logic names the C-37 rule as a required precondition by reference, not merely the block containing it.

---

## N/A Rules

**C-36, C-37**: Inapplicable to templates without [STATUS-QUO-SNAPSHOT]. Cannot contribute to `aspirational_pass`. Fixed denominator = 32 for all variations.

**C-38, C-39, C-40**: Inapplicable to templates without a named fractional sub-phase emitting a second pre-Phase-2 gate token. Cannot contribute to `aspirational_pass`. Fixed denominator = 32 for all variations.

A template satisfying none of the status-quo or sub-phase extension patterns achieves at most 27/32 aspirational points.
...
| Aspirational count | 29 | 32 |
| Total criteria | 37 | 40 |
| Formula denominator | 29 | 32 |
| New N/A scope | C-36, C-37 require [STATUS-QUO-SNAPSHOT] | C-36–C-37 require [STATUS-QUO-SNAPSHOT]; C-38–C-40 require Phase 1.5 + second seal |

**C-38** — the dual-token Phase 2 gate — is the primary signal from R13; V-05 is the only variation that introduced the pattern and all three new criteria derive from it. A template satisfying neither the [STATUS-QUO-SNAPSHOT] nor the Phase 1.5 extension achieves at most 27/32 aspirational points under v13.
