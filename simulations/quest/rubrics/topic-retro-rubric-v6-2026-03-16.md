...
**C-19 — Explicit phase boundary markers appear between consecutive phases** (`behavior | aspirational`)
Sourced from V-04 (R5), which emits `Phase boundary: Echo is now immutable. Proceed to Phase 2.` as a structural label at each phase transition. C-18 requires phases be numbered and named; C-19 requires the sealing event itself to be declared. A numbered-phase output (passing C-18) that moves silently from phase to phase does not pass — the boundary must be an explicit, auditable artifact.

**C-20 — Output includes a status-quo foil section making cost-of-inaction explicit** (`depth | aspirational`)
Sourced from V-05 (R5), which appends a Part B counterfactual table (`Without This Retro` vs `With This Retro`). C-10 requires the Echo feeds back into a change proposal; C-20 requires that proposal be grounded in a demonstrated counterfactual showing what the default assumption would have missed. An output that proposes a skill amendment (passing C-10) but omits the foil comparison does not pass this criterion.
...
The layering chain now extends to five tiers:

| Axis | T1 (v2) | T2 (v3) | T3 (v4) | T4 (v5) | T5 (v6) |
|------|---------|---------|---------|---------|---------|
| Formula | C-11: formula stated | C-13: formula in header | C-15: formula + example in header | — | — |
| Echo/Conflict | C-12: Echo before accuracy | C-14: conflict named as artifact | C-16: audit unconditional, always emits | C-17: Echo marked LOCKED | C-20: cost-of-inaction foil |
...
| BRONZE | All essential pass, composite >= 60 |
| FAIL | Any essential criterion fails |
