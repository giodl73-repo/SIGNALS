v14 written to `simulations/quest/rubrics/campaign-simulate-rubric-v14-2026-03-17.md`.

---

**Three new criteria extracted from R15:**

**C-50 — C-47 Compliance Declared as Column-Scan Sufficiency via Verbatim Field-Exclusion Proof** *(format, C-47 activation)*
V-01 added a `COLUMN-SCAN SURFACE (C-47)` annotation to the FT Verb schema, a checklist enumeration of all (F-ID, Verb) pairs without narrative fields, and a verbatim sufficiency declaration: "C-47 verified without reading Summary, Impact, Remediation, or any other narrative field." The verbatim declaration is itself the falsifiable assertion -- any finding requiring narrative reading contradicts sub-claim 3 at first instance. C-47's ceiling is cross-consistency (both columns present); C-50 requires the cross-verification to be proven column-scan-sufficient.

**C-51 — ECM SKIPPED Tier Label Is Status-Cell-Local, Enabling Single-Column Tier Verification** *(correctness, C-48 activation)*
V-02 placed `SKIPPED-T1 (commitment-present)` / `SKIPPED-T2 (none-declared)` directly in the Status cell value with a tier derivation rule above the ECM. C-48's ceiling is commitment-aware two-tier distinction; C-51 requires the tier to be determinable from the Status cell alone without cross-referencing the Examining Sub-Skill column. Entity coverage gap tower now three levels deep: C-27 -> C-48 -> C-51.

**C-52 — RQG Verb Source Carries DETECTION/CORRECTED Classification with Self-Matching F-ID** *(correctness, C-49 activation)*
V-03 restructured the Verb Source format to `DETECTION: FT[F-NN].Verb=[Verb]` (clean) or `CORRECTED: FT[F-NN].Verb=[old]->[new]; bind failure declared` (corrected), with the cited F-NN constrained to match the RQG row's own F-ID. The self-match converts the back-cite from a narrative claim into a column-value equality check. C-49's ceiling is cited-provenance; C-52 requires classified status and structural self-verification. Type-verb binding tower now four levels deep: C-31 -> C-47 -> C-49 -> C-52.

**Aspirational pool: 41 -> 44. Max score: 100 unchanged.**
Finding
- **Weight**: essential
...
**Type-verb binding tower (C-31 -> C-47 -> C-49 -> C-52):** C-31 is the single-artifact
type-verb floor (RQG Verb must match the allowed vocabulary for the finding's Type); C-47 is the
dual-artifact detection ceiling (Verb also appears in the Findings Table at detection time,
enabling cross-artifact Type-Verb consistency verification from either artifact independently);
C-49 is the cited-provenance ceiling (RQG Verb Source column names the originating Findings
Table entry, creating a three-point bidirectional chain where a discrepancy is structurally
detectable from either endpoint without manual cross-comparison); C-52 is the classified-self-
verifying-citation ceiling (Verb Source carries DETECTION/CORRECTED classification plus a
self-matching F-ID constraint, converting the back-cite into a structural invariant verifiable
by column-value equality without tracing to the Findings Table).

**Column-scan sufficiency tower (C-47 -> C-50):** C-47 is the dual-artifact cross-consistency
floor (Verb present in both FT and RQG); C-50 is the declared-sufficiency ceiling (a verbatim
field-exclusion proof declares that C-47 compliance required no narrative reading, making the
column-scan claim itself the falsifiable assertion).

**Entity coverage gap tower (C-27 -> C-48 -> C-51):** C-27 is the binary gap floor (ECM must
log all SKIPPED entities as execution gaps); C-48 is the commitment-aware ceiling (SKIPPED
entries must carry a tier label distinguishing promise-breach gaps, where an Examining Sub-Skill
was declared in the TEM but not executed, from planning gaps, where no examining commitment was
ever declared); C-51 is the status-local tier ceiling (tier label embedded directly in the
Status cell value, enabling tier verification by single-column scan without cross-referencing
the Examining Sub-Skill column).
...
**Aspirational criteria (C-26 through C-52)**: 44 criteria in pool, capped at 10 points total
...
## R15 Variation Outcomes (reference)

*Scores computed under v14 rubric (44-criterion aspirational pool).*
*All five R15 variations are supersets of R14 V-05 (the first variation to pass all 41 v13
criteria). All five inherit 41/41 v13 PASS. The v14 question is which of C-50, C-51, C-52
each variation satisfies.*

- **V-01 (~99.5):** Column-Scan Verification Axis. C-50 PASS (COLUMN-SCAN SURFACE annotation
  in FT Verb schema; checklist enumerates all (F-ID, Verb) pairs without narrative fields;
  verbatim sufficiency declaration: "C-47 verified without reading Summary, Impact, Remediation,
  or any other narrative field"). C-51 FAIL (no SKIPPED-T1/T2 labels in Status cells). C-52
  FAIL (no DETECTION/CORRECTED classification in Verb Source). 42 passing / 44 eligible = 95.45%.
  Score: 90 + 9.55 = **~99.5**.

- **V-02 (~99.5):** SKIPPED-Tier Axis. C-51 PASS (SKIPPED-T1 / SKIPPED-T2 labels in ECM Status
  cells; tier derivation rule above ECM; single-column tier verification without Examining
  Sub-Skill cross-reference). C-50 FAIL (no COLUMN-SCAN SURFACE annotation). C-52 FAIL (no
  DETECTION/CORRECTED classification). 42 passing / 44 eligible = 95.45%.
  Score: 90 + 9.55 = **~99.5**.

- **V-03 (~99.5):** Verb Source F-ID Back-Cite Axis. C-52 PASS (DETECTION: FT[F-NN].Verb=[Verb]
  or CORRECTED: FT[F-NN].Verb=[old]->[new] format; F-NN in Verb Source matches RQG row F-ID;
  CORRECTED entries carry explicit bind-failure declaration). C-50 FAIL (no COLUMN-SCAN SURFACE
  annotation). C-51 FAIL (no SKIPPED-T1/T2 tier labels). 42 passing / 44 eligible = 95.45%.
  Score: 90 + 9.55 = **~99.5**.

- **V-04 and V-05**: Full integration details pending R15 scorecard completion.

**R15 ceiling targets**: C-50 (column-scan sufficiency declaration) + C-51 (status-cell-local
tier labels) + C-52 (DETECTION/CORRECTED + self-matching F-ID) form three independent towers.
A full-integration variation satisfying all three (C-50 PASS + C-51 PASS + C-52 PASS) alongside
the 41 v13 base criteria achieves 44/44 = 100.0.
