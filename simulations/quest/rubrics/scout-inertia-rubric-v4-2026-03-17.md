Written to `simulations/quest/rubrics/scout-inertia-rubric-v4-2026-03-17.md`.
...
| ID | Criterion | Source |
|----|-----------|--------|
| C-16 | **Trigger/Impact Decomposition Uses Dedicated Table Columns** — column-level structure makes non-compliance structurally visible: a blank Impact cell is obviously malformed; a partially-compliant inline label within a merged cell is not. Labeled bullets ("Trigger: / Impact:") fail; a named Trigger column and a separate Impact column pass. | R3 pattern 1: V-02/V-05 column structure vs. V-01 labeled bullets |
| C-17 | **Synthesis Step Declares Section Heading Manifest** — the synthesis/output step opens with an explicit pre-committed list of required section headings before content begins. A missing section is then detectable at the declaration site without scanning the body. An output that happens to have correct headings passes C-14; only an output with an explicit manifest passes C-17. | R3 pattern 2: V-03/V-05 heading manifest orthogonal to role structure |

**Formula change:** aspirational denominator updated from 7 to 9, tier max stays at 10 pts.

**Design notes added to Evaluator Notes:**
- C-16 upgrades C-15: same trigger/impact separation, but column-level enforcement. Pass C-15 with labeled prose, fail C-16 without separate columns.
- C-17 upgrades C-14: same labeled sections, but pre-committed manifest. Pass C-14 with correct headings, fail C-17 without an explicit manifest.
- C-16 and C-17 are independent — a table with dedicated columns (C-16) can coexist with no heading manifest (C-17 fails), and vice versa.
core is HIGH (or
