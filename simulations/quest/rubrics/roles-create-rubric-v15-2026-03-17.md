**v15 written** to `simulations/quest/rubrics/roles-create-rubric-v15-2026-03-17.md`.

---

**What changed from v14:**

**One new criterion — C-34 — Full canonical discipline**

The R16 signal from V-01 (C-30 alone, scoring 24/26): C-33 PASS coexists with C-32 FAIL, empirically confirming that the two conjunction criteria are genuinely orthogonal. A skill can close the no-restatement surface (C-33) while still annotating PASS branches (C-32 fail). The natural meta-conjunction is C-34: both layers simultaneously clean.

**C-34 pass condition:** Both C-32 (gate annotation layer: thin condition + bare FAIL + bare PASS) and C-33 (citation surface layer: thin phase body + thin checklist + thin gate condition) pass simultaneously. C-34 implies C-32 and C-33; if either fails, C-34 fails. C-34 is the ceiling criterion — a skill that passes C-34 has satisfied every structural criterion in the aspirational tier.

**C-31 pivot point (new finding):** C-31 is the shared prerequisite for both C-32 and C-33 — the only single criterion whose failure breaks two independent conjunction criteria simultaneously. C-31 alone fail → C-32 + C-33 + C-34 = 4 total deductions, tied with C-29 as the highest-multiplier single failure mode. Deduction depth table added to the scoring notes.
...
|--|-----|-----|
| Denominator | 26 | 27 |
| Per-criterion weight | 0.3846 | 0.3704 |
| V-05 target | 26/26 = 100.00 | 27/27 = 100.00 |
C-32 + C-34 = 3 total deductions; C-31 alone fail cascades to C-32 + C-33 + C-34 = 4 total deductions. C-29 and C-31 are equally costly by deduction count but structurally distinct: C-29 breaks the gate annotation layer through a forward cascade (C-30 -> C-32 -> C-34); C-31 breaks both layers simultaneously through the pivot (C-32 + C-33 -> C-34).

**C-32/C-33 orthogonality confirmed (R16 V-01):** V-01 (C-30 alone) achieves C-33 PASS while failing C-32 FAIL. No-restatement surface closure does not require gate triple purity — a skill can have thin citations everywhere while still annotating PASS branches. The two conjunction criteria are genuinely independent, as designed in v14.

**Scoring:** Denominator 26 → 27, per-criterion weight 0.3846 → 0.3704, V-05 targets 27/27 = 100.00.
...
Aspirational (27 criteria, 0.3704 pts each): ___ / 27  =>  ___ * 10 / 27 = ___

Composite score: ___

Formula: (essential_passed * 12) + (recommended_passed * 15) + (aspirational_passed / 27 * 10)
```

**Rubric version:** v15 | **Scoring:** `5 Essential @ 12pt + 2 Recommended @ 15pt + 27 Aspirational @ 0.3704pt`
...
**C-32 decomposition:** C-34 adds the meta-conjunction level above C-32. A C-32 pass means all three gate annotation surfaces are clean -- citations may or may not restate rule content in phase bodies or checklist items. C-33 governs those surfaces independently. C-34 formalizes the conjunction of both.
...
**Cascade asymmetry summary (R15 + R16 confirmed):**
- Annotating FAIL branches (C-29 fail): C-29 + C-30 (cascade) + C-32 + C-34 = 4 deductions
- Annotating PASS branches only (C-30 alone): C-30 + C-32 + C-34 = 3 deductions (R16 V-01 confirms 2 deductions under v14; 3 under v15 with C-34 added)
- Verbose gate conditions (C-31 alone): C-31 + C-32 + C-33 + C-34 = 4 deductions (C-31 breaks both conjunction layers simultaneously)
- Full canonical compliance (all 27 pass): C-32 PASS + C-33 PASS + C-34 PASS (all conjunction criteria satisfied)

**Deduction depth table (v15):**

| Single failure | Cascade targets | Total deductions |
|----------------|----------------|-----------------|
| C-29 only | C-30, C-32, C-34 | 4 |
| C-30 only | C-32, C-34 | 3 |
| C-31 only | C-32, C-33, C-34 | 4 |

C-31 and C-29 are equally costly by deduction count (4 each) but through structurally distinct cascade paths: C-29 breaks the gate annotation layer forward (C-30 -> C-32 -> C-34); C-31 breaks both layers simultaneously through the pivot (C-32 + C-33 -> C-34).
