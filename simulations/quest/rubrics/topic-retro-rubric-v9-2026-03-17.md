Written to `simulations/quest/rubrics/topic-retro-rubric-v9-2026-03-17.md`.

---

**Two new criteria from Round 10:**

**C-28 -- Recommendation Slot Completeness Guard** (`recommendation-slot-completeness-guard`)
- Source: V-05 C-07 PASS (persistent PARTIAL in V-01–V-04)
- C-17 provides the bracket-slot template; C-28 requires the slots to be verifiably filled. An explicit SEAL check in the recommendation phase confirms no `[placeholder]` strings survive. Template present but no slot guard = PARTIAL.

**C-29 -- Phase 8 Score Copy Guard** (`phase-8-score-copy-guard`)
- Source: V-05 Phase 8 excellence signal
- Phase 8 must carry a "do not recompute" instruction requiring the score to be copied verbatim from Phase 6. Silent recomputation can produce a self-consistent output that misreports the actual verdict. Phase 8 present but no copy guard = PARTIAL.
...
| Aspirational | C-09--C-29 | 2 | **42** |
| **Total** | | | **132** |

Denominators: non-AMEND = 128, AMEND/no-exclusions = 130, AMEND/exclusions = 132.
ase-8-score-copy-guard`)
- Source: V-05 Phase 8 excellence signal (C-09--C-25 evidence block)
- The mechanism: The score rendered in Phase 8 must be copied verbatim from the Phase 6
  accuracy computation -- not silently recomputed. Silent recomputation can introduce
  divergence between the scored table and the final summary, producing a self-consistent
  output that nonetheless misreports the actual verdict. A "do not recompute" instruction
  with an explicit copy mechanism in Phase 8 prevents this failure mode.
- Phase 8 present but no copy guard = PARTIAL. Explicit "do not recompute" instruction with
  copy mechanism = PASS.
...
Denominator: non-AMEND = 128, AMEND/no-exclusions = 130, AMEND/exclusions = 132.
...
Effective denominator in non-AMEND runs = 128. In AMEND runs with no per-phase
exclusions = 130. In AMEND runs with per-phase exclusions = 132.
