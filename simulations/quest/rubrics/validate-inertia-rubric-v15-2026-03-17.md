`validate-inertia-rubric-v15-2026-03-17.md` written.

---

**What was added:**

**C-42 — Gate components not split between block instruction body and produced architecture label**

Source: V-68. The new failure mode: axis enumeration in the block instruction body, impossibility statement displaced exclusively to the produced architecture label. A model reading the block body before Phase 1 encounters three axes but no prohibition against partial coverage — the impossibility clause is emit-time (written when producing the label), not read-time. A two-axis block body sees no impossibility clause and may proceed to Phase 1 unchallenged; the gate fires too late.

The split-gate shape is structurally distinct from C-41's named PARTIAL (full gate in produced label only): in the split shape, partial read-time enforcement exists because one component IS in the block body. C-41 requires the full gate in the block body but doesn't name the split-gate as its own criterion — without C-42, split shapes earn C-41 PARTIAL via holistic genuine-attempt doctrine. C-42 makes the split pattern explicit with defined verdicts:

| Shape | C-41 | C-42 |
|-------|------|------|
| Both components in block body (V-66) | PASS | PASS |
| Axes in body, impossibility in label only (V-68) | PARTIAL | PARTIAL |
| Impossibility in body, axes in label only (mirror) | PARTIAL | PARTIAL |
| Both components in produced label only (V-67) | PARTIAL (named) | FAIL |

**Scoring delta:** 410 → 420 pts max. 33 → 34 aspirational. Aspirational subtotal: 340 pts.
nertia mapping, quantified switching cost, per-persona
...
- **34 aspirational (C-09--C-42):** scoring methodology transparency, time-dependent inertia
