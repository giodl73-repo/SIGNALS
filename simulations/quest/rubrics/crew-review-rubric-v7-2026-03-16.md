Written to `simulations/quest/rubrics/crew-review-rubric-v7-2026-03-16.md`.

---

**What changed v6 -> v7:**

Two new aspirational criteria from R7 V-05 (first perfect score):

| ID | Pattern | Source | Extends |
|----|---------|--------|---------|
| C-24 | Criterion-check table embedded in VALIDATE phase | V-05 R7 | C-23 |
| C-25 | Exhaustive anti-pattern coverage across all schema columns | V-05 R7 | C-19 |

**C-24** -- The VALIDATE phase (C-23) now must contain a structured self-verification table listing every designed-to-satisfy aspirational criterion with YES/NO/PARTIAL status and evidence citations. The phase becomes a meta-criterion checkpoint, not just a column-contract gate. V-02 and V-04 had a VALIDATE phase but no criterion-check table -- C-23 PASS, C-24 FAIL. V-05 had both -- C-23 PASS, C-24 PASS.

**C-25** -- Every schema column carries an anti-pattern exclusion (extends C-19's "at least one"). V-05's 6-column schema includes exclusions on Slot ("NOT: non-integer labels") and Severity ("NOT: combinations") -- columns that prior variations left bare. A schema where 5 of 6 columns carry anti-patterns does not pass C-25.

**Scoring:** 14 aspirational x 2.5 = 35, max 155. Golden threshold unchanged at 80.

**R7 scores under v7:** V-01 = 145.0 | V-02 = 147.5 | V-03 = 145.0 | V-04 = 147.5 | **V-05 = 155.0 (perfect)**

**Chain extensions:**
- Validation: C-13 -> C-23 -> **C-24**
- Schema anti-pattern: C-19 -> **C-25**
ma becomes a complete failure-mode specification: every cell type
documents what a bad value looks like before the model writes one.

**Scoring:** Aspirational 14 x 2.5 = 35, max 155 (was 150). Golden threshold unchanged
at 80.

**R7 scores under v7:**
- V-01: 60 + 60 + 25.0 = **145.0** (C-24 FAIL -- no VALIDATE phase; C-25 FAIL -- anti-patterns in 3 of 5 columns only)
- V-02: 60 + 60 + 27.5 = **147.5** (C-24 FAIL -- VALIDATE phase present but no criterion-check table; C-25 FAIL -- Lens + Findings only)
- V-03: 60 + 60 + 25.0 = **145.0** (C-24 FAIL -- no discrete VALIDATE phase; C-25 FAIL -- Lens + Findings only)
- V-04: 60 + 60 + 27.5 = **147.5** (C-24 FAIL -- VALIDATE phase present but no criterion-check table; C-25 FAIL -- Role + Findings + Recommendation only)
- V-05: 60 + 60 + 35.0 = **155.0** (C-24 PASS + C-25 PASS -- perfect score)

**New relationship chains:**
- Validation phase chain extended: C-13 (visible validation) -> C-23 (validation as named
  phase) -> C-24 (named phase contains embedded criterion-check audit).
- Schema anti-pattern chain extended: C-19 (at least one column names its anti-form) ->
  C-25 (every column names its anti-form -- exhaustive failure-mode coverage).
...
R4. C-22 through C-23 are from R6. C-24 through C-25 are new from R7.
...
|----------|--------|--------|--------|-----------|---------|
| All pass | 5/5 | 6/6 | 14/14 | 60+60+35=155 | YES |
| All essential + 2 recommended | 5/5 | 2/6 | 0/14 | 60+20+0=80 | YES (at threshold) |
| All essential + all recommended | 5/5 | 6/6 | 0/14 | 60+60+0=120 | YES |
| All essential + 1 recommended | 5/5 | 1/6 | 0/14 | 60+10+0=70 | NO |
| 4 essential + all recommended | 4/5 | 6/6 | 0/14 | 48+60+0=108 | NO (essential gap) |
| All essential + 3 rec + 3 asp | 5/5 | 3/6 | 3/14 | 60+30+7.5=97.5 | YES |
| V-01 R7 equivalent | 5/5 | 6/6 | 10/14 | 60+60+25.0=145.0 | YES |
| V-02 R7 equivalent | 5/5 | 6/6 | 11/14 | 60+60+27.5=147.5 | YES |
| V-03 R7 equivalent | 5/5 | 6/6 | 10/14 | 60+60+25.0=145.0 | YES |
| V-04 R7 equivalent | 5/5 | 6/6 | 11/14 | 60+60+27.5=147.5 | YES |
| V-05 R7 equivalent | 5/5 | 6/6 | 14/14 | 60+60+35.0=155.0 | YES |
...
- C-24 and C-25 are construction-time excellence signals from R7 V-05 (first perfect score:
  150.0 under v6 / 155.0 under v7). C-24 extends C-23: the named VALIDATE phase becomes
  a meta-criterion checkpoint, embedding a structured table that audits all designed-to-
  satisfy aspirational criteria with YES/NO/PARTIAL status and evidence citations before
  the final matrix is written. C-25 extends C-19: the at-least-one-column anti-pattern
  exclusion pattern is applied exhaustively to every column -- including structural columns
  like Slot -- making the schema a complete failure-mode specification.
...
- The full relationship chains:
  ...
  - Validation phase: C-13 (visible validation -- any mechanism) -> C-23 (validation as
    named discrete phase -- structural elevation mirroring C-09 -> C-15) -> C-24
    (named phase contains embedded criterion-check audit -- the phase becomes a meta-
    criterion checkpoint).
  - Each step in each chain makes the prior criterion correct-by-construction rather than
    by instruction.
- V-01 R7 equivalent: E=60, R=60, A=C-11 PASS(2.5)+C-12 FAIL(0)+C-13 PASS(2.5)+
  C-15 PASS(2.5)+C-16 PASS(2.5)+C-17 PASS(2.5)+C-18 PASS(2.5)+C-19 PASS(2.5)+
  C-20 PASS(2.5)+C-21 PASS(2.5)+C-22 PASS(2.5)+C-23 FAIL(0)+C-24 FAIL(0)+
  C-25 FAIL(0) = 25.0. Composite = 145.0.
- V-02 R7 equivalent: E=60, R=60, A=C-11 PASS(2.5)+C-12 PASS(2.5)+C-13 PASS(2.5)+
  C-15 PASS(2.5)+C-16 PASS(2.5)+C-17 PASS(2.5)+C-18 PASS(2.5)+C-19 PASS(2.5)+
  C-20 PASS(2.5)+C-21 PASS(2.5)+C-22 FAIL(0)+C-23 PASS(2.5)+C-24 FAIL(0)+
  C-25 FAIL(0) = 27.5. Composite = 147.5.
- V-03 R7 equivalent: E=60, R=60, A=C-11 PASS(2.5)+C-12 PASS(2.5)+C-13 PASS(2.5)+
  C-15 PASS(2.5)+C-16 PASS(2.5)+C-17 PASS(2.5)+C-18 PASS(2.5)+C-19 PASS(2.5)+
  C-20 PASS(2.5)+C-21 PASS(2.5)+C-22 FAIL(0)+C-23 FAIL(0)+C-24 FAIL(0)+
  C-25 FAIL(0) = 25.0. Composite = 145.0.
- V-04 R7 equivalent: E=60, R=60, A=C-11 PASS(2.5)+C-12 FAIL(0)+C-13 PASS(2.5)+
  C-15 PASS(2.5)+C-16 PASS(2.5)+C-17 PASS(2.5)+C-18 PASS(2.5)+C-19 PASS(2.5)+
  C-20 PASS(2.5)+C-21 PASS(2.5)+C-22 PASS(2.5)+C-23 PASS(2.5)+C-24 FAIL(0)+
  C-25 FAIL(0) = 27.5. Composite = 147.5.
- V-05 R7 equivalent: E=60, R=60, A=C-11 PASS(2.5)+C-12 PASS(2.5)+C-13 PASS(2.5)+
  C-15 PASS(2.5)+C-16 PASS(2.5)+C-17 PASS(2.5)+C-18 PASS(2.5)+C-19 PASS(2.5)+
  C-20 PASS(2.5)+C-21 PASS(2.5)+C-22 PASS(2.5)+C-23 PASS(2.5)+C-24 PASS(2.5)+
  C-25 PASS(2.5) = 35.0. Composite = 155.0. Perfect score.
- Scoring formula in v7: Essential 5 x 12 = 60, Recommended 6 x 10 = 60, Aspirational
  14 x 2.5 = 35. Max is 155. Golden threshold is composite >= 80 with all essential
  passing.