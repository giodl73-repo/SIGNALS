Written to `simulations/quest/rubrics/campaign-simulate-rubric-v7-2026-03-16.md`.

---

**Two patterns extracted from Round 6:**

**C-24 — Compliance Checklist Uses Sub-Claim Architecture for Multi-Part Axes**
Source: V-05 vs V-04 delta. Both score 100.0, but V-05 is auditably superior because its compliance checklist decomposes the observational discipline axis into `fired/partial/not-fired` sub-claim verdicts — and a partial verdict names the failing sub-claim. V-04's binary PASS is opaque. Gap over C-18: C-18 requires checklist + evidence citations; C-24 requires sub-claim granularity for multi-part axes.

**C-25 — T-1 Evidence Co-Located at Execution Scope**
Source: V-02 vs V-03 structural divergence. Both pass C-23, but V-03 embeds T-1 evidence in a per-scope Disposition column at each gate; V-02 defers it to a post-hoc T-1 ANNEX. Per-scope co-location makes T-1 an execution trace verifiable in place, not a claim requiring cross-reference. Gap over C-23: C-23 requires evidence T-1 fired somewhere; C-25 requires that evidence to live at the scope where it was evaluated.

**Scoring update:**
- 15 → 17 aspirational criteria
- Raw max: 110 → 114
- Divisor: 1.10 → 1.14
- Normalized max: 100 (unchanged)
iteria → raw max 114, divisor 1.14.

---

```markdown
# Quest Rubric — campaign-simulate (v7)

## Rubric Version

**v7** — Adds C-24 and C-25 extracted from Round 6 excellence signals.
Prior history: C-22/C-23 extracted from Round 5 (v6); C-19/C-20/C-21 from Round 4 (v5);
C-16/C-17/C-18 from Round 3 (v4); C-09–C-15 from earlier rounds.
...
| Aspirational | C-09–C-25 (17 criteria) | 2 | 34 |
| **Raw max** | | | **114** |
| **Divisor** | | | **1.14** |
| **Normalized max** | | | **100** |
...
## Aspirational Criteria (34 points total, 2 pts each)
...
- **C-24** closes the gap between "compliance checklist present with evidence" (C-18) and
  "compliance assertions are self-auditing for multi-part axes." V-05's `fired/partial/
  not-fired` decomposition is the signal — a binary PASS for a three-sub-claim axis cannot
  tell you which sub-claim is deficient. The sub-claim architecture makes partial compliance
  actionable, not just observable.

- **C-25** closes the gap between "T-1 was applied somewhere" (C-23) and "T-1 evidence is
  traceable to the execution scope where it was evaluated." V-03's per-scope Disposition
  column is the signal — a post-hoc ANNEX satisfies C-23 but requires the reader to
  cross-reference execution to verify. Per-scope co-location makes T-1 an execution trace,
  not a post-hoc claim.
