**`topic-retro-rubric-v12-2026-03-17.md`** written. Here is the complete updated rubric:

---

```
skill: quest-rubric | skill_target: topic-retro | version: 12
```

**Two new criteria extracted from Round 13 (V-05 bidirectional audit cross-linking):**

**C-34 -- Phase 8 SEAL Items Cross-Reference Contract Mechanism** (`phase-8-seal-items-cross-reference-contract`)
- Source: V-05 differentiator (Round 13)
- C-32 requires per-field independent verification instructions; C-34 requires each instruction also name the originating contract/phase. V-05 appends `Cross-reference: PRE-EXECUTION CONTRACT [mechanism]` to every CHECK/HOW block, making the failure path bidirectional: Phase 8 SEAL failure → navigate directly to the responsible mechanism. Independent verification present (C-32 PASS) but no cross-reference to origin = PARTIAL.

**C-35 -- Design Guarantees Table Has One Row Per SEAL Field** (`design-guarantees-table-one-row-per-seal-field`)
- Source: V-05 differentiator (Round 13)
- C-33 requires mechanism-keyed tabular form; C-35 requires at least one row per Phase 8 SEAL field (not SEAL fields grouped under a single row). V-05 expands to 12 rows, splitting the copy guard into This score / COPY-VERIFIED / COPY-SOURCE rows individually — each SEAL item's structural guarantee becomes independently locatable without reading Phase 8. Table present in mechanism-keyed form (C-33 PASS) but SEAL fields collapsed = PARTIAL.
...
Denominators: non-AMEND = **140**, AMEND/no-exclusions = **142**, AMEND/exclusions = **144**.
tement of a failed
  prediction fails.
...
## Aspirational Criteria (54 points total)
...
- **Weight**: aspirational  **Category**: correctness
- **Source**: V-05 differentiator (Round 13)
- **Pass condition**: Each Phase 8 SEAL item carries a named cross-reference to the contract
  phase or mechanism that should have guaranteed the value being verified. V-05 demonstrates
  this by appending `Cross-reference: PRE-EXECUTION CONTRACT [mechanism]` to each CHECK/HOW
  block, creating bidirectional auditability: Phase 8 references the contract that produced
  the value, and a reviewer tracing a SEAL failure can navigate directly to the responsible
  mechanism without re-reading all phases. C-32 requires per-field independent verification
  instructions; C-34 requires each verification instruction also name the originating
  contract or phase. Independent verification instruction present (C-32 PASS) but no cross-
  reference to origin mechanism = PARTIAL. Each SEAL field carries its own verification
  instruction and a named cross-reference to the contract mechanism = PASS. (2 pts full /
  1 pt partial)

### C-35 -- Design Guarantees Table Has One Row Per SEAL Field
- **Weight**: aspirational  **Category**: format
- **Source**: V-05 differentiator (Round 13)
- **Pass condition**: The design-guarantees table enumerates at least one row per Phase 8
  SEAL field -- not grouping all SEAL fields under a single "Copy guard" row. V-05 expands
  the table to 12 rows, splitting the copy guard into one row per SEAL field (This score /
  COPY-VERIFIED / COPY-SOURCE), making each SEAL item's structural guarantee independently
  locatable in the design-guarantees section without navigating Phase 8. This closes the
  loop between C-33 (mechanism-keyed tabular form) and C-32 (per-field independent
  verification): the table proves each SEAL field has a named structural mechanism backing
  it. C-33 requires mechanism names as row keys; C-35 requires the table cover each SEAL
  field individually. Mechanism-keyed table present (C-33 PASS) but SEAL fields collapsed
  under a single row = PARTIAL. One table row per SEAL field with independent mechanism
  names = PASS. (2 pts full / 1 pt partial)
...
Effective denominator in non-AMEND runs = 140. In AMEND runs with no per-phase
exclusions = 142. In AMEND runs with per-phase exclusions = 144.
...
|---------|-----------|--------|--------|
| v12 | C-34 phase-8-seal-items-cross-reference-contract | V-05 differentiator (Round 13) | V-05 appends `Cross-reference: PRE-EXECUTION CONTRACT [mechanism]` to each Phase 8 CHECK/HOW block -- bidirectional auditability: a SEAL failure navigates directly to the contract mechanism that was supposed to prevent it; C-32 requires per-field independent instructions, but V-05 shows the excellence is closing the loop from Phase 8 back to Phase 1 |
| v12 | C-35 design-guarantees-table-one-row-per-seal-field | V-05 differentiator (Round 13) | V-05 expands the DESIGN GUARANTEES table to 12 rows by splitting the copy guard into one row per SEAL field (This score / COPY-VERIFIED / COPY-SOURCE) -- C-33 requires mechanism-keyed tabular form, but V-05 shows the excellence is per-field row coverage so each SEAL item's structural guarantee is independently locatable without reading Phase 8 |
