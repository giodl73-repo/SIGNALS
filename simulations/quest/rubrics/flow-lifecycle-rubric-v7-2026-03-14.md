Written to `simulations/quest/rubrics/flow-lifecycle-rubric-v7-2026-03-14.md`.

---

**1 new criterion extracted from R6 excellence patterns:**

**C-27 — Evidence Field Axis Dual-Location Separation** (format)

R6 V-03 isolates the gap: preamble-only labeled axes (`Required format:` / `Fail condition:`) pass C-26, while per-block carries only "using the format above" — no restated labeled axes. R6 V-02 closes it: labeled axes in both locations achieves 100. C-27 encodes this dual-location requirement.

The escalation chain is now symmetric across both evidence field properties:

| Base criterion | Dual-location escalation |
|----------------|--------------------------|
| C-22 — concrete domain example (preamble sufficient) | C-24 — concrete example in both preamble + per-block |
| C-26 — labeled axes (preamble sufficient) | C-27 — labeled axes in both preamble + per-block |

**Formula**: `/18` → `/19`; aspirational total remains 10 pts.

**Full dependency chain (v7):**
- C-18→C-15; C-19→C-16; C-20→C-14+C-16; C-21→C-13+C-15+C-17
- C-22→C-19; C-23→C-20
- C-24→C-22; C-25→C-23; C-26→C-22
- **C-27→C-26**
sition Coverage -- every state has named entry condition + at least one labeled exit | correctness |
