Looking at the scorecard, two excellence patterns emerge that aren't captured by C-01 through C-24:

1. **V-01's gap (C-04/C-19 PARTIAL)** reveals that multi-role designs need an explicit stage-to-role assignment map in the scaffold — otherwise stages fall through the cracks
2. **V-02's PHASE 0** introduces a pre-trace failure mode prediction register with lifecycle resolution (CONFIRMED/EXONERATED/NEW) — structurally distinct from both C-23 (closure gate) and C-24 (scaffold)

Here is the complete v11 rubric:

---

# flow-dataflow Rubric — v11

**Status**: Active | **Last updated**: 2026-03-15 | **Criteria count**: C-01–C-26
...
| Aspirational | C-08–C-26 | 0.65 | 12.35 |
| **Total** | | | **≈ 98** |
...
## Aspirational Criteria (weight: 12.35 pts total — 0.65 pts each)
...
**Two new aspirational criteria added. Round 13 scorecard (V-01, V-02) surfaces two structural patterns not captured by C-01 through C-24.**

**C-25 — Explicit stage-to-role assignment map** is motivated by V-01's C-04 and C-19 partial scores. V-01's three-role design (Commerce: stages 1–2, Operations: stages 3–5, Finance: SLA/incumbent/recovery) leaves Stage 6 (Inventory Availability Calculation) unassigned to any role section. This is not a scoring nuance within C-24 — C-24 requires a scaffold of output tables, not a map of which role produces which stage trace. A multi-role scaffold can fully satisfy C-24 while leaving a stage without a designated producer. C-25 closes that gap: the scaffold must include a role assignment column, and every C-01 stage must appear in it. The criterion is gated to multi-role designs; single-role traces satisfy it by default.

**C-26 — Pre-trace failure mode prediction register with lifecycle resolution** is motivated by V-02's PHASE 0 design. V-02 introduces a failure mode prediction register before any stage trace, assigns FM-NN identifiers, states domain-specific consequences, and resolves each prediction as CONFIRMED / EXONERATED / NEW after all traces complete. EXONERATED entries appear in the recovery audit with explicit reasons for non-materialization. This behavior is structurally distinct from: C-08 (recovery prescriptions written after trace, not predicted before), C-23 (closure gate checks NH-NN/LP-NN coverage, not forward predictions), and C-24 (scaffold declares output tables, not failure hypotheses). The prediction register is a forward-declared failure hypothesis contract; the resolution section is its audit. No existing criterion requires pre-trace prediction with per-FM-NN lifecycle status.

**v10 criteria C-01 through C-24 are unchanged.** The new max composite is ≈ 98 (56 essential + 30 recommended + 19 × 0.65 aspirational).
