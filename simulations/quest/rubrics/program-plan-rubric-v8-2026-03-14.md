Written to `simulations/quest/rubrics/program-plan-rubric-v8-2026-03-14.md`.

---

**v8 summary — what changed:**

| New criterion | Axis | Discriminates in R7 |
|---------------|------|---------------------|
| **C-28** | Stage displacement register: each stage requires displacement-impossibility justification (why cannot run earlier + what breaks if moved) | V-01/V-05 pass; V-02/V-03/V-04 fail |
| **C-29** | Stage Cohesion Audit Table pre-computes all stage groupings before YAML | V-02/V-05 pass; V-01/V-03/V-04 fail |
| **C-30** | Skill Omission Register: explicit register of excluded skills + exclusion reasons | V-03/V-05 pass; V-01/V-02/V-04 fail |
| **C-31** | Artifact Lineage Chain: explicit artifact-to-consumer tracing across the full plan | V-05 only |

**Scoring:** 185 -> **205 pts** (23 aspirational x 5). Golden threshold: **164** (~80%).

**R7 retroactive under v8:**
- V-05: **205/205** (ceiling)
- V-04: **195/205** (C-28 + C-29)
- V-01/V-02/V-03: **190/205** (one each)

**Design principles:**
- C-28 is to stages what C-15/C-18/C-21 are to actors — displacement-impossibility reasoning was required of actors and of echo (C-26); C-28 extends it to all non-echo stages
- C-29 is the stage-level pre-computation analogue of C-25 (gate cascade audit) and the actor ordering table — the mandatory-pre-work-table pattern covers a third structural layer
- C-30 completes the skill-accounting triangle: C-11 enumerates valid skills (preventing hallucination), C-03 verifies no invalid skills in output, C-30 requires explicit accounting for valid skills that were deliberately excluded
- C-31 extends artifact dependency checking from namespace-ordering verification (C-05) to full artifact-to-consumer lineage tracing, making the dependency graph explicit as a structure

**R8 gap:** C-31 (artifact lineage chain) is the single-variation signal, making it the natural R8 anchor.
all 47 skills inline to prevent hallucination, but leaves skill exclusion implicit; C-30 makes exclusion an explicit design act. C-31 extends the multi-hop artifact-ordering check (C-05 verifies namespace ordering; C-31 requires full artifact-to-consumer lineage tracing, making the artifact dependency graph visible as a structure rather than inferring it from skill position alone).

**Scoring:** 115 pts aspirational (23 x 5) -> **205 total** -> golden threshold **164** (~80% of max).

**R7 retroactive scores under v8:**

| Variation | Score | Notes |
|-----------|-------|-------|
| V-01 | 190/205 | Pass C-28 (stage displacement register); fail C-29 (no stage cohesion audit table), C-30 (no skill omission register), C-31 (no artifact lineage chain) |
| V-02 | 190/205 | Pass C-29 (stage cohesion audit table pre-computes all stage groupings); fail C-28 (no stage displacement justification), C-30 (no skill omission register), C-31 (no artifact lineage chain) |
| V-03 | 190/205 | Pass C-30 (skill omission register present); fail C-28, C-29, C-31 |
| V-04 | 195/205 | Pass C-28 + C-29 (V-01 + V-02 combination); fail C-30, C-31 |
| V-05 | 205/205 | Pass all four: C-28 (from V-01 axis) + C-29 (from V-02 axis) + C-30 (from V-03 axis) + C-31 (artifact lineage chain) -- CEILING |
...
Weight per aspirational: 5 pts (23 criteria x 5 = 115 pts)
...
Aspirational: C-09 [ ] C-10 [ ] C-11 [ ] C-12 [ ] C-13 [ ]
              ...
              C-29 [ ] C-30 [ ] C-31 [ ]
              Pass count: ___ / 23  -> ___ * 5  = ___ pts (of 115)

Composite score: ___ / 205

Golden: all essential pass AND composite >= 164  ->  [ ] YES  [ ] NO
```
