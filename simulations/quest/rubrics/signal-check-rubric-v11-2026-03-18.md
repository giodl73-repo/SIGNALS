Written to `simulations/quest/rubrics/signal-check-rubric-v11-2026-03-18.md`.
...
|----|-----------|----------|---------|
| C-35 | Failure class encoded directly in the top-level rule heading, making diagnostic taxonomy recoverable from top-level heading scan without entering the ENFORCEMENT STACK NOTE | diagnostic-heading | C-34 -> C-21 |
| C-36 | Reader-position named explicitly in function framing -- specific role named for each register ("a committing engineer reading for what to fix" / "a reviewer reading for what is already lost"), removing generic "you" ambiguity | reader-position-framing | C-33 -> C-29 -> C-26 -> C-24 -> C-22 -> C-20 |

**Key structural notes:**

- **C-35 vs C-34**: C-34 passes if the failure class appears in an ENFORCEMENT STACK NOTE inline label; C-35 requires it in the top-level rule heading itself. Same information, different structural depth -- independently testable.
- **C-35 vs C-28**: C-28 tests the existence assertion in the heading; C-35 tests the failure class in the heading. A heading can carry both (`Rule 1 -- Absence Drift -- ABSENCE MUST BE DECLARED:`), either, or neither.
- **C-36 vs C-33**: C-33 requires action-spec / loss-model characterization with generic "you"; C-36 resolves "you" to a named role. An output passing C-33 with "tells you what to do" fails C-36 if no reader position is named.
- **Denominator**: 26 -> **28**; formula `composite = 90 + (aspirational_pass / 28) * 10`.
STACK NOTE. An output satisfying C-35 automatically satisfies C-34 (and therefore C-21); the converse is not true.

**C-36** tightens C-33: C-33 requires the non-substitutability declaration to map each register to the reader's decision logic using action-spec / loss-model framing but permits generic "you"; C-36 requires the reader's decision logic to name a specific reader position (e.g., "a committing engineer reading for what to fix" / "a reviewer reading for what is already lost"), eliminating ambiguity for multi-role teams. An output satisfying C-36 automatically satisfies C-33 (and therefore C-29, C-26, C-24, C-22, C-20); the converse is not true. C-36 is independently testable from C-28 and C-35.

**Updated parameters**: denominator 26 -> **28**, formula `composite = 90 + (aspirational_pass / 28) * 10`.
...
| ID | Criterion | Source |
|----|-----------|--------|
| C-35 | Failure class encoded directly in the top-level rule heading, making diagnostic taxonomy recoverable from a top-level heading scan without entering the ENFORCEMENT STACK NOTE block | R10-EX-01 |
| C-36 | Reader-position named explicitly in function framing -- naming a specific reader role and decision act for each register rather than using generic "you" | R10-EX-02 |

## Updated parameters

- Aspirational count: 26 -> **28**
- Formula: `composite = 90 + (aspirational_pass / 28) * 10`
- C-35 automatically satisfies C-34 (and therefore C-21)
- C-36 automatically satisfies C-33 (and therefore C-29, C-26, C-24, C-22, C-20)
- C-35 is independently testable from C-36 (heading-level diagnostic vs. body-level reader-position)
- C-35 is independently testable from C-28 (failure-class label vs. existence-assertion claim -- a heading could encode both, either, or neither)

## Subsumption chains

```
C-20 <- C-22 <- C-24 <- C-26 <- C-29 <- C-33 <- C-36
                C-24 <- C-27 <- C-30
C-20 <- C-23 <- C-25
C-22 <- C-31
C-20 <- C-32
...
```
composite = 90 + (aspirational_pass / 28) * 10
```
