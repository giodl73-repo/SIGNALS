Written to `simulations/quest/rubrics/listen-support-rubric-v6-2026-03-14.md`.

---

**What changed v5 -> v6:**

Four new aspirational criteria extracted from Round 5 PASS+ signals:

| ID | Name | Tightens | Source | Who satisfies |
|----|------|----------|--------|---------------|
| C-20 | Vocabulary Pre-Commitment Table | C-02 | V-05 C-02 PASS+ | V-05 only |
| C-21 | Named-Role Third-Person Prohibition | C-19 | V-05 C-03 PASS+ | V-05 only |
| C-22 | Phase-Character Pre-Constraint Table | C-06/C-07 | V-01/V-04 C-06/C-07 PASS+ | V-01, V-04 |
| C-23 | Dual-Pass Frontmatter Verification | C-17 | V-05 C-13 PASS+ | V-05 only |

**Tier restructure:** Aspirational 11 criteria / 55 pts -> 15 criteria / 75 pts. Total ceiling 145 -> **165**.

**New progression chains:**
- `C-02 -> C-20`: vocabulary compliance (post) -> vocabulary pre-lock (pre-generation)
- `C-19 -> C-21`: generic pronoun prohibition -> named-role title prohibition
- `C-06/C-07 -> C-22`: defensible calibration -> phase-map pre-constraint before tickets are written
- `C-17 -> C-23`: single verification pass -> dual-pass with non-overlapping failure surfaces

**C-21 -- Named-Role Third-Person Prohibition** (5 pts, behavior)
Tightens C-19. C-19 prohibits generic third-person forms ("the user", "they"); C-21 extends the prohibition to named-role forms ("the SRE", "the PM", "the customer", "the engineer"). The failure mode C-19 permits: a model in performance mode can substitute named-role titles for generic pronouns while maintaining an analyst stance -- "The SRE ran the kubectl command" is third-person distancing but contains neither "the user" nor "they", so it satisfies C-19. The named-role prohibition closes this: violations are grep-checkable by role title, and no named-role form can satisfy a prohibition that explicitly enumerates it. V-05 satisfies C-21 ("Do not write 'the user', 'they', 'the SRE', 'the PM'..."); V-01 through V-04 fail it.

**C-22 -- Phase-Character Pre-Constraint Table** (5 pts, depth)
Tightens C-06 and C-07. C-06 and C-07 pass when severity and volume assignments are defensible and differentiated after generation; C-22 requires a phase map table generated before tickets that assigns severity range and volume character per phase window (e.g., "Phase 1: severity P0/P1 expected, volume high; Phase 3: severity P2/P3 expected, volume medium/low"). The failure mode post-generation calibration permits: a model can assign uniform P2/medium across all tickets and construct prose defenses without any structure making the uniformity structurally implausible. A phase map table makes uniformity a violation before a single ticket is written: "uniform medium" contradicts a Phase 1 row that explicitly states high volume expected. The constraint is pre-generated, not derived from tickets. V-01 and V-04 satisfy C-22; V-02, V-03, and V-05 (no phase axis) fail it.

**C-23 -- Dual-Pass Frontmatter Verification** (5 pts, depth)
Tightens C-17. C-17 requires one verification pass (3-column gap-bridged table + no-orphan check); C-23 requires a second independent verification pass confirming that the pre-committed summary table (C-20) matches every ticket ID, phase label, category, persona, volume, and severity in the full card bodies. The two passes have non-overlapping failure surfaces: pass 1 (coverage table) catches missing STATUS QUO traces and orphaned gaps; pass 2 (frontmatter verify) catches post-generation vocabulary drift and phase/persona inconsistencies that the coverage table cannot see. A model can produce a coverage table that checks out while a card body has drifted from the pre-committed vocabulary; only a second pass anchored to the pre-committed values catches this. C-17 passes with one verification pass; C-23 passes only when both passes are present and structurally independent. V-05 satisfies C-23; V-01 through V-04 fail it.

**Tier restructure:**
- Aspirational: 11 criteria / 55 pts -> 15 criteria / 75 pts
- Total ceiling: 145 -> 165

**Criterion progression chains:**
- Depth: C-02 (compliance) -> C-20 (pre-commitment table before prose)
- Behavior: C-19 (generic third-person prohibition) -> C-21 (named-role prohibition)
- Depth: C-06/C-07 (defensible assignments) -> C-22 (phase-character pre-constraint table)
- Depth: C-13 (gate) -> C-15 (table) -> C-17 (gap-bridged) -> C-23 (dual-pass frontmatter verify)

**Relationship to prior criterion pairs:**
- C-02 (controlled vocabulary compliance) -> C-20 (pre-commitment table): moves vocabulary verification from post-generation compliance check to pre-generation structural lock
- C-19 (mode declaration + generic prohibition) -> C-21 (named-role prohibition): closes the role-title third-person failure mode that generic pronoun prohibition leaves open
- C-06/C-07 (defensible/differentiated assignments) -> C-22 (phase-character pre-constraint): moves calibration from post-generation defense to pre-generation constraint table
- C-17 (gap-bridged single-pass verification) -> C-23 (dual-pass frontmatter verify): adds a second non-overlapping verification pass anchored to pre-committed vocabulary values
...
## Aspirational Criteria (75 pts total -- raises the bar above 90)
...
**Essential ceiling**: 60 | **Recommended ceiling**: 90 | **Aspirational ceiling**: 165
