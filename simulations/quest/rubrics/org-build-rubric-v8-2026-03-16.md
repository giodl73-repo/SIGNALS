Reading the scorecard for extraction patterns before writing the updated rubric.

The scorecard surfaces two new patterns not yet encoded as criteria:

1. **Depth-flag-conditional count floor** (R10 C-04 failure): declaring a depth flag is insufficient — Phase 1 must bind the count range explicitly to the flag value. This is distinct from C-04 (which tests output count) because it tests the phase *instruction* mechanism.

2. **Block-internal ordering anchor** (R10 C-26 failure, labeled `preamble-as-unification-declaration-step-toward-C26`): C-26 requires the record block to be the unification artifact; C-28 tests the specific mechanism — the FORBIDDEN must be a named first *field inside the block*, not preamble text that asserts unification. A block satisfying C-26 in principle can fail C-28 operationally if the anchor is preamble-resident.

C-22's skeleton gap is already fully captured by C-22's pass condition ("a typed variable declaration with no corresponding template slot fails") — R10 correctly scored it FAIL. No new criterion needed there.
...
```
composite = (essential_pass/5 * 60) + (recommended_pass/3 * 30) + (aspirational_pass/20 * 10)
```
```
...
| Change | Rationale |
|--------|-----------|
| +C-27 (depth-flag-conditional count floor) | R10 C-04 failure: depth flag was declared but count constraint was absent from the phase instruction. C-04 tests output count; C-27 tests whether the phase instruction itself encodes the flag-conditional binding. |
| +C-28 (ordering anchor as block-first named field) | R10 C-26 failure labeled `preamble-as-unification-declaration-step-toward-C26`: the PREAMBLE asserted unification but the `=== RECORD ===` block contained no PHASE-ORDERING-GUARD field. C-26 is the design principle; C-28 is the specific mechanism that satisfies it — the anchor must be block-internal, not preamble-resident. |
| Denominator 18 → 20 | Two new aspirational criteria added. |
| C-22 unchanged | R10 C-22 failure was correctly caught by the existing pass condition ("a typed variable declaration with no corresponding template slot fails"). No new criterion needed — the signal is a reminder to prompt builders, not a rubric gap. |
