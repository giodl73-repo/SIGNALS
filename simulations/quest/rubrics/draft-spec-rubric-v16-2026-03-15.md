...
version: v16
date: 2026-03-15
source_rounds: R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11, R12, R13, R14, R15, R16
---

# Rubric — `draft-spec` — v16
...
| Composite formula denominator | 38 | 39 |

**One new aspirational criterion from R16:**

| C | Name | Axis | Category |
|---|------|------|----------|
| C-47 | Multi-input ENTER block absorbs fractional sub-phase EXIT tokens into the chain | C-44, C-39 | structure |

**N/A scope changes v15 → v16**: C-47 added to the fractional-sub-phase N/A group (joining C-38, C-39, C-40). A template without a named fractional sub-phase emitting a second pre-Phase-2 gate token now has C-38, C-39, C-40, C-47 inapplicable (4 criteria, up from 3).

**R16 excellence signal validation**: R16 V-05 confirmed C-40, C-45, C-46 as independently scoreable and architecturally sound. The new pattern surfaces when a fractional sub-phase (C-39) also participates in ENTER/EXIT ceremony: the next numbered phase's ENTER block must accumulate EXIT tokens from ALL immediate predecessor phases — both numbered and fractional — to preserve the C-44 unbroken chain. C-44 is evaluated against the five numbered phases only and does not require fractional sub-phase absorption; C-47 closes that gap.
