Reading the scorecard carefully for patterns not yet captured by v5 criteria.

**V-02 distinctive behaviors vs V-01:**

1. **Two repair loops, not one** — V-02's C-20 evidence explicitly shows two independent back-pressure loops: one for interdependency count, a separate one for type diversity. C-20 only requires "at least one." V-01 has one loop (108/114); V-02 has two (112/114). The second loop is what gets V-02 its C-18 pass.

2. **From/To columns are type-constrained at cell level** — V-02's interdependency table enforces `{HIGH, MEDIUM, LOW}` as a column-level vocabulary rule on each of the From and To columns independently. C-17 requires the prose label ("FROM... TO...") but doesn't require column-level type enforcement. V-02's approach is machine-testable per cell; C-17's is per-sentence.

These are genuine escalations on C-20 and C-17 respectively, grounded in V-02 evidence.

---

# Scout-Risk Rubric — v6
...
## Aspirational Criteria (weight: 2 pts PASS / 1 pt PARTIAL / 0 pts FAIL each, 28 pts max)
...
| Outcome | Score Range |
|---------|-------------|
| Golden (all essential pass, composite >= 80) | 80–118 |
| Passing (all essential pass, composite < 80) | 60–79 |
| Failing (any essential fails) | 0–59 |

**Max composite**: 60 (essential) + 30 (recommended) + 28 (aspirational) = **118**

**Golden threshold**: all 5 essentials PASS **and** composite >= 80 (unchanged from v5).
...
| Variate | Essential | Recommended | Aspirational | Total | Verdict |
|---------|-----------|-------------|--------------|-------|---------|
| V-01 | 60/60 | 30/30 | C-09..C-13(10)+C-14(0)+C-15(0)+C-16(2)+C-17(2)+C-18(0)+C-19(2)+C-20(2)+C-21(0)+C-22(0) = 18/28 | **108/118** | GOLDEN |
| V-02 | 60/60 | 30/30 | C-09..C-13(10)+C-14(0)+C-15(2)+C-16(2)+C-17(2)+C-18(2)+C-19(2)+C-20(2)+C-21(2)+C-22(2) = 26/28 | **116/118** | GOLDEN |
| V-03 | ~40/60 | 0/30 | 0/28 | ~40/118 | FAILING |

---

## Criterion Chain Summary

| Chain | C-05 → C-11 → C-19 |
|---|---|
| Mitigations actionable | zero-generic | zero-generic by enumeration |

| Chain | C-07 → C-12 → C-14 |
|---|---|
| Likelihood beyond binary | all trigger-qualified | IF-THEN syntactic form |

| Chain | C-09 → C-13 → C-16 → C-17 → C-22 |
|---|---|
| At least one dependency | structured section ≥2 | section ≥3 pairs | FROM/TO labels | FROM/TO type-constrained columns |

| Chain | C-11 → C-15 → C-18 |
|---|---|
| Zero-generic | positively typed | type portfolio ≥3 distinct |

| Chain | C-20 → C-21 |
|---|---|
| At least one repair loop | one loop per gate |
