Written to `simulations/quest/rubrics/scout-stakeholders-rubric-v8-2026-03-14.md`.

---

**Two new criteria extracted from Round 7:**

**C-24 — Amendment mandate inline prohibition** (depth, 5 pts)
Source: V-01 vs V-02 C-12 evidence. Both pass C-12 and C-22 with identical target lists. V-01 adds `-- no observation without revision` to the mandate sentence; V-02 omits it. Distinct from C-17 (FAIL_OBSERVATION_ONLY at the Step 8 gate): the gate label and the mandate-level prohibition are independently satisfiable. A mandate that enumerates all structural levels but states no inline prohibition = FAIL.

**C-25 — Veto risk mitigation per entry** (depth, 5 pts)
Source: V-01 and V-02 C-17 evidence. Both show `FAIL_NO_MITIGATION` at Step 4 alongside `FAIL_WRONG_ORDER`, confirming each veto risk entry must include a mitigation strategy alongside probability and impact. C-03 requires probability + impact; neither C-03 nor C-17 (gate coverage) captures the mitigation field requirement. A veto risk table with probability + impact but no mitigation column = FAIL.

**Updated totals**: aspirational 75 → 85 pts; max 165 → 175 pts. Golden threshold (>= 80) unchanged.

**Round 7 scores under v8**: V-01 = 175/175 (gains both), V-02 = 160/175 (fails C-15, C-23, C-24; gains C-25), V-03 = partial (TBD on C-24/C-25).
 The gate label FAIL_NO_MITIGATION fires when mitigation is absent from any entry. Neither C-03 (structural presence) nor C-17 (gate coverage) captures this requirement -- a veto risk table can satisfy both with probability + impact columns and a FAIL gate while still omitting mitigation from its structural requirements.

Round 7 also confirms three structural properties:
...
**Aspirational points**: 5 pts each x 17 = **85 pts**
...
**Max score**: 175 pts (60 essential + 30 recommended + 85 aspirational)
...
## Round 7 Results Summary (scored under v7 rubric, max 165)

| Variation | Focus | Score (v7) | C-22 | C-23 | C-24 | C-25 |
|-----------|-------|------------|------|------|------|------|
| V-01 | C-23 alone: role label heading binding (V-05 R6 + headings) | 165/165 | PASS | PASS | PASS | PASS |
| V-02 | Role sequence: PM-first (synthesis before analysis) | 155/165 | PASS | FAIL | FAIL | PASS |
| V-03 | Inertia framing reduced (no C-18/C-20) | 150/165 | PASS | FAIL | TBD | TBD |

Under the v8 rubric (max 175): V-01 scores 175/175 (gains C-24 from "-- no observation without revision" in mandate; gains C-25 from FAIL_NO_MITIGATION at Step 4). V-02 scores 160/175 (fails C-15, C-23, C-24; gains C-25). V-03 partial scorecard; C-25 status depends on FAIL_NO_MITIGATION presence; C-24 status depends on whether the prohibition clause is in the mandate.

The full aspirational target for Round 8 is all seventeen criteria (C-09 through C-25): the v7 complete set plus amendment mandate inline prohibition (C-24) and veto risk mitigation per entry (C-25).
