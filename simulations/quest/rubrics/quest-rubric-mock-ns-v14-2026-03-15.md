...
equivalent -- as a prefix before the row-specific navigation label; C-35 required navigation
labels pointing to preamble table rows; V-01 adds a trigger that re-activates the P-0
two-phase protocol at the exact moment of use, before the row content is written; the
trigger is the use-site counterpart to the C-36 preamble gate -- the preamble defines the
protocol, the use-site echo enforces it at each row; C-37 elevates this behavioral trigger
to a pass condition, depending on C-36; both use sites, S-3 and A-1, must carry the
trigger for a PASS).
...
Scoring denominator: aspirational 29 -> 32. Formula: `aspirational_pass / 32 * 10`.
...
- **Pass condition**: C-36 requires Phase 2 to assert that the resolution matches the
  ...
  record template -- provided in the C-37 echo at each bracket-token row to reduce the
  cognitive cost of record creation from open-form to fill-in-the-blank -- is seeded as the
  aspirational maximum for v15. Artifacts that fail C-38 fail C-40 automatically.
...
| Band | Essential (5x) | Recommended (3x) | Aspirational (32x) |
|------|---------------|-----------------|-------------------|
| Points | 60 | 30 | 10 |
| Formula | pass/5 x 60 | pass/3 x 30 | pass/32 x 10 |
...
- **C-40 fail**: Phase 2 in the preamble is a hard-stop gate (C-38 passes) but carries no
  ...
  preamble where the protocol is defined. The use-site pre-filled record template (a
  structured form embedded in the C-37 echo that the executor fills in to produce the
  record) is an aspirational maximum seeded for v15; absence of the template does not fail
  C-40 if the preamble obligation is present. Fails automatically when C-38 fails.
