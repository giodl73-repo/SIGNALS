...
# Rubric: prove-prototype (v9)

**Updated**: 2026-03-15
**Changes from v8**: Added C-29 and C-30 (Aspirational tier) derived from Round 8 excellence signals. Sharpened C-27 pass condition based on V-02 boundary finding.

**C-27 sharpened (E-1)**: V-02 demonstrated the exact binary. A `**Completeness**: FULL` labeled pair adjacent to the `PHASE N COMPLETE —` line satisfies C-22 but not C-27. The grade token must reside *inside* the `PHASE N COMPLETE —` string. Any format that moves the grade off that line breaks the scan-only auditability property. The 7-point gap between V-02 (180) and V-01 (187) is entirely attributable to this single location constraint.

**C-29** — *Baseline labeled as inertia competitor in baseline container* (E-3a): The baseline container (C-28) must include explicit language framing the baseline as the status-quo alternative the prototype must outperform — not merely a calibration number. When satisfied, the null counter-interpretation ("the prototype achieved what the status-quo would have achieved") is structurally seeded before any build activity. C-29 requires C-28 as a prerequisite.

**C-30** — *Results expressed as three-column comparison table (predicted / observed / baseline)* (E-3b): A structured table whose three columns make all comparisons simultaneously unavoidable, enforcing effect-size quantification structurally. A populated three-column table is a C-12 expression by construction. Prose restatement of the same three values does not pass — prose can omit silently; a table column gap is visible. C-30 extends C-26.

**Point totals**: Aspirational 104 → 118. Total 204 → 218.
...
| Aspirational | C-09, C-10, C-16 through C-30 | 118 |
| Excellence | C-11 through C-15 | 10 |
| **Total** | | **218** |

The full rubric is saved to `simulations/quest/rubrics/prove-prototype-rubric-v9-2026-03-14.md`.
tification in
eight rounds; a populated three-column table is a C-12 expression by construction. V-05's
three-column table (predicted / observed / B-00) is the exemplar; a prose restatement of
the same three values does not pass C-30 because prose does not impose the same structural
completeness guarantee. C-30 extends C-26. Aspirational points raised from 104 to 118.
Total raised from 204 to 218.
...
## Aspirational Criteria (118 points)
...
| C-29 | **Baseline labeled as inertia competitor in baseline container** — The baseline is not merely a reference measurement but is explicitly framed as the status-quo alternative the prototype must outperform, so that the counter-evidence section in CLOSE addresses a named concrete competitor rather than a generic one | depth | aspirational | The baseline container (satisfying C-28) includes explicit language identifying the baseline value as the current approach, status-quo method, or existing alternative — not merely as a calibration measurement. A baseline introduced only as a number (e.g., `B-00 = 42ms`) without framing it as a competitor does not pass. When C-29 is satisfied, the null counter-interpretation ("the prototype achieved what the status-quo would have achieved") is structurally seeded before any build activity, and the CLOSE counter-evidence section is positioned to address this named inertia hypothesis directly rather than constructing a generic alternative retroactively. C-29 requires C-28 as a prerequisite; it is not satisfiable without an isolated baseline container |
| C-30 | **Results expressed as three-column comparison table (predicted / observed / baseline) enabling structural effect-size quantification** — The results section contains a structured table whose predicted, observed, and baseline columns make all three comparisons simultaneously unavoidable and enforce effect-size quantification without requiring a separate prompt | structure | aspirational | A table with at least three columns — one for predicted outcome, one for observed outcome, and one for baseline or control value — appears in the results section, with all three columns populated with values drawn from the run. Column labels may vary (e.g., Expected / Actual / B-00) as long as each column unambiguously maps to one of the three required elements. A table with only two columns (predicted and observed, or observed and baseline) does not pass, because the third comparison is structurally absent. A prose restatement of the same three values does not pass, because prose does not enforce structural completeness — a partially populated table leaves a visible column gap; a prose passage can omit a value without leaving a visible gap. When C-30 is satisfied, a reader scanning only the table headers can confirm that all three comparisons are structurally present. A populated three-column results table is a structural expression of effect size (C-12) by construction: the paired columns together constitute a quantified comparison that does not require a separate estimation step. C-30 extends C-26: an output satisfying C-30 automatically satisfies C-26, but not vice versa |
