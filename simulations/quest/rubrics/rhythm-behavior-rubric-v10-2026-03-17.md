**rhythm-behavior Rubric v10** written to `simulations/quest/rubrics/rhythm-behavior-rubric-v10-2026-03-17.md`.
...
| | v9 | v10 |
|---|---|---|
| Aspirational denominator | 17 | **20** |
| New criteria | -- | C-25, C-26, C-27 |

**C-25 -- C-24 Confirmed-Position List Is Illustrative** (format, R9)
The four positions in C-24 are examples, not a closed enumeration. The operative clause is "any non-buried position." A mid-sub-skill divider-flanked slot (e.g., between trace-state and trace-contract) qualifies on equal footing. Scorers must not treat the list as exhaustive.

**C-26 -- Named Section Bodies Are Non-Disqualifying** (format, R9)
Closes the latent ambiguity in C-22's "buried" concept. Named section bodies (Synthesis body, after If-none terminator, before closing divider) are not disqualifying. The disqualifier set remains strictly: gate body and example block. "Buried" does not extend to named section bodies.

**C-27 -- 3-Slot Template Load-Bearing Elements** (depth, R9)
Operationally validates C-16 minimums. Template string + inline example are required and sufficient. Slot-name definition lines and enforcement notes are decorative -- stripping them leaves C-16 fully satisfied. Scorers must not require them as additional pass conditions.

C-16 and C-24 carry forward R9 notes cross-referencing C-27 and C-25 respectively.
`[LEVEL] because [boundary] propagates to [caller/component], [effect]`) and at least one inline example are required and sufficient. Slot-name definition lines (`[boundary] = ...`, `[caller/component] = ...`, `[effect] = ...`) and an enforcement note ("All three slots must be specific -- no generic phrases.") are decorative relative to C-16. Stripping those lines leaves C-16 fully satisfied. Scorers must not require definition lines or enforcement notes as additional pass conditions.
