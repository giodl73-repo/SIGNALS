Written to `simulations/quest/rubrics/flow-resilience-rubric-v6-2026-03-15.md`.

---

**Three new criteria extracted from Round 5:**

**C-22 -- Typed Nil-Finding Identifiers** (from V-01 C-12 PASS)
C-12 required nil findings with scope rationale. C-22 tightens it: each nil statement carries a gap-type prefix identifier (OEG-NIL, DCV-NIL, MRF-NIL) that binds it to its output target by name. A nil without a typed ID satisfies C-12 but is not queryable by type and doesn't make explicit which of the three output targets is being declared nil. Each identifier is unique within a run (OEG-NIL-1, OEG-NIL-2 if needed).

**C-23 -- Scope Declaration Closure Bracket** (from V-01 C-21 PASS)
C-21 required an upfront scope commitment and checkability. C-23 requires the commitment to be bracketed: an opening `SCOPE DECLARATION COMPLETE` confirmation closes the declaration phase before analysis begins; a terminal `Scope Verification` block closes it after analysis ends, naming each declared operation with its coverage outcome. A declaration with only one block is unverified.

**C-24 -- Template-Embedded Conditional Linkage Rules** (from V-01 C-14 PASS)
C-14 required the DCV linkage to exist. C-24 requires it to be encoded as a named conditional rule (`if verdict is [no | undefined]: append DCV-CR-NN`), not prose guidance. This makes correct cross-section behavior mandatory rather than discretionary. At least two distinct rules required to show the pattern is systemic.

**Scoring delta**: cap unchanged at 10; uncapped aspirational max rises from 26 to 32 (16 criteria × 2).
ure modes: pure prose output, domain-agnostic analysis,
  and trivial recovery paths that add no signal.
...
- **Text**: The output covers all three resilience scenario classes: (1) full connectivity loss
  (client offline), (2) partial failure (dependent service unavailable), and (3) concurrent
  writes across a partition (split-brain / eventual-consistency conflict).
...
Uncapped maximum across 16 criteria is 32; the cap rewards breadth while holding the
...
| Aspirational | 10% | C-09 through C-24 | PASS=2, PARTIAL=1, FAIL=0; capped at 10 |
...
- **Aspirational cap unchanged** at 10; uncapped maximum rises from 26 to 32 with three new
  criteria (16 criteria x PASS=2). Breadth is rewarded; the composite ceiling stays at 100.
