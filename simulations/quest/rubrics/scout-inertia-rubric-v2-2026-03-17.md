Written to `simulations/quest/rubrics/scout-inertia-rubric-v2-2026-03-17.md`.
...
| ID | Criterion | Source |
|----|-----------|--------|
| C-11 | **Inertia-Loss Conditions Include Verification Method** — beyond the threshold (C-07), specifies the concrete action/lookup a team would use to verify it applies. "Check your sprint board and count active projects" vs. "you can check project count." | V-02 "Verifiable How" column, V-05 "How to Verify" column |
| C-12 | **Steelman Rebuttal Anchored to Named Claim** — the rebuttal names the specific claim from the steelman it addresses, making the link traceable. "The advocate argues [X]. This fails because [Y]" vs. "While some teams prefer simplicity..." | V-03 two-role sequence where Role 2 must rebut the strongest named point from Role 1 |
| C-13 | **Workaround Described at Replication Fidelity** — upgrades C-01 from "reader can picture" to "another team could reproduce" — tools named by product, steps enumerated, institutional knowledge surfaced. | V-01/V-03/V-05 all converge on "specific enough to follow" |

**Formula change:** aspirational denominator updated from 2 to 5, keeping the tier max at 10 pts.

**Design note added to Evaluator Notes:** C-10/C-11/C-12 form a quality cluster (floor → anchoring → verification), and C-13 is explicitly framed as an upgrade layer on top of C-01, not a replacement.
ing") or explicit N/A with
