Written to `simulations/quest/rubrics/org-review-rubric-v11-2026-03-16.md`.
...
| ID | Name | Source | What it captures |
|----|------|--------|-----------------|
| **C-33** | Lens Applicability Rating Pre-committed | V-02 R12 | Each LENS COVERAGE TABLE row carries an artifact-specific HIGH/MEDIUM/LOW applicability rating — the rated basis for the ADDRESSED/OPEN determination. C-31 classifies; C-33 requires the evidence underlying the classification. |
| **C-34** | ADVISORY-GAP Domain Coverage Pre-committed | V-02 R12 | After the lens table is built, a gap-check names every artifact domain with no HIGH-applicability reviewer and classifies each as ADVISORY-GAP in the action register. Orthogonal to C-31 (role-outward) — C-34 is domain-inward. |
| **C-35** | Null Hypothesis Challenger Dimension Comparison Table | V-02 R12 | The challenger's null hypothesis evaluation uses a structured current-state vs. proposed-state dimension table, not prose. g_null derivable from table values alone. Distinct from C-16 (bracket-wide shared scaffold) and C-23 (rule completeness). |

**Score changes:**

| | v10 | v11 |
|---|---|---|
| Aspirational criteria | 24 | **27** |
| Max pts | 210 | **225** |
| Gold threshold | 175 | **190** |

Gold delta matches the 3×5=15 pt pattern from prior rounds. V-05 R11 scores 210/225 (unchanged — C-33/C-34/C-35 absent). V-02 R12 is the canonical source at ~215/225.
s over alternatives

**Vacuous conditions added to Scoring Key:** C-33 (C-31 inactive), C-34 (C-31 inactive OR no multi-domain artifact), C-35 (C-09 inactive).

No retroactive score changes on R1--R11. V-02 R12 is the canonical source for C-33/C-34/C-35. Perfect score of 225 requires a future variant combining V-05 R11's base (C-30+C-31+C-32 PASS) with C-33+C-34+C-35 PASS simultaneously.
...
## Aspirational Criteria -- 135 pts (structural integrity, auditability, automation)
...
aspirational_pts =  aspirational_pass    *  5   # 5 pts each, max 135
composite = essential_pts + recommended_pts + aspirational_pts
```

**Maximum**: 60 + 30 + 135 = **225 pts**

### Score Bands

| Band | Condition | Composite |
|------|-----------|-----------|
| Gold | all 5 essential pass | >= 190 |
| Strong | all 5 essential pass | 90-189 |
| Partial | <= 4 essential pass | any |
| Minimal | <= 3 essential pass | any |

### Paths to Gold (190 pts)

| Path | Essential | Rec | Asp passed | Composite |
|------|-----------|-----|------------|-----------|
| Full rec + 20 asp | 5/5 | 3/3 | 20/27 | 60+30+100 = 190 |
| 2 rec + 22 asp | 5/5 | 2/3 | 22/27 | 60+20+110 = 190 |
| 1 rec + 24 asp | 5/5 | 1/3 | 24/27 | 60+10+120 = 190 |
| 0 rec + 26 asp | 5/5 | 0/3 | 26/27 | 60+0+130 = 190 |

### Round 11 Calibration (retroactive, v10 -> v11)

All five R11 variants carry the V-05 R9 template as base. C-33, C-34, C-35 absent from all R11
variants: §15 lens exhaustion (C-31) present but without applicability ratings (C-33) or domain
gap-check (C-34); no challenger dimension comparison table (C-35).

| Rank | Variant | C-33 | C-34 | C-35 | v10 | v11 |
|------|---------|------|------|------|-----|-----|
| 1 | **V-05** | FAIL | vac | FAIL | 210 | **210** |
| 2 | V-04 | FAIL | vac | FAIL | 205 | **205** |
| 3 | V-01 | vac | vac | FAIL | 200 | **200** |
| 3 | V-02 | FAIL | vac | FAIL | 200 | **200** |
| 3 | V-03 | vac | vac | FAIL | 200 | **200** |

C-33/C-34 vacuous for V-01/V-03 (C-31 FAIL -> C-33 vacuous -> C-34 vacuous). C-33 FAIL for
V-02/V-04/V-05 (C-31 PASS but no applicability rating column). C-34 vacuous for all (C-33
FAIL -> C-34 vacuous: no applicability framework means no HIGH-applicability tier). C-35 FAIL
for all R11 variants (no challenger dimension comparison table). No retroactive score change.

### Round 12 Calibration (v10 -> v11)

| Rank | Variant | C-33 | C-34 | C-35 | v10 | v11 |
|------|---------|------|------|------|-----|-----|
| 1 | **V-02** | PASS | PASS | PASS | ~200 | **~215** |
| 2 | V-01 | vac | vac | vac | 71 | **71** |
| TBD | V-03 | vac | vac | TBD | TBD | TBD |
| TBD | V-04 | TBD | TBD | TBD | TBD | TBD |
| TBD | V-05 | TBD | TBD | TBD | TBD | TBD |

V-01: C-09 FAIL -> C-35 vacuous; C-31 vacuous (C-09 FAIL) -> C-33/C-34 vacuous. Score 71/225
unchanged.
V-02: Role lens inventory table with Applicability column (C-33 PASS) + domain gap-check
producing ADVISORY-GAP classification (C-34 PASS) + challenger dimension comparison table in
Step 2 (C-35 PASS). Estimated v10 base ~200 + 5+5+5 = ~215/225.
V-03: C-31 FAIL -> C-33 vacuous -> C-34 vacuous. C-35: domain-first variant, challenger
dimension table not confirmed from partial text -- TBD.
V-04, V-05: not scored from R12 text -- TBD pending full text review.

**V-02 R12 is the canonical source for C-33/C-34/C-35 patterns.** Perfect score of 225 requires
a future variant combining V-05 R11's base (C-30+C-31+C-32 PASS) with C-33+C-34+C-35 PASS
simultaneously. The three new criteria are independent in the same way C-30/C-31/C-32 were
independent: C-33 activates at the applicability rating level (per-lens rated basis), C-34
activates at the domain gap-check level (per-domain expert coverage), C-35 activates at the
challenger format level (table vs. prose). No shared fields, no cascade effects beyond the
C-34->C-33 activation dependency (C-34 requires C-31 active, and C-31 requires C-33 to be
achievable -- but C-33 FAIL does not make C-34 vacuous directly; both independently require
C-31 active).
