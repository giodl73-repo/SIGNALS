Written to `simulations/quest/rubrics/flow-resilience-rubric-v8-2026-03-15.md`.

---

**One new criterion extracted from Round 7:**

**C-28 -- Post-Analysis Rule Registry Audit** (from V-03 `Post-Analysis Registry Audit` terminal block)

C-27 requires a named rule registry with unique IDs. C-28 closes the execution loop: after all gates complete, a named audit block confirms per-rule invocation status — which rules fired (citing gate + entry), how many times each fired, and whether any rules went uninvoked. Uninvoked rules are classified as `scenario-absent` (triggering condition never arose) or `rule-bypassed` (condition arose but rule was not applied — an audit failure). A registry that is never post-audited cannot prove invocations actually occurred. V-03 is the first variation to include this as a named terminal section.

**Scoring delta**: cap unchanged at 10; uncapped aspirational max rises from 38 to 40 (20 criteria × 2).
nd
...
Uncapped maximum across 20 criteria is 40; the cap rewards breadth while holding the
...
| Aspirational | 10% | C-09 through C-28 | PASS=2, PARTIAL=1, FAIL=0; capped at 10 |
...
- **C-28 (new)** -- Post-analysis rule registry audit: extracted from V-03 Round 7 structural
  ...
  re-reading the full analysis. V-03 is the first variation to include this structural block
  explicitly as a terminal section distinct from the analysis gates.
