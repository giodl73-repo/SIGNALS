...
**Two new aspirational criteria extracted from R15 excellence signals:**

| ID | Criterion | Source Axis | Independence from Parent |
|----|-----------|-------------|--------------------------|
| C-40 | **Amendment table T-38 trigger entry includes inline failure-condition exemplar** | V-01 R15 T-39-fires note + V-02 R15 T-38-silent note | C-38 requires the T-37 CONDITION field to carry an inline failure exemplar quoting the exact construct that fires T-37. C-40 requires the T-38 CONDITION field to carry an inline failure exemplar quoting what an abstract-only T-37 CONDITION entry looks like — the form that fires T-38. A T-38 CONDITION entry stating only "fires if T-37 CONDITION lacks an inline failure exemplar" names the condition category without showing what a failing T-37 CONDITION looks like, and fails C-40. C-40 is independently testable: a reviewer reads the T-38 CONDITION cell and confirms whether a quoted or closely paraphrased abstract T-37 form is present. Satisfying C-38 (T-37 CONDITION carries its own exemplar) does not satisfy C-40 — the exemplar in T-38's CONDITION is independently required and concerns T-38's trigger condition, not T-37's. |
| C-41 | **Amendment table T-38 trigger entry includes correct-format prescription alongside the inline failure exemplar** | V-02 R15 T-38-silent note | C-40 requires the T-38 CONDITION field to carry an inline failure exemplar (Part 1). C-41 requires T-38 CONDITION to additionally carry an explicit correct-format prescription (Part 2) — stating what a passing T-37 CONDITION entry looks like (the minimum form that satisfies C-38: a quoted failure exemplar naming the exact construct that fires T-37). The two-part T-38 CONDITION makes it fully self-contained: a reviewer can determine both what fires T-38 and what would satisfy C-38 without consulting the rubric or T-37's entry. A T-38 CONDITION carrying only Part 1 (the abstract-T-37-form exemplar) without a correct-format prescription passes C-40 but fails C-41. Satisfying C-39 (T-37 CONDITION carries its own correct-format prescription) does not satisfy C-41 — the correct-format prescription in T-38's CONDITION is independently required and concerns what satisfies C-38, not what satisfies C-37. |

**Denominator:** `/32` → `/34`. Each aspirational criterion = `10/34` ≈ `0.2941` points.

**Score projections under v16 for R15 variations** (denominator /34):

| Variation | Fails | Notes | Composite |
|-----------|-------|-------|-----------|
| V-01 R15 (C-39; T-38 CONDITION assumed satisfies C-40/C-41) | C-24 + C-39 = 2 | C-24 fires: 39-row table < 41 required by v16 | **99.41** |
| V-02 R15 (C-37; T-38 CONDITION assumed satisfies C-40/C-41) | C-24 + C-37 = 2 | C-24 fires: 39-row table < 41 required by v16 | **99.41** |

Note: Both R15 variations carried 39-row amendment tables (correct under v15). Under v16, C-24 requires 41 rows (T-40 and T-41 absent). If either variation's T-38 CONDITION is abstract (lacks Part 1), C-40 adds a third fail and C-41 cascades to a fourth: four total fails, composite 98.82.

**Cascade invariants:**

- **C-41 cascade from C-40**: A T-38 CONDITION that is abstract (category statement only, no exemplar of an abstract T-37 form) fails both C-40 and C-41. C-40 FAIL → C-41 FAIL is deterministic.
- **C-41 independent fail**: A T-38 CONDITION carrying Part 1 (the abstract-T-37-form exemplar) but no Part 2 (correct-format prescription) passes C-40 and fails C-41. C-40 PASS does not guarantee C-41 PASS.
- **C-24 version-bump cascade**: All R15 variations had 39-row amendment tables. Under v16, all R15 variations fail C-24 (41 rows required: T-40 and T-41 absent).

**Key changes to existing criteria:**
- **C-24**: updated from "exactly 39 trigger rows" → "exactly 41 trigger rows (one per C-01..C-41)"
- **Scoring formula denominator**: `/32` → `/34`
...
- **C-24** is structurally testable at PRE-DOCUMENT: a reviewer counts the amendment table rows at initialization. For v16, expected count = 41 (one row per C-01..C-41). A count of 39 (v15 table carried forward without adding T-40, T-41) fires C-24 at PRE-DOCUMENT.
...
## Aspirational Criteria (10% of composite, denominator /34)

### C-01 through C-07 — Mandatory C-tier (not counted in /34 denominator)
...
### C-08 through C-41 — Aspirational Criteria (denominator /34)
...
| C-24 | **Amendment table row count matches v16 criterion count** | behavior | Amendment table contains exactly 41 trigger rows at initialization (one per C-01..C-41 criterion). A table built from v15's criterion list (39 rows) without adding T-40, T-41 fires C-24 at PRE-DOCUMENT. A table with any count other than 41 fails C-24. |
...
```
composite = 90 + (aspirational_pass / 34 × 10)
```
...
- Amendment table T-38 entry carries an inline abstract-T-37-form exemplar (C-40 PASS, Part 1 present) but stops there — no correct-format prescription stating what a passing T-37 CONDITION entry looks like — fails C-41; satisfies C-40
...
For early-round discriminator notes (R1–R11), see the v12 rubric file. The v16 rubric inherits all prior discriminator observations unchanged.
