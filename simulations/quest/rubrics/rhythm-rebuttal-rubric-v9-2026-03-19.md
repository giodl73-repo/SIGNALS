Written to `simulations/quest/rubrics/rhythm-rebuttal-rubric-v9-2026-03-19.md`.

---

**What changed in v9:**

Formula: `/22` → `/25`. Three new aspirational criteria:
...
**Structural distinctness**: C-31 extends C-28 (label presence → mid-workflow presence). C-32 extends C-24 (gate exists → gate recovers). C-33 extends C-32 (repair at gate → constraint visible upstream). Each new criterion catches a failure class the prior criterion cannot.

**Note on Pattern 3 reconstruction**: The scorecard text was cut off after Pattern 2's heading. C-33 was reconstructed from the V-05 verdict evidence across C-28/C-29/C-30 (anti-deferral in Phase 2 summary, entity-type in Phase 5 template brackets, forward-annotation in Phase 6a summary) — all three point to the same propagation-upstream behavior pattern.
te headers for entity-type requirements, Phase 6a table schema or section summary for cover-letter/outcome consistency; an author working through the upstream phase sees the failure condition at the moment of decision, not only when the Phase 7 gate runs |

The three additions are structurally distinct from everything preceding them:

- **C-31** closes the gap between C-28's Phase 7 gate and the workflow surface where evidence referents are first recorded — C-28 requires entity-type resolution at write time; C-31 requires it at the Phase 6a reconciliation table, making last-second insertion impossible. The same artifact can satisfy C-28 but fail C-31 if the Phase 6a table schema does not carry the entity-type column.
- **C-32** closes the gap between C-24's detection requirement and structured recovery — C-24 requires gate checks to exist and block write on failure; C-32 requires each gate check to specify what the author does next, with structurally distinct paths per failure type. Detection without prescribed recovery leaves the author to improvise; C-32 converts each gate from a stop sign into a decision fork.
- **C-33** closes the gap between C-32's repair protocols and constraint visibility during authoring — C-32 embeds repair options in Phase 7 gate checks after the fact; C-33 requires the failure condition to be visible at the upstream decision point where it can still be avoided. Forward-annotating the failure condition into Phase 2, Phase 5, and Phase 6a means the author encounters the constraint during authoring, not only after the artifact is drafted and the gate check runs.
...
| Aspirational (C-09–C-33) | 10 | (passed / **25**) × 10 |
...
| C-32 | Phase 7 gate checks include branched repair protocols | structure | Each Phase 7 gate check that can fail includes a branched repair protocol specifying two or more structurally distinct corrective paths labeled (a) / (b): the repair choice is determined by the failure type (e.g., "correct the MANUSCRIPT OUTCOME if the change was actually made; or revise Paragraph 1 if it was not") rather than left to author discretion. The protocol is embedded within the gate check text itself, not in a separate appendix. Distinct from C-24 (which requires gate checks to exist and block write) — C-32 is the only criterion that requires each blocking gate to provide structured recovery rather than only detection. |
| C-33 | Gate failure conditions forward-annotated into upstream phase text | structure | Gate constraints enforced in Phase 7 are also forward-annotated as explicit failure conditions in the upstream phases where the author makes the relevant decision: Phase 2 summary for SHIFTED anti-deferral constraints; Phase 5 MANUSCRIPT OUTCOME template headers for entity-type requirements; Phase 6a table schema or section summary for cover-letter/outcome consistency. An author working through the upstream phase sees the failure condition at the moment of decision. Distinct from C-32 (which embeds repair protocols at the Phase 7 gate after the fact) — C-33 is the only criterion that requires constraint visibility at the upstream authoring decision point where the failure can still be avoided rather than only detected. |
