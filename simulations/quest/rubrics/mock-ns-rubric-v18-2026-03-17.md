...
**Two new aspirational criteria (C-46, C-47) extracted from R17 excellence signals. Max rises from 154 to 158.**

### C-46 -- CONSTRAINT trailing-group as named intermediate failure form (R17 V-02)

R17 V-02 canonical: "Do not perform S-1, S-2, S-3.1, S-3.2, or S-3.3 until this step's emit is written. Operations blocked: skill selection, category lookup, carry, compliance detection, flag computation." -- five operation types present in the CONSTRAINT block, but in a trailing sentence ("Operations blocked: ...") separated from the step-ID prohibition clause. C-15/C-21/C-24 all pass (5 type names present anywhere in the block). C-44 fails (types not co-located per step ID).

C-46 names the trailing-group form as a discrete structural pattern, the third CONSTRAINT form alongside step-IDs-only (V-01: C-15/C-21/C-24/C-44 all fail) and per-ID-annotated (golden: all pass). The trailing-group form is the canonical demonstration that C-44 fires independently of C-15/C-21/C-24. C-46 tests for the absence of a trailing-group annotation structure -- the output CONSTRAINT must not separate type names from their step IDs into a distinct trailing clause. Failure chain for trailing-group: C-44, C-46 (-4 pts).

### C-47 -- C-44 scope boundary and C-45 type-count cascade (R17 V-03)

R17 V-03 canonical: "Do not perform S-1 (skill selection), S-2 (category lookup), or S-3 (flag computation)..." -- S-3 collapsed (C-45 FAIL), but three named IDs each carry per-ID annotations. Two simultaneous findings:

1. **C-44 scope boundary:** C-44 evaluates annotation co-location only for the step IDs that ARE named. When S-3 is collapsed, the three present IDs (S-1, S-2, S-3) each carry adjacent parenthetical annotations -- C-44 passes. C-44 does not require all possible step IDs to appear; it requires each named ID to be annotated. C-45 collapse does not cause C-44 failure when the named IDs are annotated.

2. **C-45 cascade:** C-45 FAIL reduces the distinct operation-type count from 5 to 3 (S-3.1 carry and S-3.2 compliance detection are absorbed into the collapsed S-3; only S-3's single annotation "flag computation" counts). This causes C-21 (4+ types) and C-24 (5+ types) to fail via cascade, while C-15 (3+ types) survives. The cascade is deterministic: S-3 collapse always reduces the type count by 2 (from 5 to 3).

C-47 formalizes both halves. Pass condition for C-47: when the CONSTRAINT contains a collapsed S-3 entry (C-45 failing), the evaluator correctly applies C-44 as PASS if the named IDs are annotated, and correctly identifies the cascade C-21 FAIL + C-24 FAIL from the type-count reduction. C-47 passes by default in any output that achieves C-45 PASS (no cascade) or that achieves full scoring on C-44/C-21/C-24 (no ambiguity). C-47 fires (as an evaluator-accuracy criterion) when a collapsed-S-3 CONSTRAINT is incorrectly evaluated: either C-44 is wrongly failed for the annotated IDs, or C-21/C-24 are wrongly passed despite the type count being 3. V-03 canonical score: 154 - 2(C-45) - 2(C-21) - 2(C-24) = **148/154**.

### Discriminator notes updated

| Criterion | Update |
|-----------|--------|
| C-21 | R17: C-45 cascade confirmed. S-3 collapse -> type count = 3 -> C-21 FAIL. V-03 canonical: collapsed S-3 with 3 types -> C-21 FAIL even when C-44 PASS. |
| C-24 | R17: C-45 cascade confirmed. S-3 collapse -> type count = 3 -> C-24 FAIL. V-03 canonical: collapsed S-3 with 3 types -> C-24 FAIL even when C-44 PASS. |
| C-44 | R17: Trailing-group canonical (V-02) -- 5 types present, C-15/C-21/C-24 all pass, C-44 still fires (types not per-ID co-located). C-44 independence from count criteria confirmed. Em-dash equivalence confirmed (V-04): "S-1 -- skill selection" satisfies C-44; punctuation form is flexible, structural adjacency is the requirement. |
| C-45 | R17: Cascade confirmed (V-03). C-45 FAIL -> type count drops from 5 to 3 -> C-21 FAIL + C-24 FAIL; C-15 PASS (floor at 3). C-44 independence from C-45 collapse confirmed: annotated named IDs pass C-44 regardless of sub-step completeness. |
...
**R17 under v18:**
- V-01 (step-IDs-only CONSTRAINT): fails C-15, C-21, C-24, C-44, C-46 (-10 pts) -- **148/158**
- V-02 (trailing-group): fails C-44, C-46 (-4 pts) -- **154/158**
- V-03 (collapsed S-3 with per-ID annotations): fails C-45, C-21, C-24 (-6 pts) -- **152/158**
- V-04 (em-dash per-ID annotated): all 47 criteria pass -- **158/158**
- V-05 (golden parenthetical): all 47 criteria pass -- **158/158**
...
*Raise the bar once essential and recommended are stable. Each criterion is worth 2 pts (39 x 2 = 78; max score = 158).*
