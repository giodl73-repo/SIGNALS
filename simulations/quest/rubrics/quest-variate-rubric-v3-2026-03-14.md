Looking at the R2 scorecard, the excellence signals are in three variations that went beyond baseline:

- **V-01**: Body committed before critique can interrupt (generation ordering)
- **V-02**: Per-variation self-check after each body (checkpoint timing)
- **V-03**: Named domain persona (VariationScientist) carrying behavioral priors

These are distinct from C-10/C-11/C-12 (which came from V-04's phase-gate pattern). Adding as C-13, C-14, C-15.

```markdown
# quest-variate Rubric — v3

**Skill:** `quest-variate`
**Version:** v3 (updated from R2 scorecard — 2026-03-14)
...
- **4 essential criteria** (not 5) — the skill has one core invariant per property:
  completeness, labeling, single-axis isolation, and count. A 5th would overlap.
- **C-01 (complete runnable bodies)** is the hardest fail mode in practice — models
  routinely emit diffs or partial bodies when asked to "vary" something.
- **C-03 (single-axis isolation)** enforces the scientific discipline the skill description
  calls out explicitly: single-axis passes before combinations.
- **C-07 (axis coverage breadth)** is recommended, not essential — with N=5 you can't hit
  all 6 axes, so 4-of-6 is a reasonable floor.
- **C-08/C-09** are aspirational because they only apply in specific N > 6 or
  baseline-anchored scenarios — penalizing their absence in a default N=5 run would be
  unfair.
- **C-10/C-11/C-12** were added from R1 excellence signals — V-04's phase-gate design
  surfaced structural patterns that distinguish good variations from excellent ones.
- **C-13/C-14/C-15** are new aspirational criteria added from R2 excellence signals —
  V-01's body-first ordering, V-02's per-variation checkpoint, and V-03's named persona
  each represent a distinct skill-body design choice that improves output reliability
  without being required at baseline N=5.
...
1. **Diff leak** — body contains "same as above" or diff notation instead of full content
   (fails C-01)
2. **Missing hypothesis** — label and axis present but hypothesis is a restatement of the
   axis name (fails C-02)
3. **Axis drift** — variation modifies two dimensions simultaneously, unlabeled as
   combination (fails C-03)
4. **Count mismatch** — output delivers 4 or 6 variations when 5 were requested (fails
   C-04)
5. **Trivial delta** — variation changes surface phrasing only, not the design choice the
   axis represents (fails C-06)

---

**Key changes from v2:**

- Bumped to v3 (2026-03-14)
- Added C-13 (body-first ordering) from R2/V-01 excellence signal
- Added C-14 (per-variation checkpoint) from R2/V-02 excellence signal
- Added C-15 (named domain persona) from R2/V-03 excellence signal
- Aspirational denominator updated from 5 to 8 in scoring formula
```
