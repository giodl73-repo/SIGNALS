Written to `simulations/quest/rubrics/discover-feasibility-alt-rubric-v4-2026-03-17.md`.
...
|----|--------|----------|-----------------|
| C-17 | V-02 R3 table-first ordering | C-16 | Table is the *first* element within Step 0 — before lens prose and before economics. Enforces "name contract before deriving costs." |
| C-18 | V-03 R3 column semantics | C-16 | Two sub-conditions: (1) headers use rubric-aligned vocabulary, not bare generics; (2) repeated-constraint rows use `[same]` back-reference instead of verbatim repetition. |

**Key design decisions:**
- C-17 and C-18 are scored **independently** of each other (a table can pass one and fail the other) — this is captured in the evaluation notes
- Both are **FAIL (not PARTIAL)** when their binary condition is unmet, matching C-16's pattern
- Aspirational denominator `/8` → `/10`; all-pass composite remains 100
- V-04 R3 (already 100.000) gains no new points since it already passed C-16 but did not demonstrate C-17/C-18 — those came from V-02 and V-03 respectively
ed fields | completeness | essential | Feature, Team, and Timeline are all present and non-empty in the INFERENCE GATE section before any analysis begins. Focus is optional; its absence does not fail C-01. |
