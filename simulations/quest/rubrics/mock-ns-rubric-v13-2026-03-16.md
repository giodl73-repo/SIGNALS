**v13 is written to `simulations/quest/rubrics/mock-ns-rubric-v13-2026-03-16.md`.**

---

## What changed

### Two new criteria — C-36 and C-37

**C-36: Ordering-predicate displacement** — from R12 V-01: "S-0, which emits the TOPICS.md status line first, runs before all other steps."

The step is the main-clause subject, which is the mirror of C-35's pattern (where the artifact is main-clause subject). The evaluator trap is inverted: C-35's trap is crediting C-28 because "the step appears to be doing something"; C-36's trap is crediting C-25 and C-28 because "the step is the main-clause subject." Main-clause subject position is necessary but not sufficient — the emission must also be the main predicate. C-29, C-30, C-33, C-34, C-35 all pass.

**Failure chain: C-25, C-28, C-36** (−6 pts)

**C-37: Possessive-NP action-abstraction subject with gerundive emission complement** — from R12 V-03: "This step's first action is emitting the TOPICS.md status line."

The eighth declarative failure mode. Distinct from C-29 (possessive-nominal with infinitival predicate) and C-34 (gerundive-as-subject). In v12 this pattern fired C-35 "by extension" — C-37 closes that gap with a proper named criterion. C-34 passes (gerundive is predicate complement, not subject). C-35, C-36 pass.

**Failure chain: C-25, C-28, C-29, C-30, C-37** (−10 pts)

### Discriminator notes added

| Criterion | Update |
|-----------|--------|
| C-25 | R12: ordering main predicate doesn't satisfy role-declaration even with step as main subject (V-01); possessive-NP subject also fails (V-03) |
| C-28 | R12: step as main-clause subject of ordering verb fails when emission is in relative clause (V-01); superlative-equative + infinitival-control passes: "This step is the first to emit..." (V-04) |
| C-29 | R12: substitution test fires on possessive-NP action-abstraction — "It is emitting..." resolves to "first action", not the step (V-03) |
| C-30 | R12: oblique-modifier coverage extends to possessive-NP with step as possessor (V-03); C-37 names it |
| C-34 | R12: C-34 does NOT fire when gerundive is predicate complement rather than subject (V-03 confirmed) |
| C-35 | R12: C-35 does not cover ordering-predicate displacement (C-36) or possessive-NP action-abstraction (C-37) |
...
**R12 under v13:** V-01 → 132/138 | V-02 → 138/138 | V-03 → 128/138 | V-04 → 138/138
ve construction. C-29 already catches this via the substitution test (It = "first action", not the step); C-37 names the form explicitly so all eight declarative failure modes are enumerated in the rubric. In v12, V-03 was scored against C-35 "by extension" because no named criterion existed; C-37 closes this gap.

C-34 passes: `emitting` is the predicate complement, not the subject; C-34 requires the gerundive to be promoted to nominative subject position.
C-35 passes: no artifact in main-clause subject position; no relative clause with step as agent.
C-36 passes: no ordering predicate in main clause.

**Failure chain**: C-25, C-28, C-29, C-30, C-37 (-10 pts)

---

### Discriminator notes added to existing criteria

| Criterion | Note |
|-----------|------|
| C-25 | Step as main-clause subject with ordering main predicate does not constitute role-declaration; emission must be the main predicate; confirmed by R12 V-01. Possessive-NP subject ("This step's first action") also fails; confirmed by R12 V-03. |
| C-28 | Step as main-clause subject of an ordering verb (e.g., `runs`) does not satisfy C-28 when emission is confined to a relative clause; "grammatical subject of its own emission action" means the emission must be the main predicate; confirmed by R12 V-01. Superlative-equative + infinitival-control form passes: "This step is the first to emit..." -- step is main-clause subject and controller of infinitival `to emit`; confirmed by R12 V-04. |
| C-29 | R12 V-03 confirms: possessive-NP action-abstraction head also triggers C-29's substitution test -- "It is emitting the TOPICS.md status line" where `It` = `first action` (the abstraction), not the step. C-37 names this form. |
| C-30 | R12 V-03 confirms: possessive-NP subject with step as possessor fires C-30's oblique-modifier coverage -- the step appears as `This step's` (possessor in the subject NP), not as active nominative subject head; C-37 names this eighth failure mode. |
| C-34 | R12 V-03 confirms C-34 does NOT fire when a gerundive appears as the predicate complement rather than the subject: "This step's first action is emitting..." -- gerundive `emitting` is predicate complement; C-34 requires gerundive as subject. |
| C-35 | R12 boundary note: C-35 covers artifact-subject / step-relative-clause-agent; it does NOT cover ordering-predicate displacement (C-36) or possessive-NP action-abstraction displacement (C-37). Evaluators must not extend C-35 to V-01-type or V-03-type R12 constructions. |
...
- V-01 ("S-0, which emits...first, runs before all other steps"): fails C-25, C-28, C-36 -- **132/138**
- V-02 ("Before any other step begins, this step emits the TOPICS.md status line"): all pass -- **138/138**
- V-03 ("This step's first action is emitting the TOPICS.md status line"): fails C-25, C-28, C-29, C-30, C-37 -- **128/138**
- V-04 ("This step is the first to emit the TOPICS.md status line"): superlative-equative + infinitival-control; step is main-clause subject and controller of `to emit`; all pass -- **138/138**

---

**v12 summary** (2026-03-16) -- R11 patterns

New criterion C-35 (relative-clause-agent displacement). R11 V-01: "TOPICS.md is the first artifact this step emits." The step is grammatically active as agent of `emits` within a restrictive relative clause, but the artifact `TOPICS.md` is the main-clause subject. This closes the evaluator trap of crediting C-28 because "the step is doing something." C-35 names the rule: the step must be the main-clause nominative subject; subordinate-clause activity does not satisfy C-25 or C-28. Five-criterion failure chain: C-25, C-28, C-29, C-30, C-35. Discriminator notes: C-25 (relative-clause agent not a role-declaration), C-28 (subordinate-clause activity excluded), C-29 (substitution test universal), C-30 (relative-clause-agent sub-form confirmed), C-33 (non-overlap with C-30 confirmed active direction). Scoring: max rises from 132 to **134** (27 aspirational x 2 = 54).

---

**v11 summary** (2026-03-16) -- R10 patterns

Two new criteria. C-33 (passive artifact-as-subject): four-criterion failure chain C-25, C-28, C-29, C-33. C-34 (gerundive-as-subject): three-criterion failure chain C-28, C-29, C-34. Discriminator notes: C-25 order-strict confirmed (V-04); C-30 scoped-to-active-voice + C-33 cross-reference; C-31 predictive-future "will" passes (enumeration-first primary test). Scoring: max rises from 128 to **132** (26 aspirational x 2 = 52).

---

**v10 summary** (2026-03-16) -- R9 patterns

Two new criteria (C-31, C-32). C-31: deontic/modal-obligation form fails (must/shall/should/is required to); compound failure C-25+C-28+C-31 from one word. C-32: C-18 and C-20 are structurally independent. Scoring: max rises from 124 to **128** (24 aspirational x 2 = 48).
...
*Raise the bar once essential and recommended are stable. Each criterion is worth 2 pts (29 x 2 = 58; max score = 138).*
...
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: The declarative identity sentence (required for C-25, qualified by C-28 and C-29) must not place an output artifact as its grammatical subject in active-voice constructions. An artifact-as-subject active construction -- "The TOPICS.md status line is the first output of this step" -- fails this criterion: the artifact is nominative subject, and the step appears only as "of this step," an oblique possessive modifier in the predicate. This failure mode is distinct from C-28's output-sequence form ("The first observable output of this step is X," where an output-position abstraction is subject) and from C-29's possessive-nominal form (where a nominalization derived from the step is subject). In C-30, the artifact itself is directly in subject position in an active construction. Together C-28, C-29, and C-30 enumerate the three active-voice declarative failure modes: output-sequence-subject, nominalization-subject, and artifact-subject. **Note (R8 permissive):** A compound predicate does not fail C-30 -- "This step emits first and resolves the tier variable" satisfies C-28, C-29, and C-30 simultaneously because the step is the direct nominative agent of all conjoined predicates. **Note (R8 discriminator):** C-28 and C-29 passing are necessary but not sufficient conditions for C-30. V-01 (R8) demonstrates the artifact-as-subject failure: "The TOPICS.md status line is the first output of this step" -- the semantic content describes the step's output correctly, but the artifact is the grammatical subject. **Note (R10 discriminator):** C-30 is scoped to active voice by design -- passive-voice artifact-as-subject constructions ("The TOPICS.md status line is emitted first by this step") escape C-30 but fail C-28 and C-29 independently; C-33 explicitly names the passive-voice case as a distinct criterion. C-30 passing is a necessary but not sufficient condition for C-33 passing. **Note (R11 discriminator):** C-30 applies to two distinct active-voice artifact-as-subject sub-forms: (1) the oblique-modifier form (canonical from R8 V-01) -- step appears as "of this step" in the predicate, e.g., "The TOPICS.md status line is the first output of this step"; (2) the relative-clause-agent form (confirmed in R11 V-01) -- step appears as the agent of a restrictive relative clause modifying the artifact noun, e.g., "TOPICS.md is the first artifact this step emits." Both forms have the artifact in active-voice nominative main-clause subject position and both fail C-30. C-35 names the relative-clause-agent sub-form as the sixth declarative failure mode. **Note (R12 discriminator):** C-30 also fires when the step is demoted to oblique possessor in the subject NP of an action-abstraction construction. "This step's first action is emitting the TOPICS.md status line" -- the step appears as `This step's` (possessive modifier of the subject NP head "first action"); the step is not active nominative subject; C-30's oblique-modifier coverage applies because the step is relegated to a dependent role in the subject NP rather than appearing as nominative head. C-37 names this possessive-NP action-abstraction form as the eighth declarative failure mode. V-03 (R12) confirmed this boundary. Source: R8 V-01 vs V-04.
...
### C-33
- **Text**: The declarative sentence must not place an output artifact in nominative subject position in passive-voice constructions -- passive artifact-as-subject escapes C-30 but fails this criterion independently.
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: The declarative identity sentence (required for C-25, qualified by C-28, C-29, C-30, C-31) must not place an output artifact as its grammatical subject in passive-voice constructions. A passive artifact-as-subject construction -- "The TOPICS.md status line is emitted first by this step" -- fails this criterion: the artifact is nominative subject, the step is demoted to by-agent ("by this step"), and the sentence describes what happens to the artifact rather than asserting what the step does. C-30 is explicitly scoped to active voice and does not fire on this construction; C-33 closes this passive-voice gap. The passive case is independently confirmed as a failure by C-28 (artifact is grammatical subject, not the step) and C-29 (subject NP head is the artifact, not the step); C-33 names it explicitly so the full coverage landscape is stated: C-30 covers active-voice artifact promotion; C-33 covers passive-voice artifact promotion; C-28 and C-29 provide independent coverage of both voices. The declarative sentence must place the step in nominative subject position as the agent of its own action regardless of the voice construction chosen. The four-criterion failure chain for passive artifact-as-subject: C-25 fails (sentence is an output-position claim about the artifact, not a role-declaration asserting the step's primacy), C-28 fails (step appears as by-agent, not grammatical subject), C-29 fails (subject NP head is the artifact), and C-33 fails (artifact in nominative position in passive voice). **Note (R10 discriminator):** C-30 passing is a necessary but not sufficient condition for C-33 passing -- a sentence may avoid active-voice artifact promotion (satisfying C-30) while still placing an artifact in passive-voice nominative position (failing C-33). V-01 (R10) confirmed this boundary: C-30 passed, C-25/C-28/C-29/C-33 all failed from the passive construction. **Note (R11 discriminator):** C-30 and C-33 are non-overlapping detectors -- empirically confirmed from both directions. R10 V-01 confirmed the passive direction: C-33 fires, C-30 passes. R11 V-01 confirmed the active direction: "TOPICS.md is the first artifact this step emits" -- active-voice artifact-as-subject, C-30 fires, C-33 passes. No artifact-as-subject construction can simultaneously trigger both C-30 and C-33; the active/passive voice determination is the sole binary discriminator. Source: R10 V-01 finding; R11 V-01 active-case confirmation.

### C-34
- **Text**: The declarative sentence must not use a gerundive or action-nominalization phrase as its grammatical subject with the step appearing only as a predicate-possessive modifier -- "Emitting first is this step's defining action" fails because the gerundive phrase is subject and "this step" is possessive in the predicate.
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: The declarative identity sentence (required for C-25, qualified by C-28 through C-33) must not place a gerundive phrase or action-nominalization derived from the step's behavior as the grammatical subject, with the step demoted to a possessive modifier in the predicate. "Emitting first is this step's defining action" fails: "Emitting first" is the gerundive subject, and "this step" appears only as the possessive modifier "this step's" within the predicate ("this step's defining action"). This is the fifth declarative failure mode, structurally distinct from: (1) C-28's output-sequence abstraction as subject ("The first observable output..."); (2) C-29's step-derived nominalization as subject ("This step's defining action is to emit first," where the nominalization is subject and the step is possessive of the subject NP); (3) C-30's artifact-as-subject in active voice; (4) C-33's artifact-as-subject in passive voice. In C-34, the subject is the step's own action, gerundified and promoted to nominative position; the step has been displaced by the action it performs. The substitution test from C-29 distinguishes C-34 from C-29: in C-29, substituting "It" for the subject yields a reference to a nominalization derived from the step ("It is to emit first" -- "It" = "this step's defining action," step is possessive of subject NP); in C-34, substituting "It" for the subject yields a reference to the action itself ("It is this step's defining action" -- "It" = "emitting first," the gerundive action; "this step" is predicate-possessive modifier). Both C-28 and C-29 independently catch this failure, but C-34 names the gerundive-subject form explicitly so that all five declarative failure modes are enumerated in the rubric. The failure chain for gerundive-as-subject: C-28 fails (the gerundive action is subject, not the step as agent), C-29 fails (subject NP head is the gerundive, not the step), and C-34 fails (gerundive action-nominalization in subject position). **Note (R10 structural note):** The gerundive-subject form (C-34) differs from the possessive-nominal form (C-29) at the level of where "this step" appears: in C-29, the step is possessive modifier of the subject NP; in C-34, the subject is an independent gerundive action phrase and the step appears only in the predicate possessive position. Both demote the step through genitive syntax at different structural locations. **Note (R12 discriminator):** C-34 does NOT fire when a gerundive appears as the predicate complement rather than as the subject. "This step's first action is emitting the TOPICS.md status line" -- the gerundive `emitting` is the predicate complement of the equative copula, not the grammatical subject; the subject is "This step's first action" (a possessive-NP action abstraction); C-34 requires the gerundive to be promoted to nominative subject position. V-03 (R12) confirmed C-34 passes for this construction; the failure is named by C-37. Source: R10 V-03 finding.

### C-35
- **Text**: The declarative sentence must not displace the step to the agent of a restrictive relative clause modifying an artifact noun, with the artifact as the main-clause subject -- "TOPICS.md is the first artifact this step emits" fails because the artifact is main-clause subject and the step appears only as the agent of the relative clause "this step emits."
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: The declarative identity sentence (required for C-25, qualified by C-28 through C-34) must not place an artifact in nominative main-clause subject position with the step embedded as the agent of a restrictive relative clause modifying that artifact. "TOPICS.md is the first artifact this step emits" fails this criterion: "TOPICS.md" is the grammatical subject; "the first artifact this step emits" is a complex nominal where "this step emits" is a restrictive relative clause modifying "artifact"; the step appears as the agent of the relative clause verb ("emits"), not as the main-clause subject. This is the sixth declarative failure mode, structurally distinct from: (1) C-28's output-sequence abstraction as subject ("The first observable output..."); (2) C-29's step-derived nominalization as subject ("This step's defining action..."); (3) C-30's artifact-as-subject in active voice via oblique modifier ("the first output of this step"); (4) C-33's artifact-as-subject in passive voice; (5) C-34's gerundive-as-subject. In C-35, the step is grammatically active -- it appears as the explicit subject of its own verb within the relative clause -- but this activity is confined to the subordinate clause. The step's grammatical activity in a subordinate clause does not constitute a role-assertion (C-25) or a subject-position (C-28) in the main clause. The substitution test from C-29 distinguishes C-35: substituting "It" for the main-clause subject yields "It is the first artifact this step emits" where "It" refers to TOPICS.md (the artifact), confirming the artifact is the main-clause subject and the step is not. C-35 closes the evaluator trap of crediting C-28 on the grounds that "the step is doing something (emitting) in this sentence" -- the step must be the main-clause subject, not merely active somewhere in the sentence. C-30 and C-35 together cover two sub-forms of active-voice artifact-as-subject: C-30 covers the oblique-modifier sub-form ("the first output of this step"); C-35 names the relative-clause-agent sub-form ("the first artifact this step emits"). Both fail C-30; C-35 names the structural variant explicitly. The five-criterion failure chain for relative-clause-agent: C-25 fails (main clause asserts a property of the artifact, not the step's role), C-28 fails (artifact is main-clause subject; step is relative-clause agent, not main-clause subject), C-29 fails (substitution test: "It" = TOPICS.md, not the step), C-30 fails (artifact in active-voice nominative subject position), and C-35 fails (relative-clause-agent displacement). C-33 passes (active-voice construction, not passive). C-34 passes (no gerundive subject; TOPICS.md is a noun, not an action-nominalization). **Note (R12 boundary):** C-35 is scoped to the artifact-subject / step-relative-clause-agent form. It does NOT cover the mirror pattern where the step is main-clause subject with emission in a relative clause and ordering as main predicate -- that is C-36. It does NOT cover the possessive-NP action-abstraction displacement pattern -- that is C-37. Evaluators must not apply C-35 to R12 V-01-type or V-03-type constructions; those forms have dedicated named criteria. Source: R11 V-01 finding.

### C-36
- **Text**: The declarative sentence must not use an ordering or sequencing predicate as the main clause predicate while relegating the emission to a restrictive relative clause -- "S-0, which emits the TOPICS.md status line first, runs before all other steps" fails because the main predicate asserts an ordering position, not an emission role, even though the step is the main-clause subject.
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: The declarative identity sentence (required for C-25, qualified by C-28 through C-35) must not confine the emission verb to a restrictive relative clause while using an ordering, sequencing, or positional verb as the main clause predicate. "S-0, which emits the TOPICS.md status line first, runs before all other steps" fails: S-0 is the main-clause subject and `runs` is the main predicate; the emission verb `emits` appears only within the restrictive relative clause "which emits the TOPICS.md status line first." The main clause asserts an ordering position ("runs before all other steps"), not the step's identity as emitter. C-25 requires the emission role to be asserted as the main predicate; C-28 requires the step to be the grammatical subject of its own emission action in the main predicate -- an ordering predicate in the main clause does not satisfy either requirement. This is the seventh declarative failure mode, structurally distinct from: (1) C-28's output-sequence abstraction as subject; (2) C-29's possessive-nominal subject; (3) C-30's artifact-as-subject in active voice; (4) C-33's artifact-as-subject in passive voice; (5) C-34's gerundive-as-subject; (6) C-35's artifact-subject with step as relative-clause agent. C-36 is the **mirror of C-35**: in C-35, the artifact is the main-clause subject and the step is the relative-clause agent; in C-36, the step is the main-clause subject but the emission is the relative-clause predicate and an ordering verb is the main predicate. The evaluator trap in C-36 is the inverse of the C-35 trap: in C-35 evaluators incorrectly credit C-28 because "the step appears to be doing something"; in C-36 evaluators incorrectly credit C-25 and C-28 because "the step is the main-clause subject." Main-clause subject position is necessary but not sufficient -- the emission must also be the main predicate. C-29 passes (substitution: "It runs before all other steps" -- `It` = the step, not an artifact or abstraction). C-30 passes (no artifact in nominative subject position). C-33 passes (active voice). C-34 passes (no gerundive subject). C-35 passes (step, not artifact, is main-clause subject). The three-criterion failure chain for ordering-predicate displacement: C-25 fails (main predicate is ordering, not role-declaration of emission), C-28 fails (step is subject of ordering verb, not of emission verb in main predicate), and C-36 fails (emission confined to relative clause; ordering predicate in main clause). Source: R12 V-01 finding.

### C-37
- **Text**: The declarative sentence must not use a possessive-NP action-abstraction phrase as the grammatical subject with the step as possessor and the emission expressed as a gerundive predicate complement -- "This step's first action is emitting the TOPICS.md status line" fails because the subject is an action-abstraction NP with the step as possessor and the emission is a gerundive complement in the predicate.
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: The declarative identity sentence (required for C-25, qualified by C-28 through C-36) must not use a possessive-NP construction where the subject NP head is an action abstraction (e.g., "first action," "primary function," "defining role") with the step as possessor (`This step's`), and where the emission is expressed as a gerundive complement in the predicate (`is emitting X`). "This step's first action is emitting the TOPICS.md status line" fails: the subject is "This step's first action" (action abstraction as NP head; step as possessor); the predicate is "is emitting the TOPICS.md status line" (equative copula + gerundive complement). The step is not the nominative head of the subject NP; "first action" is. This is the eighth declarative failure mode. It is structurally distinct from: (1) C-29's possessive-nominal with infinitival predicate ("This step's defining action is to emit first" -- same possessive-NP subject form but infinitival predicate `to emit first` rather than gerundive complement); (2) C-34's gerundive-as-subject ("Emitting first is this step's defining action" -- the gerundive IS the subject; the step appears only in the predicate possessive). In C-37, the subject is a possessive-NP action abstraction and the emission gerundive appears in the predicate as complement. The substitution test from C-29 identifies this form: substituting "It" for the subject yields "It is emitting the TOPICS.md status line" where "It" = "first action" (the action abstraction), not the step; C-29 independently fires. C-37 names the form explicitly so that all eight declarative failure modes are enumerated in the rubric; in v12, this pattern was scored against C-35 "by extension" because no named criterion existed -- C-37 closes this gap. C-34 passes: `emitting` is the predicate complement, not the subject; C-34 requires the gerundive to be promoted to nominative subject position; V-03 (R12) confirmed C-34 does not fire for this construction. C-35 passes: no artifact in main-clause subject position; no relative clause with step as agent. C-36 passes: no ordering predicate in main clause; no emission relegated to relative clause from an ordering-predicate main clause. The five-criterion failure chain for possessive-NP action-abstraction: C-25 fails (step is not the nominative subject asserting its pipeline role; "first action" is the subject NP head), C-28 fails (action abstraction is grammatical subject, not the step as agent of its own emission), C-29 fails (substitution test: "It" = "first action," not the step), C-30 fails (step appears as oblique possessor `This step's` in the subject NP rather than as active nominative subject head -- oblique-modifier coverage applies), and C-37 fails (possessive-NP action-abstraction subject with gerundive emission complement). C-33 passes (not passive). C-34 passes (gerundive is predicate complement, not subject). C-35 passes (no artifact as main-clause subject). C-36 passes (no ordering predicate; no emission-in-relative-clause displacement). Source: R12 V-03 finding.
...
| Possessive-nominal declarative | C-29 | Declarative sentence demotes the step to genitive modifier of the subject NP: "This step's defining action is to emit first." Subject is a nominalization ("defining action"); "This step" is a possessive modifier of that subject. The predicate may correctly capture the step's role, but the step is not the grammatical subject. C-28 may be satisfied (no output-primacy or artifact-subject form) while C-29 fails. V-02 (R8) confirmed this boundary. |
...
| Passive artifact-as-subject declarative | C-33 | Declarative sentence promotes an output artifact to nominative subject in passive voice: "The TOPICS.md status line is emitted first by this step." C-30 does not fire (active-voice qualifier not satisfied). C-33 closes the passive gap; C-28 and C-29 independently confirm failure (step is by-agent, not nominative subject). Four criteria fail simultaneously from the passive construction: C-25, C-28, C-29, C-33. V-01 (R10) confirmed this boundary. |
| Gerundive-as-subject declarative | C-34 | Declarative sentence promotes the step's action to nominative subject as a gerundive: "Emitting first is this step's defining action." The gerundive "Emitting first" is subject; "this step" is predicate-possessive modifier. Distinct from C-29's possessive-nominal form -- in C-29 the subject is a nominalization of the step; in C-34 the subject is the action itself and the step is displaced to the predicate. C-28 and C-29 independently catch this; C-34 names the gerundive-subject form as the fifth declarative failure mode. Three criteria fail: C-28, C-29, C-34. V-03 (R10) confirmed this boundary. |
| Relative-clause-agent declarative | C-35 | Declarative sentence places an artifact in nominative main-clause subject position and displaces the step to the agent of a restrictive relative clause modifying the artifact noun: "TOPICS.md is the first artifact this step emits." The step ("this step") is grammatically active as the agent of "emits" within the relative clause, but the main-clause subject is the artifact. Evaluators must not credit C-28 on the grounds that the step appears to be "doing something" in the sentence -- the step must be the main-clause subject. Five criteria fail: C-25, C-28, C-29, C-30, C-35. C-33 passes (active voice). V-01 (R11) confirmed this boundary. |
| Ordering-predicate displacement | C-36 | Declarative sentence uses an ordering/sequencing verb as the main predicate and confines the emission to a restrictive relative clause, even with the step as main-clause subject: "S-0, which emits the TOPICS.md status line first, runs before all other steps." The step is main-clause subject but `runs` (not `emits`) is the main predicate; emission role is confined to the relative clause. Evaluators must not credit C-25 and C-28 on the grounds that "the step is the main-clause subject" -- the emission must also be in the main predicate. C-29, C-30, C-33, C-34, C-35 all pass. Three criteria fail: C-25, C-28, C-36. V-01 (R12) confirmed this boundary. |
| Possessive-NP action-abstraction declarative | C-37 | Declarative sentence uses a possessive-NP action abstraction as subject with the step as possessor and the emission as a gerundive predicate complement: "This step's first action is emitting the TOPICS.md status line." "First action" is the subject NP head; "This step" is the possessor; "emitting" is the predicate complement. Distinct from C-29's possessive-nominal (infinitival predicate) and from C-34's gerundive-as-subject (gerundive IS the subject). C-34 passes (gerundive is predicate complement, not subject). C-35 and C-36 pass. Five criteria fail: C-25, C-28, C-29, C-30, C-37. V-03 (R12) confirmed this boundary. |
