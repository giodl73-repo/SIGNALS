`validate-inertia-rubric-v16-2026-03-17.md` written.

---

**Two new criteria extracted from R15:**

**C-43 — Produced architecture label cites block body as source of both gate components (gate placement receipt)**
Source: V-71. The ceiling label states "The block gate instruction in the block body names all three axes by label and declares partial axis coverage a gate failure" — explicitly naming the block body as the source of both components. Structural analogy to C-32: C-42 establishes both components ARE in the block body (C-31 verdict equivalent); C-43 requires the label to CERTIFY that placement by provenance citation (C-32 Evidence column equivalent). C-42 PASS does not imply C-43 PASS. Split shapes earn PARTIAL; V-74 earns FAIL.

**C-44 — Split-gate failure modes named as explicit disqualifiers in block instruction body**
Source: V-72, V-73. C-42 defines the split-gate shapes as external rubric criteria. C-44 requires the block instruction body to declare both shapes (axis-body/impossibility-label AND impossibility-body/axis-label) as named gate failures at read time. Structural analogy to C-39's dual-site prohibition: C-39 named forbidden derivation sources in the block body; C-44 names forbidden component placements. Naming one shape but not its mirror earns PARTIAL. C-42 PASS does not imply C-44 PASS.

**Scoring delta:** 420 → 440 pts max | 34 → 36 aspirational | aspirational subtotal: 360 pts.
at names the artifact without provenance satisfies C-42 but fails C-43.

**C-44 — Split-gate failure modes named as explicit disqualifiers in block instruction body**

Source: V-72, V-73. C-42 defines two split-gate failure modes as external rubric criteria. C-44 requires the block instruction body to declare both split-gate shapes as named gate failures within the block body itself -- making split-gate compliance a self-rejecting structural constraint the model encounters at read time. The structural analogy: C-39 required explicit prohibition of forbidden derivation sources in the named architecture block; C-44 applies the same prohibition pattern to gate component placement, requiring the block instruction body to name both split-gate configurations (axis-body/impossibility-label AND impossibility-body/axis-label) as disqualifiers. A block instruction body that names one split-gate shape but not its mirror earns PARTIAL. A block instruction body that contains both gate components without naming either split-gate failure mode fails C-44 regardless of C-42 status. C-42 PASS does not imply C-44 PASS.
...
- **36 aspirational (C-09--C-44):** scoring methodology transparency, time-dependent inertia
...
10 pts each (360 total) = **440 pts max**.
