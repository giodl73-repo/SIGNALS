Reading the scorecard carefully to identify the two new patterns before writing.

The `new_patterns` JSON names:
1. `expert-persona-zero-cost-enrichment` → structural enrichment mechanism, not yet codified
2. `registry-cross-reference-C14-pass-C11-partial` → documents the known trade-off; the aspirational ideal is the inverse: in-block rate limit completeness that makes both C-11 and C-14 achievable without sacrifice

C-16 rewards expert persona declaration. C-17 rewards the architecture that resolves the C-11/C-14 registry trade-off by keeping full rate limit data inside the call block.

```markdown
# flow-integration Rubric — v4

**Version:** 4 (updated from Round 3 excellence signals)
...
- **Aspirational** (C-08–C-17): actionability and structural discipline — ranked risk, concrete fixes, and organizational guarantees that make the other criteria structurally reliable rather than convention-dependent.
...
## Aspirational Criteria (47 pts total)
...
| Aspirational | C-08, C-09, C-10, C-11, C-12, C-13, C-14, C-15, C-16, C-17 | 47 |
| **Total** | | **137** |
