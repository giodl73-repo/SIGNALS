# Rubric: trace-component (v18)

**57 criteria across 3 tiers -- 3 new aspirational criteria added from R17 excellence signals:**
...
| Aspirational | C-09 through C-57 | 98 |

**Ceiling: 188 pts** (up from 182; 3 new criteria x 2 pts each)

---

The three R17 signals that promoted:

**C-55 . Header-Form Causal Questions** -- From R17 C-52 differential. V-01 and V-04 carry NHQ-1/NHQ-2 questions that causally derive the compound register header form; V-05 carries NHQ-1/NHQ-2 within the NULL REGISTER itself. V-02 has NCQ questions that probe only the close-marker compound form; V-03 has no NHQ questions at all. V-02 and V-03 fail C-52 despite having other structural elements in place. The differentiator is whether the causal question set includes questions that specifically ground the compound header form -- asking why a register section header requires co-declaration of label identity and epistemic-function descriptor in a single parseable compound line. NCQ questions (close-marker form) ask about the closing artifact, not the opening artifact; they do not derive the header structure. C-41 requires causal reasoning to ground the adjacency marker mechanism; C-55 requires the same causal grounding applied to the compound header form: the model must derive why headers need the compound structure, not merely be instructed to use it.

**C-56 . Schema-Declared Compound Header Template** -- From R17 C-52 differential. V-01 and V-04 both show "Schema Area 3 declares compound header template"; V-05 shows "Schema Area 3b declares compound header template with examples." V-02's Schema Area 3 addresses dual-assertion manifest closes only; V-03's Schema Area 3 addresses dual-assertion manifest closes only. Both failing variations omit the compound-header template as a schema artifact field. The mechanism: without a schema-declared template, Role 2 must adopt the compound form from prompt instructions alone. Prompt instructions are external constraints; schema fields are self-authored constraints. A Role 2 that violates a schema field it authored contradicts its own prior production -- structural self-contradiction. A Role 2 that ignores a prompt instruction contradicts an external directive -- a weaker enforcement signal. C-39 requires the model to produce the schema artifact; C-56 requires that artifact to include a named compound-header template field so the compound form is bound into the self-authored schema, not left as an advisory prompt-level instruction.

**C-57 . Exemplar-Grounded Compound Header Template in Schema** -- From R17 C-52 V-05 evidence. V-05 goes beyond V-01 and V-04: its schema area carries the compound header template "with examples." V-01 and V-04 declare the abstract pattern without concrete exemplars. The exemplar anchors the descriptor vocabulary to the epistemic-function standard: an abstract instruction `**[LABEL]** -- [descriptor]` allows the model to populate the descriptor with topic names or sequential position labels ("questions about null cases," "null-hypothesis questions before departure conditions") that satisfy the pattern syntactically but fail C-54's epistemic-function test. A worked example (`**NULL REGISTER** -- Epistemic status of null cases`) demonstrates what an epistemic-function descriptor looks like in the compound form -- the cognitive-work framing is shown, not only described. C-56 requires the schema to declare the compound-header template; C-57 requires that template to include at least one concrete worked example so the epistemic-function descriptor standard is demonstrated within the schema artifact itself, not only stated as a separate criterion.

**R17 triple structure:**

- **C-55** (header-form causal questions, per-question-block): the causal question set must include questions grounding the compound header form -- asking why the register section header requires co-declaration of label identity and epistemic function. Close-marker-only questions (NCQ) do not substitute.
- **C-56** (schema-declared compound header template, per-schema-artifact): the schema artifact must include the compound header template as a named schema field so Role 2's compliance is structural self-consistency, not instruction-following.
- **C-57** (exemplar-grounded compound header template, per-schema-template): the schema artifact's compound header template must include at least one concrete worked example demonstrating the epistemic-function descriptor vocabulary so the model can match descriptor quality to the shown standard, not only to an abstract rule.

C-55 concerns the causal question block; C-56 concerns the schema artifact; both are independent phases. C-57 requires C-56 as a structural precondition (exemplar requires a template to ground). C-56 benefits from C-55 (schema template fields are better grounded when derived from causal reasoning) but does not require it as a formal precondition -- a schema can declare a compound-header template without preceding causal questions, satisfying C-56 but not C-55. Full expression requires all three: C-55 derives the compound form causally, C-56 binds it in the schema artifact, C-57 demonstrates the descriptor standard with a worked example. V-01 and V-04 carry C-55 and C-56 but not C-57 (template without examples). V-05 carries all three. V-02 and V-03 carry none of C-55/C-56/C-57.
...
## Aspirational Criteria (weight: 98 points total, 2 pts each)
...
| Criterion | Source pattern | What v17 covered | What v18 adds |
|-----------|---------------|-----------------|---------------|
| C-55 | R17: C-52 differential -- V-01/V-04/V-05 PASS via NHQ-1/NHQ-2 questions that causally derive the compound header form; V-02 has NCQ questions (close-marker form only); V-03 has no NHQ questions at all. Both V-02 and V-03 fail C-52 despite having other structural elements. | C-41 requires causal questions grounding the adjacency marker. C-48 requires null-hypothesis and departure-property questions. Neither criterion requires questions that specifically ground the compound header form -- the header artifact is distinct from the close-line artifact. | Requires at least two causal questions specifically grounding the compound register header form -- asking why label identity and epistemic-function descriptor must be co-declared in a single parseable compound line. Close-marker-form questions (NCQ) do not substitute. |
| C-56 | R17: C-52 differential -- V-01/V-04/V-05 PASS because Schema Area 3 declares the compound header template as a named schema field. V-02's Schema Area 3 covers dual-assertion manifest closes only; V-03's Schema Area 3 covers dual-assertion manifest closes only. Both fail C-52. | C-39 requires the model to produce a schema artifact declaring the inert-default rule and manifest-adjacency rule. C-52 requires the compound header form in the output. No criterion requires the schema artifact itself to include a compound-header template field -- the compound form could be adopted from prompt instructions rather than from the self-authored schema. | Requires the schema artifact to include a named compound-header template field so Role 2 is bound by its own prior schema production, not by an external prompt instruction. Omitting the template from the schema leaves the register header form without schema-level enforcement. |
| C-57 | R17: C-52 V-05 evidence -- "Schema Area 3b declares compound header template with examples." V-01 and V-04 pass C-52 and C-56 with a template but no exemplar; V-05 passes all three with an exemplar-grounded template. | C-56 requires the schema to declare the compound-header template. C-54 requires the descriptor content to characterize epistemic function rather than topic or position. No criterion requires the schema template to include a worked example that demonstrates the epistemic-function descriptor standard. | Requires the schema artifact's compound-header template to include at least one concrete worked example (e.g., `**NULL REGISTER** -- Epistemic status of null cases`) so the descriptor standard is shown, not only declared. Without an exemplar, the abstract pattern allows descriptors that satisfy compound form syntax while failing the epistemic-function content test. |

---

**R17 triple structure:**

- **C-55** (header-form causal questions, per-question-block): the causal question set must include questions grounding the compound header form -- why the register section header requires co-declaration of label identity and epistemic-function descriptor. Close-marker-only questions (NCQ type) are a different epistemic target and do not substitute.
- **C-56** (schema-declared compound header template, per-schema-artifact): the schema artifact must include the compound-header template as a named schema field, binding Role 2's header format to its own prior schema production rather than to a prompt instruction.
- **C-57** (exemplar-grounded compound header template, per-schema-template): the schema artifact's compound-header template must include at least one concrete worked example demonstrating the epistemic-function descriptor vocabulary so the model can match descriptor quality to a shown standard.

C-55 concerns the causal question block; C-56 concerns the schema artifact -- they are independent phases. C-57 requires C-56 as a structural precondition (exemplar requires a template to ground it in). C-56 benefits from C-55 (schema templates are better grounded when derived from causal questions) but does not require C-55 as a formal precondition -- a schema can declare a compound-header template without preceding causal questions, satisfying C-56 but not C-55. Full expression requires all three: C-55 grounds the compound form in causal reasoning, C-56 binds it in the self-authored schema artifact, C-57 demonstrates the descriptor standard with a concrete exemplar. V-01 and V-04 carry C-55 and C-56 (causal questions + schema template, no exemplar) but not C-57. V-05 carries all three. V-02 and V-03 carry none.
