Reading the Round 3 scorecard for systematic gap signals before writing the updated rubric.

The two patterns to extract:

- **C-15 — Delta pre-composition**: C-09 fails across all three Round 3 variants. The universal failure signals a missing analytic step — the contrastive discovery is never named before writing begins. Parallel to C-10 (pattern pre-composition), the fix is requiring a discrete pre-writing delta identification, not just discovery language in prose.

- **C-16 — Evidence-verb self-certification**: C-07 is PARTIAL in all three variants. No variant includes a calibration gate — they list valid verbs but never require the model to name the evidence posture that produces the verb. The excellence signal: an explicit mapping sentence in the recommendation beat.

---

# Quest Rubric — `topic-story` v4
