Written to `simulations/quest/rubrics/quest-score-rubric-v8-2026-03-16.md`.

---

**Two excellence patterns extracted from R7:**

**C-23 — Model evidence locatability assertion** (correctness)
Source: V-01 MODEL CELL `"Locatability test: YES -- a reader can find this requirement in V-01's Phase 1 / 1a block."` Applies the binary-declaration pattern (established by C-12 for arithmetic, C-18 for symmetry, C-19 for entry gate) to the evidence quality of the anchor cell itself. An output can pass C-11 and C-14 (anchor exists and is placed at entry) while failing C-23 (locatability is inferrable by searching, but undeclared).

**C-24 — Bilateral audit PARTIAL-reason labeling** (correctness)
Source: V-01 Phase 1d sweep rows `"PARTIAL -- no prior-round data; N/A rule applied correctly"` and `"PARTIAL -- no discrepancy found; N/A rule applied correctly"`. Without reason phrases, a PARTIAL in the audit sweep is ambiguous between a correctly-applied N/A rule, a clean result, and a silently-skipped comparison — indistinguishable by scanning. An output can pass C-18 (sweep exists with binary verdicts) while failing C-24 (PARTIAL rows are bare).

**Formula update:** `aspirational_pass / 16 * 10` (was `/14`). N_aspirational = 5+3+16 = 24 total criteria.
ly"`.
Each PARTIAL-verdict row in the bilateral symmetry audit sweep includes a labeled reason phrase
explaining why the comparison received PARTIAL rather than Symmetric: YES or NO. Without this,
a PARTIAL verdict in the sweep is ambiguous between a correctly-applied N/A rule, a clean
result with no discrepancy, and a silently-skipped comparison -- indistinguishable by scanning.
Distinct from C-18 (bilateral audit sweep existence): C-18 requires a binary per-comparison
verdict; C-24 requires that PARTIAL verdicts carry reason labels that make the basis for the
PARTIAL auditable without reading underlying prose.

**Formula update:**
```
composite = (essential_pass / 5 * 60)
          + (recommended_pass / 3 * 30)
          + (aspirational_pass / 16 * 10)   <- was /14
```
...
| C-13 | **Formula version delta declaration** -- when the applied rubric version introduces formula parameter changes from a known prior version, the scoring output explicitly names what changed before any score is computed | correctness | A delta notice appears before or within the Phase 1 LOAD block that names: (a) the prior rubric version or formula parameter value, and (b) the current value -- e.g., "N_aspirational=16 (was 14 in v7)" or "Aspirational denominator changed: 14 -> 16". The notice must appear in the scoring output, not only in the prompt instructions. PARTIAL if the current formula is correctly loaded (C-01 PASS) but no prior-vs-current comparison is made and a version change is in scope. FAIL if a formula version change is in scope and no delta is declared, or if an outdated denominator appears in any computation. When no rubric version change is in scope, write "No prior version delta -- scoring under v[N] as the baseline version" -> PARTIAL (not FAIL). |
...
| C-23 | **Model evidence locatability assertion** -- the model evidence cell at Phase 2 entry includes a binary locatability assertion confirming the quoted or referenced material is findable at the named location in the scored output | correctness | The MODEL CELL or equivalent positive anchor at Phase 2 entry includes a labeled binary locatability assertion -- e.g., "Locatability test: YES -- a reader can find this requirement in [output ID], [section]" or equivalent named YES/NO field confirming the evidence is findable. The assertion must be part of the anchor block itself. PARTIAL if the model cell's evidence references a specific section and location (structurally locatable by inspection) but no explicit binary locatability assertion is present -- locatability is inferrable but undeclared. FAIL if no model cell exists at Phase 2 entry (implies C-11 FAIL or C-14 FAIL), or if the model cell's evidence is a criterion restatement with no locatable reference. N/A rule: if Phase 2 contains no model cell (C-11 FAIL), write "No model cell -- locatability assertion not applicable" -> PARTIAL (not FAIL). |
| C-24 | **Bilateral audit PARTIAL-reason labeling** -- each PARTIAL-verdict row in the bilateral symmetry audit sweep includes a labeled reason phrase identifying the basis for the PARTIAL verdict, making PARTIAL audit entries distinguishable from silently-skipped comparisons | correctness | Every PARTIAL-verdict row in the bilateral symmetry audit sweep (C-18) includes a reason phrase -- e.g., "PARTIAL -- no prior-round data; N/A rule applied correctly" or "PARTIAL -- no discrepancy found; N/A rule applied correctly" -- rather than a bare "PARTIAL" or "N/A" label. PARTIAL if at least one PARTIAL-verdict row includes a reason phrase but not all such rows do. FAIL if PARTIAL verdicts appear in the bilateral audit sweep without any reason labeling -- a reader cannot determine whether the PARTIAL reflects a correctly-applied N/A rule, a clean result, or a silently-skipped comparison. N/A rule: if the bilateral audit sweep is absent (C-18 FAIL) or produces no PARTIAL-verdict rows (all comparisons resolve Symmetric: YES or NO), write "No PARTIAL-verdict rows in bilateral audit -- reason labeling not applicable" -> PARTIAL (not FAIL). |
...
| Aspirational | C-09, C-10, C-11, C-12, C-13, C-14, C-15, C-16, C-17, C-18, C-19, C-20, C-21, C-22, C-23, C-24 | 16 | 10 |
...
final score is given (e.g., "E=4/5, R=2/3, A=13/16"). A final number alone is PARTIAL. Note the
aspirational denominator is now 16; any computation using /14, /12, /10, or /8 is a formula
version error.
...
current value only -- "N_aspirational=16" without "was 14 in v7". This is sufficient for C-01
...
**C-23 (model evidence locatability assertion):** C-23 is a tightening of C-11 and C-14.
C-11 asks: does a positive anchor exist? C-14 asks: is it at Phase 2 entry? C-23 asks: does
the anchor's own evidence pass a stated locatability test? An output can pass C-11 and C-14
(anchor exists and is placed at entry) while failing C-23 (no locatability assertion -- a
reader must attempt to find the referenced material to confirm it exists at the named location).
The locatability assertion applies the binary-declaration pattern used by C-12 (arithmetic),
C-18 (symmetry), and C-19 (entry gate) to the evidence quality of the model cell: rather than
having readers verify by searching, the scorer declares the result as a labeled binary field.
A "Locatability test: NO" earns PASS on C-23 (the mechanism reported accurately) while
indicating a defect in the model evidence itself. The R7 scorecard V-01 MODEL CELL's
"Locatability test: YES -- a reader can find this requirement in V-01's Phase 1 / 1a block."
was the first observed application of explicit locatability testing to the evidence anchor.

**C-24 (bilateral audit PARTIAL-reason labeling):** C-24 is a tightening of C-18. C-18 asks:
does a bilateral audit sweep exist with a binary verdict per comparison? C-24 asks: when a
comparison receives PARTIAL, does the row label the reason? An output can pass C-18 (sweep
exists with per-comparison verdicts) while failing C-24 (PARTIAL rows are bare -- a reader
cannot tell whether PARTIAL reflects an N/A rule correctly applied, a clean result, or a
silently-skipped comparison). The reason labeling makes every PARTIAL in the sweep as
auditable as its YES/NO verdicts: a reader can scan reason phrases to confirm deliberate,
rule-compliant application rather than ambiguous omission. The R7 scorecard V-01 bilateral
audit's "N/A rule applied correctly" labeling on PARTIAL rows was the first observed
application of this pattern. Score by scanning each PARTIAL-verdict row in the sweep for a
reason phrase.
...
**Why C-23 (model evidence locatability assertion) is Aspirational, not part of C-11 or C-14:**
C-11 requires that a positive anchor exists in the scored body. C-14 requires it at Phase 2
entry. C-23 requires that the anchor's own evidence carries a stated locatability test -- a
binary YES/NO confirming the referenced material is findable at the named location. An output
can pass C-11 and C-14 (anchor exists and is placed correctly) while failing C-23 (no
locatability assertion -- a reader must search the scored output to confirm the evidence is
accurate). C-23 applies the binary-declaration pattern to evidence quality: rather than
verifying the model cell's evidence by finding it, a reader can scan for the locatability
assertion and trust the scorer's self-verification. It is Aspirational because it requires
deliberate self-testing of the model cell beyond what correct placement already achieves. The
R7 scorecard V-01 MODEL CELL's "Locatability test: YES -- a reader can find this requirement
in V-01's Phase 1 / 1a block." was the first observed application of explicit locatability
testing to the evidence positive anchor.

**Why C-24 (bilateral audit PARTIAL-reason labeling) is Aspirational, not part of C-18:**
C-18 requires a bilateral audit sweep producing a binary per-comparison verdict. C-24 requires
that PARTIAL verdicts in the sweep carry labeled reason phrases. An output can pass C-18
(sweep exists with per-comparison verdicts) while failing C-24 (PARTIAL rows are bare -- the
basis for each PARTIAL is ambiguous). Without reason labeling, a PARTIAL in the audit sweep
is indistinguishable between: (a) an N/A rule correctly applied, (b) a comparison completed
with a clean result, or (c) a silently skipped comparison. C-24 closes this ambiguity by
requiring each PARTIAL row to name its basis, making the sweep's PARTIAL verdicts as auditable
as its YES/NO verdicts. It is Aspirational because it requires the scorer to add a labeled
reason phrase to each PARTIAL row beyond what C-18's binary-verdict requirement mandates.
The R7 scorecard V-01 bilateral audit's explicit "N/A rule applied correctly" labeling on
PARTIAL rows was the first observed application of this pattern.
