Reading the scorecard for Round 9 excellence signals not yet in v9. V-03's primary signal is "Phrasing Register — Imperative Micro-Instructions" — fill rules written as imperative verb commands (`PRINT`, `LOAD`, `VALIDATE`, `ENFORCE`) rather than descriptive prose. This is structurally distinct from C-28 (skeleton pre-declaration) and formalizes the instruction register that makes skeleton fill rules normatively unambiguous.
...
```markdown
# org:committee Rubric — v10

**Skill**: `org:committee`
**Version**: 10 (updated from Round 9 scoring — 2026-03-16)
**Changelog**: Added C-30 from R9 excellence signals. C-30 requires each fill rule within
the skeleton template to be written as an imperative micro-instruction (PRINT, LOAD,
VALIDATE, ENFORCE, REQUIRE) rather than descriptive prose. V-03's naming pattern
("Phrasing Register — Imperative Micro-Instructions") and its essential/recommended
pass evidence — "Phase 0 PRINT/LOAD; VALIDATE enforces challenge requirement; dissent
resolution path validation" — surface imperative phrasing as the instruction-register
complement to C-28's structural-declaration requirement. C-28 requires all structural
slots to exist as a pre-declared skeleton; C-30 requires each slot's fill rule to carry
a verb-first imperative so the skeleton's instruction set has uniform normative force.
A skeleton with correctly declared slots but descriptive fill rules (C-28 PASS, C-30
FAIL) admits softer interpretations of what filling the slot requires; imperative
micro-instructions eliminate that ambiguity structurally. Aspirational pool grows from
21 to 22 criteria; aspirational max grows from 42 to 44 pts; composite max is now 134.
...
## Aspirational Criteria (44 pts total — 22 criteria at 2 pts each)
...
composite = (essential_pass / 5 * 60)
          ...
          + (aspirational_pts / 44 * 44)   <- denominator was 21, now 22; pool was 42, now 44
```

Maximum composite: **134**
```
