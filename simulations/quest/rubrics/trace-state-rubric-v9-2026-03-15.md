Scanning the V-02 evidence column for structural patterns not yet mapped to a criterion slot.

**New signals from Round 10:**

1. **V-02 C-03 / C-06 / C-08 field labels** — e.g., `[C-03: minimum 2 per pass — never skip]`, `[C-06: uniform format throughout]` — the field label carries *both* a criterion ID and a brief behavioral directive; C-11 requires only that IDs appear in field labels, but does not require the label to also encode the compliance instruction; the instruction-fused form makes the requirement self-documenting at the point of schema generation without external cross-reference; pattern appears across multiple field labels, making it a robust structural signal. → **C-30**

2. **V-02 C-04 / C-09 sub-element decomposition** — `Defect type [C-04a]`, `Triggering operation [C-04b]`, `Reason [C-04c]`; `[C-09a]`–`[C-09d]` — multi-element criteria are decomposed into individually labeled sub-elements within the output schema; a field carrying only the parent ID (`[C-04]`) does not qualify; this is distinct from C-11 (IDs present) and C-14 (schema enforces satisfaction): this is about the *granularity* of the ID decomposition, making each element independently auditable. → **C-31**

Aspirational pool grows from 21 to 23. Cap and any-5 threshold unchanged.

---

## trace-state rubric (v25) — outer quest rubric v9

**5 essential + 3 recommended + 23 aspirational (cap 5) = 31 criteria**
...
### Aspirational (10 pts — 2 pts each, any 5 of 23)
...
- **C-30** is distinct from C-11: C-11 requires only that a criterion ID appears in a field label; C-30 requires the label to also carry a behavioral directive fused with the ID in the same annotation. A label like `[C-06]` satisfies C-11 but not C-30; a label like `[C-06: uniform format throughout]` satisfies both.
- **C-31** is distinct from C-11 and C-14: C-11 requires IDs to be present; C-14 requires the schema to make satisfaction structurally unavoidable; C-31 requires a multi-element criterion to be *decomposed* into individually labeled sub-element slots (e.g., `[C-04a]`, `[C-04b]`, `[C-04c]`). A criterion with a single schema slot carrying a parent ID does not qualify regardless of how detailed the label is. At least one multi-element criterion must be decomposed.
