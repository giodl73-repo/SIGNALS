`listen-adoption-rubric-v18-2026-03-15.md` written. Two new criteria extracted from R17 excellence signals:

---

**C-50 -- DOWNSTREAM CITATION CONTRACT rows carry Gate H Status field per I-0X** (V-03 Axis C, 5 pts)
Each I-0X row in the SECTION A DOWNSTREAM CITATION CONTRACT (C-49) carries a `Gate H Status: [PASS / FAIL -- cite location or violation]` fill field, completed by Gate H during audit. Contract row is both audit target and audit record: declaration and compliance co-present in the same structural slot. Dependency: C-49.

**C-51 -- SECTION K mirrors DOWNSTREAM CITATION CONTRACT compliance as terminal contract record** (V-02 Axis B, 5 pts)
SECTION K carries a per-I-0X compliance mirror of the DOWNSTREAM CITATION CONTRACT (C-49), recording which citation sinks were satisfied and at which structural locations. Completes the contract lifecycle: source declaration (C-49, SECTION A) → per-row audit co-location (C-50, Gate H Status) → terminal compliance record (C-51, SECTION K). Dependency: C-49, C-20.
...
**Max composite: 295 -> 305. Golden threshold (80%): 244 pts.**
