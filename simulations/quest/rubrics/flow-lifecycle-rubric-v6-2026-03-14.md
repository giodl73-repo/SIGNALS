Written to `simulations/quest/rubrics/flow-lifecycle-rubric-v6-2026-03-14.md`.

---

**3 new criteria extracted from R5 excellence patterns:**

**C-24 — Evidence Field Format Dual Redundancy** (depth)
The V-05 escalation of C-22. Preamble-only placement (V-02/V-03) passes C-22; V-05 additionally repeats the concrete named example in each B-NN per-block hint. Dual-location makes the format contract discoverable regardless of reader entry point. Neither location can substitute with a bracket template. C-24 → C-22.

**C-25 — Anti-Embedding Dual Enforcement** (format)
The V-05 escalation of C-23. R5 establishes that section-proximate and global-preamble anti-embedding are equivalent for C-23 pass; V-05 places the prohibition in both. C-25 encodes dual-location enforcement: global preamble AND opening constraint of the CHAIN STATUS section instruction. C-25 → C-23.

**C-26 — Evidence Field Axis Separation** (format)
The structural encoding of R5 pattern 3: C-19 (structure) and C-22 (contract) are independent axes. C-26 requires the Evidence field instruction to present both axes as explicitly labeled, visually distinct components -- `Required format:` and `Fail condition:` as separate labeled sub-fields. Bundling both into a single prose hint fails. C-26 → C-22.

**Formula**: `/15` → `/18`; aspirational total remains 10 pts.

**Dependency chain (v6):** C-18→C-15; C-19→C-16; C-20→C-14+C-16; C-21→C-13+C-15+C-17; C-22→C-19; C-23→C-20; **C-24→C-22**; **C-25→C-23**; **C-26→C-22**.
 specification (`Required format: [State name -- S-ID]: AT-RISK, causal source: B-[ID]`) and a contract specification (concrete filled-in domain example + `Fail condition:` label). When both axes are merged into a single prose hint, a model may satisfy one while missing the other; explicit labeling makes each axis an independent verification target. C-26 cannot pass if C-22 fails.

**Formula**: `/15` → `/18`; aspirational total remains 10 pts.

**Full dependency chain (v6):**
