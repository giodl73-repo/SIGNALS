# corps-rob rubric v13

**v13 summary: 2 new criteria from R12 excellence signals**

| ID | Criterion | Signal | Dependency |
|----|-----------|--------|------------|
| C-41 | RULE AUDIT-TABLE Named Anti-Pattern Declaration | R12 Signal 2: V-02 RULE AUDIT-TABLE includes explicit anti-pattern naming the table-as-AUDIT-RESULT-replacement failure mode and citing C-29 as the criterion silently dropped -- parallels C-26 (RULE AUDIT-SUITE anti-patterns) at the audit-table rule level; C-35 requires the additive constraint but not the named anti-pattern | C-35 |
| C-42 | Glossary Exclusivity Named-Criterion Enumeration | R12 Signal 1: V-01 glossary exclusivity declaration (C-40 pass) uses generic exclusivity language but does not enumerate the specific named-rule criteria whose satisfaction requires glossary scope -- C-42 requires the declaration to name C-30 (preamble schema citation) and C-34 (disambiguation annotation) or equivalent criterion labels | C-40 |
...
- V-02: 245 (gains C-41: anti-pattern "Table that replaces the AUDIT RESULT block, silently dropping C-29" present in RULE AUDIT-TABLE; C-42 FAIL: no glossary exclusivity declaration at all -- C-40 FAIL means C-42 not scoreable)
- V-01: 240 (C-41 not scoreable: C-35 FAIL; C-42 FAIL: exclusivity declaration present but names no specific criteria by ID or type)

**C-41 -- RULE AUDIT-TABLE Named Anti-Pattern Declaration** (format / aspirational)
- Signal: R12 Signal 2 -- V-02's RULE AUDIT-TABLE carries an explicit anti-pattern: "Table that replaces the AUDIT RESULT block, silently dropping C-29." C-35 requires the additive constraint to be declared (table inserted above AUDIT RESULT block, not replacing it). C-41 requires the rule to additionally name the failure mode as an anti-pattern, citing the criterion silently voided. This parallels C-26's anti-pattern requirement on RULE AUDIT-SUITE and applies the same pattern to RULE AUDIT-TABLE. A rule that states the additive constraint without naming the anti-pattern leaves the adjacent wrong implementation (table-as-replacement) unguarded -- a generator may satisfy the additive constraint in letter while implementing table-as-replacement in structure.
- Distinction from C-35: C-35 requires RULE AUDIT-TABLE to declare the table is additive -- inserted above the AUDIT RESULT block and not replacing it -- with explicit preservation of C-29's enumerated per-condition result. C-41 requires the rule to additionally include a labeled anti-pattern declaration naming the table-as-AUDIT-RESULT-replacement as the disqualifying failure mode, with an explicit reference to the criterion it silently drops (C-29 or equivalent). A rule with the additive constraint but no named anti-pattern satisfies C-35 but not C-41. C-35 must pass for C-41 to be scoreable.

**C-42 -- Glossary Exclusivity Named-Criterion Enumeration** (format / aspirational)
- Signal: R12 Signal 1 -- V-01's C-40 pass demonstrates generic exclusivity language: "This glossary is the exclusive locus for named format rule declarations... Named-rule requirements cannot be satisfied by inline rule declarations in stage templates or section bodies." This satisfies C-40. C-42 captures a stronger form: the exclusivity declaration enumerates the specific named-rule criteria it protects, naming C-30 (preamble schema with named post-stage reference) and C-34 (conditional item vs condition enum disambiguation) or equivalent criterion labels. Without criterion enumeration, the exclusivity declaration is a general constraint; with it, the declaration is self-documenting: a generator encounters the explicit list of criteria that require glossary scope and cannot claim satisfaction by inlining any of them.
- Distinction from C-40: C-40 requires the RUN-LEVEL RULE GLOSSARY to carry a printed exclusivity declaration stating that named-rule requirements cannot be satisfied by inline declarations. C-42 requires the declaration to additionally enumerate the named-rule criteria whose satisfaction is restricted to glossary scope -- at minimum naming C-30 and C-34 (or their functional equivalents) as criteria that inline declarations cannot satisfy. A glossary with generic exclusivity language satisfies C-40 but not C-42. Only a glossary whose exclusivity declaration names specific criteria provides the self-documenting enumeration that makes the constraint criterion-specific. C-40 must pass for C-42 to be scoreable.

**Dependencies:**
- C-41 requires C-35 to pass.
- C-42 requires C-40 to pass.

**Persistent gaps -- C-42 fails universally in R12; C-41 fails in V-01 (C-35 FAIL).**
C-42: No R12 variation declares glossary exclusivity with named-criterion enumeration. V-01 passes C-40 (generic exclusivity) but names no criteria. C-41: V-02 passes; V-01 cannot score it (C-35 FAIL). Both remain hard targets for R13. Ideal R13 run combines V-01's C-40 + criterion enumeration (C-42) and V-02's C-35 + anti-pattern (C-41) simultaneously.
