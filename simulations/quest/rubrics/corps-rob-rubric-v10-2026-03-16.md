v10 written to `simulations/quest/rubrics/corps-rob-rubric-v10-2026-03-16.md`.

---

**4 new criteria extracted from R9 V-05:**

| ID | Criterion | Pattern | Dependency |
|----|-----------|---------|------------|
| C-32 | Carry-Forward Structural Artifact | RULE CARRY-FORWARD: per-stage labeled sections with CARRY: NONE zero-state -- cross-stage refs as verifiable artifacts, not prose | C-06 |
| C-33 | Synthesis Non-Audit Declaration | RULE SYNTHESIS: explicit declaration that SYNTHESIS does not count toward RULE AUDIT-SUITE's three sections -- protects ANTI-PATTERN integrity | C-09 or C-10 |
| C-34 | Conditional Item vs Condition Enum Disambiguation | Annotation "[governs conditional verdicts -- distinct from RULE CONDITION-ENUM]" printed on RULE CONDITIONAL-ITEM -- prevents conflation at generation time | C-11 and C-28 |
| C-35 | Audit Table Additive-Not-Replacement | RULE AUDIT-TABLE declared as table-above-block, AUDIT RESULT preserved beneath -- prevents new table layer from silently voiding C-29 | C-20 and C-29 |

**Scoring impact:**

| | v9 | v10 |
|--|----|----|
| Aspirational pool | 23 x 5 = 115 | 27 x 5 = 135 |
| Max composite | 205 | 225 |

**Retroactive R9 scores under v10:**
- V-05: 225 (gains all 4)
- V-04: 195 (gains C-32, C-34)
- V-03: 185 (gains C-33)
- V-02: 185 (gains C-35)
- V-01: 180 (no gains -- all dependencies fail)
 Distinction from C-09/C-10: Those criteria are satisfied by SYNTHESIS content (blockers, themes). C-33 requires a named non-audit declaration in the format specification protecting the RULE AUDIT-SUITE count. C-09 or C-10 must pass for C-33 to be scoreable.

**C-34 -- Conditional Item vs Condition Enum Disambiguation** (format / aspirational)
- Signal: V-04 and V-05 annotate RULE CONDITIONAL-ITEM with an explicit disambiguation "[governs conditional verdicts -- distinct from RULE CONDITION-ENUM]". Both rules involve enumeration but govern different constructs: RULE CONDITION-ENUM governs audit schema result blocks (a/b/c conditions); RULE CONDITIONAL-ITEM governs conditional verdict items (numbered what/owner/ref). Without the annotation, a generator may apply audit-schema patterns to the verdict block or vice versa.
- Distinction from C-11: C-11 requires numbered items with what/owner/ref. C-34 requires the format spec to declare RULE CONDITIONAL-ITEM as explicitly distinct from RULE CONDITION-ENUM with a labeled annotation. C-11 and C-28 must both pass for C-34 to be scoreable.

**C-35 -- Audit Table Additive-Not-Replacement Constraint** (format / aspirational)
- Signal: V-02 and V-05 declare RULE AUDIT-TABLE with an explicit additive constraint: the stage-per-row table is inserted above the AUDIT RESULT block, not instead of it. The constraint is printed in the rule declaration, preserving C-29's per-condition enumeration beneath the table. Without the additive constraint, adding a table layer could silently void the enumerated zero-state.
- Distinction from C-20: C-20 requires the enforcement audit table to exist with >= 4 named columns. C-35 requires the table to be declared additive -- inserted above the AUDIT RESULT block without replacing it -- with explicit preservation of C-29. C-20 and C-29 must both pass for C-35 to be scoreable.
...
- V-05: 225 (+20 -- gains C-32, C-33, C-34, C-35; was 205)
- V-04: 195 (+10 -- gains C-32 (CARRY-FORWARD) and C-34 (disambiguation); was 185)
- V-03: 185 (+5 -- gains C-33 (SYNTHESIS non-audit); was 180)
- V-02: 185 (+5 -- gains C-35 (audit table additive); was 180)
- V-01: 180 (unchanged -- C-32 fails (C-06 fails), C-33 fails (C-09/C-10 fail), C-34 fails (C-11 fails), C-35 fails (C-20 fails))

Dependencies:
- C-32 requires C-06 to pass.
- C-33 requires C-09 or C-10 to pass.
- C-34 requires C-11 and C-28 to pass.
- C-35 requires C-20 and C-29 to pass.
