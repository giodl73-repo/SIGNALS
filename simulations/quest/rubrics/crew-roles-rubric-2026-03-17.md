...
- C-03: Output written to `.roles/{area}/` (downstream contract for org-review/org-committee)
...
- **Text**: Every role file in the output registry must contain all five required fields:
  ...
  as a missing field.
- **Pass condition**: Every role file in `.roles/{area}/` has all 5 fields present and
...
- **Weight**: essential
- **Category**: format
- **Text**: All role files are written to `.roles/{area}/` where `{area}` is derived from the
  ...
  skills.
- **Pass condition**: Files exist at `.roles/{area}/*.md`. The `{area}` segment is non-generic
...
| Files written to wrong path | C-03 | Output at `roles/` root or `output/` instead of `.roles/{area}/` |
