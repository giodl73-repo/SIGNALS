...
- Aspirational: cap 10 pts (any 12 of 24 criteria earn full cap; each criterion +1 pass toward threshold)
...
## Aspirational Criteria (10 points total, capped; any 12 of 24 earns the cap)
...
- **Weight**: aspirational
- **Category**: depth
- **Text**: The REPORT includes an ASSUMPTION CONSERVATION section that states a per-
  disposition partition equation: N_finding + N_covered + N_investigation + N_none = N_total,
  where each term is the count of assumptions carrying that disposition, and N_total equals
  the INERTIA SURFACE catalog total. A residual check confirms the equation balances
  (residual = 0). This is the strongest realization of C-28 (full disposition traceability
  chain): each disposition maps to a count, the counts partition N_total exactly, and the
  arithmetic is auditable in a single section without traversing the full document. While
  C-30 checks that all assumptions were assigned some disposition (unassigned = 0), C-31
  checks that the four disposition counts sum correctly to the catalog total -- together
  they verify both completeness (no assumption missed) and correctness (counts are
  consistent). Requires C-24 (four dispositions defined) and C-29 (INERTIA SURFACE catalog)
  to be structurally satisfiable.
- **Pass condition**: REPORT includes an ASSUMPTION CONSERVATION block (or equivalent named
  section) with per-disposition row counts as labeled numeric fields (N_finding, N_covered,
  N_investigation, N_none), a summation equation (N_finding + N_covered + N_investigation
  + N_none = N_total), and a residual check comparing N_total to the INERTIA SURFACE
  catalog count (residual = 0). Fail if the conservation equation is absent from REPORT.
  Fail if any of the four disposition counts is omitted from the summation. Partial pass
  (half credit) if per-disposition counts are present and summed but no residual check
  against the INERTIA SURFACE catalog total is stated, or if the equation omits one
  disposition type.
