Rubric v7 written to `simulations/quest/rubrics/discover-competitors-alt-rubric-v7-2026-03-17.md`.

---

**3 new aspirational criteria (C-31 -- C-33):**

| ID | Pattern | Source | Pass Condition |
|----|---------|--------|----------------|
| C-31 | Write-token-as-gate-row | R7 V-03, V-05 | The INERTIA-REF write instruction is a dedicated PASS/FAIL table row with Check, Pass condition (containing the write directive + token format), and named Failure state ("Inertia write failure"). Prose adjacent to a gate table does not satisfy. Strictly stronger than C-30. |
| C-32 | Spec-first PREFLIGHT | R7 V-02, V-05 | OUTPUT CONTRACTS is the first named subsection within PREFLIGHT — before any gate subsection. A PREFLIGHT block where OUTPUT CONTRACTS follows one or more gates does not satisfy even if it satisfies C-28. Strictly stronger than C-28. |
| C-33 | Forward-declared cross-dimensional proof slot | R7 V-05 | The output contract table includes a dedicated slot naming the cross-dimensional proof requirement (e.g., "Focus-scope evidence"). A requirement stated only within Phase 4's instruction does not satisfy. Extends C-19 and C-23 to the cross-dimensional proof specifically. |

**Scoring changes:**
- Aspirational tier: 22 → 25 criteria, max 110 → 125
- Max composite: 200 → 215
- Formula denominator: `aspirational_pass / 25 × 125`
- Grade bands rescaled: Exceptional ≥ 202, Strong ≥ 181, Passing ≥ 90 (golden threshold unchanged)

**Dependency chain:**
- C-31 → C-30 → C-22 (each strictly stronger than the next)
- C-32 → C-28 → C-26 (same chain)
- C-33 extends C-19 + C-23 to the cross-dimensional slot; does not imply either in general
...
## Aspirational Criteria (weight = 125 points total, 5 points each)
...
Grade bands rescaled proportionally from v6 (v6 max = 200; v7 max = 215).
