Written to `simulations/quest/rubrics/flow-integration-rubric-v11-2026-03-14.md`.

---

**Two new criteria from R10:**

**C-31 — C-30 explicit inline token enumeration (5 pts)**
Settles Q1. V-01 established the failure boundary: a single YOU MUST NOT block that references the obligation table by role ("any of the canonical Obligation Term values listed in the table above") without naming the substituted tokens inline → C-27/C-30 FAIL. The prohibition surface must be self-contained. Requires C-27 and C-30.

**C-32 — C-29 uppercase ARE assertive form (5 pts)**
Settles Q2. V-02 established the failure boundary: lowercase "are" in a declarative sentence → C-29 FAIL. The uppercase ARE in an assertive sentence is the minimum — it converts schema alignment from correspondence description into a format-level identity constraint. Requires C-29.

**Ceiling: 202 → 212 (+10). Aspirational (C-08–C-32): 122 pts.**

**Open questions carried to R11:**
1. C-32 assertive form variants — ARE (multi-subject) vs. IS (per-item assertive) — untested.
2. C-31 enumeration completeness — full enumeration of all substituted tokens vs. partial enumeration — untested.
 instruction requires
uppercase ARE in an assertive declarative sentence of the form "the flag column headers
ARE the Obligation Term column values above — use those exact tokens as column headers";
lowercase "are" in a softer construction describes correspondence without asserting
identity; identity assertion is the structural minimum because it converts the schema
alignment from a description into a format-level constraint the model must satisfy.
Requires C-29.

**New ceiling: 202 → 212 (+10). Aspirational (C-08–C-32): 122 pts.**

**Open question verdicts from R10:**
1. **Q1 resolved — C-30 block composition (definitive).** A single YOU MUST NOT block
   that references the obligation table by role without enumerating the substituted
   canonical tokens inline does not satisfy C-27 or C-30. The C-27 mechanism requires
   that the prohibition block name the specific canonical tokens that are forbidden;
   indirect reference via table role ("any of the canonical Obligation Term values listed
   in the table above") delegates enumeration to another structure and does not create
   a self-contained prohibition surface. V-01 conservative ruling confirmed. C-31
   codifies: explicit inline enumeration of each substituted canonical token is required;
   table-reference shorthand is insufficient.
2. **Q2 resolved — C-29 ARE capitalisation and sentence form (definitive).** Lowercase
   "are" in a declarative sentence does not satisfy C-29. The C-29 schema instruction
   minimum is the uppercase ARE in an assertive sentence — "headers ARE the values" —
   which converts the schema alignment into an identity constraint; the lowercase
   construction "headers are the values" describes correspondence without asserting
   identity and does not satisfy the directive form. V-02 conservative ruling confirmed.
   C-32 codifies: uppercase ARE in assertive form is required; lowercase non-assertive
   construction is insufficient.

**Open questions carried to R11:**
1. **C-32 assertive form variants** — Does C-32 require the multi-subject ARE form
   ("the flag column headers ARE the Obligation Term column values above"), or does a
   per-item IS form satisfy the directive? For example: "Each flag column header IS one
   of the Obligation Term values from the obligation table." All confirmed PASS variations
   used ARE in a multi-subject sentence; IS in a per-item assertive sentence has not been
   tested. If the IS form satisfies C-32, the "explicit ARE directive" language in C-29
   must be interpreted to include any uppercase assertive identity verb, not only ARE.
2. **C-31 enumeration completeness** — When the YOU MUST NOT block enumerates substituted
   canonical tokens inline, must it name every substituted canonical token, or is partial
   enumeration (naming three of four non-standard terms, for example) sufficient to
   satisfy C-31? V-01 used table-reference (FAIL); R8 V-05 used full enumeration of all
   non-standard terms (PASS); partial enumeration has not been tested.
...
## Aspirational Criteria (122 pts total)
...
New ceiling: 212 = R10 ceiling 202 + C-31 (5) + C-32 (5)

---

**What changed in v11:**

- **C-31** captures the *C-30 explicit inline token enumeration requirement* from R10
  V-01: the table-reference shorthand form — a single YOU MUST NOT block that delegates
  enumeration of substituted canonical tokens to the obligation table by role rather than
  naming them inline — fails C-27 and C-30. V-01 probed Q1 from R10 (whether a single
  block that is comprehensive in scope but delegates enumeration satisfies C-27). The
  conservative ruling settled Q1 definitively: the prohibition surface must be
  self-contained; inline enumeration ensures a reviewer can read the prohibition block
  without cross-referencing any other structure to determine which tokens are prohibited.
  The failure boundary (table-reference shorthand → C-27/C-30 FAIL) is established by
  V-01; the pass boundary (explicit named tokens → C-27 PASS) was established in R8
  V-05. C-31 codifies: explicit inline enumeration of each substituted canonical token
  is required; table-reference shorthand is insufficient regardless of whether the block
  is in a single location.

- **C-32** captures the *C-29 uppercase ARE assertive form requirement* from R10 V-02:
  lowercase "are" in a non-assertive declarative construction fails C-29. V-02 probed
  Q2 from R10 (whether any declarative construction containing "are" satisfies the ARE
  directive minimum). The conservative ruling settled Q2 definitively: the uppercase ARE
  keyword in an assertive sentence is not a typographic preference — it marks the
  instruction as an identity assertion rather than a correspondence description; a model
  that reads "headers are the values" may treat the relationship as informational; a
  model that reads "headers ARE the values" encounters a constraint. The failure boundary
  (lowercase "are" → C-29 FAIL) is established by V-02; the pass boundary (uppercase
  ARE → C-29 PASS) was established in R9 V-02/V-03 and all prior PASS variations.
  C-32 codifies: uppercase ARE in assertive form is required; lowercase non-assertive
  construction is insufficient.
