`listen-adoption-rubric-v14-2026-03-14.md` written. Two new criteria extracted from R13 excellence signals:

---

**C-40 — PART 2 expansion decomposes into CHASM-A/B/C EXPANSION structural subsections with PHASE 3 back-references** (5 pts)
Source: V-01 R13. C-39 enforcement pattern applied one generation layer deeper. PHASE 3's CHASM-A/B/C slots propagate into expansion depth: "Expanding CHASM-A from PHASE 3 --" makes each expansion element a named structural slot. A missing element in PART 2 is a missing named subsection, not a missing list item. Dependency: C-39.

**C-41 — TABLE 1 Inertia ID column — three contracts co-present per row** (5 pts)
Source: V-02 R13. Extends TABLE 1 from two co-present contracts (C-01 + C-37) to three by adding `Inertia ID: [cite I-0X from SECTION A]` as a 4th column. Forces C-11 citation at TABLE 1 row generation time rather than once in SECTION A. Same re-assertion pattern as C-37 applied to C-11. Dependency: C-37, C-11.

**Max 245 → 255.** New dependency chain additions:
```
C-39 -> C-40
C-37 -> C-41  (also requires C-11)
```

TABLE 1 column chain is now four deep: `C-01 -> C-26 -> C-37 -> C-41`. Projected ceiling for R14 V carrying both axes: 250/255 (C-19 paradox persists).
three by adding `Inertia ID: [cite I-0X from SECTION A -- named inertia explaining this
archetype's resistance]` as a 4th column. This forces the named inertia citation (C-11) to be
satisfied per role at TABLE 1 generation time, not once per archetype in SECTION A. Same
generation-time re-assertion pattern as C-37 (incumbent dependency per row) applied to C-11.
C-41 tests whether the three-contract co-presence is encoded structurally in TABLE 1.
Dependency: C-37, C-11.
...
The TABLE 1 column chain now reads `C-01 -> C-26 -> C-37 -> C-41` (four deep, three co-present
contracts per row). Projected scores for R14 V carrying both new axes: 250/255 (C-19 paradox
persists).
...
**Max composite: 245 -> 255. Golden threshold (80%) unchanged.**
