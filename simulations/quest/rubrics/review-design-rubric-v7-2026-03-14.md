Written to `simulations/quest/rubrics/review-design-rubric-v7-2026-03-14.md`.

---

**Three new aspirational criteria extracted from R6:**

**C-21 | BLOCK 0 Pre-Scan Signal Catalogue**
- Confirmed by: V-03/V-04/V-05 introduced it; V-01/V-02 scored 100 without it
- Pattern: extends anchor-then-verify (F-09 for experts, F-12 for P1 counts) upstream to signal detection itself — signals locked in BLOCK 0 before BLOCK 1 runs, with a formal SHALL constraint preventing post-hoc rationalization

**C-22 | BLOCK 3 Consensus in 4-Column Structural Table Form**
- Confirmed by: V-02/V-04/V-05 introduced it; V-01/V-03 scored 100 without it
- Pattern: closes the last structural gap in column-enforcement — every mandatory lifecycle block (BLOCK 1, 1.5, 2, 3, 5) now has a table-form criterion; the Synthesis column makes C-18/F-11 fire on an empty cell rather than absent prose

**C-23 | Register Isolation for Formal Lifecycle Block Headers**
- Confirmed by: V-01/V-04/V-05 introduced it; V-02/V-03 scored 100 without it
- Pattern: formalizes the R2 mechanism — informal calibration language in a block header erodes enforcement even when the requirement is correctly stated; register isolation becomes a named, precisely localizable property

**Formula:** denominator 13 → 16, each aspirational criterion ~0.625 pts.
-conformant, fires, halt). Informal calibration language in headers erodes
enforcement even when the underlying requirement is correctly stated -- R2 confirmed this
mechanism for C-08. Register isolation is now a named, testable property rather than an
incidental style choice. V-02/V-03 scored 100 without it -- aspirational confirmed.

**Formula:** denominator 13 -> 16. Each aspirational criterion now worth ~0.625 pts. Score
ceiling 100 preserved.
...
Note: aspirational denominator changed from 13 to 16 in v7 (added C-21, C-22, C-23).
Each aspirational criterion contributes ~0.625 points at full pass.
