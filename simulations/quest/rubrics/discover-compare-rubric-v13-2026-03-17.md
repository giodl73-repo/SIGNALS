`discover-compare-rubric-v13-2026-03-17.md` written.

**What v13 contains vs. v12:**

The criteria table is unchanged (C-01 through C-33, denominator /26). R14 produced no structural observations beyond what C-32 and C-33 already encode -- V-01 hit the 100.00 ceiling with all 26/26 aspirational passing.
...
|---------|-----------|-------|
| `deviation-only-routing-scope-declaration` | C-32 | note |
| `body-order-layer-token` | C-33 | note |
| `c32-c33-additivity` | C-32, C-33 | note |
| `body-order-layer-none-pass` | C-33 | note |

The C-33 criterion body also gains one explicit sentence: **`BODY-ORDER-LAYER: none` satisfies C-33 PASS** -- R14 V-05 confirmed declared-absence is a full structural declaration, not partial. That interpretation is now locked in the criterion text rather than living only in the watch list.
xact form as an explicit example. This interpretation is locked.

**Denominator ladder confirmed**: /26 -> /25 (one not scored) -> /24 (C-30 and C-32 both not scored when no standalone BODY ORDER). V-05 pattern: D = /24 when C-30 not scored (no BODY ORDER) and C-32 not scored (same condition).

---

**No new criteria added.** R14 produced no structural observations beyond what C-32 and C-33 already encode. Ceiling is 100.00. Rubric is stable.

**Watch list additions (four new rows):**
- `deviation-only-routing-scope-declaration` (C-32): R14 confirmed. One header line (`ROUTING: deviation from BODY ORDER only.`) is the fix. Penalty = 0.19.
- `body-order-layer-token` (C-33): R14 confirmed. `BODY-ORDER-LAYER: active` or `none`. Declared-absence = PASS. Penalty = 0.19.
- `c32-c33-additivity`: C-32 and C-33 strictly additive. Compound partial = 0.38. No interaction. Confirmed V-04.
- `body-order-layer-none-pass`: `BODY-ORDER-LAYER: none` satisfies C-33 PASS. Declared absence is structural declaration; no routing inspection required. Confirmed V-05.
...
| deviation-only-routing-scope-declaration: C-32 fix is one header line; `Routing: if REG = ...` is a branch condition, not a scope declaration -- label alone carries no scope content; confirmed R14 V-02 | C-32 | note |
| body-order-layer-token: C-33 fix is one named token; prose directive `BODY ORDER: ...` is not a token declaration; `BODY-ORDER-LAYER: none` satisfies C-33 PASS -- declared absence = full structural declaration; confirmed R14 V-03, V-05 | C-33 | note |
| c32-c33-additivity: C-32 and C-33 are strictly additive; compound partial = 0.38 (2 x 0.19); no interaction effect; confirmed R14 V-04 | C-32, C-33 | note |
| body-order-layer-none-pass: `BODY-ORDER-LAYER: none` satisfies C-33 PASS; evaluator does not need to inspect routing to determine N/A; declared-absence is a structural declaration, not PARTIAL; confirmed R14 V-05 | C-33 | note |
