**v19 rubric written.** Here's a summary of what was extracted and added:
...
**C-48 — CONVICTION GAP DIAGNOSIS Dual In-Table Sentinel (N+Blank STRUCTURAL FAIL Rule)** (+5):
- Signal source: All R18 ceiling variants (V-02, V-03) carry a *single* RULE row in CONVICTION GAP DIAGNOSIS (C-46 FULL) — none carry a second row
- TRACEABILITY AUDIT already has dual rows under C-45 (named-absence + N+blank STRUCTURAL FAIL)
- C-48 applies that same dual-row architecture to CONVICTION GAP DIAGNOSIS
- Passes only when a second in-table RULE sentinel declares: a CGD artifact row with all substantive cells blank/dash-filled is a STRUCTURAL FAIL, not a present-but-incomplete entry

**C-49 — Complete Five-Table Dual-Sentinel Architecture** (+5):
- Strict superset of C-48
- All five structural tables (MAP, CONVICTION DELTA, SCOUT TRACEABILITY TABLE, TRACEABILITY AUDIT, CONVICTION GAP DIAGNOSIS) carry dual in-table RULE rows simultaneously
- No structural table operates on single-sentinel basis

**D19** establishes:
- CONVICTION GAP DIAGNOSIS single-sentinel ceiling confirmed (all R18 variants)
- Register invariance extends to C-48/C-49 (V-03's conversational register doesn't supply the missing second row)
- Extended 12-step chain test for C-49

**Retroactive R18 scores:** V-01 stays at 263, V-02/V-03 stay at 273 — no R18 variant earns C-48 or C-49.
ach, new ceiling 283):**
