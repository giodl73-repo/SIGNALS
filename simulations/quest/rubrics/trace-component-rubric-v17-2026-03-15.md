# Rubric: trace-component (v17)

**54 criteria across 3 tiers -- 3 new aspirational criteria added from R16 excellence signals:**
...
| Aspirational | C-09 through C-54 | 92 |

**Ceiling: 182 pts** (up from 176; 3 new criteria x 2 pts each)

---

The three R16 signals that promoted:

**C-52 . Compound Register Header Format** -- From R16 C-49 V-01 PASS. V-01's named register sections carry compound section headers: `**NULL REGISTER** -- Epistemic status of null cases` and `**DEPARTURE REGISTER** -- Properties of departure claims`. The compound form has three components: bold label (structural weighting), em-dash separator (field boundary), and a descriptor (functional characterization). C-49 requires named section labels; C-52 requires that the label adopt the compound form so the register's identity and its epistemic function are co-declared in a single parseable header line. A plain bold label ("**NULL REGISTER**") satisfies C-49 but not C-52; C-52's compound form adds a descriptor field that makes the header simultaneously name the register and characterize what reasoning occurs within it.

**C-53 . Horizontal-Rule Boundary Separators Between Named Registers** -- From R16 C-49 V-01 PASS ("with horizontal rule separators"). V-01 delimits each named register section with horizontal rules -- document-level structural separators independent of the section header label. Blank-line separation provides whitespace without document-structure demarcation; heading-level changes provide hierarchy without explicit boundary markers. A horizontal rule creates an independent structural signal at the document level: its presence at the register boundary is a distinct document element that a model must explicitly contradict to insert content across the boundary without detection. C-49 requires named register labels; C-53 requires that the boundaries between named registers be marked by structural document separators (horizontal rules) in addition to label changes, so the register boundary is a document-level event rather than a positional inference from header depth.

**C-54 . Epistemic-Function Descriptor on Named Register Headers** -- From R16 C-49 V-01 PASS ("Epistemic status of null cases", "Properties of departure claims"). V-01's compound register headers carry descriptors that characterize the epistemic or cognitive function of each register -- what kind of reasoning occurs within it. "Epistemic status of null cases" names the cognitive work: determining the evidential standing of baseline conditions before any departure is asserted. "Properties of departure claims" names the cognitive work: identifying the logical properties that departure-state assertions must satisfy. C-52 requires a descriptor field in the compound header; C-54 requires that the descriptor's content characterize epistemic function rather than naming the register's topic, its questions, or its structural position. A descriptor naming content ("questions about null cases") or sequence ("null-hypothesis questions before departure conditions") does not pass C-54; only a descriptor characterizing what kind of reasoning or evidential work the register performs passes. The epistemic-function descriptor makes the register's cognitive purpose inspectable from the header alone: a reviewer scanning headers can verify that each register's epistemic role is declared without reading its questions.

**R16 triple structure:**

- **C-52** (compound header format, per-register-label): label identity and epistemic-function descriptor co-declared in a single parseable compound header line -- bold label + em-dash + descriptor.
- **C-53** (horizontal-rule boundary, per-register-boundary): each named register section boundary is a document-level structural event (horizontal rule), not only a heading-level change or blank-line gap.
- **C-54** (epistemic-function descriptor content, per-register-header): the descriptor in C-52's compound header must characterize what cognitive/epistemic work the register performs, not what it contains or where it sits in sequence.

C-52 can be satisfied without C-54 (compound header form present but descriptor names topic rather than epistemic function). C-54 requires C-52 as a structural precondition (epistemic-function descriptor requires a compound header to carry it). C-53 is independent of both C-52 and C-54 (horizontal rule separators exist regardless of header format). Full expression requires all three: C-52 declares identity and function in a compound header, C-53 makes the boundary a document-level structural event, C-54 ensures the function descriptor characterizes cognitive work rather than content or sequence. V-01 carries evidence of all three in the R16 C-49 PASS; earlier variations satisfy C-49 (named sections) but carry none of C-52/C-53/C-54.
...
## Aspirational Criteria (weight: 92 points total, 2 pts each)
...
| Criterion | Source pattern | What v16 covered | What v17 adds |
|-----------|---------------|-----------------|---------------|
| C-52 | R16: C-49 V-01 PASS evidence -- register headers appear as `**NULL REGISTER** -- Epistemic status of null cases` (bold label + em-dash + descriptor). V-01 and V-04 variations in R15 carried plain named sections. C-49 requires a named section label as a distinct document element. | C-49 requires named register sections with labeled section headers. Neither C-49 nor C-50 requires the header to adopt a compound form carrying both a bold label and a descriptor field. | Requires the compound header form: bold register label + em-dash + descriptor. The compound form makes the register's identity and its epistemic function co-declared in a single parseable line; a plain label provides only identity. |
| C-53 | R16: C-49 V-01 PASS evidence -- "with horizontal rule separators"; named register sections delimited by `---` horizontal rules, not blank lines or heading-level changes. C-49 V-05 R15 PASS evidence for comparison: named sections present but horizontal rule delimiters not specifically required or evidenced. | C-49 requires named register labels as distinct document elements. C-50 requires per-register close markers within the causal phase. Neither criterion specifies horizontal rules as the boundary separator mechanism. | Requires horizontal-rule document separators at named register boundaries, making each register boundary a document-level structural event -- an independent structural element that must be explicitly contradicted to insert content across the boundary undetected. |
| C-54 | R16: C-49 V-01 PASS evidence -- descriptor text "Epistemic status of null cases" and "Properties of departure claims" characterize the cognitive/epistemic function of each register, not its topic or position. C-52 requires a descriptor field; the content of the descriptor is unspecified by C-52. | C-52 requires the compound header form with a descriptor field present. C-46 requires causal questions to be framed at the epistemic register; C-54 extends the epistemic-register requirement to the register headers themselves. | Requires the descriptor content to characterize what epistemic or cognitive work occurs in the register, not what the register contains or where it sits. The epistemic-function descriptor makes the register's cognitive purpose inspectable from the header, independent of reading the questions inside it. |

---

**R16 triple structure:**

- **C-52** (compound header format, per-register-label): C-49 names the register; C-52 adds the compound form (bold + em-dash + descriptor) so identity and function are co-declared in a single parseable header line.
- **C-53** (horizontal-rule boundary, per-register-boundary): C-49 makes register labels distinct document elements; C-53 makes the boundary between registers a document-level structural event by requiring horizontal-rule separators, independent of the label change.
- **C-54** (epistemic-function descriptor content, per-register-header): C-52 requires a descriptor field in the compound header; C-54 requires that field to characterize the epistemic or cognitive function of the register -- what reasoning occurs there -- not its topic, contents, or structural position.

C-52 can be satisfied without C-54 (compound form present; descriptor characterizes topic rather than epistemic function). C-54 requires C-52 as a structural precondition (epistemic-function descriptor requires a compound header to carry it). C-53 is independent of both C-52 and C-54 (horizontal rule separators can exist regardless of header format; named sections with horizontal rules but plain labels satisfy C-53 but not C-52). Full expression requires all three: C-52 declares identity and function in a single compound line, C-53 elevates the register boundary to a document-level structural event, C-54 ensures the function descriptor characterizes cognitive work rather than naming topics. V-01 carries evidence of all three in the R16 C-49 PASS; earlier variations satisfy C-49 and C-50 but carry none of C-52/C-53/C-54.
