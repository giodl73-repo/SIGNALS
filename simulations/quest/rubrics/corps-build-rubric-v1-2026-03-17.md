...
skills (org-review, org-committee) reference .craft/roles/ paths that do not exist. Not
partial generation with most roles present, but complete generation where every declared
role has a corresponding file.
**Pass condition**:
- LOCATION: .craft/roles/ directory tree
...
**Pass condition**:
- LOCATION: .craft/roles/{area}/ subdirectories
...
**Pass condition**:
- LOCATION: .craft/roles/ directory structure
...
**Pass condition**:
- LOCATION: each .craft/roles/{area}/{role}.md
...
**Pass condition**:
- LOCATION: scope or role-type field in each .craft/roles/ file
...
**Pass condition**:
- LOCATION: org-chart.md headcount Total row AND .craft/roles/ file count
- OBSERVABLE: numeric equality between chart total and file count
- STANDARD: Total in org-chart.md = count of .craft/roles/ files (excluding index/README);
  no role referenced in the chart is absent from .craft/roles/
