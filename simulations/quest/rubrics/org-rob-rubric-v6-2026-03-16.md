Written to `simulations/quest/rubrics/org-rob-rubric-v6-2026-03-16.md`.

---

**One new criterion extracted from R6:**

**C-20 -- Named Violation Taxonomy** (format, aspirational)

The structural pattern unique to V-03 R6: every rule expressed as both a positive requirement and a named `VIOLATION-NN` anti-pattern. This makes compliance mechanistic — "was VIOLATION-08 committed?" is independently checkable without prose interpretation. The VIOLATION-08 entry is also what drives C-10 from `+` to `++` in V-03 (prohibiting single-stage-only theme elevation, vs. V-02 R6's manual detection).

**Scoring changes:**

| | v5 | v6 |
|---|---|---|
| Aspirational criteria | C-09--C-19 (11) | C-09--C-20 (12) |
| Aspirational max | 22 | 24 |
| Total max | 112 | **114** |

**R6 scores under v6:**

| Variation | v5 | v6 | Delta |
|-----------|----|----|-------|
| V-01 | 108 | 108 | 0 (C-20=o) |
| V-02 | 107 | 107 | 0 (C-20=o) |
| V-03 | 112 | **114** | +2 (C-20=++) |

C-20 is structurally independent of C-18 and C-19: V-01 and V-02 R6 each achieve both C-18 and C-19 without naming violations. Same divergence pattern as R5.
?"
without prose interpretation. A schema that states requirements positively but has no
enumerated violation taxonomy does not satisfy C-20 regardless of how many structural
rules it contains. V-03 R6 is the first variation to achieve 112 under v5 and earns C-20
++ by naming VIOLATION-01 through VIOLATION-08+; this pushes its v6 score to 114.
...
| Variation | R6 score (v5) | R6 score (v6) | Delta |
|-----------|--------------|--------------|-------|
| V-01 | 108 | **108** | 0 (C-20=o: positive-requirement schema, no violation IDs) |
| V-02 | 107 | **107** | 0 (C-20=o: positive-requirement schema, no violation IDs) |
| V-03 | 112 | **114** | +2 (C-20=++) |

C-20 is structurally independent of C-18 and C-19: V-01 and V-02 R6 each achieve both
C-18 and C-19 without naming violations; V-03 achieves C-20 ++ by assigning VIOLATION-NN
identifiers to every rule. Same divergence pattern as V-01/V-02 on C-18/C-19 in R5.
...
- **Source**: V-01 R5 (Phase Gate Contracts with Finding-ID Certification) --
  `orientation.frame` extracted from .craft/roles/ and cited in ROLE: line of each
...
- **Source**: V-03 R6 (Prohibition-Mode Enforcement) -- all structural rules
  ...
  the schema self-policing. VIOLATION-01 requires Frame: in every ROLE: block
  (drives C-18 ++); VIOLATION-02/VIOLATION-03 require Via: as second column with
  no blank values (drives C-19 ++); VIOLATION-06 prohibits go/no-go buried in prose
  (drives C-05 ++); VIOLATION-08 prohibits cross-cutting themes named only in a
  single stage (drives C-10 ++ -- mechanism that distinguishes V-03 R6 at ++ from
  V-02 R6 at + on C-10). No R1--R5 variation enumerated violations by name and
  number; all prior schemas stated requirements positively. C-20 is structurally
  independent of C-19 (column-position lens enforcement) and C-15 (column-invariant
  verdict): V-01 and V-02 R6 each achieve both C-18 and C-19 without naming
  violations; V-03 R6 achieves C-20 ++ alongside all 11 prior aspirational
  criteria, reaching 114.
...
| Aspirational | C-09 to C-20 | 24 | 2 | 2 | 1 | 0 |
...
**Baseline golden** -- all essential pass, 2/3 recommended, 0/12 aspirational:

```
composite = (5/5 * 60) + (2/3 * 30) + (0/12 * 24)
          = 60 + 20 + 0
          = 80
```
...
C-10/C-14 at ++, C-09/C-12/C-13 at +, C-11/C-15/C-16/C-17/C-18/C-19/C-20 at o:

```
aspirational = 1 + 2 + 0 + 1 + 1 + 2 + 0 + 0 + 0 + 0 + 0 + 0 = 7
...
C-09/C-10/C-12/C-13/C-15/C-16 at ++, C-11/C-14/C-17/C-18/C-19/C-20 at o:

```
aspirational = 2 + 2 + 0 + 2 + 2 + 0 + 2 + 2 + 0 + 0 + 0 + 0 = 12
...
schema (C-19=o), no violation taxonomy (C-20=o).

---

**V-03 R3** -- all essential ++, all recommended ++,
C-09/C-10/C-12/C-13/C-16/C-17 at ++, C-11/C-14/C-15/C-18/C-19/C-20 at o:

```
aspirational = 2 + 2 + 0 + 2 + 2 + 0 + 0 + 2 + 2 + 0 + 0 + 0 = 12
composite    = 60 + 30 + 12 = 102
```
...
C-14/C-15/C-17/C-19/C-20 at o:

```
aspirational = 2 + 1 + 2 + 2 + 2 + 0 + 0 + 2 + 0 + 2 + 0 + 0 = 13
...
taxonomy (C-20=o). C-18 ++ is the R5 addition.

---

**V-02 R5** -- all essential ++, all recommended ++,
C-09/C-10/C-12/C-13/C-14/C-15/C-16/C-19 at ++, C-11/C-17/C-18/C-20 at o:

```
aspirational = 2 + 2 + 0 + 2 + 2 + 2 + 2 + 2 + 0 + 0 + 2 + 0 = 18
...
```
aspirational = 2 + 2 + 0 + 2 + 2 + 2 + 2 + 2 + 0 + 2 + 2 + 0 = 18
composite    = 60 + 30 + 18 = 108
```
...
stage ledger (C-17=o), no violation taxonomy (C-20=o).
...
```
aspirational = 2 + 1 + 2 + 2 + 2 + 2 + 0 + 2 + 0 + 2 + 2 + 0 = 17
...
table (C-15=o), no stage ledger (C-17=o), no violation taxonomy (C-20=o).

---

**V-03 R6** -- all essential ++, all recommended ++,
all aspirational C-09--C-20 at ++:

```
aspirational = 2 + 2 + 2 + 2 + 2 + 2 + 2 + 2 + 2 + 2 + 2 + 2 = 24
...
**Perfect score** -- all essential ++, 3/3 recommended ++, 12/12 aspirational ++:
