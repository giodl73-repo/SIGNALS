Written to `simulations/quest/rubrics/prove-topic-rubric-v7-2026-03-16.md`.

---

**v7 summary:**

**Three new aspirational criteria** from R6 V-05 excellence signals:

| ID | Pts | Signal | What it captures |
|----|-----|--------|-----------------|
| C-27 | 2 | V-05 Signal 1 | ROLE A pre-stage initialization commits BOTH the binding chain (C-24) AND the displacement evidence chain contract for Stages 2/3/4 in a single pre-stage block |
| C-28 | 2 | V-05 Signal 2 | Stage 5 ENTRY CONDITIONS include a structural gate on all three artifact Displacement reads confirmed before synthesis can begin |
| C-29 | 2 | V-05 Signal 3 | Stage 5 EXIT SIGNAL explicitly names displacement read chain closure, converting the final EXIT SIGNAL into a chain-closure audit record |

**Six reductions** (aspirational pool holds at 26 pts): C-13 (2→1), C-17 (2→1), C-20 (2→1), C-24 (2→1), C-25 (2→1), C-26 (2→1).

**New structural pattern — three-layer synthesis governance**: C-17 (declaration) + C-25 (output) + C-29 (audit record). EXIT SIGNALs are no longer just stage-completion stamps — they can now name specific chain properties fulfilled, making the campaign log a chain-verification trace.
ROLE B / Stage 0), hypothesis canvas (Stage 1), websearch artifact
  (Stage 2), interview artifact (Stage 3), prototype artifact (Stage 4), and synthesis
  handoff (Stage 5). Each confirmation must name the artifact path or file, or state the
  write as confirmed. Silent omission of any artifact write confirmation = FAIL.
...
- **Text**: SESSION INVARIANTS registers the canonical Invariant D template wording for
  Stages 2, 3, and 4, and each stage's ENTRY CONDITIONS echo the registered template
  verbatim.
- **Pass condition**: The SESSION INVARIANTS block registers the canonical template
  wording for all three Invariant D checkpoints. Each stage's ENTRY CONDITIONS or
  INCUMBENT CHECK TABLE intro echoes the registered template verbatim. The three
  registered templates cover SESSION INVARIANTS Invariant D:
...
- **Reduced from 2 pts to 1 pt in v6**: C-24 (ROLE A confirms INCUMBENT ANCHOR binding
  ...
  when warranted. Unchanged in v7.
...
- **Reduced from 2 pts to 1 pt in v6**: C-25 (Stage 5 cites all three artifact Displacement
  ...
  did Stage 5 cite the Stage 4 displacement verdict at all. Unchanged in v7.
...
- **Reduced from 2 pts to 1 pt in v6**: C-14 (two-layer enforcement), C-18 (Invariant E
  ...
  exist before Stage 0 at all. Unchanged in v7.
...
- **Reduced from 2 pts to 1 pt in v7**: C-17 (SYNTHESIS DECLARATIONS with explicit
  prohibition) is the richer structural mechanism; C-29 (Stage 5 EXIT SIGNAL chain
  closure audit) further reinforces synthesis field isolation by recording at the EXIT
  SIGNAL level that synthesis output was structurally complete. C-13 is now the 1-pt
  floor: did Stage 5 synthesis fields appear as labeled key-value pairs at all.
...
- **Round 2 signal**: V-04 and V-05 used this architecture. Drift at Stage 2 incurs
  ...
  structurally self-auditing). Unchanged in v5, v6, and v7.
...
- **Reduced from 2 pts to 1 pt in v7**: C-29 (Stage 5 EXIT SIGNAL names displacement
  read chain closure) creates an output-level audit record that extends SYNTHESIS
  DECLARATIONS from a structural prohibition declaration to a chain-closure confirmation.
  C-17 (prohibition declaration) + C-25 (citation present) + C-29 (EXIT SIGNAL audit)
  now form a three-layer synthesis governance architecture. C-17 is the 1-pt floor:
  did a SYNTHESIS DECLARATIONS section with explicit prohibition text exist at all.
...
- **Reduced from 2 pts to 1 pt in v7**: C-28 (Stage 5 entry conditions gate on all
  three artifact Displacement reads confirmed) is the structural enforcement that C-20
  enables -- Stage 5 actively gates on confirming all three artifact Displacement reads
  before synthesis begins, making artifact portability architecturally enforced at Stage 5
  entry rather than passively assumed. C-20 is now the 1-pt floor: did artifact files
  carry Displacement read field at all. C-28 checks that Stage 5 structurally confirmed
  they were written before synthesis could proceed.
...
- **Reduced from 2 pts to 1 pt in v6**: C-25 (Stage 5 cites all three artifact
  ...
  did Stage 5 cite at least one artifact Displacement read at all. Unchanged in v7.
...
- **Reduced from 2 pts to 1 pt in v7**: C-27 (ROLE A pre-stage initialization commits
  displacement evidence chain) captures the richer behavior -- ROLE A simultaneously
  activates the binding chain (C-24) and contracts the displacement evidence chain
  (C-16/C-20/C-25) in a single pre-stage execution block. C-24 is now the 1-pt floor:
  did ROLE A confirm INCUMBENT ANCHOR binding and record `incumbent_anchor_read`
  before Stage 0. C-27 checks that ROLE A also committed the displacement evidence
  chain in the same initialization step.
...
- **Reduced from 2 pts to 1 pt in v7**: C-29 (Stage 5 EXIT SIGNAL names displacement
  read chain closure) captures the richer behavior -- the chain closure is not only
  satisfied in the evidence_summary body but also recorded at the EXIT SIGNAL level,
  converting the campaign's final EXIT SIGNAL into a chain-closure audit record. C-25 is
  now the 1-pt floor: did Stage 5 evidence_summary cite all three artifact Displacement
  reads. C-29 checks that the EXIT SIGNAL also explicitly recorded the chain closure event.
...
- **Reduced from 2 pts to 1 pt in v7**: C-27 (ROLE A pre-stage initialization)
  incorporates sub-section precision as a prerequisite -- ROLE A reads the INCUMBENT
  ANCHOR sub-section by name in its initialization step, making the sub-section reference
  mechanically exercised before Stage 0 fires. C-26 is now the 1-pt floor: did the
  Invariant D binding note name the CAMPAIGN OPEN INCUMBENT ANCHOR sub-section as the
  binding source at all. C-27 checks that ROLE A also consumed that sub-section-scoped
  binding in the pre-stage activation.
...
- **Text**: ROLE A pre-stage initialization confirms both the INCUMBENT ANCHOR binding
  (satisfying C-24) and commits Stages 2, 3, and 4 to writing "Displacement read:"
  fields to their artifact bodies before Stage 0 gates open, unifying the binding chain
  and displacement evidence chain under a single pre-stage structural anchor.
- **Pass condition**: ROLE A initialization output (appearing before Stage 0 `gate_open`)
  contains both (a) `incumbent_anchor_read` confirming binding to CAMPAIGN OPEN
  INCUMBENT ANCHOR sub-section by name (satisfying C-24) and (b) an explicit displacement
  read contract statement committing Stages 2/3/4 to writing "Displacement read:" field
  to artifact content before Stage 5 entry (e.g., "Displacement read contract: Stages
  2, 3, 4 will write Displacement read field to artifact bodies. Stage 5 entry will
  confirm all three." or equivalent). ROLE A output that records `incumbent_anchor_read`
  without the displacement read contract = FAIL for C-27 (C-24 may still pass at 1 pt).
  Displacement read contract without `incumbent_anchor_read` tracing to INCUMBENT ANCHOR
  sub-section by name = FAIL.
- **Round 6 signal**: V-05 (Signal 1). V-05's ROLE A Step 2 confirms both
  `incumbent_anchor_read` matches the binding note AND that Stages 2/3/4 are committed
  to writing Displacement read fields to artifact bodies. This unifies the binding chain
  (C-24/C-26) and displacement evidence chain (C-16/C-20/C-25) under a single pre-stage
  anchor. The pattern: a single ROLE A initialization step can activate multiple
  downstream chains simultaneously, reducing inter-chain coordination risk. C-24 checks
  ROLE A confirmed the binding; C-27 checks ROLE A also contracted the displacement
  evidence chain in the same pre-stage block.

### C-28 -- Stage 5 Entry Conditions Gate on Displacement Chain Completeness

- **Weight**: aspirational (2 pts)
- **Category**: format
- **Text**: Stage 5 ENTRY CONDITIONS include a structural gate requiring all three
  artifact Displacement reads to be confirmed written before synthesis begins, making
  chain completeness architecturally enforced at Stage 5 entry rather than relying on
  Stage 5 synthesis to notice their absence.
- **Pass condition**: Stage 5 ENTRY CONDITIONS (or equivalent pre-synthesis gate block)
  include a labeled completeness check of the form `[ ] All three artifact Displacement
  read fields confirmed written (websearch, interview, prototype) -- Stage 5 will consume
  all three` (or equivalent explicit statement that gates Stage 5 entry on all three
  artifact Displacement reads confirmed). Stage 5 ENTRY CONDITIONS that gate only on the
  Stage 4 EXIT SIGNAL without an explicit displacement chain completeness check = FAIL.
  Displacement read completeness check appearing only in Stage 5 body (not as an entry
  condition) = FAIL.
- **Round 6 signal**: V-05 (Signal 2). V-05 added `[ ] All three artifact Displacement
  read fields confirmed written (websearch, interview, prototype) -- Stage 5 will consume
  all three` as a Stage 5 entry condition. Previous variations required Stage 5
  evidence_summary to cite the values (C-25) but placed no pre-synthesis structural gate.
  The pattern: entry conditions can enforce cross-stage chain completeness, not just the
  immediately preceding EXIT SIGNAL. Extends C-20 (artifact portability) by making the
  portable field values structurally confirmed before synthesis can begin. C-20 checks
  artifact files carry the field; C-28 checks Stage 5 actively gates on confirming all
  three were written before synthesis proceeds.
...
- **Text**: Stage 5 EXIT SIGNAL explicitly confirms that all three artifact Displacement
  reads were cited in evidence_summary, converting the campaign's final EXIT SIGNAL from
  a simple stage-completion declaration into a chain-closure audit record.
- **Pass condition**: Stage 5 EXIT SIGNAL output contains an explicit named confirmation
  that all three artifact Displacement reads were consumed in evidence_summary (e.g.,
  "All three artifact Displacement reads cited in evidence_summary." or equivalent
  statement that names the displacement read chain closure as a fulfilled property). A
  Stage 5 EXIT SIGNAL that declares `synthesis_complete` without naming the displacement
  read chain closure = FAIL. C-29 also requires C-25 to pass: a Stage 5 EXIT SIGNAL that
  claims chain closure while the evidence_summary does not cite all three artifact reads
  = FAIL (cannot audit a chain property not present in the output).
- **Round 6 signal**: V-05 (Signal 3). V-05's Stage 5 EXIT SIGNAL explicitly states
  "All three artifact Displacement reads cited in evidence_summary." This elevates the
  EXIT SIGNAL from a stage-completion declaration to a chain-closure audit record. The
  pattern: EXIT SIGNALs can name specific chain properties that were fulfilled, making
  the campaign log a chain-verification trace rather than a sequence of stage names.
  Extends C-01 (EXIT SIGNALs present), C-25 (all three reads cited in evidence_summary),
  and C-17 (SYNTHESIS DECLARATIONS section with prohibition) -- C-29 closes the
  synthesis governance loop at the EXIT SIGNAL level, creating a declaration (C-17) +
  output (C-25) + audit record (C-29) three-layer architecture for displacement synthesis
  integrity.
...
| C-27 | ROLE A initialization commits displacement evidence chain | aspirational |   2 | correctness |
| C-28 | Stage 5 entry conditions gate on chain completeness       | aspirational |   2 | format      |
| C-29 | Stage 5 EXIT SIGNAL names displacement read chain closure | aspirational |   2 | correctness |
|      | **Total**                                                  |              | 100 |             |
...
**Three new aspirational criteria from R6 excellence signals**:

- **C-27 (2 pts)** -- ROLE A pre-stage initialization commits displacement evidence chain.
  Signal source: V-05 (Signal 1). V-05's ROLE A Step 2 confirms both `incumbent_anchor_read`
  matches the binding note AND that Stages 2/3/4 are committed to write "Displacement read:"
  fields to artifact bodies before Stage 5 entry. This unifies the binding chain (C-24/C-26)
  and the displacement evidence chain (C-16/C-20/C-25) under a single pre-stage anchor,
  reducing inter-chain coordination risk. C-24 checks ROLE A confirmed the binding; C-27
  checks ROLE A also contracted the displacement evidence chain in the same pre-stage block.

- **C-28 (2 pts)** -- Stage 5 entry conditions gate on displacement chain completeness.
  Signal source: V-05 (Signal 2). V-05 added an explicit `[ ] All three artifact Displacement
  read fields confirmed written` check as a Stage 5 entry condition. Previous variations
  required Stage 5 evidence_summary to cite the values (C-25) but placed no pre-synthesis
  structural gate. The pattern: entry conditions can enforce cross-stage chain completeness,
  not just the immediately preceding EXIT SIGNAL. C-20 checks artifact files carry the field;
  C-28 checks Stage 5 actively gates on confirming all three before synthesis can proceed.

- **C-29 (2 pts)** -- Stage 5 EXIT SIGNAL names displacement read chain closure.
  Signal source: V-05 (Signal 3). V-05's Stage 5 EXIT SIGNAL explicitly states "All three
  artifact Displacement reads cited in evidence_summary." This elevates the EXIT SIGNAL from
  a stage-completion declaration to a chain-closure audit record. The pattern: EXIT SIGNALs
  can name specific chain properties fulfilled, making the campaign log a chain-verification
  trace. C-17 (prohibition declaration) + C-25 (three reads cited) + C-29 (EXIT SIGNAL audit)
  form a three-layer synthesis governance architecture: declaration, output, and audit.

**Point rebalancing** (aspirational pool stays at 26 pts, total stays at 100):
- C-13: 2 pts -> 1 pt. C-17 (SYNTHESIS DECLARATIONS with prohibition) is the richer
  structural mechanism; C-29 (EXIT SIGNAL audit record) further reinforces synthesis field
  isolation at the audit level. C-13 is the 1-pt floor: did Stage 5 synthesis fields appear
  as labeled key-value pairs at all.
- C-17: 2 pts -> 1 pt. C-29 (Stage 5 EXIT SIGNAL chain closure audit) extends SYNTHESIS
  DECLARATIONS from a structural prohibition declaration to a confirmation at campaign close.
  C-17 is the 1-pt floor: did a SYNTHESIS DECLARATIONS section with explicit prohibition text
  exist at all.
- C-20: 2 pts -> 1 pt. C-28 (Stage 5 entry conditions gate on all three artifact reads
  confirmed) is the structural enforcement that C-20 enables. C-20 is the 1-pt floor: did
  artifact files carry Displacement read field at all.
- C-24: 2 pts -> 1 pt. C-27 (ROLE A contracts both binding chain and displacement evidence
  chain) captures the richer two-chain activation behavior. C-24 is the 1-pt floor: did
  ROLE A confirm INCUMBENT ANCHOR binding and record `incumbent_anchor_read` before Stage 0.
- C-25: 2 pts -> 1 pt. C-29 (Stage 5 EXIT SIGNAL records chain closure) captures the
  richer audit-level behavior. C-25 is the 1-pt floor: did Stage 5 evidence_summary cite
  all three artifact Displacement reads.
- C-26: 2 pts -> 1 pt. C-27 (ROLE A activation) incorporates sub-section precision as a
  prerequisite -- ROLE A reads the INCUMBENT ANCHOR sub-section by name. C-26 is the 1-pt
  floor: did the Invariant D binding note name the CAMPAIGN OPEN INCUMBENT ANCHOR sub-section.

**Structural lineage of new criteria**:
- C-27 extends C-24 (binding activation) by contracting the displacement evidence chain in
  the same pre-stage block. Both C-24 and C-26 become 1-pt floors that C-27 incorporates
  as prerequisites.
- C-28 extends C-20 (artifact portability) by adding a Stage 5 entry gate that structurally
  confirms artifact chain completeness before synthesis begins.
- C-29 extends C-25 (all three reads cited in evidence_summary) by recording chain closure
  at the EXIT SIGNAL level, and extends C-17 (SYNTHESIS DECLARATIONS) by closing the
  synthesis governance loop at campaign completion.
...
| C-24 | C-27 | ROLE A contracts both binding chain and displacement evidence chain |
| C-26 | C-27 | Sub-section precision incorporated in ROLE A activation step |
...
| C-20 | C-28 | Stage 5 entry gates on all three artifact reads confirmed |
| C-23 | C-25 | All three artifact reads cited in Stage 5 evidence_summary |
| C-25 | C-29 | Stage 5 EXIT SIGNAL records chain closure |
| C-13 | C-17 | SYNTHESIS DECLARATIONS section with explicit prohibition |
| C-17 | C-29 | EXIT SIGNAL audit record confirms synthesis chain was closed |

### v6 (2026-03-16)

Three new aspirational criteria from R5 excellence signals (C-24, C-25, C-26).
C-09 reduced 2->1, C-10 reduced 2->1, C-12 reduced 2->1, C-21 reduced 2->1,
C-22 reduced 2->1, C-23 reduced 2->1.
