Written to `simulations/quest/rubrics/signal-rubric-v6-2026-03-16.md` (273 lines).

---

**Three new criteria extracted from R6 excellence signals:**

**C-20** (aspirational / depth) -- Self-verifying count assertion in the C-17 rule
The RULE enforcing C-17 must contain an explicit count assertion ("This list contains exactly N rules, one per aspirational criterion C-09 through C-NN. Count = N. None outside.") embedded in the RULE itself. Makes completeness O(1) machine-checkable by count rather than O(N) by enumeration.
Source: V-03 -- RULE 9 (C-17) asserts "Exactly 11 rules, one per aspirational criterion C-09 through C-19."

**C-21** (aspirational / behavior) -- Quantified pass threshold in compliance gate
The compliance gate must state both halves of the binary: the FAIL gate (C-19) AND an explicit "N/N checked = valid output" PASS threshold. A gate with only a failure branch leaves the success condition implicit.
Source: V-04 -- COMPLIANCE AUDIT includes "11/11 checked = valid output" threshold line.

**C-22** (aspirational / depth) -- Gate criterion self-represented as a RULE entry
The criterion that mandates the gate (C-19) must itself appear as RULE N (C-19) in the numbered rule list, completing the bijection through the gate criterion itself. Without it, the gate enforces N criteria but the rule list covers only N-1, breaking C-18's bijection requirement at exactly the gate criterion.
Source: V-02 -- adds RULE 11 (C-19) to restore the bijection broken by V-01's 10-rule set.

**Formula updated**: aspirational_pass / 14 x 10 (~0.71 pts each, was ~0.91)
 interchangeable across namespaces |
