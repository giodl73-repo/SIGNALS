Written to `simulations/quest/rubrics/flow-resilience-rubric-v10-2026-03-15.md`.
...
- Signal: V-01's named "Bypass Gate-Reopening Protocol (DS Expert Authority)" section declared *before Gate 1*, not reactively when a bypass is first detected.
- The gap it closes: C-29 requires the bypass chain to execute correctly; C-31 requires the chain's structure to be declared upfront — analogous to C-21 (scope declaration precedes analysis). Without it, the bypass protocol is unverifiable until first invocation.
- Pass requires all four elements: numbered remediation steps / amendment sub-gate convention / authorized actors / COMPLETE-blocking statement.

**C-32 -- Bypass-Trigger Column Scanability**
- Signal: V-02/V-05's BYPASS-TRIGGER column in the registry audit table — each cell must be a trigger citation or explicit N/A/SCENARIO-ABSENT, never blank.
- The gap it closes: C-28 requires invocation completeness in prose; C-32 requires bypass detection to be *column-scannable* independent of prose traversal — orthogonal audit path on the same data. Blank cells are invalid because they conflate "trigger never arose" with "trigger unrecorded."
- Incentivizes table-structured registry audits; analyses with prose-only audits score FAIL.

**Scoring delta**: aspirational cap unchanged at 10; uncapped max rises from 44 → 48 (24 criteria × 2).
 -- how the
...
Uncapped maximum across 24 criteria is 48; the cap rewards breadth while holding the
...
- **Text**: The bypass remediation chain required by C-29 is declared as a named pre-analysis
  ...
  detected. The pre-analysis section enumerates: (1) the numbered steps of the bypass
  remediation chain (reopen gate, apply bypassed rule, re-close under amendment sub-gate,
  update audit entry to INVOKED); (2) the amendment sub-gate naming convention to be used
  (e.g., GATE N-bypass: AMENDED); (3) the actors authorized to execute each remediation step;
  and (4) the explicit blocking statement that COMPLETE may not be declared while any
  RULE-BYPASSED entry remains unresolved. C-29 requires the bypass chain to execute correctly
  when triggered; C-31 requires the chain's structure to be visible and independently
  auditable from the outset, before any bypass condition arises. Without a pre-declared
  protocol, a reader encountering the first RULE-BYPASSED entry must infer the remediation
  procedure from gate prose -- the protocol's correctness cannot be verified before its first
  invocation. A pre-declared bypass chain makes the remediation procedure checkable at the
  start of the analysis, not only at the point of first use. (Signal: V-01 Round 9 C-29
  PASS evidence -- named pre-analysis "Bypass Gate-Reopening Protocol (DS Expert Authority)"
  section with numbered remediation steps, amendment sub-gate convention, bypass chain
  structure, and explicit COMPLETE-blocking statement, declared before Gate 1.)
- **Pass condition**: A named pre-analysis bypass chain section (or equivalent named block)
  is present before the first analysis gate. The section enumerates the numbered remediation
  steps, names the amendment sub-gate convention, identifies authorized actors, and states
  the COMPLETE-blocking condition explicitly. The section must be pre-analysis (appearing
  before Gate 1), not embedded in a terminal block or post-analysis audit.
- **Partial**: Pre-analysis bypass section present but missing one of the four required
  ...
  statement); or bypass chain described in the analysis preamble in prose without a named
  section block; or section appears mid-analysis rather than before Gate 1.
- **Fail**: No pre-analysis bypass chain section; bypass remediation protocol is either
  absent (which also fails C-29) or appears only reactively at the point of first
  RULE-BYPASSED detection; the reader cannot verify the protocol's structure before
  encountering its first invocation.
...
- **Text**: When the post-analysis registry audit is structured as a table, each registered
  ...
  is either (1) non-empty, naming the gate and entry where the rule's trigger condition arose
  and the rule was not applied; or (2) explicitly marked N/A or SCENARIO-ABSENT, confirming
  that the rule's trigger condition never arose during the run. A blank cell is not a valid
  entry in either case: blank does not distinguish "trigger condition never arose" from
  "trigger condition arose but was not recorded." The column's purpose is horizontal
  scanability: a reader can recover all RULE-BYPASSED entries for an analysis run by scanning
  a single column, without reading gate prose. This is orthogonal to C-28's requirement that
  invocations be cited by gate and entry -- C-28 requires completeness of audit prose; C-32
  requires that bypass detection be column-scannable independent of prose traversal.
  (Signal: V-02 Round 9 C-29 PASS evidence -- "BYPASS-TRIGGER column forces non-empty cell
  beside every RULE-BYPASSED row, making bypass detection scannable by column audit"; V-05
  Round 9 -- BYPASS-TRIGGER column + named Bypass Owner column as the mechanism for
  actor-accountable, column-scannable bypass detection.)
- **Pass condition**: A BYPASS-TRIGGER column is present in the rule registry audit table.
  Every registered rule's cell in this column is either non-empty (citing the gate and entry
  that triggered bypass detection) or explicitly marked N/A / SCENARIO-ABSENT (confirming no
  trigger arose). A reader can scan the column and recover all RULE-BYPASSED entries without
  reading gate prose. Blank cells fail this criterion regardless of correctness in other
  columns.
- **Partial**: BYPASS-TRIGGER column present but some cells are blank rather than explicitly
  ...
  gate and entry that triggered bypass detection; or bypass detection information is presented
  in a separate prose block rather than a dedicated table column.
...
| Aspirational | 10% | C-09 through C-32 | PASS=2, PARTIAL=1, FAIL=0; capped at 10 |
...
- **C-31 (new)** -- Pre-analysis bypass chain declaration: extracted from V-01 Round 9 C-29
  PASS evidence (named pre-analysis "Bypass Gate-Reopening Protocol (DS Expert Authority)"
  section with numbered steps and COMPLETE-blocking statement, declared before Gate 1). C-29
  requires the bypass remediation chain to execute correctly when triggered; C-31 requires
  that chain structure to be declared before execution begins -- before Gate 1 -- analogous
  to C-21's pre-analysis commerce scope declaration. Without a pre-declared protocol, the
  bypass remediation procedure emerges reactively only at the point of first RULE-BYPASSED
  detection: the reader cannot verify the protocol's structure before its first invocation,
  and the analysis's approach to bypass handling is not independently auditable at the outset.
  C-31 closes this gap by making the bypass chain a pre-analysis commitment, not a reactive
  discovery. The four required elements (numbered steps / amendment convention / authorized
  actors / COMPLETE-blocking statement) mirror the four-field completeness pattern used in
  C-02 and C-20: each element is independently checkable and any missing element downgrades
  to PARTIAL.
- **C-32 (new)** -- Bypass-trigger column scanability: extracted from V-02 and V-05 Round 9
  C-29 PASS evidence ("BYPASS-TRIGGER column forces non-empty cell beside every RULE-BYPASSED
  row, making bypass detection scannable by column audit"; V-05 additionally introduced a
  named Bypass Owner column). When the registry audit is table-structured, a dedicated
  BYPASS-TRIGGER column enables horizontal bypass detection: a reader recovers all
  RULE-BYPASSED entries by scanning one column, without traversing gate prose. Each cell must
  be explicitly non-empty (trigger citation) or N/A / SCENARIO-ABSENT (confirmed no trigger);
  blank cells are invalid because they do not distinguish "trigger never arose" from "trigger
  arose, not recorded." C-28 requires invocation completeness in audit prose; C-32 requires
  bypass detection to be column-scannable independent of prose traversal -- orthogonal
  auditability paths on the same audit data. Analyses where the registry audit is not
  table-structured cannot satisfy this criterion and score FAIL; the incentive is to prefer
  table structure in registry audits.
