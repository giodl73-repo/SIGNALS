Written to `simulations/quest/rubrics/topic-roadmap-rubric-v13-2026-03-17.md`.
...
|--------|--------|
| Version bump | 12 -> 13 |
| Formula denominator | `/40 * 10` -> `/44 * 10` (22 aspirational × 2 = 44 max) |
| Aspirational range | C-09--C-28 -> C-09--C-30 |
| **C-29 added** | Three-way consequence field name identity: defeat assessment column, proposal column, and exit criterion field name all use the same string |
| **C-30 added** | Proposal table bias column present at row level: "Bias blocked by guard" column in proposal table, independently testable from phase-level annotations |

---

**C-29 source:** R12 V-02 isolates the three-site naming problem. C-27 tests defeat-assessment-column↔exit-criterion identity (2 of 3 sites). V-02 passes C-28 full but scores C-27 partial and C-08 partial from the same root — the defeat assessment column is "Consequence if HOLDS" while the proposal column and exit criterion are "Consequence if unchanged". C-29 extends the identity requirement to all three sites simultaneously.

**C-30 source:** R12 V-01 and V-02 both score C-17=1 for the same reason. Per-phase bias annotations are present at all guards; the proposal table lacks a "Bias blocked by guard" column at the row level. This is the consistent remaining gap across both rounds. C-30 isolates the row-level column as an independently testable structural feature.

**Why two:** Orthogonal. A variation can fail C-29 (three names inconsistent) while passing C-30 (proposal bias column present), or vice versa. V-02 is partial on both from independent causes.

**Score ceiling at R12 base:** V-01 and V-02 both scored 38/40 (9.50). Under v13 they would score 38/44 ≈ 8.64 without targeting C-29/C-30, providing headroom for R13 to chase both.
DED / UNEXPECTED / CHALLENGED -- is present
...
## Aspirational Criteria (scored 0 / 1 / 2 per criterion; max = 44)
...
## What changed from v12 -> v13

| Change | Detail |
|--------|--------|
| Version bump | 12 -> 13 |
| Formula denominator | `/40 * 10` -> `/44 * 10` (22 aspirational x 2 = 44 max) |
| Aspirational range | C-09--C-28 -> C-09--C-30 |
| **C-29 added** | Three-way consequence field name identity: defeat assessment column, proposal column, and exit criterion field name all use the same string |
| **C-30 added** | Proposal table bias column present at row level: "Bias blocked by guard" column in proposal table, independently testable from phase-level annotations |

**C-29 in brief:** R12 V-02 isolates the three-site naming problem. C-27 tests
defeat-assessment-column↔exit-criterion identity (2 of 3 sites). V-02 passes C-28
full (both blocking sites self-contained) but scores C-27 partial and C-08 partial
from the same root: the defeat assessment column is "Consequence if HOLDS" while
the proposal column and exit criterion are "Consequence if unchanged". C-29 extends
the lexical identity requirement to all three sites simultaneously, closing the gap
where any two sites can be updated as a matched pair while the third drifts.

**C-30 in brief:** R12 V-01 and V-02 both score C-17=1 for the same reason -- per-
phase bias annotations are present at all guards, but the proposal table lacks a
"Bias blocked by guard" column at the row level. This is the consistent remaining
gap across both rounds. C-17 tests both levels required; C-30 isolates the row-
level column as a separate testable feature, allowing a variation to target full
C-30 without necessarily achieving full C-17 (phase annotations may be incomplete
while the proposal column is present, and vice versa).

**Why two, not one:** C-29 and C-30 are orthogonal. A variation can fail C-29
(three sites present, naming inconsistent) while passing C-30 (proposal table
has bias column), or fail C-30 (no proposal table bias column) while passing C-29
(all three consequence field names identical). The V-02 pattern fails C-29
(partial) and C-30 (partial, since C-17=1). A variation targeting both C-29 and
C-30 full would require: lexically identical field names at all three sites AND a
"Bias blocked by guard" column in the proposal table.

**Orthogonality matrix:**

- C-29 fail, C-30 fail: naming inconsistent across consequence sites; no proposal
  table bias column (R12 V-02 pattern for C-29; persistent C-17=1 pattern for C-30).
- C-29 pass, C-30 fail: all three consequence names identical; proposal table lacks
  bias column.
- C-29 fail, C-30 pass: proposal table has bias column; consequence names drift
  between defeat assessment, proposals, and exit criterion.
- C-29 pass, C-30 pass: three-way name identity achieved; proposal table carries
  per-row bias labels (target state for R13).
