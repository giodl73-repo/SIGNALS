35 criteria total (C-01 through C-35), 414 lines. Here is the complete updated rubric output:

---

**v12 is written to `simulations/quest/rubrics/mock-ns-rubric-v12-2026-03-16.md`.**

---

## What changed

### New criterion — C-35 (relative-clause-agent displacement)

R11 V-01 introduced a structural form not previously named: **"TOPICS.md is the first artifact this step emits."**

The step (`this step`) is grammatically active — it is the explicit subject of its own verb `emits` in the relative clause — but this activity is confined to a subordinate clause. The artifact `TOPICS.md` is the main-clause subject.

This closes a specific **evaluator trap**: seeing `this step emits` in the sentence and incorrectly crediting C-28 on the grounds that "the step is doing something." C-35 names the rule explicitly: the step must be the **main-clause** nominative subject; activity in a subordinate clause does not satisfy C-25 or C-28.

**Failure chain**: C-25, C-28, C-29, C-30, C-35 (−10 pts)
**C-33 passes** (active voice — not passive)

C-30 now has two named sub-forms:
1. Oblique-modifier: "the first output *of this step*" (canonical, R8)
2. Relative-clause-agent: "the first artifact *this step emits*" (new, R11)

### Discriminator notes added to existing criteria

| Criterion | Note |
|-----------|------|
| C-25 | Step as relative-clause agent does not constitute a role-declaration; main-clause subject required |
| C-28 | "Grammatical subject of its own emission action" means main-clause subject; subordinate-clause activity excluded |
| C-29 | Substitution test is universal — fires on artifact subjects, not only step-derived nominalizations |
| C-30 | Relative-clause-agent sub-form confirmed alongside oblique-modifier sub-form; C-35 names it |
| C-33 | C-30/C-33 non-overlap empirically confirmed from both directions: R10 V-01 (passive) + R11 V-01 (active) |
...
**V-01 (R11) under v12**: fails C-25, C-28, C-29, C-30, C-35 → **124/134**
e-clause subject; "grammatical subject of its own emission action" in C-28's pass condition means main-clause subject
- **C-29**: R11 discriminator note added -- C-29's substitution test fires on artifact-as-subject forms as well as step-derived nominalizations; the substitution test is the universal discriminator: when "It" refers to any non-step entity (artifact, abstraction, nominalization, or gerundive), C-29 fails; the criterion title's "nominalization" example is not an exhaustive list of failing subject types
- **C-30**: R11 discriminator note added -- C-30 applies to both the oblique-modifier sub-form (step appears as "of this step" in the predicate) and the relative-clause-agent sub-form (step appears as the agent of a restrictive relative clause modifying the artifact noun); both forms have the artifact in active-voice nominative subject position; C-35 names the relative-clause sub-form as the sixth declarative failure mode
- **C-33**: R11 discriminator note added -- non-overlap with C-30 empirically confirmed from the active side; R11 V-01 is the first case where an active-voice artifact-as-subject construction (C-30 fails) is tested against C-33 (C-33 passes); together with R10 V-01 (passive case, C-33 fails, C-30 passes), the non-overlap is confirmed in both directions

**Scoring:** max rises from 132 to **134** (27 aspirational × 2 = 54). V-01 (R11) fails C-25, C-28, C-29, C-30, C-35 (124/134 under v12 scoring).
...
**C-33** (passive artifact-as-subject) — closes the C-30 active-voice gap. "The TOPICS.md status line is emitted first by this step" places the artifact as grammatical subject in a passive construction; C-30's "in active voice" qualifier does not fire; C-33 names this as a distinct fifth failure mode. Four-criterion failure chain: C-25, C-28, C-29, C-33.

**C-34** (gerundive-as-subject) — names the action-nominalization displacement pattern. "Emitting first is this step's defining action" promotes the gerundive action to nominative subject; the step is relegated to predicate-possessive. This is the fifth declarative failure mode, distinct from C-29 (where a step-derived nominalization is the subject NP head). C-28 and C-29 catch it independently; C-34 makes it explicitly named. Three-criterion failure chain: C-28, C-29, C-34.

**Discriminator notes added:**
- **C-25**: order-strict confirmed — reversed ordering (V-04) fails even with a perfect identity predicate as sentence 2
- **C-30**: scoped-to-active-voice note + C-33 cross-reference
- **C-31**: enumeration-first is the primary test; "will" is not on the prohibited list so V-02's predictive-future passes without reaching the substitution test

**Scoring:** max rises from 128 to **132** (26 aspirational × 2 = 52).
 failure mode enumerated in the C-28 through C-34 chain. V-02 (128/128) confirms that predictive-future "will" passes C-31 because enumeration-first is the primary test: "will" is not in the prohibited modal list, so the deontic failure does not apply.

**Scoring:** max rises from 128 to **132** (26 aspirational x 2 = 52). V-01 fails C-25, C-28, C-29, C-33 (120/132 under v11 scoring). V-02 passes 132/132. V-03 fails C-28, C-29, C-34 (126/132 under v11 scoring). V-04 fails C-25 (130/132).

**Existing criteria updated:**
- **C-25**: R10 discriminator note added -- C-25 is order-strict; V-04 confirms that reversed ordering (imperative first, declarative second) fails C-25 even when the declarative sentence itself is a structurally perfect identity predicate
- **C-30**: R10 discriminator note added -- C-30's "in active voice" qualifier is a scope restriction, not an oversight; passive artifact-as-subject constructions fall through to C-28 and C-29; C-33 names the passive case as a distinct criterion; C-30 passing is necessary but not sufficient for C-33 passing
- **C-31**: R10 discriminator note added -- predictive-future "will" is not in the enumerated deontic list (must/shall/should/is required to) and does not trigger C-31; enumeration-first is the primary test; the substitution test is a secondary probe used only when the modal is on the list or is a borderline case
...
### v10 (2026-03-16) -- R9 patterns promoted to aspirational criteria

**Two new criteria (C-31, C-32) from R9.** Two new failure traps. Scoring max rises from 124 to **128** (24 aspirational x 2 = 48).
...
*Raise the bar once essential and recommended are stable. Each criterion is worth 2 pts (27 x 2 = 54; max score = 134).*
