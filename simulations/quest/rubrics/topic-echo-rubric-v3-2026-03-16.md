Written to `simulations/quest/rubrics/topic-echo-rubric-v3-2026-03-16.md`.
...
Derived from V-05 Excellence Signal Pattern 1. C-11 and C-12 each verify that a section exists. C-13 verifies that the two sections are genuinely independent -- made with different information, in a different temporal position. Independence fails in two directions: a PBI entry that names a handle label by its final form (the handle was predictable at PBI time), or a handle that echoes PBI entry language verbatim (the handle was constructed by inverting a prediction, not naming a finding). Applicability gate: C-11 and C-12 must both pass.

**C-14 -- Single-entry audit trail completeness** (behavior)
Derived from V-05 Excellence Signal Pattern 2. Because C-11 and C-12 are non-competing, their composition test is per-entry: each surprise entry must carry all three pointers -- Handle Ledger title, PBI identifier, source signal artifact -- so a reviewer can verify the full chain from a single entry without navigating to other sections. This is what "first-class citable artifact" means in practice. Applicability gate: C-11 and C-12 must both pass.

**Score model update:** Aspirational: 20 pts (4 criteria) -> 30 pts (6 criteria, 5 pts each). Max composite: 110 -> 120. Golden threshold unchanged.

**New relationship entries:**
- C-11 + C-12 / C-13: C-13 requires both; tests their independence at the section level
- C-13 + C-14: C-14 tests the same composition at the entry level; both are needed for a fully auditable artifact
ry language verbatim (the handle
