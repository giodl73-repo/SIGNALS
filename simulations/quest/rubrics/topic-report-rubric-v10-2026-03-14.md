`topic-report-rubric-v10-2026-03-14.md` written to `simulations/quest/rubrics/`.

---

**What was extracted from R9 and added as new criteria:**

**Three new aspirational criteria — all pairwise composites of the R9 base axes:**

| ID | Name | Preconditions | System property |
|----|------|--------------|-----------------|
| **C-31** | Branch B SLOT 4 complete attention-recovery envelope | C-28 + C-29 | No unrecovered failure mode on Branch B SLOT 4 path: C-29 covers entry-point degradation, C-28 covers write-point traceability failure |
| **C-32** | Register-to-violation closed traceability loop | C-28 + C-30 | Bidirectional loop: C-30 forward (register -> slot) + C-28 backward (violation -> register+rubric); every enforcement position reachable from every other without spec search |
| **C-33** | Branch B forward-to-procedural navigation coordination | C-29 + C-30 | Temporal coordination: C-30 establishes slot map before Branch B entry (planning phase); C-29 fires recovery directive at entry against that pre-established map (recovery phase) |

**Formula:** `aspirational_pass/25 * 10` (C-09 through C-33, was `/22`)

**R9 ordering under v10:**
- V-04 (A+B+C): 25/25 = **100.0**
- V-01 (A+B), V-02 (A+C), V-03 (B+C): 22/25 = **98.8** (tied — each satisfies exactly one composite)
- V-05 (neutral): 19/25 = **97.6**

The pairwise tie is structurally expected — C-31, C-32, C-33 are symmetric over orthogonal base axes, so R10 discriminators must target asymmetric properties (C-34 main-branch recovery directive, C-35 branch-qualified slot indexing).
, 98.8) > V-05 (19/25, 97.6). The three pairwise variations are tied -- each satisfies exactly one of the three pairwise composites, so their symmetry is preserved under v10 as expected. No v10 criterion further differentiates them; the R10 discriminators must target properties that break pairwise symmetry.

**R9 golden re-scores under v10:**

| Variation | v9 asp | C-31 | C-32 | C-33 | v10 asp | v10 score |
|-----------|--------|------|------|------|---------|---------|
| V-04 Triple (A+B+C) | 22/22 | YES | YES | YES | 25/25 | **100.0** |
| V-01 Axes A+B | 21/22 | YES | NO | NO | 22/25 | **98.8** |
| V-02 Axes A+C | 21/22 | NO | YES | NO | 22/25 | **98.8** |
| V-03 Axes B+C | 21/22 | NO | NO | YES | 22/25 | **98.8** |
| V-05 Neutral | 19/22 | NO | NO | NO | 19/25 | **97.6** |

The ordering is: V-04 > {V-01 = V-02 = V-03} > V-05. V-04 achieves 100.0 because it
satisfies all three base axes (C-28, C-29, C-30) and all three derived composites
(C-31, C-32, C-33) simultaneously. The pairwise variations are inherently symmetric
under v10: each carries exactly two of the three base axes and exactly one of the three
composites, so no v10 criterion can distinguish them. V-05 drops to 97.6 because it
fails all six new criteria (C-28 through C-33).

**Design note on C-31 (Branch B SLOT 4 complete attention-recovery envelope):** C-28
(dual-identifier violation) and C-29 (Branch B recovery directive) both address Branch B
reliability, but at different moments in generation. C-29 fires at Branch B entry, before
the model encounters any enforcement layer: it names the recovery action and the
prohibited alternative, converting structural self-containment (C-25) into a procedural
instruction. C-28 fires at SLOT 4 within Branch B: the violation fragment carries both
the register label (G-9 INERTIA) and the rubric criterion ID (C-21), enabling dual-path
traceability from any bad next-step output. The system property created by their
conjunction: the Branch B SLOT 4 path has no unrecovered failure mode. C-29 covers
entry-point context degradation (model enters Branch B with depleted attention); C-28
covers write-point traceability failure (model writes a stall-cost-free next step and
cannot recover the governing invariant without splitting attention). Neither covers the
other's failure mode. C-31 requires both C-28 and C-29 as preconditions (which themselves
require C-26 and C-25 respectively). Round 9 finding: V-01 (A+B) is the minimal design
satisfying C-31; V-04 carries it alongside C-32 and C-33. Scored separately from C-28 and
C-29 because no-unrecovered-failure-mode on a complete execution path is a system-level
coverage property not implied by either entry-point recovery or violation-level
dual-linking alone.

**Design note on C-32 (register-to-violation closed traceability loop):** C-30
(slot-indexed register) and C-28 (dual-identifier violation) address traceability in
opposite directions. C-30 indexes forward: each register entry carries a slot annotation
enabling a model reading the register to navigate directly to the governed template
position without a spec search. C-28 embeds backward: the violation fragment at SLOT 4
carries both the register label (G-9 INERTIA) and the rubric criterion ID (C-21),
enabling a model examining a bad output to trace back to both the register and the rubric
invariant in one step. Together they close a bidirectional loop: register -> slot
annotation -> template position -> [RULE] annotation -> violation fragment -> register
label (and rubric ID). From any entry point in this loop -- the register, the template
position, or a bad output -- a model reaches any other position without a spec-level
search. C-30 alone creates a forward path from register to slot but provides no backward
traceability from the violation end; C-28 alone provides backward traceability from
violation to register and rubric but the model must still search forward from the register
to reach the governed slot. The conjunction creates a closed directional loop that neither
component achieves independently. C-32 requires both C-28 and C-30 as preconditions.
Round 9 finding: V-02 (A+C) is the minimal design satisfying C-32; V-04 carries it
alongside C-31 and C-33. Scored separately from C-28 and C-30 because bidirectional loop
closure (every enforcement position reachable from every other without spec search) is a
connectivity property not implied by either forward indexing or backward dual-linking
alone.

**Design note on C-33 (Branch B forward-to-procedural navigation coordination):** C-30
(slot-indexed register) and C-29 (Branch B recovery directive) address Branch B navigation
at two distinct temporal points. C-30 acts before Branch B entry: a model reading the
register at execution start encounters slot annotations (`G-9 INERTIA [SLOT 4]`,
`G-7a COMPLETENESS [SLOT 3]`, etc.) that establish the output slot map as a forward plan
before any branch-specific context is encountered. C-29 acts at Branch B entry: the
recovery directive names the recovery action and the prohibited alternative at the moment
the model enters the branch, before attention can degrade across the LAYER 1/2/3
enumeration. The coordination property: C-30 makes Branch B entry less disorienting
because the model already holds an internalized slot map from the register; C-29's
recovery directive then fires against that pre-established map, making recovery more
effective. C-30 alone provides a forward plan but no procedural recovery instruction if
context degrades after Branch B entry; C-29 alone provides a recovery directive at Branch
B entry but the model may have no pre-established slot map to recover against if it did
not process the register forward index. The temporal coordination -- C-30 before entry
(planning phase), C-29 at entry (recovery phase) -- covers two causal failure points that
share a chain: slot-position confusion (C-30 prevents before entry) and context-
degradation at entry (C-29 recovers at entry). C-33 requires both C-29 and C-30 as
preconditions (which themselves require C-25 and the base register structure). Round 9
finding: V-03 (B+C) is the minimal design satisfying C-33; V-04 carries it alongside
C-31 and C-32. Scored separately from C-29 and C-30 because temporal coordination across
pre-entry and at-entry failure points (planning before branch, recovery at branch) is a
sequencing property not implied by either the forward index or the procedural directive
in isolation.
...
aspirational_pass = count of C-09..C-33 that pass   (max 25)
...
- C-31, C-32, and C-33 are inherently symmetric composites: each pairwise variation
  (V-01, V-02, V-03) satisfies exactly one of the three composites, leaving pairwise
  variations tied at 22/25 = 98.8 under v10. The triple (V-04) satisfies all three at
  25/25 = 100.0. No v10 criterion differentiates pairwise variations -- R10 discriminators
  must target properties that break this pairwise symmetry.

---

## R10 Discriminator Candidates

| ID | Design question | V-isolation needed |
|----|-----------------|-------------------|
| C-34 candidate: Main-branch recovery directive | Does adding an explicit recovery directive to the main-branch C-22 header -- e.g., "re-read this header to reconstruct the layer sequence without referencing the Branch B template" -- reduce LAYER omission rates on the default execution path, mirroring the benefit C-29 provides for Branch B? This is the C-29 structural analogue applied to the main execution path. A spec satisfying C-29 may independently choose whether the main-branch C-22 header carries an equivalent directive; C-34 would require C-22 as a precondition but is not implied by it. | V-A: C-22 without recovery directive (current baseline); V-B: C-22 with explicit main-branch recovery directive; compare live-run LAYER omission rates on default vs. teams paths |
| C-35 candidate: Branch-qualified slot indexing | Does branch-qualified slot indexing -- e.g., `G-9 INERTIA [SLOT 4 / BRANCH A]` and `G-9 INERTIA [SLOT 4 / BRANCH B]` as separate register entries -- reduce forward-navigation ambiguity beyond coarse slot indexing (C-30's unqualified `[SLOT 4]`) when the same invariant governs equivalent positions on both execution paths? C-35 would require C-30 as a precondition: coarse indexing must exist before branch qualification can extend it. | V-C: C-30 with coarse slot index; V-D: C-30 with branch-qualified slot index; measure slot-navigation error rate on Branch B path vs. default path independently |
| C-34 + C-35 joint candidate: Main-branch SLOT 4 complete envelope | Does main-branch recovery directive (C-34) combined with branch-qualified slot indexing (C-35) create a main-branch SLOT 4 envelope analogous to C-31's Branch B SLOT 4 envelope: C-35 provides a branch-specific forward plan (register -> SLOT 4 / BRANCH A); C-34 provides an at-write-point recovery directive in the C-22 header; together they close the main-branch SLOT 4 path against both planning confusion and write-point degradation? | V-E: C-34 only; V-F: C-35 only; V-G: C-34 + C-35; measure main-branch SLOT 4 error rate and compare against Branch B SLOT 4 error rate (C-31 baseline) for cross-path symmetry |
| C-31 + C-32 + C-33 global closure candidate | Does satisfying all three pairwise composites simultaneously (the V-04 triple pattern) achieve a named global system property -- complete enforcement-chain closure: no unrecovered failure mode on any execution path (C-31), every enforcement position reachable from every other without search (C-32), and Branch B navigation temporally coordinated (C-33) -- that is measurably lower in global error rate than any two composites alone, confirming the triple is super-additive relative to pairwise? | V-H: any two composites (pairwise designs); V-I: all three composites (V-04 pattern); measure global error rate across all execution paths and write points |
