Written to `simulations/quest/rubrics/listen-support-rubric-v9-2026-03-14.md`.

---

**What changed v8 -> v9:**

Two new criteria extracted from R8 above-ceiling signals:

| ID | Name | Tightens | Who satisfies |
|----|------|----------|---------------|
| C-28 | Phase-Architecture Severity Inference Rule | C-22 | V-04 |
| C-29 | Dedicated Switching-Friction Gap Sub-Section | C-27 P3 | V-04 |

**Tier:** Aspirational 19 / 95 pts -> 21 / 105 pts. Total ceiling **185 -> 195**.

---

**Signal extraction rationale:**

**C-28** comes from V-04's C-06 evidence: "Severity logic reinforced by phase structure — P1 adoption pain maps to P2/P3 severity; P3 parity gaps map to P1/P0." C-22 requires a phase map table with severity ranges, but doesn't bind the direction. C-28 requires stating *why* early-phase tickets produce lower severity (adoption friction is expected, workaround available) and why late-phase tickets produce higher severity (parity gaps block committed users). Without this rule, a model can satisfy C-22 with any defensible range-per-phase assignment; the inference rule makes a directional reversal a structural violation, not a judgment call. It also closes the architectural loop with C-27: a P1 body template framing dual-tool tension maps naturally to P2/P3 severity because the fallback tool is still in play.

**C-29** comes from V-04's C-04 evidence: "switching-friction sub-section is *additional*, not replacement" with "structured format including competitor field." C-27 P3 requires at least one switching-friction gap anywhere in the gap analysis — that gap can be buried inside the doc or design sub-sections. C-29 requires a dedicated fourth sub-section where every entry has a `Migrating from: [tool]` field, making the switching-friction inventory separately enumerable and every competitor reference grep-checkable by field rather than by reading prose.
ld. The dedicated sub-section makes switching-friction inventory separately enumerable; the per-entry field makes every entry's migration-source reference grep-checkable without reading gap descriptions. C-27 P3 passes with any competitor-named switching-friction gap anywhere in the gap analysis; C-29 passes only when switching-friction gaps occupy their own labeled sub-section with per-entry structured format.
...
## Aspirational Criteria (105 pts total -- raises the bar above 90)
...
**Essential ceiling**: 60 | **Recommended ceiling**: 90 | **Aspirational ceiling**: 195
