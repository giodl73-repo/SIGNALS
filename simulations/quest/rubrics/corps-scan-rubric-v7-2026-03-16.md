Written to `simulations/quest/rubrics/corps-scan-rubric-v7-2026-03-16.md`.

---

**Four new criteria extracted from R6 excellence signals:**

**C-29 -- Criterion-pair bundling in pre-check items** (behavior)
The only R6 criterion universal across all three variations. Pre-check items commit to criterion families as compound bundles (e.g., `"C-11+C-21+C-22+C-25+C-26"`, `"C-23+C-27"`). Distinct from C-18 (one C-NN per item as primary key) -- C-29 requires multiple criteria bundled on a single item, making intra-criterion dependencies explicit at declaration time.

**C-30 -- Post-write block as multi-criterion satisfaction declaration** (behavior)
From V-01 + V-02 independently. The post-write section cites 4+ C-NN criterion IDs simultaneously as a comprehensive satisfaction inventory. Both V-01 and V-02 converged on the same 6-criterion set (`C-14, C-02, C-27, C-25, C-26, C-28`). Distinct from C-19 (at least one criterion cited).

**C-31 -- Purpose-named phase pipeline structure** (behavior)
From V-03 exclusively. Structural units named by pipeline function (GATE / SCAN / DELIBERATE / DRAFT / FINALIZE) where each name encodes purpose before the reader enters the section. Distinct from C-20 (structural ordering enforcement) -- C-31 requires the unit names themselves to carry semantic pipeline meaning.

**C-32 -- Three-state execution vocabulary in pre-check** (behavior)
From V-01 exclusively. Pre-check distinguishes three states: CONFIRMED (already satisfied), SCHEDULED (committed to future section), ACKNOWLEDGED (constraint recorded but not committed to execution). Distinct from C-28 (2-state CONFIRMED vs. SCHEDULED).

**Scoring change:** 190 → 210 pts (24 aspirationals × 5 pts). Golden threshold stays at 80 pts.

**R6 projected under v7:** V-01 205 (C-29 + C-30 + C-32), V-02 200 (C-29 + C-30), V-03 existing + C-29 + C-31 -- breaks the R6 three-way tie at 190, with V-01 leading via execution-state depth cluster, V-02 second via post-write comprehensiveness, and V-03 earning the unique phase-naming credit.
ly skill whose sole output is org.yaml. The
...
strong outputs from excellent ones. 24 criteria x 5 pts = 120 pts available.
...
- Aspirational (C-09 to C-32): 5 pts each = 120 pts
- **Total: 210 pts**
...
- 0/24 aspirational: 0 pts
- Composite: 80 -- passes golden threshold

**Strong output**:
- All 5 essential pass: 60 pts
- All 3 recommended pass: 30 pts
- 4/24 aspirational pass: 20 pts
- Composite: 110

**Ceiling output**:
- All 5 essential pass: 60 pts
- All 3 recommended pass: 30 pts
- All 24 aspirational pass: 120 pts
- Composite: 210

**Failing output (common failure)**:
- C-05 fails (wrote .roles/): essential fail -- not golden regardless of composite score

**R6 reference scores (all scored 190/190 under v6; v7 separates V-01 from V-02 and V-03)**:
- V-01 (Role Sequence: Universal Labels + Tri-Part Pivot): 205/210 -- C-29 PASS (criterion-pair
  bundles in pre-check), C-30 PASS (6 criteria cited in post-write), C-31 FAIL (role-sequence
  naming, not phase naming), C-32 PASS (CONFIRMED/SCHEDULED/ACKNOWLEDGED three-state)
- V-02 (Output Format: Universal Labels + Tri-Part + SCHEDULED Pre-Check): 200/210 -- C-29
  PASS (compound bundles in pre-check), C-30 PASS (6 criteria cited in post-write), C-31 FAIL
  (section-numbered naming, not phase naming), C-32 FAIL (2-state CONFIRMED/SCHEDULED only)
- V-03 (Lifecycle Emphasis: 5-Phase Pipeline): scores under v6 + C-29 PASS (bundles present)
  + C-31 PASS (GATE/SCAN/DELIBERATE/DRAFT/FINALIZE phase naming); C-30 FAIL (post-write cites
  C-14 only, not 4+ criteria), C-32 FAIL (2-state CONFIRMED/SCHEDULED only)

V-01 reaches 205, the highest R6 score, by combining the C-29 bundle pattern (universal across
all R6 variations) with C-30 multi-criterion post-write and C-32 three-state vocabulary. V-02
reaches 200 by sharing C-29 and C-30 but lacking the ACKNOWLEDGED state. V-03 earns C-29 and
C-31 (the phase-naming pattern that no other R6 variation uses) but misses C-30 and C-32. The
new criteria correctly surface V-01 as the R6 leader via a technique cluster (execution-state
depth + post-write comprehensiveness) rather than structural novelty alone.
