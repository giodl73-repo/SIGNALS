`topic-story-rubric-v8-2026-03-14.md` written.
...
From the C-22 PASS note on V-01: "Template preamble states explicitly: 'No item tests content quality; each item locates a label string or reports it missing.'" C-22 and C-25 require the right behavior but leave the audit standard implicit -- the author must re-derive the locate-and-find requirement each time. A stated design invariant converts the per-item audit from a cognitive reconstruction into a single-sentence match. C-25 fires on per-item discipline; C-28 fires on whether the checklist documents the invariant that makes C-25 auditable without re-derivation.

**C-29 -- Inertia Measurement in the Recommendation Section Is Expressed via Required Labeled Fields**
From the C-13 PASS note: "`**Prior position**:`, `**Baseline distance**:`, `**Signals establish**:` carry the inertia measurement." C-13 fires on the inertia habit being present in two non-synthesis sections; no criterion fires on the structural form in Section 4. An instruction to compare against prior position allows the three operations (name the prior, characterize the distance, identify what signals establish) to collapse into a single qualifying sentence. Required labeled fields force them apart and make the measurement extractable for audit.

**C-30 -- Recommendation Sentence Form Carries a Structural Cross-Reference to the Disproof Condition**
From the C-13 PASS note (second form): "This call stands unless [**Disproof condition**] is shown to hold." C-15 and C-20 produce the disproof condition in Section 2 but leave Section 4 structurally disconnected from it. Without a required cross-reference, the disproof condition is logically present but structurally inert in the recommendation. V-01's required sentence form closes the loop: the verdict is issued and its revision condition is named in the same template slot.

**Scoring updated**: 23 aspirational criteria (C-08--C-30) sharing 10 pts, approximately 0.43 pts each (10/23). Formula unchanged.
t as a section-level prose instruction.

**C-30 -- Recommendation Sentence Form Carries a Structural Cross-Reference to the Disproof Condition**
From R7 Signal 3 (C-13 PASS, second form, on V-01). C-15 requires the falsifiability self-check to be explicit in the synthesis section; C-20 requires `**Disproof condition**:` to appear as a required labeled field. Both criteria produce the disproof condition in Section 2 but leave Section 4 structurally disconnected from it. V-01's required sentence form -- "I recommend [verdict] -- specifically [action], because [**Pattern claim**] forces this verdict. This call stands unless [**Disproof condition**] is shown to hold" -- creates a structural closure: the recommendation inherits the condition under which it should be revised. Without this cross-reference, the disproof condition is a quality signal in Section 2 that imposes no constraint on Section 4. With it, Section 4's recommendation is structurally conditioned by Section 2's falsifiability analysis: the verdict is issued and its revision condition is named in the same required sentence form. C-30 fires on whether the Section 4 template includes a required sentence form or required labeled field that names the disproof condition by cross-reference to the field produced in Section 2.
...
*Raise the bar once essential and recommended are stable. 10 pts total across C-08--C-30, approximately 0.43 pts each (10/23). Partial credit allowed.*
...
- Aspirational: 10 pts total across C-08--C-30, approximately 0.43 pts each (10/23). Partial credit allowed.
