`simulations/quest/rubrics/program-plan-rubric-v10-2026-03-14.md` written.

---

**v10 summary:**

Four new aspirational criteria extracted from R9 excellence signals:

| ID | Axis | Source | Discriminates |
|----|------|--------|---------------|
| **C-36** | Gate `requires:` clause — backward-looking epistemic precondition | R9-Signal-1 | V-04 + V-05 pass; V-01/V-02/V-03 fail |
| **C-37** | Gate `grounded-in:` — inter-gate epistemic chain completing the triad | R9-Signal-2 | V-04 + V-05 pass; V-01/V-02/V-03 fail |
| **C-38** | Gate `falsified-if:` — retroactive invalidation conditions | R9-Signal-3 | V-05 only |
| **C-39** | `arc-asserts:` — program-level epistemic synthesis | R9-Signal-4 | V-05 only (R10 anchor) |

**Scoring:** 235 → 255 pts (31 aspirational × 5). Golden threshold: **204** (~80%).

**R9 retroactive under v10:**

| Variation | Score | Notes |
|-----------|-------|-------|
| V-01 | 225/255 | No new criteria pass |
| V-02 | 225/255 | No new criteria pass |
| V-03 | 225/255 | No new criteria pass |
| V-04 | 235/255 | Pass C-36 + C-37 (bidirectional gate triad); fail C-38 + C-39 |
| V-05 | **245/255** | Pass all four — CEILING |

**Design family positions:**
- C-36 extends C-35 (asserting: → bidirectional: asserting: + requires:): gates become epistemic connectors, not isolated threshold checks
- C-37 extends C-27 (consequence chain → epistemic chain): where C-27 links gates by cascade propagation, C-37 links them by claim dependency
- C-38 extends C-35 (assertion → assertion + defeasibility): adds the conditions under which the C-35 claim is overturned
- C-39 extends C-35 from gate scope to arc scope: the program-level epistemic certificate that cannot be derived from individual gate assertions alone

**R10 gap:** C-39 (`arc-asserts:` — program-level synthesis) is the single highest-order V-05-only signal and the natural R10 anchor.
nal counts (e.g., "scout-feasibility artifact present", "3+ scout signals present") rather than stating abstract completion. |
...
## Aspirational Criteria (155 pts total)
...
| C-36 | **Gate Bidirectional Requirement: `requires:` clause names upstream epistemic precondition** | behavior | The gate format string contains a `requires:` clause naming the upstream epistemic precondition this gate depends on -- specifically, which prior gate's assertion must still hold for this gate's threshold count to be meaningful. C-35 adds the forward-looking `asserting:` claim (what this gate establishes when its threshold passes); C-36 adds the backward-looking `requires:` clause (what upstream claim this gate presupposes). Together, `asserting:` + `requires:` make each gate bidirectional: it both presupposes a prior epistemic state AND advances the evidence state when its threshold passes. A gate without `requires:` is semantically one-directional: it counts artifacts and asserts a new claim but does not declare its epistemic dependency on upstream gates. This exposes that threshold counts are not context-independent -- gate N's `>=3 signals` only means something if gate N-1's claim still holds. Pass if the gate format string in the gate-writing instruction contains a `requires:` clause (or semantic equivalent: "presupposes:", "contingent on:", "given that:", "depends on:") naming the upstream epistemic precondition by reference to a prior gate's assertion; fail if the gate template encodes threshold + cascade + assertion (C-22 + C-35 passing) but contains no backward-looking dependency declaration. Source: R9-Signal-1 -- V-04's `requires:` clause completes the bidirectional gate structure; combined with C-35's `asserting:`, gates declare both what they presuppose and what they establish. |
| C-37 | **Gate Epistemic Chain: `grounded-in:` links adjacent gate assertions** | depth | The gate format string (or an explicit inter-gate linkage instruction) includes a `grounded-in:` element that names which prior gate's `asserting:` claim the current gate's threshold is contingent on -- creating an inter-gate semantic chain where gate N's `requires:` clause is satisfied by gate N-1's `asserting:` clause. C-27 creates a consequence chain (removing gate N names the cascade consequence for gate N+1); C-37 creates an epistemic chain (gate N's threshold is only meaningful given gate N-1's established claim). The `asserting:` + `requires:` + `grounded-in:` triad (with C-35 supplying `asserting:`, C-36 supplying `requires:`) makes each gate epistemically self-describing: what it establishes, what it presupposes, and which prior gate's claim grounds it. Where C-27 registers consequence propagation (if gate N is removed, gate N+1's cascade breaks), C-37 registers epistemic dependency (gate N's count only holds given gate N-1's assertion). Pass if the variation requires each gate to name the specific prior gate assertion it is grounded in, making the epistemic dependency graph between gates explicit; fail if `requires:` and `asserting:` clauses are present (C-36 + C-35 passing) but the inter-gate epistemic linkage is not explicitly named. The first gate in the arc (scout handoff) has no prior gate to ground in; a variation passes C-37 if it specifies that the first gate's `grounded-in:` is anchored to the program's initial topic definition or evidence state, establishing the epistemic chain's origin. Source: R9-Signal-2 -- V-04's `grounded-in:` triad completes the semantic chain between adjacent gates, the epistemic counterpart to C-27's consequence chain. |
| C-38 | **Gate Falsification Clause: `falsified-if:` specifies retroactive invalidation conditions** | behavior | The gate format string includes a `falsified-if:` clause specifying conditions under which the gate's `asserting:` claim would be retroactively invalidated after the gate passes -- making gates epistemically accountable rather than permanently satisfied once their threshold count is reached. C-35 establishes what a passing gate asserts; C-38 requires the gate to specify the conditions that would overrule that assertion after the fact. For example: "[Gate: >=3 scout signals -- asserting: feasibility is sufficiently established -- falsified-if: scout:compliance subsequently reveals a blocking regulatory constraint]". Falsification awareness exposes that gate assertions are provisional: a threshold count establishes a claim given the evidence state at that time, but downstream evidence (especially from later-running scout:compliance or prove:hypothesis) can retroactively invalidate it. This completes the epistemic status model for gates: what is claimed (C-35), what it depends on (C-36/C-37), and when it can be overturned (C-38). The falsification clause does not reopen a passed gate -- it makes the conditions for re-evaluation explicit and auditable. Pass if the gate format string contains a `falsified-if:` clause (or semantic equivalent: "invalidated-if:", "overturned-by:", "defeasible-if:", "voided-by:") naming specific conditions under which the gate's assertion no longer holds; fail if the gate encodes threshold + cascade + assertion + grounded-in (C-22 + C-35 + C-36 + C-37 passing) but contains no falsification clause. Source: R9-Signal-3 -- V-05's `falsified-if:` clause is the first gate-template element to treat gate assertions as provisional epistemic claims with explicit invalidation conditions, not permanent checkboxes. |
| C-39 | **Arc-Level Semantic Assertion: `arc-asserts:` at program level** | depth | The variation requires an explicit `arc-asserts:` field (or equivalent program-level semantic claim) declaring the epistemic state established by the complete arc when all gates have passed -- extending the gate-level `asserting:` pattern (C-35) from individual stage boundaries to the arc as a whole. C-35 makes each gate semantically verifiable (the threshold established X); C-39 makes the full program semantically verifiable as a unit (the completed arc establishes Y). The `arc-asserts:` claim must synthesize what the full arc has established and be stronger than any individual gate's `asserting:` clause -- it names the program-level epistemic outcome, not a repetition of the final gate's claim. For example: "arc-asserts: sufficient evidence exists across all signal dimensions -- scout, draft, prove, review, flow, trace, listen, topic -- to commit this feature to development without residual epistemic gaps." The arc assertion is the semantic certificate of program completion: a plan whose gates all pass but whose arc-level claim is not stated has verified count conditions without establishing a unified epistemic conclusion. Pass if the variation requires an explicit arc-level semantic assertion -- by `arc-asserts:`, "program establishes:", "the arc asserts:", "this plan concludes:", or equivalent -- that synthesizes the program's epistemic claim as a single statement stronger than any individual gate's assertion; fail if semantic assertions exist only at the gate level (C-35 passing) without a program-level synthesis declaration. Source: R9-Signal-4 -- V-05's `arc-asserts:` field is the arc-level analogue of the gate-level `asserting:` clause, capping the full program with an explicit epistemic conclusion. |

Weight per aspirational: 5 pts (31 criteria x 5 = 155 pts)
...
Aspirational: C-09 [ ] C-10 [ ] C-11 [ ] C-12 [ ] C-13 [ ]
              ...
              C-39 [ ]
              Pass count: ___ / 31  -> ___ * 5  = ___ pts (of 155)

Composite score: ___ / 255

Golden: all essential pass AND composite >= 204  ->  [ ] YES  [ ] NO
```
...
- **C-36**: Evaluated on the variation text -- specifically the gate format string in the
  gate-writing instruction. Distinguished from C-35 (asserting: adds forward-looking claim) by
  direction: C-36 requires a backward-looking `requires:` clause naming what this gate depends on
  from upstream. The two clauses are complementary but independent: a gate can carry `asserting:`
  (C-35 passing) without carrying `requires:` (C-36 failing) if it states its claim but not its
  epistemic dependency. A gate can also carry `requires:` without `asserting:` (C-35 failing,
  C-36 passing) though this is architecturally incomplete. Full bidirectionality requires both.
  The `requires:` clause must reference a prior gate's assertion by name or stage, not just a
  count threshold (e.g., "requires: scout feasibility gate's claim holds" not just "requires:
  >=2 signals"). Pairing with C-35: together they make every gate an epistemic connector -- a
  node that both consumes an upstream claim and produces a downstream claim.
- **C-37**: Evaluated on the variation text -- specifically whether the inter-gate epistemic
  linkage is named explicitly. Distinguished from C-27 (gate-to-gate consequence chain: removing
  gate N causes a consequence in gate N+1's cascade reasoning) and from C-36 (requires: clause:
  backward dependency exists). C-37 requires the variation to instruct the model to name the
  specific prior gate assertion that grounds the current gate -- making the epistemic graph
  between gates explicit, not just declaring that a dependency exists. The `grounded-in:` element
  completes the triad: (a) what this gate asserts when passing (`asserting:` -- C-35), (b) what
  it requires from upstream (`requires:` -- C-36), and (c) which prior gate's claim provides that
  grounding (`grounded-in:` -- C-37). A variation passes C-37 if it instructs the model to name
  the specific prior gate assertion being depended on, not merely to declare that a dependency
  exists (C-36). First-gate anchoring: the first gate in the arc (scout handoff) has no prior
  gate to ground in; the variation should specify that the first gate's `grounded-in:` references
  the program's initial topic definition or evidence state, establishing the epistemic chain's
  origin point. A variation that requires `grounded-in:` for all subsequent gates but leaves the
  first gate's origin unspecified fails C-37.
- **C-38**: Evaluated on the variation text -- specifically the gate format string in the
  gate-writing instruction. Distinguished from C-35 (asserting: states what the threshold
  establishes), C-36 (requires: states upstream precondition), and C-22 (cascade-if-removed:
  states consequence of gate removal). C-38 is the fourth gate-template semantic layer:
  `falsified-if:` states conditions that would invalidate the gate's assertion after the fact.
  The boundary cases: (a) a cascade-if-removed clause (C-22 passing) is not a falsification
  clause -- it states what breaks if the gate is removed, not what overturns the gate's claim
  after it passes; (b) an asserting: clause (C-35 passing) is not a falsification clause -- it
  states what is claimed, not what would invalidate the claim. The falsification clause must be
  prospective ("if X subsequently occurs, this gate's assertion no longer holds") not retrospective.
  Specificity required: "falsified-if: any subsequent signal contradicts this" fails (too vague);
  "falsified-if: scout:compliance reveals a blocking constraint" passes (specific condition naming
  a real skill and outcome type). Source: R9-Signal-3 -- V-05 is the first variation to make gate
  assertions epistemically provisional by specifying explicit invalidation conditions.
- **C-39**: Evaluated on the variation text -- specifically whether an arc-level semantic assertion
  field is required as a distinct program-level output element. Distinguished from C-35 (gate-level
  asserting: clause: claims at each stage boundary) and C-09 (strategic pacing: arc-level
  breadth/depth strategy). C-39 requires a single synthesizing claim at the program level that
  cannot be derived by reading individual gate assertions in sequence -- it must integrate the
  full evidence picture into one epistemic conclusion. A variation that requires all gates to
  carry `asserting:` clauses (C-35 passing) but does not require an `arc-asserts:` synthesis
  field fails C-39. The arc assertion is the epistemic certificate of program-level completion:
  the claim established not by any single gate but by the full arc as a unit. Format is flexible
  (`arc-asserts:` field in YAML header, prose statement after echo stage, closing synthesis
  instruction step) -- what matters is program-level scope and epistemic synthesis. The arc
  assertion must name at least two evidence dimensions (e.g., "scout signals and prove artifacts")
  to demonstrate synthesis rather than restatement of a single gate's claim. Source: R9-Signal-4
  -- V-05's `arc-asserts:` is the only R9 variation to cap the full program with a unified
  epistemic conclusion; all other R9 variations encode gate-level semantics without an arc-level
  synthesis claim.
...
| Change | Detail |
|--------|--------|
| Added C-36 | Gate Bidirectional Requirement: `requires:` clause names upstream epistemic precondition -- from R9-Signal-1: V-04's `requires:` clause extends C-35's forward-looking `asserting:` with a backward-looking epistemic dependency declaration. C-35 states what a gate establishes; C-36 states what a gate presupposes. Together they make gates bidirectional -- counting artifacts AND declaring the upstream claim that makes the count meaningful. V-04 and V-05 pass; V-01/V-02/V-03 fail. |
| Added C-37 | Gate Epistemic Chain: `grounded-in:` links adjacent gate assertions -- from R9-Signal-2: V-04's `grounded-in:` element completes the `asserting:` + `requires:` + `grounded-in:` semantic triad, naming which prior gate's claim grounds the current gate's threshold. C-27 creates a consequence chain between adjacent gates; C-37 creates an epistemic chain. Where C-27 registers "removing gate N breaks gate N+1's cascade", C-37 registers "gate N's count only means something given gate N-1's assertion." V-04 and V-05 pass; V-01/V-02/V-03 fail. |
| Added C-38 | Gate Falsification Clause: `falsified-if:` specifies retroactive invalidation conditions -- from R9-Signal-3: V-05's `falsified-if:` clause makes gate assertions epistemically provisional by naming specific conditions under which the gate's `asserting:` claim would be retroactively invalidated after the gate passes. C-35 establishes what is claimed; C-38 establishes when the claim no longer holds. Together they complete the epistemic lifecycle of a gate assertion: what is established (C-35), what grounds it (C-36/C-37), and when it is overturned (C-38). V-05 only. |
| Added C-39 | Arc-Level Semantic Assertion: `arc-asserts:` at program level -- from R9-Signal-4: V-05's `arc-asserts:` field extends the gate-level `asserting:` pattern (C-35) to the arc as a whole, declaring the unified epistemic claim established when all gates have passed. Individual gate assertions (C-35) verify evidence state at each boundary; the arc assertion synthesizes the program-level epistemic conclusion. The arc assertion is the semantic certificate of program completion -- a plan whose gates all pass but whose arc-level claim is unstated has verified counts without establishing a unified conclusion. V-05 only (R10 anchor). |
| Aspirational tier | 27 criteria x 5 pts -> 31 criteria x 5 pts = 155 pts |
| Total points | 235 -> 255 |
| Golden threshold | composite >= 188 -> composite >= 204 (same ~80% of max) |
| R9 retroactive scores under v10 | V-05: 245/255 (ceiling -- passes all four new criteria: C-36 + C-37 + C-38 + C-39); V-04: 235/255 (passes C-36 + C-37, fails C-38 + C-39); V-01/V-02/V-03: 225/255 (pass no new criteria -- all above golden). C-38 (gate falsification) and C-39 (arc-level assertion) are V-05-only signals; C-39 (`arc-asserts:` program-level synthesis) is the natural R10 anchor as it caps the full epistemic arc rather than a single gate boundary. |
| Design family positions | C-36 extends C-35 (asserting: -> bidirectional: asserting: + requires:): a gate that only asserts is semantically one-directional; C-36 adds the upstream dependency making it bidirectional. C-37 extends C-27 (consequence chain -> epistemic chain): C-27 links gates by cascade propagation; C-37 links them by claim dependency -- the epistemic counterpart to the consequence register. C-38 extends C-35 (assertion -> assertion + defeasibility): epistemic claims are provisional; C-38 adds the conditions under which C-35's claim no longer holds. C-39 extends C-35 from gate scope to arc scope: the same asserting: pattern applied to the full program as a unit, producing a program-level epistemic certificate that summarizes what the entire arc has established. |
