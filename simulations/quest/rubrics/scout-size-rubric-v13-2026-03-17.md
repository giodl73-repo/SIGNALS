Written to `simulations/quest/rubrics/scout-size-rubric-v13-2026-03-17.md`.

---

## v13 summary

**3 new criteria extracted from R12 excellence signals. Aspirational denominator: 27 → 30.**

| ID | Name | Source | Axis | Relationship |
|----|------|--------|------|-------------|
| C-36 | Constraint tags cover ALL vocab-constrained fields | V-01 R12 | Coverage | C-35 extension — minimum (C-01+C-02 only) does not satisfy |
| C-37 | Inertia uses 5-field format (adds Cost Trajectory) | V-02 R12 | Temporal resolution | C-34 extension — Direction (vector) ≠ Trajectory (shape over time) |
| C-38 | PHASE SEALED blocks at end of each phase | V-03 R12 | Completion | C-20 complement — C-20 prevents contamination, C-38 confirms completeness |

**R12 scores under v13:**

| Variation | Asp. | Score | Δ from champion |
|-----------|------|-------|----------------|
| V-01 | 28/30 | 99.33 | C-36 PASS, C-37/C-38 FAIL |
| V-02 | 28/30 | 99.33 | C-37 PASS, C-36/C-38 FAIL |
| V-03 | 28/30 | 99.33 | C-38 PASS, C-36/C-37 FAIL |

**v13 champion**: Not yet achieved. Requires a single variation combining comprehensive constraint tag coverage (C-36) + 5-field inertia with Cost Trajectory (C-37) + PHASE SEALED blocks (C-38). Target for R13.

**Scoring model updated**: `aspirational_pass / 30 * 10` (was `/27`).

**C-29 and C-30 scope updated** to include the new criteria in their respective categories (C-36 and C-38 join the structural set; C-37 joins the depth set).
 cost is growing linearly differs from one that is compounding or plateauing — Trajectory
  informs urgency calibration in a way Direction alone cannot. V-02 R12 is the first variation
  to add this fifth field.

- **C-38** completes the phase governance *completion* axis: C-20 (complete closure mesh)
  prevents cross-phase production by requiring each phase to name what it exclusively owns and
  exclude. C-38 adds a terminal confirmation artifact — a PHASE SEALED block with a structured
  checklist at the end of each phase, confirming all phase-required fields are resolved before
  the next phase opens. C-20 prevents contamination; C-38 confirms completion. Together they
  form a two-layer phase governance system. V-03 R12 is the first variation to introduce
  per-phase SEALED blocks.

**R12 scores under v13:**

| Variation | Aspirational | Score | Axis advanced |
|-----------|-------------|-------|--------------|
| V-01 (C-35 extended to all vocab fields) | 28/30 (fails C-37, C-38) | 99.33 | C-36 |
| V-02 (5-field inertia — Cost Trajectory) | 28/30 (fails C-36, C-38) | 99.33 | C-37 |
| V-03 (PHASE SEALED blocks) | 28/30 (fails C-36, C-37) | 99.33 | C-38 |
| V-04 | TBD | TBD | — |
| V-05 | TBD | TBD | — |

No R12 variation achieves all three new criteria simultaneously. The v13 champion requires
combining comprehensive constraint tag coverage (C-36), 5-field inertia (C-37), and PHASE
SEALED blocks (C-38). Target for R13.

---

## Purpose

This rubric scores the output of the `scout-size` skill against 38 criteria grouped into three
...
- **Description**: Refinement of C-28. Every structural criterion (C-18 through C-27, C-33,
  C-35, C-36, C-38) in the self-check carries both a pass condition and an exact disqualifying
  form — a specific structural pattern that looks like compliance but fails. Older structural
  criteria must not regress to pass-only descriptions as the denominator grows.
- **Pass condition**: Self-check items for all structural criteria (C-18 through C-27, C-33,
  C-35, C-36, C-38) include both a pass condition and an exact structural disqualifying form.
  An output that satisfies C-28 with exact disqualifying forms only for the latest-round
  criteria fails C-29 unless every structural criterion carries equivalent precision.
...
- C-29 is a refinement of C-28, sourced from R8. C-28 requires the self-check to exist.
  C-29 requires structural criteria items (C-18-C-27, C-33, C-35, C-36, C-38) to include
  exact disqualifying forms. Coverage uniformity across structural criteria is the
  discriminator. Note: v13 expands the structural criteria set to include C-36 and C-38.
- C-30 is a category extension of C-29, sourced from R9. C-29 achieves structural-category
  ...
  self-check carries only a pass condition. Note: v13 expands the depth criteria set to
  include C-37.
