Written to `simulations/quest/rubrics/topic-echo-rubric-v12-2026-03-16.md`.

---

# topic:echo Rubric -- v12

**Three key changes from v11:**
- Max composite: 230 -> 245
- Aspirational criteria: 28 -> 31
- New relationships: C-34/C-37, C-35/C-38, C-36/C-39

**Three new criteria (C-37, C-38, C-39):**

| ID | Gate | Pattern | What it adds |
|----|------|---------|--------------|
| C-37 | C-34 | BC-STEP1-SUBPHASE-DECOMPOSITION | C-34 makes BC's coverage record auditable as an output. C-37 makes BC's internal process observable: Step 1 is decomposed into [SCAN] / [FREEZE] / [COVERAGE AUDIT] sub-phases, each with a distinct function, BC-COVERAGE-RECORD produced as the [COVERAGE AUDIT] gate return. BC is auditable not just at the output boundary but at each internal transition. |
| C-38 | C-35 | PHASE-HANDOVER-TABLE-SCHEMA | C-35 makes role exits auditable via PHASE-HANDOVER tokens. C-38 standardizes token format: each token is a 5-row markdown table with fixed schema fields (Completing Role / Step Completed / Output Produced / Exclusion In Effect / Receiving Role). Tokens are schema-comparable and field-complete by inspection, not free-form prose. |
| C-39 | C-36 | FILTER-IMPERATIVE-FORM | C-36 requires typed DISCARD-[N] entries for every non-accepted candidate. C-39 makes that completeness structurally enforced: the STILL-LIVE FILTER is written as four unconditional imperative commands with no conditional branching. No candidate evaluation can reach the next step without producing a written record, because there is no conditional branch to skip. |
...
| Essential | 60 | 5 | 12 |
| Recommended | 30 | 3 | 10 |
| Aspirational | **155** | **31** | 5 |
| **Max composite** | **245** | | |

**R11 scoring under v12:**

| Variation | C-34 | C-35 | C-36 | C-37 | C-38 | C-39 | v11 | v12 | delta |
|-----------|:----:|:----:|:----:|:----:|:----:|:----:|:---:|:---:|:-----:|
| V-01 | PASS | FAIL | FAIL | PASS | FAIL | FAIL | 220 | 225 | +5 |
| V-02 | FAIL | PASS | FAIL | FAIL | PASS | FAIL | 220 | 225 | +5 |
| V-03 | FAIL | FAIL | PASS | FAIL | FAIL | PASS | 220 | 225 | +5 |
| V-04 | PASS | PASS | FAIL | PASS | PASS | FAIL | 225 | 235 | +10 |
| **V-05** | PASS | PASS | PASS | PASS | PASS | PASS | 230 | **245** | +15 |

**Round 11 progression layer:** ... -> symmetric provenance / phase-exit observability / selection transparency -> **BC internal-process observability / handover schema fidelity / filter structural completeness**.

**The unified R11 thesis:** Structural presence is not structural enforcement. The R10 thesis required symmetric auditability. The R11 thesis requires each auditability mechanism be structurally enforced: sub-phase decomposition makes the coverage record non-defeatable by monolithic step collapse; table schema makes handover records non-defeatable by format collapse; imperative form makes the discard log non-defeatable by conditional branching.

---

The three new criteria extract the same architectural move applied in parallel across all three R10 mechanisms: each R11 criterion is the enforcement layer of its R10 parent. The gate chain is now: R10 output auditable -> R11 output non-defeatable.
nforcement is a property of the mechanism's structure, not its presence.
...
## Aspirational Criteria (weight: 155 points total, 5 pts each)
...
- Aspirational: 155 points total (31 criteria, 5 points each)
- Max composite: 245
...
- Aspirational: 1/31 pass -> 1/31 * 155 = 5
...
- Essential: 5/5 -> 60
- Recommended: 3/3 -> 30
- Aspirational: 31/31 -> 155
- **Composite = 245** -- maximum
...
candidates are accounted for. Practical test: can a reviewer determine whether a surprise was
excluded from the distillation layer, and why? C-20 alone: no. C-20 + C-36: yes.

**C-34 / C-37**: C-34 is the output layer (BC produces BC-COVERAGE-RECORD making its scope
observable). C-37 is the process layer (Step 1 is decomposed into [SCAN] / [FREEZE] /
[COVERAGE AUDIT] sub-phases, making BC's internal transitions observable rather than only its
final output). An artifact can satisfy C-34 (coverage record is present and complete) while
failing C-37 (Step 1 is monolithic; the record appears without structural evidence of when
within Step 1 it was produced or what scope was in effect). Without C-37, BC's output is
observable but BC's process is not: a reviewer knows what BC produced at the end of Step 1
but cannot confirm it was produced at the correct phase boundary. With C-37, BC's process is
structurally transparent: the [COVERAGE AUDIT] sub-phase ensures BC-COVERAGE-RECORD is
produced at a named boundary before exclusions take effect, not as an undifferentiated step
output. Practical test: can a reviewer confirm that BC-COVERAGE-RECORD was produced before
BC exclusion was applied, from the artifact specification alone? C-34 alone: no. C-34 + C-37: yes.

**C-35 / C-38**: C-35 is the token layer (PHASE-HANDOVER tokens at EF-exit and BC-exit make role
transitions exit-visible). C-38 is the schema layer (each token is rendered as a 5-row markdown
table with fixed fields -- Completing Role, Step Completed, Output Produced, Exclusion In Effect,
Receiving Role). An artifact can satisfy C-35 (both tokens present, both name what was produced)
while failing C-38 (tokens are prose declarations; fields may be missing or inconsistently
structured across the two tokens). Without C-38, PHASE-HANDOVER tokens are individually auditable
but format-variable: a reviewer must parse free-form prose to extract field values, and there is
no structural guarantee that a field absent in prose was not omitted rather than inapplicable.
With C-38, both tokens are schema-identical: field presence is binary (present / absent) rather
than a parsing judgment, and cross-role comparison is possible by column inspection. C-38 applies
the same principle as C-32 (evidentiary quotation makes tokens self-evidencing rather than
declarative) to handover records: schema makes them self-verifying rather than self-reporting.
Practical test: can a reviewer confirm all required fields are present in both PHASE-HANDOVER
tokens without prose parsing? C-35 alone: no. C-35 + C-38: yes.

**C-36 / C-39**: C-36 is the completeness layer (every rejected candidate produces a typed
DISCARD-[N] entry; DISCARD-LOG-COMPLETE confirms all candidates are accounted for). C-39 is
the enforcement layer (the STILL-LIVE FILTER is written as unconditional imperative commands,
structurally eliminating the conditional branch through which the discard record step could be
skipped). An artifact can satisfy C-36 (DISCARD LOG present, entries typed, DISCARD-LOG-COMPLETE
emitted) while failing C-39 (filter uses conditional "if NO" structure; completeness depends on
the conditional branch being followed rather than being structurally enforced). Without C-39,
C-36's completeness requirement is a specification obligation: the filter is specified to produce
discard entries, but the conditional structure means a downstream implementation could reach
the next step without producing the required record by omitting the NO branch. With C-39,
completeness is a structural property: no candidate evaluation can complete without writing
a record, because there is no conditional branch to skip. The filter specification itself
enforces what C-36 requires. Practical test: can a correct implementation of the STILL-LIVE
FILTER specification produce fewer DISCARD-[N] entries than there are rejected candidates?
C-36 alone: yes (conditional branch omission). C-36 + C-39: no (unconditional form prevents it).

---

## Historical criterion notes

**v12 -- three new criteria from Round 11:**

C-37 (structure, gate: C-34): BC Step 1 sub-phase decomposition. C-34 makes BC's coverage record
auditable as an output; C-37 makes BC's internal process observable -- Step 1 is decomposed into
[SCAN] / [FREEZE] / [COVERAGE AUDIT] sub-phases, making BC-COVERAGE-RECORD's production boundary
structurally verifiable rather than inferred from output alone.

C-38 (structure, gate: C-35): PHASE-HANDOVER table schema. C-35 makes role-exit transitions
auditable via PHASE-HANDOVER tokens; C-38 standardizes the token format to a 5-row markdown
table with fixed schema fields -- converting free-form prose tokens into schema-comparable,
field-complete records that are self-verifying by column inspection.

C-39 (structure, gate: C-36): STILL-LIVE FILTER as unconditional imperative commands. C-36
requires typed DISCARD-[N] entries for every rejected candidate; C-39 makes that completeness
structurally enforced by eliminating conditional branching in the filter specification --
the filter cannot proceed without producing the required record because there is no conditional
branch to skip.
