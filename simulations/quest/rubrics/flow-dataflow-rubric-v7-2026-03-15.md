Reading the Round 7 evidence for patterns not captured by C-01 through C-18:

**C-19** — V-01/V-04: Typed exit manifests with field-typed assertions (`field: TYPE(precision)`) that serve as sole downstream authority. C-04 passes on description/diff alone; this is a structurally higher tier.

**C-20** — V-01/V-04: Entity-at-risk annotation names entity **and** a specific field from that entity's typed manifest (`entity.field_name — risk`). C-17 passes on entity-name-only; this is one tier above.

**C-21** — V-02/V-04: Boundary latency decomposed into named additive sub-components (Transport + Processing Overhead) with separate numeric columns. C-05 passes on total characterization; decomposed components are new.

**C-22** — V-02/V-04: Cumulative SLA% consumed per boundary with explicit domination-point identification. No existing criterion covers budget accounting.

**C-23** — V-03: A dedicated closure gate (structurally separate from the recovery table) that forward-checks every declared NH/LP identifier has a recovery row. C-13 enforces visibility within a single table; this is meta-structural across all tables.
...
| C-21 | Decomposed boundary latency            | depth       | Boundary latency (C-05) is decomposed into at least two named additive sub-components — e.g., Transport Latency and Processing Overhead — each carrying a distinct numeric value or range in a separate column or labeled entry. Sub-component values must be coherent with the total boundary latency. A single total latency figure, even with a range or characterization label, does not qualify. |
| C-22 | Cumulative SLA budget tracking         | depth       | A dedicated column or structural row tracks cumulative SLA budget consumed (%) at each boundary, accumulating from the first boundary through the last. The boundary that consumes the largest share of total budget is explicitly identified as the budget domination point. A trace that characterizes per-stage or per-boundary latency without cumulative budget accounting and domination-point identification does not qualify. |
| C-23 | Declaration-to-reference closure gate  | structure   | A dedicated structural closure check — physically separate from the recovery table itself — cross-verifies that every identifier in the NH/LP scheme has a corresponding recovery row. The gate must produce a structurally visible pass/fail signal (checklist, closure table, or explicit gate block) so that any missing row is immediately visible without cross-referencing both sections manually. A prose assertion of completeness ("all NH and LP identifiers have been addressed") does not qualify. |
```
