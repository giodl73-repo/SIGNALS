Rubric v11 written to `simulations/quest/rubrics/discover-competitors-alt-rubric-v11-2026-03-17.md`.

---

**New criterion extracted from R11:**

**C-43 — FOCUS-STATE declared as named output contract slot** (behavior, 5 pts)

The differentiating pattern from R11: V-02 and V-03 scored PARTIAL on C-34 because GATE 0 produced only control flow — no named output slot. So slot 5's "Required by" column could only express a gate reference ("Requires GATE 0") rather than a slot reference ("Requires slot 0 (FOCUS-STATE)"), crossing a type boundary that breaks the uniformity of the dependency graph.

V-01/V-04/V-05 declared FOCUS-STATE as slot 0 with a format template, making slot 5's dependency machine-readable by slot number — the same mechanism as every other inter-slot dependency.

**Scoring update:**
| | v10 | v11 |
|--|-----|-----|
| Aspirational count | 34 | 35 |
| Aspirational max | 170 | 175 |
| Max composite | 260 | 265 |
| Formula | `/ 34 x 170` | `/ 35 x 175` |
| Exceptional | >= 245 | >= 250 |
| Strong | >= 219 | >= 223 |
| Passing | >= 90 | >= 90 |

**Dependency:** C-43 is independent of C-40 (gate structure vs. output slot declaration). Together they close both surfaces of the focus dimension: C-40 makes focus validation first-class at the gate level; C-43 makes focus resolution first-class at the output slot level. C-43 is a prerequisite for uniform slot-level C-34 satisfaction on focus-dependent slots but does not imply C-34 in general.
 for focus-dependent downstream slots; C-43 does not imply C-34 in general.
- C-43 is independent of C-32: a skill can satisfy C-32 (OUTPUT CONTRACTS first in PREFLIGHT) without declaring a FOCUS-STATE slot, and vice versa.
...
## Aspirational Criteria (weight = 175 points total, 5 points each)
...
Grade bands rescaled proportionally from v10 (v10 max = 260; v11 max = 265).
