...
|----|-----------|-------------|--------------------------|
| C-38 | **Amendment table T-37 trigger entry includes inline failure-condition exemplar** | V-02 R13 excellence signal | C-18 requires the amendment table to be front-loaded with trigger rules naming which criterion fires under which condition. C-29 requires a PHASE column with populated values. C-38 requires the T-37 trigger entry specifically to carry an inline exemplar note within the CONDITION field — a parenthetical or note quoting the exact document construct that fires T-37. A T-37 entry stating "C-37: per-column attribution absent from Domain 2" satisfies C-18 (condition named) and C-29 (PHASE present) but fails C-38 (no inline exemplar of what row-level-only Domain 2 confirmation looks like). V-02 R13 demonstrated the target behavior: "row-level confirmation 'R-NN IDs applied to risk row' fires T-37" — the trigger entry itself tells a reviewer what to look for. C-38 makes T-37 self-documenting at the amendment table level, removing the need to consult the rubric to interpret the trigger boundary. |

**Denominator:** `/30` → `/31`. Each aspirational criterion = `10/31` ≈ `0.323` points.

**Score projections under v14 for R13 variations:**
| Variation | v13 fails | v14 cascade adds | Total fails | Composite |
|-----------|-----------|-----------------|-------------|-----------|
| V-02 R13 (C-37 FAIL, C-38 PASS, C-24 FAIL: 37 rows < 38) | C-37 | C-24 (38-row req) | 2 | **99.35** |
| V-01 R13 (C-36 FAIL, C-38 FAIL: T-37 exemplar absent, C-24 FAIL: 37 rows < 38) | C-36 | C-24 (38-row req), C-38 | 3 | **99.03** |
| V-03 R13 (C-24, C-36, C-37 FAIL) | C-24, C-36, C-37 | C-38 (T-37 absent from stale table) | 4 | **98.71** |

**Cascade invariants:**

- **C-38 cascade from C-24 stale table**: Any variation failing C-24 due to an amendment table missing T-37 (stale table omits T-37 entry entirely) also fails C-38 — an inline exemplar cannot appear in an entry that was never authored. The cascade is deterministic: C-24 FAIL from T-37 absence → C-38 FAIL.
- **C-38 independent fail**: A variation can pass C-24 (38-row table present) and C-18 (trigger rules named) while failing C-38 when the T-37 entry carries an abstract condition statement without an inline exemplar quote. V-01 R13 demonstrates this: 37-row table (which under v14 cascades to C-24 FAIL), but the structural independence holds — a 38-row table with an abstract T-37 entry would fail C-38 while passing C-24.

**V-02 R13 as C-38 exemplar**: T-37 entry in the amendment spec carries: "row-level confirmation 'R-NN IDs applied to risk row' fires T-37" — the inline note quotes the exact failure pattern, not just the criterion category. This is the target behavior for C-38.
...
- **C-24** is structurally testable at PRE-DOCUMENT: a reviewer counts the amendment table rows at initialization. For v14, expected count = 38 (one row per C-01..C-38). A count of 37 (v13 table carried forward without adding T-38) fires C-24 at PRE-DOCUMENT.
...
## Aspirational Criteria (10% of composite, denominator /31)

### C-01 through C-07 — Mandatory C-tier (not counted in /31 denominator)
...
### C-08 through C-38 — Aspirational Criteria (denominator /31)
...
| C-24 | **Amendment table row count matches v14 criterion count** | behavior | Amendment table contains exactly 38 trigger rows at initialization (one per C-01..C-38 criterion). A table built from v13's criterion list (37 rows) without adding T-38 fires C-24 at PRE-DOCUMENT. A table with any count other than 38 fails C-24. |
...
```
composite = 90 + (aspirational_pass / 31 × 10)
```
...
- Amendment table T-37 entry states trigger condition abstractly ("C-37: per-column attribution absent from Domain 2") without an inline exemplar quoting the failure construct — fails C-38 even when C-18 (front-loaded trigger rules) and C-29 (PHASE column) both pass
...
For early-round discriminator notes (R1–R11), see the v12 rubric file. The v14 rubric inherits all prior discriminator observations unchanged.
