v9 written. Here's what was extracted and why each is a new criterion rather than a pass-condition tightening:

---

**C-30 — Citation Architecture preamble as single-read authority** (behavior)
C-28/C-29 require correct behavior at each boundary individually. C-30 requires all four artifact definitions and exact-copy requirements to be front-loaded into a single named preamble table before Phase 1. V-39 hit 290/290 via distributed per-phase instructions; V-40 hit the same score but added a `CITATION ARCHITECTURE DECLARED` meta-artifact table that concentrates all four definitions in one place. The structural argument: distributed instructions are vulnerable to drift across 1500+ tokens — a Phase 2 definition may be read 1000 tokens before Phase 5(2) is written. A single-read preamble eliminates that drift surface. C-28 can pass without C-30; C-30 requires the preamble form specifically.

**C-31 — Citation chain completeness self-audit** (behavior)
C-28/C-29 are per-boundary criteria — you can PARTIAL one and PASS the other. C-31 captures the binary verifiability insight: a single description link defeats the entire chain's checkability by label comparison. It requires the output itself to enumerate all (source → consumer) pairs after all production phases, declare a binary reference-chain/description-chain verdict per link, and explicitly label any description link as a chain integrity failure. V-36 and V-38 each closed one gap and both scored 280; neither included an audit. C-31 makes partial chain compliance structurally visible inside the output itself.

---

**Scoring:** 310 pts max (was 290). 23 aspirational criteria (was 21).
8 closed the other (Phase 6 Part B boundary), but both scored
280/290 because a single description link makes the entire chain non-verifiable by label
comparison -- the chain is either fully a reference chain or it is not. Neither V-36 nor V-38
included a completeness audit; their partial compliance was only visible through per-phase
analysis. An explicit audit before any summary section makes chain integrity checkable in one
place: an auditor can confirm full verifiability without reading every production phase. C-28
and C-29 can both pass without C-31; C-31 requires an enumerated audit of all (source artifact
-> consumer site) pairs with a binary reference-chain / description-chain verdict per link, and
an explicit declaration that any description link is a chain integrity failure.
...
- **23 aspirational (C-09--C-31):** scoring methodology transparency, time-dependent inertia
...
10 pts each (230 total) = **310 pts max**.
