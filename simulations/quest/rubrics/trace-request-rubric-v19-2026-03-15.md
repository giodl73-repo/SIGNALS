# trace-request Rubric — v19 (2026-03-15)

**Scale:** 280 (Essential 60 + Recommended 30 + Aspirational 190)
**Golden threshold:** all 4 essential PASS AND composite >= 80% (224/280)

---

## One new criterion from R20 excellence signals

| New criterion | R20 excellence signal | What is testable |
|---|---|---|
| **C-45** GATE-8B VALIDATION-DERIVATION inline derivation token | ES-1 (R19 V-03 + R20 V-01): Two independent variation axes — first-axis structured field blocks (R19 V-03, single-axis C-45 isolation, no CONSISTENCY-RULE confound) and second-axis phrasing register (R20 V-01, no CONSISTENCY-RULE confound) — both produced `VALIDATION-DERIVATION: [derive outcome from matches value at point of emission: "yes" → "Contract prediction confirmed"; "no" → "falsified -- actual halt boundary was [BC-N label]"]` as a named structural token in the EVENT: CONTRADICTION / Step 8b block, co-located with DECLARE CONTRADICTION before Step 8c is populated; R20 V-03 provides a third confirming axis (structured field blocks, independent surface); R20 V-02 confirms that when CONSISTENCY-RULE (C-44) is present, the inline surface is intentionally suppressed — the two surfaces are complementary design choices, C-44 declaring the derivation as a pre-trace algorithm rule, C-45 surfacing it inline at the point the contradiction event fires, adding a third cross-reference point to the HALT-EXPECTED-BOUNDARY consistency chain at Step 8b between the C-42 match flag and the C-39 post-execution validation token | Is `VALIDATION-DERIVATION:` present as a named structural token in the EVENT: CONTRADICTION / Step 8b block? Does it appear co-located with DECLARE CONTRADICTION before Step 8c is populated? Does it derive the TRACE CONTRACT validation outcome from the `matches HALT-EXPECTED-BOUNDARY: [yes/no]` boolean at the point of emission? Does it declare the `yes` branch derivation → "Contract prediction confirmed"? Does it declare the `no` branch derivation → "falsified -- actual halt boundary was [BC-N label]"? Is the token machine-greppable via `VALIDATION-DERIVATION:` without semantic prose reading? Is the derivation inline (at Step 8b emission point) rather than deferred to post-execution TRACE CONTRACT validation? |

**Scale:** 275 → **280** (aspirational tier: 37 → 38 criteria, 185 → 190 pts). Golden threshold stays at >= 80%.

---

## Key design decisions

**C-45 vs C-44:** C-44 tests that the CHECKER ALGORITHM additionally declares a CONSISTENCY-RULE in Phase 0B encoding the structural derivation path between the C-42 `matches HALT-EXPECTED-BOUNDARY:` boolean and the C-39 TRACE CONTRACT validation outcome as a pre-trace named algorithm rule, machine-checkable from the algorithm block alone without reading Step 8b prose. C-45 tests that the VALIDATION-DERIVATION outcome is additionally surfaced inline in the EVENT: CONTRADICTION / Step 8b block at the point the contradiction fires, co-located with DECLARE CONTRADICTION before Step 8c is populated — adding a third cross-reference point between C-42 (match flag) and C-39 (post-execution validation token) at the Step 8b surface where the contradiction event fires. C-44 present but no inline VALIDATION-DERIVATION = C-44 PASS + C-45 FAIL (V-02 pattern: CONSISTENCY-RULE encodes the derivation path, inline emission is redundant and explicitly suppressed). VALIDATION-DERIVATION inline with both branches declared and no CONSISTENCY-RULE = C-45 PASS + C-44 FAIL (V-01 / V-03 pattern). Both present simultaneously = C-44 PASS + C-45 PASS (combined pattern).

**C-45 vs C-40:** C-40 tests that `DECLARE CONTRADICTION: BC-N -- [label] -- arm: [which arm]` is present as a named structural token in Step 8b prose before Step 8c is populated — the three-field contradiction event is machine-greppable at the Step 8b prose surface. C-45 tests that a `VALIDATION-DERIVATION:` token is additionally co-located with DECLARE CONTRADICTION at the same Step 8b prose surface, inline-deriving the TRACE CONTRACT validation outcome from the `matches HALT-EXPECTED-BOUNDARY:` boolean at the point of emission rather than deferring derivation to the post-execution TRACE CONTRACT validation token. DECLARE CONTRADICTION present (C-40 PASS) but no inline VALIDATION-DERIVATION = C-40 PASS + C-45 FAIL. VALIDATION-DERIVATION inline with both branches declared = C-40 PASS + C-45 PASS.

**C-45 vs C-42:** C-42 tests that a fourth field `matches HALT-EXPECTED-BOUNDARY: [yes/no]` is present in DECLARE CONTRADICTION, creating an intertextual cross-reference between the Step 8b contradiction event and the Phase 0A TRACE CONTRACT pre-declaration — the boolean field is the source of machine-checkable cross-reference without reading the TRACE CONTRACT validation token. C-45 tests that a VALIDATION-DERIVATION token is additionally present, consuming the C-42 boolean at the point of emission to derive the validation outcome inline — making the full derivation chain (match flag → validation outcome) readable at the Step 8b surface without navigating to the post-execution TRACE CONTRACT validation token. C-42 match flag present (C-42 PASS) but no inline derivation = C-42 PASS + C-45 FAIL. VALIDATION-DERIVATION inline deriving outcome from match flag with both branches declared = C-42 PASS + C-45 PASS.

**New dependency edge:**
- C-45 depends on C-42 AND C-40 (C-42 establishes `matches HALT-EXPECTED-BOUNDARY: [yes/no]` as the source boolean for the inline derivation — VALIDATION-DERIVATION consumes this field at the point of emission; C-40 establishes the DECLARE CONTRADICTION block in Step 8b as the structural location where VALIDATION-DERIVATION is co-located before Step 8c is populated; C-39 establishes TRACE CONTRACT validation outcome as the named target vocabulary — "Contract prediction confirmed" and "falsified -- actual halt boundary was [BC-N label]" — that VALIDATION-DERIVATION pre-derives inline)

---

## One new criterion from R19 excellence signals (carried from v18)
...
**Scale:** 270 → **275** (aspirational tier: 36 → 37 criteria, 180 → 185 pts). (Carried from v18.)

---

## Key design decisions (R19, carried from v18)
...
## Two new criteria from R18 excellence signals (carried from v18)
...
## Key design decisions (R18, carried from v18)
...
## Two new criteria from R17 excellence signals (carried from v18)
...
## Key design decisions (R17, carried from v18)
...
## One new criterion from R16 excellence signal (carried from v18)
...
## Key design decisions (R16, carried from v18)
...
## One new criterion from R15 excellence signal (carried from v18)
...
## Key design decisions (R15, carried from v18)
...
## One new criterion from R13 excellence signal (carried from v18)
...
## Key design decisions (R13, carried from v18)
...
## Three new criteria from R12 excellence signals (carried from v18)
...
## Key design decisions (R12, carried from v18)
...
## Three new criteria from R11 excellence signals (carried from v18)
...
## Three new criteria from R10 excellence signals (carried from v18)
...
## Full aspirational tier — C-21 through C-45
...
| C-45 | GATE-8B VALIDATION-DERIVATION inline derivation token | 5 | C-42, C-40 |

**Aspirational subtotal (C-21–C-45):** 25 criteria × 5 = 125 pts of the 190 pt aspirational tier. Criteria C-08 through C-20 (13 criteria × 5 = 65 pts) are stable and inherited from v12 without modification.
...
| Aspirational | C-08–C-45 | 190 |
| **Total** | **45 criteria** | **280** |

**Golden threshold:** all 4 essential PASS AND composite >= 224/280 (80%).

---

## Dependency chain — checker contract stack (C-28 → C-45)
  ...
  EXPECTED-BOUNDARY)       \              C-39 (pre-execution halt boundary) <---+
     ...
     (also depends on C-39                     \                                 |
      ...
      token to cross-reference)                      + HALT-EXPECTED-BOUNDARY ref)
            ...
            |
            +---> C-45 (VALIDATION-DERIVATION inline) <-- C-42, C-40
                  (inline Step 8b derivation: matches flag -> validation outcome)
```
...
**C-44 and C-45 are complementary surfaces on the same consistency chain.** The chain reads: HALT-EXPECTED-BOUNDARY (Phase 0A, C-39) → HALT-EXPECTED-BOUNDARY in HALT-RULE (Phase 0B, C-41) → `matches HALT-EXPECTED-BOUNDARY: [yes/no]` in DECLARE CONTRADICTION (Step 8b, C-42) → TRACE CONTRACT validation outcome (post-execution, C-39) → CONSISTENCY-RULE structural derivation (CHECKER ALGORITHM, C-44) + VALIDATION-DERIVATION inline token (Step 8b, C-45).

C-44 (CONSISTENCY-RULE) encodes the derivation path as a pre-trace named algorithm rule in the CHECKER ALGORITHM block: `matches HALT-EXPECTED-BOUNDARY: yes` → "Contract prediction confirmed"; `matches HALT-EXPECTED-BOUNDARY: no` → "falsified -- actual halt boundary was [BC-N label]". C-45 (VALIDATION-DERIVATION) surfaces the same derivation inline at Step 8b co-located with DECLARE CONTRADICTION at the point the contradiction event fires, making the validation outcome derivable at the Step 8b surface without navigating to the post-execution TRACE CONTRACT validation token. The two surfaces are design choices: C-44 present with C-45 suppressed (V-02 pattern), C-45 present without C-44 (V-01 / V-03 pattern), or both present simultaneously. Both criteria can be independently scored.
```
