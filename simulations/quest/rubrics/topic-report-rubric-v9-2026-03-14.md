`topic-report-rubric-v9-2026-03-14.md` written to `simulations/quest/rubrics/`.

**What was added:**

| ID | Name | Precondition | Mechanism |
|----|------|-------------|-----------|
| **C-28** | Dual-identifier violation example | C-26 | Violation text carries both register label and criterion ID as one compound fragment -- single fragment, both chains reachable -- vs. C-26's two separate chain endpoints at the annotation |
| **C-29** | Explicit named recovery directive in Branch B header | C-25 | Procedural instruction layered on top of structural self-containment: names the recovery action AND the prohibited alternative at Branch B entry |
| **C-30** | Slot-indexed contract register | C-19/C-24 independent | Forward indexing (register -> slot), orthogonal to C-19/C-24 backward chaining (annotation -> register) |

**Formula:** `aspirational_pass/22 * 10`

**R8 ordering under v9:** V-05 (22/22, 100.0) > V-04 (21/22, 99.5) > V-01 = V-02 = V-03 (20/22, 99.1). The three single-axis variations are tied -- C-28, C-29, and C-30 are mutually orthogonal, so their pairwise and triple combinations are the live R9 discriminators.
 COMPLETENESS [SLOT 3]`, `G-1 [PHASE 4]` -- creates a forward-navigation path (register -> slot) orthogonal to C-19/C-24 backward traceability (annotation -> register) |

**Formula:** `aspirational_pass/22 * 10` (was `/19`)

**R8 golden re-scores under v9:**

| Variation | v8 asp | C-28 | C-29 | C-30 | v9 asp | v9 score |
|-----------|--------|------|------|------|--------|---------|
| V-05 Ceiling (A+B+C) | 19/19 | YES | YES | YES | 22/22 | **100.0** |
| V-04 Axes A+B | 19/19 | YES | YES | NO | 21/22 | **99.5** |
| V-01 Axis A | 19/19 | YES | NO | NO | 20/22 | **99.1** |
| V-02 Axis B | 19/19 | NO | YES | NO | 20/22 | **99.1** |
| V-03 Axis C | 19/19 | NO | NO | YES | 20/22 | **99.1** |

The ordering is now: V-05 > V-04 > {V-01 = V-02 = V-03}. V-04 (Axes A+B) pulls ahead of single-axis variations because it satisfies both C-28 (dual-identifier violation) and C-29 (Branch B recovery directive) simultaneously. The single-axis variations are tied at 99.1 -- no v9 criterion differentiates them because C-28, C-29, and C-30 are mutually orthogonal. No R8 variation other than V-05 achieves 22/22.

**Design note on C-28:** C-26 requires both the register-linked label (C-24) and the criterion-tagged violation (C-23) to be co-present at the same `[RULE]` annotation, creating two independent recovery chains accessible from the annotation slot. C-28 is a stronger property at the violation-text level: the violation example itself carries both the register label and the rubric criterion ID as a single compound fragment -- e.g., `(no stall cost -- G-9 INERTIA / C-21 violation)` -- so that a model tracing from any bad output reaches both the contract register and the rubric invariant definition from one text fragment without needing to read the annotation label separately. C-26 guarantees co-presence at the annotation level (two chain endpoints); C-28 guarantees the violation fragment is itself dual-linked (one fragment, both chains reachable). A spec satisfying C-26 may independently choose whether the violation example carries one identifier (C-26 only) or both (C-26 + C-28); C-28 requires C-26 as a precondition but is not implied by it. Round 8 finding: V-01 demonstrates the dual-identifier violation in isolation; V-04 and V-05 carry it alongside C-29. Scored separately from C-26 because the mechanism (single fragment tracing to both chains vs. two separate chain endpoints at the annotation) addresses a different failure mode: split-attention during violation parsing vs. independent recovery path availability.

**Design note on C-29:** C-25 requires the Branch B `=== THREE-LAYER WRITE POINT (BRANCH B) ===` header to enumerate LAYER 1/2/3 independently (structural self-containment without cross-branch reference). C-29 adds a meta-instruction layer: an explicit named recovery directive within the Branch B header that names both the recovery action and the prohibited alternative -- e.g., "re-read this BRANCH B header to reconstruct the required layer sequence without referencing BRANCH A" -- converting structural self-containment into a procedural instruction the model encounters at Branch B entry before encountering any enforcement layer. C-25 makes cross-branch lookup unnecessary; C-29 makes the recovery procedure explicit at the moment the model enters Branch B, before attention can degrade across the LAYER 1/2/3 enumeration. The two properties are additive: C-25 provides the structural precondition; C-29 layers a procedural recovery instruction on top. A spec satisfying C-25 may independently choose whether to include the recovery directive; C-29 requires C-25 as a precondition but is not implied by it. Round 8 finding: V-02 carries the recovery directive; V-04 and V-05 carry it alongside C-28. C-29 is orthogonal to C-28 -- each can be present without the other. Scored separately from C-25 because the mechanism (named recovery action instruction vs. structural self-containment) addresses a different failure mode: attention-recovery after degradation vs. structural independence from Branch A.

**Design note on C-30:** C-19 (bidirectional list invariants) and C-24 (single-position invariants) both chain invariant labels backward from annotation positions to the contract register, enabling recovery from a degraded annotation. C-30 adds forward indexing: each register entry carries a slot annotation naming the output position it governs -- e.g., `G-9 INERTIA [SLOT 4]`, `G-7a COMPLETENESS [SLOT 3]`, `G-7b EXCLUSIVITY [SLOT 3]`, `G-1 [PHASE 4]` -- so that a model reading the register at the start of execution can navigate forward to the governed slot without searching the spec. The direction is reversed from C-19/C-24: backward chaining lets a model recover the register obligation from a degraded annotation; forward indexing lets a model plan its execution path from the register before encountering any annotation. The properties are orthogonal: a spec can satisfy C-19 and C-24 (full backward chaining) without slot indexing, or add slot indexing without satisfying C-19 or C-24. Round 8 finding: V-03 is the first design to carry slot-indexed register entries; the indexing mechanism is directionally orthogonal to C-19/C-24 backward traceability, confirming it is a distinct navigation property. Scored separately from C-19 and C-24 because forward register-to-slot navigation addresses a different failure mode from backward annotation-to-register traceability: a model reading the register to plan execution (forward pass) vs. a model reconstructing the register obligation from a degraded annotation (backward recovery).
...
- **C-19 (contract label chaining from register to annotation to scan header)** is
  aspirational -- inline [RULE] annotations (C-17) and three-layer co-location (C-18)
  make each write-point self-contained, but do not guarantee that the rule names used at
  the write point are recoverable from a top-level contract register. A model whose
  attention degrades mid-generation may correctly apply a rule locally while losing the
  connection to the contract guarantee that motivated it. Assigning named labels at the
  contract register (e.g., G-7a, G-7b) and propagating those exact labels through inline
  [RULE G-7a/G-7b] annotations and scan headers (G-7a COMPLETENESS SCAN / G-7b
  EXCLUSIVITY SCAN) makes each invariant independently recoverable at three structural
  levels -- register, annotation, scan header -- without any cross-position lookup. Round 4
  finding: V-05 is the only R4 design that achieves this chaining; V-04 uses generic
  [RULE COMPLETENESS] / [RULE EXCLUSIVITY] labels that are locally correct but not
  contract-linked. Scored separately from C-17 and C-18 because the mechanism (label
  propagation across structural levels) addresses a different failure mode:
  invariant-context loss rather than mapping distance or phase drift.
- **C-20 (worked example within each `[RULE]` annotation)** is aspirational -- a [RULE]
  ...
  Embedding compact correct/violation examples inside each inline annotation (e.g.,
  "[RULE COMPLETENESS: ... correct: 'Not ready -- missing prove-analysis and
...
- **C-23 (criterion-tagged violation example in `[RULE]` annotation)** is aspirational --
  ...
  example that states only the bad output pattern (e.g., "Run /scout:feasibility
  ...
  encountering the violation knows which invariant is at stake and can trace it back to the
  register and governing rule without a scan. Round 6 finding: V-04 is the first design to
  carry this tag; the tagging pattern applies wherever C-20 is satisfied with a named
  invariant that has a rubric ID. Scored separately from C-20 because the mechanism
  (criterion ID embedded in violation text vs. contrast pair without ID) addresses a
  different failure mode: invariant traceability rather than format demonstration alone.
...
- **C-25 (Branch B independent THREE-LAYER WRITE POINT header)** is aspirational -- C-22
  ...
  own equivalent header. A model executing Branch B must either locate the main-branch
  header across a branch boundary or reconstruct the layer sequence from surrounding spec
  context, both of which require a cross-branch lookup step. A self-contained
  `=== THREE-LAYER WRITE POINT (BRANCH B) ===` header within Branch B enumerates LAYER 1
  DECLARE / LAYER 2 ANCHOR / LAYER 3 VERIFY independently, so a model in Branch B
  reconstructs the enforcement sequence without any cross-branch reference. Round 6
  finding: V-05 is the only R6 design to carry a Branch B independent header; V-02, V-03,
  and V-04 pass C-22 via the main-branch header only. Scored separately from C-22 because
  the mechanism (branch-local recovery point vs. shared header) addresses a different
  failure mode: cross-branch attention loss on the `--format teams` path vs.
  mid-generation layer forgetting on the default path.
- **C-26 (dual-chain co-presence at governed annotation slot)** is aspirational -- C-23
  ...
  criterion-recoverable (model traces from bad output to rubric invariant); C-24 makes the
  invariant register-recoverable (model traces from annotation to contract register). When
  both are co-present at the same `[RULE]` annotation -- the label carries the register
  name (e.g., `[RULE G-9 INERTIA]`) providing the register-recovery path, while the
  violation example appends the rubric criterion ID (e.g., `(no stall cost -- C-21
  violation)`) providing the rubric-recovery path -- the annotation supports two
  independent recovery chains at one structural position without conflict. A spec
  satisfying C-23 alone or C-24 alone does not satisfy this criterion; both must be
  co-present at the same annotation position. Round 7 finding: V-04 and V-05 demonstrate
  co-presence without conflict; V-01 (C-23 only) and V-02 (C-24 only) isolate each chain,
  confirming they are separable but jointly stronger. Scored separately from C-23 and C-24
  because the mechanism (two independent recovery paths at one position) is a composite
  property not implied by either criterion in isolation.
- **C-27 (dual-branch three-layer closure)** is aspirational -- C-22 (main-branch
  THREE-LAYER WRITE POINT header) and C-25 (Branch B independent THREE-LAYER WRITE POINT
  header) each close the attention-recovery gap for one execution path. When both are
  present, the spec achieves complete dual-branch closure: a model executing the default
  path recovers the enforcement layer sequence from the C-22 header; a model executing the
  `--format teams` path recovers the enforcement layer sequence from the C-25 header; no
  cross-branch lookup or cross-spec search is required at any point on either path. The
  property is conjunctive, not additive: either header alone leaves one execution path
  exposed. A spec satisfying C-22 without C-25, or C-25 without C-22, does not satisfy
  this criterion. Round 7 finding: V-05 is the only R7 design achieving dual-branch
  closure; V-03 satisfies C-25 and passes C-22 via the neutral base, making it the minimal
  R7 design to achieve C-27 without also achieving C-26. Scored separately from C-22 and
  C-25 because complete dual-branch self-containment (neither path exposes a cross-branch
  dependency) is a system-level property not implied by either header criterion in
  isolation.
- **C-28 (dual-identifier violation example)** is aspirational -- C-26 requires both the
  ...
  both chains reachable). A spec satisfying C-26 may independently choose whether the
  violation example carries one identifier (C-26 only) or both (C-26 + C-28); C-28
  requires C-26 as a precondition but is not implied by it. Round 8 finding: V-01
  demonstrates the dual-identifier violation in isolation; V-04 and V-05 carry it
  alongside C-29. Scored separately from C-26 because the mechanism (single fragment
  tracing to both chains vs. two separate chain endpoints at the annotation) addresses a
  different failure mode: split-attention during violation parsing vs. independent recovery
  path availability.
- **C-29 (explicit named recovery directive in Branch B header)** is aspirational -- C-25
  requires the Branch B `=== THREE-LAYER WRITE POINT (BRANCH B) ===` header to enumerate
  LAYER 1/2/3 independently (structural self-containment without cross-branch reference).
  C-29 adds a meta-instruction layer: an explicit named recovery directive within the
  Branch B header that names both the recovery action and the prohibited alternative --
  e.g., "re-read this BRANCH B header to reconstruct the required layer sequence without
  referencing BRANCH A" -- converting structural self-containment into a procedural
  instruction the model encounters at Branch B entry before encountering any enforcement
  layer. C-25 makes cross-branch lookup unnecessary; C-29 makes the recovery procedure
  explicit at the moment the model enters Branch B, before attention can degrade across
  the LAYER 1/2/3 enumeration. The two properties are additive: C-25 provides the
  structural precondition; C-29 layers a procedural recovery instruction on top. A spec
  satisfying C-25 may independently choose whether to include the recovery directive;
  C-29 requires C-25 as a precondition but is not implied by it. Round 8 finding: V-02
  carries the recovery directive; V-04 and V-05 carry it alongside C-28. C-29 is
  orthogonal to C-28 -- each can be present without the other. Scored separately from C-25
  because the mechanism (named recovery action instruction vs. structural self-containment)
  addresses a different failure mode: attention-recovery after degradation vs. structural
  independence from Branch A.
- **C-30 (slot-indexed contract register)** is aspirational -- C-19 (bidirectional list
  invariants) and C-24 (single-position invariants) both chain invariant labels backward
  from annotation positions to the contract register, enabling recovery from a degraded
  annotation. C-30 adds forward indexing: each register entry carries a slot annotation
  naming the output position it governs -- e.g., `G-9 INERTIA [SLOT 4]`,
  `G-7a COMPLETENESS [SLOT 3]`, `G-7b EXCLUSIVITY [SLOT 3]`, `G-1 [PHASE 4]` -- so that
  a model reading the register at the start of execution can navigate forward to the
  governed slot without searching the spec. The direction is reversed from C-19/C-24:
  backward chaining lets a model recover the register obligation from a degraded
  annotation; forward indexing lets a model plan its execution path from the register
  before encountering any annotation. The properties are orthogonal: a spec can satisfy
  C-19 and C-24 (full backward chaining) without slot indexing, or add slot indexing
  without satisfying C-19 or C-24. Round 8 finding: V-03 is the first design to carry
  slot-indexed register entries; the indexing mechanism is directionally orthogonal to
  C-19/C-24 backward traceability, confirming it is a distinct navigation property.
  Scored separately from C-19 and C-24 because forward register-to-slot navigation
  addresses a different failure mode from backward annotation-to-register traceability:
  a model reading the register to plan execution (forward pass) vs. a model reconstructing
  the register obligation from a degraded annotation (backward recovery).
...
| C-27 | **Dual-branch three-layer closure** | format | aspirational | Both C-22 (main-branch `=== THREE-LAYER WRITE POINT ===` header) and C-25 (Branch B `=== THREE-LAYER WRITE POINT (BRANCH B) ===` header) are present, so that neither execution path requires cross-branch lookup or cross-spec search to reconstruct the enforcement layer sequence at any point during generation; the property is conjunctive -- either header alone leaves one execution path exposed; satisfying C-22 without C-25, or C-25 without C-22, does not satisfy this criterion; C-27 requires both C-22 and C-25 as preconditions |
| C-28 | **Dual-identifier violation example** | depth | aspirational | Within the `[RULE]` annotation at SLOT 4 (NEXT STEP), the violation example carries both the contract register label and the rubric criterion ID as a single compound fragment -- e.g., `(no stall cost -- G-9 INERTIA / C-21 violation)` -- so that a model tracing from any bad next-step output reaches both the register obligation and the rubric invariant definition from one text fragment without splitting attention between the annotation label and the violation text; a violation example that carries only one identifier (criterion ID alone or register label alone) does not satisfy this criterion; C-28 requires C-26 as a precondition (both chains must be co-present at the annotation) but is not implied by it |
...
aspirational_pass = count of C-09..C-30 that pass   (max 22)
...
- C-23 (criterion-tagged violation) requires C-20 as a precondition -- a spec cannot carry
  ...
  Round 6 finding: V-04 is the only R6 design with a criterion-tagged violation; the
  pattern applies wherever C-20 is satisfied with a named invariant that has a rubric ID.
...
- C-26 (dual-chain co-presence) requires C-23 and C-24 as preconditions. Both must be
  ...
  a different slot does not satisfy C-26. Round 7 finding: V-04 and V-05 demonstrate
  co-presence without structural conflict; the composite property adds non-trivial
  discriminative power beyond either criterion alone.
- C-27 (dual-branch closure) requires C-22 and C-25 as preconditions. Both must be
  present for the conjunctive property to hold. The property is system-level: it describes
  the absence of any cross-branch dependency across all execution paths, not merely the
  presence of two headers. Round 7 finding: V-05 achieves full dual-branch closure; V-03
  satisfies C-25 and passes C-22 via the neutral base, making it the minimal R7 design to
  achieve C-27 while failing C-26.
- C-28 (dual-identifier violation) requires C-26 as a precondition -- both chain endpoints
  must already be co-present at the annotation before the violation fragment can carry both
  identifiers. C-28 is not implied by C-26: any spec satisfying C-26 may carry a violation
  example with only one identifier. Round 8 finding: V-01 demonstrates C-28 in isolation
  (Axis A only); V-04 and V-05 carry it alongside C-29.
- C-29 (Branch B recovery directive) requires C-25 as a precondition -- the Branch B
  header must already enumerate LAYER 1/2/3 before a recovery directive can be layered on
  top. C-29 is not implied by C-25: any spec satisfying C-25 may carry the header without
  an explicit recovery directive. C-29 is orthogonal to C-28: neither implies the other.
  Round 8 finding: V-02 demonstrates C-29 in isolation (Axis B only); V-04 and V-05 carry
  it alongside C-28.
- C-30 (slot-indexed register) is orthogonal to C-19 and C-24 -- forward indexing
  (register to slot) and backward chaining (annotation to register) are independent
  navigation properties. A spec can satisfy C-19 + C-24 (full backward chaining) without
  slot indexing, or add slot indexing without satisfying C-19 or C-24, or satisfy all
  three. Round 8 finding: V-03 demonstrates C-30 in isolation (Axis C only); V-05 carries
  all three axes.
- Under v9 the R8 single-axis variations (V-01, V-02, V-03) are tied at 20/22 = 99.1;
  V-04 (Axes A+B) rises to 21/22 = 99.5; V-05 (Axes A+B+C) achieves 22/22 = 100.0.
  The single-axis variations are not further differentiated by any v9 criterion -- C-28,
  C-29, and C-30 are orthogonal, making their pairwise and triple combinations the live
  R9 discriminators.

---

## R9 Discriminator Candidates

| ID | Design question | V-isolation needed |
|----|-----------------|-------------------|
| C-28 x C-29 composite | Does the combination of dual-identifier violation (C-28) and Branch B recovery directive (C-29) achieve a system property not implied by either alone -- specifically, does dual-identifier at SLOT 4 reduce violation-parsing confusion on both execution paths when the Branch B path also carries an explicit recovery instruction? | V-A: C-28 only; V-B: C-29 only; V-C: C-28 + C-29 (= V-04 pattern); measure cross-path violation-parsing error rates independently |
| C-28 x C-30 composite | Does dual-identifier violation (C-28, annotation-level) combined with slot-indexed register (C-30, register-level) create a closed navigation loop -- register -> slot -> annotation -> violation -> register -- not achievable by either mechanism alone? | V-D: C-28 only; V-E: C-30 only; V-F: C-28 + C-30; measure round-trip traceability from register to bad output and back |
| C-29 x C-30 composite | Does Branch B recovery directive (C-29, procedural) combined with slot-indexed register (C-30, structural forward index) jointly reduce Branch B entry errors -- model enters Branch B, reads register forward index to find SLOT 4, then has explicit recovery instruction if attention degrades -- beyond either mechanism alone? | V-G: C-29 only; V-H: C-30 only; V-I: C-29 + C-30; measure Branch B SLOT 4 error rate |
| C-29 analogue for main branch | Does adding an explicit recovery directive to the main-branch C-22 header -- e.g., "re-read this header to reconstruct the layer sequence" -- reduce LAYER omission rates on the default execution path, mirroring the benefit C-29 provides for Branch B? | V-J: C-22 without recovery directive (current baseline); V-K: C-22 with explicit recovery directive (main-branch C-29 analogue); compare live-run LAYER omission rates on default vs. teams path |
| Slot index granularity | Does branch-qualified slot indexing -- e.g., `G-9 INERTIA [SLOT 4 / BRANCH A]` and `G-9 INERTIA [SLOT 4 / BRANCH B]` as separate entries -- reduce forward-navigation ambiguity beyond coarse slot indexing (C-30's `[SLOT 4]`) when the same invariant governs equivalent positions on both execution paths? | V-L: C-30 with coarse slot index; V-M: C-30 with branch-qualified slot index; measure slot-navigation error rate on Teams card path vs. default path independently |
