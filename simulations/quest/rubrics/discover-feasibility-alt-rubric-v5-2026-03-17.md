Written to `simulations/quest/rubrics/discover-feasibility-alt-rubric-v5-2026-03-17.md`.

---

**One new criterion extracted from R4 excellence signals:**

| ID | Source | Upgrades | Core distinction |
|----|--------|----------|-----------------|
| C-19 | V-05 R4 Stated Effect variable binding | C-15, C-18 | Stated Effect cells in the propagation contract table reference the exact formula variable (`focus_adjusted_total`) by name — creating a closed contract-to-formula loop at the token level. Eliminates prose-drift: the table column becomes a lexical binding between routing contract and arithmetic formula. |

**Key design decisions:**
- C-19 has two prerequisites (C-15 PASS + C-16 PASS) before it can be scored — captured in the evaluation note
- The pass test is **lexical, not semantic**: the variable name must appear in both locations (table cell and formula section); prose descriptions of the same effect do not satisfy it
- FAIL (not PARTIAL) when the binary token-binding condition is unmet, consistent with C-16/C-17/C-18 pattern
- Aspirational denominator `/10` → `/11`; all-pass composite remains 100
- V-04 and V-05 R4 both scored 100 under v4; under v5, V-04 would score 99.09 (C-19 absent) and V-05 would remain 100.000
sponding RED Blockers entry with all three sub-fields: blocker statement, Lift condition, and Do-nothing cost. "No RED components." is acceptable iff no RED rows exist. |
