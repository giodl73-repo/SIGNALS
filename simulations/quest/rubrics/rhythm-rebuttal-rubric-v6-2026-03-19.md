...
# Rubric: rhythm-rebuttal (v6)

## Change Log

### v6 (2026-03-19) — R5 excellence patterns

**Formula update**: aspirational denominator changes from `/14` to `/16` (14 criteria → 16 criteria).

**Two new aspirational criteria**:

| ID | Source | What it captures |
|----|--------|-----------------|
| C-23 | Pattern 1 `P1 concern-level pre-commitment as position stability audit` | Phase 2 P1-NC block requires the author to pre-declare each anticipated P1 no-change position with a named valid evidence form before decomposition begins — individual concern-level commitments rather than a narrative-level cover letter; Phase 6a audits HELD vs SHIFTED outcomes, making drafting pressure visible as detectable outcome data; a SHIFTED pre-commitment is not a failure but becomes a named event rather than a silent position change |
| C-24 | Pattern 2 `count-based structural gate as pre-write integrity verification` | Phase 7 runs five mechanical checks after Phase 6 and before artifact write: (1) REVIEWER SAID count = R-ID count, (2) all MANUSCRIPT OUTCOMEs resolved, (3) cover letter paragraph structure verified, (4) all Phase 4 FLAGGED R-IDs have validated Phase 6b outcomes, (5) frontmatter integers computed from Phase 5 outcomes; the gate catches silent omissions that satisfy every prior criterion but would fail at editorial submission — a category of failure that no earlier phase can detect |
...
| Aspirational (C-09–C-24) | 10 | (passed / **16**) * 10 |

Golden threshold: >= 90 AND all essential pass.

---

The two additions are structurally distinct from everything preceding them:

- **C-23** operates at *concern level* before decomposition — C-12 commits the narrative cover letter, C-23 commits individual positions. The HELD/SHIFTED audit makes position drift a logged event rather than invisible pressure.
- **C-24** operates *after all phases* — nothing in C-01 through C-23 touches the post-Phase-6 window. The five-check gate is the only criterion that can catch omissions that pass every phase but fail at submission.

File written to `simulations/quest/rubrics/rhythm-rebuttal-rubric-v6-2026-03-19.md`.
sclassification propagates to the forecast table, exchange, or cover letter |
...
Denominator: 16. Score = (criteria passed / 16) * 10.
...
| **C-23** | **P1 concern-level pre-commitment block with HELD/SHIFTED audit** | **depth** | **A Phase 2 P1-NC block requires the author to pre-declare each anticipated P1 no-change position with a named valid evidence form before decomposition begins — targeting individual concern-level commitments rather than the narrative-level cover letter (C-12). Phase 6a audits each pre-committed position as HELD or SHIFTED, making drafting pressure visible as detectable outcome data; a SHIFTED outcome is not scored as a failure but is a named, logged event rather than a silent position change. Pass if the P1-NC pre-commitment block appears in Phase 2 (before Phase 3), names a valid evidence form per anticipated no-change concern, and Phase 6a contains an explicit HELD/SHIFTED audit. Distinct from C-12 (cover letter before decomposition — narrative commitment) — C-23 is concern-level position commitment with outcome tracking.** |
| **C-24** | **Phase 7 count-based structural gate before artifact write** | **format** | **A Phase 7 step runs five mechanical checks after Phase 6 and before the artifact is written: (1) REVIEWER SAID count equals R-ID count, (2) all MANUSCRIPT OUTCOMEs are resolved (not placeholder), (3) cover letter paragraph structure verified (two paragraphs present), (4) all Phase 4 FLAGGED R-IDs have corresponding validated Phase 6b outcomes, (5) frontmatter integers computed directly from Phase 5 outcomes rather than estimated. The gate catches silent omissions — missing exchanges, unresolved outcomes, unvalidated flags, integer mismatches — that satisfy every prior criterion but would fail at editorial submission. Pass if all five checks are present as explicit numbered steps in Phase 7 and Phase 7 appears after Phase 6 and before the write instruction. Nothing in C-01 through C-23 covers post-Phase-6 verification — C-24 is the only criterion that targets this failure category.** |
```
