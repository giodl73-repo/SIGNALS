...
Source: V-01 R5. The ROLE: line in each stage cites the specific `orientation.frame` field extracted from `.craft/roles/`, not just the role name. Makes role loading verifiable at stage level, independent of per-finding Via: fields. C-02 asks "does the review reflect the role?"; C-18 asks "is the governing frame named so the claim is checkable?"
