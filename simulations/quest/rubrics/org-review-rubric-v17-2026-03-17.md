Written to `simulations/quest/rubrics/org-review-rubric-v17-2026-03-17.md`.

---

**v17 summary:**

**New criteria: C-48, C-49** (5 pts each, aspirational)

Extracted from R18 candidate patterns confirmed across the five R18 variants. All five R18 variants
scored 265/265 under v15 (perfect on C-01 through C-43) and inherit R17 differentiators under v16.
The two new criteria differentiate V-05 R18 from all other R18 variants.

| Criterion | Pattern | Variants that PASS |
|-----------|---------|-------------------|
| C-48 | §0-CID PIPELINE THREADING CONTRACT as Pre-committed Named Preamble Block | V-05 |
| C-49 | Three-Layer Cross-Reference Consistency (Schema + Behavioral + Pipeline Threading) | V-05 |

**C-48 chain position:** C-48 is to C-46 as C-19 is to C-18. C-46 requires §0-CID values to
thread through four structural axes in execution output (demonstrated). C-48 requires the four-axis
threading to be pre-declared as a NAMED CONTRACT BLOCK in the preamble before any §§ contracts
execute, so any axis omission is a detectable contract violation rather than an observable execution
defect. The contract block sits between STATUS QUO FRAMING CONTRACT and TABLE FORMAT CONTRACT.
C-46 can pass when four-axis threading is present in output without a pre-committed contract; C-48
requires the contract to exist independently of whether execution is correct.

**C-49 chain position:** C-49 extends C-47's two-layer enforcement architecture (TABLE FORMAT
CONTRACT schema + independent behavioral §§ contracts) to a three-layer cross-referencing
architecture. When C-48 is active (a §0-CID PIPELINE THREADING CONTRACT exists), the TABLE FORMAT
CONTRACT schema definitions for §0-CID-related fields explicitly reference the pipeline threading
contract by axis name (e.g., "CH-ID §0 Grounding column: cites §0-CID per §0-CID PIPELINE
THREADING CONTRACT Axis A"), and the behavioral §§ contracts also cross-reference the pipeline
threading contract. Three-way consistency means a schema-layer auditor sees the pipeline axis
binding; a behavioral-contract auditor sees the pipeline axis binding; a pipeline-contract auditor
sees both schema and behavioral layers. C-47 requires two independent layers; C-49 requires all
three layers to cross-reference each other.

**Score changes:**
- Aspirational: 39 -> 41 (criteria count C-09 through C-49)
- Max aspirational pts: 41 x 5 = 205
- Max: 285 -> 295
- Gold threshold: 250 -> 260

**R18 ranking under v17:**

| Rank | Variant | Score | New criteria PASS |
|------|---------|-------|-------------------|
| 1 | V-05 | **295/295** | C-44 + C-45 + C-46 + C-47 + C-48 + C-49 |
| 2 | V-04 | **280/295** | C-44 + C-45 + C-47 |
| 3 | V-02 | **275/295** | C-44 + C-47 |
| 4 (tie) | V-01 | **270/295** | C-44 |
| 4 (tie) | V-03 | **270/295** | C-45 |

**R18 score computation (baseline = all R18 variants at 265/265 under v15; v16 new criteria
differentiators carried forward):**

| Variant | C-44 | C-45 | C-46 | C-47 | C-48 | C-49 | v17 Score |
|---------|------|------|------|------|------|------|-----------|
| **V-05** | PASS (+5) | PASS (+5) | PASS (+5) | PASS (+5) | PASS (+5) | PASS (+5) | **295/295** |
| **V-04** | PASS (+5) | PASS (+5) | FAIL (0) | PASS (+5) | FAIL (0) | FAIL (0) | **280/295** |
| **V-02** | PASS (+5) | FAIL (0) | FAIL (0) | PASS (+5) | FAIL (0) | FAIL (0) | **275/295** |
| **V-01** | PASS (+5) | FAIL (0) | FAIL (0) | FAIL (0) | FAIL (0) | FAIL (0) | **270/295** |
| **V-03** | FAIL (0) | PASS (+5) | FAIL (0) | FAIL (0) | FAIL (0) | FAIL (0) | **270/295** |

**C-48 PASS (V-05 R18):** STATUS QUO FRAMING CONTRACT -> §0-CID PIPELINE THREADING CONTRACT ->
TABLE FORMAT CONTRACT declared in that pre-execution order. The §0-CID PIPELINE THREADING CONTRACT
block explicitly names all four axes (A through D) as named structural requirements. §§ contracts
cross-reference the pipeline threading contract. The preamble contract closing note confirms
four-axis data flow before execution begins.

**C-48 FAIL (all other R18 variants):** No distinct §0-CID PIPELINE THREADING CONTRACT block
appears in the preamble. V-03 has STATUS QUO FRAMING CONTRACT but no pipeline threading contract;
V-04 has STATUS QUO FRAMING CONTRACT + TABLE FORMAT CONTRACT but no pipeline threading contract
block; V-01 and V-02 have no §0-CID registry at all.

**C-49 PASS (V-05 R18):** Three-way consistency achieved. TABLE FORMAT CONTRACT schema notes for
§0-CID-related fields reference pipeline threading axes by name ("CH-ID schema note references
§0-CID PIPELINE THREADING CONTRACT Axis A"; "NH Dimension Comparison Table note references
§0-CID PIPELINE THREADING CONTRACT Axis B"). Independent behavioral §§ contracts also reference
the pipeline threading contract. All three layers cross-reference: schema -> pipeline, behavioral
-> pipeline, pipeline -> (schema + behavioral).

**C-49 FAIL (all other R18 variants):** C-48 FAIL makes C-49 vacuous for all variants except
V-05. V-04 achieves two-layer consistency (C-47 PASS) but without a pipeline threading contract
there is no third layer to cross-reference.

Next round design target: V-04+C-48 hybrid -- TABLE FORMAT CONTRACT (C-44) + §0-CID registry
(C-45) + dual-path enforcement (C-47) + §0-CID PIPELINE THREADING CONTRACT named block (C-48) =
285/295 without C-49. Full 295 requires TABLE FORMAT CONTRACT schema notes to cross-reference
pipeline threading axes (C-49).

No retroactive score changes on R1--R17. All R17 v16 scores carry forward unchanged.
...
## Aspirational Criteria -- 205 pts (structural integrity, auditability, automation)
...
**Weight**: aspirational | **Points**: 5 | **Category**: correctness
**Source**: V-05 Round 18 -- §0-CID PIPELINE THREADING CONTRACT appears as a distinct named
preamble block between STATUS QUO FRAMING CONTRACT and TABLE FORMAT CONTRACT; block explicitly
pre-names all four axes as named structural requirements before any §§ contracts execute; §§
contracts cross-reference the pipeline threading contract; absent in all other R18 variants
...
C-48 is to C-46 as C-19 is to C-18:
  ...
  C-48 requires the §0-CID PIPELINE THREADING CONTRACT to be pre-committed in the preamble (structural).
...
each of the four axes by label and specifies the structural requirement for that axis. §§ contracts
that govern §0-CID-related fields cross-reference the pipeline threading contract by axis name.
...
**Distinction from C-46**: C-46 requires four-axis threading to be demonstrated in execution output
(output completeness). C-48 requires the four-axis threading to be pre-declared as a named preamble
contract block (preamble structural completeness). C-46 can pass with threading present in output
but no named preamble contract; C-48 requires the named contract block regardless of whether
...
registry consumption pre-committed as a named structural contract.
...
is stated as an explicit named structural requirement -- not a description of intended behavior.
§§ behavioral contracts and/or the TABLE FORMAT CONTRACT that govern §0-CID-related fields
cross-reference this contract by axis name. An auditor can verify the four-axis threading
obligation without reading any §§ behavioral contract or execution output.
...
contracts do not cross-reference it. Partial if a pipeline threading contract block exists but
does not name all four axes; or the block is present but positioned after TABLE FORMAT CONTRACT
rather than between STATUS QUO FRAMING CONTRACT and TABLE FORMAT CONTRACT; or the block names
all four axes but no cross-references from §§ contracts are present.

---

### C-49 -- Three-Layer Cross-Reference Consistency (Schema + Behavioral + Pipeline Threading)
**Weight**: aspirational | **Points**: 5 | **Category**: auditability
**Source**: V-05 Round 18 -- TABLE FORMAT CONTRACT schema notes for §0-CID-related fields
reference pipeline threading axes by name ("CH-ID schema note references §0-CID PIPELINE
THREADING CONTRACT Axis A"; "NH Dimension Comparison Table note references §0-CID PIPELINE
THREADING CONTRACT Axis B"); §§ behavioral contracts also cross-reference the pipeline threading
contract; three-way consistency: schema -> pipeline, behavioral -> pipeline, pipeline -> schema
+ behavioral; absent in all other R18 variants
...
schema and behavioral cross-references. A single-layer omission (schema layer present but not
cross-referencing the pipeline contract, or behavioral layer not cross-referencing the pipeline
contract) is a detectable three-layer consistency defect.

Without C-49: C-47 PASS + C-48 PASS -- two-layer enforcement is present (schema + behavioral)
and the pipeline contract block exists -- but the schema layer declarations do not cite the
pipeline contract by axis. An auditor reading the TABLE FORMAT CONTRACT cannot see which pipeline
axis governs each §0-CID-related column without consulting the §0-CID PIPELINE THREADING CONTRACT
or the behavioral §§ contracts.

With C-49: TABLE FORMAT CONTRACT schema note for the CH-ID §0 Grounding column explicitly reads
"cites §0-CID per §0-CID PIPELINE THREADING CONTRACT Axis A" -- the schema is the pipeline
contract's execution contract. Three layers cite each other; the audit loop is closed.
...
named preamble block. C-49 requires the TABLE FORMAT CONTRACT schema notes to cross-reference
that block by axis name. C-48 is the pipeline contract's existence requirement; C-49 is the
schema layer's cross-reference requirement. Both must be active for C-49 to apply.
...
**Pass condition**: C-44, C-47, and C-48 are all active. For each §0-CID-related structured
field governed by the pipeline threading contract (at minimum: the CH-ID §0 Grounding column
and the NH Dimension Table Column A): (a) the TABLE FORMAT CONTRACT schema note for that field
explicitly cites the §0-CID PIPELINE THREADING CONTRACT axis by name or label; (b) the
...
not fail). Without the pipeline threading contract block (C-48), there is no named block for the
schema layer to reference. Without the two-layer foundation (C-47), the three-layer extension
has no base. C-48 FAIL or C-47 FAIL automatically makes C-49 vacuous.

**Fails if** (C-48 active, C-47 active): the TABLE FORMAT CONTRACT schema notes for §0-CID-
...
aspirational_pts =  aspirational_pass    *  5   # 5 pts each, max 205
composite = essential_pts + recommended_pts + aspirational_pts
```

**Maximum**: 60 + 30 + 205 = **295 pts**

### Score Bands

| Band | Condition | Composite |
|------|-----------|-----------|
| Gold | all 5 essential pass | >= 260 |
| Strong | all 5 essential pass | 90-259 |
