Written to `simulations/quest/rubrics/review-design-rubric-v8-2026-03-14.md`.
...
- Confirmed by: V-02/V-03 scored 100 without F-16; V-05 introduced it
- Pattern: extends the identity-integrity chain (F-10: BLOCK 1->1.5; F-15: 1.5->BLOCK 3) to BLOCK 4 unique catches -- the one downstream block that names reviewers without a verification halt; `none` token explicitly exempt

**C-25 | BLOCK 5 Re-run Scope Reviewer Identity Verification**
- Confirmed by: V-01/V-03 scored 100 without F-17; V-05 introduced it
- Pattern: closes the identity chain at its final link -- Re-run Scope names amend targets but had no halt verifying they exist in the committed roster; F-17 completes the four-leg chain (F-10, F-15, F-16, F-17) with no lifecycle gap remaining

**C-26 | BLOCK 0 Signal Disposition Completeness (Bidirectional Gate)**
- Confirmed by: V-01/V-02/V-04 scored 100 without F-18; V-05 introduced it
- Pattern: introduces the sixth enforcement class (catalogue disposition completeness) -- F-13 enforced only the inbound direction (BLOCK 1 draws from BLOCK 0); F-18 closes the outbound direction (every BLOCK 0 signal must be resolved or explicitly dispositioned); F-13 + F-18 = first bidirectional block contract

**Formula:** denominator 16 → 19. Each aspirational criterion now worth ~0.526 pts. Score ceiling 100 preserved.
lection is traceable to a signal in the input design (e.g., "RBAC mentioned -> security
  architect added").
...
Note: aspirational denominator changed from 16 to 19 in v8 (added C-24, C-25, C-26).
Each aspirational criterion contributes ~0.526 points at full pass.
