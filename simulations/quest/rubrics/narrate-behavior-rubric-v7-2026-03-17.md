Written to `simulations/quest/rubrics/narrate-behavior-rubric-v7-2026-03-17.md`.

---

**What changed — summary:**

**3 new criteria extracted from R6 scorecard:**

| Criterion | Pattern | Source | Key rule |
|-----------|---------|--------|----------|
| C-23 -- Unconditional Discovery-Pathway-Ratio REQUIRE | `unconditional-discovery-pathway-ratio-require` | SIGNAL-1, V-27, V-29, V-30 | All three ratio directions (inertia > contract / contract > inertia / equal) must produce a required meta-finding statement; equality is also a signal |
| C-24 -- Exhaustive Inertia-Disposition Completeness Check | `exhaustive-inertia-disposition-completeness-check` | SIGNAL-2, V-30 | REQUIRE check that every named INERTIA assumption carries one of four dispositions (FINDING / COVERED / INVESTIGATION / NONE); completeness verified at aggregate level |
| C-25 -- Inertia-Disposition Coverage Score | `inertia-disposition-ratio-as-coverage-score` | SIGNAL-3, V-30 | REPORT computes % FINDING / % COVERED / % INVESTIGATION density score across all inertia assumptions; distinct from C-23 registry-type ratio |

**1 open question resolved (not codified):** unconditional-meta-finding-additive-to-audit-table -- V-27's unconditional REQUIRE exceeds C-22 rather than replacing it. Noted in C-22 pass condition.

**Tier rebalancing:** Aspirational 15 -> 18. Threshold 8 -> 10. Cap unchanged at 10 pts.

---

**Design notes on the three new criteria:**

- **C-23** targets the DEFINITIONS + REPORT REQUIRE pairing: the ratio rule must be declared in DEFINITIONS (three explicit forms) and enforced in REPORT REQUIRE ("required regardless of ratio direction"). A variation with only a conditional REPORT observation fails even if C-22 passes.

- **C-24** introduces a four-outcome disposition system. COVERED and INVESTIGATION are the new cases: COVERED closes C-09 zero-finding rationale cleanly; INVESTIGATION carves out ambiguity without claiming a spec-gap (preserving C-14 precision). The REQUIRE check is aggregate -- completeness over all INERTIA blocks, not section by section.

- **C-25** is the density score layer on top of C-24's exhaustive assignment. Requires percentages + interpretation statement in REPORT. Explicitly scoped away from C-23 (discovery pathway ratio) and C-22 (audit table) -- three orthogonal views of the same inertia data.
eclared order. Reordering sub-skills mid-execution without updating the declaration
  is a violation.
...
## Aspirational Criteria (10 points total, capped; any 10 of 18 earns the cap)
...
- **Text**: The variation promotes the discovery-pathway-ratio observation from a conditional
  ...
  block (confirming the meta-finding statement is present).
- **Pass condition**: The variation's DEFINITIONS section declares three explicit ratio forms
  ...
  DEFINITIONS-level declaration. Partial pass (half credit) if the unconditional form is
  declared in DEFINITIONS but the REPORT REQUIRE check is absent.
