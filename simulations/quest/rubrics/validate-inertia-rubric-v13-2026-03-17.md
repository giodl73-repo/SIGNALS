`validate-inertia-rubric-v13-2026-03-17.md` written.

**Changes from v12:**

**New criterion: C-40 — Per-axis explicit gate for three-axis architecture block**

Source: V-60 from R12. The V-60 pattern adds a gate to the three-axis block (C-37/C-38/C-39) that names each axis by label and declares two-of-three a gate failure. All four ceiling variations (V-57--V-60) tied at 390/390 on C-38+C-39; C-40 captures the enforcement advantage V-60 demonstrated -- converting partial-axis omission from detectable-at-analysis to structurally impossible.

Pass condition: gate must enumerate all three axes by name AND declare partial satisfaction a gate failure. A gate naming fewer than three axes earns PARTIAL. A gate declaring partial invalid without naming individual axes earns PARTIAL.

**Scoring delta:** 390 -> 400 pts max. 31 -> 32 aspirational. Aspirational subtotal: 320 pts.

**No criterion text changes:** The two other R12 patterns (prohibition form as sole separator, minimal footprint sufficient) confirm C-38/C-39 pass conditions already encoded; no edits required.
n text changes required.

**Scoring:** 400 pts max (was 390). 32 aspirational (was 31). Aspirational subtotal: 320 pts.
...
- **32 aspirational (C-09--C-40):** scoring methodology transparency, time-dependent inertia
...
10 pts each (320 total) = **400 pts max**.
...
- C-40 codifies the per-axis explicit gate pattern from R12: V-60 extended the three-axis block
  ...
  C-37/C-38/C-39 can all pass without C-40; C-40 requires the gate itself to be the
  impossibility mechanism, not merely a completion prompt.
