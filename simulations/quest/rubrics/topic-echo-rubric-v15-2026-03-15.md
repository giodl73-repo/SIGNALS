Written to `simulations/quest/rubrics/topic-echo-rubric-v15-2026-03-15.md`.

---

## What changed -- v14 -> v15

Three new aspirational criteria from R14 V-05 excellence signals:

**C-50 -- Per-site dependency graph entries for same-token multi-site consumption**
Source: R14 V-05 excellence signal 1. BACK-FILL-VERDICT (NODE 14) appeared in *two* separate named dep graph entries -- one for the C-46 header dep and one for the C-48 authority block citation. Same token consumed at two structurally distinct sites (step header vs. authority block body) produces two separate entries, not one. Prior rounds treated a token's dep loops as a single entry. C-50 requires per-site granularity when consumption is structurally distinct. Distinct from C-31 (all deps in graph); this governs entry granularity.

**C-51 -- Precondition citation precedes compliance footprint within shared block**
Source: R14 V-05 excellence signal 2. When C-48 (precondition citation) and C-47 (satisfied-criteria enumeration) coexist in the same authority block, the precondition citation is LINE 1 -- before the identity contract. Precondition established before compliance footprint declared: the enumeration can truthfully list C-43 knowing the PASS was already in-hand. Distinct from C-48 (which governs citation before non-vacuity count); this governs C-48 vs. C-47 ordering when they share a block.

**C-52 -- Version-aligned closing tag is self-inventorying**
Source: R14 V-05 excellence signal 3. The closing tag `[C-46 ...; C-47 ...; C-48 ...; C-49 version-aligned tag]` includes C-49 *as an entry in its own tag*. The tag's existence and inclusion of C-49 proves C-49 is satisfied by its own presence -- self-verifying. Converts the closing tag from a passive list into a self-referential structural closure. Distinct from C-49 (which requires all new criteria to be named); this requires the tag to include itself.

**Scoring:** 60 + 21 + 132 = **213** (up from 204). Aspirational band 41 -> 44 criteria (max 123 -> 132 pts).
from a passive
list of other criteria into a self-referential structural closure. Distinct from C-49
(which requires the tag to name all new criteria); this requires the tag to include
itself as one of those entries.

**Scoring:** 60 + 21 + 132 = **213** (up from 204). Aspirational band 41 -> 44 criteria
(max 123 -> 132 pts).
