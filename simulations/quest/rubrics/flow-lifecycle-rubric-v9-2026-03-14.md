`flow-lifecycle-rubric-v9-2026-03-14.md` written.

---

**3 new criteria added from Round 8 excellence signals:**

| ID | Criterion | Category | Signal source |
|----|-----------|----------|---------------|
| C-31 | Scaffold Gate Forward-Reference Structural Position | format | V-02 C-29 evidence: "declared below" scopes the gate by document order; C-29 requires gate existence, C-31 requires pre-block position + forward-reference language |
| C-32 | Dual-Location Consolidated Block Canonical Axis Citation | format | V-02 C-30 evidence: consolidated block cited `` `Required format:` ``/`` `Fail condition:` `` by exact canonical labels, not paraphrases; mirrors C-18's V-03/V-02 distinction (content names vs. ordinal labels) |
| C-33 | Preamble Concrete Example B-ID Alignment | depth | V-02 C-24+C-28 joint evidence: preamble example uses B-01, first per-block uses B-01 — string-comparison detectability extended to the preamble boundary; C-33 is to C-24 what C-28 is to C-27 |

**Formula**: `/22` → `/25`; max composite unchanged.

**Dependency chain additions:**
- `C-31→C-29` (depth-3 from C-27; positional gate escalation)
- `C-32→C-30` (depth-3 from C-24+C-27; label fidelity escalation)
- `C-33→C-28+C-24` (dual-root; preamble-to-per-block instantiation alignment)
9 with "SCAFFOLD REQUIREMENT: ALL B-NN blocks declared below must carry
an Evidence field". The "declared below" is forward-reference language: it scopes the
gate to B-NN blocks following it in document order, making the prerequisite constraint
positionally verifiable. C-29 requires the gate to exist; C-31 requires the gate to
appear before the first B-NN block header in document order AND to use forward-reference
language establishing that scope. Without positional precedence the scaffold gate is an
instruction without a verifiable scope boundary; with "declared below" the domain of ALL
is determined by document position, not by reader inference. C-31→C-29.

**C-32 — Dual-Location Consolidated Block Canonical Axis Citation** (format)

V-02 passes C-30 with a consolidated block naming "(2) labeled axes
(`Required format:`/`Fail condition:`) at both locations". The block cites the canonical
axis labels by their exact names rather than by description or paraphrase. C-30 requires
the consolidated block to enumerate both locality properties; C-32 requires the
enumeration to use the canonical label names as they appear in the per-block hints
(`Required format:` and `Fail condition:`, or the equivalent labels used throughout the
rubric). This makes the consolidated block a navigable cross-reference: a reviewer can
match "Required format:" in the consolidated declaration directly to the label in each
B-NN hint without label translation. Paraphrased descriptions such as "format
specification" and "contract specification" satisfy C-30 but not C-32. C-32→C-30.

**C-33 — Preamble Concrete Example B-ID Alignment** (depth)

V-02 passes both C-24 and C-28. When C-28 is satisfied, each B-NN per-block hint
instantiates the owning B-ID: B-01's hint states `causal source: B-01`, B-02's states
`causal source: B-02`. The preamble concrete example required by C-24 uses `B-01` as its
exemplar bottleneck. C-33 requires the preamble concrete example to name the same B-ID
as the first declared B-NN block, aligning the preamble anchor with the per-block
instantiation chain. This extends the string-comparison detectability property of C-28
to the preamble-to-per-block boundary: a preamble example citing `B-01` and a first
per-block hint citing `B-01` are consistent by inspection; a preamble citing `B-[ID]`
or a different B-ID introduces a preamble-level generic placeholder that C-28 already
eliminates at per-block scope. C-33 cannot pass if C-24 or C-28 fails. C-33→C-28+C-24.

**Formula**: `/22` → `/25`; aspirational total remains 10 pts.

**Full dependency chain (v9):**
