## quest-rubric rubric v14 -- complete

### Changes from v13

Two new aspirational criteria (C-40, C-41) extracted from Round 13 excellence signals.
Denominator /31 -> /33. Round 13 ceiling 100.000 (V-05 R13 under v14, all criteria PASS);
prior ceiling holders V-01 and V-04 drop to 99.697 (C-41 absent); V-02 drops to 99.545
(C-40 absent, C-11 PARTIAL); V-03 drops to 99.242 (C-40 absent, C-41 absent, C-11 PARTIAL).

| # | Name | Signal it formalizes |
|---|------|---------------------|
| **C-40** | anchor-template-in-definer | V-01/V-04/V-05 R13 Signal 1 -- C-11 PASS is achieved in V-01, V-04, and V-05 because the Definer produces a third template (Anchor: Reference / Passes / Fails at) alongside the Text-direction and Pass-observability templates; Builder A gate enforces Anchor format for every aspirational competitor; V-02 and V-03 remain C-11 PARTIAL because their Definer produces only two templates and Anchor-format compliance depends solely on Builder-level instruction without Definer backing; C-40 formalizes the generation-gate mechanism that closes C-11 by extending the dual-template Definer (C-19) to a triple-template Definer |
| **C-41** | verification-step-competitor-coverage-complete | V-02/V-05 R13 Signal 2 -- C-39 is satisfied at TWO levels in V-02 and V-05: a Holistic-Audit-Reader placed at the isolation-audit step (closing the audit-format specification gap) and a Tier-Blind Categorizer at the divergence-check step (closing the divergence-check gap); V-05 adds a C-39 VERIFICATION block in Role 8 that checks both levels explicitly; V-01, V-03, V-04 satisfy C-39 at one level only (divergence check); C-41 formalizes the completeness requirement for the verification-step competitor set -- one per verification-step-gap criterion, in bijection, analogous to C-37 for construction steps |

**Denominator:** `/31` -> `/33`

**New ceiling math:** V-05 R13 under v13 scored 31/31 x 10 = 10.000, composite 100.000. Under
v14 (/33): V-05 R13 PASS C-40 (Role 1 produces three templates; Anchor template present as third
required output; Role 7 enforces Anchor format for numbered competitors; coverage check verifies
"Anchor format: all competitors have three-field Anchor format? YES") and PASS C-41 (Role 3
Holistic-Audit-Reader at isolation-audit step + Role 4 Tier-Blind Categorizer at divergence-check
step; C-39 VERIFICATION block in Role 8 checks both levels: isolation-audit YES, divergence-check
YES). New aspirational: 33/33 x 10 = 10.000, composite 100.000. V-01 and V-04 recalibrate to
32/33 x 10 = 9.697, composite 99.697 (C-40 PASS, C-41 FAIL: one verification-step level only,
isolation-audit competitor absent). V-02 recalibrates to 31.5/33 x 10 = 9.545, composite 99.545
(C-11 PARTIAL, C-40 FAIL: no Anchor template in Definer, C-41 PASS). V-03 recalibrates to
30.5/33 x 10 = 9.242, composite 99.242 (C-11 PARTIAL, C-40 FAIL, C-41 FAIL).

**Distinction notes:**

- C-40 != C-11: C-11 requires a structured Reference / Passes / Fails anchor to exist in the
  aspirational section and for each aspirational competitor to carry the three-field format;
  C-40 requires that anchor structure to originate from the Definer role as a third required
  template output, not from Builder-level instruction alone. A rubric satisfies C-11 (anchor
  exists, Builder A gate enforces Anchor format for every competitor via direct instruction)
  while failing C-40 if the Definer does not produce the Anchor template -- C-11 can be
  satisfied through the Builder's own gating without Definer backing, but C-40 requires the
  generation-gate mechanism to carry the Anchor template. The distinction tracks where the
  structural guarantee originates: C-11 tests whether the anchor structure is present and
  used; C-40 tests whether its production is Definer-issued, making it architecturally prior
  to the Builder's instructions. V-02 and V-03 satisfy neither C-11 (PARTIAL: competitors in
  narrative format) nor C-40 (no Anchor template from Definer). V-01, V-04, V-05 satisfy
  both by placing the Anchor template inside the Definer's required three-output block.

- C-40 != C-19: C-19 requires the Definer role to produce the Text-direction template and the
  Pass-observability template as a unified paired output before the Builder runs, making both
  enforcement paths structurally symmetric by construction; C-40 requires the Definer to
  extend its output to a unified triple adding the Anchor template (Reference / Passes /
  Fails at three-field format). A rubric satisfies C-19 (dual-template Definer, Text + Pass
  pair as unified output, direction and observability gated by the same Definer step) while
  failing C-40 if the Anchor template is absent from the Definer output. C-40 is the
  incremental extension of C-19 from a two-template to a three-template Definer output,
  closing the generation-gate gap for C-11 in the same architectural movement that C-19 used
  to close C-17 and C-18 simultaneously. Evidence: V-01 and V-04 title their Definer step
  "Three binding templates" and enumerate Text, Pass, and Anchor as co-required outputs;
  V-02 and V-03 title their Definer step "Two binding templates" -- the template count
  distinguishes v14 C-40 PASS from FAIL precisely.

- C-40 != C-18: C-18 requires a dedicated pre-construction role or step that reads the skill
  spec and produces skill-specific instantiations before the Builder constructs any criterion,
  establishing definitional priority at the role-boundary level; C-40 requires the Definer
  output within that prior role to include the Anchor template alongside the direction and
  observability templates. C-18 is about role priority and structural separation of
  instantiation from construction; C-40 is about template completeness within the Definer's
  required output. A rubric satisfying C-18 (Definer role present, definitionally prior,
  produces direction templates) fails C-40 if the Anchor template is not part of the
  Definer's required output list -- the role exists and runs first, but one of the three
  templates is missing from its contract.

- C-41 != C-39: C-39 requires at least one named verification-step competitor placed inline at
  a verification step, constituting the gap specification for that step, with the competitor
  block ending in a derivation instruction; C-41 requires the set of verification-step
  competitors to be in bijection with the set of verification-step-gap criteria -- one
  competitor per gap, every gap covered, total count equal. A rubric satisfies C-39 (one
  verification-step competitor present, correctly placed at the divergence-check step, with
  derivation instruction) while failing C-41 if a second verification-step-gap criterion
  exists (e.g., the isolation-audit format specification gap) with no companion competitor.
  The distinction parallels C-27 vs C-37 on the construction axis: C-39 requires presence
  (at least one verification-step competitor); C-41 requires completeness (every
  verification-step gap covered). A rubric can demonstrate deep C-39 compliance for one
  verification step while leaving another verification step navigated only by direct
  procedural instruction.

- C-41 != C-37: C-37 requires the construction-step competitor set to be in bijection with
  the novel-requirement construction criteria introduced in the aspirational tier; C-41
  requires the verification-step competitor set to be in bijection with verification-step-gap
  criteria. Both require bijection, but the two sets are independent decision classes
  (construction decisions vs verification decisions). A rubric satisfying C-37 (full
  construction bijection, all novel construction criteria covered by exactly one competitor
  each) fails C-41 if verification-step coverage is incomplete, and a rubric satisfying C-41
  (full verification bijection) fails C-37 if some novel construction criterion has no
  competitor. The independence means both can be tested separately; neither implies the other.

- C-41 != C-35: C-35 requires the divergence-check step to be present, named, structured, and
  STOP-gated, making tier-category verification independently falsifiable; C-41 requires
  every verification-step gap to be closed by a verification-step competitor, not just the
  divergence-check gap that C-35 operationalizes. A rubric satisfies C-35 (divergence check
  operationalized, Tier-Blind Categorizer placed at the step by derivation) and C-39 (one
  verification-step competitor present at the divergence-check step) while failing C-41 if
  the isolation-audit step gap has no companion competitor. C-35 establishes what the
  divergence-check step must do; C-41 requires the verification-step competitor mechanism to
  be applied to every equivalent gap, not only the one C-35 names.
...
## Aspirational Criteria (C-09--C-41) -- 10 points

Denominator: **/33**. Each criterion scores 1 (full pass), 0.5 (partial), or 0 (fail).
Score = (sum) / 33 x 10.
...
**C-40** (anchor-template-in-definer) -- The Definer role produces the Anchor template
(Reference / Passes / Fails at -- three-field format) as a third required output alongside
the Text-direction template and the Pass-observability template, making C-11 compliance a
generation-gate obligation rather than a Builder-level instruction. When the Definer outputs
all three templates as a unified triple, the Builder receives the Anchor structure before
writing any aspirational competitor, and the Builder A gate enforces Anchor format because
the template already exists as the Definer's binding output. Without the Anchor template in
the Definer output, C-11 compliance depends solely on a Builder A instruction that tells the
Builder to construct the anchor format on-the-fly -- a weaker structural guarantee because
the format must be invented during construction rather than received from a prior definitional
step. Not "Anchor format instruction present in Builder A" but "Anchor template produced by
the Definer as its third required output alongside Text-direction and Pass-observability, with
Builder A receiving and enforcing the Definer's Anchor template for every aspirational
competitor block." Tests whether: (a) the Definer role names three required template outputs
(Text-direction, Pass-observability, Anchor) rather than two; (b) the Anchor template is in
the Reference / Passes / Fails at three-field format; (c) the Anchor template is produced as
the Definer's required output before Builder A runs; and (d) Builder A's gate instruction
requires Anchor-format competitors by reference to the Definer's Anchor template, not by
independent format specification. The distinction from C-11: C-11 requires a structured
anchor to exist in the aspirational section and for competitors to use it; C-40 requires
that anchor structure to originate from the Definer as a third template, not from Builder-
level instruction. A rubric satisfies C-11 (Anchor format enforced by Builder A gate,
competitors carry three-field format) while failing C-40 if the Definer produces only two
templates and the Anchor format is first specified inside Builder A -- the anchor exists, but
its generation is not Definer-gated. C-40 requires C-11 compliance to trace to the generation
gate, not to a Builder gate that constructs the format from scratch. The distinction from
C-19: C-19 requires the Definer to produce Text + Pass as a unified pair, making the two-
field generation gate symmetric; C-40 requires the Definer output to be a unified triple,
adding the Anchor template as a co-required third output. A rubric satisfies C-19 (dual-
template Definer, Text + Pass pair, symmetric two-field gate) while failing C-40 if the
Anchor template is absent from the Definer output. The distinction from C-18: C-18 requires
a dedicated pre-construction role or step that is logically prior to construction and produces
skill-specific instantiations; C-40 requires the content of that prior role's output to
include the Anchor template as a third named required output. C-18 establishes role-boundary
priority; C-40 specifies template completeness within that boundary. Evidence: V-01 and V-04
title their Definer step "Three binding templates" and enumerate Text-direction, Pass-
observability, and Anchor as co-required outputs; Builder A gate references "Anchor template
from Definer" for each aspirational competitor, satisfying C-40. V-02 and V-03 title their
Definer step "Two binding templates" (Text + Pass); the Anchor format first appears in
Builder A as a Builder-level instruction, not a Definer output -- C-11 is PARTIAL (narrative
competitors) and C-40 fails (no third template from Definer). V-05 achieves C-40 via Role 1
(three templates) with Role 7 enforcing "Anchor-format numbered competitors" and the Coverage
Confirmation checking "Anchor format: all competitors have three-field Anchor format? YES."

**C-41** (verification-step-competitor-coverage-complete) -- For every criterion that requires
a named verification step -- a step that checks whether a design requirement is met after
criteria are constructed -- there is exactly one named verification-step competitor placed
inline at that step (C-39-style), and the total count of verification-step competitors equals
the total count of verification-step-gap criteria. The verification-step competitor set is in
bijection with the verification-step-gap criteria set, making the coverage complete and
enumerable: a reader who counts the verification-step competitors knows how many verification-
step gaps the rubric closes without reading the criteria themselves. Without complete coverage,
a rubric can satisfy C-39 at one verification step while leaving a second verification step
navigated only by direct procedural instruction -- the wrong form of the second verification
is not named before the step is specified, and the derivation benefit of the competitor-then-
imperative axis does not apply to that gap. Not "one verification-step competitor present at
one step" but "every verification-step-gap criterion has exactly one verification-step
competitor, the two sets are in bijection, and a named completeness check confirms the count
matches." Tests whether: (a) the rubric identifies all criteria that require named verification
steps; (b) each identified gap has exactly one verification-step competitor placed inline at
that step with a derivation instruction (satisfying C-39 for each); (c) the total count of
verification-step competitors equals the total count of verification-step-gap criteria; and
(d) a named verification block or coverage check confirms the completeness of the bijection
-- analogous to the SEVEN-COMPETITOR COVERAGE CONFIRMATION block that confirms C-37 and C-38,
but applied to the verification-step axis. The distinction from C-39: C-39 requires at least
one named verification-step competitor placed inline at a verification step, with derivation
instruction, constituting the gap specification; C-41 requires coverage to be complete across
all verification-step gaps -- a rubric satisfies C-39 (one verification-step competitor
present, correctly placed, with derivation instruction) while failing C-41 if a second
verification-step-gap criterion exists with no companion competitor. The distinction parallels
C-27 vs C-37 on the construction axis: C-39 is to C-41 as C-27 is to C-37. The distinction
from C-37: C-37 requires the construction-step competitor set to be in bijection with the
novel-requirement construction criteria; C-41 requires the verification-step competitor set
to be in bijection with the verification-step-gap criteria. Both require bijection, but the
two covered sets belong to independent decision classes (construction decisions vs verification
decisions). A rubric satisfies C-37 (full construction bijection) while failing C-41
(verification bijection incomplete), and vice versa; neither implies the other. The
distinction from C-35: C-35 requires the divergence-check step to be present, named,
structured, and STOP-gated; C-41 requires every verification-step gap -- not just the
divergence-check gap -- to have a verification-step competitor. A rubric satisfies C-35
(divergence check operationalized via Tier-Blind Categorizer) and C-39 (one verification-step
competitor at the divergence-check step) while failing C-41 if the isolation-audit format-
specification gap is closed by direct procedural instruction rather than by a named
verification-step competitor (e.g., Holistic-Audit-Reader inline at the isolation-audit step).
Evidence: V-02 places a Holistic-Audit-Reader in the AUDIT-FORM SPECIFIER role at the
isolation-audit step ("This competitor IS the gap specification for C-39 at the isolation
audit step. From the description above, derive...") AND a Tier-Blind Categorizer at the
divergence-check step -- two independent verification-step competitors at their respective
steps, both with derivation instructions, covering both verification-step gaps: C-41 PASS.
V-05 achieves C-41 via Role 3 (Holistic-Audit-Reader at isolation-audit step) + Role 4
(Tier-Blind Categorizer at divergence-check step) with a dedicated C-39 VERIFICATION block
in Role 8 checking both levels: "Isolation audit: Holistic-Audit-Reader competitor placed in
Role 3 before audit format specified? YES. Divergence check: Tier-Blind Categorizer placed
in Role 4 before divergence check format? YES." V-01, V-03, and V-04 place the Tier-Blind
Categorizer at the divergence-check step only -- one level, C-39 PASS, C-41 FAIL by absence
of the isolation-audit competitor.
...
Aspirational: (C-09 through C-41) / 33 x 10                        max 10  (partials = 0.5)

Composite = Essential + Recommended + Aspirational                  max 100
Golden threshold: >= 90
```

**Scoring note:** The aspirational denominator shifts from `/31` to `/33` to reflect the
addition of C-40 and C-41.

*Round 13 ceiling recalibration.* V-05 R13 under v13 scored 31/31 x 10 = 10.000 aspirational,
composite 100.000. Under v14 (/33): V-05 R13 PASS C-40 (Role 1 produces three templates
including Anchor; Role 7 enforces Anchor-format numbered competitors; Coverage Confirmation
in Role 8 verifies "Anchor format: all competitors have three-field Anchor format? YES") and
PASS C-41 (Role 3 Holistic-Audit-Reader at isolation-audit step; Role 4 Tier-Blind Categorizer
at divergence-check step; C-39 VERIFICATION block in Role 8 checks both levels). New
aspirational: 33/33 x 10 = 10.000, composite 100.000. V-01 and V-04 recalibrate to 32/33 x 10
= 9.697, composite 99.697 (C-40 PASS, C-41 FAIL: isolation-audit competitor absent). V-02
recalibrates to 31.5/33 x 10 = 9.545, composite 99.545 (C-11 PARTIAL, C-40 FAIL, C-41 PASS).
V-03 recalibrates to 30.5/33 x 10 = 9.242, composite 99.242 (C-11 PARTIAL, C-40 FAIL, C-41
FAIL). Sole path to 100.000 under v14: all 33 criteria PASS, requiring C-11 PASS (three-
template Definer with Anchor output, Builder A gate enforcement), C-40 PASS (Anchor template
produced by Definer as third required output), and C-41 PASS (verification-step competitors
at both isolation-audit and divergence-check steps, completeness confirmed by named block).

*C-40 closes the generation-gate gap for C-11.* C-19 established the dual-template Definer
(Text + Pass pair) as the generation-gate mechanism for C-17 and C-18. The same architectural
move applied to C-11 requires a third template: the Anchor (Reference / Passes / Fails at).
When the Definer produces all three templates, C-11 compliance is structurally prior to
construction -- the Builder receives the Anchor format as a Definer output, not as a self-
constructed format to be invented in Builder A. The evidence pattern confirms the discriminant:
V-02 and V-03 use two-template Definers and achieve C-11 PARTIAL; V-01, V-04, and V-05 use
three-template Definers and achieve C-11 PASS. Template count is the observable.

*C-41 completes the verification-step axis.* C-39 established the competitor-then-imperative
pattern for verification steps by analogy with C-36 (construction-step placement). C-41
extends the completeness requirement from the construction axis (C-37) to the verification
axis: every verification-step gap must have exactly one verification-step competitor, in
bijection, verified by count. V-02 and V-05 satisfy C-41 by placing verification-step
competitors at both identified gaps (isolation-audit and divergence-check). V-05 adds a
dedicated C-39 VERIFICATION block that makes the bijection explicitly checkable, parallel
to the SEVEN-COMPETITOR COVERAGE CONFIRMATION block that makes C-38 checkable.

*C-40 and C-41 form a symmetric pair of generation-gate and coverage extensions.* C-40
extends the Definer output from two templates to three, closing the generation-gate gap for
C-11 at the architectural level. C-41 extends the verification-step competitor requirement
from presence (C-39) to completeness, closing the coverage gap for verification steps at the
bijection level. The structural chain is now fully symmetric: C-27 -> C-36 -> C-37 -> C-38
covers construction steps (existence -> placement -> completeness -> self-reference);
C-39 -> C-41 covers verification steps (existence -> completeness). C-18 -> C-19 -> C-40
covers the Definer output (role-boundary priority -> dual-template -> triple-template). A
rubric satisfying C-27, C-36, C-37, C-38, C-39, C-40, and C-41 has a complete triple-
template Definer, complete self-demonstrating construction-step competitor coverage, and
complete verification-step competitor coverage -- making the negative-space specification
and generation-gate architecture exhaustive across all decision classes and template types.
