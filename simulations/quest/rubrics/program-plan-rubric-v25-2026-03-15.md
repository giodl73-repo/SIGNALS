...
## Aspirational Criteria (10 pts total — 22 criteria, 10/22 ≈ 0.45 pts each)
...
aspirational_pass = ___ / 22 -> ___ * (10/22) = ___ pts  (of 10)
...
**Aspirational tier expanded: 20 → 22 criteria, per-criterion value: 0.50 → 10/22 ≈ 0.45 pts**

| New ID | Pattern | Source signal | Distinction |
|--------|---------|---------------|-------------|
| C-28 | **BAD example framed as causal process narrative** — the inertia contrast takes the form of or is preceded by a realistic human-process narrative showing how a team produced the bad artifact (a meeting, a planning exchange), making the process failure visible alongside the artifact failure | R8 V-02 C-16 notable distinction: "a 4-sentence planning sync that ends with 'Put done for now.' The BAD YAML is what the meeting produced" — the narrative makes the decision visible, not just its output | C-10 = BAD example present in any form; C-28 = BAD example is framed by a causal narrative showing how it was produced; a raw bad YAML without process context passes C-10 but not C-28 |
| C-29 | **Identity claim extends to process replacement** — the contrast-grounded identity claim explicitly scopes the skill's replacement to include both the artifact AND the conversation/process that produced it; canonical form: "exists to replace this conversation and the plan it produces" | R8 V-02 C-16 notable distinction: identity claim "exists to replace this conversation and the plan it produces" — explicitly claims to replace the meeting, not just the artifact | C-16 = identity established via contrast in opening position; C-29 = that identity claim names the process/conversation as what is replaced, not the artifact alone; "exists to replace this pattern" passes C-16 but not C-29 |

**Dependency chain for new criteria:**
- C-28 requires C-10 PASS (causal narrative is a stricter form of the inertia contrast requirement; the narrative must contain or constitute the BAD example)
- C-29 requires C-16 PASS (process-scope claim is a stricter form of the contrast-grounded identity requirement; the claim must already satisfy C-16's opening + contrast conditions)
- C-28 and C-29 are independent of each other: a causal narrative BAD example (C-28) does not require the identity claim to name the process (C-29), and a process-scope identity claim (C-29) does not require the BAD example to be a narrative (though in practice both appear together in V-02's richest form)

**Scoring arithmetic**: 22 criteria × (10/22) = 10.00 pts total. The formula `aspirational_pass/22 * 10` yields exact values. Perfect aspirational score is still 10 pts.

**Re-scoring R8 at v25:**

| Variation | C-28 | C-29 | Aspirational | Composite |
|-----------|------|------|-------------|-----------|
| V-01 (Formal Specification Register) | FAIL (BAD YAML presented without causal narrative) | FAIL (identity claim: "SHALL replace PROGRAM-BAD-01" — artifact replacement only, no conversation scope) | 20/22 | 99.09 |
| V-02 (Narrative Inertia Framing) | PASS (4-sentence planning sync ending with "Put 'done' for now" produces the BAD YAML) | PASS (identity claim: "exists to replace this conversation and the plan it produces") | 22/22 | 100.00 |
