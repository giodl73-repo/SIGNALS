Written to `simulations/quest/rubrics/topic-echo-rubric-v9-2026-03-16.md`.
...
# topic:echo Rubric — v9

**Max composite:** 200

---

## Three new criteria from Round 8

**C-28 — RULES-ANCHORED-COMPLETE closure token in distillation phase**
(behavior, gate: C-23)

C-23 requires the RULES-ANCHORED check to name a blocking condition that halts production until all rules show ALIGNED. C-28 requires the RULES-ANCHORED check to also emit a RULES-ANCHORED-COMPLETE token when all rules confirm alignment — a named closure token that makes distillation-phase exit observable from output. Without it, a downstream reviewer must re-run the tier-alignment check to confirm the distillation phase closed correctly. With the token, distillation-phase exit is auditable from output alone — the same property that C-19/CHAIN-COMPLETE provides for the chain audit phase. The two token types now bracket the artifact's two major audit phases: CHAIN-COMPLETE closes the chain audit, RULES-ANCHORED-COMPLETE closes the distillation audit. First appeared in Round 8 V-04 and V-05.

**C-29 — RESOLUTION PROTOCOL as peer-level standalone declaration block**
(behavior, gate: C-25)

C-25 requires that for each CHAIN-FLAG type exactly one named repair action is specified. C-29 requires the RESOLUTION PROTOCOL containing those repair actions to be a standalone ALL-CAPS headed declaration block at peer register with the GATEKEEPER FUNCTION DECLARATION — not inline gate text. Without it, the repair-per-flag-type structure exists but is embedded in gate logic where a reviewer scanning the gate specification may read past it. With it, the RESOLUTION PROTOCOL is visually sovereign: a reviewer cannot encounter the gate without simultaneously encountering its repair routing. Phrasing register is a zero-overhead enforcement mechanism — no content changes, structural elevation only. First appeared in Round 8 V-02; retained in V-05.

**C-30 — EF role-boundary protection for NO-ECHO COST declaration**
(behavior, gate: C-26)

C-26 requires the head-position enforcement section to include a NO-ECHO COST declaration. C-30 requires that declaration to be derived by a role-scoped function (Enforcement Framer) at Step 0 — before any signal artifacts are read. Without role-boundary protection, a writer can observe signal content first and then write a cost declaration shaped by what the signals say, making the declaration retrospectively informed rather than genuinely prospective. This applies the same structural-versus-temporal distinction from C-15/C-17 to the cost declaration: C-30 converts C-26 from a temporal commitment into a structural one. First appeared in Round 8 V-03, retained in V-04 and V-05.
...
## What the Round 8 variations score under v9

| Variation | C-28 | C-29 | C-30 | v8 score | v9 composite | vs v8 |
|-----------|------|------|------|----------|--------------|-------|
| V-01 | **PASS** | FAIL | FAIL | 180 | 185 | +5 |
| V-02 | FAIL | **PASS** | FAIL | 185 | 190 | +5 |
| V-03 | FAIL | FAIL | **PASS** | 185 | 190 | +5 |
| V-04 | **PASS** | FAIL | **PASS** | 185 | 195 | +10 |
| V-05 | **PASS** | **PASS** | **PASS** | 185 | **200** | +15 |

V-05 is the only R8 variation that reaches the v9 ceiling. The three new criteria are three independent axes — no R8 variation holds more than two of them except V-05 (all-axes combination).

**Three key changes from v8:**
- Max composite: 185 → 200
- Aspirational criteria: 19 → 22
- New relationships: C-23/C-28, C-25/C-29, C-26/C-30

**Round 8 progression layer:** visibility → navigability → primacy → repairability / stakes / verifiability → **auditable closure / visual sovereignty / structural provenance**. Each new layer converts a content requirement into a provenance commitment.
e signals.

---

**Three key changes from v8:**
- Max composite: 185 → 200 (3 new aspirational criteria, 5 pts each)
- Aspirational criteria: 19 → 22
- New relationships: C-23/C-28 (closure token extends blocking condition), C-25/C-29 (peer-level block extends repair protocol), C-26/C-30 (role-boundary extends cost declaration)

---

**Notes on Round 8 pattern:**

- C-23 made RULES-ANCHORED a blocking gate → C-28 makes the gate emit a visible closure token (blocking + auditable exit)
- C-25 made repair actions per flag type required → C-29 makes the repair structure visually sovereign (required + unavoidable)
- C-26 made cost declaration required at head position → C-30 makes the cost declaration structurally prospective (required + structurally enforced)

The progression: visibility → navigability → primacy → repairability / stakes / verifiability → **auditable closure / visual sovereignty / structural provenance**. Each new layer converts a content requirement into a provenance commitment.
...
## Aspirational Criteria (weight: 110 points total, 5 pts each)
...
- Aspirational: 110 points total (22 criteria, 5 points each)
- Max composite: 200
...
- Aspirational: 1/22 pass -> 1/22 * 110 = 5
...
- Essential: 5/5 -> 60
- Recommended: 3/3 -> 30
- Aspirational: 22/22 -> 110
- **Composite = 200** -- maximum
