Reading the R12 scorecard to extract excellence signals from the top-scoring cluster (V-01 through V-05, all ~122.5 under v12) and identifying the two new structural patterns that exceed existing criteria.

**Excellence signals identified from R12:**

**C-44 — Per-signal annotated inventory with role classification** — V-04's implementation goes beyond C-39's phase classification summary: it requires a per-signal annotation pass in which every signal is individually labeled with both its evidence phase (Explore/Test/Validate) AND a role classification (Primary/Supporting/Contextual), producing an enumerable intermediate inventory before ceiling computation. The ceiling declaration (C-42) is extended to include Primary signal counts by phase, making the ceiling value derivable by inspection of the annotation set rather than asserted from a gestalt impression. C-39 requires a ceiling mechanism; C-42 requires the declaration; C-44 requires that the ceiling be *derived* from an explicit per-signal annotation inventory with role taxonomy.

**C-45 — Two-dimensional ceiling table (phase × orthogonal coverage dimension)** — V-05's implementation extends C-39's 1D phase ceiling table to two orthogonal dimensions: evidence phase and inertia coverage (whether any signal includes a direct comparison to the status-quo competitor). The ceiling is the intersection of both dimensions. At Test phase, inertia-absent ceiling = 45 while inertia-present ceiling = 60 — confirming independent influence of the second dimension at fixed phase level. The mandatory declaration (C-42) is extended to name the inertia coverage state alongside phase and ceiling value. C-39 requires a ceiling mechanism with at least one dimension; C-45 requires a second orthogonal dimension with distinct ceiling values per intersection cell.

**Calibration preview:** V-04 earns C-44 PASS (per-signal phase + role annotation inventory, declaration extended to Primary counts by phase, C-39/C-42 prerequisites satisfied); V-04 C-45 FAIL (no inertia dimension; second dimension is role taxonomy not inertia coverage). V-05 earns C-45 PASS (2D ceiling table phase × inertia coverage, distinct ceilings at Test level by inertia state, C-39/C-42 prerequisites satisfied); V-05 C-44 FAIL (uses phase + inertia dimensions, not Primary/Supporting/Contextual role taxonomy; declaration names inertia state not Primary counts by phase). V-01/V-02/V-03 unchanged (~122.5/~90.0/~122.5). V-04 moves ~122.5 → ~125.0. V-05 moves ~122.5 → ~125.0.

---

```markdown
# prove-synthesize-rubric-v13-2026-03-15.md

## prove-synthesize — Rubric v13

**Version**: v13 (2026-03-15)
**Predecessor**: v12 (2026-03-15)
**Golden threshold**: all essential PASS AND composite >= 90
**Max composite**: 182.5

---

## New in v13 — Two Aspirational Criteria from R12 Excellence Signals
...
| | v9 | v10 | v11 | v12 | v13 |
|-|----|-----|-----|-----|-----|
| Aspirational | 65.0 pts / 26 criteria | 75.0 pts / 30 criteria | 82.5 pts / 33 criteria | 87.5 pts / 35 criteria | **92.5 pts / 37 criteria** |
| Max composite | 155.0 | 165.0 | 172.5 | 177.5 | **182.5** |
| Golden threshold | >= 90 | >= 90 | >= 90 | >= 90 | **>= 90 (unchanged)** |

---

## R12 Calibration Under v13
...
| | Result | Evidence |
|---|---|---|
| V-04 | **PASS** | Phase 1 annotates each signal with evidence phase (Explore/Test/Validate) AND signal role (Primary/Supporting/Contextual) as enumerable mandatory pre-synthesis step; C-42 declaration extended to "Primary signals by phase: [count per phase]"; C-39/C-42 prerequisites satisfied |
| V-01/V-02/V-03/V-05 | **FAIL** | No per-signal annotation pass with role classification (Primary/Supporting/Contextual or equivalent) present; ceiling computed from phase coverage summary, not derived from enumerable per-signal annotation inventory |

### C-45 — Two-dimensional ceiling table (phase × orthogonal coverage dimension) (Aspirational, NEW v13)

| | Result | Evidence |
|---|---|---|
| V-05 | **PASS** | Ceiling table uses two orthogonal dimensions: evidence phase (Explore/Test/Validate) and inertia coverage (absent/present); distinct ceilings at Test level: inertia-absent = 45, inertia-present = 60 confirms independent influence; C-42 declaration extended to include inertia coverage state; C-39 prerequisite satisfied |
| V-01/V-02/V-03/V-04 | **FAIL** | Ceiling table is 1D (phase only); no orthogonal second dimension with distinct intersection ceiling values present |

---

## Composite Score Computation

**Anchors from R12 calibration (v13 adjusted):**
- R12 V-01 (v13): ~122.5 — earns full C-35/C-39/C-40/C-41/C-42/C-43 cluster; C-44/C-45 absent
- R12 V-04 (v13): ~125.0 — V-01 base + C-44 PASS (+2.5)
- R12 V-05 (v13): ~125.0 — V-01 base + C-45 PASS (+2.5)

Each criterion (essential and aspirational) treated as 2.5 pts for composite delta computation.

### V-04 — Per-signal annotated inventory with role classification

| Criterion | R12 (v12) | C-44 delta | v13 |
|-----------|-----------|------------|-----|
| C-35 | PASS | 0 | PASS |
| C-36 | PASS | 0 | PASS |
| C-37 | PASS | 0 | PASS |
...
**v13 V-04 = 122.5 + 2.5 = ~125.0**

Essential status: C-11 ✓ C-13 ✓ C-30 ✓ — all-essential PASS.
Golden: 125.0 ≥ 90 ✓ — **GOLD**

### V-05 — Two-dimensional ceiling table (phase × inertia)

| Criterion | R12 (v12) | C-45 delta | v13 |
|-----------|-----------|------------|-----|
| C-35 | PASS | 0 | PASS |
| C-36 | PASS | 0 | PASS |
| C-37 | PASS | 0 | PASS |
