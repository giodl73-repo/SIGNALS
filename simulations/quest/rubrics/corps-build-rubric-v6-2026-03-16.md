Written to `simulations/quest/rubrics/corps-build-rubric-v6-2026-03-16.md`.

---

**What changed in v6:**

One new criterion extracted from R5:

| ID | Pattern | Source |
|----|---------|--------|
| C-21 | TRANSCRIPTION-CLEAR as named structural re-audit gate | V-01 PARTIAL on C-19 |

**Extraction signal:** V-01 scores 99.58 and gets PARTIAL on C-19. The diagnostic is precise: V-01 rewrites the one REVISED artifact and asserts TRANSCRIPTION-PASS — but the assertion covers only that artifact. A model can reach TRANSCRIPTION-PASS while leaving the other two artifacts unverified in the same pass. V-02 (100 pts) fixes this with a named PHASE-EXIT GUARD that re-audits all three verdicts and gates CHART-WRITTEN on an explicit TRANSCRIPTION-CLEAR block.

**Pair logic added:**
- C-19: self-correction-trigger (REVISED -> rewrite, phase cannot exit REVISED)
- C-21: structural-re-audit-guarantee (after any rewrite, all artifacts re-checked in a named block before CHART-WRITTEN)

The pair parallels the existing C-17/C-20 isolation pair: C-19 establishes the correction obligation, C-21 makes the exit gate structural.

**V-03 deferred.** The "declarative output-forward framing" axis is named in the variation title but the aspirational table is empty — no scored evidence exists. R6 will need to score V-03's aspirational cells before extraction is possible.

**Formula:** `/12 -> /13`. Max aspirational points unchanged at 10.
framing") names a
candidate axis but produces no evidence cells. Extraction deferred to R6.
