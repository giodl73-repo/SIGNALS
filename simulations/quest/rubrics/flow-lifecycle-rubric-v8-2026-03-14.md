Written to `simulations/quest/rubrics/flow-lifecycle-rubric-v8-2026-03-14.md`.

---

**3 new criteria extracted, 1 per R7 excellence pattern:**

| New | Pattern | Dependency |
|-----|---------|------------|
| C-28 — Evidence Format Pattern B-ID Instantiation | Cascade depth grows: C-28→C-27 reaches depth-4 | C-28→C-27 |
| C-29 — B-NN Scaffold Completeness Prerequisite Declaration | Scaffold symmetry is a prerequisite for dual-location testability | C-29→C-27 |
| C-30 — Dual-Location Requirements Consolidated Declaration | Locality invariant: at maturity, gaps are always locality, not content | C-30→C-24+C-27 |

**Key changes:**
- Formula: `aspirational_pass / 19` → `aspirational_pass / 22`; max composite unchanged
- Dependency chain extended: `C-22→C-26→C-27→C-28` is now depth-4 from root
- C-28 closes the per-block `Required format:` gap — generic `B-[ID]` placeholder vs. instantiated `B-01`/`B-02`
- C-29 makes scaffold completeness a named gate (not derivable from "each B-NN" language alone)
- C-30 consolidates the two symmetric escalation pairs into one named locality inventory block
it into a named structural gate: the template must explicitly
declare that ALL B-NN blocks carry the Evidence field. Asymmetric scaffolds mask
dual-location criteria by making per-block scope structurally absent for some blocks.
C-29→C-27.

**C-30 — Dual-Location Requirements Consolidated Declaration** (format)

Two symmetric escalation pairs (C-22/C-24 and C-26/C-27) have accumulated through R4-R7.
The locality invariant has stabilized: at high rubric maturity, the discriminating
aspirational gap is consistently a locality failure, not a content failure. C-30 encodes
this invariant: the template contains a consolidated dual-location requirements block
listing BOTH locality properties (concrete example at both locations; labeled axes at both
locations) in one named declaration. C-30→C-24+C-27.

**Formula**: `/19` → `/22`; aspirational total remains 10 pts.

**Full dependency chain (v8):**
