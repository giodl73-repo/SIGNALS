...
|----|-----------|----------|----------------|
| C-01 | Role source traceability | correctness | Every reviewer named in the output corresponds to a file present in `.roles/`. The role name used in output must match the filename stem exactly. No invented or inferred role names are permitted. |
...
| C-04 | Depth mode compliance | behavior | Standard mode: reviewer count is a subset selected for relevance to the artifact type. Deep mode (`--depth deep`): all roles in `.roles/` are included. If depth is not specified, standard mode applies. Pass condition: the role count in the output is consistent with the mode used. |
...
|---------|------------------------|
| Reviewer roles not from `.roles/` | C-01 |
