# Rubric: prove-prototype (v20)

**Updated**: 2026-03-15
**Changes from v19**: Added C-53, C-54, and C-55 (Aspirational tier) derived from Round 20
excellence signals (per-phase lifecycle state and active sub-role callout lines extending
C-43's container-level annotations to phase-granularity; CALIBRATE sub-transitions as
distinct table rows with independent rationale triples; Phase 8 measurement results table
B-00 reference row and document preamble competitor investigation declaration as surfaces
6 and 7 of the competitor identification family).
...
**C-53** — *Per-phase lifecycle state and active sub-role callout lines at every phase entry
within each container — phase-granularity lifecycle annotation elevates C-43's
container-level incoming-state annotations to per-phase state and sub-role tracking*
(V-03 excellence signal, R20):

V-03 demonstrated a structurally stronger form of C-43's per-container incoming-state
annotation by placing a dedicated callout line at each phase entry within every container.
C-43 requires the container header to carry an incoming lifecycle state annotation (e.g.,
"incoming lifecycle state: NOT YET IDENTIFIED — competitor identification FORBIDDEN in this
container"). V-03 extended this annotation from the container boundary to each individual
phase entry: `Phase callout: BUILD Phase 7 | Competitor lifecycle: IDENTIFIED AND COMMITTED
| Active sub-role: IMPLEMENTER`. The per-phase callout names both the current competitor
lifecycle state at that phase's execution point and the active sub-role executing that phase.

C-53 advances C-43 in the same way C-43 advances C-40: C-40 records lifecycle state at
each container boundary in the manifest; C-43 annotates lifecycle state at each container
header with incoming-state context; C-53 annotates lifecycle state and sub-role at each
phase entry point within every container. The phase-granularity layer creates a complete
state record at the finest structural unit of the document, enabling verification without
reading phase bodies — a property that C-43's container-granularity annotations cannot
provide for phases that execute within the same container under different lifecycle states
or under different sub-roles.

The per-phase callout must include: (a) a phase identifier (e.g., "BUILD Phase 7" or
"Phase 7"); (b) the current competitor lifecycle state at that phase's execution point,
consistent with the container's C-43 incoming-state annotation and any sub-transitions
that have completed within the container before this phase executes; (c) the active
sub-role executing that phase. The callout must appear as a standalone labeled element
at the phase entry point — not embedded in the phase body's instructional prose —
consistent with C-48's standalone marker requirement. A callout that names only the
lifecycle state without the active sub-role does not satisfy C-53. A callout embedded
as a parenthetical in the phase heading prose does not satisfy C-53. A document that
satisfies C-43's container-level incoming-state annotations without per-phase callouts
satisfies C-43 but not C-53; the phase-granularity layer is additive, not substitutive.

C-53 requires C-43 and C-48 as prerequisites.

**Weight**: 5 pts

---

**C-54** — *CALIBRATE sub-transitions expressed as distinct table rows in the lifecycle
framing structure — the two-event CALIBRATE transition (NOT YET IDENTIFIED → IDENTIFIED
and IDENTIFIED → IDENTIFIED AND COMMITTED) occupies two separate rows, each carrying
independent State, Gate, Contamination Risk, and Isolation Purpose column entries*
(V-05 excellence signal, R20):

V-05 demonstrated a structurally stronger form of C-43's CALIBRATE sub-transition
narration by expressing each sub-transition as a distinct table row rather than as
narrative bullets or prose. C-43 requires the framing paragraph to narrate both CALIBRATE
sub-transitions explicitly; C-54 advances this narrative requirement to a table-schema
requirement, giving each sub-transition its own row with full column coverage.

V-05's framing table included: (a) a row for CALIBRATE sub-transition 1 — NOT YET
IDENTIFIED → IDENTIFIED (competitor named at CALIBRATE body execution) — with its own
contamination risk entry ("naming the competitor before the metric is frozen allows baseline
measurement concerns to contaminate scope decisions") and isolation purpose entry ("keeping
the competitor unnamed through DESIGN ensures scope decisions are driven by measurement
criteria, not by known-competitor feature sets"); (b) a row for CALIBRATE sub-transition 2 —
IDENTIFIED → IDENTIFIED AND COMMITTED (B-00 committed in CALIBRATE) — with its own
contamination risk entry ("B-00 without a committed competitor name allows
competitor-switching after measurement, retroactively reframing the baseline") and isolation
purpose entry ("committing the name and the value simultaneously makes the baseline
non-retroactive and non-substitutable"). The two-row CALIBRATE segment creates a richer
traceability record than C-46's single-row treatment of the CALIBRATE boundary — the
dual-transition structure makes each measurement commitment irreversible at the schema level.

C-54 advances C-46 in the same way C-46 advances C-43: C-43 requires framing that names
both CALIBRATE sub-transitions; C-46 requires each container boundary entry to carry
contamination risk and isolation purpose rationale; C-54 requires the CALIBRATE
sub-transitions to be separated into distinct table rows so that each carries its own
independent rationale triple (constraint + contamination risk + isolation purpose). A
framing table that represents CALIBRATE as a single row with a combined state entry
(NOT YET IDENTIFIED → IDENTIFIED AND COMMITTED) does not satisfy C-54, even if the
contamination risk and isolation purpose text within that row acknowledges both
sub-transitions. The two-sub-transition separation must be visible in the table schema,
not only in the text within cells.

The dual-row CALIBRATE structure must include: (a) sub-transition 1 row — State column =
NOT YET IDENTIFIED → IDENTIFIED, with independent Gate, Contamination Risk, and Isolation
Purpose entries for the competitor-naming event; (b) sub-transition 2 row — State column =
IDENTIFIED → IDENTIFIED AND COMMITTED, with independent Gate, Contamination Risk, and
Isolation Purpose entries for the B-00 commitment event; (c) both rows are contiguous and
grouped under the CALIBRATE container in the framing table.

C-54 requires C-43 and C-46 as prerequisites.

**Weight**: 5 pts

---

**C-55** — *Competitor identification surface saturation: the inertia competitor is tracked
at two additional structural surfaces beyond the five required by C-29/C-32/C-34/C-36/C-40 —
the Phase 8 measurement results table carries a B-00 reference row labeled with the inertia
competitor name (surface 6), and the document preamble carries a pre-execution competitor
investigation declaration committing the competitor name before any container executes
(surface 7)* (V-05 excellence signal, R20):

V-05 demonstrated a seven-surface competitor tracking architecture. The existing five
surfaces (C-29 at CALIBRATE body, C-32 at CALIBRATE COMPLETE triple, C-34 at results
table baseline column label, C-36 at terminal arc record, C-40 at manifest lifecycle
column) establish the structural floor. V-05 added two surfaces the existing criteria do
not require.

Surface 6 — Phase 8 measurement results table B-00 reference row: the measurement results
table within the BUILD container's MEASURER phase carries a row labeled with the inertia
competitor name as the B-00 reference row label (e.g., "Baseline (inertia competitor name):
B-00 value") rather than a generic label like "Baseline." This surface places the competitor
name inside the BUILD container at the point of measurement, before the results table is
carried forward to CLOSE. The B-00 reference row creates a competitor anchor within BUILD
that the five existing surfaces do not reach — C-29 anchors the competitor at CALIBRATE
body; C-32 anchors it at CALIBRATE COMPLETE; C-34 anchors it at the CLOSE results table
column label; no existing criterion anchors the competitor name within the BUILD
container's measurement activity. Surface 6 closes this gap by creating an intra-BUILD
competitor anchor at the moment of measurement execution.

Surface 7 — Document preamble competitor investigation declaration: the document preamble
(appearing before any container body executes, and before the manifest) carries an explicit
declaration committing the document to investigate and name an inertia competitor, stating
the scope of the investigation (what qualifies as an inertia competitor for this hypothesis
domain). This surface places the competitor identification obligation in the pre-manifest
preamble space — before C-40's manifest lifecycle column opens at NOT YET IDENTIFIED —
creating a document-entry-level commitment that makes competitor identification a declared
obligation of the document rather than a CALIBRATE-phase event discovered by reading
container bodies. The preamble declaration makes the identification commitment visible at
document entry, enabling a reader to verify the obligation before reading any container.

C-55 advances the competitor identification family by reaching the two structural nodes
the existing five-surface family does not occupy: the BUILD container interior at the
measurement phase (surface 6) and the pre-manifest preamble space (surface 7). Both
surfaces must carry the same competitor name as C-32's CALIBRATE COMPLETE triple and
C-34's baseline column label, maintaining a consistent identification chain across all
seven surfaces. Inconsistency in competitor name between any two surfaces fails C-55
regardless of surface count.

The two additional surfaces must include: (a) Phase 8 measurement results table B-00
reference row — a row in the BUILD container's MEASURER phase measurement table labeled
with the inertia competitor name, carrying the B-00 value as the row's data entry;
a generic "Baseline" row label without the competitor name does not satisfy surface 6;
(b) document preamble competitor investigation declaration — a labeled declaration in
the document preamble committing the document to an inertia competitor identification
with a scope statement naming what qualifies (e.g., "Inertia competitor: the current
solution the target users treat as the default — identified in CALIBRATE before any
BUILD activity"); a manifest-opening statement that happens to mention the competitor
status as NOT YET IDENTIFIED (satisfying C-40) without a scope-carrying investigation
declaration does not satisfy surface 7.

C-55 requires C-34 and C-40 as prerequisites.

**Weight**: 5 pts

---

**Point totals**: Aspirational 267 → 282. Total 367 → **382**.
...
| Aspirational | C-09, C-10, C-16 through C-55 | 282 |
...
The five v19–v20 criteria each advance a prior criterion by adding a structural layer the
prior criterion cannot reach:
...
| C-53 | C-43, C-48 | Per-phase lifecycle state and active sub-role callout lines — extends container-level incoming-state annotations to phase-granularity tracking within each container |
| C-54 | C-43, C-46 | CALIBRATE sub-transitions as distinct table rows — each sub-transition carries independent State, Gate, Contamination Risk, and Isolation Purpose columns, making the two-event CALIBRATE commitment structurally irreversible at schema level |
| C-55 | C-34, C-40 | Extended competitor surfaces 6 and 7 — Phase 8 B-00 reference row labeled with competitor name (BUILD interior anchor) and document preamble competitor investigation declaration (pre-manifest preamble anchor) |

The v20 criteria (C-53, C-54, C-55) complete two structural patterns that v19 opened:

**C-53** vs. C-43: C-43 requires a per-container incoming-state annotation at the container
header — a coarse-grained record at the container boundary. C-53 requires a per-phase
callout at every phase entry within every container — a fine-grained record at the phase
boundary. The per-phase layer makes sub-role state visible at the execution unit without
requiring the reader to infer which sub-role is active from phase numbering.

**C-54** vs. C-46: C-46 requires contamination risk and isolation purpose entries for
each container boundary in the framing paragraph. The CALIBRATE container has two
state-transition events but C-46's one-boundary-per-container model represents them as
one row. C-54 separates the two CALIBRATE sub-transitions into distinct rows, giving
each its own rationale triple and making the measurement commitment irreversible at the
schema level rather than only at the narrative level.

**C-55** vs. C-34 + C-40: C-34 places the competitor name in the CLOSE results table
column label; C-40 tracks competitor status in the manifest lifecycle column. Neither
reaches the BUILD container interior (measurement phase) or the pre-manifest preamble
space. C-55 closes both gaps, saturating the full document spine with competitor anchors
from preamble entry through BUILD measurement to CLOSE discharge.
...
## Aspirational Criteria (282 points)
...
| C-53 | **Per-phase lifecycle state and active sub-role callout lines at every phase entry within each container — phase-granularity lifecycle annotation elevates C-43's container-level incoming-state annotations to per-phase state and sub-role tracking** | structure | aspirational | At every phase entry within every container, a dedicated callout line appears naming: (a) a phase identifier; (b) the current competitor lifecycle state at that phase's execution point, consistent with the container's C-43 incoming-state annotation and any sub-transitions completed within the container before this phase executes; (c) the active sub-role executing that phase. The callout must appear as a standalone labeled element at the phase entry point, not embedded in the phase body's instructional prose, consistent with C-48's standalone marker requirement. A callout naming only the lifecycle state without the active sub-role does not satisfy C-53. A callout embedded as a parenthetical in the phase heading prose does not satisfy C-53. A document satisfying C-43's container-level incoming-state annotations without per-phase callouts satisfies C-43 but not C-53. C-53 requires C-43 and C-48 as prerequisites |
| C-54 | **CALIBRATE sub-transitions expressed as distinct table rows in the lifecycle framing structure — the two-event CALIBRATE transition occupies two separate rows, each carrying independent State, Gate, Contamination Risk, and Isolation Purpose column entries** | structure | aspirational | The lifecycle framing table contains two separate rows for the CALIBRATE container: sub-transition 1 row (State = NOT YET IDENTIFIED -> IDENTIFIED, with independent Gate, Contamination Risk, and Isolation Purpose entries for the competitor-naming event) and sub-transition 2 row (State = IDENTIFIED -> IDENTIFIED AND COMMITTED, with independent Gate, Contamination Risk, and Isolation Purpose entries for the B-00 commitment event). Both rows are contiguous and grouped under the CALIBRATE container in the framing table. A framing table representing CALIBRATE as a single row with a combined state entry (NOT YET IDENTIFIED -> IDENTIFIED AND COMMITTED) does not satisfy C-54 even if the cell text acknowledges both sub-transitions. The two-sub-transition separation must be visible in the table schema. C-54 requires C-43 and C-46 as prerequisites |
| C-55 | **Competitor identification surface saturation: the inertia competitor is tracked at two additional structural surfaces beyond the five required by C-29/C-32/C-34/C-36/C-40 — Phase 8 measurement results table B-00 reference row labeled with the competitor name (surface 6) and document preamble competitor investigation declaration before any container executes (surface 7)** | structure | aspirational | Surface 6: the measurement results table within the BUILD container's MEASURER phase carries a row labeled with the inertia competitor name as the B-00 reference row label (not a generic "Baseline" label), carrying the B-00 value as the row's data entry. Surface 7: the document preamble (appearing before the manifest and before any container body) carries a labeled declaration committing the document to an inertia competitor identification, with a scope statement naming what qualifies as an inertia competitor for this hypothesis domain. Both surfaces must carry the same competitor name as C-32's CALIBRATE COMPLETE triple and C-34's baseline column label. A document satisfying C-34's baseline column label without surface 6's BUILD-internal measurement table label does not satisfy C-55 for surface 6. A document opening with a manifest lifecycle column at NOT YET IDENTIFIED status (satisfying C-40) without a preamble commitment section before the manifest does not satisfy C-55 for surface 7. C-55 requires C-34 and C-40 as prerequisites |
...
**Design notes for C-51 through C-55 in relation to their structural families:**
...
justification, phase-granularity narration, sub-transition table, and surface saturation
(updated v20):**
...
| Phase-granularity narration layer | C-53 | Per-phase (within every container) | Phase callout: lifecycle state + active sub-role at each phase entry point |
| CALIBRATE sub-transition table | C-54 | Document-level (framing table) | Two CALIBRATE sub-transition rows with independent contamination risk and isolation purpose rationale triples |
| Extended surfaces (6 and 7) | C-55 | BUILD interior + Document preamble | Phase 8 B-00 reference row (surface 6: BUILD interior anchor) + pre-manifest competitor investigation declaration (surface 7: preamble anchor) |

C-53, C-54, and C-55 each occupy a gap the existing family cannot reach:

**C-53** vs. C-43: C-43 annotates lifecycle state at container headers (container
granularity); C-53 annotates lifecycle state and active sub-role at every phase entry
within each container (phase granularity). The phase-granularity layer makes sub-role
state visible at the execution unit without requiring the reader to infer active sub-role
from phase numbering or container-level tables.

**C-54** vs. C-46: C-46 requires contamination risk and isolation purpose entries for
each container boundary in the framing paragraph. CALIBRATE has two state-transition
events (competitor named, then B-00 committed) but C-46's one-boundary-per-container
model represents both as one row. C-54 separates the two CALIBRATE sub-transitions into
distinct rows, giving each its own rationale triple and making each commitment irreversible
at the schema level rather than only at the narrative level.

**C-55** vs. C-34 + C-40: C-34 places the competitor name in the CLOSE results table
column label; C-40 tracks competitor status in the manifest lifecycle column. Neither
reaches the BUILD container interior (measurement phase) or the pre-manifest preamble
space. C-55 closes both gaps, saturating the full document spine — from pre-manifest
preamble through DESIGN, CALIBRATE, BUILD measurement, to CLOSE discharge — with
competitor anchors at every structural node.

**Role contamination defense family — four layers plus bidirectional verification plus
BUILD sub-role split plus phase-entry sub-role declaration (updated v20):**
...
| Phase-entry sub-role declaration | C-53 | Per-phase callout names active sub-role at each phase entry — phase-granularity companion to C-38's phase-level prohibition co-location |

C-53 complements C-38 in the role contamination defense family: C-38 requires the
prohibition to be co-located at the phase execution point; C-53 requires the active
sub-role identity to be declared at the phase entry point. Together they form a
phase-level role declaration pair: identity declared at entry (C-53), prohibition
stated at the same entry point (C-38).
...
| **v20** | **C-53** | **Per-phase lifecycle state and active sub-role callout lines — extends C-43's container-level incoming-state annotations to phase-granularity tracking, naming both lifecycle state and active sub-role at each phase entry point within every container** |
| **v20** | **C-54** | **CALIBRATE sub-transition table rows — each of the two CALIBRATE sub-transitions (NOT YET IDENTIFIED -> IDENTIFIED and IDENTIFIED -> IDENTIFIED AND COMMITTED) occupies a distinct row in the framing table with independent Gate, Contamination Risk, and Isolation Purpose entries** |
| **v20** | **C-55** | **Extended competitor identification surfaces 6 and 7 — Phase 8 measurement results table B-00 reference row labeled with competitor name (BUILD interior anchor) and pre-manifest preamble competitor investigation declaration (preamble-space anchor), saturating the full document spine** |

Ceiling moves from **367 -> 382**. The R20 seed scores 367/367 under v19 (all five V-01–V-05
variations pass C-51 and C-52). The R21 seed should combine: per-phase lifecycle state and
active sub-role callout lines at every phase entry (C-53), CALIBRATE sub-transitions as
distinct table rows with independent rationale triples (C-54), and Phase 8 measurement
results table B-00 reference row plus document preamble competitor investigation declaration
(C-55), expected to reach **382**.
