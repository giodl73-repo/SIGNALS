Written to `simulations/quest/rubrics/scout-size-rubric-v5-2026-03-17.md`.

---

**Two new aspirational criteria added (C-20 through C-21):**

| ID | Pattern source | What it checks |
|----|----------------|----------------|
| **C-20** | V-02 R4 "Comprehensive bilateral closure" | Every canonical home section is surrounded — ALL adjacent sections carry prohibition rules in all directions, not just at least one. C-19 minimum pass (one lock) vs. full containment mesh. |
| **C-21** | V-01 + V-02 R4 structural gap — C-15 fails in both | The pre-flight gate (C-18) elicits a preliminary hypothesis commitment before any analysis section opens, making C-15 structurally anchored rather than freestanding. Passing C-18 and C-15 independently does not satisfy C-21. |

**What makes these genuinely new (not redundant with v4):**

- C-20 is not a refinement of C-19. C-19 checks whether at least one adjacent section is closed. C-20 checks whether every adjacent section for every canonical home is closed. V-01 R4 passes C-19 (CONFIDENCE closed against unknowns) and fails C-20 (SURFACE AREA and COMPLEXITY carry no prohibition against scope exclusions). The distinction is one lock vs. a complete mesh.

- C-21 is not a refinement of C-18 or C-15. C-18 checks that a structural gate exists. C-15 checks that a preliminary hypothesis exists. C-21 checks that the hypothesis is elicited *inside* the gate — the site of the commitment. Both V-01 and V-02 pass C-18 and fail C-15 because the hypothesis has no structural anchor; an output that passes both independently still fails C-21 if the hypothesis is placed outside the gate.

**Scoring model:** aspirational denominator bumped 11 → 13. Max composite unchanged at 100.
plicitly states 0-1 with reasoning.
