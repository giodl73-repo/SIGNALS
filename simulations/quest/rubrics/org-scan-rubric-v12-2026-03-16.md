Reading the R11 scorecard for structural differentiators beyond the existing 44 criteria — one distinct excellence signal emerges explicitly from the V-02 note:

**V-02** — *table role attribution*: "Produced by: ROLE" annotation under each table header in the SCHEMA DECLARATION (e.g., "*Produced by: PREDICTOR before any scan evidence is collected.*"); the schema becomes self-documenting about which role in the phase sequence is responsible for producing each output artifact, rather than requiring that responsibility to be inferred from the instruction sequence. The V-02 note calls this out explicitly: "This is not captured by any existing criterion."

That yields C-45. Aspirational set grows to 45 criteria (raw ceiling 90, cap unchanged at 10).

---

# Scoring Rubric: org-scan (v12)

## Changes from v11

One new aspirational criterion added from R11 excellence signal. Aspirational band expands to 45 criteria.

**Scoring note**: With 45 aspirational criteria at 2 pts each the aspirational ceiling is 90 pts. Cap on aspirational contribution remains 10. A variation scoring all 45 still earns 10.

| New Criterion | Source Signal |
|---|---|
| C-45 | Table role attribution from V-02: each table definition in the schema declaration carries a production attribution annotation (e.g., "*Produced by: PREDICTOR before any scan evidence is collected.*") naming the role responsible for producing that table, making the schema self-documenting about role responsibility for each output artifact rather than requiring inference from the instruction sequence |
