Written to `simulations/quest/rubrics/org-review-rubric-v16-2026-03-17.md`.

---

**v16 summary:**

**New criteria: C-44, C-45, C-46, C-47** (5 pts each, aspirational)

Extracted from R17 candidate patterns confirmed across the five R17 variants. All five R17 variants
scored 265/265 under v15 (perfect on C-41 through C-43); the four new criteria differentiate them.

| Criterion | Pattern | Variants that PASS |
|-----------|---------|-------------------|
| C-44 | TABLE FORMAT CONTRACT -- Unified Output Schema Pre-declaration | V-02, V-04, V-05 |
| C-45 | STATUS QUO FRAMING CONTRACT -- Named §0-CID Artifact Registry | V-03, V-05 |
| C-46 | §0-CID Four-Axis Pipeline Threading | V-05 |
| C-47 | Dual-Path Contract Enforcement (Schema Layer + Behavioral) | V-04, V-05 |

**C-44 chain position:** C-44 is the schema-layer meta-contract that co-locates the output table
schemas for C-42 (F-ID in finding table), C-43 (§0 Grounding in CH-ID table), C-40/C-41 (NH
Dimension Score sub-table), and C-19 (gate ledger row) in a single TABLE FORMAT CONTRACT block
before any §§ behavioral contracts. Prior versions declared schemas scattered across their respective
§§ contracts (§5 for CH-ID table, §9 for finding table, §15 for NH sub-tables). C-44 elevates all
schema declarations to a single structural block that is the single source-of-truth for output
format. §§ contracts reference rather than redeclare schemas.

**C-45 chain position:** C-45 extends C-43's backward-traceability contract. C-43 requires the §0
grounding column to exist in the CH-ID table (populated at challenge registration time). C-45
requires the column values to be named §0-CID artifact identifiers from a pre-registered STATUS QUO
FRAMING CONTRACT inventory, not generic preamble §§ section citations. C-43 PASS with "§1
DISPOSITION CONTRACT" as the §0 Grounding value does not satisfy C-45; C-45 requires "§0-C03
current deployment overhead is acceptable" from a named registry.

**C-46 chain position:** C-46 extends C-45's registry creation into exhaustive pipeline consumption.
C-45 requires named §0-CID artifacts to exist and be cited in the CH-ID §0 Grounding column.
C-46 requires the same §0-CID values to thread through three additional structural positions: NH
Dimension Table Column A initialization (the status-quo baseline scores derive from §0-CID claims),
BRACKET CLOSING OVERRIDE justification (the override names the §0-CID commitment being superseded),
and PRIMARY DRIVER attribution language (§0-CID values inform attribution when the no-build
argument drives the disposition). C-45 establishes the registry; C-46 requires it to be exhaustively
consumed at all four structural positions where the no-build state is referenced.

**C-47 chain position:** C-47 formalizes the two-layer enforcement architecture that V-04 and V-05
demonstrated. C-44 provides the schema layer (TABLE FORMAT CONTRACT defines what each field is and
its structural requirements). C-47 requires that each critical structured-field criterion (at minimum
C-42 F-ID and C-43 §0 Grounding) also has an independent behavioral §§ contract (defining when
the field must be populated and what constitutes a violation). The two layers are independent: an
auditor can verify compliance via either the schema or the behavioral contract without reading the
other. C-44 can pass with schema declarations only; C-47 requires a behavioral contract alongside
every schema declaration for critical field criteria.

**Score changes:**
- Aspirational: 35 -> 39 (criteria count C-09 through C-47)
- Max aspirational pts: 39 x 5 = 195
- Max: 265 -> 285
- Gold threshold: 230 -> 250

**R17 ranking under v16:**

| Rank | Variant | Score | New criteria PASS |
|------|---------|-------|-------------------|
| 1 | V-05 | **285/285** | C-44 + C-45 + C-46 + C-47 |
| 2 | V-04 | **275/285** | C-44 + C-47 |
| 3 (tie) | V-02 | **270/285** | C-44 |
| 3 (tie) | V-03 | **270/285** | C-45 |
| 5 | V-01 | **265/285** | (none) |

**R17 score computation (baseline = all R17 variants at 265/265 under v15):**

| Variant | C-44 | C-45 | C-46 | C-47 | Delta | v16 Score |
|---------|------|------|------|------|-------|-----------|
| **V-05** | PASS (+5) | PASS (+5) | PASS (+5) | PASS (+5) | +20 | **285/285** |
| **V-04** | PASS (+5) | FAIL (0) | FAIL (0) | PASS (+5) | +10 | **275/285** |
| **V-02** | PASS (+5) | FAIL (0) | FAIL (0) | FAIL (0) | +5 | **270/285** |
| **V-03** | FAIL (0) | PASS (+5) | FAIL (0) | FAIL (0) | +5 | **270/285** |
| **V-01** | FAIL (0) | FAIL (0) | FAIL (0) | FAIL (0) | +0 | **265/285** |

**C-44 PASS (V-02, V-04, V-05):** TABLE FORMAT CONTRACT block declared before all §§ contracts.
Block pre-declares schemas for: finding table (F-ID column), CH-ID challenge table (§0 Grounding
column), NH Dimension Score sub-table, gate ledger row. §§ behavioral contracts reference the TABLE
FORMAT CONTRACT schemas rather than declaring them inline.

**C-44 FAIL (V-01, V-03):** Schemas for finding table, CH-ID table, and NH sub-tables are declared
inline within their respective §§ behavioral contracts. No unified TABLE FORMAT CONTRACT block.

**C-45 PASS (V-03, V-05):** STATUS QUO FRAMING CONTRACT registers named §0-CID artifacts
(e.g., §0-C01, §0-C02...) as specific no-build strength claims. CH-ID §0 Grounding column cites
§0-CID identifiers rather than generic preamble §§ section numbers. Grounding is richer and
artifact-specific.

**C-45 FAIL (V-01, V-02, V-04):** CH-ID §0 Grounding column exists (C-43 PASS) but cites generic
preamble §§ section references ("§1 DISPOSITION CONTRACT"). No §0-CID artifact registry declared.

**C-46 PASS (V-05):** §0-CID values thread through all four axes: (a) CH-ID §0 Grounding citations,
(b) NH Dimension Table Column A initialized from §0-CID claims, (c) BRACKET CLOSING OVERRIDE
justification names the §0-CID commitment being superseded, (d) PRIMARY DRIVER attribution cites
§0-CID values. Execution closing note confirms complete four-axis data flow.

**C-46 FAIL (V-01, V-02, V-03, V-04):** §0-CID values appear in fewer than four structural axes.
V-03 and V-05 have §0-CID registry (C-45 PASS) but V-03 threads only axis (a). V-04 has no §0-CID
registry (C-45 FAIL) making C-46 vacuous for V-04 scoring purposes; scored as FAIL for v16
ranking clarity.

**C-47 PASS (V-04, V-05):** C-42 F-ID and C-43 §0 Grounding each have two independent enforcement
paths: (1) TABLE FORMAT CONTRACT schema declaration + (2) dedicated behavioral §§ contract. The
schema declares what the field is; the §§ contract declares when and how it must be populated and
what constitutes a violation. Two independent declarations prevent single-path omission.

**C-47 FAIL (V-01, V-02, V-03):** V-02 declares C-42 and C-43 schemas in TABLE FORMAT CONTRACT
only, with no independent behavioral §§ contracts. V-01 and V-03 have behavioral §§ contracts only,
with no TABLE FORMAT CONTRACT schema layer (C-44 FAIL makes C-47 vacuous; scored as FAIL for
ranking clarity).

Next round design target: V-04+V-03 hybrid -- TABLE FORMAT CONTRACT (C-44) + §0-CID registry
(C-45) + dual-path enforcement (C-47) = 280/285 without C-46. Full 285 requires adding §0-CID
four-axis threading (C-46).

No retroactive score changes on R1--R16. All R16 v15 scores carry forward unchanged.
...
## Aspirational Criteria -- 195 pts (structural integrity, auditability, automation)
...
aspirational_pts =  aspirational_pass    *  5   # 5 pts each, max 195
composite = essential_pts + recommended_pts + aspirational_pts
```

**Maximum**: 60 + 30 + 195 = **285 pts**

### Score Bands

| Band | Condition | Composite |
|------|-----------|-----------|
| Gold | all 5 essential pass | >= 250 |
| Strong | all 5 essential pass | 90-249 |
| Partial | <= 4 essential pass | any |
| Minimal | <= 3 essential pass | any |

### Paths to Gold (250 pts)

| Path | Essential | Rec | Asp passed | Composite |
|------|-----------|-----|------------|-----------|
| Full rec + 32 asp | 5/5 | 3/3 | 32/39 | 60+30+160 = 250 |
| 2 rec + 34 asp | 5/5 | 2/3 | 34/39 | 60+20+170 = 250 |
| 1 rec + 36 asp | 5/5 | 1/3 | 36/39 | 60+10+180 = 250 |
| 0 rec + 38 asp | 5/5 | 0/3 | 38/39 | 60+0+190 = 250 |

### Round 11 Calibration (retroactive, v10 -> v16)

All R11 variants: C-33/C-34/C-35 absent -> C-36 through C-47 all vacuous or FAIL.
No retroactive score change; all R11 v11 scores carry forward unchanged.

### Round 12 Calibration (retroactive, v11 -> v16)

C-36/C-37/C-38/C-39 status same as v13 assessment. C-40/C-41: vacuous (C-39 absent). C-42/C-43:
not present. C-44 through C-47: C-42/C-43 absent -> C-44/C-45/C-46/C-47 all vacuous or FAIL.
No retroactive score change.

### Round 13 Calibration (v11 -> v16)

No change from v15 calibration. C-44 through C-47: C-42/C-43 FAIL in all R13 variants ->
C-44/C-45/C-46 all FAIL or vacuous; C-47 vacuous (C-44 not active). No score change.

### Round 14 Calibration (v13 -> v16)

No change from v15 calibration. C-44 through C-47: C-42/C-43 FAIL in all R14 variants ->
no contribution. No score change.

### Round 15 Calibration (v14 -> v16)

No change from v15 calibration. C-44 through C-47: C-42/C-43 not present in any R15 variant ->
C-44/C-45/C-46 all FAIL; C-47 vacuous (C-44 not active). No score change.

### Round 16 Calibration (v15 -> v16)

Baseline = R16 v15 scores. C-44/C-45/C-46/C-47 are the open criteria.

| Variant | v15 | C-44 | C-45 | C-46 | C-47 | v16 |
|---------|-----|------|------|------|------|-----|
| **V-04** (v1-FL) | 260 | FAIL (0) | FAIL (0) | vac (0) | vac (0) | **260/285** |
| **V-05** (v1-NL) | 260 | FAIL (0) | FAIL (0) | vac (0) | vac (0) | **260/285** |
| **V-01** (v1-NH) | 255 | FAIL (0) | FAIL (0) | vac (0) | vac (0) | **255/285** |
| **V-02** (v1-FID) | 255 | FAIL (0) | FAIL (0) | vac (0) | vac (0) | **255/285** |
| **V-03** (v1-LC) | 255 | FAIL (0) | FAIL (0) | vac (0) | vac (0) | **255/285** |

C-44/C-45: R16 variants do not declare TABLE FORMAT CONTRACT or STATUS QUO FRAMING CONTRACT
blocks. All FAIL. C-46 vacuous (C-45 inactive). C-47 vacuous (C-44 inactive). No score change
from v15 to v16 for R16 variants.

### Round 17 Calibration (v15 -> v16)

Baseline = all R17 variants at 265/265 under v15. C-44/C-45/C-46/C-47 differentiate R17.

| Variant | v15 | C-44 | C-45 | C-46 | C-47 | Delta | v16 |
|---------|-----|------|------|------|------|-------|-----|
| **V-05** | 265 | PASS (+5) | PASS (+5) | PASS (+5) | PASS (+5) | +20 | **285/285** |
| **V-04** | 265 | PASS (+5) | FAIL (0) | vac (0) | PASS (+5) | +10 | **275/285** |
| **V-02** | 265 | PASS (+5) | FAIL (0) | vac (0) | FAIL (0) | +5 | **270/285** |
| **V-03** | 265 | FAIL (0) | PASS (+5) | FAIL (0) | vac (0) | +5 | **270/285** |
| **V-01** | 265 | FAIL (0) | FAIL (0) | vac (0) | vac (0) | +0 | **265/285** |

**C-44 PASS (V-02, V-04, V-05):** TABLE FORMAT CONTRACT block declared before all §§ contracts.
Block pre-declares schemas for finding table (F-ID), CH-ID table (§0 Grounding), NH Dimension
Score sub-table, gate ledger row. §§ behavioral contracts reference rather than redeclare schemas.

**C-44 FAIL (V-01, V-03):** No TABLE FORMAT CONTRACT block. Schemas declared inline within their
respective §§ behavioral contracts.

**C-45 PASS (V-03, V-05):** STATUS QUO FRAMING CONTRACT registers named §0-CID artifacts before
§§ contracts. CH-ID §0 Grounding column values cite §0-CID identifiers. Grounding is named and
artifact-specific, not generic §§ section references.

**C-45 FAIL (V-01, V-02, V-04):** CH-ID §0 Grounding column exists (C-43 PASS) but values cite
generic preamble §§ section references. No §0-CID artifact registry declared.

**C-46 PASS (V-05):** §0-CID values thread through all four axes. Execution closing note confirms
complete four-axis data flow: "§0-CID values flow through: (a) CH-ID §0 Grounding citations,
(b) NH Dimension Table Column A initialization, (c) BRACKET CLOSING OVERRIDE justification when
invoked, (d) PRIMARY DRIVER attribution language."

**C-46 FAIL (V-03):** §0-CID registry present (C-45 PASS) but §0-CID values appear only in
CH-ID §0 Grounding column (axis a). Axes (b), (c), (d) absent. Threading is partial.

**C-46 vacuous (V-01, V-02, V-04):** C-45 FAIL or vacuous -> C-46 vacuous. Scored 0.

**C-47 PASS (V-04, V-05):** Both C-42 F-ID and C-43 §0 Grounding have two independent enforcement
paths: TABLE FORMAT CONTRACT schema + independent behavioral §§ contract. V-04: TABLE FORMAT
CONTRACT declares F-ID and §0 Grounding schemas; §9 behavioral contract independently declares
F-ID requirement with violation conditions; §10 behavioral contract independently declares
§0 Grounding requirement. Two paths. V-05: TABLE FORMAT CONTRACT + §9/§10 + STATUS QUO FRAMING
CONTRACT audit requirement. Three paths (exceeds requirement).

**C-47 FAIL (V-02):** TABLE FORMAT CONTRACT present (C-44 PASS) but C-42/C-43 field requirements
are declared in the TABLE FORMAT CONTRACT only; no independent behavioral §§ contracts add
violation conditions or downstream-reference obligations beyond the schema.

**C-47 vacuous (V-01, V-03):** C-44 FAIL -> C-47 vacuous. Scored 0.
...
C-44 no vacuous condition -- FAIL if unified schema block not present.
C-45 vacuous (C-43 inactive) = 0, not partial.
C-46 vacuous (C-45 inactive) = 0, not partial.
C-47 vacuous (C-44 inactive) = 0, not partial.
