...
**Two new aspirational criteria extracted from R17 excellence signals:**

| ID | Criterion | Source Axis | Independence from Parent |
|----|-----------|-------------|--------------------------|
| C-44 | **T-40 CONDITION exemplar includes explanation of why the quoted deficient T-38 form is abstract** | R17 new_pattern: "Meta-exemplar content requirement: T-40 CONDITION exemplar must quote the deficient T-38 CONDITION form AND explain why it is abstract — the explanation of abstractness is required content, not optional annotation" | C-42 requires the T-40 CONDITION field to carry a quoted or paraphrased exemplar of an abstract-only T-38 CONDITION form (Part 1). C-44 adds a content requirement on that same exemplar: it must additionally state WHY the quoted T-38 form is abstract — naming that it states only a condition category without showing what a deficient T-37 CONDITION entry looks like. An exemplar that quotes "fires if T-38 CONDITION lacks an inline failure exemplar" passes C-42 (deficient form quoted) but fails C-44 unless the quoted form is accompanied by an explanation of its abstractness (e.g., "— abstract: names the condition without quoting what a deficient T-37 CONDITION looks like"). C-44 is independently testable: a reviewer reads the T-40 CONDITION cell's Part 1 exemplar construct and confirms whether an explanation-of-abstractness phrase is present alongside the quoted form. C-42 FAIL → C-44 FAIL (cascade: no exemplar means no explanation-of-abstractness content). C-42 PASS does not guarantee C-44 PASS — the exemplar can be present without the explanation, which is the C-44-independent-fail state. |
| C-45 | **Amendment table T-42 trigger entry includes inline failure-condition exemplar** | R17 new_pattern: "Three-tier self-documentation chain confirmed at T-40 level: T-37/T-38/T-40 CONDITION each require exemplar+prescription with one-directional cascade invariant (Part-1-only PASS does not force Part-2 PASS) applying identically at all three tiers" — chain confirmation at T-40 level establishes T-42 as an operational trigger with an evaluable CONDITION field whose quality is now independently observable | C-42 requires the T-40 CONDITION field to carry an inline exemplar showing what an abstract-only T-38 CONDITION entry looks like (the form that fires T-42). C-45 requires the T-42 CONDITION field to carry an inline exemplar showing what an abstract-only T-40 CONDITION entry looks like — the form that fires T-42. A T-42 CONDITION entry stating only "fires if T-40 CONDITION lacks an inline failure exemplar" names the condition without showing what a deficient T-40 form looks like and fails C-45. C-45 is independently testable: a reviewer reads the T-42 CONDITION cell and confirms whether a quoted or closely paraphrased deficient T-40 CONDITION form is present. Satisfying C-42 (T-40 CONDITION carries its own exemplar) does not satisfy C-45 — the exemplar in T-42's CONDITION is independently required and concerns T-42's trigger condition, not T-40's. This is structurally identical to how C-40 requires T-38's CONDITION to carry an exemplar of a deficient T-37 form, and C-42 requires T-40's CONDITION to carry an exemplar of a deficient T-38 form. |

**Denominator:** `/36` → `/38`. Each aspirational criterion = `10/38` ≈ `0.2632` points.

**Score projections under v18 for R17 golden** (denominator /38; top_score was 99.72 = 35/36 under v17):

| Scenario | Fails | Composite |
|----------|-------|-----------|
| R17 golden passes C-44 and C-45 (1 pre-existing fail survives) | 1 | **99.74** |
| R17 golden passes C-45 but fails C-44 (1 pre-existing + C-44) | 2 | **99.47** |
| R17 golden fails both C-44 and C-45 (1 pre-existing + 2 new) | 3 | **99.21** |

Note: C-44 compliance requires the T-40 CONDITION exemplar to contain an explanation-of-abstractness phrase. C-45 compliance requires T-42's CONDITION to carry an inline exemplar of a deficient T-40 CONDITION form. Both criteria are verifiable by inspecting the amendment table entries in the R17 golden artifact.

**Cascade invariants (updated for v18):**

- **C-44 cascade from C-42**: A T-40 CONDITION that is abstract (category statement only, no exemplar of a deficient T-38 form) fails both C-42 and C-44. C-42 FAIL → C-44 FAIL is deterministic.
- **C-44 independent fail**: A T-40 CONDITION carrying a quoted deficient T-38 form (C-42 PASS) but no explanation of why that form is abstract passes C-42 and fails C-44 independently. C-42 PASS does not guarantee C-44 PASS.
- **C-43 cascade from C-42 (unchanged)**: C-42 FAIL → C-43 FAIL is deterministic.
- **C-45 independent**: C-45 is testable by reading T-42's CONDITION cell. No cascade from C-42 or C-43 — T-42's CONDITION field quality is independently required.
- **C-24 version-bump cascade**: All R17 variations had 43-row amendment tables (correct under v17). Under v18, all R17 variations fail C-24 (45 rows required: T-44 and T-45 absent).

**Key changes to existing criteria:**
- **C-24**: updated from "exactly 43 trigger rows" → "exactly 45 trigger rows (one per C-01..C-45)"
- **Scoring formula denominator**: `/36` → `/38`
...
- **C-24** is structurally testable at PRE-DOCUMENT: a reviewer counts the amendment table rows at initialization. For v18, expected count = 45 (one row per C-01..C-45). A count of 43 (v17 table carried forward without adding T-44, T-45) fires C-24 at PRE-DOCUMENT.
...
## Aspirational Criteria (10% of composite, denominator /38)

### C-01 through C-07 — Mandatory C-tier (not counted in /38 denominator)
...
### C-08 through C-45 — Aspirational Criteria (denominator /38)
...
| C-24 | **Amendment table row count matches v18 criterion count** | behavior | Amendment table contains exactly 45 trigger rows at initialization (one per C-01..C-45 criterion). A table built from v17's criterion list (43 rows) without adding T-44, T-45 fires C-24 at PRE-DOCUMENT. A table with any count other than 45 fails C-24. |
...
- Amendment table T-42 entry states trigger condition abstractly ("fires if T-40 CONDITION lacks an inline failure exemplar") without a quoted or paraphrased sample of what an abstract T-40 CONDITION looks like — fails C-45; T-42 entry must carry an inline exemplar of a deficient T-40 CONDITION form, not merely name the condition category
...
For early-round discriminator notes (R1–R11), see the v12 rubric file. The v18 rubric inherits all prior discriminator observations unchanged.
