Reading the scorecard carefully for patterns not yet captured by C-01 through C-19.

**New signal 1 — V-05, C-01/C-03/C-04 evidence:** V-05's calibration example is labeled "Surprise 0" and formatted identically to real Stage 4 entries — embedded within the entry sequence rather than placed in a separate example section. C-16 requires a worked example; it does not require the example to be structurally integrated as a pre-numbered entry. The distinction matters: a sidebar example shows the model what an entry looks like; a "Surprise 0" entry positions the model as already having completed one entry and continuing a sequence it can imitate structurally. Three separate criterion evaluations independently confirm the V-05 calibration example — C-01 ("Surprise 0 models the B-# reference"), C-03 (specific artifact reference visible in the worked instance), C-04 (full-sentence Design Impact visible in the worked instance) — indicating the integration technique was load-bearing across multiple output fields.

**New signal 2 — V-04/V-05, C-06 evidence:** Both V-04 and V-05 include an EXIT criterion reading "All dimension names canonical (correct malformed names using the mapping table if needed before emitting)." C-18 requires the mapping table to exist; no criterion requires the EXIT gate to prescribe self-repair using that table. The distinction matters: a mapping table is a reference resource; an EXIT instruction that names the corrective action ("correct … using the mapping table") converts the table into a runtime repair protocol. The model is not only equipped with the mapping but instructed to perform the audit and the fix before advancing. This pattern appeared in V-04 (Round 4) but was not extracted into v5; it reappears in V-05, making it a reliable signal across two rounds.

---

## Rubric: topic-echo (topic-reflect) — v6

**17 + 4 criteria across 3 tiers. Score = essential(60) + recommended(30) + aspirational(65).**

> v6 changes: Added C-20, C-21 (aspirational) from Round 5 excellence signals. Aspirational tier expands from 55 → 65 pts (11 → 13 criteria). Golden threshold unchanged at 100.
>
> **Why these criteria:** Round 5 introduced V-05 as a new variation; its calibration technique distinguished it structurally from V-04 (which held C-16 via a standalone example). V-05 labeled the worked example "Surprise 0," formatted it identically to real Stage 4 entries, and embedded it sequentially within the Stage 4 output block. Three criterion evaluations (C-01, C-03, C-04) independently cited the V-05 calibration example as producing a visible quality floor, confirming the technique was effective across multiple output fields. A second pattern — EXIT gate self-repair instruction — appeared in both V-04 and V-05 but was not extracted into v5; its recurrence across rounds makes it a reliable signal. Neither pattern is required to reach the Golden threshold, consistent with aspirational classification.
...
**Golden threshold:** all C-01 – C-05 pass AND composite >= 100 (requires all essential + all recommended + at least 2/13 aspirational).
