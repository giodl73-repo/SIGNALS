Reading the scorecard, I see two distinct excellence patterns:

1. **V-01 PASS+** cluster (C-16, C-18, C-23, C-27 all marked PASS+): The insight is *schema-as-sole-truth* — not just that schemas exist, but that phase bodies contain **zero inline prose**, eliminating the dual-truth channel where prose and schema can diverge on revision.

2. **V-02** augments C-42/C-43/C-44 with a "Stakeholder Most Harmed" column in the INERTIA REGISTER — making inertia vulnerability concrete (a named role) rather than abstract (an ID) at every enforcement point.

New criteria: **C-45** (schema-only phases) and **C-46** (stakeholder-named inertia). Denominator 36 → 38.

---

```markdown
# topic-new Rubric — v14

> Bumped from v13 (2026-03-14). Two new aspirational criteria from Round 15 excellence signals.
...
**Aspirational denominator**: 36 → 38
...
**Summary of changes v13 → v14:**

| Delta | Detail |
|-------|--------|
| +C-45 | Phase bodies contain zero inline prose constraints — closes schema-drift channel identified by V-01 PASS+ cluster on C-16/C-18/C-23/C-27 |
| +C-46 | INERTIA REGISTER "Stakeholder Most Harmed" column + role name travels with all IR-NN citations — closes role-abstraction axis identified by V-02 ES augmentation |
| Denominator | 36 → 38 |
| Formula | `aspirational_pass / 38 × 10` |
