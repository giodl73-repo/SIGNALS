Written to `simulations/quest/rubrics/flow-resilience-rubric-v13-2026-03-15.md`.
...
**Scoring delta**: uncapped max 60 → 66 (33 criteria × 2). Cap unchanged at 10.
...
**Open for R13:** C-40 PASS for V-05 (one prohibition clause away from 66/66); C-38 PASS for V-01–V-04 (bidirectional ID-level matrix absent); C-15/C-41 PASS for V-02/V-03/V-04 (Gate 1 quality postcondition absent). No fully uncracked criteria remain — C-15's persistent PARTIAL block resolves in R12 via C-41.

**Design rationale highlights:**
- **C-39** is orthogonal to both C-18 (entry conditions) and C-31 (pre-analysis declaration) — a constraint can be declared upfront, stated as an entry condition, AND confirmed at gate-entry time; only C-39 requires the last step
- **C-40** is why V-05 scores 65/66 not 66/66 — its CLOSE has the positive confirmation but lacks "no paraphrase, no independent calibration"; positive and prohibition clauses are independently load-bearing  
- **C-41** closes the C-15 quality gap at the enforcement layer: C-15 requires the field and examples to exist; C-41 requires the CLOSE to verify their intent was honored — a description-absence argument satisfies C-15's presence check but fails C-41's quality check
1/V-03 carry copy instructions ("copy threshold expression from Gate 2") rather than
  confirmable checkpoints, scoring FAIL.
- **C-40** closes the semantically-equivalent loophole that positive CLOSE conditions cannot
  cover -- "verbatim Gate 2 threshold expression" satisfies C-36's "same threshold expression as
  Trigger Condition," but a paraphrase ("equivalent threshold condition") passes the positive check
  while introducing interpretive drift. "No paraphrase, no independent calibration" in the CLOSE
  explicitly forecloses the paraphrase form. V-05's CLOSE confirms "(identity assertion satisfied)"
  without a prohibition clause, scoring PARTIAL -- demonstrating that the positive confirmation
  and the prohibition clause are independently load-bearing.
- **C-41** closes the quality-vs-presence gap at Gate 1 closure -- C-15 requires the DS Primitive
  cited field and VALID/INVALID examples in the template. A Gate 1 CLOSE that checks "DS Primitive
  cited: field non-empty" verifies presence only; a description-absence argument ("the topic does
  not describe a distributed cache") satisfies presence while failing the architecture-basis
  standard. V-01/V-05 name the prohibited form in the CLOSE postcondition -- "architecture basis,
  not description absence" -- making quality a gate-closure requirement rather than an honor system
  enforced only by field design. Also resolves C-15's persistent PARTIAL block: with the Gate 1
  quality postcondition in place, V-01 and V-05 achieve C-15 PASS in R12.

**C-39 -- Gate-Open Precondition Acknowledgment Checkpoint (E-25)**
- Signal: V-02's Gate 2b OPEN precondition: `[ ] Identity assertion acknowledged: chaos activation
  boundary = Gate 2 threshold expression for each scenario, verbatim -- not a derivative`. V-04's
  Gate 2b OPEN: "Identity assertion acknowledged: Trigger Condition Reference = verbatim Gate 2
  threshold expression -- identical, not a derivative." Both require the analyst to confirm the
  identity constraint before any gate-2b column filling begins.
- The gap it closes: Column Contracts and pre-analysis declarations (C-31) describe constraints
  before execution starts. An analyst familiar with the template can open a gate without
  re-confirming the constraint that governs it. The acknowledgment checkpoint closes this gap:
  the confirmation must occur at Gate OPEN time, independently for each gate invocation. C-18
  requires named entry conditions (which prior gates must be CLOSED); C-39 requires a named
  constraint acknowledgment (which operational rule must be confirmed). These are orthogonal: a
  gate can have both entry conditions and acknowledgment checkpoints.
- Pass requires: At least one gate OPEN precondition block contains an explicit named
  acknowledgment statement confirming a specific structural constraint (identity assertion, coverage
  minimum, quality standard, or equivalent). The acknowledgment must name the constraint it
  confirms -- not a generic "ready to proceed" or status check. It must be positioned in the Gate
  OPEN precondition block (not in a column Fill Requirement, preamble, or row descriptor). Checkbox
  form (`[ ]`) is preferred; named prose acknowledgment in the Gate OPEN block satisfies this
  criterion. A copy instruction or fill guidance is not an acknowledgment checkpoint.

**C-40 -- Gate-Close Prohibition Clause (E-26)**
- Signal: V-02's Gate 2b CLOSE: "Every Trigger Condition Reference contains verbatim Gate 2
  threshold expression (identity assertion satisfied -- no paraphrase, no independent calibration)."
  V-04's Gate 2b CLOSE carries the same structure. V-05's Gate 2b CLOSE: "(identity assertion
  satisfied)" -- the positive confirmation is present but the prohibition clause is absent.
- The gap it closes: Standard gate CLOSE conditions verify presence ("every X has Y"). A CLOSE
  that verifies presence can be satisfied while a prohibited variant exists -- a threshold
  expression semantically equivalent to the Gate 2 specification but differently phrased satisfies
  the positive assertion while remaining an unauthorized paraphrase. The prohibition clause
  explicitly verifies absence. Prohibition clauses are not redundant with criterion Pass conditions:
  a Pass condition defines success; a CLOSE prohibition clause verifies failure mode absence in the
  produced output even where the nominal success condition is satisfied.
- Pass requires: At least one gate CLOSE condition block contains an explicit prohibition clause
  ...
  specific failure mode -- generic language ("no errors") does not satisfy this criterion. The
  prohibition must appear in the gate CLOSE block, not only in a Pass condition or column Fill
  Requirement.

**C-41 -- Impossibility Argument Quality Gate Postcondition (E-27)**
- Signal: V-01's Gate 1 CLOSE postcondition: "Every Argued-Impossible has DS Primitive cited:
  field completed (architecture basis, not description absence)." V-05's Gate 1 CLOSE: identical
  structure. V-02/V-03/V-04: advisory prose about DS Primitive naming but no Gate 1 CLOSE
  postcondition auditing argument quality.
- The gap it closes: C-15 requires the template to carry the DS Primitive cited field with inline
  VALID/INVALID examples. A gate CLOSE condition that checks only "DS Primitive cited: field is
  non-empty" verifies presence without quality: a description-absence argument ("the topic does
  not describe a distributed cache") fills the field without meeting the architecture-basis
  standard. The quality postcondition names the prohibited form in the CLOSE -- "not description
  absence" -- making the architecture-basis standard a gate-closure requirement rather than relying
  solely on field design and inline examples. C-15 requires the quality bar to be defined in the
  template; C-41 requires it to be enforced at gate-close time.
- Pass requires: The Gate 1 (or equivalent confidence triage gate) CLOSE postcondition explicitly
  verifies both (a) the DS Primitive cited field is completed and (b) the basis is architecture-
  grounded, naming the prohibited basis type ("description absence" or equivalent) by name in the
  same postcondition statement. The postcondition must appear in the CLOSE block. Presence-only
  CLOSE conditions ("DS Primitive cited: field non-empty") do not satisfy this criterion.

**Scoring delta**: aspirational cap unchanged at 10; uncapped max rises from 60 -> 66
(33 criteria x PASS=2).

**R12 discrimination under v13 (uncapped aspirational tiebreak; all composite = 100/100):**

| Rank | Variation | C-15 | C-36 | C-38 | C-39 | C-40 | C-41 | Uncapped Asp. |
|------|-----------|------|------|------|------|------|------|--------------|
| 1 | **V-05** | PASS | PASS | PASS | PASS | PARTIAL | PASS | 65/66 |
| 2 (tie) | **V-02** | PARTIAL | PASS | PARTIAL | PASS | PASS | FAIL | 62/66 |
| 2 (tie) | **V-04** | PARTIAL | PASS | PARTIAL | PASS | PASS | FAIL | 62/66 |
...
- Signal: V-05's Column Contract for the chaos gate explicitly states that the Gate 2b activation
  boundary expression IS the same threshold expression as the Trigger Condition -- an identity
  claim rather than a reference link. V-01/V-03/V-04 provide a Trigger Condition Reference that
  copies or links to the threshold without asserting identity.
- The gap it closes: A cross-reference link allows the chaos activation parameter to differ from
  the production trigger threshold by analytic choice or by drift. Without an identity assertion,
  chaos tests may execute at injection parameters that do not correspond to the production trigger
  boundary -- making chaos results non-predictive of production behavior. An identity assertion
  forecloses divergence by design.
- Pass requires: Explicit statement in the chaos gate contract (column contract, gate precondition,
  or named rule) asserting that the chaos activation boundary expression is identical to the
  Trigger Condition threshold expression -- not merely a link or reference.

**C-37 -- Observable Signal Detection Horizon (E-23)**
- Signal: V-04's Gate 7 Observability Manifest and V-05's Gate 3 Observable Signal both require
  Detection Horizon as a third named component alongside (1) named signal and (2) rationale.
  Detection Horizon defines the maximum latency within which the signal is expected to appear after
  fault injection or threshold crossing.
- The gap it closes: C-10 requires named signal + rationale -- WHAT and WHY, but not WHEN to
  declare the signal undetected. Without a Detection Horizon, signal absence is undefined: the
  operator cannot distinguish a signal not yet arrived from a signal permanently absent.
- Pass requires: Detection Horizon explicitly required as a named component in the Observable
  Signal specification. The horizon must specify a concrete time window. Generic statements
  ("should appear promptly") do not satisfy this criterion.

**C-38 -- Chaos-Observability Bidirectional Coverage Matrix (E-24)**
- Signal: V-05's named Chaos-Observability Coverage Matrix cross-referencing every chaos scenario
  (C-09) against >= 1 Observable Signal (C-10) and every Observable Signal against >= 1 chaos
  scenario. V-04 satisfies C-09 and C-10 as separate outputs but does not produce a bridging
  matrix.
- The gap it closes: C-09 and C-10 can each be independently satisfied without verifying
  bidirectional coverage. A chaos scenario that fires but produces no Observable Signal cannot be
  confirmed as detected. An Observable Signal not linked to any chaos scenario cannot be validated
  via chaos testing. Without the matrix, "dark chaos" scenarios and "untested observability" hooks
  are invisible.
- Pass requires: A named cross-reference artifact linking each chaos scenario by ID to >= 1
  Observable Signal by ID, and each Observable Signal by ID to >= 1 chaos scenario by ID. The
  matrix emits a named gap finding when either direction is uncovered.
...
Open for R12: C-38 cracked only by V-05; C-36 PASS form cracked only by V-05. Persistent
uncracked: C-15 PASS form.
...
Uncapped maximum across 33 criteria is 66; the cap rewards breadth while holding the composite
ceiling at 100.
...
| Aspirational | 10% | C-09 through C-41 | PASS=2, PARTIAL=1, FAIL=0; capped at 10 |
...
- R12 V-03 (partial on all six target criteria; no new patterns): 57/66 uncapped
  -> composite = 60 + 30 + 10 = **100** (capped)
- R12 V-01 (C-15+C-41 PASS; C-36+C-38 PARTIAL; C-39+C-40 FAIL): 60/66 uncapped
  -> composite = **100** (capped)
- R12 V-02 (C-36+C-39+C-40 PASS; C-15+C-38 PARTIAL; C-41 FAIL): 62/66 uncapped
  -> composite = **100** (capped; tied with V-04)
- R12 V-04 (C-36+C-39+C-40 PASS; C-15+C-38 PARTIAL; C-41 FAIL): 62/66 uncapped
  -> composite = **100** (capped; tied with V-02)
- R12 V-05 (C-15+C-36+C-38+C-39+C-41 PASS; C-40 PARTIAL): 65/66 uncapped
  -> composite = **100** (new uncapped ceiling: 65/66)
- Open for R13: C-40 PASS for V-05 (PARTIAL -- one prohibition clause away from 66/66);
  C-38 PASS for V-01/V-02/V-03/V-04 (bidirectional ID-level matrix absent);
  C-15/C-41 PASS for V-02/V-03/V-04 (Gate 1 quality postcondition absent).
