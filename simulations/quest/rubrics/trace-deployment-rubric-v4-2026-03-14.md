Reading the scorecard carefully to extract the two new R3 patterns before writing the updated rubric.

The R3 patterns are:
- **`gate-markers-not-load-bearing`** — template field count floors essential criteria without gate enforcement text; automation hook fields are the only structural casualty of full gate stripping
- **`c15-compression-confirms-specificity-floor`** — R2 V-04 verbosity was not load-bearing; the three C-15 structural requirements hold at single-paragraph-per-phase density

These become C-16 and C-17. Aspirational tier goes from 35 → 45 pts; total from 125 → 135.

---

```markdown
# Rubric: trace-deployment (v4)

**Skill**: `trace:deployment`
**Version**: 4 (updated from R3 scorecard — 2 excellence patterns promoted to aspirational criteria)
**Date**: 2026-03-14
**Criteria**: 17 across 4 tiers | **Total**: 135 pts
...
## Aspirational Criteria (45 pts — structural excellence)

C-09 through C-15 carried over from v3. C-16 and C-17 are new — promoted from R3 excellence
patterns.
...
| Aspirational | C-09 through C-17 (5 pts each) | 45 |
| **Total** | | **135** |
...
| v4 | +2 aspirational criteria (C-16, C-17) from R3 excellence signals. C-16: gate-free essential coverage via template field scaffolding — gates are not load-bearing; field count is the floor, with automation hook fields the only structural casualty of full gate stripping. C-17: C-15 achieved at minimum prose density — verbosity is not the C-15 floor; single-paragraph-per-phase satisfies all three structural requirements. Aspirational tier expands to 45 pts; total becomes 135. Golden threshold unchanged (>=80). |
```

---

Two notes on the new criteria:

**C-16** is intentionally strict about the pass condition direction — a prompt *with* gates doesn't fail C-16, it simply doesn't pass it. The criterion rewards the specific design decision to rely on field count alone, which R3 confirmed as structurally sufficient (with the V-01 finding that stripping hooks costs exactly 5 pts on C-10).

**C-17** tightens C-15 rather than duplicating it — C-15 establishes that prose-alone works; C-17 establishes the compression floor. A verbose C-15-passing prompt contributes 5 pts; a compressed C-15-passing prompt contributes 10. The exclusivity from C-14/C-15 carries forward: C-17 requires C-15 to pass first, so apparatus-bearing prompts are still blocked from this slot.
