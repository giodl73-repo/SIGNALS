Reading the R10 scorecard for consistent PARTIALs before writing the new criteria.

The pattern mirrors R9→R10:

| R10 PARTIAL (consistent across V-01, V-02) | What's present | What's absent |
|---|---|---|
| **C-21** | SQ citation form present | Positive structural form rule not declared; "verdict echo" error not named |
| **C-24** | Artifact state defined in decision block | Next-steps entry FORMAT does not structurally mandate the field |

Same derivation logic as R9: concept present, mechanical encoding absent → new criterion names the encoding requirement.

- **C-32**: named verdict-echo error in SQ citation template (deepens C-21)
- **C-33**: next-steps entry format declares artifact state as required slot (deepens C-24)
...
| C-32 | Verdict-echo error named in SQ citation template | The SQ answer citation template carries an explicitly named verdict-echo error — a labeled, structural prohibition identifying the specific error form where a sub-question answer restates the verdict rather than citing an artifact. The template names this error by label (e.g., `VERDICT ECHO`) and states the distinguishing form: present-tense artifact naming ("Section 3.2 lists no fallback path") is the required form; past-tense assessment restatement ("Architect found no fallback path") is the named error. A template that only prohibits restatement without naming the error ("do not restate the verdict") does not satisfy this criterion; the violation must be identifiable by type from the template definition alone. This deepens C-21: C-21 requires a positive structural signal and a named prohibition, C-32 requires the prohibition to carry a named error label so that a violation can be classified — not merely detected — at the template site. |
| C-33 | Next-steps entry format declares artifact state as required slot | The next-steps entry format carries an explicit structural declaration that the propagated artifact state field is a required slot — for example, a FORMAT block that names `/{skill-id} {topic} — {artifact state}` with `{artifact state}` marked as mandatory, not optional. A next-steps format that accepts `/{skill-id} {topic}` without structurally requiring `{artifact state}` in the entry template does not satisfy this criterion, even if the artifact state field is defined upstream in the decision block. The entry format must itself declare the artifact state slot as required, making it impossible to construct a valid entry without the propagated state. This deepens C-24: C-24 requires artifact state propagation from decision block to next-steps entry, C-33 requires the next-steps entry FORMAT to structurally mandate the field — closing the escape where propagation is conceptually present but the format slot remains optional. |
...
| C-04 Status fields written back | | |
| C-05 Next-steps list in priority order | | |
| C-06 Rule trigger named per REAL-REQUIRED | | |
| C-07 PM/Architect/Strategy evaluation shown | | |
| C-08 Tier flag respected | | |
| C-09 Coverage gap synthesis | | |
| C-10 Namespace-specific MOCK-ACCEPTED rationale | | |
| C-11 Forbidden-output enumeration for auto-rules | | |
| C-12 Zero-remaining confirmation gate | | |
| C-13 Verifiable role-step separation | | |
| C-14 Sub-question answer citation in verdict traceability | | |
...
| C-25 Dedicated contrast sub-slot in MOCK-ACCEPTED template | | |
| C-26 Role-sequence gate as contamination control | | |
| C-27 Triad DO NOT coverage (complete forbidden-output set) | | |
| C-28 Step-name anchor in forward reference | | |
...
| C-32 Verdict-echo error named in SQ citation template | | |
| C-33 Next-steps entry format declares artifact state as required slot | | |
```
...
| Source signal | PARTIAL evidence | New criterion | Deepens |
|---|---|---|---|
| C-21 PARTIAL (V-01, V-02) | "verdict echo" error not named in template | C-32 | C-21 |
| C-24 PARTIAL (V-01, V-02) | next-steps format does not mandate artifact state slot | C-33 | C-24 |

Both follow the same structural pattern as R9→R10 (C-23→C-30, C-25→C-31): concept present in a higher criterion, mechanical encoding absent → new criterion names the encoding requirement at the template/format definition site. Denominator moves 23 → 25.
