Written to `simulations/quest/rubrics/flow-resilience-rubric-v12-2026-03-14.md`.
...
| Denominator | 29 | 30 |
| New criteria | -- | C-38 |
| Formally defined | -- | C-09 through C-16 |
| R11 ceiling | 97.24 | 97.00 |
| R12 ceiling | 100.00 (V-05, 29/29) | 100.00 (V-05, 30/30) |
| Open for R13 | none | none |

**Eight criteria defined (C-09 to C-16)** — these were already in the denominator from R12; v12 writes their full definitions with pass conditions:

- **C-09** (E-22) — Per-Scenario Chaos Test Block: four-component spec (Inject / Expected Behavior / Pass Signal / Fail Signal) in a structural meta-table per row
- **C-10** (E-23) — Per-Finding Observability Hooks: Metric / Alert / Owner inline at finding granularity
- **C-11** (E-24) — Actor-Chain Notation: `client ->` / `server ->` / `operator ->` / `user ->` prefixes on at least 3 of 4 recovery stages per row
- **C-12** (E-25) — Constrained Conflict Vocabulary: canonical labels only (`last-write-wins`, `merge`, `manual-reconcile`, `reject-stale`), free-text explicitly negated
- **C-13** (E-26) — Gap-Merge Prevention: self-verifying count field ("Finding types present: ___ of 3") as model-generated output
- **C-14** (E-27) — Chaos Blocks Co-Located: anti-boundary instruction negates 2+ consolidation forms; Section Order Requirement gates each row before the next
- **C-15** (E-28) — Observability Hook Co-Located: anti-boundary instruction negates standalone observability section at finding granularity
- **C-16** (E-29) — Completeness Checklist as Output: model fills the count, making omission manifest as "2 of 3"

**One new criterion (C-38 / E-30)** — Anti-Boundary Umbrella Instruction: a single instruction closes all four structural escape forms (table split + intra-table rule + chaos consolidation + observability consolidation) simultaneously. Extends C-25's two-escape pattern to four. Denominator: 29 → 30. V-05 cracks it via its unified four-escape anti-boundary block.
it-brain / eventual-consistency conflict).
...
**Scoring formula**: `aspirational_pass/30 * 10`

**Ceiling references under v12**:
- V-05 (Round 7 best, re-scored): C-17 through C-27 PASS, C-28 through C-38 FAIL
  -> 11/30 x 10 = 3.67 -> composite = 60 + 30 + 3.67 = **93.67**
- V-05 (Round 8, all three axes, re-scored): C-17 through C-29 PASS, C-30 through C-38 FAIL
  -> 13/30 x 10 = 4.33 -> composite = 60 + 30 + 4.33 = **94.33**
- V-05 (Round 9, all three axes, re-scored): C-17 through C-31 PASS, C-32 through C-38 FAIL
  -> 15/30 x 10 = 5.00 -> composite = 60 + 30 + 5.00 = **95.00**
- V-05 (Round 10, all three axes, re-scored): C-17 through C-34 PASS, C-35 through C-38 FAIL
  -> 18/30 x 10 = 6.00 -> composite = 60 + 30 + 6.00 = **96.00**
- V-03 / V-05 (Round 11, inertia axis): C-17 through C-37 PASS; C-09 to C-16, C-38 FAIL
  -> 21/30 x 10 = 7.00 -> composite = 60 + 30 + 7.00 = **97.00** (R11 ceiling)
- V-01 (Round 12, Axis D only): C-09, C-14, C-17 through C-37 PASS; C-10 to C-13, C-15, C-16, C-38 FAIL
  -> 23/30 x 10 = 7.67 -> composite = **97.67**
- V-03 (Round 12, Axis F only): C-11, C-12, C-17 through C-37 PASS; C-09, C-10, C-13 to C-16, C-38 FAIL
  -> 23/30 x 10 = 7.67 -> composite = **97.67**
- V-02 (Round 12, Axis E only): C-10, C-13, C-15, C-16, C-17 through C-37 PASS; C-09, C-11, C-12, C-14, C-38 FAIL
  -> 25/30 x 10 = 8.33 -> composite = **98.33**
- V-04 (Round 12, Axes D + E): C-09, C-10, C-13, C-14, C-15, C-16, C-17 through C-37 PASS; C-11, C-12, C-38 FAIL
  -> 27/30 x 10 = 9.00 -> composite = **99.00**
- V-05 (Round 12, Axes D + E + F): all 30 criteria PASS
  -> 30/30 x 10 = 10.00 -> composite = **100.00** (R12 ceiling)
- Open for R13: none -- V-05 cracks all 30 criteria including C-38.
