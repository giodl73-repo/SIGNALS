...
date: 2026-03-15
version: 6
prior_version: quest-rubric-variate-R5-2026-03-15.md
---

# Rubric: quest-variate (v6)
...
**v6 changes**: Added C-28 through C-31 from Round 5 excellence signals. No essential or
recommended criteria changed. Aspirational denominator updated from /19 to /23.

---

## What changed v5 -> v6

4 new aspirational criteria (C-28 through C-31), each traceable to a specific R5 excellence signal:

| ID | Criterion | Source signal | Distinction from prior criteria |
|----|-----------|---------------|---------------------------------|
| C-28 | **Count-verifiable axis-freeze completeness gate** | E-01: "A freeze list with fewer than six entries is incomplete -- do not proceed" | C-26 requires naming each axis by name inside the generation loop. C-28 requires the freeze protocol to also declare the expected entry count as a numeric threshold, making completeness verifiable without reading the content of each entry -- a count audit rather than a name audit. |
| C-29 | **VARIATION MAP declared as authoritative read-source for the in-loop freeze protocol** | E-02: "The AXIS-FREEZE PROTOCOL in Phase 2 reads axis assignments from this table" at section level | C-27 requires standalone artifact + immutability instruction. C-26 requires per-body freeze. C-29 requires the VARIATION MAP to explicitly declare that the freeze protocol reads from it -- linking pre-generation commitment to in-loop enforcement into a traceable chain with a single source of truth. Only activated when both C-26 and C-27 are present. |
| C-30 | **Failure-condition predictions embedded in the pre-generation variation map** | E-03: `failure-condition-outcome prediction` and `failure-condition-implementation prediction` columns per variation row in standalone VARIATION MAP | C-24 requires per-variation per-criterion directional predictions (which criteria improve). C-30 requires the map to also pre-commit the expected failure signature -- which failure mode (outcome vs implementation) is expected if the hypothesis is false -- as a separately labeled column per variation. A map with criterion improvement predictions but no failure mode predictions does not satisfy C-30. |
| C-31 | **Cross-artifact axis consistency gate in the generation loop** | E-04: "Verify VARIATION MAP axis matches Phase 1 header axis before advancing to Phase 3" | C-15 requires an inline gate that names criteria. C-29 requires the freeze protocol to read from the VARIATION MAP. C-31 requires the inline gate to explicitly verify that the axis declared in the VARIATION MAP matches the axis declared in the per-variation planning header -- a consistency check across two artifacts that detects silent divergence before the body is written. Only activated when a standalone VARIATION MAP (C-27) and per-variation planning headers (C-03 format) are both present. |

**Scoring formula**: aspirational denominator bumped from `/19` to `/23`. All bands and weights unchanged.
