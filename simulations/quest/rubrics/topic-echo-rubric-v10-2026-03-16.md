...
# topic:echo Rubric — v10

**Three key changes from v9:**
- Max composite: 200 → 215
- Aspirational criteria: 22 → 25
- New relationships: C-29/C-31, C-28/C-32, C-30/C-33

**Three new criteria (C-31, C-32, C-33):**

| ID | Gate | Pattern | What it adds |
|----|------|---------|--------------|
| C-31 | C-29 | RESOLUTION PROTOCOL with named verifier role per flag type | C-29 makes repair routing visible; C-31 makes clearing role-accountable. Self-certification blocked: only the designated role (e.g., BC for PBI-Ref repairs) can attest completion. |
| C-32 | C-28 | Evidentiary tier quotation in RULES-ANCHORED-COMPLETE token | C-28 makes phase-close observable; C-32 makes the token evidentiary. Quotes the actual `[HIGH]`/`[MEDIUM]` string per rule — applies C-18's evidence-disclosure principle to phase-close tokens. |
| C-33 | C-30 | EF-INVOCATION-RECORD at Step 0 | C-30 makes the cost declaration structurally prospective; C-33 makes the invocation auditable. The record names materials consulted, signal artifacts excluded by name, and cost derivation basis — converts the role-boundary from a specification property into an observable output. |

**Round 9 progression layer:** structural commitment → auditable record. Each criterion takes something that was enforced by specification (C-29: visible routing, C-28: closure token, C-30: role boundary) and makes it observable from output alone.
ation, the token declares that all rules are aligned but a downstream reviewer must re-run the RULES-ANCHORED check to confirm which tier annotation was verified for each rule. With per-rule quotation, the token is evidentiary: the closure record shows exactly what was checked, not merely that checking occurred. This applies the same principle as C-18 (evidentiary quotation in per-entry attestations) to phase-close tokens: the closure token becomes a disclosure of evidence rather than a declaration of action. Pass requires the RULES-ANCHORED-COMPLETE token to quote the actual tier annotation string per rule (e.g., `R-01 "[HIGH]" ALIGNED, R-02 "[MEDIUM]" ALIGNED`) before emitting the distillation-phase-closed declaration. First appeared in Round 9 V-02.

**C-33 — EF-INVOCATION-RECORD at Step 0**
(behavior, gate: C-30)

C-30 requires the NO-ECHO COST declaration to be derived by a role-scoped function (EF) before any signal artifacts are read, converting the temporal commitment of C-26 into a structural one. C-33 requires that derivation to produce an EF-INVOCATION-RECORD: a discrete block immediately following the ENFORCEMENT MECHANISM DECLARATION that lists (1) pre-investigation materials consulted, (2) signal artifacts excluded by name, and (3) the cost derivation basis. Without an INVOCATION-RECORD, C-30's structural boundary is a specification requirement that is not observable from the artifact: a reviewer can confirm the EF function is scoped, but cannot verify which materials were consulted or which were excluded during Step 0. With an INVOCATION-RECORD, the boundary is made observable — the exact pre-investigation materials are named, the exclusions are enumerated, and the derivation basis is stated. The C-30 structural commitment becomes an auditable record of what the EF function saw and did not see. Pass requires the Step 0 output to include an EF-INVOCATION-RECORD block immediately after the ENFORCEMENT MECHANISM DECLARATION, naming all three elements: materials consulted, signal artifacts excluded by name, and cost derivation basis. First appeared in Round 9 V-03.
...
## What the Round 9 variations score under v10

| Variation | C-31 | C-32 | C-33 | v9 score | v10 composite | vs v9 |
|-----------|------|------|------|----------|---------------|-------|
| V-01 | **PASS** | FAIL | FAIL | 200 | 205 | +5 |
| V-02 | FAIL | **PASS** | FAIL | 200 | 205 | +5 |
| V-03 | FAIL | FAIL | **PASS** | 200 | 205 | +5 |
| V-04 | **PASS** | FAIL | **PASS** | 200 | 210 | +10 |
| V-05 | **PASS** | **PASS** | **PASS** | 200 | **215** | +15 |

V-05 is the only R9 variation that reaches the v10 ceiling. The three new criteria are three independent axes — no R9 variation holds more than two of them except V-05 (all-axes combination).
...
**Round 9 progression layer:** visibility → navigability → primacy → repairability / stakes / verifiability → auditable closure / visual sovereignty / structural provenance → **role accountability / evidentiary phase tokens / invocation observability**. Each new layer converts a structural commitment into an auditable record.

---

**Notes on Round 9 pattern:**

- C-29 made the RESOLUTION PROTOCOL visually sovereign → C-31 makes clearing role-accountable (visible routing + named verifier per flag type)
- C-28 made distillation-phase exit observable via closure token → C-32 makes the token evidentiary (observable exit + what was checked per rule)
- C-30 made the cost declaration structurally prospective via EF role-boundary → C-33 makes the invocation observable (structural boundary + invocation record of what was seen and excluded)
...
## Aspirational Criteria (weight: 125 points total, 5 pts each)
...
- Aspirational: 125 points total (25 criteria, 5 points each)
- Max composite: 215
...
- Aspirational: 1/25 pass -> 1/25 * 125 = 5
...
- Essential: 5/5 -> 60
- Recommended: 3/3 -> 30
- Aspirational: 25/25 -> 125
- **Composite = 215** -- maximum
...
CHAIN-FLAG (C-19) for the chain audit, RULES-ANCHORED-COMPLETE for the distillation audit. The
practical effect: a reviewer inspecting the completed artifact can confirm both audit phases
closed correctly without re-running either check. C-23 alone: blockage visible, success silent.
C-23 + C-28: both states visible.
...
encountering the repair routing, not finding it. The repair structure is forced into the
reviewer's attention by phrasing register alone, with no content change required.
...
of instruction. This mirrors the C-15/C-17 distinction (temporal vs structural provenance for
the PBI) applied to the cost declaration. Practical test: could a writer reconstruct the cost
declaration from signal content after observing it? C-26 alone: yes (temporal ordering
instructed but not enforced). C-26 + C-30: no (role-scope exclusion enforced).
...
that the named repair was correctly applied. The practical difference: two reviewers with
different role assignments will not both certify the same CHAIN-FLAG resolution -- only the
named verifier role can close the flag. C-29 prevents the routing from being missed; C-31
...
contains the evidence: each rule's annotation string is quoted, confirming exactly what the
RULES-ANCHORED check examined. C-32 applies the same principle as C-18 (per-entry attestation
with evidentiary quotation) to phase-close tokens: declarations become evidence disclosures at
the phase level.
...
With C-33, the EF function's invocation is publicly observable: the record confirms that the
boundary was exercised, which materials informed the cost derivation, and which content was
structurally excluded. C-30 makes the boundary structural; C-33 makes it auditable.
