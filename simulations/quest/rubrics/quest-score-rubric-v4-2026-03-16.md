Written to `simulations/quest/rubrics/quest-score-rubric-v4-2026-03-16.md`.

---

**What changed from v3 to v4:**

Two new Aspirational criteria. N_aspirational bumped 6 -> 8.

---

**C-15 -- Symmetric comparison completeness** (correctness)

Source: R3 Signal 2 (V-03 Axis I annotation ceiling). Axis I named the current denominator (`N_aspirational=6, v3`) on every calculation row but never stated the prior value (4). This earned PARTIAL on C-13. The failure generalizes: any delta comparison that names only the current state cannot confirm the transition was registered. C-13 enforces this for formula version deltas specifically; C-15 enforces it for *all* delta comparisons in the output -- regression verdicts, arithmetic discrepancies, formula parameters.

Distinction from C-13: C-13 asks "did the scorer declare the formula version delta?" C-15 asks "for every comparison between a prior state and a current state, are both sides present?"

Pass condition: Every delta comparison supplies both prior-state and current-state values explicitly. PARTIAL if current values are all correct but one or more prior values are absent. FAIL if the majority of comparisons name only the current side. N/A rule -> PARTIAL (write prescribed statement).

---

**C-16 -- Phase-distinct mechanism distribution** (behavior)

Source: R3 Signal 1 (V-04/V-05 lifecycle-distributed enforcement). V-04's Axis G (delta block at Phase 1) + Axis H (entry lock at Phase 2) produced fully additive gains on C-13 and C-14 because they operate at *different phases* targeting *different failure modes*. V-03 (Axis I alone) placed all new enforcement at Phase 3, reinforcing an already-PASS criterion (C-10) and reaching the ceiling only for V-04/V-05.

Distinction from C-14: C-14 asks "is the positive anchor at Phase 2 entry?" C-16 asks "does the output contain active mechanisms at two or more distinct phases?" An output can pass C-14 while failing C-16 (all other mechanisms clustered at Phase 3, Phase 1 unguarded).

Phase definitions: Phase 1 = formula delta blocks, version declarations, complete load summaries; Phase 2 = positive anchor rows, pre-labeled entry cells; Phase 3 = re-derivation blocks, discrepancy declaration fields, regression sections.

Pass condition: Mechanisms at 2+ distinct phases. PARTIAL if mechanisms exist at exactly one phase. FAIL if no structural enforcement mechanisms exist beyond bare verdict labels.
...
```
r gains on C-13 and C-14 are fully
additive -- neither can satisfy the other's criterion, and neither redundantly covers the same
failure mode. The ceiling score for R3 (99.17) was reachable only by V-04 and V-05 because they
were the only variations with active mechanisms at two or more phases. V-03 (Axis I alone) placed
all new enforcement at Phase 3 (per-row annotation during calculation), which reinforced an already-
PASS criterion (C-10) and prevented neither of the Phase 1 / Phase 2 failure modes.

Distinction from C-14: C-14 asks "is the positive evidence anchor placed at Phase 2 entry?" C-16
asks "does the output as a whole contain active structural mechanisms at two or more distinct
lifecycle phases?" An output can pass C-14 (anchor is at Phase 2 entry) while failing C-16 (all
other mechanisms are at Phase 3, leaving Phase 1 unguarded). The criterion does not require any
specific phase to be covered -- it requires the output to demonstrate intentional distribution
across the lifecycle rather than clustering all enforcement at one point.

Phase definitions for this criterion:
- Phase 1 (pre-scoring): formula delta blocks, version declarations, load summaries with all four
  elements, N/A rule acknowledgments written before scoring begins
- Phase 2 (during evidence writing): positive anchor rows, pre-labeled entry cells, model evidence
  cells that fire before any verdict is written
- Phase 3 (post-scoring): arithmetic re-derivation blocks, discrepancy declaration fields,
  regression comparison sections

Pass condition: The scoring output contains identifiable structural enforcement mechanisms at two or
more distinct lifecycle phases. Each qualifying mechanism must actively enforce at least one rubric
criterion; bare section headers and boilerplate labels do not qualify. PARTIAL if mechanisms exist
at exactly one phase -- the output enforces quality at one lifecycle point but leaves others
unguarded. FAIL if the output contains no identifiable structural enforcement mechanisms beyond the
required verdict labels and bare evidence cells (implies C-10, C-11, C-12, C-13, C-14 are all FAIL
or PARTIAL from absence).
...
| C-13 | **Formula version delta declaration** -- when the applied rubric version introduces formula parameter changes from a known prior version, the scoring output explicitly names what changed before any score is computed | correctness | A delta notice appears before or within the Phase 1 LOAD block that names: (a) the prior rubric version or formula parameter value, and (b) the current value -- e.g., "N_aspirational=8 (was 6 in v3)" or "Aspirational denominator changed: 6 -> 8". The notice must appear in the scoring output, not only in the prompt instructions. PARTIAL if the current formula is correctly loaded (C-01 PASS) but no prior-vs-current comparison is made and a version change is in scope. FAIL if a formula version change is in scope and no delta is declared, or if an outdated denominator appears in any computation. When no rubric version change is in scope (first-ever scoring of this rubric, or scorer cannot determine prior version), write "No prior version delta -- scoring under v[N] as the baseline version" and the criterion receives PARTIAL (not FAIL). |
...
| Aspirational | C-09, C-10, C-11, C-12, C-13, C-14, C-15, C-16 | 8 | 10 |
...
final score is given (e.g., "E=4/5, R=2/3, A=5/8"). A final number alone is PARTIAL.
...
current value only -- "N_aspirational=8" without "was 6 in v3". This is sufficient for C-01
...
entries, and arithmetic discrepancies. If any comparison gives only the current-state value
with no prior-state counterpart, score PARTIAL. The N/A rule fires when the scoring context
has no version change in scope, no prior-round data, and no arithmetic discrepancy to compare
against -- write the prescribed statement to earn PARTIAL.
...
making a specific failure mode structurally harder to commit -- a pre-labeled entry cell makes
it harder to skip the model cell; a delta block makes it harder to carry over a wrong
denominator; a discrepancy field makes it harder to report an arithmetic error silently. Bare
section headers ("### Phase 3 Verification") without enforcement content do not qualify.
...
C-15 generalizes this requirement to every delta comparison the output constructs --
regression verdict pairs, arithmetic discrepancy pairs, and any other prior-vs-current
contrast. The R3 V-03 signal (Axis I naming only the current denominator) revealed that
this failure mode is not limited to formula version changes: it appears wherever a comparison
is required and the scorer states only the endpoint they reached rather than the full
before-and-after. C-15 is Aspirational rather than an extension of C-13 because the
generalization requires the scorer to audit their entire output for incomplete comparisons,
not just check one block. This is a higher discipline bar than C-13's single targeted check.
...
the output as a whole distributes enforcement mechanisms across the lifecycle rather than
clustering them at one phase. An output can pass C-14 (anchor at Phase 2 entry) while failing
C-16 (all other mechanisms are at Phase 3, leaving Phase 1 completely unguarded). The R3
V-04 ceiling signal showed that the most defensible scoring outputs deploy at least two phases
of active enforcement: a Phase 1 mechanism (delta block) prevents a different class of error
than a Phase 2 mechanism (entry anchor), and both are needed to reach the v3 aspirational
ceiling. C-16 is Aspirational because it requires the scorer to think about phase coverage
holistically -- not just whether any one mechanism is correctly placed, but whether the
collection of mechanisms spans enough of the lifecycle to prevent errors at multiple points.
