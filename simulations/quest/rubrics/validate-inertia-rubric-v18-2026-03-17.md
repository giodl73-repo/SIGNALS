**v18 written.** Two new aspirational criteria added from R17 excellence signals:

**C-47 — C-45 FAIL condition declared as explicit disqualifier in block instruction body**
Source: R17 prediction error (V-85 predicted FAIL, scored PARTIAL). The FAIL shape (neither closure function named) existed only in rubric language, not as a block-body disqualifier. Analogy: C-44 named both split-gate failure modes as block-body disqualifiers. Implication-only coverage ("both functions must be named") earns PARTIAL.

**C-48 — Both C-45 PARTIAL paths enumerated as distinct disqualifiers in block instruction body**
Source: R17 new_patterns[0] — 455-triple instead of designed 455-pair+450-pair. C-45 PARTIAL has two structurally distinct shapes (both-named-not-distinct / one-named-not-other) but the block body merges them into a single condition. Analogy: C-42 named both split-gate displacement shapes as distinct disqualifiers. A merged single condition ("naming both without distinctness, or naming only one, earns PARTIAL") earns PARTIAL on C-48.

**Scoring delta:** 460 → 480 pts max | 38 → 40 aspirational | aspirational subtotal 400 pts.

**R18 gap carried forward:** The missing variation is a block body naming *neither* closure function (C-45 FAIL) while satisfying C-46 (bidirectional non-propagation declared) — the zero-function-named / bidirectional-non-propagation combination.
stinct disqualifiers rather than a single
merged PARTIAL condition. A block body that merges both PARTIAL paths into a single condition
("naming both without distinctness, or naming only one, earns PARTIAL") earns PARTIAL on C-48.

**Scoring:** 460 -> 480 pts max | 38 -> 40 aspirational | aspirational subtotal 400 pts.
...
- **40 aspirational (C-09--C-48):** scoring methodology transparency, time-dependent inertia
...
10 pts each (400 total) = **480 pts max**.
...
- C-47 codifies the C-45 FAIL disqualifier requirement from R17: The R17 prediction error
  ...
  R18 gap identified: the missing variation is a block body naming neither closure function while
  satisfying C-46 -- the zero-function-named / bidirectional-non-propagation combination.
