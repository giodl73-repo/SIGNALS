Reading the scorecard carefully to extract the C-42 and C-43 patterns before writing.

From the scope boundary analysis:
- **C-42** fires on universal criterion-ID labeling — V-03/V-05 PASS by covering TRIAD header, AUTO-RULE guard header, inline SQ rows, step headings, slot headers, and canary block. V-01/V-04 are PARTIAL (some elements labeled, TRIAD header missing). V-02 FAIL (only C-40 core blocks).
- **C-43** fires on the dedicated leftmost `Row #` column — V-02/V-04/V-05 PASS with `| Row # | Sub-question |` table form in all three evaluation steps. V-01/V-03 FAIL with inline `SQ-N:` prefix format.

Here is the complete v17 rubric:

---

```markdown
# Quest Rubric — /quest:mock-review
**Version**: v17 | **Date**: 2026-03-15

---

## Scoring Formula

`(essential_pass/5 × 60) + (recommended_pass/3 × 30) + (aspirational_pass/35 × 10)`
...
| C-42 | Universal criterion-ID labeling (maximum structural scope) | Criterion-ID labels extend beyond the C-40 core blocks to every structural element throughout the skill that encodes criterion-specific behavior: FORBIDDEN OUTPUTS TRIAD block header, AUTO-RULE CONTAMINATION GUARD header, inline sub-question row labels, step headings, gate headers, slot headers, and canary block. A skill that labels only the four C-40 core template blocks does not satisfy this criterion; C-40 establishes the floor, C-42 requires the ceiling. The test is whether every structural element that enforces a named criterion carries that criterion's ID — making the label-to-element mapping mechanically scannable across the full skill without reference to surrounding prose. A PARTIAL is appropriate when labeling extends beyond the four core blocks but misses one or more additional structural elements (e.g., slot headers labeled, TRIAD block header missing). A FAIL is appropriate when coverage does not exceed the C-40 floor. |
| C-43 | Dedicated Row # column in evaluation sub-question tables | Each evaluation step (Strategy, Architect, PM) presents its sub-question list as a markdown table with a dedicated leftmost `Row #` column — for example, `\| Row # \| Sub-question (C-15, C-43) \|`. The dedicated column makes row numbers a first-class scannable field, independently readable without parsing sub-question text. An inline prefix format (`SQ-N:` or `SQ-N (C-15):` preceding question text in a prose list or single-column enumeration) does not satisfy this criterion even if every sub-question is enumerated and criterion-labeled. The structural test is: can the row number be found by scanning the left column alone, without reading question text? A dedicated column passes; an inline prefix fails. A skill that applies the table format in some but not all three evaluation steps does not satisfy this criterion; all three evaluation steps must carry the dedicated column. |
```
...
| Version | v16 → v17 |
| Denominator | 33 → 35 |
| C-39, C-40, C-41 | Promoted to baseline — all variants PASS |
| C-42 (new) | Universal criterion-ID labeling beyond C-40 core blocks — deepens C-40 from floor to ceiling |
| C-43 (new) | Dedicated `Row #` column in evaluation tables — structural superiority over inline `SQ-N:` prefix form |

**Criterion lineage note:** C-42 and C-43 are structurally related to C-40 and C-41 respectively — each deepens its predecessor from a minimum sufficient form to a maximum structural form. C-40/C-41 establish what is required; C-42/C-43 reward going all the way.
