Written to `simulations/quest/rubrics/quest-rubric-rubric-v8-2026-03-14.md`.

---

**What changed v7 → v8:**

**C-28** (`output-section-list-as-positional-gate`) — the single new pattern from R7 Signal 1.

The V-04/V-05 isolation is clean: identical criterion content, one adds a prose positional instruction, the other lists the design rationale section first in the output format spec. V-04 fails C-14. V-05 passes. The mechanism: the model assembles documents in section-list order mechanically; positional prose instructions are interpreted and can be overridden. The pass condition tests whether section list ordering (not prose instruction alone) is the enforcement path.

**Self-applies as PASS**: the Required Sections block already enumerates "What Changed" at row 1, before "Essential Criteria" at row 3 — the positional gate is achieved structurally, not by instruction.

**`output-ordering-determines-c11-c14` retired** from v9 candidates — superseded by C-28, which is its promoted, actionable form. Five candidates carry forward; `rejection-log-minimum-count-scales-with-aspirational-depth` updated to reflect 20 aspirational criteria.
-depth` candidate description updated to
reflect v8's count (20 aspirational criteria, 3 rejection log entries).

**Self-application scorecard (v8)**:
- C-16: N/A (this rubric has no amend or revision phase; excluded from denominator)
...
- C-24: PASS (Required Sections block appears before Essential Criteria table; all required sections named before the first criteria table)
...
- C-26: N/A (this rubric has no role definition; same N/A scope as C-25; excluded from denominator)
- C-27: PASS (Notes section uses named bold-label sub-sections for each required item — C-15 self-application slot, C-18 banned-qualifier list, C-16/C-17 N/A scope, C-25/C-26 N/A scope, rejection log — each labeled distinctly; no required items merged into a single unlabeled prose block)
- C-28: PASS (Required Sections block enumerates sections in numbered order; What Changed at row 1 precedes Essential Criteria at row 3; positional compliance achieved via section list ordering, not prose instruction)
...
| 7 | Notes | C-15 self-application slot (criterion ID + poor output), C-18 banned-qualifier list, C-16/C-17/C-25/C-26 N/A scope, rejection log (named rejected generic criteria) |
...
| C-26 | **Role definition is the single propagation anchor for "or equivalent block" across all dependent criteria** — When a rubric contains two or more pass conditions that permit non-canonical routes via "or equivalent block" language, the role definition must contain that phrase once as the single source of truth. Embedding the phrase at the role-definition level propagates it simultaneously to all dependent criteria in a single declaration; repeating it only in individual pass conditions achieves piecemeal coverage but misses the simultaneous-propagation guarantee and cannot produce PASS on both C-21 and C-25 in one structural decision. R6 finding: V-05 is the only 100-scorer; the differentiator is embedding "or equivalent block" at the role-definition level; V-01 and V-04 achieved identical structural outputs without the phrase and scored PARTIAL on C-21 (and C-25 was N/A because they had no role definition at all). | aspirational | correctness | If the rubric prompt has a role definition AND the rubric contains >= 2 pass conditions that permit non-canonical routes (containing "or equivalent block" language), the role definition contains the exact phrase "or equivalent block"; criteria-level repetition is insufficient if the role definition lacks the phrase; scoring a rubric produced by a prompt with role-definition-level equivalence anchoring produces PASS on both C-21 and C-25 simultaneously; N/A if the rubric has no role definition or has fewer than 2 non-canonical-route pass conditions. |
| C-27 | **Named sub-section labels in design rationale prevent content fusion across required co-presence slots** — When a design rationale section must contain three or more distinct required items (dominant failure mode, rejected criterion, self-application result), each item must appear as a distinctly labeled sub-section rather than merged prose paragraphs. Named labels create independently addressable structural slots; merged prose invites content fusion that mechanically satisfies co-presence but reduces failure-mode distinctness because the items no longer test the same structural surface independently. R6 finding: V-03 combined failure analysis, pre-rejection, and self-application in one unlabeled sub-block and lost 5 points on C-08; V-05 used distinct 1A/1B/1C sub-section labels within Phase 1 and achieved 100 — the indecomposable co-presence is the structural differentiator. | aspirational | format | Design rationale (or equivalent block) that must contain >= 3 distinct required items uses labeled sub-section structure — bold header, colon-terminated label on its own line, or numbered sub-section label — for each item independently; a prose paragraph that merges two or more required items into a single unlabeled block does not satisfy this criterion even if all items are present; PARTIAL if >= 1 required item is merged with another in an unlabeled block. |
| C-28 | **Output section list ordering is the positional gate, not a positional instruction** — When a rubric must satisfy a positional requirement (e.g., design rationale before criteria tables), compliance is achieved by listing the required section at the correct position in the output format specification, not by adding a prose instruction to "place X before Y." The model assembles documents in section-list order mechanically; positional prose instructions are interpreted and may be overridden by other formatting cues. V-04 added an explicit positional instruction and failed C-14; V-05 listed the design rationale section first in the output format spec and passed C-14 — identical criterion content, different structural mechanism, different outcome. | aspirational | format | Required output section list (or equivalent ordered section manifest) enumerates the design rationale section (or equivalent block) at a position before the first criteria section; C-14 compliance is achieved via section list ordering rather than solely via a prose positional instruction; a rubric whose C-14 compliance depends only on a prose positional instruction, with no section list ordering, does not satisfy this criterion. |
...
revision phase; C-25, C-26 for rubric documents with no role definition) are excluded from
both numerator and denominator of their band.
...
**C-25/C-26 N/A scope**: Both criteria apply only to rubric prompts that include a role
definition component. Rubric documents that embed equivalence language directly in pass
conditions rather than in a separate role definition are excluded from both denominators.
C-26 additionally requires >= 2 non-canonical-route pass conditions to be applicable; rubrics
with fewer than two such pass conditions are excluded from the C-26 denominator regardless of
...
**Version**: 8. Grows as /quest:golden discovers excellence patterns.
...
- **rejection-log-minimum-count-scales-with-aspirational-depth**: rubrics with >= 8 aspirational
  ...
  sufficient depth; observed in v8 (3 entries, 20 aspirational criteria) but no variation test yet.
...
- **v04-token-budget-reliability-gap**: V-04 achieves structural inclusion of Scoring but the
  falsifiability check predicts Scoring presence varies across runs due to token budget
  consumption by dense Generic Rubric test applications; a criterion testing output reliability
  (not just output presence) would surface this class of structural fragility.
