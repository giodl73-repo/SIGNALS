The complete v12 rubric is written. Here is a summary of what changed:

---

## v12 changes — 3 new aspirational criteria

**Aspirational denominator**: 24 → 27. Max composite unchanged at 100.

| ID | Name | Pattern source | Axis |
|----|------|---------------|------|
| C-33 | Inertia check is the structural opener | V-01 R11 | Position |
| C-34 | Inertia check uses 4-field structured format | V-01 R11 | Precision |
| C-35 | Disqualifying forms appear as inline constraint tags at write-time | V-04 R11 | Prevention |

**Why each is genuinely new:**

- **C-33** completes the inertia *position* axis: C-04 tests presence, C-33 tests structural priority. The inertia check must fire as Step 1 before any complexity tier, timeline, surface area, or confidence value is assigned — otherwise those values are calibrated against an unknown baseline.

- **C-34** completes the inertia *precision* axis: C-04 tests minimum depth (workaround + one cost), C-34 requires the full 4-field treatment (Workaround / Ongoing Cost / Cost Direction / Key Driver). Cost Direction adds temporal signal (is the workaround getting worse?); Key Driver adds causal attribution (what does removing it actually fix?).

- **C-35** completes the disqualifying-form *distribution* axis: C-32 achieves post-completion detection via the self-check, C-35 achieves write-time prevention via inline `[FAIL: ...]` tags on column headers. Detection + prevention = two-layer compliance system.

**R11 scores under v12:**

| Variation | Aspirational | Score |
|-----------|-------------|-------|
| V-01 (inertia as structural bookend + 4-column) | 26/27 (fails C-35) | 99.63 |
| V-02 (prose-lead sections) | 24/27 (fails C-33, C-34, C-35) | 98.89 |
| V-03 (lifecycle emphasis, handoff receipts) | 24/27 (fails C-33, C-34, C-35) | 98.89 |
| V-04 (phrasing register + constraint tags) | 25/27 (fails C-33, C-34) | 99.26 |
| V-05 (role sequence + inertia framing, combined) | 27/27 | **100.00** |

V-05 is the new champion — the first variation to combine inertia-first ordering (C-33), structured 4-field cost analysis (C-34), and point-of-use constraint tags (C-35) simultaneously.
put that names the workaround and its cost
but omits direction and causal driver fails C-34, even if it passes C-04 and C-10.

**C-35 — Disqualifying forms appear as inline constraint tags at write-time**

**Pattern source**: V-04 R11 — column headers carry inline `[FAIL: ...]` tags (e.g.,
`[FAIL: MODERATE, medium-high, 3/5, or any non-vocabulary term]`) at the point where
the author fills in the field.

**Why it is genuinely new**: C-32 requires exact disqualifying forms in the per-criterion
self-check — a post-completion review artifact that detects violations after the output is
written. C-35 requires those disqualifying forms to also appear at *write-time* as inline
constraint tags in column headers or field labels, so the author encounters the constraint
when filling in the field rather than only during post-completion review. The relationship
is prevention (C-35) vs. detection (C-32): C-32 catches violations retroactively; C-35
prevents them proactively by making the disqualifying form visible at the moment of
production. Together they form a two-layer compliance system. V-04 R11 is the first variation
to achieve this: self-check artifact (C-32) plus point-of-use column tags (C-35) applied to
at least the vocabulary-constrained fields C-01 and C-02.

**Scoring update**: aspirational denominator 24 → 27. Max composite unchanged at 100.

| Variation | Aspirational | Score |
|-----------|-------------|-------|
| V-01 (inertia as structural bookend, 4-column table) | 26/27 (fails C-35) | 99.63 |
| V-02 (prose-lead sections before tables) | 24/27 (fails C-33, C-34, C-35) | 98.89 |
| V-03 (lifecycle emphasis, handoff receipts) | 24/27 (fails C-33, C-34, C-35) | 98.89 |
| V-04 (phrasing register + structural encoding) | 25/27 (fails C-33, C-34) | 99.26 |
| V-05 (role sequence + inertia framing, combined) | 27/27 | **100.00** |

V-05 is the new champion: combines inertia-first structural ordering and 4-column format (C-33,
C-34) with point-of-use constraint tag encoding (C-35). No single-axis R11 variation achieves
all three; V-05 is the first to close all three gaps simultaneously.

---

## Purpose

This rubric scores the output of the `scout-size` skill against 35 criteria grouped into three
...
- **Weight**: aspirational
- **Category**: structure
- **Description**: Refinement of C-28. Every structural criterion (C-18 through C-27) in the
  self-check carries both a pass condition and an exact disqualifying form — a specific
  structural pattern that looks like compliance but fails. Older structural criteria must not
  regress to pass-only descriptions as the denominator grows.
- **Pass condition**: Self-check items for all structural criteria (C-18 through C-27) include
  both a pass condition and an exact structural disqualifying form. An output that satisfies
  C-28 with exact disqualifying forms only for the latest-round criteria fails C-29 unless
  every structural criterion carries equivalent precision.
...
- **Description**: Refinement of C-29. C-29 requires exact disqualifying forms for structural
  criteria (C-18-C-27). C-30 extends this requirement to depth and behavior criteria (C-09
  through C-17). An output can pass C-29 with full structural precision while depth/behavior
...
- **Pass condition**: Self-check items for all depth and behavior aspirational criteria (C-09
  through C-17) include both a pass condition and an exact disqualifying form. An output that
  satisfies C-29 with exact disqualifying forms only for structural criteria fails C-30 unless
  every depth/behavior criterion carries equivalent disqualifying-form precision.
...
- **Pass condition**: The per-criterion self-check (satisfying C-28) includes coverage of all
  ...
  satisfies C-28+C-29+C-30 with a complete 24-criterion aspirational audit fails C-31 if
  essential or recommended criteria are absent from the verification trace.
...
- **Pass condition**: Inertia check entry contains four explicitly named fields or columns:
  ...
  omits direction and causal driver fails C-34.
...
- C-29 is a refinement of C-28, sourced from R8. C-28 requires the self-check to exist.
  C-29 requires structural criteria items (C-18-C-27) to include exact disqualifying forms.
  Coverage uniformity across structural criteria is the discriminator.
- C-30 is a category extension of C-29, sourced from R9. C-29 achieves structural-category
  uniformity (C-18-C-27 all have exact disqualifying forms). C-30 requires the same treatment
  for depth/behavior criteria (C-09-C-17). An output can pass C-29 and fail C-30 if any
  depth/behavior criterion in the self-check carries only a pass condition.
- C-31 is a scope extension of C-28, sourced from R9. C-28 bounds the self-check at
  ...
  recommended criteria. An output can satisfy C-28+C-29+C-30 with a 24-criterion aspirational
...
- C-35 is a write-time enforcement criterion sourced from R11. C-32 achieves disqualifying-form
  ...
  header level. An output can pass C-32 with a complete 32-criterion self-check and fail C-35
  if no vocabulary-constrained column header carries an inline `[FAIL: ...]` constraint tag.
...
| v12 | 2026-03-17 | Added C-33 (inertia as structural opener), C-34 (inertia 4-field format), C-35 (point-of-use constraint tags); aspirational denominator 24 -> 27; sourced from R11 — two new axes introduced: V-01 establishes inertia-first structural position (C-33) and structured cost analysis (C-34); V-04 establishes write-time disqualifying-form enforcement via column-level constraint tags (C-35); V-05 is first to combine all three, achieving 100.00 under v12 |
