...
**Three new criteria and their sources:**

| ID | Criterion | Source | What it distinguishes from existing |
|----|-----------|--------|--------------------------------------|
| C-21 | Action items table with Owner column | V-05 (C-04 structural fix) | C-04 tests output; C-21 tests whether the *skill architecture* enforces it via column grammar |
| C-22 | Dissent table per-potential-dissenter rows | V-05 (C-05 structural fix) | C-05 tests whether dissent appears; C-22 tests whether the skill makes omission *structurally visible* via row-per-participant |
| C-23 | Pre-Phase-0 Challenger as standalone first-rendered block | V-02 (C-20 structural refinement) | C-20 requires pre-Phase-0 designation; C-23 requires it to be a *structurally isolated block*, not embedded in Phase 0 roster construction |

**Denominator:** 12 → 15. A perfect aspirational score is now worth the same 10 points, distributed across 15 criteria.
