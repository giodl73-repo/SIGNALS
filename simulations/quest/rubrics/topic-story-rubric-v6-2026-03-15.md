...
Two new excellence patterns emerge from Round 5:

1. **V-02 C-17 evidence**: "Its presence in Bridge Builder Output does not substitute" — the non-substitution rule. V-02 explicitly states that producing an artifact at an earlier stage does not satisfy the requirement for its placement at a later stage. No existing criterion captures this — C-17 requires the bridge sentence in the recommendation beat but doesn't prevent a claim that prior-stage presence counts.

2. **V-02 C-07 evidence**: "Triple visibility: Signal Analyst posture → Bridge Builder verb rationale → Narrator Beat 5 certification; inconsistency cannot survive all three" — multi-stage consistency enforcement. Critical claims (posture, pattern, verb) appear at multiple mandated positions; inconsistency becomes self-revealing across stages. No existing criterion captures this redundancy-as-enforcement pattern.

New criteria:
- **C-19 — Non-substitution constraint**: closes the gap where a bridge or artifact produced in an earlier stage or block is treated as satisfying the requirement for its appearance in a later stage
- **C-20 — Multi-stage consistency enforcement**: formalizes the triple-visibility pattern as a requirement that critical claims appear at multiple designated positions, making cross-stage inconsistency self-revealing

---

# Quest Rubric — `topic-story` v6
...
Where N_essential = 4, N_recommended = 3, N_aspirational = 13.
