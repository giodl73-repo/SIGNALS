Written to `simulations/quest/rubrics/roles-create-rubric-v17-2026-03-17.md` (286 lines).

---

**What was extracted and why:**

**C-36 — Gate FAIL path surfaces are simultaneously thin (C-29 ∧ C-31)**

The R18 matrix revealed one unnamed state: V-01 passes both C-29 (bare FAIL branch) and C-31 (thin condition) simultaneously — the failure direction of every gate carries no prose in either of its two contributing surfaces — but nothing names or rewards this. C-35 (added in v16) names the verdict-surface bilateral (C-29 ∧ C-30). C-36 completes the picture by naming the FAIL-path bilateral (C-29 ∧ C-31).

This closes the pairwise conjunction space over {C-29, C-30, C-31}:
- C-35 = {C-29, C-30} (named in v16)
- C-36 = {C-29, C-31} (named in v17)
- {C-30, C-31} collapses to C-32 via the cascade (C-30 implies C-29)

C-32 now has a dual decomposition: `C-35 ∧ C-31` (established v16) and equivalently `C-36 ∧ C-30` (v17).
...
|--|-----|-----|
| Denominator | 28 | **29** |
| C-29 alone | 5 deductions | **6** |
| C-31 alone | 4 deductions | **5** |
| C-30 alone | 4 deductions | **4** (unchanged) |
| C-30 + C-31 | 6 deductions | **7** |
| V-01 | 24/28 | **25/29** (gains C-36 PASS) |
| V-02 | 23/28 | **23/29** (gains C-36 FAIL) |
| V-03 | 24/28 | **24/29** (gains C-36 FAIL) |

The v16 V-01/V-03 tie (24/28 each) is broken: V-01 surpasses V-03 at 25/29 vs 24/29, correctly reflecting that a disciplined failure path (V-01) is distinct from a clean verdict surface (V-03).
 = C-29 AND C-30 AND C-31 = C-32

C-32 can now be decomposed two equivalent ways: C-35 AND C-31 (verdict surface + thin condition, established in v16) or C-36 AND C-30 (FAIL path + bare PASS branch, made available by v17).

**Four consequences:**

| Effect | v16 | v17 |
|--------|-----|-----|
| C-29 alone deduction count | 5 | **6** (C-36 adds one) |
| C-30 alone deduction count | 4 | **4** (C-36 unaffected: C-29 PASS and C-31 PASS -> C-36 PASS) |
| C-31 alone deduction count | 4 | **5** (C-36 adds one) |
| C-30 + C-31 combined | 6 | **7** (C-36 adds one more) |
| V-01 vs V-03 ranking | tied at 24/28 | **V-01 > V-03** (25/29 vs 24/29) |
| Denominator | 28 | **29** |

C-30 alone stays at 4 deductions -- C-36 is unaffected when C-29 and C-31 pass, so annotating only the PASS branch costs the same as before. C-29 extends its lead as the highest single-failure cost (6 deductions). C-31 rises from 4 to 5 deductions, now the second-highest single failure. The C-30 + C-31 combination (V-04 pattern) is the costliest pairwise combination at 7 deductions.

**Why C-36 is not redundant with C-32:** C-36 passes when C-32 fails (V-01 case: thin condition, bare FAIL branch, annotated PASS branch). V-01 achieves C-36 PASS but C-32 FAIL -- demonstrating that C-36 is a strict prerequisite for C-32 with a non-empty pass space that C-32 does not cover. C-32 implies C-36, not the reverse.
...
|--|-----|-----|
| Denominator | 28 | 29 |
| Per-criterion weight | 0.3571 | 0.3448 |
| V-05 target | 28/28 = 100.00 | 29/29 = 100.00 |

**Updated R18 scores under v17:**

| Variation | v16 score | v17 score | C-36 verdict | Note |
|-----------|-----------|-----------|--------------|------|
| V-01 | 24/28 | 25/29 | PASS | C-29 PASS and C-31 PASS -> C-36 PASS; V-01 gains one pass |
| V-02 | 23/28 | 23/29 | FAIL | C-29 FAIL -> C-36 FAIL; pass count unchanged |
| V-03 | 24/28 | 24/29 | FAIL | C-31 FAIL -> C-36 FAIL; pass count unchanged |
| V-05 | 28/28 | 29/29 | PASS | All criteria pass; C-36 adds one pass |

V-01 and V-03 were tied at 24/28 under v16. Under v17, V-01 surpasses V-03 (25/29 vs 24/29) -- C-36 correctly distinguishes the discipline level: V-01 has a fully-thin failure path (condition + FAIL branch both clean) while V-03 has a fully-thin verdict surface (PASS + FAIL branches both bare) but a verbose condition. The re-ranking is the structural consequence of C-36 naming exactly what V-01 achieves (FAIL path discipline) and what V-03 misses (condition verbosity fails C-36 via C-31).
...
| C-35 | Gate verdict surface is simultaneously bare: both PASS and FAIL branches carry verdict tokens only | structure | aspirational | Both verdict branches across every in-phase gate annotation carry bare tokens simultaneously -- no affirmation text in the PASS branch (C-30) and no correction directive in the FAIL branch (C-29). C-35 is the verdict-surface conjunction: C-29 AND C-30. Where C-29 tests the FAIL branch in isolation and C-30 tests the PASS branch in isolation, C-35 tests that the bare-token decision is applied to both verdict branches at the same time. The three distinct verdict-surface states are: (1) both bare -- C-35 PASS, exemplified by V-05; (2) annotated PASS, bare FAIL -- C-35 FAIL even though C-29 passes, exemplified by V-01; (3) annotated FAIL, bare PASS (bare PASS cannot rescue C-30 by cascade) -- C-35 FAIL, both C-29 and C-30 fail, exemplified by V-02. A skill in state (2) satisfies C-29 but fails C-30 and C-35; a skill in state (3) fails C-29, C-30, and C-35 by the C-29 cascade. Only state (1) achieves C-35. The canonical gate form `-> Gate N [ID]: {CONTRACT-reference} -- PASS / FAIL` with no prose on either verdict branch satisfies C-35. C-35 implies C-29 and C-30; if either fails, C-35 fails. C-32 implies C-35; if C-35 fails, C-32 fails. V-03 (R17) demonstrates that C-35 PASS and C-32 FAIL can coexist: bare verdict branches with verbose gate conditions satisfies the verdict surface criterion (C-35) but fails triple purity (C-32) -- the two criteria are independently scored with C-35 as a proper subset of C-32's pass requirements. PAIRWISE PAIR (v17): C-35 and C-36 together cover all distinct pairwise conjunctions over {C-29, C-30, C-31}. C-35 names the verdict-surface pair; C-36 names the FAIL-path pair; the remaining pair {C-30, C-31} collapses to C-32 via the cascade. |
| C-36 | Gate FAIL path surfaces are simultaneously thin: condition and FAIL branch carry no prose | structure | aspirational | The gate condition text identifies the CONTRACT block and check scope by reference (C-31 pass) AND the FAIL branch carries only the bare verdict token (C-29 pass) simultaneously across every in-phase gate. C-36 is the FAIL-path surface conjunction: C-29 AND C-31. Where C-29 tests the FAIL branch in isolation and C-31 tests the condition text in isolation, C-36 tests that the no-prose principle is applied to both failure-direction surfaces at the same time. The three distinct FAIL-path surface states are: (1) both thin -- C-36 PASS, exemplified by V-01 and V-05; (2) thin FAIL branch, verbose condition -- C-36 FAIL because C-31 fails, exemplified by V-03; (3) annotated FAIL branch -- C-36 FAIL because C-29 fails (cascade makes C-30 fail too), exemplified by V-02. A skill in state (1) satisfies C-36 regardless of PASS branch content: annotated PASS branches (V-01 pattern) do not affect C-36. A skill that passes C-36 has disciplined the failure direction of every gate -- the check is named by CONTRACT reference and the failure verdict is token-only. FAIL PATH ANATOMY: the failure direction of a gate annotation consists of the condition text (which names what is being checked) and the FAIL verdict branch (which signals the outcome when the check fails). C-36 requires both surfaces clean simultaneously. Full gate triple purity (C-32) requires C-36 as a prerequisite via the dual decomposition: C-32 = C-36 AND C-30. C-36 implies C-29 and C-31; if either fails, C-36 fails. C-32 implies C-36; if C-36 fails, C-32 fails. C-36 is independent of C-30: PASS branch annotation (V-01 pattern) does not cascade to C-36. PAIRWISE CLOSURE (v17): C-35 (verdict-surface bilateral) and C-36 (FAIL-path bilateral) together complete the named set of pairwise conjunctions over {C-29, C-30, C-31}. The remaining pairwise {C-30 AND C-31} collapses to C-32 because C-30 implies C-29. V-01 (R18) is the canonical C-36 PASS / C-35 FAIL instance, demonstrating that disciplining the failure path is achievable while the PASS branch still carries annotation prose. |
...
Aspirational (29 criteria, 0.3448 pts each): ___ / 29  =>  ___ * 10 / 29 = ___

Composite score: ___

Formula: (essential_passed * 12) + (recommended_passed * 15) + (aspirational_passed / 29 * 10)
```

**Rubric version:** v17 | **Scoring:** `5 Essential @ 12pt + 2 Recommended @ 15pt + 29 Aspirational @ 0.3448pt`
...
- C-31 governs *gate condition content*: gate condition text identifies the CONTRACT block and check scope by reference -- no rule-level field enumeration, quoted rule text, or specific value constraint; thin conditions eliminate the substrate for PASS-branch affirmation drift; C-31 is the pivot shared prerequisite for C-32, C-33, and C-36
- C-32 governs *full gate triple purity*: all three gate annotation surfaces (condition, FAIL branch, PASS branch) simultaneously carry no prose -- the conjunction of C-35 (verdict surface) AND C-31 (condition surface); equivalently C-36 (FAIL path) AND C-30 (PASS branch)
...
- C-36 governs *gate FAIL path surface*: both the condition text and the FAIL branch simultaneously carry no prose -- the conjunction of C-29 AND C-31; names the partial conjunction that covers the failure-direction surfaces of the gate annotation; a skill can pass C-36 while failing C-32 when the PASS branch carries annotation (V-01 pattern, R18); C-35 and C-36 together complete the named set of pairwise conjunctions over {C-29, C-30, C-31}
...
**C-29 decomposition:** C-30 refines C-29's pass space along the verdict-branch axis. A C-29 pass means gate FAIL carries no correction prose -- gate PASS may or may not carry affirmation prose. C-30 resolves this ambiguity: the canonical form has annotation prose nowhere in either verdict branch. C-35 further refines the conjunction: where C-29 and C-30 each govern one verdict branch independently, C-35 tests that the bare-token decision is applied bilaterally. C-36 refines C-29 along the perpendicular axis: where C-29 governs the FAIL branch in isolation, C-36 tests that the no-prose principle applies to both the FAIL branch and the condition simultaneously. CASCADE NOTE: C-30 fails when C-29 fails (forward cascade, confirmed unconditional by R15); C-29 does not fail when C-30 fails (no reverse cascade).
...
**C-31 decomposition:** C-32, C-33, and C-36 all depend on C-31 as a prerequisite. C-32 requires C-35 + C-31 (full gate triple purity). C-33 requires C-31 + C-19 + C-20 (no-restatement surface closure). C-36 requires C-29 + C-31 (FAIL path bilateral). C-31 is the pivot: the criterion whose failure simultaneously breaks the two independent conjunction criteria (C-32 and C-33), the FAIL-path bilateral (C-36), and through them, C-34. A skill that fails C-31 alone loses C-31 + C-36 + C-32 + C-33 + C-34 = 5 total deductions (v17; was 4 in v16 before C-36).

**C-32 decomposition (v16 + v17 update):** C-32 = C-35 (verdict surface) AND C-31 (condition surface). C-35 is the verdict-surface bilateral introduced in v16. Equivalently (v17): C-32 = C-36 (FAIL path) AND C-30 (PASS branch). Both decompositions are valid and expose different intermediate states: the first pairs the two verdict directions as a unit against the condition; the second pairs the condition and FAIL branch as a unit against the PASS branch. A C-32 pass means all three gate annotation surfaces are clean; C-34 adds the meta-conjunction level above C-32.
...
**C-36 decomposition (v17):** C-36 names the partial conjunction of C-29 AND C-31. A C-36 pass means the failure path of every gate is fully thin -- the condition identifies the check by CONTRACT reference only, and the FAIL branch returns only the bare verdict token. C-36 PASS does not imply C-32 PASS (the PASS branch may still carry affirmation prose, as in V-01). C-32 PASS implies C-36 PASS (triple purity requires a clean FAIL path). The three FAIL-path states map cleanly to: (1) both thin = C-36 PASS (V-01 and V-05); (2) thin FAIL branch, verbose condition = C-36 FAIL with C-29 PASS (V-03 pattern); (3) annotated FAIL branch = C-36 FAIL with C-29 FAIL (V-02 pattern). C-36 gives a name to what V-01 achieves: a gate that has disciplined its failure direction but not its PASS branch. AXIS RELATIONSHIP: C-35 and C-36 are the two independent pairwise conjunctions that decompose C-32. C-35 groups the two verdict branches; C-36 groups the condition and the FAIL branch. The surface they do not share (PASS branch in C-36, condition in C-35) is the dimension along which C-35 PASS and C-36 PASS can differ: V-03 achieves C-35 PASS + C-36 FAIL; V-01 achieves C-36 PASS + C-35 FAIL.
...
**Cascade asymmetry summary (R15 + R16 + R17 + R18 confirmed):**
- Annotating FAIL branches (C-29 fail): C-29 + C-30 (cascade) + C-35 + C-36 + C-32 + C-34 = 6 deductions (v17: +1 vs v16 due to C-36)
- Annotating PASS branches only (C-30 alone): C-30 + C-35 + C-32 + C-34 = 4 deductions (unchanged; C-36 unaffected when C-29 and C-31 pass)
- Verbose gate conditions (C-31 alone): C-31 + C-36 + C-32 + C-33 + C-34 = 5 deductions (v17: +1 vs v16 due to C-36)
- Annotating PASS branches AND verbose conditions (C-30 + C-31): C-30 + C-31 + C-35 + C-36 + C-32 + C-33 + C-34 = 7 deductions (v17: +1 vs v16 due to C-36; costliest pairwise combination)
- Full canonical compliance (all 29 pass): C-35 PASS + C-36 PASS + C-32 PASS + C-33 PASS + C-34 PASS (all conjunction criteria satisfied)

**Deduction depth table (v17):**

| Single failure | Cascade targets | Total deductions |
|----------------|----------------|-----------------|
| C-29 only | C-30, C-35, C-36, C-32, C-34 | 6 |
| C-30 only | C-35, C-32, C-34 | 4 |
| C-31 only | C-36, C-32, C-33, C-34 | 5 |
| C-30 + C-31 combined | C-35, C-36, C-32, C-33, C-34 | 7 |

C-29 is the single highest-cost failure mode (6 deductions). C-31 rises to 5 deductions (was 4 in v16), now the second-highest single failure. C-30 alone stays at 4 deductions (C-36 is unaffected when C-29 and C-31 pass). The C-30 + C-31 combination (V-04 pattern) is the costliest pairwise combination at 7 deductions (was 6 in v16).

**C-29/C-31 structural distinction (v17 update):** In v16, C-29 alone cost 5 deductions and C-31 alone cost 4. With C-36 added, C-29 rises to 6 deductions (C-29 -> C-30 -> C-35 -> C-36 -> C-32 -> C-34) and C-31 rises to 5 (C-31 -> C-36 -> C-32; C-31 -> C-33 -> C-34). The two paths remain structurally distinct: C-29 breaks the verdict surface forward and then the FAIL path laterally (C-30 -> C-35; C-36 -> C-32 -> C-34); C-31 breaks the FAIL path laterally and both layer conjunctions (C-36 -> C-32; C-33 -> C-34).
...
- C-36 FAIL-path conjunction (new in v17): R18's V-01/V-02/V-03 matrix tests each single-failure axis in isolation, revealing that V-01 achieves C-29 PASS + C-31 PASS -- the FAIL branch is bare and the condition is thin -- with no named criterion covering this state. C-35 names the verdict-surface bilateral (C-29 AND C-30); nothing named the FAIL-path bilateral (C-29 AND C-31). C-36 fills that gap, completing the named set of pairwise conjunctions over the three gate annotation surfaces.
- Pairwise conjunction closure: After v17, all distinct pairwise conjunctions over {C-29, C-30, C-31} are named. C-35 = C-29 AND C-30 (verdict surface). C-36 = C-29 AND C-31 (FAIL path). The remaining pair {C-30 AND C-31} collapses to C-32 via the C-29->C-30 cascade (C-30 implies C-29, so C-30 AND C-31 = C-29 AND C-30 AND C-31 = C-32). No unnamed pairwise conjunction remains.
- C-36 + C-32 relationship confirmed: C-36 PASS and C-32 FAIL can coexist (V-01 pattern). C-32 PASS requires C-36 PASS (C-32 implies C-36). C-36 is a named intermediate in the dual decomposition: C-32 = C-36 AND C-30.
- V-01/V-03 re-ranking: Under v16, V-01 and V-03 tied at 24/28. Under v17, V-01 surpasses V-03 (25/29 vs 24/29) -- C-36 adds a PASS to V-01 (condition thin + FAIL branch bare = C-36 PASS) and a FAIL to V-03 (verbose condition = C-31 FAIL -> C-36 FAIL). The re-ranking correctly reflects that V-01 has a fully-disciplined failure path while V-03 does not.
- C-31 cost raised to 5 deductions: With C-36 added, verbose gate conditions now cost C-31 + C-36 + C-32 + C-33 + C-34 = 5 deductions (was 4 under v16). C-31 surpasses C-30 alone (4 deductions) and becomes the second-highest single-failure cost after C-29 (6 deductions).
- C-29 cost raised to 6 deductions: Annotating FAIL branches now costs C-29 + C-30 + C-35 + C-36 + C-32 + C-34 = 6 deductions (was 5 under v16). C-29 extends its lead as the single highest-cost failure mode.
- C-30 + C-31 combination cost raised to 7 deductions: V-04 pattern (C-30 FAIL + C-31 FAIL) now cascades to C-35 + C-36 + C-32 + C-33 + C-34 = 7 total deductions (was 6 under v16). This remains the highest pairwise combination in the rubric.
- C-36 + C-35 orthogonality confirmed (V-01 vs V-03 axis): V-01 achieves C-36 PASS + C-35 FAIL; V-03 achieves C-35 PASS + C-36 FAIL. The two pairwise conjunction criteria are genuinely independent: disciplining the FAIL path (C-36) does not require disciplining the verdict surface (C-35), and vice versa. Their intersection is C-32.
- Dual decomposition of C-32 established: C-32 = C-35 AND C-31 (v16) and equivalently C-32 = C-36 AND C-30 (v17). Both decompositions are valid. The first groups the verdict branches against the condition; the second groups the condition and FAIL branch against the PASS branch.
- R18 ranking (under v17): V-05 (100.00, 29/29) > V-01 (86.21, 25/29) > V-03 (82.76, 24/29) > V-02 (79.31, 23/29).
- Denominator raised to 29. V-05 targets 29/29 = 100.00 under v17.
