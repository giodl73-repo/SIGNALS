Reading the scorecard to extract the Round 16 excellence signals before writing.

**Two new patterns visible in R16:**

1. **Count derivation arithmetic in threshold annotation** — V-05 declares 12 items where N = 9 declared schemas + 3 structural checks. The threshold annotation that exposes this arithmetic ("9 declared schemas + 3 structural checks = 12 items total; passes when all 12 confirmed") makes the item count derivable by inspection from the Phase −1 schema inventory. V-01–V-04 state N without exposing the derivation; adding or removing a schema block in Phase −1 does not surface as a gate inconsistency until the checklist is counted manually.

2. **Pre-proposal evaluation as declared schema block** — V-05 promotes §5b (pre-proposal evaluation) from an inline Phase 5 instruction to a named schema block [A9] in the Phase −1 schema inventory. This causes §5b to participate in the Gate-0 checklist, condition line conjunction, and pass-threshold count, making it independently auditable and omission-detectable. V-01–V-04 treat §5b as an implicit instruction; it does not appear in Gate-0 and cannot be gated.

Those become C-52 and C-53. Aspirational count: 43 → 45. Total: 51 → 53.

---

## Rubric — topic:plan (v17)

**5 essential / 3 recommended / 45 aspirational — 53 criteria total**
...
### Aspirational (C-09 — C-53, 45 total)
...
**What C-52 adds over C-46:** C-46 requires the threshold annotation to state the gate-pass threshold as "passes when all N confirmed." C-52 requires the annotation to additionally expose the arithmetic by which N was derived: `M schemas + K structural checks = N`. Round 16 showed V-01–V-04 stating N = 11 as a constant, and V-05 extending to N = 12 by adding [A9]. A gate that states only "passes when all 12 confirmed" satisfies C-46 whether 12 was derived correctly or not — the reader must manually recount Phase −1 schema blocks to audit the figure. Exposing the arithmetic (`9 declared schemas + 3 structural checks = 12 items total`) makes the derivation auditable by substitution: a reader can count §IDs in Phase −1, add the fixed structural check count, and verify the sum against the annotated N in one pass. This creates a two-point consistency check (schema inventory → gate count) that a constant-only annotation cannot provide.

**What C-53 adds over C-45 and C-52:** C-45 requires one Gate-0 checklist item per declared schema block. C-52 requires the item count to be derivable from the Phase −1 schema inventory. C-53 requires that procedural evaluation steps — steps that must complete before a proposal can be written, not merely data schemas that must be populated — appear in the Phase −1 schema inventory as named §ID blocks rather than as inline instructions. Round 16 showed V-05 declaring §5b pre-proposal evaluation as [A9] with its own checklist item, condition-line label, and count contribution. V-01–V-04 treat the pre-proposal evaluation as an implicit Phase 5 step: it is instructed but not gated. A step that is instructed but not gated can be skipped without Gate-0 firing. Declaring it as a schema block closes that gap: omitting §5b sets its checklist item to FAIL, which triggers the halt declaration (C-48). The criterion generalizes: any procedural step whose completion is a precondition for proposal integrity belongs in the schema inventory, not in the phase body as a reminder.
