Done. v6 rubric written to `simulations/quest/rubrics/discover-literature-rubric-v6-2026-03-19.md`.

---

**One new criterion: C-21 Symmetric Dual-Obligation Typed Schema Contract**

| Axis | What it captures |
|------|-----------------|
| Source | V-04/V-05 R5 -- both variations declared typed schema blocks for BOTH obligations |
| Gap it closes | C-19 rewards the first typed schema; the second obligation remaining prose-specified creates an asymmetric enforcement surface where abbreviation is still possible |
| Pass condition | OBLIGATION A (RECOVERY NOTE:) and OBLIGATION B (TIER EMPTY:) both have Token/Schema/Fields/Required when/Unacceptable blocks; each obligation's Fields: variable names appear verbatim in the producing phase |
| Independence | C-21 is orthogonal to C-20 -- V-04 PASS on C-21, FAIL on C-20; V-03 PASS on C-20, FAIL on C-21; V-05 PASS on both |
| Dependency | C-15 -> C-17 -> C-19 -> C-21 |

**Scoring delta:**

| | v5 | v6 |
|--|----|----|
| Aspirational criteria | 12 | 13 |
| Aspirational ceiling | 60 pts | 65 pts |
| Total ceiling | 150 | 155 |

The three aspirational precision/scale/symmetry criteria now form a coherent progression: C-19 (one obligation schema-typed), C-20 (two tokens each multi-criterion, declared), C-21 (both obligations schema-typed). Each closes a distinct enforcement gap.
l ceiling | 150 | 155 |

- C-21 depends on C-19 -- symmetric dual-schema is an extension of the single typed schema block; a contract with zero typed schema blocks cannot be symmetric. C-19 FAIL blocks C-21.
- C-21 is independent of C-20 -- V-04 R5 proves dual schemas can PASS without C-20 declaration; C-21 is a structural/precision criterion, not a declaration criterion.
- C-21 rewards coverage completeness: C-19 closes the slot-label mismatch for one obligation; C-21 closes it for both. An output with one typed schema and one prose obligation has a known vulnerability surface; an output with two typed schemas does not.
- Ceiling trajectory: v1: 120 -> v2: 125 -> v3: 130 -> v4: 140 -> v5: 150 -> v6: 155. Each increment captures patterns demonstrably achievable (V-04/V-05 R5 = 145/150) but not rewarded under the prior ceiling.

| Tier | Criteria | Pts each | Total |
|------|----------|----------|-------|
| Essential | C-01..C-05 | 12 | 60 |
| Recommended | C-06..C-08 | 10 | 30 |
| Aspirational | C-09..C-21 | 5 | 65 |
| **Total** | | | **155** |
...
## ASPIRATIONAL Criteria (65 points -- raise the bar once essential/recommended are stable)
...
- **Precision vs. scale vs. symmetry** -- C-19 closes the slot-label mismatch for one obligation (precision); C-20 raises the multi-criterion bar from one token to two (scale); C-21 extends schema precision to both obligations (symmetry). Each addresses a different enforcement gap.
- **Ceiling trajectory**: v1: 120 -> v2: 125 -> v3: 130 -> v4: 140 -> v5: 150 -> v6: 155. Each increment captures patterns that were achievable but not rewarded under the prior ceiling.
