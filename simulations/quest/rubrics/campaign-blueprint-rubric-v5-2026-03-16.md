Written to `simulations/quest/rubrics/campaign-blueprint-rubric-v5-2026-03-16.md`.

---

## What changed in v5

**Three new aspirational criteria from R4 excellence patterns:**

**C-19 — Structural double-prohibition** (5 pts) — V-01's phase-gate encoding. The `CAMPAIGN REFLECTION` phase makes the "not before, not during" prohibition a structural constraint rather than a remembered clause. C-18 tests whether the text says it; C-19 tests whether the structure enforces it.

**C-20 — Contract proximity anchoring** (5 pts) — V-02's per-artifact reminder pattern. The global matrix declares; each artifact's execution site restates READ and PRESERVE inline. Reduces enforcement distance, most directly strengthens C-06 PRESERVE.

**C-21 — Conviction typing** (5 pts) — V-03's CONVICTION TYPE label on each artifact (Factual / Optionality / Emotional) plus a typed CONVICTION DELTA audit. Makes C-09 structurally testable rather than subjectively assessed.

**Two diagnostic flags resolved:**

- **D4**: 5-field C-15 superset earns full credit — extra fields don't confuse scoring
- **D5**: Functional equivalence rule — any labeled post-execution audit block satisfies C-16 regardless of name (CONVICTION DELTA = FINDINGS)

**Scale:** 135 base + 8 excellence = **143 ceiling**

**Retroactive R4 scoring** under v5 gives three distinct bands (133 / 133 / 135.5) from R4's flat 128 — V-03 edges ahead on C-21 full + C-20 partial.
eiterates at minimum its READ and PRESERVE obligations;
partial when only some artifacts carry a reminder or when the reminder is incomplete
(e.g., READ only); no credit when obligations appear only in the global matrix.

Relationship to C-15: C-15 tests whether a global pre-execution contract exists with
all four fields. C-20 tests whether each artifact's obligations are restated at the
point of execution. A campaign can earn full C-15 without earning full C-20.

**C-21 — Conviction typing** (5 pts)

V-03 (R4) labels each artifact's narrative role with a declared CONVICTION TYPE
(Factual / Optionality / Emotional), making the narrative arc structurally distinct
rather than inferred. The post-execution audit block uses a typed delta question
(CONVICTION DELTA: "what conviction of type X does this version add...") that
interrogates the specific conviction type rather than issuing an open-ended question.
Full credit when all three artifacts carry a CONVICTION TYPE label and the
post-execution block is typed to the pitch's conviction type; partial when only some
artifacts are labeled or when the post-execution block does not reference the type; no
credit when conviction roles are implicit or undeclared.

Relationship to C-09 and C-13: C-09 tests whether each artifact amplifies conviction
without repeating content; C-21 tests whether the conviction role is declared, not
inferred. C-13 tests whether the FINDINGS block asks the right audit questions; C-21
tests whether those questions are typed to the declared conviction role.

**V-03 diagnostic resolutions for v5:**

**D4 — 5-field C-15 superset earns full credit.** A contract matrix with more than the
four required fields (e.g., READ / WRITE / PRESERVE / CARRIES FORWARD / CONVICTION TYPE)
satisfies C-15. The extra field does not confuse scoring because the four required fields
are explicitly named. Superset contracts are encouraged; only absence of a required field
causes PARTIAL.

**D5 — Functional equivalence rule for C-16 FINDINGS block.** Any labeled, post-execution
audit block satisfies C-16 regardless of exact block name. "CONVICTION DELTA," "FINDINGS,"
"POST-EXECUTION REVIEW," and similar names all qualify provided the block is labeled and
written after the artifact. The label must be present; an unlabeled post-execution
reflection does not satisfy C-16. This resolves the V-03 ambiguity — CONVICTION DELTA
earns C-16 FULL credit.

**Scale updates:**
- Aspirational: 30 → 45
- Base ceiling: 120 → 135
- Full ceiling: **143** (135 + 8 excellence)

**Retroactive R4 scoring under v5** — three new criteria applied to R4 variants:

| Variant | R4 Base (v4) | C-19 | C-20 | C-21 | Total (v5) |
|---------|-------------|------|------|------|------------|
...
restated per-site. C-20 PARTIAL (2.5) pending commissioner assessment of whether
a conviction-only reminder satisfies the READ+PRESERVE restatement requirement.

---

# campaign-blueprint Rubric — v5
...
performers from the passing tier. Scored as a bonus above the 135-pt base.
...
Aspirational = sum of passed/partial aspirational × weight  (max 45)
...
is the strongest implementation: declared roles make repetition diagnostically
detectable rather than subjectively assessed.
...
### C-19: Structural double-prohibition (aspirational) — new in v5
...
### C-20: Contract proximity anchoring (aspirational) — new in v5
...
### C-21: Conviction typing (aspirational) — new in v5
