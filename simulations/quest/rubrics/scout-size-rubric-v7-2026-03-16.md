Reading the scorecard carefully. Two distinct patterns emerge from R6:

1. **WRONG/CORRECT completeness** — V-01 earns a C-17 PARTIAL because it has CORRECT examples and abstract charter-violation patterns but no named WRONG instance for C-16's same-tier failure and no WRONG counterpart for C-15. V-02 passes C-17 because it provides a concrete named WRONG: *"A gap that says 'API contract is not yet confirmed' when the basis says 'API contract is established' violates this rule."* C-17's adjacency requirement doesn't specify both directions; the new criterion closes that gap.

2. **Relational constraints in the dependent field's label** — V-01 earns a C-18 PARTIAL because C-13 gets a `[LEVEL]` slot but C-16's "must differ" rule is prose-only. V-02 passes by encoding `[must be HIGHER than the tier assigned above]` / `[must be LOWER than the tier assigned above]` directly in the column headers, and `[must differ from current tier: fill with LOW / MEDIUM / HIGH / XL]` in the C-13 label. When a field-to-field relational constraint lives in a prose bullet, a model fills the dependent field before encountering the rule. When it lives in the field label, the constraint is active at production time for that specific field.

```markdown
# Scout-Size Rubric — v7

**Skill**: scout-size
**Version**: v7 (updated from v6 — added C-21, C-22 from R6 excellence signals)
**Date**: 2026-03-16
...
This rubric scores the output of the `scout-size` skill against 22 criteria grouped
...
## Failure Patterns to Watch (v7 additions in bold)
...
- **Half-example** — CORRECT-only or abstract charter-violation pattern without a named WRONG instance fails C-21.
- **Floating relational rule** — "must differ" stated as a prose bullet rather than in the dependent field's label satisfies C-16 as a rule but fails C-22 because the constraint is inactive during field production.
```
