Written to `simulations/quest/rubrics/campaign-simulate-rubric-v4-2026-03-17.md`.

---

**What changed from v3 to v4:**

Three new aspirational criteria extracted from Round 4 excellence signals:

| ID | New Criterion | Source Pattern |
|----|---------------|----------------|
| C-19 | Finding Type drawn from closed vocabulary | R4 C-03 evidence: V-02/V-04/V-05 Type Vocabulary Map + copy-from-map for Type |
...
**Scoring**: aspirational bucket stays at 10 pts max; now 13 criteria × 1 pt each with a cap. Max score = 100 unchanged.

**Key design rationale:**

- **C-19** is the companion to C-14. C-14 closes System Boundary to 5 fixed labels; C-19 applies the same closed-vocabulary discipline to the Finding Type field. Together they form a closed-vocabulary pair that eliminates model judgment from the two fields most susceptible to hallucination.
- **C-20** separates assertion from evidence on blast-radius tier assignments. C-02 proves ranking happened; C-20 proves the top-tier assignments are defensible — each CRITICAL/system-wide finding needs at least one rationale sentence.
- **C-21** promotes C-06 from a prose requirement to a structural one. C-06 passes when the report mentions exception paths; C-21 passes only when a named column tracks them per row, making coverage verifiable by inspection.
uditable. C-02 requires an explicit
