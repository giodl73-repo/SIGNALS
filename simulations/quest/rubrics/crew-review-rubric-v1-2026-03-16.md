...
`crew-review` runs any artifact through the full org. It reads `.roles/`, selects
...
**Text**: Every reviewer row in the matrix names a role that exists in `.roles/`. No
roles are invented. For standard depth, selected roles are relevant to the artifact type
(not arbitrary). For deep depth, all roles in `.roles/` appear.

**Pass condition**: All reviewer names in the matrix match entries in `.roles/`. Zero
