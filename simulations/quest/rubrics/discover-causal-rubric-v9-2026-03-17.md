**v9 rubric written** to `simulations/quest/rubrics/discover-causal-rubric-v9-2026-03-17.md`.

---

## What changed: v8 -> v9

**1 new criterion (C-30)** from the R8 V9 candidate signal, plus precision refinements to C-27/C-28/C-29 pass conditions.

**C-30: Phase 6 Falsification tracks Phase 3 anchor update**
- When Phase 3 upgrades the preliminary anchor to a higher-confidence step, Phase 6 must carry the Phase 3 step -- not the Phase 1 verbatim anchor
- Requires explicit integration rule naming the stale-anchor failure mode
- Requires `anchor_updated_from_phase1: true | false | n/a` ARTIFACT field
- Gradeability: C-26 PASS + C-28 PASS; N/A when Phase 3 confirms verbatim (no update)

**Pass condition refinements:**
- **C-27**: block label not required -- the "does not pass" phrase is the load-bearing element, inline satisfies
- **C-28**: "already on record" (or equivalent) is the load-bearing element -- instruction-to-confirm alone does not pass
- **C-29**: any Phase 6 position satisfies -- single integration rules list bullet is sufficient

**Scoring:**
- Aspirational: 105 → 110 pts (22 criteria x 5 pts)
- Total: 195 → **200 pts**
- Golden **>= 195** | Acceptable **180-194** | Passing **165-179**
- Previous ceiling (195) = new Golden floor

**V9 scores for R8 variations:**

| Variation | v8 | C-30 | v9 | Tier |
|-----------|-----|------|----|------|
| V-01 | 182 | FAIL | 182 | Acceptable |
| V-02 | 175 | FAIL | 175 | Passing |
| V-03 | 190 | FAIL | 190 | Acceptable |
| V-04 | 190 | FAIL | 190 | Acceptable |
| V-05 | 195 | PASS | **200** | Golden (ceiling) |
** Requires C-26 PASS and C-28 PASS. If Phase 1 anchor does not exist (C-26 N/A or FAIL), there is no preliminary anchor for Phase 3 to update. If Phase 3 does not frame its work as confirmation of the Phase 1 anchor (C-28 FAIL), the update path is not established. Both are prerequisites for checking whether Phase 6 carries the updated step.
...
**V-01 through V-05 under v9:**

| Variation | v8 score | C-30 | v9 score | Tier |
|-----------|---------|------|----------|------|
| V-01 | 182 | FAIL | **182** | Acceptable |
| V-02 | 175 | FAIL | **175** | Passing |
| V-03 | 190 | FAIL | **190** | Acceptable |
...
V-01 through V-04 do not include a stale-anchor propagation check -- Phase 6 would carry the Phase 1 verbatim anchor even when Phase 3 updated to a higher-confidence step. V-05 adds an integration rule and `anchor_updated_from_phase1: true | false | n/a` to the ARTIFACT frontmatter, closing the propagation gap.
...
| Aspirational | C-09 through C-30 | 110 |

**C-21 through C-30 relationship to earlier criteria:** These ten criteria do not replace C-12, C-15, C-18, C-19, C-20, or C-26 -- they capture the stricter pass conditions those criteria require at full depth. A response can pass C-26 (anchor at Phase 1) while failing C-27 (no prohibition named at Phase 1), C-28 (Phase 3 does not back-reference it), C-29 (no Phase 6 integration rule), or C-30 (Phase 6 carries stale Phase 1 anchor instead of Phase 3 updated anchor). A response can pass C-27 while failing C-28. A response can pass C-28 while failing C-29 or C-30.
...
**Design rationale for C-30:**

C-30 closes the ceiling gap exposed by V-05's stale-anchor propagation check: when Phase 3 upgrades the preliminary anchor from Phase 1 to a higher-confidence step, Phase 6 must follow that update. V-01 through V-04 all pass C-26, C-27, C-28, and C-29 -- they produce the Phase 1 anchor, name the prohibition, frame Phase 3 as confirmation, and include a Phase 6 integrity rule. But none of them include a mechanism that tracks whether Phase 6 carries the Phase 3 updated step rather than the Phase 1 original. V-05 adds: (1) a Phase 6 integration rule explicitly naming the stale-anchor failure mode, and (2) `anchor_updated_from_phase1: true | false | n/a` in the ARTIFACT frontmatter. C-30 captures this propagation-tracking requirement as an explicit criterion.

The distinction from C-26, C-28, and C-29:
...
|-------|---------------|
| All essential pass + >= 195 | Golden -- meets bar for production use |
| All essential pass + 180-194 | Acceptable -- useful but shallow on aspirational |
| All essential pass + 165-179 | Passing -- essential + recommended coverage only |
| Any essential fail | Below bar -- output is unreliable regardless of score |
| < 165 | Below bar |

**Maximum: 200 pts** (60 essential + 30 recommended + 110 aspirational)
