v11 rubric written to `discover-compare-rubric-v11-2026-03-17.md`.

---

**Two new criteria added (C-30, C-31). Denominator: /22 → /24.**

---

**C-30 — Routing block handles only the deviation case; exec ordering is not re-specified** (from `c28-c29-co-emergence`)

When a standalone BODY ORDER directive is present, the routing block must be deviation-only — no exec branch re-confirming recommendation-first. The body layer establishes the default; the routing layer handles exceptions only. Re-stating exec ordering in a conditional alongside the standalone BODY ORDER creates a second, conditional source of the same specification, coupling the two layers.

- Deviation-only routing block = **pass**
- Routing block with redundant exec branch alongside a standalone BODY ORDER = **partial**
- No standalone BODY ORDER = **not scored** (C-28/C-29 apply)

---

**C-31 — Routing block vocabulary is positionally descriptive, not token-name referential** (from `c28-name-reference-boundary`)

The routing block's branches must use positional language ("RECOMMENDATION precedes DECISION MATRIX") rather than token-name labels ("follow BODY ORDER"). A name reference is a vocabulary-level coupling even when both elements are physically separate. Physical separation is necessary but not sufficient for C-28 PASS; lexical independence is the additional requirement.

- Positional vocabulary throughout all routing branches = **pass**
- At least one routing branch names BODY ORDER or equivalent label = **partial**
- No routing block = **not scored** (C-27/C-25 apply)

---

Two watch list notes added for both R12 patterns (`c28-c29-co-emergence`, `c28-name-reference-boundary`).
structurally dependent on the body-order element's name -- isolation requires not only physical separation but lexical independence. Positional vocabulary throughout all routing branches = pass. At least one routing branch names BODY ORDER (or equivalent directive label) = partial. No routing block = not scored (C-27/C-25 apply independently).

**Denominator: /22 --> /24**. Two new watch list entries added for both R12 patterns.
...
Aspirational passed: ___ / 24  =>  ___ * 10 / 24 = ___
