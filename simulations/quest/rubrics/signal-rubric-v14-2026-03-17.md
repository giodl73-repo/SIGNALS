All structural elements confirmed. Written to `simulations/quest/rubrics/signal-rubric-v14-2026-03-17.md` (111,227 chars).
...
|----|------|----------|------|
| **C-42** | X | C-29/C-40 | FILTER gate Check 3 must verbatim embed and label each SECTION FORMAT sub-element explicitly. FILTER analog of C-40 — folds both verbatim-embedding and labeling into a single criterion (no prior FILTER analog of C-37 existed). |
| **C-43** | Y | C-41 | BARE group offsets must be organized into a named formal **BARE GROUP OFFSET TABLE** with absolute line ranges and first/last canonical command anchors per row. Gate references table rather than re-embedding. Fourth named formal table in the rubric (after ALIGNMENT WIDTH TABLE, DOMAIN NOUN TABLE, COMPLIANCE GATE COVERAGE MATRIX). |
| **C-44** | Z | C-39 | Matrix cells must carry both check number AND criterion ID (e.g. "Ch1 (C-25)"). Closes the reverse-lookup gap: bidirectional navigation from matrix to gate section and back. |

**Retroactive R14 under v14:** V-05 (X+Y) leads at 99.72; V-02/V-03/V-04 at 99.44; V-01 at 99.17.

**R15 plan:** single-axis isolation V-01=X, V-02=Y, V-03=Z; dual V-04=X+Y, V-05=X+Z; convergence V-06=X+Y+Z. Each axis predicted at +0.278 (1/36 × 10). If independence holds, R15/R16 V-06 saturates v14 at 100.00.
s into a named self-documenting table.
