...
**v27 changes from v26 (v8 from v7)**: Added C-23 (Role Candidate Context Injection with Specificity Guard) and C-24 (As-Is Baseline Single-Source Declaration) from R7 excellence signals.

C-23 captures the LIFECYCLE CONTEXT block pattern from V-01: a named block of domain-qualified role candidates is injected before FIELD CONTENT RULES, and Step 1 becomes a "confirm-and-extend" gate rather than a from-scratch assignment. The excellence is not merely providing context -- it is pairing the context injection with an explicit guard that the specificity requirement applies with equal strictness to selections from the context list as to from-scratch assignments. Without the guard, context injection degrades into a label-copying shortcut that satisfies C-05 structurally while undermining it semantically.

C-24 captures the AS-IS ANCHOR block pattern from V-03: when variant framing (as-is vs. to-be) is in scope, the current-state baseline is declared once in a named anchor block before any field instructions that require it. Step 13 (bottleneck trajectory) references this anchor explicitly and includes a "do not reconstruct here" directive. This prevents reconstruction drift -- the subtle divergence that occurs when each step independently generates its own version of the as-is state. V-03 also demonstrates an extension of C-22's post-fill checklist: an anchor-reference verification item ("As-Is State drawn from AS-IS ANCHOR") confirms single-source provenance alongside the FM-A/FM-B failure-mode checks.

Aspirational pool: 14 criteria at 70 pts (v26) -> 16 criteria at 80 pts (v27). 5 pts/criterion rate preserved. Max composite: 160 -> 170.
...
```
score = (essential_pass/5 * 60) + (recommended_pass/3 * 30) + (aspirational_pass/16 * 80)
```
...
> Raise the bar once essential and recommended are stable. C-09 through C-13 added in v21; C-14 and C-15 added in v22; C-16 and C-17 added in v23; C-18 added in v24; C-19 and C-20 added in v25; C-21 and C-22 added in v26; C-23 and C-24 added in v27.
...
(5/5 * 60) + (3/3 * 30) + (0/16 * 80) = 60 + 30 + 0 = 90 -- golden

**All essential pass, 2/3 recommended, no aspirational**:
(5/5 * 60) + (2/3 * 30) + (0/16 * 80) = 60 + 20 + 0 = 80 -- golden (threshold met)

**4/5 essential pass, all recommended pass, all aspirational**:
(4/5 * 60) + (3/3 * 30) + (16/16 * 80) = 48 + 30 + 80 = 158 -- NOT golden (essential incomplete)

**All essential pass, 1/3 recommended, all aspirational**:
(5/5 * 60) + (1/3 * 30) + (16/16 * 80) = 60 + 10 + 80 = 150 -- golden

**All essential + all recommended + all aspirational**:
(5/5 * 60) + (3/3 * 30) + (16/16 * 80) = 60 + 30 + 80 = 170 -- golden (max)

**R7 V-01/V-02/V-03 (all 22 criteria pass; C-23/C-24 not yet assessed)**:
(5/5 * 60) + (3/3 * 30) + (14/16 * 80) = 60 + 30 + 70 = 160 -- golden

**R7 V-01 under v27 (C-23 pass from LIFECYCLE CONTEXT + guard; C-24 N/A -- no variant framing, declared)**:
(5/5 * 60) + (3/3 * 30) + (16/16 * 80) = 60 + 30 + 80 = 170 -- golden (MAX)

**R7 V-03 under v27 (C-24 pass from AS-IS ANCHOR + no-reconstruct directive; C-23 pass if LIFECYCLE CONTEXT present)**:
(5/5 * 60) + (3/3 * 30) + (16/16 * 80) = 60 + 30 + 80 = 170 -- golden (MAX)

**R6 V-01/V-03/V-04/V-05 equivalent under v27 (C-23/C-24 not assessed)**:
(5/5 * 60) + (3/3 * 30) + (14/16 * 80) = 60 + 30 + 70 = 160 -- golden

**R6 V-02 equivalent under v27 (C-19 partial, C-23/C-24 not assessed)**:
(5/5 * 60) + (3/3 * 30) + (13.5/16 * 80) = 60 + 30 + 67.5 = 157.5 -- golden

**R5 V-05 equivalent under v27 (C-21/C-22/C-23/C-24 not assessed)**:
(5/5 * 60) + (3/3 * 30) + (12/16 * 80) = 60 + 30 + 60 = 150 -- golden

**R5 V-01-V-04 equivalent under v27 (C-17 partial, C-19 partial, C-20 partial, C-21/C-22/C-23/C-24 not assessed)**:
(5/5 * 60) + (3/3 * 30) + (10.5/16 * 80) = 60 + 30 + 52.5 = 142.5 -- golden

**R3 equivalent under v27 (8.5/10 aspirational C-09-C-17, C-18 through C-24 not assessed)**:
(5/5 * 60) + (3/3 * 30) + (8.5/16 * 80) = 60 + 30 + 42.5 = 132.5 -- golden

**R2 V-02 equivalent (C-07 partial, C-09-C-15 pass, C-16 through C-24 not assessed)**:
(5/5 * 60) + (2.5/3 * 30) + (7/16 * 80) = 60 + 25 + 35 = 120 -- golden
...
- **C-22 extensibility**: The post-fill checklist may include additional verification items beyond FM-A/FM-B (e.g., confirming as-is data was drawn from a named anchor block per C-24). Additional items do not change the C-22 assessment -- the criterion requires FM-A and FM-B verification; supplementary items are evidence of excellence, not a separate requirement.
