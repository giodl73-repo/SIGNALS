Reading the R17 scorecard carefully for the new excellence signals before writing.

**Two new patterns visible in R17:**

1. **Typed component labels in arithmetic decomposition** — V-01 and V-03 both embed arithmetic as `8 declared schemas + 3 structural checks = 11 items total`, where each addend carries a type descriptor ("declared schemas" vs "structural checks"). An unlabeled form like `8 + 3 = 11` provides a derivable N (satisfying C-52) but does not expose *which* type each component counts. A misclassification — e.g., a promoted procedural step (C-53) counted in K instead of M — is undetectable from the arithmetic alone without cross-referencing Phase −1. Typed labels make classification errors visible at the arithmetic level.

2. **Attestation clause cross-references arithmetic item count** — C-49 requires an attestation declaring completeness; C-52 requires arithmetic deriving N. Neither requires the attestation to state the same N the arithmetic derives. A gate where arithmetic says N=12 and the attestation says "enumerates all 11 items required" would satisfy both C-49 and C-52 independently while embedding a count discrepancy that is invisible unless the reader compares the two fields. Requiring the attestation to name N cross-validates the two self-describing claims, turning a numeric inconsistency into an immediately detectable surface error.

Those become C-54 and C-55. Aspirational count: 45 → 47. Total: 53 → 55.

---

## Rubric — topic:plan (v18)

**5 essential / 3 recommended / 47 aspirational — 55 criteria total**
...
### Aspirational (C-09 — C-55, 47 total)
...
**Formula**: `(essential/5 × 60) + (recommended/3 × 30) + (aspirational/47 × 10)`
