Written to `simulations/quest/rubrics/signal-check-rubric-v15-2026-03-18.md`.

---

**What was added (v14 -> v15):**

| ID | Criterion | Category | Source |
|----|-----------|----------|--------|
| C-43 | Consulting directory's completeness assertion uses all-roles claim form rather than count-annotation form | directory-all-roles-claim | R14-EX-01 |

**Extraction rationale:**

C-42 tests *whether* a completeness assertion is present (either form passes). C-43 tests *which form* the assertion takes. The R14 scorecard observed that V-01's count-annotation form and V-05's all-roles claim form both satisfy C-42, but they differ along a clear axis: count-annotation requires the reader to count entries and compare to a stated number to verify coverage; the all-roles claim asserts coverage as a direct proposition the reader accepts or challenges without arithmetic. The scorecard explicitly flagged this as "a live R15 axis." An output satisfying C-42 via count-annotation satisfies C-42 but not C-43; a mismatched count annotation fails both.

**Updated parameters:**
- Aspirational denominator: 34 -> **35**
- Formula: `composite = 90 + (aspirational_pass / 35) * 10`
- Subsumption: `C-42 <- C-43`; C-43 automatically satisfies C-42 and the full chain to C-20
- C-43 independently testable from C-41 (directory-level vs. NOTE-level structural locations)
`C-42 <- C-43`
- C-43 requires C-42 as precondition; C-42 does not imply C-43
...
| ID | Criterion | Source |
|----|-----------|--------|
| C-43 | The completeness assertion required by C-42 is expressed as an explicit all-roles claim -- e.g., "All reader roles for this block are listed above." -- rather than a structural count annotation matching the number of distinct roles governed by the block; the all-roles claim asserts coverage as a direct proposition that the reader accepts or challenges without counting, transforming the directory into a coverage contract under which the author is accountable to an explicit assertion rather than to the reader's arithmetic; an output satisfying C-42 via count-annotation form (count present and matching entry count) satisfies C-42 but not C-43: a reader verifying coverage must still count entries and compare to the annotation | R14-EX-01 |

## Updated parameters

- Aspirational count: 34 -> **35**
- Formula: `composite = 90 + (aspirational_pass / 35) * 10`
- C-43 requires C-42 as precondition; C-42 does not imply C-43
- C-43 automatically satisfies C-42 (and therefore C-40 and the full chain to C-20)
- C-43 is independently testable from C-41: the all-roles claim form is a directory-level property; the NOTE ownership attribution is a separate structural location

## Subsumption chains

```
C-20 <- C-22 <- C-24 <- C-26 <- C-29 <- C-33 <- C-36 <- C-38 <- C-40 <- C-42 <- C-43
                C-24 <- C-27 <- C-30
C-20 <- C-23 <- C-25
C-22 <- C-31
C-20 <- C-32
...
```
composite = 90 + (aspirational_pass / 35) * 10
```
