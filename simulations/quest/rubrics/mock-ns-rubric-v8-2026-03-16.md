Written to `simulations/quest/rubrics/mock-ns-rubric-v8-2026-03-16.md`.

---

**v8 summary: 2 new criteria, 2 new failure traps**

| ID | Text | Source |
|----|------|--------|
| C-27 | S-0 preamble gate is count-strict exactly two sentences -- no elaborative or expository sentences inserted around the declarative identity and imperative pair | R7 V-03 three-sentence-gate boundary |
| C-28 | The declarative sentence asserts the step's ROLE with the step as grammatical subject -- not an output-position assertion naming what the first output is | R7 V-04 output-primacy-vs-step-identity boundary |

**Extracted patterns:**

- **C-27** (from V-03 vs V-05): V-03 inserts an elaborative sentence ("Its single obligation before advancing is to report TOPICS.md status.") between declarative identity and imperative, producing a three-sentence gate. Both required types are present and in order, so C-25 is ambiguous (type-coverage vs count-strict). C-27 resolves that ambiguity: count-strict, exactly two sentences, no elaboration.

- **C-28** (from V-04 vs V-05): V-04's declarative is "The first observable output of this step is the TOPICS.md status line" — an output-sequence assertion. V-05's is "This step emits first" — a step-role assertion. The grammatical subject is the test: step-as-subject (identity) vs output-as-subject (sequence). An LLM can satisfy output-primacy by reordering artifacts; it can only satisfy step-identity by modeling the step's structural role.

**Two new discriminator notes** added to C-25: "C-25 passing does not imply C-27 passing" and "C-25 passing does not imply C-28 passing."

**Scoring:** max rises from 116 to **120** (20 aspirational × 2 = 40). V-05 scores 120/120. V-01 and V-02 each score 118/120 (one prior-round failure each). V-03 and V-04 each score 116/120 (prior-round C-25 failure + corresponding new R7 criterion failure — C-27 and C-28 respectively).
ty declarations have the step as subject asserting its function; output-primacy declarations have an artifact as subject asserting its position. An LLM satisfying V-04's gate need only order artifacts correctly; an LLM satisfying V-05's gate must understand the step's structural role. C-25 already requires a "declarative identity sentence asserting the step's emit primacy as a statement of the step's ROLE" -- C-28 makes that semantic constraint independently testable with V-04 as the canonical failing example.

**Two new failure traps:**
- "Elaborative three-sentence gate" (V-03): adding an expository sentence between declarative identity and imperative instruction introduces a third sentence -- fails C-27 even when C-25 passes on type-coverage reading and C-23 passes (gate is opening)
- "Output-primacy declarative" (V-04): "The first observable output of this step is X" declares output ordering, not step identity -- fails C-28 even when the declarative sentence comes first and C-23/C-25 structural requirements appear satisfied

**Two new discriminator notes** added to C-25:
- "C-25 passing does not imply C-27 passing" -- a three-sentence elaborated gate may satisfy C-25 on a type-coverage reading while failing C-27's count-strict requirement
- "C-25 passing does not imply C-28 passing" -- an output-primacy declarative may be judged to satisfy C-25's role-assertion language while failing C-28's grammatical-subject test

**R7 variate scores under v8 (max 120):**

| Variate | C-25 | C-26 | C-27 | C-28 | Score |
|---------|------|------|------|------|-------|
| V-05 (Convergent: Zero-Lexeme Synonym Depth) | PASS | PASS | PASS | PASS | 120/120 |
| V-01 (Imperative-First Two-Sentence Gate) | FAIL | PASS | PASS | PASS | 118/120 |
| V-02 (4/5 Synonymized, Op-5 Canonical) | PASS | FAIL | PASS | PASS | 118/120 |
| V-03 (Three-Sentence Gate) | FAIL | PASS | FAIL | PASS | 116/120 |
| V-04 (Output-Primacy Declarative) | FAIL | PASS | PASS | FAIL | 116/120 |

V-05 is the only variate to achieve 120/120. V-01 and V-02 each fail one criterion from a prior round (-2 each). V-03 and V-04 each fail a prior-round criterion plus its corresponding new R7 criterion (-4 each), cleanly separating the two new criteria as independent discriminators.
...
*Raise the bar once essential and recommended are stable. Each criterion is worth 2 pts (20 x 2 = 40; max score = 120).*
...
| Elaborative three-sentence gate | C-27 | A third sentence is inserted between the declarative identity sentence and the imperative instruction -- e.g., "This step emits first. Its single obligation before advancing is to report TOPICS.md status. Write the TOPICS.md status line before any other work begins." Both required sentence types are present (C-25 may pass on type-coverage reading) but the count exceeds two (C-27 fails). The elaborative sentence is not prohibited by content but by count. V-03 (R7) confirmed this trap. |
| Output-primacy declarative | C-28 | Declarative sentence names the first output artifact rather than the step's role: "The first observable output of this step is the TOPICS.md status line." Grammatical subject is an output artifact, not the step. C-25 structural requirements may appear satisfied (declarative sentence present, before imperative) but C-28 fails because step identity requires the step as subject, not an artifact in output position. V-04 (R7) confirmed this boundary. |
