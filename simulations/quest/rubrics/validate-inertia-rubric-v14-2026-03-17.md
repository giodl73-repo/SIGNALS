`validate-inertia-rubric-v14-2026-03-17.md` written.

---

**What changed from v13:**

**C-41 (new)** — Per-axis gate declared in block body, not in produced label. Source: V-65. The V-65 failure mode: block gate instruction was minimal ("Do not proceed until AUDIT ARCHITECTURE DECLARED is written above") with no axis enumeration; per-axis gate lived exclusively in the produced label. A model reading the block body encounters only the completion trigger, not the three-axis constraint — the per-axis impossibility statement is emit-time (written when producing the label), not read-time (encountered before Phase 1 begins). C-40 can pass without C-41; C-41 requires the per-axis gate in the block instruction body.

**C-40 updated** — Added a third PARTIAL condition (V-63 case): a gate naming all three axes without any partial-failure declaration earns PARTIAL. Also added V-61 confirmation that functional descriptors satisfy "by name" equivalently to ordinal form, encoded in both the pass condition text and the C-40 note.

**Scoring:** 400 → 410 pts max | 32 → 33 aspirational | aspirational subtotal 320 → 330 pts.
n begins.

Pass condition: the C-40 per-axis gate must appear in the block instruction body. A per-axis
gate present only in the produced architecture label earns PARTIAL.

**C-40 PARTIAL condition clarification (V-63 case):**

R13 V-63 demonstrated a gap: all three axes named by ordinal labels, no partial-failure
declaration present. Neither explicit PARTIAL condition covered this combination ("fewer than
three axes" and "partial invalid without naming axes" are exhaustive only for the other two
partial-pass shapes). C-40 now encodes a third PARTIAL condition: a gate naming all three axes
without any partial-failure declaration earns PARTIAL. This resolves the gap via genuine attempt
doctrine -- naming all three axes is the harder half of the pass condition; missing only the
impossibility statement is substantive partial satisfaction, not failure.

R13 V-61 confirmed that functional content descriptors (e.g., "duality framing," "audit
format," "derivation direction") satisfy C-40's "names each axis by label" pass condition
equivalently to ordinal "Axis N" form; no text change to the pass condition was required, but
the V-61 ruling is recorded as a note.

**Scoring delta:** 400 -> 410 pts max. 32 -> 33 aspirational. Aspirational subtotal: 330 pts.
...
- **33 aspirational (C-09--C-41):** scoring methodology transparency, time-dependent inertia
  ...
  per-axis gate declared in block body not in produced label

Scoring: Essential = 10 pts each (50 total), Recommended = 10 pts each (30 total), Aspirational =
10 pts each (330 total) = **410 pts max**.
