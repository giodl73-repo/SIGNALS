**rhythm-behavior Rubric v13** written to `simulations/quest/rubrics/rhythm-behavior-rubric-v13-2026-03-17.md`.
...
| | v12 | v13 |
|---|---|---|
| Aspirational denominator | 27 | **30** |
| New criteria | -- | C-35, C-36, C-37 |

Three new criteria from R12 patterns:

**C-35 -- All Five Sub-Skill Section Bodies Individually Confirmed** (format, R12)
Flow-conversation (V-02) and trace-contract (V-03) close the remaining gaps. All six named positions now confirmed: positions 1-5 (all sub-skill section bodies) + Synthesis. C-32's count of four is superseded. C-32 updated with R12 note; C-31, C-26, C-22 all get R12 cross-references.

**C-36 -- Descriptive Register and Section-Body Placement Compose Without Interaction** (format, R12)
C-34 and C-31 are orthogonal -- first simultaneous test (V-04). A descriptive sentence in a named section body passes C-22 identically to either property alone. C-34 and C-22 updated with R12 notes.

**C-37 -- C-33 Inertness Rule Has No N-Ceiling** (format, R12)
N=3 triple body placement (V-05) confirmed. Pattern generalizes without bound: first qualifying occurrence satisfies C-22, all subsequent inert regardless of count. C-33 updated with R12 note.
ompose Without Interaction** (format)
C-34 (register invariance) and C-31 (named section body qualifies) are orthogonal and independent criteria. A descriptive standalone sentence placed within a named section body satisfies C-22 identically to a labeled imperative form in the same position. Neither property interacts with or restricts the other -- both apply simultaneously without mutual interference. This is the first simultaneous test of both properties together (R12 V-04). Scorers must not treat the combination of descriptive register + section-body placement as a more restrictive case than either property tested in isolation.

**C-37 -- C-33 Inertness Rule Has No N-Ceiling** (format)
C-33's inertness rule for multiple qualifying named section-body placements has no count ceiling. N=3 triple body placement (flow-lifecycle, trace-state, trace-permissions) confirmed in R12 V-05, extending the N=2 dual-body pattern from R11 V-04. The principle generalizes without bound: the first qualifying occurrence of the axis-header rule satisfies C-22; all subsequent occurrences are inert regardless of N. Scorers must not infer an N-ceiling from the N=2 confirmation in R11 or from any prior round. Fail this criterion if a scoring judgment withholds C-22 credit or applies a penalty on grounds of excessive repetition for a prompt with three or more qualifying named section-body placements.

Cross-references updated on C-22, C-26, C-31, C-32, C-33, and C-34.
