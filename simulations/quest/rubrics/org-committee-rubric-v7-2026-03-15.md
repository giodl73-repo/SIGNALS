Reading the Round 6 scorecard carefully for patterns not yet formalized in v6.

**New patterns R6 introduces:**

1. **C-24: Investigation rewrite attempts carry sequential labels** — All three variations show a numbered-attempt pattern: V-01 uses `INVESTIGATION-REVISION-1:`, V-02 uses `INVESTIGATION-ATTEMPT-N+1`, and V-03 (primary axis) names it explicitly: *"Sequential INVESTIGATION-ATTEMPT-N / GATE-N labels make iteration count visible."* C-22 requires the rewrite to happen and the gate to re-fire; C-24 requires the rewrite to be labeled with a sequential attempt number so the iteration count is auditable. This is C-22's traceability enforcement — the same upgrade pattern C-16 → C-22 applied to the rewrite loop itself.

2. **C-25: Tier sort validation appears as a named discrete gate** — `SORT-CHECK` appears in all three variations: V-01 *"SORT-CHECK validation,"* V-02 *"SORT-CHECK: 'Tier 1 and Tier 2 both empty? YES — mis-sorted; reassigning,'"* V-03 *"Three-tier sort + SORT-CHECK in Phase 2."* C-11 requires correct challenger-first ordering; C-18 requires all-approve self-correction. C-25 requires the sort validation to be a named, discrete gate step with an explicit test condition and declared result — correct output alone is insufficient. This is the structural enforcement mechanism for C-11 and C-18 together, parallel to how C-16 enforces C-09.

3. **C-26: Simulation is organized into explicitly labeled, sequential phases** — V-03's primary structural innovation: Phase 0 (committee declaration) → Phase 1 (investigation) → Phase 2 (sort) → Phase 3 (voices) → Phase 4 (tally) → Phase 5 (minutes). Each phase output commits the model before the next phase begins. Phase-labeled structure prevents retroactive editing of prior commitments and makes the simulation's progression visible and auditable. This is the sequential-commitment architecture that makes all earlier structural criteria (C-13, C-14, C-15, C-16, C-22, C-24, C-25) reliably enforced: each criterion belongs to a phase, and phases execute in order.

Aspirational pool grows from 15 to 18 criteria; pool max from 30 to 36 pts; composite max becomes **126**.

---

# org:committee Rubric — v7

**Skill**: `org:committee`
**Version**: 7 (updated from Round 6 scoring — 2026-03-16)
**Changelog**: Added C-24, C-25, C-26 from R6 excellence signals. C-24 requires investigation rewrite attempts to carry sequential labeled identifiers, making the C-22 correction loop auditable (traceability enforcement for C-22). C-25 requires tier sort validation to appear as a named discrete gate with explicit test condition and declared result (structural enforcement for C-11 and C-18). C-26 requires the simulation to be organized into explicitly labeled, sequential phases where each phase commits before the next begins (sequential-commitment architecture that makes all prior structural criteria reliably enforceable). Aspirational pool grows from 15 to 18 criteria; aspirational max grows from 30 to 36 pts; composite max is now 126.
...
## Aspirational Criteria (36 pts total — 18 criteria at 2 pts each)
...
composite = (essential_pass / 5 * 60)
          ...
          + (aspirational_pass / 18 * 36)   ← denominator was 15, now 18; pool was 30, now 36
```

Maximum composite: **126**
