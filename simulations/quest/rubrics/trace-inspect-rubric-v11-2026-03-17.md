v11 written. Here's the summary of what changed:

**New criteria from R14 V-05 excellence signals:**

| ID | Name | Pattern | Source |
|----|------|---------|--------|
| C-32 | **Inertia checklist closure for C-30** | When C-30 passes, a named INERTIA checklist item must name the "ID substitution" attack vector and explicitly state it closes C-30. Count equality alone is not enough — the item must explain *why* per-ID membership exists. | V-05 vs V-04 in R14 |
| C-33 | **Inertia checklist closure for C-31** | When C-31 passes, a named INERTIA checklist item must name the "forward-reference opacity" anti-pattern and explicitly state it closes C-31. | V-05 vs V-04 in R14 |

**Grand total**: 104.5 + 0.5 + 0.5 = **105.5**

The design logic: C-32/C-33 address the same durability gap that motivated C-27's named PROMOTION COMPLETENESS GATE — without INERTIA entries, a future inheritor satisfying C-30 or C-31 has no in-trace signal explaining why those patterns exist, forcing re-read of the criterion text. The INERTIA checklist makes both criteria self-certifying across round boundaries.
GATE block in v8.

These become **C-32** and **C-33**. Grand total: 104.5 + 0.5 + 0.5 = **105.5**

---

```markdown
# Trace-Inspect Rubric — v11

**Skill**: `trace-inspect`
**Version**: v11 (updated from v10 — added C-32 and C-33 from R14 excellence signals;
aspirational block: C-09 and C-10 at 2 pts each; C-11 through C-33 at 0.5 pts each;
grand total 105.5)
**Applies to**: Trace outputs produced by any variation of the trace-inspect skill
**Scoring**: 105.5-point composite. Golden threshold: all 5 essential criteria pass AND
composite >= 80.
...
> future inheritance). C-32 and C-33 are new in v11, extracted from R14 excellence signals:
> C-32 from the INERTIA CHECKLIST CLOSURE FOR C-30 pattern (V-05 in R14 — when C-30 is
> satisfied, a named INERTIA checklist entry must document the ID-substitution attack vector and
> explicitly state it closes C-30, making C-30 self-certifying for future round inheritance
> without re-reading the criterion text) and C-33 from the INERTIA CHECKLIST CLOSURE FOR C-31
> pattern (V-05 in R14 — when C-31 is satisfied, a named INERTIA checklist entry must document
> the forward-reference opacity anti-pattern and explicitly state it closes C-31, making C-31
> self-certifying for future round inheritance).
