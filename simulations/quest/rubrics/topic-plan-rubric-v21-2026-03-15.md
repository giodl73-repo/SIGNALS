## Rubric — topic:plan (v21)

**5 essential / 3 recommended / 51 aspirational — 59 criteria total**
...
### Aspirational (C-09 — C-59, 51 total)
...
| ID | Criterion | Category |
|----|-----------|----------|
| C-56 | *(unchanged from v19)* | behavior |
| C-57 | *(unchanged from v19)* | behavior |
...
| ID | Criterion | Category |
|----|-----------|----------|
| C-58 | **Defense basis temporal audit** — the Defense basis column uses a three-way temporal legend (PRE-READ / POST-READ / POST-SIGNAL) rather than a binary PRE-REGISTERED / NEWLY-CONSTRUCTED distinction, providing an epistemic audit trail showing when each defense was constructed relative to the evaluation stages; a Defense basis column that distinguishes only pre-registered from newly-constructed without encoding the evaluation stage at which each defense was formed does not satisfy this criterion; the three-way legend makes the column an audit trail rather than a binary classification — PRE-READ defenses existed before any signals were read, POST-READ defenses were constructed after signal inventory but before signal content influenced the evaluation, and POST-SIGNAL defenses were constructed after signal content was known — enabling detection of post-hoc rationalization at stage-level granularity rather than binary pre/post registration | behavior |
...
| ID | Criterion | Category |
|----|-----------|----------|
| C-59 | **Inventory-first epistemic ordering** — the skill completes all 9 namespace signal tables (satisfying C-02) before strategy.md is opened or read (satisfying C-01), ensuring the artifact vocabulary and classification decisions in the signal inventory are formed without exposure to the strategy's existing vocabulary and framing; a skill that opens strategy.md before completing the namespace tables does not satisfy this criterion even if SIGNAL READ-LOCK (preventing re-reading) is enforced; inventory-first makes the evidence base vocabulary-independent before the prior frame is visible — strategy vocabulary cannot anchor artifact classification on the first pass if strategy.md has not yet been read; SIGNAL READ-LOCK blocks re-reading but not strategy-anchored initial labeling, so enforcing inventory-first ordering closes an epistemic gap that SIGNAL READ-LOCK alone leaves open; the ordering constraint is satisfied when the skill's Phase −1 or preamble declares an explicit sequencing rule of the form "complete all namespace tables before reading strategy.md", making the ordering a declared structural invariant rather than an incidental execution property | behavior |

---

**Aspirational summary: C-09 – C-59 (51 total)**

| Range | Source version | Count |
|-------|---------------|-------|
| C-09 – C-42 | v12 and earlier | 34 |
| C-43 – C-45 | v13 | 3 |
| C-46 – C-47 | v14 | 2 |
| C-48 – C-49 | v15 | 2 |
| C-50 – C-51 | v16 | 2 |
