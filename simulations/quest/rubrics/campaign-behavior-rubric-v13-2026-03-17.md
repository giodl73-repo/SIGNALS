**v13 written** — `simulations/quest/rubrics/campaign-behavior-rubric-v13-2026-03-17.md`

Three new criteria extracted from R14 excellence signals:

---

**C-40 — Field 7 remediation-tier sub-structure with Q8 4th check** (category: structure)
- Source: R14 V-01
- Field 7 partitioned into `spec: [...] | contract: [...] | implementation: [...]` in every finding block (F-NN and M-NN)
- Q8 gains a 4th audit dimension: remediation-tier completeness alongside the three existing checks
- Pass: all finding blocks use the three-tier sub-structure AND Q8 explicitly audits tier completeness; PARTIAL if sub-structure present but Q8 4th check absent

**C-41 — M-NN meta-finding elevation into EXECUTIVE SUMMARY blast-radius pool** (category: correctness)
- Source: R14 V-02
- M-NN blocks are first-class 7-field structures entering the blast-radius ranking pool; can outrank F-NN findings
- VALIDITY RULES rule 2 explicitly covers the M-NN token format alongside F-NN inline evidence format; Q8 audits M-NN blocks
- Pass: M-NN blocks have 7 fields, appear in EXECUTIVE SUMMARY ranked by blast radius, VALIDITY RULES names M-NN format, Q8 covers M-NN; PARTIAL if M-NN blocks present but not in EXECUTIVE SUMMARY pool
- Requires C-19, C-28, C-32, C-39

**C-42 — H-NN hypothesis pre-declaration with QH outcome gate** (category: rigor)
- Source: R14 V-03
- H-NN table declared in Phase 0; QH fires after Q6 and before EXECUTIVE SUMMARY write site, assigning CONFIRMED/REFUTED/NO-EVIDENCE to each H-NN
- NO-EVIDENCE is a distinct traceable outcome — "looked, found nothing" vs "never evaluated"
- CONSOLIDATE gets a `Hypothesis outcomes` section; VERDICT reports counts by outcome
- Pass: H-NN table + QH with three-state outcomes + hypothesis outcomes in CONSOLIDATE + VERDICT counts; PARTIAL if QH present but binary outcome only
- Requires C-18 and C-25

---

**Structural notes:**
- Denominator 31 → 34; formula `(passed/34) x 10`
- V-01/V-02/V-03 each score 32/34 = 9.41 → 9, total **89**
- V-04 scores 33/34 = 9.71 → 10, total **90**
- V-05 scores 34/34, total **90** (true perfect score)
- The new ceiling requires all 16 aspirational landmarks C-27 through C-42
a clear "none found" statement. Omitting the section
  entirely = FAIL.
...
## Aspirational Criteria (34 criteria, score = passed/34 x 10)
...
**Source:** R14 scorecard — V-01 remediation-tier excellence signal; V-02 M-NN
elevation excellence signal; V-03 H-NN hypothesis pre-declaration excellence signal;
V-05 full composability confirmation

| Criterion | Source | Failure mode it addresses |
|-----------|--------|--------------------------|
| C-40 | R14 V-01 field 7 remediation-tier sub-structure + Q8 4th check | Field 7 without sub-structure cannot be mechanically audited for fix-layer ownership — a reviewer must re-read prose to determine whether spec, contract, and implementation layers are all addressed; Q8 with only three check dimensions cannot flag a finding that assigns all remediation to one layer |
| C-41 | R14 V-02 M-NN meta-finding elevation into blast-radius pool | M-NN items as prose annotations are invisible to blast-radius ranking and to Q8's structural audit; an assumption-gap finding that would rank above several F-NN findings is silently omitted from the EXECUTIVE SUMMARY top-3; VALIDITY RULES without explicit M-NN token handling creates a gap where M-NN confirmation slots produce C-31 violations |
| C-42 | R14 V-03 H-NN hypothesis pre-declaration + QH three-state outcome gate | Without H-NN pre-declaration, hypotheses are never stated; without QH, evaluated hypotheses have no outcome record; without NO-EVIDENCE as a distinct state, a hypothesis the campaign evaluated but could not confirm is indistinguishable from a hypothesis the campaign never considered |

**Denominator:** 31 -> 34. Scoring formula updated: `(passed/34) x 10`.

**Finding structure expansion:** C-40 deepens field 7 from a single recommendation
slot into a three-tier ownership contract, making fix assignment auditable at the
block level. C-41 expands the finding class from F-NN only to F-NN + M-NN, making
assumption-gap findings full participants in blast-radius ranking. C-42 adds a
pre-declared hypothesis class (H-NN) with a dedicated outcome gate (QH), converting
the campaign from a finding-only report into a hypothesis-resolution artifact.

**Cross-criterion dependencies introduced:**
- C-40 requires C-19 (Field 7 requires the 7-field block) and C-37 (Q8 4th check requires Q8)
- C-41 requires C-19, C-28, C-32, and C-39 (7-field blocks, EXECUTIVE SUMMARY section, VALIDITY RULES gate, and I-NN inventory for M-NN code references)
- C-42 requires C-18 (QH extends calibration question count) and C-25 (Q6 must exist as QH fires after it)

**R14 variations under v13:**

| Variation | C-40 | C-41 | C-42 | Aspirational | Asp score | Total |
|-----------|------|------|------|-------------|-----------|-------|
| V-01      | PASS | FAIL | FAIL | 32/34       | 9         | 89    |
| V-02      | FAIL | PASS | FAIL | 32/34       | 9         | 89    |
| V-03      | FAIL | FAIL | PASS | 32/34       | 9         | 89    |
| V-04      | PASS | PASS | FAIL | 33/34       | 10        | 90    |
| V-05      | PASS | PASS | PASS | 34/34       | 10        | 90    |

32/34 = 9.41 -> rounds to 9, total 89. 33/34 = 9.71 -> rounds to 10, total 90.
V-01, V-02, and V-03 each achieve 32/34. V-04 achieves 33/34. V-05 is the only
variation achieving the true perfect score of 34/34. The new ceiling requires all
sixteen aspirational landmarks C-27 through C-42 — the full dual-pipeline enforcement
stack, phase coverage expansion, and finding structure expansion.
