All checks pass. The v18 rubric is written to `simulations/quest/rubrics/flow-resilience-rubric-v18-2026-03-15.md`.

**Summary of changes:**

| What changed | Detail |
|---|---|
| Header count | 50 → 53 `### C-` headings; 42 → 45 aspirational |
| **C-51** (new, E-37) | Per-stage enforcement restatement at stage-specification level. V-03's 4th hierarchy level — Rule B appears within the Recover/TTR stage entry itself, firing at exact fill moment rather than at column-scan time. Cracked by V-03 only. |
| **C-52** (new, E-38) | Rule Constraint column in pre-commitment assessment table. V-02's dedicated column architecturally separates enforcement citations from fill instructions, making C-49 compliance column-scannable. Cracked by V-02 only. |
| **C-53** (new, E-39) | Structured Inertia Case with mandatory step cross-references. V-05's three-component verdict template requires explicit citations to Step 1a / Step 1b+Gap Register / Step 1c, making the verdict traceable to pre-committed values. Cracked by V-05 only. |
| Scoring formula | `C-09 through C-53`; uncapped max 84 → 90 (45 criteria × 2) |
| Ceiling references | R18 V-02/V-03/V-05 at 86/90; V-01/V-04 at 84/90 |
| R18 discrimination | Three-way tie at 86/90; open for R19: all three new criteria for non-cracking variations |
r steps (Step 1a / Step 1b + Gap Register finding / Step 1c); all other variations allow free-form prose verdict | V-05 only |

**Scoring delta**: uncapped max 84 -> 90 (45 criteria x 2). Composite cap unchanged at 10.

**R18 final ranking**: V-02/V-03/V-05 each crack one new criterion and reach 86/90; V-01/V-04 hold at
84/90. **Open for R19**: C-51/C-52/C-53 for the non-cracking variations.

**Orthogonality argument:**
- **C-51** is orthogonal to C-50: C-50 requires three or more structurally independent hierarchy
  levels with at least one explicit restatement label outside the preamble -- satisfied by preamble
  + sub-part inline + column-contract (three levels). C-51 requires a fourth level at
  stage-specification granularity within the Recovery Path stage entries themselves. A template
  satisfying C-50 via column-contract restatement (L3) may carry no stage-specification enforcement
  (L4 absent); C-51 requires the rule to appear within an individual stage sub-entry with an
  explicit restatement label naming the stage-specification level. C-51 is also orthogonal to C-35
  (three-component recovery stage specification -- mechanism + SLA + VS; C-51 requires enforcement
  restatement to appear as an additional named component within the stage entry, not merely that
  the three components are present).
- **C-52** is orthogonal to C-49: C-49 requires inline named-label citations in sub-parts but
  permits prose-embedded citations. C-52 requires those citations to be architecturally separated
  into a dedicated table column, making citation presence independently verifiable by column scan.
  A template satisfying C-49 via prose-embedded citations fails C-52; a template satisfying C-52
  necessarily satisfies C-49. C-52 is also orthogonal to C-32 (bypass-trigger column scanability,
  which requires a BYPASS-TRIGGER column in the post-analysis rule audit table; C-52 requires a
  Rule Constraint column in the pre-commitment sub-part table -- different table, different phase,
  different purpose).
- **C-53** is orthogonal to all prior criteria: no existing criterion governs the Inertia Verdict's
  internal structure or requires cross-references between the verdict and Pre-Commitment Document
  steps. C-53 closes the gap between the pre-commitment phase (where carrying costs and tipping
  points are established) and the synthesis phase (where the verdict is rendered). Prior criteria
  govern pre-commitment document enforcement (C-48/C-49/C-50) and gate structure (C-16/C-18/C-19/
  C-20) but none require the synthesis verdict to cite pre-committed step values by identifier.

**R18 discrimination under v18 (uncapped aspirational tiebreak; all composite = 100/100):**

| Rank | Variation | C-51 | C-52 | C-53 | Uncapped Asp. |
|------|-----------|------|------|------|--------------|
| 1 (tie) | **V-03** | PASS | FAIL | FAIL | 86/90 |
| 1 (tie) | **V-02** | FAIL | PASS | FAIL | 86/90 |
| 1 (tie) | **V-05** | FAIL | FAIL | PASS | 86/90 |
| 4 (tie) | **V-01** | FAIL | FAIL | FAIL | 84/90 |
| 4 (tie) | **V-04** | FAIL | FAIL | FAIL | 84/90 |

Three-way tie at 86/90. Open for R19: C-51 for V-01/V-02/V-04/V-05; C-52 for V-01/V-03/V-04/V-05;
C-53 for V-01/V-02/V-03/V-04.
...
| Criterion | E-tag | Signal | Cracked by |
|-----------|-------|--------|------------|
| **C-45** -- Gate Independence Completeness Verification Block | E-31 | V-03/V-04/V-05 carry a named block at COMPLETE time enumerating all gate CLOSE blocks eligible for clause independence and confirming each one's status | V-03, V-04, V-05 |
| **C-46** -- Gap Registry Nil Confirmation Structural Form | E-32 | V-03/V-05 produce nil confirmations in the same three-field form as gap entries (Source: N/A / Missing link: N/A / Consequence: nil -- coverage complete) | V-03, V-05 |
...
| Criterion | E-tag | Signal | Cracked by |
|-----------|-------|--------|------------|
| **C-42** -- Gate-Close Clause Structural Independence | E-28 | V-01/V-04/V-05 split Gate 2b CLOSE into TWO independent checkboxes | V-01, V-04, V-05 |
| **C-43** -- Impossibility Argument Basis Clause Independence | E-29 | V-02/V-04/V-05 split Gate 1 CLOSE into TWO independent checkboxes | V-02, V-04, V-05 |
| **C-44** -- Bidirectional Gap Registry Artifact Structure | E-30 | V-03/V-05 formal PART A + PART B GAP REGISTRY with three-field entries | V-03, V-05 |
...
| Criterion | E-tag | Signal | Cracked by |
|-----------|-------|--------|------------|
| **C-39** -- Gate-Open Precondition Acknowledgment Checkpoint | E-25 | V-02/V-04 Gate 2b OPEN precondition carries explicit named acknowledgment | V-02, V-04, V-05 |
| **C-40** -- Gate-Close Prohibition Clause | E-26 | V-02/V-04 Gate 2b CLOSE adds prohibition clause alongside positive confirmation | V-02, V-04 |
| **C-41** -- Impossibility Argument Quality Gate Postcondition | E-27 | V-01/V-05 Gate 1 CLOSE postcondition names required basis AND prohibited basis | V-01, V-05 |
...
|-----------|-------|--------|------------|
| **C-36** -- Chaos-Trigger Threshold Identity Assertion | E-22 | V-05: identity claim at Column Contract level | V-05 only |
| **C-37** -- Observable Signal Detection Horizon | E-23 | V-04/V-05: Detection Horizon as third required component | V-04, V-05 |
| **C-38** -- Chaos-Observability Bidirectional Coverage Matrix | E-24 | V-05: named cross-reference artifact | V-05 only |
...
- Signal: V-01's named "Bypass Gate-Reopening Protocol" declared before Gate 1.
- Pass requires: numbered remediation steps / amendment sub-gate convention / authorized actors /
  COMPLETE-blocking statement.

**C-32 -- Bypass-Trigger Column Scanability**
- Signal: V-02/V-05's BYPASS-TRIGGER column -- each cell a trigger citation or explicit N/A.
- The gap it closes: C-28 requires invocation completeness in prose; C-32 requires column-scan.

**Scoring delta**: uncapped max rises from 44 -> 48 (24 criteria x 2).
...
Uncapped maximum across 45 criteria is 90; the cap rewards breadth while holding the composite
ceiling at 100.
...
### C-51 -- Per-Stage Enforcement Restatement at Stage-Specification Level *(E-37)*
The enforcement content (at least one named rule from the C-48 contract) appears within the
stage-specification text of at least one individual Recovery Path stage entry -- a fourth
structurally independent hierarchy level (L4) below preamble (L1), sub-part inline (L2), and
column-contract (L3). The stage-specification level is a discrete named entry within an individual
stage sub-specification (e.g., within the Recover/TTR or Contain/TTC stage entry), not a sub-bullet
of the column-contract header. The restatement carries an explicit label naming the structural level
(e.g., "restated at stage-specification level", "Rule B restated at stage level", or equivalent).
This criterion extends C-50's multi-level restatement requirement to stage-specification granularity:
where C-50 is satisfied by a column-contract restatement (visible at column-scan time), C-51
requires a restatement that fires at the exact fill moment -- when the filler is authoring a
specific stage SLA value, not merely when scanning the column header. C-51 is orthogonal to C-35
(three-component recovery stage specification -- mechanism + SLA + VS; C-51 requires enforcement
restatement to appear as an additional named component within the stage entry, not merely that the
three components are present). (Signal: V-03 Round 18 -- "SLA from Step 1d, Class N, TTR column --
Rule B restated at stage-specification level: any TTR value not present in Step 1d Class N is a
named contract violation" within the Recover/TTR per-stage sub-specification; all other variations
stop at column-contract as the deepest enforcement level.)

**Pass condition**: The enforcement content (at least one named rule from C-48) appears within the
stage-specification text of at least one Recovery Path stage entry (Recover/TTR, Contain/TTC,
Detect/TTD, or Validate/TTV). The restatement carries an explicit label naming the structural level
(e.g., "restated at stage-specification level" or equivalent). The stage-specification entry must
be structurally independent from the column-contract header -- a column-contract note describing
per-stage requirements does not qualify; the restatement must appear within the individual stage
entry itself. C-50 FAIL subsumes C-51.

**Partial**: Rule cited by label within a stage sub-specification but without an explicit
restatement label naming the level context ("Rule B applies" without "restated at
stage-specification level"); or restatement label naming "stage-specification level" present but
embedded in the column-contract header rather than within an individual stage entry; or
stage-specification restatement present but the label does not name the structural level context.

**Fail**: Enforcement content at stage-specification granularity absent; enforcement appears only
at preamble, sub-part inline, and column-contract levels with no stage-entry-level restatement; or
C-50 FAIL subsumes C-51 (fewer than three independent levels established).

### C-52 -- Rule Constraint Column in Pre-Commitment Assessment Table *(E-38)*
When the sub-parts governed by the enforcement contract (C-48) are rendered as rows in a structured
table, the inline rule citations required by C-49 are architecturally separated into a dedicated
named column -- the "Rule Constraint" column or equivalently named enforcement column -- rather than
embedded within the content cell's prose. The Rule Constraint column carries the named-label
citation(s) (e.g., "**Rule A applies**"; "**Rule A applies**; **Rule B applies**") as the sole
content for that cell, independent of the fill instruction text in the adjacent Required Content
cell. This structural separation is orthogonal to C-49: C-49 requires inline named-label citations
in sub-parts; C-52 requires those citations to be architecturally isolated into a scannable column.
A template satisfying C-49 via prose-embedded citations would fail C-52 if the citations are not
column-separated. C-52 is also orthogonal to C-32 (bypass-trigger column scanability, which
requires a BYPASS-TRIGGER column in the post-analysis rule audit table; C-52 requires a Rule
Constraint column in the pre-commitment sub-part table -- different table, different phase,
different purpose). (Signal: V-02 Round 18 -- "Rule Constraint" column in Pre-Commitment Assessment
Table carries "Rule A applies" / "Rule A applies; Rule B applies" per row as a dedicated cell,
independently scannable without parsing the Required Content column; all other variations embed
citations in prose sub-part instructions.)

**Pass condition**: The pre-commitment sub-parts are rendered as rows in a table structure. A
dedicated named column (e.g., "Rule Constraint", "Enforcement", "Rule Applies", or equivalent) is
present in the table, distinct from the content/instruction column. The column carries the
named-label citation(s) from the C-48 enforcement contract as the cell content for each sub-part
row, using the label identifier rather than prose paraphrase. At least two sub-part rows carry
populated Rule Constraint cells. The column must be independently scannable -- a reader can verify
rule citation presence across all sub-parts by reading only the Rule Constraint column without
parsing the content column. C-49 FAIL subsumes C-52.

**Partial**: Table structure present with rule citations in a column but the column also carries
fill instruction text (citations not architecturally separated from content); or dedicated Rule
Constraint column present but some sub-part rows have blank rather than populated cells; or rule
citations isolated in a column that also carries other enforcement annotations rather than being
dedicated to citations alone; or C-49 PARTIAL with table structure and at least one Rule Constraint
cell populated.

**Fail**: Sub-part rule citations are prose-embedded within instruction cells with no dedicated
Rule Constraint column; or sub-parts are not table-structured (no column separation possible); or
C-49 FAIL subsumes C-52.

### C-53 -- Structured Inertia Case with Mandatory Step Cross-References *(E-39)*
The Inertia Verdict section (or equivalently named synthesis section at the close of the template's
analysis) is structured as a named multi-component synthesis template -- not free-form prose --
where each component mandates explicit labeled cross-references to named steps or sections of the
Pre-Commitment Document. At minimum three components are required: (1) a named-threat component
referencing the framing established in a prior step (e.g., "reference Step 1a framing"); (2) a
carrying-cost component referencing both a prior carrying-cost step and a named Gap Register finding
(e.g., "reference Step 1b + named Gap Register finding"); and (3) a tipping-point component
referencing a prior threshold step (e.g., "reference Step 1c threshold"). Each component carries a
labeled cross-reference instruction naming the specific prior step by identifier -- not a generic
"draw from prior analysis" instruction. This criterion is orthogonal to all prior criteria: no
existing criterion governs Inertia Verdict structure or mandates traceability between the verdict
and Pre-Commitment Document step outputs. The Pre-Commitment Document criteria (C-48/C-49/C-50)
govern the document's enforcement structure, not its downstream synthesis usage. (Signal: V-05
Round 18 -- Inertia Case structured as labeled three-component template with mandatory citations
to Step 1a / Step 1b + Gap Register finding / Step 1c by identifier; all other variations allow
free-form prose verdict.)

**Pass condition**: A named Inertia Case (or equivalently named synthesis block) is present with
three or more named components, each carrying an explicit labeled cross-reference instruction naming
the specific prior step by identifier (e.g., "Step 1a", "Step 1b", "§ 1a", or equivalent). Generic
instructions ("draw from prior framing") do not satisfy this criterion. At least one component must
reference a named Gap Register finding (or equivalent named analysis artifact), not only a prior
step. The block must be a named structural template element, not a prose description of what a
verdict should contain.

**Partial**: Inertia Case structured with multiple components but cross-references use generic
descriptors rather than named step identifiers; or named step identifiers present but not all three
required component types represented (threat / carrying-cost / tipping-point); or three-component
structure present with step identifiers but no component requires a named Gap Register finding
reference; or structured template with step identifiers rendered as prose instructions rather than
as a labeled template with distinct per-component entries.

**Fail**: Inertia Verdict present only as free-form prose without a named multi-component synthesis
template; or structured components present but with no labeled step cross-reference instructions;
or no Inertia Verdict section in the template.
...
| Aspirational | 10% | C-09 through C-53 | PASS=2, PARTIAL=1, FAIL=0; capped at 10 |
...
**Ceiling references under v18:**
- R18 V-01/V-04 (C-51/C-52/C-53 all FAIL): 84/90 uncapped
  -> composite = 60 + 30 + 10 = **100** (capped)
- R18 V-02/V-03/V-05 (one new criterion PASS each): 86/90 uncapped
  -> composite = **100** (capped; uncapped ceiling: 86/90)
- Open for R19: C-51/C-52/C-53 for non-cracking variations.
...
- **C-51 (new)** -- Per-stage enforcement restatement at stage-specification level: extracted from
  V-03 Round 18 structural excellence. V-03 introduces a fourth enforcement hierarchy level (L4)
  below column-contract (L3), carrying Rule B within the Recover/TTR per-stage sub-specification
  with an explicit restatement label "restated at stage-specification level". C-50 can be satisfied
  via column-contract restatement alone; C-51 requires the rule to appear at the exact stage-fill
  moment, not merely at column-scan time. Cracked by V-03 only.
- **C-52 (new)** -- Rule Constraint column in pre-commitment assessment table: extracted from V-02
  Round 18 structural excellence. V-02 renders sub-part rule citations as a dedicated "Rule
  Constraint" column in a structured table, making C-49 compliance independently scannable by
  column review without parsing the content column. Prose-embedded citations satisfy C-49; the
  column form satisfies C-52 by architecturally separating enforcement obligations from fill
  instructions and preventing citation dropout during sub-part prose revision. Cracked by V-02
  only.
- **C-53 (new)** -- Structured Inertia Case with mandatory step cross-references: extracted from
  V-05 Round 18 structural excellence. V-05 structures the Inertia Verdict as a labeled
  three-component synthesis template requiring explicit citations to Step 1a, Step 1b + Gap
  Register finding, and Step 1c by identifier. All other variations allow free-form prose verdict
  that can acknowledge inertia risk without closing the loop to pre-committed carrying-cost and
  tipping-point values. C-53 makes verdict traceability verifiable by label-scan. Cracked by
  V-05 only.
- **V-02/V-03/V-05 advance to 86/90** -- three-way tie at the new discrimination ceiling.
  V-01/V-04 hold at 84/90 (neither introduces a new structural pattern in R18).
- **Aspirational cap unchanged** at 10; uncapped maximum rises from 84 to 90 (45 criteria x
  PASS=2).
...
- **C-45 (new)** -- Gate independence completeness verification block: extracted from V-03/V-04/
  V-05 Round 15 structural excellence. Cracked by V-03, V-04, V-05.
- **C-46 (new)** -- Gap registry nil confirmation structural form: extracted from V-03/V-05 Round
  15 structural excellence. Cracked by V-03, V-05.
