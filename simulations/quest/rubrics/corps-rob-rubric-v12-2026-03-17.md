# corps-rob rubric v12

**v12 summary: 3 new criteria from R11 excellence signals**

| ID | Criterion | Signal | Dependency |
|----|-----------|--------|------------|
| C-38 | Carry Forward Inertia-ID Dual-Baseline Attribution Column | R11 Signal 2: V-04/V-05 carry blocks include Inertia ID column tagging each carried finding IB-01/IB-02/both/N/A -- closes dual-baseline audit chain at the cross-stage handoff artifact | C-32, C-37 |
| C-39 | IB-02 Urgency Gradient Downstream Citation Cascade | R11 Signal 3: V-05 temporal framing makes Urgency Gradient = HIGH a verdict-shaping cascade -- Go/No-Go must cite IB-02, Risk Register must include delay-compounding entry, Inertia Pressure Summary must name compounding path | C-37 |
| C-40 | Named Rule Glossary Exclusivity Declaration | R11 Signal 1: glossary is structural prerequisite for both C-30 and C-34 -- variations without a glossary lose both criteria regardless of other compliance; criterion requires the glossary to explicitly declare itself as exclusive named-rule locus | C-30 |

**Scoring impact:**

| | v11 | v12 |
|--|-----|-----|
| Aspirational pool | 29 x 5 = 145 | 32 x 5 = 160 |
| Max composite | 235 | 250 |
| Golden threshold | 80 (unchanged) | 80 (unchanged) |

**Retroactive R11 under v12:**
- V-05: 240 (gains C-38: Inertia ID column present; gains C-39: temporal urgency cascade; C-40 FAIL: no glossary exclusivity declaration)
- V-04: 235 (gains C-38: Inertia ID column present; C-39 FAIL: dimensional framing has no urgency gradient cascade; C-40 FAIL)
- V-01: 220 (C-38 FAIL: carry forward present, dual baselines present, but no Inertia ID column; C-39 FAIL; C-40 FAIL)
- V-02: 220 (C-38 not scoreable: C-37 FAIL; C-39 FAIL; C-40 FAIL)
- V-03: 215 (C-38 not scoreable: C-32 FAIL; C-39 FAIL; C-40 FAIL)

**C-38 -- Carry Forward Inertia-ID Dual-Baseline Attribution Column** (format / aspirational)
- Signal: R11 Signal 2 -- V-04/V-05 carry blocks include an Inertia ID column that tags each carried finding by IB-01, IB-02, both, or N/A. This operationalizes dual-baseline traceability at the structural artifact level: a TPM assigning risk register entries to a specific baseline does not need to re-read stage prose; the attribution is settled at the carry block. The column is cost-free to implement but makes IB-01/IB-02 assignment structurally verifiable across the cross-stage handoff chain.
- Distinction from C-32: C-32 requires the CARRY FORWARD block to exist with labeled entries and a CARRY: NONE zero-state when nothing carries. C-38 requires the block to additionally include an Inertia ID column tagging each entry's baseline attribution. A CARRY FORWARD block without the Inertia ID column satisfies C-32 but not C-38. C-32 and C-37 must both pass for C-38 to be scoreable.

**C-39 -- IB-02 Urgency Gradient Downstream Citation Cascade** (depth / aspirational)
- Signal: R11 Signal 3 -- V-05's temporal framing introduces an IB-02 Urgency Gradient value that cascades into mandatory downstream citations. When Urgency Gradient = HIGH, the Go/No-Go table must cite IB-02, at least one Risk Register entry must name delay-compounding as the inertia driver, and the Inertia Pressure Summary must name the compounding path explicitly. This cascade makes delay-cost analysis a verdict-shaping constraint rather than additive documentation -- the IB-02 Urgency Gradient value propagates into structural requirements on downstream sections. V-04's dimensional framing (technical vs organizational axes) does not generate this cascade because it does not produce a gradient value that can be HIGH.
- Distinction from C-37: C-37 requires paired illustrative baseline examples demonstrating structurally distinct cases. C-39 requires an IB-02 Urgency Gradient to cascade into mandatory downstream citations when the gradient is HIGH -- a causal chain from baseline attribute to verdict constraints. A run can satisfy C-37 (two baselines present and structurally distinct) without satisfying C-39 (no urgency gradient cascade declared). C-37 must pass for C-39 to be scoreable.

**C-40 -- Named Rule Glossary Exclusivity Declaration** (format / aspirational)
- Signal: R11 Signal 1 -- V-01 and V-03 fail both C-30 and C-34 for the same root cause: no RUN-LEVEL RULE GLOSSARY. V-04 and V-05 gain 10 points over V-01/V-03 from this single structural decision. The glossary is not documentation -- it is the structural prerequisite that simultaneously enables C-30 (preamble schema with named post-stage reference) and C-34 (disambiguation annotation at rule-declaration level). Neither can be satisfied by inline declarations in stage templates. C-40 captures this constraint as an explicit format rule: the glossary must declare itself as the exclusive locus for named rule declarations, so a generator understands that inlining a rule in a stage template does not satisfy a named-rule criterion.
- Distinction from C-30: C-30 requires at least one named schema to be declared in a run-level preamble with post-stage sections citing it by name. C-40 requires the RUN-LEVEL RULE GLOSSARY itself to carry an explicit self-declaration that it is the exclusive site for named format rule declarations -- and that named-rule requirements (C-30, C-34, and any analogous criteria) cannot be satisfied by inline rule declarations in stage templates or section bodies. A glossary that declares preamble schemas without this exclusivity statement satisfies C-30 but not C-40. C-30 must pass for C-40 to be scoreable.

**Dependencies:**
- C-38 requires C-32 and C-37 to pass.
- C-39 requires C-37 to pass.
- C-40 requires C-30 to pass.

**Persistent gap -- C-35 and C-40 both fail universally in R11.**
C-35: No variation declares RULE AUDIT-TABLE with an explicit additive constraint. C-40: No variation declares glossary exclusivity. Both remain hard targets for R12.
