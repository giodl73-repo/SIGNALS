`roles-create-rubric-v12-2026-03-17.md` written.

---

**What changed: v11 → v12**

**New criterion: C-30 — Gate PASS branches carry verdict tokens only**

The R13 signal: all four non-canonical variations failed C-29, up from one in R12. That universality reveals the ungoverned dimension — the PASS side of every gate annotation. V-05's canonical form has bare `PASS / FAIL` tokens throughout; neither branch carries prose. C-29 governed the FAIL side; C-30 closes the gap on the PASS side.

**Implication chain completed:**
- C-28: no correction prose in body
- C-29: no correction prose in gate FAIL branch
- C-30: no affirmation prose in gate PASS branch
- Together: gates are pure signal in both directions; Phase 5 is the sole interpretive location

**C-30 pass condition:** Any PASS branch carrying explanatory text — "PASS: All 12 fields present and correctly typed", "PASS: Frontmatter complete" — fails. Bare `PASS` only.

**C-30 implies C-29.** The violations are independent (a skill may pass one while failing the other), but C-30's pass space presupposes C-29 is already satisfied.

**Denominator:** 22 → 23 | Per-criterion weight: 0.4545 → 0.4348 | V-05: 23/23 = 100.00

C-30 retrospective scoring against R13 deferred to R14 (PASS branch content not captured in the R13 scorecard evidence).
s are `PASS / FAIL` throughout -- no FAIL branch directive, no PASS branch commentary.
...
|--|-----|-----|
| Denominator | 22 | 23 |
| Per-criterion weight | 0.4545 | 0.4348 |
| V-05 | 22/22 = 100.00 | 23/23 = 100.00 |

R13 variations' C-30 status cannot be retrospectively resolved without reviewing gate PASS branch content. C-30 assessment is deferred to R14.
...
**Denominator:** 22 -> 23 | Formula denominator updated throughout.

**R13 finding:** All four non-canonical variations fail C-29 in R13 (V-01, V-02, V-03, V-04). This is a broadening from R12 (only V-01 failed C-29; V-02 passed). V-03 and V-04 each fail three criteria in the discrimination zone, with C-29 failures co-occurring with C-25 failures -- confirming that skills with excess phase body surface tend to also carry correction prose in gate FAIL branches. V-05 (canonical) holds the sole 100 score at 22/22. The R13 pattern exposes the ungoverned PASS branch dimension, motivating C-30.
...
| C-29 | Gate FAIL branches carry verdict tokens only | structure | aspirational | Every in-phase gate annotation across all content-producing phases is limited to verdict tokens (PASS / FAIL or equivalent) in its FAIL branch. No correction instruction, repair directive, or "before advancing" clause appears in any gate FAIL branch. Phase 5 (or equivalent final confirmation phase) is the sole correction path -- it handles repair implicitly by re-running the pre-declared AUDIT-CHECKLIST against current state. A gate annotation of the form `-> Gate 3 [FR]: All 12 fields present -- FAIL: Regenerate missing fields before advancing` = fail, even though it passes C-28 (correction is in the gate, not the body). A gate annotation of the form `-> Gate 3 [FR]: All 12 fields present -- PASS / FAIL` = pass. C-29 implies C-28 and C-15; if either prerequisite fails, C-29 fails. A skill that passes C-25 (minimum surface) and C-29 (verdict-only FAIL gates) has no correction prose anywhere in its phase structure -- Phase 5 is the universal repair loop. |
| C-30 | Gate PASS branches carry verdict tokens only | structure | aspirational | Every in-phase gate annotation across all content-producing phases is limited to verdict tokens (PASS / FAIL or equivalent) in its PASS branch. No affirmation text, confirmation summary, "all checks satisfied" annotation, or equivalent positive commentary appears in any gate PASS branch. A gate annotation of the form `-> Gate 3 [FR]: All 12 fields present -- PASS: Frontmatter complete and correctly typed. / FAIL` = fail, even though it passes C-29 (FAIL branch is verdict-only). A gate annotation of the form `-> Gate 3 [FR]: All 12 fields present -- PASS / FAIL` = pass. Gates are pure signal in both directions -- neither verdict branch carries prose. C-30 implies C-29; if C-29 fails, C-30 fails. A skill that passes C-25 (minimum surface), C-29 (verdict-only FAIL gates), and C-30 (verdict-only PASS gates) has eliminated annotation prose from all structural positions. Phase 5 is the sole location where state is interpreted and acted upon. |
...
Aspirational (23 criteria, 0.4348 pts each): ___ / 23  =>  ___ * 10 / 23 = ___
...
**Rubric version:** v12 | **Scoring:** `5 Essential @ 12pt + 2 Recommended @ 15pt + 23 Aspirational @ 0.4348pt`
