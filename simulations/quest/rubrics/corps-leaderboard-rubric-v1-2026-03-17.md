...
- **Weight**: aspirational
- **Category**: depth
- **Text**: The output names the team's current stagnation pattern (e.g., "Lone Wolf" — single contributor dominates; "Namespace Tunnel" — all signals in one namespace; "Empty Start" — no signals yet; "Stale Coverage" — signals exist but no recent activity) and ties at least one recommended next action back to breaking that named pattern.
...
`5/5 * 60 + 0/3 * 30 + 0/2 * 10 = 60` — below golden threshold.
