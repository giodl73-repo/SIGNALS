# Rubric: trace-component (v16)

**51 criteria across 3 tiers -- 3 new aspirational criteria added from R15 excellence signals:**
...
| Aspirational | C-09 through C-51 | 86 |

**Ceiling: 176 pts** (up from 170; 3 new criteria x 2 pts each)

---

The three R15 signals that promoted:

**C-49 . Named-Register Subdivision of Causal Phase** -- From R15 C-47 V-05 advance. V-05 subdivides the causal question block into explicitly labeled sub-sections: NULL REGISTER and DEPARTURE REGISTER, each carrying its own section header as a distinct document element. V-01 and V-04 present causal questions in a single undivided CAUSAL PHASE block with no internal named sections. The subdivision makes the boundary between null-baseline interrogation and departure-property interrogation structurally visible in the prompt's document structure rather than inferred from question ordering. C-48 requires null questions to precede departure questions; C-49 requires the two categories to be labeled as structurally distinct named registers. A causal phase with questions sequenced correctly but without named section labels does not satisfy C-49.

**C-50 . Per-Register Close Marker Within Causal Phase** -- From R15 C-47 V-05 `DEPARTURE REGISTER CLOSE` structure. V-05's causal phase ends with `DEPARTURE REGISTER CLOSE` -- a register-level completion gate applied to the departure-property register specifically, not a gate on the causal phase as a whole. V-01 and V-04 use `CAUSAL PHASE CLOSE` -- a single gate at the phase boundary that does not gate the internal register boundary. The per-register close is structurally analogous to the manifest-close adjacency marker (C-38): it prevents content from bleeding between the departure register's last question and the schema authorship slot by making insertion self-contradictory at point-of-production. C-47 requires a structural phase boundary between causal questions and schema authorship; C-50 requires each named register within the causal phase to carry its own close marker so that the boundaries between registers are gated independently of the phase boundary.

**C-51 . Successor-Naming in Manifest-Close Adjacency Marker** -- From R15 C-38 V-05 PASS vs. V-01--V-04 PARTIAL. V-05's manifest close explicitly names the successor artifact: "This is the last content line of MANIFEST-N. TABLE-N header follows directly below." V-01 through V-04 close with `*[...verbatim, as this manifest's last line. Reproduce now.]*` -- establishing terminal position but not naming the specific successor. Successor-naming binds the close marker to a specific phase-keyed artifact: a model that writes the close line and then inserts content before TABLE-N must contradict not only the terminal-position claim but also the named-successor claim simultaneously, strengthening the self-contradiction signal. A close marker asserting only terminal position without naming the successor table provides half the constraint: position is established but the specific artifact that must appear next is unspecified, leaving a gap that the position assertion alone cannot close. C-38 requires an explicit prohibition marker at the manifest's close stating that the analysis table begins directly below; C-51 requires that statement to name the specific phase-keyed successor table (TABLE-N) rather than referring generically to the analysis block or omitting successor identification.

**R15 triple structure:**

- **C-49** (named-register subdivision, per-section-label): ordering is not enough; the null/departure boundary must be structurally visible as a named section label, not inferred from question sequence.
- **C-50** (per-register close gate, per-register-boundary): the phase-level close gates the causal→schema boundary; the register-level close gates the null→departure boundary within the phase -- a finer-grained structural commitment.
- **C-51** (successor-naming, per-close-line): terminal-position assertion establishes what this line IS; successor-naming assertion establishes what MUST follow -- the two assertions address different failure modes and must co-occur.

C-49 can be satisfied without C-50 (named registers, single phase-level close). C-50 requires C-49 as a precondition (register-level close requires named registers to close). C-51 can be satisfied without C-49 or C-50 (manifest close lines name successors, no causal-phase named registers). Full expression requires all three: C-49 makes the register boundary visible, C-50 gates the boundary so the departure register is complete before schema entry, C-51 binds the manifest close to the specific next artifact so positional assertion and successor identity are co-asserted. V-05 carries evidence of all three; V-01 and V-04 satisfy C-47 (phase-level close) but carry none of C-49/C-50/C-51.
...
## Aspirational Criteria (weight: 86 points total, 2 pts each)
...
| Criterion | Source pattern | What v15 covered | What v16 adds |
|-----------|---------------|-----------------|---------------|
| C-49 | R15: V-05 causal phase structure showing NULL REGISTER and DEPARTURE REGISTER as explicitly labeled sub-sections within CAUSAL PHASE; V-01 and V-04 present all questions in a single undivided block with no named section labels. C-47 V-05 PASS evidence: "CAUSAL PHASE (NULL REGISTER + DEPARTURE REGISTER)" -- the parenthetical reveals named sub-structure that V-01/V-04 lack. | C-48 requires null questions before departure questions (ordering). Neither C-47 nor C-48 requires the two categories to be labeled as distinct named sections. | Requires the null/departure boundary to be structurally visible as a named section label, not inferred from question ordering. Named sections make the register boundary an explicit document element that can be independently verified. |
| C-50 | R15: V-05 C-47 PASS evidence: "`DEPARTURE REGISTER CLOSE`" as the mechanism enforcing causal-to-schema boundary. V-01/V-04 use `CAUSAL PHASE CLOSE` (phase-level). The register-level close gates the internal register boundary within the causal phase, not only the external phase-to-schema boundary. This is the register analog of C-38's manifest-close marker: just as C-38 prevents content insertion between manifest close and table header, C-50 prevents content insertion between departure register close and schema authorship. | C-47 requires a structural phase boundary between causal questions and schema authorship (causal phase close). C-38 requires manifest-close markers within each manifest block. Neither requires per-register close markers within the causal phase. | Requires each named register within the causal phase to carry its own close marker, gating the internal register-to-register and register-to-schema boundaries independently of the phase-level close. |
| C-51 | R15: V-05 C-38 PASS evidence: "Every manifest slot explicitly states: 'This is the last content line of MANIFEST-N. TABLE-N header follows directly below.'" vs. V-01--V-04 PARTIAL: "establishes terminal position but no explicit 'TABLE-N begins immediately below' text." The distinction is successor-naming: V-05 names the specific artifact that must follow; V-01--V-04 establish terminal position without naming the successor. | C-38 requires an explicit prohibition marker as the manifest's closing line stating that the analysis table begins directly below. C-40 requires dual-constraint form (content + position) in placeholders. Neither requires the close marker to name the specific phase-keyed successor artifact. | Requires the manifest-close line to name the specific successor table (TABLE-N) in addition to asserting terminal position, binding the close marker to a named artifact so that both the position assertion and the successor-identity assertion must be contradicted simultaneously to insert intervening content. |

---

**R15 triple structure:**

- **C-49** (named-register subdivision, per-section-label): question ordering enforces sequence; named section labels enforce structural visibility -- the null/departure boundary must be a document element, not a positional inference.
- **C-50** (per-register close gate, per-register-boundary): the phase-level close (C-47) gates the causal-to-schema transition; the register-level close (C-50) gates each internal register transition -- a finer-grained gate that prevents content from bleeding between registers within the phase.
- **C-51** (successor-naming, per-close-line): terminal-position assertion establishes what this line IS (the manifest's last content); successor-naming assertion establishes what artifact MUST follow (TABLE-N header) -- the two assertions address independent failure modes (position drift vs. successor substitution) and must co-occur in a single close line.

C-49 can be satisfied without C-50 (named registers exist but use a single phase-level close). C-50 requires C-49 as a structural precondition (register-level close requires named registers to close). C-51 can be satisfied without C-49 or C-50 (manifest close lines name successors; causal phase has no named registers). Full expression requires all three: C-49 makes the null/departure boundary a named document structure, C-50 gates each named register boundary within the causal phase, C-51 extends the manifest-close marker from terminal-position assertion to terminal-position-plus-successor-identity assertion. V-05 carries evidence of all three; V-01 and V-04 satisfy C-47 but carry none of C-49/C-50/C-51.
