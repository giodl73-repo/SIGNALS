Written to `simulations/quest/rubrics/campaign-blueprint-rubric-v6-2026-03-16.md`.

---

## What changed in v6

**Three new aspirational criteria from R5 excellence patterns:**

**C-22 — Per-site conviction type restatement** (5 pts) — V-01's three-field per-site reminder pattern. C-20 tests READ + PRESERVE per-site (two fields); C-22 tests READ + PRESERVE + CONVICTION TYPE per-site (three fields). V-01, V-03, V-04, V-05 all carry the three-field form and earn FULL; V-02 earns NO CREDIT because its per-site reminders are incomplete at two out of three artifacts. Full C-22 implies full C-20.

**C-23 — Inertia-anchored conviction arc** (5 pts) — V-05's pattern. Conviction types grounded in the competitive baseline rather than abstract labels: spec names the problem the inertia baseline creates, proposal prices the cost of inertia, pitch makes the inertia cost visceral. Makes C-21's Factual/Optionality/Emotional labels operationally precise. Requires an inertia model defined at the campaign level (D7) — a campaign-level architectural choice, not a local improvement. Only V-05 earns FULL.

**C-24 — Scout-to-must-have traceability** (5 pts) — V-05's pattern. C-10 requires a unidirectional scout → artifact pull; C-24 requires the reverse linkage: each spec Must-have cites its originating scout-requirements friction. Makes proposal options and pitch CTA auditable against the scout evidence chain. Only V-05 earns FULL.

**Scale:** 150 base + 8 excellence = **158 ceiling**

**Retroactive R5 under v6:** V-02 = 140.5, V-01 = 145.5, V-03/V-04 = 148, V-05 = 158 (ceiling). V-05 stands alone — the only R5 variant to reach the new ceiling.
plete three-field per-site reminder automatically satisfies C-20.
- **D7**: C-23 and C-24 require an inertia baseline defined at the campaign level.
  Campaigns without an explicit inertia model cannot earn these criteria regardless of
  conviction typing quality. The inertia baseline is a campaign-level architectural
  choice, not a local improvement.

**Scale:** 150 base + 8 excellence = **158 ceiling**

**Retroactive R5 scoring** under v6 differentiates all five variants where v5 produced
a three-way tie at 143:
...
V-05 alone reaches the v6 ceiling. V-02's lack of structured per-site reminders costs
it C-22; no variant other than V-05 declares inertia-anchored conviction types or
labeled scout-to-must-have traceability.
...
performers from the passing tier. Scored as a bonus above the 150-pt base.
...
Aspirational = sum of passed/partial aspirational × weight  (max 60)
...
friction) is the strongest implementation; see C-24.
...
### C-22: Per-site conviction type restatement (aspirational) — new in v6
...
carries `Contract reminder — READ: … PRESERVE: … CONVICTION TYPE: …` — all
three obligations restated at the site where execution begins.
...
### C-23: Inertia-anchored conviction arc (aspirational) — new in v6
...
the inertia baseline; the CONVICTION DELTA audit is typed to the inertia-cost
form of the pitch's conviction role.
Partial (2.5 pts): some artifacts declare inertia-anchored roles, or the audit
is typed to the inertia role but the spec/proposal roles are not
inertia-grounded.
...
### C-24: Scout-to-must-have traceability (aspirational) — new in v6
...
V-05 (R5) is the reference implementation. The pattern: "Each Must-have must
trace to a specific friction the inertia baseline creates." The spec section
for each must-have includes a labeled reference to its originating scout
signal, making the proposal's option evaluation and the pitch's CTA auditable
against the scout evidence chain — an option addresses must-haves that trace
to real signals, not constructed requirements.
...
or the citations are informal (embedded in prose rather than labeled).
...
- V-01–V-04: CONVICTION TYPE labels use abstract categories (Factual /
...
- V-01–V-04: Scout signals pulled unconditionally into the spec (C-10 FULL)
