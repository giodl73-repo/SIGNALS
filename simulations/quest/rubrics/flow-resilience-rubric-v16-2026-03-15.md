All landmarks are correct. Written to `simulations/quest/rubrics/flow-resilience-rubric-v16-2026-03-15.md` (1715 lines).

**v16 changes summary:**

- **C-47 -- VS Cross-Reference Registry (Act 1 Terminal) *(E-33)*** added as the 39th aspirational criterion, cracked by V-05 only in R16
- **Orthogonality**: C-35 (per-row VS) / C-28 (rule audit) / C-38 (chaos-obs matrix) — the C-35→C-47 escalation mirrors C-38→C-44
- **Scoring delta**: uncapped max 76 → 78; composite cap unchanged at 10
- **R16 discrimination**: V-05 78/78 alone; V-02 and V-04 tied at 76/78; V-03 at 74/78; V-01 at 72/78
- **Open for R17**: C-47 for all four non-V-05 variations; C-45 still open for V-03; C-46 still open for V-01
---------|
| 1 | V-05 | PASS | **78/78** |
| 2 (tie) | V-02 | FAIL | **76/78** |
| 2 (tie) | V-04 | FAIL | **76/78** |
| 4 | V-03 | FAIL | **74/78** |
| 5 | V-01 | FAIL | **72/78** |

The two-way tie between V-02 and V-04 at 76/78 remains unbroken under v16 (both are at uncapped
ceiling for the round, differing only in construction path -- V-02 via R15-V04 base with C-45
inherited; V-04 via R15-V01 base with C-44+C-45+C-46 added simultaneously). V-05 advances to
78/78 as the new perfect uncapped ceiling. Open for R17: C-47 PASS for V-01/V-02/V-03/V-04.

**Orthogonality argument:**
- **C-47** is orthogonal to C-35 (C-35 requires a named VS per recovery stage per scenario row;
  C-47 requires a terminal Act 1 artifact enumerating all VS from all rows with Observable Signal
  cross-references -- the per-row requirement and the terminal catalog are independently satisfiable
  and independently auditable). It is also orthogonal to C-28 (post-analysis rule registry audit,
  which confirms rule invocation completeness; C-47 confirms VS production completeness and
  VS-to-observability linkage). It is also orthogonal to C-38 (chaos-observability bidirectional
  matrix, which cross-references chaos scenarios to Observable Signals; C-47 cross-references
  recovery stage VS to Observable Signals). The C-35/C-47 split mirrors the C-38/C-44 split:
  C-38 requires gap findings from the matrix; C-44 requires those findings in a formal three-field
  registry structure. C-35 requires VS per stage; C-47 requires a terminal registry confirming all
  VS were produced and linking each to the observability layer.

**C-47 -- VS Cross-Reference Registry (Act 1 Terminal) (E-33)**
- Signal: V-05 in R16 produces a named VS Cross-Reference Registry at Act 1 terminal position.
  The registry enumerates every Verification Signal from every recovery stage (Detect, Contain,
  Recover, Validate) across all scenarios in Act 1. Each registry entry carries: the scenario ID,
  the recovery stage name, the VS text, and an Observable Signal cross-reference (Observable Signal
  ID from the C-10/C-37 observability layer, or an explicit nil marker when no correspondence
  exists). V-01/V-02/V-03/V-04 all carry C-35 PASS (per-row VS present in each stage) but none
  produce a terminal VS catalog.
- The gap it closes: C-35 requires a named, observable, mechanism-distinct VS per recovery stage
  per scenario. When multiple scenarios each carry four stages, the VS population grows
  proportionally. Without a terminal registry, the VS catalog is distributed across scenario rows
  and is not auditable at catalog level -- a reviewer confirming VS completeness must traverse all
  scenario prose. The registry closes this gap in three directions: (1) completeness -- the
  terminal artifact confirms all scenario x stage slots produced a VS, making omissions immediately
  detectable; (2) deduplication -- VS that are identical or near-identical across scenarios are
  visible in column view, revealing scenarios that share a VS rather than carrying independently
  named observables; (3) observability linkage -- the VS-to-Observable-Signal cross-reference
  identifies VS that have no corresponding observability layer coverage (a recovery stage can be
  confirmed closed by a signal that production monitoring does not track) and Observable Signals
  that no VS references (monitoring coverage that no recovery path trigger exercises). The registry
  also establishes a pre-COMPLETE Act 1 audit checkpoint: COMPLETE may not be declared while any
  scenario x stage slot in the registry carries a blank VS or an unresolved cross-reference status.
- Pass requires: A named VS Cross-Reference Registry (or equivalently named terminal artifact)
  present at Act 1 close position. The registry enumerates every VS from every recovery stage in
  every Act 1 scenario, identified by scenario ID and stage name. Each VS entry carries an
  Observable Signal cross-reference field (Observable Signal ID or explicit N/A nil marker -- not
  blank). The artifact must be positioned at Act 1 terminal level (not embedded within an
  individual scenario row or gate prose block). At least one entry illustrates a concrete Observable
  Signal cross-reference by ID. C-35 FAIL subsumes C-47.
- Partial: Terminal VS catalog present but entries are not organized by scenario ID and stage
  (VS list without scenario x stage indexing); or cross-reference present but Observable Signal
  linkage field absent for all entries (enumeration without cross-reference); or catalog covers only
  some scenarios or some stages rather than the complete Act 1 VS population; or catalog embedded
  within an individual scenario row rather than as a terminal artifact; or C-35 PARTIAL with
  terminal catalog covering only the stages where VS exist.
- Fail: No terminal VS Cross-Reference Registry; VS distributed across scenario rows with no Act 1
  terminal catalog artifact; or C-35 FAIL subsumes C-47.

**Scoring delta**: aspirational cap unchanged at 10; uncapped max rises from 76 -> 78
(39 criteria x PASS=2).

**R16 discrimination under v16 (uncapped aspirational tiebreak; all composite = 100/100):**

| Rank | Variation | C-44 | C-45 | C-46 | C-47 | Uncapped Asp. |
|------|-----------|------|------|------|------|--------------|
| 1 | **V-05** | PASS | PASS | PASS | PASS | 78/78 |
| 2 (tie) | **V-02** | PASS | PASS | PASS | FAIL | 76/78 |
| 2 (tie) | **V-04** | PASS | PASS | PASS | FAIL | 76/78 |
| 4 | **V-03** | PASS | FAIL | PASS | FAIL | 74/78 |
| 5 | **V-01** | PASS | FAIL | FAIL | FAIL | 72/78 |

Open for R17: C-47 PASS for V-01/V-02/V-03/V-04 (no terminal VS catalog present); C-45 PASS for
V-03 (no completeness block); C-46 PASS for V-01 (prose nil; three-field form absent).
...
Uncapped maximum across 39 criteria is 78; the cap rewards breadth while holding the composite
ceiling at 100.
...
| Aspirational | 10% | C-09 through C-47 | PASS=2, PARTIAL=1, FAIL=0; capped at 10 |
...
**Ceiling references under v16:**
- R16 V-01 (C-45/C-46 FAIL; C-47 FAIL): 72/78 uncapped
  -> composite = 60 + 30 + 10 = **100** (capped)
- R16 V-03 (C-45 FAIL; C-47 FAIL): 74/78 uncapped
  -> composite = **100** (capped)
- R16 V-02 (C-47 FAIL): 76/78 uncapped
  -> composite = **100** (capped)
- R16 V-04 (C-47 FAIL): 76/78 uncapped
  -> composite = **100** (capped)
- R16 V-05 (all 39 criteria PASS): 78/78 uncapped
  -> composite = **100** (new uncapped ceiling: 78/78 -- first perfect score under v16)
- Open for R17: C-47 PASS for V-01/V-02/V-03/V-04 (no terminal VS catalog present); C-45 PASS
  for V-03 (no completeness block); C-46 PASS for V-01 (prose nil in both directions).
