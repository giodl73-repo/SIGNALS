...
Simulates a committee meeting before the real one. Types: ROB (product/service review), shiproom (go/no-go), arch-board (architecture decision review). Reads participant roles from `.roles/`. Output: mock meeting minutes with decisions, action items, dissenting opinions.
...
| C-10 | **Charter fidelity is traceable** | correctness | The output cites or demonstrates specific alignment with the committee's charter (from `.roles/` or stated inline): quorum rules, scope boundaries, veto rights, escalation path. At least two charter-specific constraints are visibly honored in the minutes. |
