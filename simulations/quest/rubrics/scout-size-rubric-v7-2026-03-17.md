Written to `simulations/quest/rubrics/scout-size-rubric-v7-2026-03-17.md`.

---

**Two new aspirational criteria added (C-24 through C-25):**

| ID | Pattern source | What it checks |
|----|----------------|----------------|
| **C-24** | R6 V-03 "No self-check machinery" / V-04/V-05 R6 | Synthesis provides a structured three-step trace (gate commitment / analysis result / verdict) on separate labeled lines — not folded into prose. Passing C-22 (synthesis names gate twice in prose) does not satisfy C-24 if the chain is embedded in a paragraph that requires re-reading to verify logical continuity. |
| **C-25** | R6 V-05 PRE-FLIGHT GATE forward-reference | PRE-FLIGHT GATE names the downstream sections that enforce its scope exclusions and hypothesis commitment — the outbound direction of the navigational mesh. Passing C-23 (guards name the canonical home) does not satisfy C-25 if the gate itself does not list its guarantors. |

**What makes these genuinely new (not redundant with v6):**

- C-24 is not a refinement of C-22. C-22 checks whether synthesis names PRE-FLIGHT GATE at both anchor and verdict close. C-24 checks whether synthesis exposes the three-step commitment chain in a scannable structure. An output can pass C-22 with "look back at PRE-FLIGHT GATE — the preliminary hypothesis committed there was MEDIUM / 3-5 sprints; analysis confirms. The preliminary hypothesis committed in PRE-FLIGHT GATE was correct." — and fail C-24 because the chain is prose, not a labeled trace.

- C-25 is not a refinement of C-23. C-23 checks whether guards in adjacent sections name the canonical home (inbound arrows). C-25 checks whether the canonical home names its enforcement sections (outbound arrows). An output can pass C-23 with every guard reading "scope was resolved in PRE-FLIGHT GATE" and still fail C-25 if PRE-FLIGHT GATE does not list which sections enforce it.

**Scoring model:** aspirational denominator bumped 15 → 17. Max composite unchanged at 100.

**Expected R6 scores under v7:**

| Variation | Aspirational | Score |
|-----------|-------------|-------|
| V-01 (synthesis named chain, C-22 only) | 13/17 (fails C-20, C-23, C-24, C-25) | 97.65 |
| V-02 (navigational guards, C-23 only) | 13/17 (fails C-21, C-22, C-24, C-25) | 97.65 |
| V-03 (minimal combination, no self-check) | 15/17 (fails C-24, C-25) | 98.82 |
| V-04/V-05 (with self-check machinery) | 17/17 | 100 |
h basis
- **Weight**: essential
- **Category**: depth
