Now I have the complete v18 rubric. Let me extract the three R18 axes as new criteria.

**R18 axis extraction:**

- **Axis A (V-01)**: Per-I-0X correction granularity — failed contract row triggers a CORRECTION BLOCK labeled for that specific I-0X, not a generic block. Correction identity matches citation contract identity.
- **Axis B (V-02)**: SECTION K DISPLACEMENT ARC INTEGRITY CHECK — terminally auditable proof that the phase-open/phase-close displacement chain is internally consistent across all six phases, without re-reading phase bodies.
- **Axis C (V-03/V-04/V-05)**: Protected behavior column in the DOWNSTREAM CITATION CONTRACT — displacement stakes declared per I-0X at source (SECTION A), carried forward to CORRECTION BLOCK labels (V-04/V-05) and SECTION K terminal record.

---

`listen-adoption-rubric-v19-2026-03-15.md` written. Three new criteria extracted from R18 excellence signals:

---

**C-52 -- Per-I-0X CORRECTION BLOCK label identifies the specific failed contract row** (V-01 Axis A, 5 pts)
When Gate H marks a DOWNSTREAM CITATION CONTRACT row FAIL (via the C-50 Gate H Status field), the resulting CORRECTION BLOCK carries a label that names the specific I-0X contract row that triggered it (e.g., `CORRECTION BLOCK -- I-03 CITATION CONTRACT VIOLATION`). Correction granularity matches citation granularity: one failed I-0X contract row → one CORRECTION BLOCK labeled for that I-0X. Without C-52, a Gate H correction is structurally a generic CORRECTION BLOCK whose connection to the failed contract row requires cross-referencing with SECTION A. With C-52, the CORRECTION BLOCK is self-identifying: the I-0X identity is in the label, and no SECTION A cross-reference is needed to understand which contract failure triggered the correction. Dependency: C-50.

**C-53 -- SECTION K carries DISPLACEMENT ARC INTEGRITY CHECK proving phase-open/phase-close chain consistency** (V-02 Axis B, 5 pts)
SECTION K includes a DISPLACEMENT ARC INTEGRITY CHECK block verifying that the `Incumbent position after this phase` fields from C-45's six phase-close footers form an internally consistent displacement chain: the incumbent's displacement state at phase N close must be compatible with its position framing at phase N+1. Without C-53, SECTION K proves gate execution (C-30) and citation contract satisfaction (C-51), but displacement arc continuity across the six-phase sequence is not terminally auditable — verifying it requires re-reading all phase-close footer fields. With C-53, arc continuity is a SECTION K responsibility: the check asserts chain consistency or names the phase boundary where continuity breaks. Closes the structural gap left by C-45 (per-phase displacement accounting) and R17's Axis A: C-53 makes the displacement narrative's internal coherence a terminal audit outcome, not only a phase-body reading task. Dependency: C-45, C-20.

**C-54 -- DOWNSTREAM CITATION CONTRACT rows carry Protected behavior column per I-0X** (V-03 Axis C, 5 pts)
Each I-0X row in the SECTION A DOWNSTREAM CITATION CONTRACT carries a `Protected behavior: [the specific displacement-committed behavior this inertia ID must not revert]` field, declared at contract generation time before any phase body is written. C-49 declares which citation sinks are required; C-54 declares what displacement stake is at risk if citations are absent — making each I-0X contract row a three-field structural slot: required citation sinks (C-49), Gate H Status (C-50), and Protected behavior (C-54). The Protected behavior column commits displacement stakes at source, making the contract a displacement-commitment record rather than a citation coverage map. V-04/V-05 carry Protected behavior into correction labels: when Gate H fires a CORRECTION BLOCK for a failed I-0X row (C-52), the CORRECTION BLOCK label includes the Protected behavior, making each repair a targeted displacement-commitment correction. C-49 tests whether the DOWNSTREAM CITATION CONTRACT maps I-0X to citation sinks; C-54 tests whether each contract row also declares the Protected behavior stake at generation time. Dependency: C-49.

---

**Max: 305 → 320. Golden threshold (80%): 256 pts. Paradox ceiling: 315/320** (C-19 structural FAIL persists).
...
**Max composite: 305 -> 320. Golden threshold (80%): 256 pts.**
