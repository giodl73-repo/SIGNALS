Written to `simulations/quest/rubrics/listen-support-rubric-v4-2026-03-14.md`.

---

**What changed from v3 to v4:**

**C-15 -- Table-Form Coverage Enumeration** (5 pts, depth)
Drawn from Round 3 excellence signal 1. C-13 passes with any ticket-ID enumeration; C-15 passes only when the enumeration is a table (one row per ticket). The key failure mode in text-format: "T-01 and T-02 both trace to X. Count: 2. Passes." -- IDs present, per-ticket independence collapsed. A table makes row count self-verifying without parsing sentence structure; single-sentence summaries spanning multiple tickets are structurally impossible. V-01 (table) satisfies C-15. V-02 (text check with example) fails C-15.

**C-16 -- Container-Free Consequence Field Format** (5 pts, behavior)
Drawn from Round 3 excellence signal 2. C-14 passes when consequence components are in structurally separate fields (including nested); C-16 passes only when those fields are flat top-level siblings with no parent "Consequence:" container. The key failure mode: the parent label anchors a generic response at container level while appearing to satisfy each nested bullet. Flat fields at the same level as "Pattern name:" carry no parent anchor -- each is independently checkable, nothing to hide behind. V-02 (flat fields) satisfies C-16. V-01 (nested under "Consequence:") fails C-16.

**Tier restructure:**
- Aspirational: 6 criteria / 30 pts → 8 criteria / 40 pts
- Total ceiling: 120 → 130

The relationship between the criteria pairs is now explicit: C-13 is the gate (enumeration exists), C-15 is the format (table); C-14 is the structure (separate fields), C-16 is the topology (no container). Both pairs follow the same pattern as C-11/C-13 and C-12/C-14 -- each round tightens the structural requirement one level further.
load-bearing mechanism for partial compliance.

**Tier restructure:**
- Aspirational: 6 criteria / 30 pts -> 8 criteria / 40 pts
- Total ceiling: 120 -> 130 (scores above 120 require all eight aspirational criteria)
...
## Aspirational Criteria (40 pts total -- raises the bar above 90)
...
**Essential ceiling**: 60 | **Recommended ceiling**: 90 | **Aspirational ceiling**: 130
