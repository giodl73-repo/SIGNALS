`discover-causal-rubric-v13-2026-03-17.md` written.

---

**What changed: v12 -> v13**

**1 new criterion: C-35** -- "C-34 structural requirement named at Phase 6 synthesis"

C-35 closes the synthesis-point naming chain. The pattern: every enforcement-chain criterion gets a corresponding Phase 6 naming criterion. C-29 names C-26, C-32 names C-31, and now C-35 names C-34.

**C-35 pass condition:** Phase 6 must name two elements: (1) absence of `phase1_anchor_prohibition_stated` from ARTIFACT frontmatter fails C-34 separately from the C-27 fail; (2) `false` vs absent is the E-02 gradeability distinction. Both required -- field-absence alone without the `false`-vs-absent clause does not pass.

**Gradeability:** C-35 requires C-34 PASS (parallel to C-32 requiring C-31 PASS). V-04 (C-34 FAIL) gets C-35 N/A and drops from Golden to Acceptable under v13.

**Scoring:**
- 27 aspirational x 5 = 135 pts | Total **225 pts**
- Golden **>= 215** | Acceptable **200-214** | Passing **185-199**
- Previous ceiling (220) = new Golden floor

**R12 retro-scores under v13:** V-01/V-02/V-03 score 215 (Golden floor, all fail C-35); V-04 scores 210 (Acceptable, C-35 N/A); V-05 scores 225 (ceiling, passes C-35).
e` not absent -- `false` vs absent is the E-02 gradeability distinction. Both elements are required; naming field absence alone without the `false`-vs-absent distinction does not pass.

**Gradeability:** C-35 requires C-34 PASS. If C-34 N/A (complete pathway) or FAIL (field absent), C-35 is not scored. Parallel to C-32 requiring C-31 PASS.

**Scoring update:**
- Aspirational: 130 -> 135 pts (27 criteria x 5 pts)
- Total: 220 -> **225 pts**
- Golden **>= 215** | Acceptable **200-214** | Passing **185-199**
- Previous ceiling (220) = new Golden floor
...
| Variation | v12 score | C-35 | v13 score | Tier |
|-----------|-----------|------|-----------|------|
| V-01 (C-34 stripped) | 215 | FAIL | **215** | Golden (floor) |
| V-02 (C-34 body-placement) | 215 | FAIL | **215** | Golden (floor) |
| V-03 (C-27 stripped + field false) | 215 | FAIL | **215** | Golden (floor) |
| V-04 (C-27 stripped + field absent) | 210 | N/A | **210** | Acceptable |
| V-05 (full ceiling) | 220 | PASS | **225** | Golden (ceiling) |

C-35 FAIL for V-01 through V-03: all pass C-34 (field present) but none carry a Phase 6 integration rule naming the C-34 requirement and E-02 `false`-vs-absent distinction. C-35 N/A for V-04: C-34 FAIL (field absent) means C-35 is not gradeable. V-04 drops to Acceptable under v13 because the N/A on C-35 means it does not benefit from the new 5-pt band.
...
| Aspirational | C-09 through C-35 | 135 |

**C-21 through C-35 relationship to earlier criteria:** These fifteen criteria do not replace C-12, C-15, C-18, C-19, C-20, or C-26 -- they capture the stricter pass conditions those criteria require at full depth. A response can pass C-26 (anchor at Phase 1) while failing C-27 (no prohibition named at Phase 1), C-28 (Phase 3 does not back-reference it), C-29 (no Phase 6 integration rule), C-30 (Phase 6 carries stale Phase 1 anchor instead of Phase 3 updated anchor), or C-31 (Phase 3 update point carries no prohibited form). A response can pass C-31 while failing C-32: Phase 3 carries the self-referential prohibited form, but Phase 6 never names that structural requirement at synthesis. A response can pass C-30 while failing C-33: anchor provenance is tracked in the ARTIFACT frontmatter, but the Phase 3 prohibited form status is not. A response can pass C-27 while failing C-34: the Phase 1 prohibition phrase is present in the response, but the `phase1_anchor_prohibition_stated` ARTIFACT field is absent. A response can pass C-34 while failing C-35: the `phase1_anchor_prohibition_stated` ARTIFACT field is present and accurate, but Phase 6 never names the C-34 structural requirement or E-02 `false`-vs-absent gradeability distinction at synthesis.
...
The distinction from C-26, C-28, C-29, C-31, C-32, C-33, C-34, and C-35:
...
- C-35: Phase 6 names the C-34 structural requirement (field absence fails C-34 independently; `false` vs absent is the gradeability distinction)

A response can pass C-26, C-27, C-28, and C-29 while failing C-30: all cross-phase integrity rules are named, Phase 1 and Phase 3 are correctly sequenced, but Phase 6 silently carries the Phase 1 step when Phase 3 upgraded it. A response can pass C-30 while failing C-31: Phase 6 names the stale-anchor failure mode, but Phase 3 never names that its own update record is insufficient to satisfy Phase 6. A response can pass C-31 while failing C-32: Phase 3 carries the prohibited form, but Phase 6 never names that C-31 structural requirement at synthesis. A response can pass C-30 while failing C-33: anchor provenance ARTIFACT field present, but Phase 3 co-location status field absent. A response can pass C-27 while failing C-34: Phase 1 prohibition phrase present in response text, but `phase1_anchor_prohibition_stated` ARTIFACT field absent. A response can pass C-34 while failing C-35: ARTIFACT field present and accurate, but Phase 6 never names the C-34 structural requirement or E-02 `false`-vs-absent gradeability distinction at synthesis.
...
|-------|---------------|
| All essential pass + >= 215 | Golden -- meets bar for production use |
| All essential pass + 200-214 | Acceptable -- useful but shallow on aspirational |
| All essential pass + 185-199 | Passing -- essential + recommended coverage only |
| Any essential fail | Below bar -- output is unreliable regardless of score |
| < 185 | Below bar |

**Maximum: 225 pts** (60 essential + 30 recommended + 135 aspirational)
...
**Note on C-35 gradeability:** C-35 requires C-34 PASS. If C-34 is N/A (complete pathway, incompleteness not declared) or FAIL (field absent from ARTIFACT), C-35 is not scored -- there is no ARTIFACT field in place for Phase 6 to name its structural requirements. C-35 captures the synthesis-phase naming requirement for the C-34 structural requirement, parallel to how C-29 captures it for C-26 and C-32 captures it for C-31. Any Phase 6 position satisfies the criterion -- a single entry in the integration rules list is sufficient. Both elements must be named: (1) field absence fails C-34 independently of C-27 fail; (2) `false` vs absent is the gradeability distinction.
