# trace-request Rubric — v17 (2026-03-15)

**Scale:** 270 (Essential 60 + Recommended 30 + Aspirational 180)
**Golden threshold:** all 4 essential PASS AND composite >= 80% (216/270)
...
**Scale:** 260 → **270** (aspirational tier: 34 → 36 criteria, 170 → 180 pts). Golden threshold stays at >= 80%.
...
## Signals not promoted in v17 — C-44 design surface

1. **TRACE CONTRACT validation outcome derivable from DECLARE CONTRADICTION match flag (C-44 design surface, opened by R18 V-02):** When DECLARE CONTRADICTION fires with `matches HALT-EXPECTED-BOUNDARY: no`, the TRACE CONTRACT validation structurally should produce the "falsified -- actual halt boundary was [label]" variant; when `matches HALT-EXPECTED-BOUNDARY: yes`, the validation should produce "Contract prediction confirmed." The machine-verifiable consistency constraint between the Step 8b `matches HALT-EXPECTED-BOUNDARY: [yes/no]` field (C-42) and the TRACE CONTRACT validation outcome token (C-39) is structurally designable but has not yet been tested under live execution conditions where the falsification path actually fires — R18 V-02 specified the falsification branch in GATE-TC and surfaced `matches HALT-EXPECTED-BOUNDARY: no` as a required output variant, but actual paired execution evidence showing the two tokens agree across confirmation and falsification branches is needed before the consistency constraint can be frozen as a testable criterion. Needs multi-round evidence showing matching (DECLARE CONTRADICTION match-field value) → (TRACE CONTRACT validation outcome) pairs across both yes/confirmed and no/falsified execution branches.

---

## Two new criteria from R17 excellence signals (carried from v16)
...
**Scale:** 250 → **260** (aspirational tier: 32 → 34 criteria, 160 → 170 pts).

---

## Key design decisions (R17, carried from v16)
...
## Signals not promoted in v16 — promoted to C-42 and C-43 in v17

Both C-42 design surfaces identified in v16 have now been promoted:

1. **DECLARE CONTRADICTION intertextual HALT-EXPECTED-BOUNDARY match flag** → promoted to **C-42** above (V-02 single-axis + V-05 combined = two independent axes)
2. **Error path category taxonomy** → promoted to **C-43** above (V-03 single-axis + V-05 combined = two independent axes with frozen six-item vocabulary)

---

## One new criterion from R16 excellence signal (carried from v16)
...
## Key design decisions (R16, carried from v16)
...
## One new criterion from R15 excellence signal (carried from v16)
...
## Key design decisions (R15, carried from v16)
...
## One new criterion from R13 excellence signal (carried from v16)
...
## Key design decisions (R13, carried from v16)
...
## Three new criteria from R12 excellence signals (carried from v16)
...
## Key design decisions (R12, carried from v16)
...
## Three new criteria from R11 excellence signals (carried from v16)
...
## Three new criteria from R10 excellence signals (carried from v16)
...
## Full aspirational tier — C-21 through C-43
...
**Aspirational subtotal (C-21–C-43):** 23 criteria × 5 = 115 pts of the 180 pt aspirational tier. Criteria C-08 through C-20 (13 criteria × 5 = 65 pts) are stable and inherited from v12 without modification.
...
| Aspirational | C-08–C-43 | 180 |
| **Total** | **43 criteria** | **270** |

**Golden threshold:** all 4 essential PASS AND composite >= 216/270 (80%).

---

## Dependency chain — checker contract stack (C-28 → C-43)
...
**C-42** closes the intertextual cross-reference chain: C-39 pre-declares the halt boundary as HALT-EXPECTED-BOUNDARY; C-40 names the contradiction event as DECLARE CONTRADICTION with BC-N, label, and arm fields in Step 8b prose; C-42 adds the fourth field `matches HALT-EXPECTED-BOUNDARY: [yes/no]` that makes the relationship between the contradiction event and the pre-declared prediction machine-checkable at the Step 8b surface without navigating to the TRACE CONTRACT validation token. The `yes/no` value is structurally determined: `yes` iff the BC-N in DECLARE CONTRADICTION matches the BC-N in HALT-EXPECTED-BOUNDARY; `no` iff it differs. This creates a three-token consistency surface: HALT-EXPECTED-BOUNDARY (Phase 0A) → HALT-EXPECTED-BOUNDARY in HALT-RULE (Phase 0B, C-41) → `matches HALT-EXPECTED-BOUNDARY: [yes/no]` in DECLARE CONTRADICTION (Step 8b, C-42) → TRACE CONTRACT validation outcome (C-39 post-execution). The C-44 design surface (not yet promoted) targets the machine-verifiable consistency between the C-42 match field and the C-39 validation outcome token.
