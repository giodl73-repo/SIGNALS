Written to `simulations/quest/rubrics/program-plan-rubric-v2-2026-03-15.md`.
...
A BAD/GOOD pair for at least one essential failure mode makes the violation visible as a concrete output shape rather than an abstract prohibition. Source: V-05's inertia contrast was the clearest C-04 pass in R1; V-02's inline BAD/EXAMPLE comments achieved the same at lower scaffolding cost.

**C-11 — Structural Enforcement of Essential Requirement**
At least one essential requirement is embedded in document structure rather than stated as a prose rule — making correct output the path of least resistance. Source: V-04's lifecycle bands (Prove It / Design It / Simulate It / Listen Ahead) converted C-05 from PARTIAL in three variates to a clean PASS by encoding namespace order as structural headers; V-02's `# REQUIRED` annotation made the echo slot unforgettable.

**C-12 — Dual-Anchor Reinforcement of Essential Criteria**
Each essential criterion (C-01–C-04) appears through at least two independent mechanisms, so a model that skips one location still encounters the requirement elsewhere. Source: V-01 (header + YAML comment), V-04 (band framing + per-band YAML), V-05 (opening paragraph + YAML comment + GOOD example) all show this pattern lifting scores on criteria V-02 and V-03 only partially satisfied.

**Scoring change:** Aspirational tier expanded from 2 to 5 criteria, 10 → 25 pts. Total max is now 115. Golden threshold (all essential pass AND composite ≥ 80) is unchanged.
tage. The `auto: true`
