...
ical to C-40: string comparison only, no reader inference.

**C-43 design rationale.**
C-41 filled the entry contract's backward `Prior phase:` slot. C-43 fills the symmetric forward slot on the exit gate: a named `Next phase:` sub-field in every PHASE EXIT GATE declares which phase follows by literal identifier. The last phase carries NONE/END sentinel. Together C-41+C-43 complete the bidirectional Phase->Phase gate-to-gate linkage: entry contract declares the predecessor phase backward; exit gate declares the successor phase forward. Out-of-order phase rendering is now detectable in both directions by identifier mismatch, fully independent of document position. The exit gate's `Next phase:` sub-field is orthogonal to its `Blocked bottleneck:` sub-field (C-37) -- one carries a Phase label, the other carries a B-ID, independently verifiable. CHAIN STATUS gains a `Phase-exit-sequence` direction. C-43 is to C-41 what C-37 is to C-36 on the exit side: a named sub-field that converts an implicit successor ordering into a string-comparable traceability artifact.

**Full dependency chain (v13):**
...
**Formula:** `aspirational_pass / 35 * 10` -- golden threshold unchanged.

---

# flow-lifecycle -- Rubric v13
...
- **Weight**: aspirational
- **Category**: depth
- **Text**: Round 12 signal: C-40 closed the bidirectional Role<->Exception loop. C-42
  closes the symmetric B-NN<->Exception loop. Each EX block carries a named
  `Bottleneck Ref:` sub-field citing the B-ID that the exception exposes or is caused by
  (e.g., `Bottleneck Ref: B-02`) -- or NONE if the exception is not traceable to a
  declared bottleneck. The Bottleneck Analysis section additionally gains a gate check
  validating that every declared B-ID appears in at least one EX block's `Bottleneck Ref:`
  sub-field, closing the chain bidirectionally (B-NN->Exception in the gate;
  Exception->B-NN in the EX block). "Dark bottlenecks" -- declared in the bottleneck
  analysis but with no observable exception-phase trace -- become named gate violations
  detectable by string comparison. CHAIN STATUS gains a `B-NN->Exception` direction.

  C-42 is the fifth orthogonal traceability direction alongside SLA<->B-NN (C-14/C-16),
  Baseline->B-NN (C-34), Phase->B-NN (C-37), and Role<->Exception (C-40). The structural
  property is identical to C-40: string comparison only, no reader inference. The gate
  closure (B-NN->Exception) ensures that no declared bottleneck is a "dark bottleneck"
  relative to the exception output, making the bottleneck analysis jointly auditable
  against exception traces just as C-40 makes the role registry jointly auditable against
  exception traces. C-42 cannot pass if C-04 fails (bottlenecks must be declared as B-ID
  entries before EX block citations can be string-compared) or C-13 fails (exception
  traces must be phase-scoped for per-phase B-ID assignment to be operationally
  meaningful).
- **Pass condition**: (a) Each EX block carries a named sub-field -- labeled
  "Bottleneck Ref:", "B-ID Ref:", "Causal Bottleneck:", or equivalent -- that carries
  either a literal B-ID matching a declared bottleneck block (e.g., `Bottleneck Ref: B-02`)
  or an explicit NONE declaration. A free-text description of the bottleneck without a
  B-ID identifier fails; a cited B-ID absent from the Bottleneck Analysis section fails;
  a blank sub-field fails. AND (b) the Bottleneck Analysis section (C-04 output) or a
  dedicated gate block immediately preceding or following the bottleneck entries includes
  an explicit gate validation confirming that each declared B-ID appears in at least one
  EX block's bottleneck-citation sub-field -- the gate must be a named structural element
  (labeled check, validation instruction, or equivalent), not an implicit assumption.
  CHAIN STATUS must enumerate a `B-NN->Exception` direction. Both (a) and (b) must be
  independently satisfied. Fail if any EX block lacks a bottleneck-citation sub-field, if
  a cited B-ID does not match a declared bottleneck block identifier, or if the gate
  validation is absent. C-42 cannot pass if C-04 fails or C-13 fails.

### C-43 - Phase Exit Gate Forward Phase Sequential Linkage
- **Weight**: aspirational
- **Category**: format
- **Text**: Round 12 signal: C-41 filled the entry contract's backward `Prior phase:` slot,
  making the phase sequence backward-declarable by identifier. The exit gate carries a
  `Blocked bottleneck:` sub-field (C-37) establishing it as the forward-reference anchor
  of the gate architecture, but it has no forward Phase reference. C-43 fills the
  symmetric forward slot: a named `Next phase:` sub-field in every PHASE EXIT GATE
  declares which phase follows by literal identifier (e.g., `Next phase: PHASE 2 --
  Evaluation`). The last phase carries an explicit sentinel (NONE, END, or equivalent)
  indicating no successor. Together C-41 and C-43 complete the bidirectional Phase->Phase
  gate-to-gate linkage: the entry contract declares the predecessor phase backward; the
  exit gate declares the successor phase forward. Out-of-order phase rendering is now
  detectable in both directions by identifier mismatch, fully independent of document
  position.

  The exit gate's `Next phase:` sub-field is structurally orthogonal to its
  `Blocked bottleneck:` sub-field (C-37): one carries a Phase label, the other carries a
  B-ID -- independently verifiable string comparisons. CHAIN STATUS gains a
  `Phase-exit-sequence` direction verifying that each exit gate's `Next phase:` identifier
  string-matches the label of the subsequent phase block in document order. C-43 is to
  C-41 what C-37 is to C-36 on the exit side: a named sub-field that converts an implicit
  successor ordering into a string-comparable traceability artifact. C-43 cannot pass if
  C-37 fails (the exit gate must already carry forward-reference sub-fields, establishing
  the structural pattern that C-43 extends) or C-41 fails (the backward Phase->Phase
  direction must be declared before the forward direction can complete the bidirectional
  chain).
- **Pass condition**: Each PHASE EXIT GATE block (per C-37 pass condition) contains a
  named `Next phase:` sub-field (or equivalent: "Successor phase:", "Following phase:", or
  a canonically labeled sub-field with the same semantics) carrying a literal phase
  identifier. For all phases except the last: the identifier must match the label of the
  immediately following phase block in document order (e.g., if PHASE 2 is labeled
  "PHASE 2 -- Evaluation", the Next phase: field in PHASE 1's exit gate must cite
  "PHASE 2" or "PHASE 2 -- Evaluation" as a recognizable string match). For the last
  phase: the sub-field must carry an explicit sentinel value (NONE, END, TERMINAL, or
  equivalent) rather than a phase identifier or blank. The sub-field must be a named
  labeled element within the exit gate block -- not a prose annotation appended to the
  Exit Condition field -- and must be structurally distinct from the `Blocked bottleneck:`
  sub-field. CHAIN STATUS must enumerate a `Phase-exit-sequence` direction. Fail if any
  PHASE EXIT GATE lacks the Next phase: sub-field, if a non-final phase carries a sentinel
  instead of a phase identifier, if the identifier does not string-match the following
  phase label, or if the final phase carries a non-sentinel value. C-43 cannot pass if
  C-37 fails or C-41 fails.
...
- For C-42 verify TWO independent conditions: (a) inspect each EX block individually for
  a named bottleneck-citation sub-field ("Bottleneck Ref:", "B-ID Ref:", "Causal
  Bottleneck:", or equivalent). The sub-field must carry either a literal B-ID matching
  a declared bottleneck block identifier or the token NONE. A prose description of the
  bottleneck without a B-ID identifier fails; a cited B-ID absent from the Bottleneck
  Analysis section fails; a blank sub-field fails. (b) Locate a named gate validation in
  the Bottleneck Analysis section confirming that each declared B-ID appears in at least
  one EX block's bottleneck-citation sub-field. The gate must be a named structural
  element -- a labeled check, validation instruction, or gate block -- not an implicit
  assumption. CHAIN STATUS must name a `B-NN->Exception` direction. Check all EX blocks
  independently; omission in any EX block fails (a). If any B-ID lacks corresponding EX
  block coverage in the gate validation, (b) fails. C-42 cannot pass if C-04 fails or
  C-13 fails.
- For C-43 locate each PHASE EXIT GATE block (C-37 pass condition already required) and
  check for a `Next phase:` sub-field (or equivalent labeled sub-field) distinct from the
  `Blocked bottleneck:` sub-field. For the last phase: the sub-field must carry a sentinel
  value (NONE, END, TERMINAL, or equivalent) -- a blank or omitted sub-field fails; a
  phase identifier also fails. For all phases except the last: the sub-field must carry a
  literal string that string-matches the label of the immediately following phase block in
  document order. A `Next phase:` value expressed as a prose description instead of the
  label identifier fails. CHAIN STATUS must name a `Phase-exit-sequence` direction. Check
  every phase's exit gate independently; omission in any phase fails C-43 overall. C-43
  cannot pass if C-37 fails or C-41 fails.
