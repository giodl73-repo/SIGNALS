...
From the C-19/C-20 structural pairing visible across V-01 and V-02 in R5. The two criteria encode the same logic at different ends of the writing act: the checklist verifies before writing that a required element will be present; the labeled field confirms after writing where to find it. C-24 requires the pairing to be complete -- every required labeled output field has a corresponding checklist item, and every checklist item verifying a required output element has a corresponding labeled field. A template can pass both C-19 and C-20 without C-24 if they don't correspond. Orphaned checklist items and orphaned labeled fields both fail.

**Scoring updated**: 17 aspirational criteria (C-08--C-24) sharing 10 pts, ~0.588 pts each (10/17). Formula unchanged.
d field confirms after writing where to find it. C-24 requires the pairing to be complete -- every required labeled output field in the template has a corresponding pre-artifact checklist item, and every checklist item verifying a required output element has a corresponding labeled field in the template. A template can pass both C-19 and C-20 without C-24 if the checklist covers areas not represented as labeled fields, or if labeled fields exist without checklist items. C-24 fires on the correspondence between the two enforcement mechanisms, not on either mechanism alone.
...
*Raise the bar once essential and recommended are stable. 10 pts total across C-08--C-24, ~0.588 pts each (10/17), partial credit allowed.*
...
- Aspirational: 10 pts total across C-08--C-24, ~0.588 pts per criterion (10/17). Partial credit allowed.
