Reading the R4 scorecard, two new patterns emerge that aren't captured by existing criteria:

1. **One-axis-per-gate rule** — C-18 counts gates by axis coverage, not block count. A compound gate absorbing two axes under one correction instruction collapses them to one gate. This is a structural constraint C-18 doesn't state explicitly.

2. **Axis-named repair verb** — V-03 proves format (block vs. bullet) is irrelevant; what matters is that each gate carries a repair verb that names its specific axis ("add" / "rewrite" / "fix"), giving the model a structural cue to fire independently. No existing criterion captures this.

These become **C-19** and **C-20**. Aspirational denominator bumps 11 → 13.

---

# rhythm-behavior Rubric — v5
