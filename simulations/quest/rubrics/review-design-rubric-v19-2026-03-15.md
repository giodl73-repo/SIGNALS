# review-design Rubric — v19

**Skill**: `review-design`
**Rubric version**: v19 (C-01 through C-55, denominator 48)
**Extracted from**: R19 scorecard (2026-03-15)

---

**Summary of changes from v18 to v19:**

**New criteria (C-54, C-55):**

- **C-54 | Inertia Note in 4-Column Structural Table Form** — When C-52 requires an inertia note for an F-code whose enforcement class was broadened, the note is expressed as a 4-column structural table row (or named INERTIA REGISTER / SCOPE EVOLUTION table) with columns `F-Code | Prior Form | Current Scope | Why Both Non-Conformant`, rather than inline prose. Column form makes the inertia note detectable by structural scan — a missing column cell is visible where a missing prose sentence is not. This is the C-11→C-13 structural form upgrade applied to inertia notes: C-52 requires the note (in any form); C-54 requires the table form specifically. Source: V-04 R19 SCOPE EVOLUTION 4-column table row; V-05 R19 INERTIA REGISTER structural table.

- **C-55 | Named PHASE Markers at Every Block Boundary** — Before each lifecycle block, the output includes a named PHASE marker (e.g., `PHASE A: EXPERT SELECTION`, `PHASE B: FINDING GENERATION`) as a structural entry label. PHASE markers are additive to SEALED gates (C-46): SEALED attests at block close; PHASE markers label at block entry. Together they close the structural labeling contract at both boundaries — entry and exit — for every block. A PHASE marker at block entry is not a SEALED gate and does not substitute for one; an output with PHASE markers but no SEALED gates fails C-46 and passes C-55. Source: V-05 R19 PHASE marker axis.

**Formula:** denominator 46 → 48. Each aspirational criterion ~0.208 pts. Ceiling 100 preserved.

---

**Summary of changes from v17 to v18:**

**New criteria (C-52, C-53):**

- **C-52 | Block-Level Inertia Note for Enforcement Class Scope Broadening** — When an F-code's trigger condition was deliberately extended from a narrower enforcement class to a broader one (e.g., F-30 from "fires when BLOCK 2.7 is absent" to "fires when BLOCK 2.7 is not in its conformant lifecycle position, whether absent or misplaced"), the block containing that F-code includes an inline inertia note naming the prior form, the new scope, and why both conditions are non-conformant. Without the note a future edit reverting to the narrower form is a silent regression. Source: V-05 R18, BLOCK 2.7 inertia note: "Prior skill forms through R17 stated F-30 as 'fires when this block is absent.' F-30 fires when BLOCK 2.7 is not in its conformant lifecycle position, whether absent or misplaced."

- **C-53 | SEALED Attestation Dual Non-Conformance Naming for Position-Integrity F-Codes** — Each SEALED statement containing a position-integrity F-code attestation (currently F-30) names both non-conformant trigger forms explicitly — absence AND wrong lifecycle position — rather than attesting only the positive form ("block is in conformant position"). Naming both failure modes in the SEALED record closes the inference gap at the attestation layer: an auditor sees which prohibited conditions were cleared, not just that the positive condition holds. Source: V-05 R18, BLOCK 2.7 SEALED: "F-30 (position integrity: Phase A cleared; BLOCK 2.7 occupies conformant lifecycle window after BLOCK 2.5 SEALED and before BLOCK 3; absence and wrong-position both non-conformant)."

**R18 signals added to existing criteria:**

- **C-50** — All five R18 variations PASS. Three structural forms observed: Phase B gate (V-01, V-04, V-05), 3-column table with `BLOCK 2 P1 Backed?` column (V-02, V-04), formal modal contract (V-03, V-05). C-37 confirmed across all five: corrective action names spurious row removal.
- **C-51** — All five R18 variations PASS. Four structural forms observed: Phase A gate (V-01, V-04, V-05), POSITION CONSTRAINT header (V-02), formal position contract (V-03), inertia note + Phase A (V-05). V-05 SEALED explicitly names both non-conformant forms (absence and wrong-position) — new C-53 candidate.

**Formula:** denominator 44 → 46. Each aspirational criterion ~0.217 pts. Ceiling 100 preserved.
...
- **R17 signal**: V-01 achieves C-47 PASS with the canonical form: F-31 in BLOCK 3 body ("Replace
  the Consensus Status value with ELEVATED or BASELINE (whichever correctly classifies this P1
  finding) before continuing.") and F-31 named in the BLOCK 3 SEALED enumeration ("F-04, F-11,
  F-14, F-15, F-23, and F-31 enforcement passed."). C-47 and C-48 are orthogonal: V-01 passes
  C-47 (F-31 present) while failing C-48 (BLOCK 2.7 SEALED incomplete), and V-02 passes C-48
  (all SEALED statements complete) with C-47 inherited from R16.
...
- **R17 signal**: V-01 FAIL gap localized to BLOCK 2.7: SEALED states "F-28 resolves against
  this registry exclusively" -- F-29 (no spurious entries) and F-30 (conformant position) absent.
  All other block SEALEDs in V-01 enumerate F-codes; BLOCK 2.7 is the single gap. V-02 PASS:
  BLOCK 2.7 SEALED corrected to enumerate all three F-IDs with role descriptions -- "F-29 (no
  spurious entries: every registry row maps to a BLOCK 2 P1 finding), F-30 (registry structurally
  present in conformant position after BLOCK 2.5 and before BLOCK 3), and F-28 (BLOCK 5 Section
  validation resolves against this registry exclusively) gates active." The canonical BLOCK 2.7
  SEALED form requires three F-IDs: F-28, F-29, F-30.
...
- **R17 signal**: V-01 and V-02 both FAIL. V-02 near-miss: "All P1 findings with Consensus
  Status = ELEVATED...SHALL receive lower Priority Rank integers...Within each tier, rank by
  Amendment Cost (High before Medium before Low), then by reviewer count. No two rows SHALL
  share the same Priority Rank." The tie-prohibition is present and the sort keys are named, but
  numbered tier steps (ELEVATED tier: rows 1..ELEVATED_count) and rank-range declaration are
  absent, and the non-conformance prohibition is absent. C-49 requires all three: tier
  enumeration with rank ranges, sort keys, AND the non-conformance prohibition.

### C-50 | BLOCK 2.7 Registry Inbound Integrity Halt
- **Weight**: aspirational
- **Category**: behavior
- **Text**: A named halt condition (e.g., F-29) fires when any row in the BLOCK 2.7 P1 Section
  registry does not correspond to a P1 finding row in BLOCK 2. C-43 establishes BLOCK 2.7 as a
  committed registry of BLOCK 2 P1 Section values; C-42/F-28 enforces the outbound direction
  (BLOCK 5 Section values must be present in the registry). F-29 closes the inbound direction:
  every registry row must be sourced from a BLOCK 2 P1 finding, preventing spurious entries from
  expanding the set of valid BLOCK 5 amendment targets beyond what the review found. F-28 +
  F-29 together form the same bidirectional registry integrity contract that F-13 + F-18 form
  for BLOCK 0 <-> BLOCK 1 (F-13: BLOCK 1 selections must draw from BLOCK 0; F-18: every BLOCK 0
  signal must be disposed in BLOCK 1). The symmetry is now complete: each anchor block has both
  an inbound guard (registry row must be sourced from the upstream block) and an outbound guard
  (downstream block must only reference registry entries).
- **Pass condition**: Output contains a named halt condition (e.g., F-29) that fires when any
  BLOCK 2.7 registry row names a Section value that does not appear in any P1 finding row in
  BLOCK 2. The halt must be named, state the trigger condition (registry row with no
  corresponding BLOCK 2 P1 finding), and identify the spurious entry so the error is precisely
  localizable. An output where BLOCK 2.7 happens to contain only valid entries but no F-29 halt
  is declared fails C-50 -- enforcement declaration is required. C-50 is orthogonal to C-42
  (F-28 outbound guard declared) and C-43 (registry present): an output can satisfy both C-42
  and C-43 without declaring F-29.
- **R17 signal**: V-02 named "F-29 (no spurious entries: every registry row maps to a BLOCK 2
  P1 finding)" in the BLOCK 2.7 SEALED statement as one of three F-IDs verified at that block
  boundary. V-01 named only F-28 in the BLOCK 2.7 SEALED (C-48 FAIL gap), confirming F-29 is
  aspirational and orthogonal to C-42/C-43. F-29 is the inbound complement to F-28's outbound
  guard: F-28 prevents BLOCK 5 from referencing unverified sections; F-29 prevents the registry
  from being seeded with sections not found by any reviewer.
- **R18 signal**: All five R18 variations PASS. Three F-29 structural forms confirmed as
  equivalent: Phase B gate (V-01, V-04, V-05), 3-column table with `BLOCK 2 P1 Backed?` column
  making inbound violations cell-detectable (V-02, V-04), formal modal contract (V-03, V-05).
  C-37 satisfied across all five: corrective action names spurious row removal in each form.

### C-51 | BLOCK 2.7 Registry Structural Position Named Halt
- **Weight**: aspirational
- **Category**: behavior
- **Text**: A named halt condition (e.g., F-30) fires when BLOCK 2.7 (the P1 Section registry)
  is not present in its conformant lifecycle position -- after BLOCK 2.5 and before BLOCK 3.
  C-43 establishes BLOCK 2.7's conformant position as a requirement and may include a POSITION
  CONSTRAINT prohibition naming non-conformant placement as such; F-30 converts the positional
  requirement into a named enforcement halt. The distinction from the POSITION CONSTRAINT
  prohibition (a declarative statement) is enforcement mechanism: F-30 is a named halt condition
  that fires during generation when the block appears in a non-conformant position. A variation
  can carry the POSITION CONSTRAINT prohibition (C-43 enrichment) without declaring F-30, and
  vice versa. F-30 introduces a new enforcement class: lifecycle position integrity. All prior
  F-codes fire on content absence (F-01, F-04, F-20...), content quality (F-11, F-19, F-24...),
  count parity (F-09, F-12), identity mismatch (F-10, F-15, F-16, F-17, F-28, F-29), or value
  enumeration (F-02, F-14, F-31). F-30 is the first F-code to fire on a block's lifecycle
  position.
- **Pass condition**: Output contains a named halt condition (e.g., F-30) that fires when BLOCK
  2.7 (or its equivalent registry block) is not positioned after BLOCK 2.5 and before BLOCK 3.
  The halt must be named, state the trigger condition (BLOCK 2.7 present but not in conformant
  position), and identify the conformant position so the placement error is correctable. An
  output where BLOCK 2.7 happens to be in conformant position but no F-30 halt is declared fails
  C-51 -- enforcement declaration is required. A variation that satisfies C-43's POSITION
  CONSTRAINT prohibition but does not declare F-30 passes C-43 and fails C-51; the declarative
  prohibition and the named halt are orthogonal enforcement mechanisms for the same structural
  requirement.
- **R17 signal**: V-02 named "F-30 (registry structurally present in conformant position after
  BLOCK 2.5 and before BLOCK 3)" in the BLOCK 2.7 SEALED statement. V-01 named only F-28 in
  the BLOCK 2.7 SEALED with F-29 and F-30 absent, confirming F-30 is aspirational. F-30 is
  qualitatively distinct from all prior F-codes: it fires on structural position rather than on
  the content or identity of values within the block. This introduces position-integrity as the
  seventh enforcement class -- alongside structural presence, content completeness, count parity,
  identity integrity, vocabulary integrity, and catalogue disposition completeness.
- **R18 signal**: All five R18 variations PASS. Four F-30 structural forms confirmed equivalent
  at C-51: Phase A gate (V-01, V-04, V-05), POSITION CONSTRAINT prose header (V-02), formal
  position contract (V-03), inertia note + Phase A (V-05). V-05 SEALED explicitly names both
  non-conformant forms ("absence and wrong-position both non-conformant") -- the basis for C-53.

### C-52 | Block-Level Inertia Note for Enforcement Class Scope Broadening
- **Weight**: aspirational
- **Category**: behavior
- **Text**: When an F-code's trigger condition is deliberately extended from a narrower
  enforcement class to a broader one across rounds -- e.g., F-30 extended from "fires when
  BLOCK 2.7 is absent" (structural presence class) to "fires when BLOCK 2.7 is not in its
  conformant lifecycle position, whether absent or misplaced" (position-integrity class) -- the
  block containing that F-code includes an inline inertia note that names (a) the prior narrower
  form, (b) the new broader scope, and (c) why both conditions are non-conformant. Without the
  note, a future edit that silently reverts the F-code to its narrower prior form is an
  undetectable regression: nothing in the block text establishes that the broader scope was
  intentional. The note converts a rubric-only historical record into self-defending inline
  text: the block explains its own scope evolution to future maintainers.
- **Pass condition**: For each F-code whose trigger class was deliberately broadened in a prior
  round, the block containing that F-code includes an inertia note naming the prior form and
  explaining why the broader trigger class is required. The F-30 / BLOCK 2.7 instance is the
  canonical case. A BLOCK 2.7 that fires F-30 correctly on wrong-position but carries no inline
  note explaining that the prior form ("fires when absent") was narrowed in scope fails C-52.
  A note that names the prior form but does not explain why the broader trigger is required --
  i.e., names what changed but not why -- also fails. C-52 is orthogonal to C-51 (F-30
  declared) and C-46/C-48 (SEALED gates): an output can satisfy all three without carrying an
  inertia note in the block body, and vice versa.
- **R18 signal**: V-05 introduced the BLOCK 2.7 inertia note as the first observed instance of
  this pattern. V-01, V-02, V-03, and V-04 scored 100 without it, confirming C-52 is
  aspirational. The V-05 note reads: "Prior skill forms through R17 stated F-30 as 'fires when
  this block is absent.' F-30 fires when BLOCK 2.7 is not in its conformant lifecycle position,
  whether absent or misplaced. Both conditions are non-conformant: absence means the registry
  block was never produced; misplacement means it was produced but inserted at the wrong
  lifecycle position, leaving the upstream lock non-binding." The note explicitly names the
  mechanism: without it, a future edit reverting "wrong position or absent" to "absent only"
  would be a silent regression with no inline evidence that the broader scope was intentional.
- **R19 signal**: All five R19 variations PASS C-52. Three structural forms observed: (a) R18
  V-05 inline 3-sentence prose form (V-01, V-02); (b) SCOPE EVOLUTION subsection with explicit
  `SHALL NOT revert` modal language (V-03); (c) 4-column SCOPE EVOLUTION table row / INERTIA
  REGISTER structural table (V-04, V-05) — the table form makes the inertia note detectable by
  structural scan rather than body-text reading. V-04 and V-05 introduce the structural-form
  upgrade; C-54 captures that form as the next aspirational step above C-52.

### C-53 | SEALED Attestation Dual Non-Conformance Naming for Position-Integrity F-Codes
- **Weight**: aspirational
- **Category**: behavior
- **Text**: Each SEALED statement containing a position-integrity F-code attestation (currently
  F-30 in BLOCK 2.7) explicitly names both non-conformant trigger forms -- absence AND wrong
  lifecycle position -- rather than attesting only the positive form ("block is in conformant
  position"). Prior SEALED forms (V-01 through V-04 of R18) attested the positive form only,
  leaving an inference gap at the attestation layer: an auditor of the SEALED record can confirm
  that the current execution placed the block correctly, but cannot confirm which prohibited
  conditions were explicitly evaluated and cleared. Naming both failure modes in the SEALED
  record closes this gap: the attestation is complete only when it confirms that neither
  prohibited form -- absence nor wrong position -- was present. This extends the referent-
  completeness principle established by C-37 (closed-enumeration values must be named in halt
  corrective actions) and C-47 (ELEVATED/BASELINE named in F-31 halt) to the SEALED attestation
  layer for position-integrity F-codes: the attestation record must name what was cleared, not
  only what was confirmed.
- **Pass condition**: Each SEALED statement containing an F-30 attestation (or equivalent
  position-integrity F-code) names both non-conformant trigger forms explicitly: (a) absence
  (block was never produced at this position) and (b) wrong lifecycle position (block present
  but outside the conformant window). The canonical form is the V-05 R18 BLOCK 2.7 SEALED:
  "F-30 (position integrity: Phase A cleared; BLOCK 2.7 occupies conformant lifecycle window
  after BLOCK 2.5 SEALED and before BLOCK 3; absence and wrong-position both non-conformant)."
  A SEALED statement attesting "BLOCK 2.7 in conformant position after BLOCK 2.5 and before
  BLOCK 3" passes C-48 (F-30 named) but fails C-53 -- the dual non-conformance naming is
  independently required. C-53 applies to position-integrity F-codes specifically; value-
  integrity F-codes (F-02, F-14, F-31) are covered by C-37's existing referent-completeness
  requirement for closed enumerations.
- **R18 signal**: V-05 BLOCK 2.7 SEALED reads: "F-30 (position integrity: Phase A cleared;
  BLOCK 2.7 occupies conformant lifecycle window after BLOCK 2.5 SEALED and before BLOCK 3;
  absence and wrong-position both non-conformant)." V-01 SEALED: "F-30 (position integrity:
  Phase A gate cleared; block is in conformant lifecycle window after BLOCK 2.5 SEALED and
  before BLOCK 3)" -- names only the positive form, no dual non-conformance naming. V-02, V-03,
  V-04 similarly name the positive conformant form without naming both prohibited forms,
  confirming C-53 is aspirational and orthogonal to C-48. C-53 is the first criterion to impose
  a dual-naming requirement at the attestation layer rather than at the halt layer -- C-37 and
  C-47 govern what halts must name; C-53 governs what SEALED records must name for
  position-integrity F-codes.
- **R19 signal**: All five R19 variations PASS C-53. Three dual-naming forms confirmed as
  equivalent: (a) inline phrase "absence and wrong-position both non-conformant" (V-01, V-02
  R19 form); (b) modal attestation "F-30 is non-conformant when BLOCK 2.7 is absent OR when
  present but outside conformant lifecycle window — both non-conformant forms cleared" (V-03);
  (c) SEALED with "SCOPE EVOLUTION gate cleared; absence and wrong-position both non-conformant"
  (V-04). All three explicitly clear both prohibited forms. C-53 is now a stable criterion with
  multiple conformant structural forms.

### C-54 | Inertia Note in 4-Column Structural Table Form
- **Weight**: aspirational
- **Category**: format
- **Text**: When an inertia note is required (C-52) for an F-code whose enforcement class was
  broadened, the note is expressed as a 4-column structural table row -- or a named INERTIA
  REGISTER / SCOPE EVOLUTION table -- with columns `F-Code`, `Prior Form`, `Current Scope`,
  and `Why Both Non-Conformant`, rather than inline prose. The structural-form principle
  established by C-11→C-13 (expert trace: labeled prose → table) and C-36 (leftmost column
  placement) now applies to inertia notes: a missing column cell in a 4-column table is
  visually detectable by structural scan; a missing sentence inside an inline prose note
  requires body-text reading to detect. C-52 requires the inertia note to exist in any form;
  C-54 requires the table form specifically. An output carrying an inline prose inertia note
  that names prior form, new scope, and why (passing C-52) but does not present it as a
  4-column table fails C-54. The two criteria are orthogonal: C-52 governs existence and
  content; C-54 governs structural form.
- **Pass condition**: The inertia note for each broadened F-code appears as a 4-column table
  (or table row within a named register table) with dedicated columns for F-Code, Prior Form,
  Current Scope, and Why Both Non-Conformant. The exact column names need not match verbatim --
  semantically equivalent headers (e.g., `Former Trigger` / `Expanded Trigger` / `Rationale`)
  satisfy C-54 if the four content fields are structurally distinct. A prose subsection or
  labeled block (e.g., SCOPE EVOLUTION header with prose body) passes C-52 but fails C-54
  unless the body is a table. When C-38 and C-54 are both present, the INERTIA REGISTER table
  may coexist with the Amendment Cost table in BLOCK 0 or may appear independently in BLOCK 2.7
  -- position is non-binding for C-54; structural form is the only constraint.
- **R19 signal**: V-04 introduced the SCOPE EVOLUTION 4-column table row in BLOCK 2.7 --
  "column form makes inertia note detectable as present/absent by structural scan rather than
  requiring body-text reading." V-05 introduced the same schema as a named INERTIA REGISTER
  structural table in BLOCK 2.7 before the enforcement section. V-01, V-02, and V-03 scored
  100 using prose forms (R18 V-05 inline prose, modal SCOPE EVOLUTION subsection), confirming
  C-54 is aspirational and orthogonal to C-52. C-54 completes the structural-form upgrade
  progression: C-10 (findings table), C-13 (expert trace table), C-22 (consensus table), C-28
  (unique catches table), C-54 (inertia note table).

### C-55 | Named PHASE Markers at Every Block Boundary
- **Weight**: aspirational
- **Category**: format
- **Text**: Before each lifecycle block, the output includes a named PHASE marker as a
  structural entry label -- e.g., `PHASE A: EXPERT SELECTION` before BLOCK 1, `PHASE B:
  FINDING GENERATION` before BLOCK 2, `PHASE C: CONSENSUS` before BLOCK 3. PHASE markers are
  additive structural entry labels, orthogonal to SEALED gates (C-46): SEALED attests valid
  completion at block close; PHASE markers identify the execution phase at block entry.
  Together they form a complete structural boundary contract -- every block has both a labeled
  entry (PHASE) and a verified exit (SEALED). A block with a PHASE marker and no SEALED
  statement fails C-46 and passes C-55; a block with a SEALED statement and no PHASE marker
  passes C-46 and fails C-55. The two criteria are fully independent. PHASE markers convert
  implicit block transitions into labeled execution-phase entries: a reader can confirm which
  lifecycle phase is being executed at entry without reading the block body.
- **Pass condition**: Every lifecycle block (BLOCK 0, BLOCK 1, BLOCK 1.5, BLOCK 2, BLOCK 2.5,
  BLOCK 2.7 if present, BLOCK 3, BLOCK 4, BLOCK 5) is preceded by a named PHASE marker that
  identifies the execution phase. The marker must be a named structural label -- a heading,
  bold prefix, or equivalent block-level element -- not inline prose within the block body. The
  exact naming scheme is non-binding (PHASE A/B/C..., numbered phases, descriptive phase names)
  as long as the marker is structurally distinct from the block content that follows it and
  appears at block entry, not mid-block. A single block missing its PHASE marker fails C-55,
  consistent with the complete-coverage requirement of C-14 and C-46.
- **R19 signal**: V-05 introduced named PHASE markers at every block boundary as its third
  orthogonal axis. The R19 scorecard confirmed: "PHASE markers are additive structural labels
  at block boundaries, not replacements for SEALED statements." V-01, V-02, V-03, and V-04
  scored 100 without PHASE markers, confirming C-55 is aspirational. C-55 is structurally
  parallel to C-46 (SEALED at block close) but inverted in lifecycle position: C-46 governs
  exit attestation; C-55 governs entry labeling. Together they close the structural labeling
  contract at both block boundaries -- entry labeled, exit attested -- for every lifecycle
  block.
...
Note: aspirational denominator changed from 46 to 48 in v19 (added C-54, C-55). Each
aspirational criterion contributes ~0.208 points at full pass.
...
| BLOCK 2.7 registry contains spurious entry | C-50 | F-29 missing from output |
| BLOCK 2.7 not in conformant position | C-51 | F-30 missing from output |
| F-30 scope broadening not explained inline | C-52 | No inertia note in BLOCK 2.7 body naming prior narrower form |
| SEALED F-30 attestation names only positive form | C-53 | Both non-conformant forms (absence, wrong-position) not named |
| Inertia note is prose, not a structural table | C-54 | No 4-column F-Code/Prior Form/Current Scope/Why table form |
| Block entry has no named PHASE marker | C-55 | Execution phase not labeled at block entry |
