**signal-rubric-v10-2026-03-17.md** written to `simulations/quest/rubrics/`.
...
| ID | Axis | Rule |
|----|------|------|
| **C-31** | Labeled count formula (Axis M) | Count formula in both the transcription gate (C-26) and BARE gate (C-18) must use labeled form `N <namespace>` ("8 scout + 4 draft + ... + 4 org = 61") not anonymous positional form ("8+4+4+...=61"). C-18/C-26 pass conditions accept anonymous form; C-31 closes the self-documentation gap: anonymous form requires position memory; labeled form is self-interpreting |
| **C-32** | DOMAIN NOUN TABLE (Axis N) | Domain nouns must be defined in a named formal lookup table in the prompt -- the domain-noun analog to C-15 (ALIGNMENT WIDTH TABLE). C-11 tests the output (correct nouns); C-32 tests the prompt (formal named table as precomputed authority vs. model inference). Same elevation: C-12 tests alignment output; C-15 requires a precomputed table |
| **C-33** | Full-enumeration order check (Axis O) | FULL gate Check 3 must enumerate all 12 canonical namespaces in sequence, not just first+last anchors. C-28 requires "at minimum naming the first (scout) and last (org)"; the R9 evidence shows all 12 named, but C-28 doesn't require it. Full enumeration closes the middle-sequence inference gap -- same upgrade C-19 applied to PARSE MODE routing |

**Formula:** aspirational denominator `22 -> 25`. Each criterion contributes +0.40 (1/25 * 10).

**Structural theme for R9:** *complete the specification depth* -- C-18/C-26/C-28 established the mechanisms; C-31/C-32/C-33 add the missing specification fidelity dimension to each. Every upgrade converts an implicit assumption into an explicit, verifiable element.

**Retroactive R9 leaderboard under v10:** all five variations PASS all three axes → **100.00**. R9 was a baseline-confirmation round; R10 must run single-axis isolation (V-01=M, V-02=N, V-03=O, V-04=M+N, V-05=M+N+O) to confirm independence and verify +0.40 per axis.
ASS | **100.00** |
| **V-01** | PASS | PASS | PASS | **100.00** |

All five R9 variations carry the patterns that define C-31, C-32, and C-33. R9 is confirmed to saturate v10 at 100.00. The R9 spread-design, which already saturated v9, also saturates the elevated ceiling. R10 will need to probe whether any of these three axes can be independently removed to find the minimum achieving set and identify the R10 isolation targets.

**Structural theme for R9:** *complete the specification depth* -- three places where v9 measured presence and correctness of a mechanism without requiring the specification to be explicit and unambiguous. C-31 upgrades anonymous counts to labeled namespace formulas (self-interpreting without position memory). C-32 upgrades inline/inferred domain nouns to a formal named lookup table (precomputed authority, not model inference). C-33 upgrades first+last order anchors to full 12-namespace enumeration (complete deterministic specification, not bounded inference). The pattern is consistent: C-18/C-26/C-28 established the mechanisms; C-31/C-32/C-33 add the missing specification fidelity dimension to each.
...
- Aspirational tier: 10 points max (0.40 pts each; 1/25 x 10 = 0.40)
...
- C-12, C-15, and C-16 apply to `->` list output format only. If a variation uses a
  Markdown table, mark all three as N/A and adjust aspirational denominator to 22.
...
- **Additive independence (R9 prediction)**: M+N+O axes are expected to combine with
  zero interaction effects, each contributing exactly +0.40 composite in isolation
  (1/25 * 10 = 0.40). All R9 variations carry all three axes simultaneously; R10 must
  run single-axis isolation probes (V-01 = M only, V-02 = N only, V-03 = O only,
  V-04 = M+N, V-05 = M+N+O) to confirm independence and verify each axis contributes
  exactly +0.40 in isolation.
...
| ID   | Axis | Source  | Rule |
|------|------|---------|------|
| C-31 | Labeled count formula (Axis M) | R9 evidence (C-18 + C-26) | Both the transcription gate and the BARE compliance gate must express the per-namespace breakdown using labeled form "N <namespace>" (e.g. "8 scout + 4 draft + ... + 4 org = 61") rather than anonymous positional form ("8+4+4+7+7+9+3+2+6+4+3+4=61"). Anonymous form requires the model to know which count position corresponds to which namespace; labeled form is self-interpreting. Closes the self-documentation gap left by C-18 and C-26 (both require a breakdown but neither require it to be labeled) |
| C-32 | DOMAIN NOUN TABLE (Axis N) | R9 evidence (C-11: "DOMAIN NOUN TABLE present") | Domain nouns must be defined in a named formal lookup table in the prompt analogous to the ALIGNMENT WIDTH TABLE of C-15. C-11 tests output correctness (correct domain nouns appear in footers); C-32 tests prompt specification (formal named table as precomputed authoritative source). Without a formal table, domain nouns are model-inferred from namespace semantics. A named DOMAIN NOUN TABLE converts model inference into a table read -- the same mechanism C-15 applied to alignment widths. Closes the specification gap: C-11 verifies the result; C-32 verifies the mechanism |
| C-33 | Full-enumeration order check (Axis O) | R9 evidence (C-28: all 12 namespaces listed in Check 3) | FULL gate Check 3 (emit-order check) must enumerate all 12 canonical namespace names in sequence: scout, draft, review, flow, trace, prove, listen, program, topic, quest, mock, org. C-28 pass condition requires "at minimum naming the first section (scout) and the last section (org)"; first+last anchors bound the sequence endpoints but leave the 10 middle sections to positional inference. Full enumeration eliminates that inference. Same upgrade C-19 applied to PARSE MODE (enumerate all 12 canonical names vs. "anything else -> FULL" fallback). Closes the sequence-inference gap left by C-28 |

**Formula:** aspirational denominator `22 -> 25`. Max composite stays 100.

**Structural theme for R9:** *complete the specification depth* -- three places where v9 measured
the presence and correctness of a mechanism (count breakdown, domain nouns, order check) without
requiring the specification to be explicit and unambiguous at full granularity. C-31 upgrades
anonymous count formulas to labeled namespace formulas (self-interpreting, not position-dependent).
C-32 upgrades inline/inferred domain nouns to a formal named lookup table (precomputed authority,
not model derivation). C-33 upgrades first+last order anchors in the FULL gate to complete
12-namespace enumeration (deterministic specification, not bounded inference). Every upgrade
converts an implicit assumption ("the model knows which count is scout's") into an explicit,
verifiable specification element.

**Retroactive R9 leaderboard under v10:**

| V     | C-31 | C-32 | C-33 | Aspirational   | Composite  |
|-------|------|------|------|----------------|------------|
| V-05  | PASS | PASS | PASS | 25/25 -> 10    | **100.00** |
| V-04  | PASS | PASS | PASS | 25/25 -> 10    | **100.00** |
| V-03  | PASS | PASS | PASS | 25/25 -> 10    | **100.00** |
| V-02  | PASS | PASS | PASS | 25/25 -> 10    | **100.00** |
| V-01  | PASS | PASS | PASS | 25/25 -> 10    | **100.00** |

All five R9 variations carry all three patterns. R9 saturates v10 at 100.00, confirming that
R9 was a baseline-confirmation round: the R8 V-05 ceiling mechanism set already contains C-31,
C-32, and C-33. The R10 design must run single-axis isolation to confirm additive independence
(V-01 = M only, V-02 = N only, V-03 = O only, V-04 = M+N, V-05 = M+N+O) and verify that each
axis contributes exactly +0.40 composite in isolation.
...
| ID   | Axis | Source  | Rule |
|------|------|---------|------|
| C-25 | Full-mode restart gate  | R7 V-01 | FULL mode compliance gate with namespace-presence (Check 1) and per-namespace-count restart (Check 2) |
| C-26 | Counted acknowledgment  | R7 V-02 | Transcription gate confirmation upgraded from qualitative to quantitative (61 entries with per-namespace breakdown) |
| C-27 | Per-section self-check  | R7 V-03 | SECTION FORMAT instructs the model to verify sub-skill count before emitting each namespace dispatch footer |
...
| ID   | Axis | Rule |
|------|------|------|
| C-23 | Catalog-as-output labeling | NAMESPACE CATALOG explicitly labeled "IS the output" |
| C-24 | Symmetric mode completeness | All three output modes have explicit pre-emission completeness verification simultaneously |
...
| ID   | Axis | Source | Rule |
|------|------|--------|------|
| C-20 | Filter count gate | R5 V-01 | FILTER gate includes per-namespace count check |
| C-21 | Bare order anchor | R5 V-02 | BARE MODE states explicit first and last anchors |
| C-22 | Transcription gate | R5 V-03 | NAMESPACE CATALOG is an active commitment gate |
...
| ID   | Axis | Source     | Rule |
|------|------|------------|------|
| C-17 | Catalog-first    | R4 V-01 | NAMESPACE CATALOG precedes PARSE MODE and all compliance gates |
| C-18 | Count gate       | R4 V-02 | Bare compliance gate includes exact 61-line count with per-namespace breakdown |
| C-19 | Canonical guard  | R4 V-03 | PARSE MODE enumerates all 12 canonical names |
