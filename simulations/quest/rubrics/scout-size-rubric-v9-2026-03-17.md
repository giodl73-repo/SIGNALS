Written to `simulations/quest/rubrics/scout-size-rubric-v9-2026-03-17.md`.

---

**Two new aspirational criteria added (C-28 through C-29):**

| ID | Pattern source | What it checks |
|----|----------------|----------------|
| **C-28** | V-04 R8 "27-criterion self-check" | Output includes a distinct per-criterion self-check trace covering all aspirational criteria by ID with pass/fail and evidence — a meta-verification layer above section-level FAILURE MODE blocks. Passing C-27 (dedicated FAILURE MODE blocks in every aspirational section) does not satisfy C-28 if no collective per-criterion audit artifact exists. |
| **C-29** | V-05 R8 "structural disqualifying forms extended" | Self-check items for all structural criteria (C-18 through C-27) include exact structural disqualifying forms — the precise pattern that looks like compliance but fails — not just pass conditions. Passing C-28 with exact disqualifying forms only for C-26/C-27 does not satisfy C-29 if structural criteria C-18 through C-25 carry only pass-condition descriptions. |

**What makes these genuinely new (not redundant with v8):**

- C-28 is not a refinement of C-27. C-27 requires every aspirational section to carry a dedicated FAILURE MODE block — section-internal structural signals. C-28 requires a separate collective artifact that evaluates each criterion by ID. V-03 passes all 19 v8 criteria including C-27 with no self-check trace; the self-check is a distinct output-level verification protocol, not a stronger form of section-level failure-mode coverage.

- C-29 is not a refinement of C-28. C-28 requires the self-check to exist. C-29 requires the structural criteria items within that self-check to carry exact disqualifying forms for every structural criterion uniformly. V-04 satisfies C-28 and includes exact disqualifying conditions for C-26 and C-27 — the two new v8 criteria — but not for C-18 through C-25. Coverage uniformity is the discriminator.

**Scoring model:** aspirational denominator bumped 19 → 21. Max composite unchanged at 100.

**Expected R8 scores under v9:**

| Variation | Aspirational | Score |
|-----------|-------------|-------|
| V-01 (structural AMEND directive only) | 16/21 (fails C-27, C-28, C-29) | 97.62 |
| V-02 (dedicated FAILURE MODE blocks only) | 16/21 (fails C-26, C-28, C-29) | 97.62 |
| V-03 (C-26 + C-27, no self-check) | 19/21 (fails C-28, C-29) | 99.05 |
| V-04 (self-check + definition-level C-26/C-27 items) | 20/21 (fails C-29) | 99.52 |
| V-05 (full integration + uniform structural precision) | 21/21 | **100.00** |

V-05 is the first R8 variation to achieve 21/21. The two new criteria split across V-04 (C-28 only) and V-05 (C-28 + C-29), establishing that the R9 gold standard requires extending structural disqualifying-form coverage beyond the latest-round criteria to all structural criteria uniformly.
tence explains
...
- **Description**: V-04 R8 adds a distinct self-check block after the complete output
  ...
  compliance-evaluation itself a structural artifact: a reader can verify the model's
  own assessment of each criterion without re-reading the entire output. Sourced from
  V-04 R8: the "27-criterion self-check" — the first variation to add a per-criterion
  evaluation trace as a distinct structural layer above content.
...
- **Description**: Refinement of C-28. V-04 includes definition-level discriminators in
  its self-check for C-26 and C-27 — the two new v8 criteria — specifying an exact
  structural form that would disqualify: "a directive ending 'it fails C-16' fails C-26
  even if present in synthesis" and "an inline anti-pattern sentence embedded in a prose
  blockquote fails C-27." V-05 extends this treatment to all structural criteria tracked
  in the self-check (C-18 through C-27): every structural criterion carries an exact
  disqualifying form alongside its pass condition, not just the two most recently added.
  The distinction between V-04 and V-05 is coverage: V-04 achieves structural precision
  for the latest-round criteria only; V-05 achieves it uniformly across the full set of
  structural criteria, ensuring that older structural criteria do not regress to pass-only
  descriptions as the denominator grows. An output can satisfy C-28 with a complete
  self-check trace, and include exact disqualifying conditions for C-26 and C-27, yet
  fail C-29 if structural criteria C-18 through C-25 carry only pass-condition
  descriptions without matching disqualifying-form specifications. Sourced from V-05 R8:
  "structural disqualifying forms for C-26/C-27 extended to all structural criteria" —
  the only R8 variation where the self-check provides uniform structural precision across
  every structural criterion regardless of which round introduced it.
- **Pass condition**: Self-check items for all structural criteria (C-18 through C-27
  minimum) include both a pass condition and an exact structural disqualifying form —
  a specific structural pattern that looks like compliance but fails. An output that
  satisfies C-28 with exact disqualifying forms only for C-26 and C-27 fails C-29
  unless every other structural criterion carries equivalent precision.
...
| V-02 (dedicated FAILURE MODE blocks only) | 16/21 (fails C-26, C-28, C-29) | 97.62 |
| V-03 (C-26 + C-27, no self-check) | 19/21 (fails C-28, C-29) | 99.05 |
| V-04 (C-26 + C-27 + self-check with definition-level C-26/C-27 items) | 20/21 (fails C-29) | 99.52 |
| V-05 (full integration + exact disqualifying forms for all structural criteria) | 21/21 | 100.00 |

Note: V-05 is the first R8 variation to achieve 21/21. The two new criteria split
across V-04 (C-28 only) and V-05 (C-28 + C-29), establishing that the R9 gold
standard must extend self-check structural precision beyond the latest-round criteria
to the full set of structural criteria uniformly.
...
- C-28 is a meta-layer above C-27, sourced from R8. C-27 ensures every aspirational
  section has a failure-mode block. C-28 requires a separate collective self-check
  artifact that evaluates every criterion by ID. An output can pass C-27 with dedicated
  FAILURE MODE blocks in every aspirational section and still fail C-28 if no explicit
  per-criterion audit trail exists. The self-check is not a section; it is an output-
  level verification protocol. Sourced from V-04 R8 "27-criterion self-check" — the
  first variation to treat criterion compliance as itself a structured, auditable output.
- C-29 is a refinement of C-28, sourced from R8. C-28 requires a self-check trace to
  exist. C-29 requires the structural criteria items (C-18 through C-27) within that
  trace to include exact disqualifying forms — not just pass conditions. V-04 satisfies
  C-28 and includes exact disqualifying forms for C-26 and C-27 but not for C-18 through
  C-25; V-05 extends uniform structural precision to all structural criteria. An output
  can pass C-28 with a complete self-check trace and fail C-29 if earlier structural
  criteria carry only pass-condition descriptions without matching disqualifying-form
  specifications. Coverage uniformity is the discriminator: V-04 achieves precision for
  the latest criteria; V-05 achieves it for all.
...
| v9 | 2026-03-17 | Added C-28 (per-criterion self-check trace covering all aspirational criteria), C-29 (self-check items for all structural criteria include exact structural disqualifying forms); aspirational denominator 19 -> 21; sourced from R8 — V-04 "27-criterion self-check" introduces a meta-verification layer above section-level FAILURE MODE blocks (V-03 passes C-27 with no self-check machinery; C-28 targets V-01/V-02/V-03 which carry no per-criterion audit trail), and V-05 extends exact disqualifying-form coverage from C-26/C-27 only (V-04) to all structural criteria C-18 through C-27 (C-29 targets V-01 through V-04 which lack uniform structural precision across all structural criteria) |
