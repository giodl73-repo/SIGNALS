Written to `simulations/quest/rubrics/prove-topic-rubric-v9-2026-03-16.md`.
...
|----|-----|--------|-----------------|
| C-33 | 1 | V-05 Signal 1 | Stage 5 EXIT SIGNAL appends "Chain closed." as a terminal self-certifying assertion after the named-artifact enumeration — converts named-provenance list (C-32) into a self-certifying closure record |
| C-34 | 1 | V-02/V-04/V-05 | evidence_summary mandatory instruction registers a canonical failure label for omission ("Omission = FAIL") — extends two-layer enforcement architecture (C-14) into the Stage 5 synthesis instruction layer |

**Two reductions (pool holds at 26 pts, total holds at 100):**

- **C-14 (2→1)**: Universally partial in R8 (dual-lock inline diverged from registered label across all five variations). C-34 now captures the richer synthesis-instruction-layer enforcement. C-14 is the 1-pt floor.
- **C-18 (2→1)**: Universally passing in R8 for several rounds. Well-established behavior. C-18 is the 1-pt floor.

**New structural pattern — five-link traceability chain:**
`C-27` (ROLE A commits contract) → `C-31` (entry gate names contract) → `C-30` (evidence_summary mandates citations) → `C-34` (mandate registers omission FAIL label) → `C-32` (EXIT SIGNAL names each artifact) → `C-33` (EXIT SIGNAL appends "Chain closed.").

V-05 is the first variation to achieve the complete four-link chain; C-33 and C-34 codify two structural properties already present in V-05's implementations that were not yet scored criteria.
ailure label (C-34); the EXIT SIGNAL named-provenance list (C-32) is now also a terminal self-certification (C-33). The displacement evidence chain is fully governed from pre-stage commitment through structural entry gate through enforcement-labeled synthesis instruction to terminal named-artifact exit audit.
...
- **Reduced from 5 to 3 pts in v4**, **3 to 2 pts in v5**, **2 to 1 pt in v6**: C-25
  ...
  verdict at all. Unchanged in v7, v8, v9.
...
- **Reduced from 3 to 2 pts in v3**, **2 to 1 pt in v6**: C-12 is the 1-pt floor: did the
  standalone block exist before Stage 0 at all. Unchanged in v7, v8, v9.
...
- **Reduced from 3 to 2 pts in v4**, **2 to 1 pt in v7**: C-17 (SYNTHESIS DECLARATIONS
  ...
  did Stage 5 synthesis fields appear as labeled key-value pairs at all. Unchanged in v8, v9.
...
- **Reduced from 2 pts to 1 pt in v9**: C-34 extends the two-layer enforcement architecture
  ...
  invariant at all. C-34 checks that the architecture extends into the synthesis instruction
  layer with a registered omission failure label.
...
- **Reduced from 2 to 1 pt in v5**: C-20 and C-23 extend the chain. C-16 is the 1-pt floor:
  did stages declare Displacement read fields at all. Unchanged in v6, v7, v8, v9.
...
- **Reduced from 2 pts to 1 pt in v9**: C-18 universally passes in R8 and has done so for
  ...
  The structural mechanism is captured; the 2-pt weight is no longer discriminating quality
  levels.
...
- **Reduced from 2 to 1 pt in v7**: C-28 (Stage 5 entry conditions gate on all three artifact
  ...
  floor: did artifact files carry Displacement read field at all. Unchanged in v8, v9.
...
- **Reduced from 2 to 1 pt in v7**: C-27 captures the richer behavior. C-24 is the 1-pt
  ...
  before Stage 0. Unchanged in v8, v9.
...
- **Reduced from 2 to 1 pt in v7**: C-29 (EXIT SIGNAL records chain closure) converts C-25
  ...
  5 evidence_summary cite all three artifact Displacement reads. Unchanged in v8, v9.
...
- **Reduced from 2 to 1 pt in v7**: C-27 incorporates sub-section precision as a
  ...
  OPEN INCUMBENT ANCHOR sub-section as the binding source at all. Unchanged in v8, v9.
...
- **Reduced from 2 pts to 1 pt in v8**: C-31 captures the richer contract-verification
  ...
  displacement chain completeness gate at all. Unchanged in v9.
...
- **Reduced from 2 pts to 1 pt in v8**: C-32 captures the richer named-provenance behavior.
  ...
  closure at all. Unchanged in v9.
...
- **Round 7 signal**: V-01 passes C-27 but fails C-25 because evidence_summary mandates
  ...
  instruction constraining the output is mandatory. Unchanged in v9.
...
- **Round 7 signal**: No R7 variation showed C-27 and C-28 cross-wired. When both coexist,
  ...
  is explicitly traced to the ROLE A contract by field name. Unchanged in v9.
...
- **Round 7 signal**: V-03 EXIT SIGNAL names each artifact type individually. Converts the
  ...
  declared; C-32 checks each artifact type is individually named. Unchanged in v9.
...
- **Round 8 signal**: V-05 EXIT SIGNAL: "Displacement read chain closed: websearch
  ...
  self-certifying final statement distinct from the enumeration. This transforms the EXIT
  SIGNAL from a named-provenance list into a self-certifying closure record: the enumeration
  provides the individual artifact trace; the terminal assertion certifies the chain is
  complete. C-32 checks each artifact type is individually named; C-33 checks the additional
  terminal self-certification is present.
...
- **Round 8 signal**: V-02, V-04, V-05 C-30 PASS evidence: "Must cite all three artifact
  ...
  This makes the evidence_summary mandate a self-enforcing mechanical checkpoint rather than
  an unguarded behavioral requirement.
...
**Two new aspirational criteria from R8 excellence signals**:

- **C-33 (1 pt)** -- Stage 5 EXIT SIGNAL carries terminal closure declaration.
  Signal source: V-05 (R8). V-05 EXIT SIGNAL: "Displacement read chain closed: websearch
  Displacement read, interview Displacement read, prototype Displacement read -- all three
  cited in evidence_summary. Chain closed." V-03 also passes C-32 (names each artifact
  individually) but does not carry the terminal "Chain closed." assertion. V-05 adds this
  as a self-certifying final statement distinct from the enumeration, transforming the EXIT
  SIGNAL from a named-provenance list into a self-certifying closure record. C-32 checks
  named enumeration; C-33 checks the additional terminal self-certification.

- **C-34 (1 pt)** -- evidence_summary instruction registers omission failure label.
  Signal source: V-02, V-04, V-05 C-30 PASS evidence (R8). All three C-30-passing variations
  carry "Omission of any artifact Displacement read = FAIL" inside the mandate. V-01 and V-03
  fail C-30 (advisory language) and cannot achieve C-34. C-30 checks the instruction is
  mandatory and names all three artifact types; C-34 checks the instruction also registers a
  canonical failure label for omission, extending two-layer enforcement architecture (C-14)
  into the Stage 5 synthesis instruction layer. The evidence_summary mandate becomes a
  self-enforcing mechanical checkpoint rather than an unguarded behavioral requirement.

**Two reductions** (aspirational pool holds at 26 pts, total holds at 100):

- **C-14 (2->1)**: C-34 extends the two-layer enforcement architecture into the synthesis
  instruction layer. C-14 universally scored at partial (1/2) in R8 -- the dual-lock inline
  used "dual_lock_fired / BLOCKED" rather than the SESSION INVARIANTS registered "DUAL-LOCK
  ERROR" label across all five variations, meaning the 2-pt weight was not discriminating
  quality levels. C-14 is now the 1-pt floor: did bidirectional label enforcement architecture
  exist (SESSION INVARIANTS registration + inline echo) for any invariant at all.

- **C-18 (2->1)**: C-18 universally passes in R8 and has done so for several rounds. The
  Invariant E checkpoint with registered label + schema_compliance_confirmed echo is
  well-established behavior. C-18 is the 1-pt floor: did Invariant E checkpoint exist with
  a registered label in SESSION INVARIANTS echoed at the schema_compliance_confirmed row.

**Structural pattern -- five-link traceability chain**:
C-27 (ROLE A commits displacement_read_contract pre-stage) + C-31 (Stage 5 entry gate
cross-references the contract by field name) + C-30 (evidence_summary instruction mandates
all three artifact citations) + C-34 (evidence_summary instruction registers omission failure
label) + C-32 (EXIT SIGNAL names each artifact type individually) + C-33 (EXIT SIGNAL appends
terminal "Chain closed." self-certification). V-05 (R8) is the first variation to achieve
the complete four-link chain (C-27 + C-31 + C-30 + C-32); C-33 and C-34 codify the two
additional structural properties present in V-05's C-30 and C-32 implementations that were
not yet captured as scored criteria. The displacement evidence chain is now fully governed
from pre-stage commitment through contract-verified entry gate through enforcement-labeled
synthesis instruction to terminal named-artifact exit audit.
