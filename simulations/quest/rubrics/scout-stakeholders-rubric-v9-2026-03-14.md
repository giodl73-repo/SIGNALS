Written to `simulations/quest/rubrics/scout-stakeholders-rubric-v9-2026-03-14.md`.

---

**One new criterion extracted from Round 8:**

**C-26 — Inter-phase transition confirmation gates** (depth, 5 pts)

Source: V-01 ("Phase-Transition Confirmation Gates"). The pattern: a dedicated gate step appears between each analysis phase (Strategy->UX, UX->PM) firing FAIL_INCOMPLETE_PHASE1 / FAIL_INCOMPLETE_PHASE2 if the prior phase is incomplete before the next begins.

**Why it's distinct from existing criteria:**
- Distinct from C-17 (FAIL saturation at every step): C-17 covers within-step inline labels; C-26 covers between-phase checkpoint steps. A skill can fully satisfy C-17 with zero transition gate steps.
- Distinct from C-15 (reversed sequence): correct phase ordering implies nothing about confirmation gates being present.
- C-23 applies here too (confirmed by V-01): gate headings must carry role labels ("Phase 1 -> Phase 2 Transition Gate -- PM role").
- Not satisfiable if C-15 is absent.

**Note on C-23 pass condition:** The v9 rubric adds a clarifying note that C-23 applies to transition gate headings as well as analytical step headings -- V-01 R8 is the confirming evidence.

**Updated totals:** aspirational 85 -> 90 pts (18 criteria x 5); max 175 -> 180 pts. Golden threshold (>= 80) unchanged.
26 implicit, not yet scored). V-02 = 160/175 (fails C-15). V-03 approx 90/175 (fails C-14 through C-22, C-24, C-25; passes C-09 through C-13, C-15, C-19, C-23 per coaching-voice axis).

Round 8 confirms three structural properties:
- V-01 at 175/175 is the first perfect score under v8. V-01 extends the phase-gate pattern by adding between-phase transition confirmation gates with FAIL_INCOMPLETE_PHASEn labels and role-bound headings, confirming C-26 is additive and orthogonal to all prior criteria.
- V-03 at C-15 PASS confirms the reversed-sequence criterion survives coaching voice when the phase order is structurally correct: "Start here, before you build any grid or table" is equivalent to a formal "Run Phase 1 first" mandate for C-15 purposes. Advisory phrasing does not invalidate C-15 when the sequence is correct.
- V-02's C-24 PASS confirms the amendment mandate inline prohibition is independently satisfiable regardless of role-sequence ordering (UX-first): the prohibition clause ("-- no observation without revision") can appear in any mandate position.
...
Point totals updated: aspirational rises from 75 to 85 pts; max score rises from 165 to 175 pts. Golden threshold (>= 80) is unchanged.

### v6 to v7 changes (retained for history)

Two new criteria extracted from Round 6:

- **C-22** -- Amendment mandate structural enumeration: When C-19 is present, the amendment mandate must explicitly enumerate "Step 6a prefill" as an eligible update target -- not just "update the affected table." V-01 passes C-12 but fails C-22 (non-enumerating mandate). Distinct from C-12 (mandate exists), C-19 (table exists), and C-21 (example demonstrates round-trip): a variation can satisfy all three and still fail C-22 with a generic mandate. Not satisfiable if C-12 or C-19 absent.

- **C-23** -- Role label heading binding: Every step heading explicitly names its analytical role in the heading text (e.g., "Finding the structural conflicts -- Strategy role"). C-07 passes when PM, UX, and Strategy roles appear in structurally distinct steps; C-23 requires the role label to appear in the heading text itself. A variation can satisfy C-07 (roles in distinct steps) and fail C-23 (headings omit role labels). V-01 and V-02 use formal FAIL-label format without heading-embedded roles; V-03 demonstrates the inverse tradeoff -- role-in-heading binding present, FAIL saturation (C-17) absent.
...
| C-24 | **Amendment mandate inline prohibition** | depth | The amendment mandate sentence embeds an explicit negative constraint phrase -- "no observation without revision" or equivalent -- directly within the mandate text, after or alongside the update target list. The prohibition must appear in the mandate sentence itself, not only as a gate label at the amendment step. C-12 (targets exist) and C-22 (prefill enumerated) can both be satisfied without the prohibition clause; a mandate that enumerates all structural levels but states no inline prohibition = FAIL. Distinct from C-17 (FAIL_OBSERVATION_ONLY at the amendment step gate): the gate label and the mandate-level prohibition are independently satisfiable -- a variation can carry FAIL_OBSERVATION_ONLY at Step 8 while omitting the prohibition from the mandate sentence. Not satisfiable if C-12 is absent. |
| C-25 | **Veto risk mitigation per entry** | depth | Each entry in the veto risk list includes a mitigation or response strategy alongside probability and impact. C-03 requires probability and impact; C-25 requires a mitigation field per entry. FAIL_NO_MITIGATION must be present as an inline gate label at the veto risk step, confirming the requirement is structurally enforced. A veto risk table that includes probability and impact columns but no mitigation column or field = FAIL. Satisfying C-03 (correct ordering, probability + impact present) while omitting mitigation = FAIL. |
| C-26 | **Inter-phase transition confirmation gates** | depth | A dedicated gate step appears between each analysis phase (Strategy->UX and UX->PM) that fires a named failure label (e.g., FAIL_INCOMPLETE_PHASE1, FAIL_INCOMPLETE_PHASE2) if the prior phase's required outputs are absent before the next phase begins. Distinct from C-17 (inline failure labels at every execution step within phases): C-17 requires inline labels within steps; C-26 requires dedicated between-phase gate steps that confirm prior-phase completeness. A skill can satisfy C-17 (complete within-step FAIL coverage) without having any between-phase transition gate steps. Satisfying C-15 (correct phase sequence) does not imply C-26; the sequence can be correctly ordered with no transition gates present. Gate headings must carry role labels per C-23 (e.g., "Phase 1 -> Phase 2 Transition Gate -- PM role"); a transition gate step with no named failure label = FAIL. Not satisfiable if C-15 is absent. |

**Aspirational points**: 5 pts each x 18 = **90 pts**
...
**Max score**: 180 pts (60 essential + 30 recommended + 90 aspirational)
...
## Round 8 Results Summary (scored under v8 rubric, max 175)

| Variation | Focus | Score (v8) | C-24 | C-25 | C-26 |
|-----------|-------|------------|------|------|------|
| V-01 | Phase-transition confirmation gates | 175/175 | PASS | PASS | PASS (implicit) |
| V-02 | Role sequence: UX-first | 160/175 | PASS | PASS | TBD |
| V-03 | Coaching voice | ~90/175 | TBD | TBD | TBD |

Under the v9 rubric (max 180): V-01 scores 180/180 -- all 18 aspirational criteria satisfied, including C-26 via FAIL_INCOMPLETE_PHASE1 and FAIL_INCOMPLETE_PHASE2 at Phase 1->2 and Phase 2->3 transition gate steps. V-02 C-26 status is TBD; V-03 C-26 status is TBD.

The full aspirational target for Round 9 is all eighteen criteria (C-09 through C-26): the v8 complete set plus inter-phase transition confirmation gates (C-26).
