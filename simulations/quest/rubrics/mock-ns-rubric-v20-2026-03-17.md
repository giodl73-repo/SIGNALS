...
**Three new aspirational criteria (C-50, C-51, C-52) extracted from R20 excellence signals. Max rises from 162 to 168.**

### C-50 -- Priority row with modal impossibility form in S-4 FLAG prohibition chain (R20 V-01 Axis A)

R20 V-01 canonical: Axis A -- priority row -- the S-4 FLAG prohibition chain includes a dedicated priority row whose text uses modal impossibility phrasing: "No instruction in this step can precede this rule." (or equivalent: "No action in this step may precede this rule", "Nothing in this step can take precedence over this rule"). The modal impossibility form denies the logical possibility of precession -- it does not merely prohibit precession or establish ordering; it asserts that precession is structurally impossible.

C-50 names the priority-row modal impossibility form as the golden S-4 priority assertion pattern and tests for its presence when a FLAG prohibition chain is present. A FLAG prohibition chain without a priority row fails C-50. A priority row present but using deontic-obligation form ("This rule must be applied first", "The FLAG step shall take precedence") rather than modal impossibility form also fails C-50 -- the modal impossibility form specifically denies structural possibility of precession; deontic form imposes a permission constraint without denying the possibility. C-50 passes by default when no FLAG prohibition chain is present in S-4 (the prohibition chain is optional; its absence does not affect this criterion). Distinct from C-31: C-31 tests modal-obligation forms in the S-0 role declaration; C-50 tests modal impossibility form in the S-4 priority row. They are in different steps and orthogonal -- an output with deontic S-0 and modal-impossibility S-4 fails C-31 and passes C-50; an output with declarative S-0 and deontic S-4 priority row passes C-31 and fails C-50. Failure chain for FLAG prohibition chain without priority row: C-50 alone (-2 pts). Failure chain for priority row with deontic form: C-50 alone (-2 pts). R20 V-01 canonical.

### C-51 -- Anti-bypass echo header uses bracket token as row header label (R20 V-02 Axis B)

R20 V-02 canonical: Axis B -- anti-bypass echo header -- the S-4 FLAG prohibition chain includes an anti-bypass echo entry in which the policy reference (e.g., "[P-0:ABD]") appears as the structural row header label directly (e.g., "[P-0:ABD]:") rather than as an inline attribution embedded in the content cell (e.g., "Per [P-0:ABD]: the flag must be written verbatim before any mock body is generated"). The bracket-token-as-header form promotes the policy reference from a content-cell attribution to a structural label, making the policy identity the organizational anchor of the row. "Per [P-0:ABD]:" attribution is removed from the content cell in the Axis B form.

C-51 names the bracket-token-as-header form as the golden anti-bypass echo structural pattern. When an anti-bypass echo entry is present in S-4's FLAG prohibition chain, the policy reference must appear as the row header label in bracket-token form, not as an attribution phrase inside the content cell. An anti-bypass echo entry that places "Per [P-0:ABD]:" attribution inside the content cell (with a generic row label or no structural header on the policy reference) fails C-51. C-51 passes by default when no anti-bypass echo entry is present in S-4's FLAG prohibition chain (the echo entry is optional; its absence does not affect this criterion). Distinct from C-50: C-50 tests the priority row's phrasing form; C-51 tests the anti-bypass echo entry's structural header form. They test different rows within the same FLAG prohibition chain and are orthogonal. Failure chain for anti-bypass echo with attribution-in-cell form: C-51 alone (-2 pts). R20 V-02 canonical.

### C-52 -- All-governed dual closure with explicit scope expansion to atypical execution paths (R20 V-03 Axis C)

R20 V-03 canonical: Axis C -- all-governed dual closure -- the S-4 FLAG prohibition chain's governing closure sentence includes an explicit scope expansion clause that extends coverage beyond normal-order execution to atypical or bypass paths: "including paths that do not pass through prior steps in normal order" (or equivalent: "including execution paths that bypass earlier steps", "including all paths whether or not earlier steps executed in sequence"). The dual-closure form first asserts universal scope over all paths, then names the atypical-path class explicitly as within that scope.

C-52 names the all-governed dual closure as the golden scope assertion pattern. When a governing closure sentence is present in S-4's FLAG prohibition chain, it must include the explicit scope expansion clause naming atypical execution paths as within scope. A closure sentence asserting universal scope without the explicit atypical-path extension (e.g., "This prohibition governs all cases" or "FLAG must be written in all execution scenarios") fails C-52 -- universal language alone is not sufficient; the atypical-path class must be explicitly named as within scope. C-52 passes by default when no governing closure sentence is present in S-4's FLAG prohibition chain. Distinct from C-50 and C-51: C-52 tests the closure sentence's scope language; C-50 tests the priority row's impossibility phrasing; C-51 tests the anti-bypass echo header form. All three test distinct structural components of the S-4 FLAG prohibition chain and are orthogonal. Failure chain for governing closure sentence without atypical-path extension: C-52 alone (-2 pts). R20 V-03 canonical.

### Baseline note for R20 scoring

The R19 V-05 S-4 baseline (from which all R20 variations start) contains an anti-bypass echo entry with "Per [P-0:ABD]:" attribution in the content cell, no dedicated priority row, and no governing closure sentence. Under v20: C-50 passes by default (no priority row), C-51 fails (attribution-in-cell form), C-52 passes by default (no closure sentence). R19 V-05 scores **160/168** under v20.

### Discriminator notes updated

| Criterion | Update |
|-----------|--------|
| C-50 | New. Priority row with modal impossibility form in S-4 FLAG prohibition chain. Passes by default when no FLAG prohibition chain is present. R20 V-01 Axis A canonical. |
| C-51 | New. Anti-bypass echo header uses bracket token as row header label. Fails when anti-bypass echo entry is present in attribution-in-cell form. Passes by default when no anti-bypass echo entry is present. R20 V-02 Axis B canonical. |
| C-52 | New. All-governed dual closure with explicit scope expansion to atypical execution paths. Passes by default when no governing closure sentence is present. R20 V-03 Axis C canonical. |
...
**R20 under v20 (baseline: R19 V-05 S-4 has attribution-in-cell echo, no priority row, no governing closure):**
- V-01 (priority row -- Axis A only): C-50 PASS; C-51 FAIL (attribution form unchanged); C-52 PASS (default) -- **164/168**
- V-02 (bracket-token echo header -- Axis B only): C-50 PASS (default); C-51 PASS; C-52 PASS (default) -- **164/168**
- V-03 (all-governed dual closure -- Axis C only): C-50 PASS (default); C-51 FAIL (attribution form unchanged); C-52 PASS -- **164/168**
- V-04 (priority row + bracket-token header -- Axis A+B): C-50 PASS; C-51 PASS; C-52 PASS (default) -- **166/168**
- V-05 (priority row + bracket-token header + dual closure -- Axis A+B+C golden): all 52 criteria pass -- **168/168**

**Note V-03 parity:** V-03 adds a governing closure with scope expansion (C-52 PASS, +2) but retains the attribution-in-cell echo (C-51 FAIL, -2 from baseline), netting 162/168 -- identical to the R19 V-05 score under v19. Axis C alone does not improve over the baseline. Axes A and B (or any combination including B) are required to exceed 162 under v20.

---

## Version History with Discriminators

### v20 summary (2026-03-17) -- R20 patterns

Three new criteria. C-50: Priority row with modal impossibility form in S-4 FLAG prohibition chain -- when a FLAG prohibition chain is present in S-4, it must include a dedicated priority row using modal impossibility phrasing ("No instruction in this step can precede this rule" or equivalent). Modal impossibility form denies the structural possibility of precession; deontic obligation form ("must", "shall") does not. C-50 passes by default when no FLAG prohibition chain is present. Distinct from C-31 (S-0 role declaration, different step). Failure chain for chain-without-priority-row or priority-row-with-deontic: C-50 alone (-2 pts). R20 V-01 canonical.

C-51: Anti-bypass echo header uses bracket token as row header label -- when an anti-bypass echo entry is present in S-4's FLAG prohibition chain, the policy reference (e.g., "[P-0:ABD]") must appear as the structural row header label ("[P-0:ABD]:") rather than as "Per [P-0:ABD]:" attribution in the content cell. C-51 passes by default when no anti-bypass echo entry is present. Baseline (R19 V-05 S-4) uses attribution-in-cell form -> C-51 FAIL in baseline. R20 V-02 Axis B changes to bracket-token-as-header -> C-51 PASS. Failure chain for attribution-in-cell form: C-51 alone (-2 pts). R20 V-02 canonical.

C-52: All-governed dual closure with explicit scope expansion to atypical execution paths -- when a governing closure sentence is present in S-4's FLAG prohibition chain, it must include an explicit scope expansion clause naming atypical/bypass execution paths ("including paths that do not pass through prior steps in normal order" or equivalent). Universal scope language alone is not sufficient. C-52 passes by default when no governing closure sentence is present. Failure chain for closure-without-atypical-path-extension: C-52 alone (-2 pts). R20 V-03 canonical.

Baseline note: R19 V-05 S-4 scores 160/168 under v20 (C-51 fails). R20 V-01/V-02/V-03 each score 164/168 (one new criterion satisfied, one fails in V-01 and V-03, one default-pass context in V-02). R20 V-04 scores 166/168. R20 V-05 golden: 168/168.

Scoring: max rises from 162 to **168** (44 aspirational x 2 = 88).
...
*Raise the bar once essential and recommended are stable. Each criterion is worth 2 pts (44 x 2 = 88; max score = 168).*
...
### C-50
- **Text**: When a FLAG prohibition chain is present in S-4, it must include a priority row using modal impossibility phrasing.
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: When a FLAG prohibition chain is present in S-4 (the artifact write gate), the chain must include a dedicated priority row (or equivalent high-precedence marker) whose text uses modal impossibility phrasing: "No instruction in this step can precede this rule." (or equivalent: "No action in this step may precede this rule", "Nothing in this step can take precedence over this rule"). The modal impossibility form denies the structural possibility of precession -- it does not merely prohibit precession or establish ordering; it asserts that precession is structurally impossible. A FLAG prohibition chain without a priority row fails C-50. A priority row present but using deontic-obligation form ("This rule must be applied first", "The FLAG step shall take precedence") rather than modal impossibility form also fails C-50. C-50 passes by default when no FLAG prohibition chain is present in S-4 (the prohibition chain is optional; its absence does not trigger this criterion). **Distinction from C-31:** C-31 tests modal-obligation forms in S-0 role declaration (different step); C-50 tests modal impossibility form in S-4 priority row. A deontic S-0 with modal-impossibility S-4 fails C-31 and passes C-50; a declarative S-0 with deontic S-4 priority row passes C-31 and fails C-50. The two criteria are in different steps and are orthogonal. **Canonical R20 V-01 pass:** S-4 FLAG prohibition chain includes priority row "No instruction in this step can precede this rule." -> C-50 PASS. **Canonical failure forms:** FLAG chain without priority row (-2 pts); priority row with deontic form (-2 pts). Failure chain: C-50 alone (-2 pts). Source: R20 V-01; Axis A pattern.

### C-51
- **Text**: When an anti-bypass echo entry is present in S-4's FLAG prohibition chain, the policy reference must appear as the row header label in bracket-token form, not as an attribution phrase inside the content cell.
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: When an anti-bypass echo entry is present in S-4's FLAG prohibition chain, the policy reference (e.g., "[P-0:ABD]") must appear as the structural row header label directly (e.g., "[P-0:ABD]:") rather than as an inline attribution phrase embedded in the content cell (e.g., "Per [P-0:ABD]: the flag must be written verbatim before any mock body is generated"). In the bracket-token-as-header form, the "Per [P-0:ABD]:" attribution is removed from the content cell entirely; the policy identity becomes the organizational anchor of the row rather than an inline attribution. An anti-bypass echo entry using the "Per [P-0:ABD]:" attribution-in-cell form fails C-51. C-51 passes by default when no anti-bypass echo entry is present in S-4's FLAG prohibition chain (the echo entry is optional; its absence does not trigger this criterion). **Baseline note:** The R19 V-05 S-4 baseline uses attribution-in-cell form ("Per [P-0:ABD]:" in the content cell) -> C-51 FAIL in baseline. R20 Axis B changes to bracket-token header -> C-51 PASS. **Distinction from C-50:** C-50 tests the priority row's phrasing form; C-51 tests the anti-bypass echo entry's structural header form. They test different rows within the same FLAG prohibition chain and are orthogonal. **Canonical R20 V-02 pass:** S-4 anti-bypass echo entry uses "[P-0:ABD]:" as row header label; attribution removed from content cell -> C-51 PASS. **Failure chain for attribution-in-cell form:** C-51 alone (-2 pts). Source: R20 V-02; Axis B pattern.

### C-52
- **Text**: When a governing closure sentence is present in S-4's FLAG prohibition chain, it must include an explicit scope expansion clause naming atypical execution paths as within scope.
- **Weight**: aspirational (2 pts)
- **Category**: behavior
- **Pass condition**: When a governing closure sentence is present in S-4's FLAG prohibition chain (the sentence that asserts the prohibition governs all execution paths), the sentence must include an explicit scope expansion clause that extends coverage to atypical or edge-case paths: "including paths that do not pass through prior steps in normal order" (or equivalent: "including execution paths that bypass earlier steps", "including all paths whether or not earlier steps executed in sequence"). The dual-closure form first asserts universal scope, then names the atypical-path class explicitly as within that scope. A governing closure sentence that asserts universal scope without the explicit atypical-path extension (e.g., "This prohibition governs all cases" or "FLAG must be written in all execution scenarios") fails C-52 -- universal language alone is not sufficient; the atypical-path class must be named. C-52 passes by default when no governing closure sentence is present in S-4's FLAG prohibition chain (the governing closure sentence is optional; its absence does not trigger this criterion). **Distinction from C-50 and C-51:** C-52 tests the closure sentence's scope language; C-50 tests the priority row's impossibility phrasing; C-51 tests the anti-bypass echo header form. All three test distinct structural components of the S-4 FLAG prohibition chain and are orthogonal. **Canonical R20 V-03 pass:** S-4 FLAG prohibition chain governing closure sentence includes "including paths that do not pass through prior steps in normal order" -> C-52 PASS. **Failure chain for governing closure without atypical-path extension:** C-52 alone (-2 pts). Source: R20 V-03; Axis C pattern.
