# Rubric: trace-component (v19)

**58 criteria across 3 tiers -- 1 new aspirational criterion added from R18 excellence signals:**
...
| Aspirational | C-09 through C-58 | 100 |

**Ceiling: 190 pts** (up from 188; 1 new criterion x 2 pts)

---

The R18 signal that promoted:

**C-58 . Schema Area 3 Structured Enforcement Table** -- From R18 C-12 differential. V-02 passes C-12 because its Schema Area 3 is presented as a boundary table with an `Enforcement Rationale` column populated for every row; the column functions as a "why this form vs. no enforcement" comparison. V-01 fails C-12 because its Schema Area 3 is prose with labeled sub-sections -- no table format, therefore no comparison column. The distinction is not whether the compound-header template is present (C-56) or whether it includes exemplars (C-57), but whether the template section is structured as a table whose Enforcement Rationale column makes the enforcement logic for each template component inspectable row-by-row. C-56 requires the schema to declare the compound-header template; C-57 requires that template to include at least one worked exemplar; C-58 requires the template section itself to be in table format so that the rationale for each template element is a discrete, independently verifiable cell rather than embedded prose.

**R18 single-signal structure:**

- **C-58** (schema area 3 as enforcement table, per-schema-artifact): the compound-header template section of the schema artifact must be presented as a structured table -- not prose -- with at minimum a template-element column and an Enforcement Rationale column explaining why the compound form is required vs. simpler alternatives for each row.

C-58 requires C-56 as a structural precondition (enforcement table requires a declared template to organize into rows). C-58 and C-57 are independent at the same layer: a schema can include a worked exemplar in prose (C-57 PASS, C-58 FAIL) or in a table row (both PASS). Full expression at this layer requires all three: C-56 binds the compound form to the schema artifact, C-57 demonstrates the descriptor standard with a concrete example, C-58 makes each template component's enforcement rationale row-addressable and independently verifiable. V-02 carries C-56 + C-57 + C-58 (table with exemplar rows and Enforcement Rationale column). V-01 carries C-56 + C-57 but fails C-58 (prose schema with exemplars, no table format for Schema Area 3).
...
## Aspirational Criteria (weight: 100 points total, 2 pts each)
...
**One new criterion -- rationale:**

| Criterion | Source pattern | What v18 covered | What v19 adds |
|-----------|---------------|-----------------|---------------|
| C-58 | R18: C-12 differential -- V-02 passes C-12 because Schema Area 3 is a boundary table with `Enforcement Rationale` column populated for every row; V-01 fails because Schema Area 3 is prose with sub-sections (no table format, no comparison column). | C-56 requires the schema to declare the compound-header template as a named schema field. C-57 requires that template to include at least one concrete worked exemplar. Neither criterion requires Schema Area 3 to be in table format -- the template and exemplar can satisfy C-56 and C-57 in prose form. C-12 requires a comparison column in at least one table, which V-02 satisfies via Schema Area 3, but C-12 does not require the comparison column to be in the schema artifact's template section specifically. | Requires Schema Area 3 (or equivalent schema artifact compound-header template section) to be presented as a structured table with at minimum a template-element column and an Enforcement Rationale column populated for every row. The table format makes the enforcement logic for each template component inspectable row-by-row and independently verifiable, rather than embedded in prose. |

---

**R18 single-signal structure:**

- **C-58** (schema area 3 as enforcement table, per-schema-artifact): the compound-header template section of the schema artifact must be a structured table with an Enforcement Rationale column -- not prose -- so each template component's required-vs-alternative comparison is a discrete table cell.

C-58 requires C-56 as a structural precondition (enforcement table requires a declared template to organize into rows). C-58 is independent of C-57 at the same layer: a schema can carry a worked exemplar in prose (C-57 PASS, C-58 FAIL), in a table row without an Enforcement Rationale column (C-57 PASS, C-58 FAIL), or in a table row with an Enforcement Rationale column (C-57 PASS, C-58 PASS). Full expression at this layer: C-56 binds the compound form to the schema artifact, C-57 demonstrates the descriptor standard with a concrete exemplar, C-58 makes each template component's enforcement rationale row-addressable. V-02 carries all three: Schema Area 3 is a boundary table with exemplar rows and Enforcement Rationale column. V-01 carries C-56 and C-57 but fails C-58: Schema Area 3 is prose with sub-sections.
