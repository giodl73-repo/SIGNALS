Written to `simulations/quest/rubrics/mock-accept-rubric-v6-2026-03-17.md`.

---

# Rubric — mock-accept v6

## What changed from v5

Three new aspirational criteria (C-26 to C-28) extracted from R5 excellence signals:

| ID | Pattern | Source |
|----|---------|--------|
| C-26 | **Halt co-location at each CONSTRAINT site** | V-03's per-site halt repetition (PARTIAL on C-23 for missing Standing Rule); V-05 combines both |
| C-27 | **GATE F as standalone dedicated numbered step** | V-03/V-05 STEP 9 elevation; "more durable against context compression" |
| C-28 | **Slot 2 violation table** | V-02's 3-row x 2-col table vs. V-01's prose note |

---

## Scoring update

- Denominator: **17 → 20**. Aspirational pool = `(passes / 20) × 10`.
- **C-09 N/A cascade extended**: C-09 → C-15 → C-20 → C-23 → **C-26**; denominator when C-09 absent = **15** (was 13).
- C-24, C-25, C-27, C-28 independent of C-09 status.

---

## R5 ceiling re-scored against v6

| Variation | C-26 | C-27 | C-28 | Asp passes | Asp score | Total |
|-----------|------|------|------|------------|-----------|-------|
| V-05 (V-01+V-02+V-03) | PASS (CONSTRAINT co-location from V-03) | PASS (STEP 9 from V-03) | PASS (table from V-02) | 20/20 | 10.0 | **100.0** |
| V-04 (V-01+V-02) | FAIL (Standing Rule only, no per-site repetition) | FAIL (GATE F in STEP 8) | PASS | 18/20 | 9.0 | **99.0** |

V-05 remains the golden. R6 variations must address C-26 and C-27 — the V-04 gap — to push the minimal path toward 100.

---

Three new failure modes added (MA-24 to MA-26):

| ID | Failure Pattern |
|----|----------------|
| MA-24 | C-26: halt in Standing Rule only; evaluator must cross-reference to detect CONSTRAINT violations |
| MA-25 | C-27: GATE F embedded in existing step; no dedicated step number; not independently referenceable |
| MA-26 | C-28: Slot 2 examples listed in prose; no two-column table; requires parsing to locate |
...
| ID | Pattern | Source |
|----|---------|--------|
| C-26 | **Halt co-location at each CONSTRAINT site** -- the three-step halt instruction (C-23) is also repeated inline at every CONSTRAINT block throughout all phases; having the halt only in the opening Standing Rule satisfies C-23 but not C-26; the per-site form eliminates the need to cross-reference the Standing Rule from inside a constrained field; applies to all phases containing constrained fields | V-03 C-23: "Three-step imperatives co-located at each CONSTRAINT site" -- achieves per-site halt repetition; partial on C-23 for lacking Standing Rule block; V-05 combines both forms |
| C-27 | **GATE F as standalone dedicated numbered step** -- the Phase 6 completeness gate (GATE F or equivalent) occupies its own step with a distinct step number separate from the review artifact Write instruction; embedding GATE F within an existing step satisfies C-24 but not C-27; a dedicated step makes the gate independently referenceable by step number and resistant to context compression | V-03 / V-05: GATE F "elevated to dedicated STEP 9 with explicit section IDs -- strongest anchor"; ranking note: "more durable against context compression and future variation" vs. V-04 GATE F embedded in STEP 8 |
| C-28 | **Slot 2 violation table** -- the Slot 2 paraphrase examples (C-25) are presented as a structured two-column table (quoted near-miss form / violation type); a prose note referencing the violation class or listing examples inline satisfies C-25 (if >= 2 quoted forms present) but not C-28; the table format makes violation examples scannable by inspection without parsing prose | V-02: "SLOT 2 VIOLATION TABLE: 3 rows x 2 columns (quoted near-miss form / violation type); all examples are quoted altered-token forms" |

Scoring formula updated: aspirational denominator changes from 17 to 20. Aspirational pool = `(passes / 20) x 10`.

C-09 N/A cascade updated: if C-09 is absent -> C-15 is N/A -> C-20 is N/A -> C-23 is N/A -> C-26 is N/A; denominator becomes 15 (20 - 5). C-24, C-25, C-27, and C-28 remain in denominator regardless of C-09 status.

Three new failure modes added (MA-24 to MA-26), one per new criterion.

The R5 ceiling: V-05 at 100.0 against v5 (17/17 aspirational). Against v6: V-05 passes C-26 (per-CONSTRAINT halt from V-03), C-27 (STEP 9 from V-03), and C-28 (Slot 2 table from V-02) -> 20/20 aspirational -> (20/20) x 10 = 10.0 -> **total: 100.0**. V-04 fails C-26 (Standing Rule only, no CONSTRAINT site repetition) and C-27 (GATE F embedded in STEP 8) -> 18/20 -> 9.0 -> **total: 99.0**. R6 variations must address C-26 and C-27 to push V-04-style paths toward 100.
...
### Aspirational -- pool of 10 pts / `(passes / 20) x 10`
...
**C-09 N/A cascade:** if C-09 is absent -> C-15 N/A -> C-20 N/A -> C-23 N/A -> C-26 N/A; active denominator becomes 15 (20 - 5). C-24, C-25, C-27, and C-28 are independent of C-09 status.
