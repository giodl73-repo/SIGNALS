Written to `simulations/quest/rubrics/crew-review-rubric-v9-2026-03-16.md`.

---

**What changed v8 -> v9:** one new criterion, **C-28**.

**New criterion extracted from R9 V-01:**

| ID | Pattern | Source | Extends |
|----|---------|--------|---------|
| C-28 | VALIDATE phase criterion-failure remediation loop | V-01 R9 | C-26 |

**The signal:** V-01 R9 earned C-27 **PARTIAL** (not FAIL) because its criterion-check table entry for C-27 carried a conditional remediation instruction: *"If C-27 = NO: revise R3 synthesis so every slot receives a verdict before proceeding to output."* The VALIDATE phase stopped being a static audit and became a self-correcting execution loop. A table where every row ends with a bare YES/NO/PARTIAL passes C-26 but fails C-28.

**Updated chain tail:**
```
C-13 -> C-23 -> C-24 -> C-26 -> C-28
```
*(visible validation -> named phase -> criterion-check table -> self-referential completeness -> self-correcting remediation loop)*

**Scoring delta:**

| | v8 | v9 |
|-|----|----|
| Aspirational count | 16 | 17 |
| Aspirational max | 40 | 42.5 |
| Total max | 160 | 162.5 |
| V-01 R9 | 158.75 | **161.25** |
| V-02 R9 | 157.5 | 157.5 (unchanged) |
en PARTIAL and
PASS is the R3 text itself (resolved by V-02 R8 via C-27). The gap between FAIL and PARTIAL
is the presence of the remediation loop -- resolved by C-28.

**Scoring:** 17 aspirational x 2.5 = 42.5, max **162.5** (was 160). Golden threshold
unchanged at 80.

**R9 scores under v9:**
- V-01: 60 + 60 + 41.25 = **161.25**
  (C-26 PASS 2.5; C-27 PARTIAL 1.25; C-28 PASS 2.5; all others 35.0)
- V-02: 60 + 60 + 37.5 = **157.5**
  (C-27 PASS 2.5; C-26 FAIL 0; C-28 FAIL 0; all others 35.0)

**New chain tail:**
- Validation: C-13 -> C-23 -> C-24 -> C-26 -> **C-28**

**New relationship:**
- Criterion-check self-correction: C-24 (phase contains criterion-check table) ->
  C-26 (table includes its own governing criteria -- self-referential completeness) ->
  C-28 (table rows carry conditional remediation paths -- self-correcting execution loop).
...
|----------|--------|--------|--------|-----------|---------|
| All pass | 5/5 | 6/6 | 17/17 | 60+60+42.5=162.5 | YES |
| All essential + 2 recommended | 5/5 | 2/6 | 0/17 | 60+20+0=80 | YES (at threshold) |
| All essential + all recommended | 5/5 | 6/6 | 0/17 | 60+60+0=120 | YES |
| All essential + 1 recommended | 5/5 | 1/6 | 0/17 | 60+10+0=70 | NO |
| 4 essential + all recommended | 4/5 | 6/6 | 0/17 | 48+60+0=108 | NO (essential gap) |
| All essential + 3 rec + 3 asp | 5/5 | 3/6 | 3/17 | 60+30+7.5=97.5 | YES |
| V-05 R7 equivalent (v7 perfect) | 5/5 | 6/6 | 14/17 | 60+60+35.0=155.0 | YES |
| V-01 R8 equivalent | 5/5 | 6/6 | 15/17 | 60+60+37.5=157.5 | YES |
| V-02 R8 equivalent | 5/5 | 6/6 | 15/17 | 60+60+37.5=157.5 | YES |
| V-01 R9 equivalent (under v9) | 5/5 | 6/6 | 16.5/17 | 60+60+41.25=161.25 | YES |
| V-02 R9 equivalent (under v9) | 5/5 | 6/6 | 15/17 | 60+60+37.5=157.5 | YES |
| Perfect (v9) | 5/5 | 6/6 | 17/17 | 60+60+42.5=162.5 | YES |
...
- C-28 is a construction-time excellence signal from R9 V-01. C-28 extends C-26: the
  ...
  (table audits its own existence and the schema's anti-pattern coverage); C-28 makes the
  table self-correcting (at least one table entry carries a conditional remediation
  instruction that routes the model back to an earlier phase when a criterion fires
  NO/PARTIAL). V-01 R9 introduced this pattern via a C-27 row with force-function: "If
  C-27 = NO: revise R3 synthesis so every slot receives a verdict before proceeding to
  output." The VALIDATE phase transitions from a static audit to an execution loop. The
  same structural escalation that C-26 applied to self-reference is now applied to
  self-correction: the table does not just record -- it acts.
...
- The full relationship chains:
  ...
  - Validation phase: C-13 (visible validation -- any mechanism) -> C-23 (validation as
    ...
    self-referential completeness) -> C-28 (criterion-check table rows carry conditional
    remediation paths -- self-correcting execution loop).
  ...
  - Each step in each chain makes the prior criterion correct-by-construction rather than
    by instruction.
- V-05 R7 equivalent under v9: E=60, R=60, A=C-11 through C-25 all PASS (35.0) +
  C-26 FAIL(0) + C-27 FAIL(0) + C-28 FAIL(0) = 35.0. Composite = 155.0.
- V-01 R8 equivalent under v9: E=60, R=60, A=C-11 through C-25 all PASS (35.0) +
  C-26 PASS(2.5) + C-27 FAIL(0) + C-28 FAIL(0) = 37.5. Composite = 157.5.
- V-02 R8 equivalent under v9: E=60, R=60, A=C-11 through C-25 all PASS (35.0) +
  C-26 FAIL(0) + C-27 PASS(2.5) + C-28 FAIL(0) = 37.5. Composite = 157.5.
- V-01 R9 equivalent under v9: E=60, R=60, A=C-11 through C-25 all PASS (35.0) +
  C-26 PASS(2.5) + C-27 PARTIAL(1.25) + C-28 PASS(2.5) = 41.25. Composite = 161.25.
- V-02 R9 equivalent under v9: E=60, R=60, A=C-11 through C-25 all PASS (35.0) +
  C-26 FAIL(0) + C-27 PASS(2.5) + C-28 FAIL(0) = 37.5. Composite = 157.5.
- Scoring formula in v9: Essential 5 x 12 = 60, Recommended 6 x 10 = 60, Aspirational
  17 x 2.5 = 42.5. Max is 162.5. Golden threshold is composite >= 80 with all essential
  passing.
