Written to `simulations/quest/rubrics/listen-adoption-rubric-v4-2026-03-15.md`.

---

**Changes from v3 to v4:**

One new aspirational criterion extracted from R3 excellence:

**C-16: Pre-committed Adoption Schedule with Reconciliation** — from V-03. The distinction from C-01, C-02, and C-07:

- **vs C-01** (earliest adoption month): C-01 describes when a role *could* adopt; C-16 requires a *committed prediction* per role entered before the simulation begins — a falsifiable forecast, not a range.
- **vs C-02** (phase-close ledgers): C-02 ledgers confirm who adopted in a given phase; C-16's SCHEDULE RECONCILIATION compares pre-committed month against actual month for all 16 roles and names deviations.
- **vs C-07** (quantified S-curve): C-07 checks counts are internally consistent; C-16 checks whether the simulation confirmed or refuted its own upfront prediction.

The structural device: each simulation month block opens with "Roles committed to M[N]:", each phase close includes "Scheduled roles met:", and PHASE 5 CLOSE carries the full SCHEDULE RECONCILIATION table.

**Scoring formula update:** aspirational denominator changes from 7 to 8. Maximum composite remains 100; aspirational band (10 points) is now split across 8 criteria at 1.25 points each. An output passing all 16 criteria scores 100.
y identifies the chasm: the gap between early-adopter exit and early-majority
...
aspirational_score = (aspirational criteria passed / 8) * 10
