...
v9 adds two aspirational criteria (C-27 to C-28) extracted from Round 9 excellence
...
pointer, and V-02 was the first form to achieve this bidirectional property: the S-3 table
names the A-1 row, and the A-1 table is the explicit reference target; C-28 elevates
location-specific navigation anchoring to a pass condition, depending on C-26 and C-27).
...
- **Pass condition**: C-26 requires the compute-site consequence to cross-reference the
  ...
  at this specific, findable location). In its complete form, the bidirectional property
  is achieved when the consumption-site target (A-1) correspondingly marks itself as the
  reference destination for the compute-site cross-reference -- e.g., a note at A-1 such
  as "cross-referenced from S-3 Failure consequence" -- so that a reader arriving at
  A-1 from any direction can identify the structural link. A one-directional anchor (S-3
  names A-1's location) satisfies C-28; the bidirectional form (A-1 also names S-3 as the
  source) is the aspirational maximum. The gap closed: a generic "at the consumption site"
  cross-reference (C-26 pass) tells the reader that a companion statement exists but
...
| Band | Essential (5x) | Recommended (3x) | Aspirational (20x) |
|------|---------------|-----------------|-------------------|
| Points | 60 | 30 | 10 |
| Formula | pass/5 x 60 | pass/3 x 30 | pass/20 x 10 |
...
- **C-28 fail**: Cross-site reference is present and structurally isolated (C-26 and C-27
  ...
  find C-20's statement rather than being able to navigate directly to it. The partial
  form (S-3 names A-1's location without a reciprocal marker at A-1) passes C-28; the
  full bidirectional form (A-1 also names S-3 as the source of the cross-reference)
  represents the aspirational maximum and would be captured as a v10 excellence signal if
  consistently demonstrated. Fails automatically when C-26 or C-27 fails.
