**v8 rubric written** to `simulations/quest/rubrics/prove-topic-rubric-v8-2026-03-15.md`.

Three new aspirational criteria extracted from Round 8 excellence signals:

| ID | Axis | Pattern | Exemplars |
|----|------|---------|-----------|
| **C-25** | Handoff derivation annotation | Every handoff field carries `[derived from: X]` inline; "unlabeled field = format error" declared as separate enforcement rule | V-01, V-04, V-05 (FULL); V-03 (PARTIAL — only `incumbent_defense_closed` labeled); V-02 (FAIL — labels in schema table, bare fields at synthesis) |
| **C-26** | Pre-committed handoff schema | Complete field+source schema locked at CAMPAIGN OPEN; `schema_compliance_confirmed` at synthesis; "no additions, no omissions, source mismatch = format error" | V-02, V-04, V-05 (FULL); V-01, V-03 (FAIL — schema assembled reactively) |
| **C-27** | Independence path chain | `incumbent_defense_closed` derivation annotation includes explicit chain proving no path through `co_activation_confirmed`/`dual_lock_fired` | V-03's `positive_derivation:` + `independence_chain:` — extends C-23 from structural independence to demonstrated independence |

**Weight outcome:** Max rises 144 → 156. Aspirational expands to 19 criteria × 4 pts = 76 pts. Golden threshold unchanged at 80.

**New handoff contract chain documented** alongside the null-CE severity stack: C-25 (annotation) → C-26 (pre-commitment) → C-27 (independence proof). A v7-ceiling variation can fail all three: fields present and correctly valued, but no inline labels, no pre-committed schema, no independence chain.
 (10 pts each)
...
annotation of the form `[derived from: X]`, naming the upstream artifact, stage output, or
computation that produced the field value. A DERIVATION ANNOTATION RULE must be declared
as a separate enforcement clause — distinct from the schema compliance check — with an
explicit consequence for violations (e.g., "unlabeled field = format error"). A handoff
where derivation sources appear only in a pre-committed schema table at CAMPAIGN OPEN but
are not repeated as per-field inline labels at synthesis time is a FAIL. A handoff where
a subset of fields carry derivation labels while others are bare is a PARTIAL (2 pts).
V-01's and V-05's DERIVATION ANNOTATION RULE with "Unlabeled field = format error" and
all 8/9 fields labeled are the exemplars.

Failure mode addressed: C-23 ensures `incumbent_defense_closed` is present and independent;
C-21 ensures `co_activation_confirmed` propagates into the handoff. Neither requires the
consumer to know where each field's value originated. A handoff can satisfy C-21, C-23,
and C-26 (pre-committed schema) while being fully unauditable at the field level — every
field is present and schema-compliant, but no label connects any field to its source stage
or artifact. C-25 makes derivation provenance a structural requirement of the synthesis
output itself, enabling field-level audit without session replay. A variation can satisfy
the full v7 stack and still fail C-25: all handoff fields present and correctly valued
(C-21, C-23 PASS), schema declared at CAMPAIGN OPEN (C-26 PASS if present), but no field
carries a `[derived from: X]` label.
...
instruction must reference this pre-committed schema by name and include a compliance check
(e.g., `schema_compliance_confirmed: true`) confirming no fields were added, removed, or
derivation-source-mismatched. A handoff schema assembled reactively at synthesis time (no
schema declared before Stage 0) is a FAIL. A pre-committed schema without a compliance
check at synthesis — or a compliance check without a named reference to the CAMPAIGN OPEN
schema — is a PARTIAL (2 pts). FAIL if C-19 is FAIL (no CAMPAIGN OPEN block). V-02's
8-field DECLARED FIELDS table with derivation sources at CAMPAIGN OPEN and
`schema_compliance_confirmed` at synthesis, V-04's PRE-COMMITTED HANDOFF SCHEMA with
compliance check before synthesis writes, and V-05's schema including
...
still be assembled reactively during synthesis — a builder satisfying C-25 by labeling all
fields at synthesis time could silently add or omit fields, or mislabel sources, without
any auditable contract to check against. C-26 locks the handoff contract before evidence
gathering begins, making schema deviations detectable from the CAMPAIGN OPEN block alone.
A variation can satisfy C-25 (all fields labeled inline) and still fail C-26: every field
carries a `[derived from: X]` label, but no schema was declared before Stage 0, leaving
the field list and source assignments without a pre-registered baseline to enforce.
...
C-25 requires a derivation annotation naming the source. But a derivation annotation alone
does not prove independence: a builder can label a field `[derived from: dual_lock_result]`
and the annotation satisfies C-25 while violating C-23. C-27 requires the independence
claim to be made explicit as a traceable chain — enumerating the stage outputs that feed
the derivation and confirming none of them pass through the dual-lock mechanism. A
variation can satisfy C-23 (field is present) and C-25 (field is labeled) and still fail
C-27: `incumbent_defense_closed` carries a `[derived from: X]` label, but no path chain
exists to verify the independence assertion.
...
Targets the failure modes unique to orchestration. The nineteen criteria form six pairs,
one triple, one quartet, and two standalones:
...
- **C-23, C-24**: Structural enforcement (added v7). Each tightens a v6 audit criterion
  ...
  independent of the dual-lock mechanism. C-24 extends C-22 by requiring role-structural
  ownership of each attestation field.
...
- **C-25, C-26, C-27**: Handoff contract integrity (added v8). Each extends the handoff
  ...
  contract. C-25 requires every handoff field to carry an inline `[derived from: X]` label
  — making the handoff auditable at field level without session replay. C-26 requires the
  complete handoff schema (field names plus derivation sources) to be locked at CAMPAIGN
  OPEN and checked for compliance at synthesis — making schema deviations detectable from
  the contract alone. C-27 requires the derivation annotation on `incumbent_defense_closed`
  to include an explicit independence path chain — making the independence claim of C-23
  verifiable from the annotation itself, not just asserted by field separation.
  A variation can satisfy the full v7 stack and still fail all three: handoff fields
  present and correctly valued (C-21, C-23 PASS), but no inline labels (C-25 FAIL), no
  pre-committed schema (C-26 FAIL), no independence chain (C-27 FAIL).
...
The null-CE severity stack is unchanged from v7. C-25, C-26, and C-27 form a separate
handoff contract chain that extends C-21 and C-23 from structural presence to derivation
transparency and pre-registered compliance.

---

## Handoff Contract Chain: C-25 / C-26 / C-27

Three criteria form a complete handoff contract integrity chain spanning annotation,
pre-commitment, and independence proof:
...
from session open — deviations detectable without reading synthesis output. C-27 closes the
gap between structural independence (C-23: the field exists separately) and demonstrated
independence (the derivation path is enumerated and verifiably clean). A variation
satisfying C-21 and C-23 completely can fail all three: co-activation echoed and campaign
outcome present (v7 PASS), but no derivation labels (C-25 FAIL), no pre-committed schema
(C-26 FAIL), no independence chain (C-27 FAIL).
...
### Weight change note (v7 -> v8)

v7: max 144, aspirational 16 criteria x 4 pts = 64 pts.
...
### New criteria source table (v8)

| ID | Source | Pattern |
|----|--------|---------|
| C-25 | V-01/V-04/V-05 (Round 8) | Handoff fields carry inline derivation annotations. V-01, V-04, and V-05 declare a DERIVATION ANNOTATION RULE and label all 8-9 handoff fields with `[derived from: X]` syntax with "unlabeled field = format error" enforcement. V-02 lists derivation sources in a pre-committed schema table but produces bare fields at synthesis (FAIL). V-03 labels only `incumbent_defense_closed` while leaving seven other fields unlabeled (PARTIAL). |
| C-26 | V-02/V-04/V-05 (Round 8) | Handoff schema pre-committed at CAMPAIGN OPEN. V-02's 8-field DECLARED FIELDS table, V-04's PRE-COMMITTED HANDOFF SCHEMA, and V-05's 9-field schema (including `schema_compliance_confirmed` as a declared field) lock the complete field-and-source contract before Stage 0 and verify compliance at synthesis. V-01 and V-03 assemble handoff schema reactively at synthesis with no pre-committed contract (FAIL). |
| C-27 | V-03 (Round 8) | Independence path chain on `incumbent_defense_closed`. V-03's `positive_derivation:` and `independence_chain:` fields enumerate the specific stage outputs feeding `incumbent_defense_closed` and confirm none pass through `co_activation_confirmed` or `dual_lock_fired`. Extends C-23 from structural independence (field exists separately) to demonstrated independence (derivation path is enumerated and verifiably clean). |
