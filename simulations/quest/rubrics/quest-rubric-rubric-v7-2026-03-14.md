`simulations/quest/rubrics/quest-rubric-rubric-v7-2026-03-14.md` written.

---

**What changed v6 → v7:**

**C-26** (role-definition-as-equivalence-anchor) — promoted from R6 scorecard. When a rubric has >= 2 "or equivalent block" pass conditions, the role definition must contain that phrase once as the single propagation anchor. V-05's differentiator: it embeds the phrase at role-definition level, which propagates to C-21 and C-25 simultaneously in one structural decision. V-01/V-04 achieved identical outputs without the phrase and scored PARTIAL on both. Self-applies as **N/A** (no role definition; same scope as C-25).

**C-27** (sub-section-naming-forces-co-presence) — promoted from R6 scorecard. When a DR section must contain >= 3 distinct required items, each must appear under its own structural label; merged prose invites content fusion that degrades C-08 failure-mode distinctness. V-03's 5-point C-08 loss vs. V-05's 100 is the evidence. Self-applies as **PASS** (Notes section uses named bold-label sub-sections for every required item).

**v8 candidates**: the two promoted patterns are removed; six v7 candidates carry forward unchanged. The `rejection-log-minimum-count-scales-with-aspirational-depth` candidate description updated to reflect v7's count (19 aspirational criteria, 3 rejection log entries).
/A (this rubric has no amend or revision phase; excluded from denominator)
...
**Version**: 7. Grows as /quest:golden discovers excellence patterns.
...
- **rejection-log-minimum-count-scales-with-aspirational-depth**: rubrics with >= 8 aspirational
  ...
  sufficient depth; observed in v7 (3 entries, 19 aspirational criteria) but no variation test yet.
