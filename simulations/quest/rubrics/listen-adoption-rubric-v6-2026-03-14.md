Written to `simulations/quest/rubrics/listen-adoption-rubric-v6-2026-03-14.md`.

---

**What changed from v5:**

Two new criteria from R5 Signals 1 and 2. Signal 3 (WRITE GATE) and Signal 2 (per-criterion log format) were not elevated — reasons below.

---

**C-19 — Correction transaction content visible in output** (5 pts)
Source: the distinguishing property of V-04 and V-05 over V-01/V-02/V-03. C-18 accepts "I revised the champion entry." C-19 requires the revised champion row to actually appear and be readable. V-01's COMMIT CONFIRMATION, V-03's WRITE GATE boolean, and V-02's behavioral contract all satisfy C-18 while failing C-19 — they record that revision occurred, not what it was. C-19 fails automatically if C-18 fails.

**C-20 — Single named terminal section consolidates gate states** (5 pts)
Source: V-04's SECTION K pattern, the reason V-04 ranked above V-05 on evaluability. V-04's single named section beats V-05's distributed per-criterion sub-blocks because an evaluator checks one place, not three. The section must record initial result, revision performed (yes/no), and final result for each of C-13, C-14, C-15 — even on an all-pass audit. C-20 fails automatically if C-16 fails; it does **not** chain from C-18 (the terminal section is required even when no revision was needed).

**Signals not elevated:**
- Signal 3 (WRITE GATE / COMMIT READY boolean): mechanism, not a new output property — the artifact's existence already implies the gate passed.
- Signal 2 (per-criterion revision log format): an implementation of C-19, not a standalone criterion; noted in the C-19 evaluator note as one of two acceptable formats alongside the meta-gate section.

**Max composite: 140 → 150. Golden threshold (80) unchanged.**

The dependency chain is now four deep:
```
C-16 (audit exists) -> C-18 (correction triggered) -> C-19 (content visible)
C-16 (audit exists) -> C-20 (gate states consolidated)  [parallel, not C-18 dependent]
```
 self-audit block (C-16), which performs the check; this section records the outcome. C-20 fails automatically if C-16 fails.

The distinction across the four audit criteria in four lines:
C-16 tests whether the audit *exists and names criteria with specific checks*.
C-18 tests whether the audit *triggers correction when it finds a deficiency*.
C-19 tests whether the *corrected content is visible in the output*.
...
|---------|-------------------|
| Perfect | All 5 essential + all 3 recommended + all 12 aspirational = 150 |
...
## Criterion Dependency Chain

```
C-16 (audit exists and names criteria)
  |
  +-- C-18 (correction triggered on deficiency)
        |
        +-- C-19 (corrected content visible in output)
        |
        +-- C-20 (gate states consolidated in terminal section)
             [C-20 also fails if C-16 fails; does not chain from C-18]
```

C-16 failure cascades: C-18, C-19, and C-20 all fail automatically.
C-18 failure cascades: C-19 fails automatically (no revision occurred, no content to show).
C-20 is parallel to C-19 (both depend on C-16; C-20 does not depend on C-18 -- even an all-pass audit requires the terminal gate state section).
...
- **C-19**: The test is readable corrected content, not a correction claim. An output that says "I revised the Step 3c champion table to add the EM inertia ID for Tech Lead" passes C-18 but fails C-19 unless the updated champion table row (showing the added inertia ID and named inertia) is present and readable in the artifact. The evaluator locates the corrected structural element and confirms it now satisfies the underlying criterion's pass condition. Two implementation patterns that satisfy C-19: (a) V-04-style meta-gate section that quotes or references the revised content inline with the gate record; (b) V-05-style per-criterion log format (INITIAL RUN -> REVISION LOG -> RE-RUN -> FINAL RESULT) where REVISION LOG describes the change and RE-RUN shows the corrected content satisfying the pass condition. Either is acceptable -- the requirement is readable corrected content, not a prescribed format. C-19 fails automatically if C-18 fails.
- **C-20**: The evaluator test is: go directly to the named terminal section and verify C-18 without reading any other part of the document. The section must list all three criteria by ID or name, show initial and final states, and indicate whether revision occurred. A section that lists only final states (omitting initial states) fails C-20 because an evaluator cannot determine whether the trivial path (all initial PASS) or the revision path (initial FAIL -> revised -> final PASS) was taken. The section need not describe what was changed (that is C-19's requirement) -- it records the gate outcome. C-20 applies even when no revision was needed: an all-pass audit still requires the terminal section to confirm initial PASS for each criterion. C-20 fails automatically if C-16 fails.
...
### R5 (source for C-19, C-20)

**C-19 source -- R5 distinguishing property of V-04 and V-05**: All five R5 variations scored 140/140. Within-score ordering by C-18 evaluability robustness ranked V-04 first and V-05 second. The distinguishing property: V-04 and V-05 require the *content* of the revision to appear in the output; V-01/V-02/V-03 require only evidence that revision occurred. V-01's COMMIT CONFIRMATION records whether revision occurred but not what changed. V-03's WRITE GATE records the boolean outcome (COMMIT READY = YES/NO) but not the correction content. V-02's behavioral contract records the obligation but not its execution. C-19 lifts this content-visibility requirement into an output-level criterion: the evaluator verifies the correction by reading the corrected section, not by trusting a self-report. Signal 3 (conditional WRITE GATE / COMMIT READY boolean) was not elevated -- the WRITE GATE is a prompt engineering mechanism; the artifact's existence already implies the gate passed, so the gate state adds no independently evaluable output property beyond C-18.

**C-20 source -- V-04 SECTION K meta-gate pattern**: V-04 ranked first among the five variations for C-18 evaluability because SECTION K provides a single named structural location that consolidates initial/final gate states for all three aspirational audit criteria. An evaluator checking C-18 goes to SECTION K without tracing individual gate histories -- one section to check, one failure point. V-05's per-criterion revision log (INITIAL RUN -> REVISION LOG -> RE-RUN -> FINAL RESULT format) achieves similar content visibility but distributes the evidence across three per-criterion sub-blocks, requiring the evaluator to check each one. Both formats satisfy C-19; V-04's SECTION K additionally satisfies C-20's single-location requirement. Signal 2 (per-criterion revision log format) was not elevated as a standalone criterion because it is an implementation pattern that achieves C-19 rather than a distinct output property; it is noted in the C-19 evaluator note as one acceptable implementation alongside the meta-gate section.
