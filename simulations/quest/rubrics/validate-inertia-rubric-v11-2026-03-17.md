...
- **29 aspirational (C-09--C-37):** scoring methodology transparency, time-dependent inertia
...
10 pts each (290 total) = **370 pts max**.
...
| C-35 | **Atomic audit format declaration (no pointer indirection)** | behavior | C-34 requires the preamble to declare the audit's exact column structure before Phase 1. C-35 requires that declaration to be atomic: the column names and Evidence-column semantics are inline at the declaration site, not split across a pointer to a separate subsection. A two-step pointer read (preamble row cites "see Row 5 format subsection") creates a skip surface -- a model may read the preamble row, treat the pointer as deferred content, and never follow it to the subsection. An inline declaration that embeds the full column spec and Evidence-column instruction in a single cell ("5-column table: Source | Consumer | Verdict | Evidence [write actual downstream cite text verbatim] | Integrity") is read atomically at the declaration site with no indirection. V-47 demonstrated that the Row 5 cell can carry the full column spec and Evidence semantics without a downstream subsection. C-34 can pass without C-35; C-35 requires the audit format declaration to be self-contained at its declaration site, eliminating any pointer-indirection drift surface between the preamble reference and the format specification. |
...
- C-35 codifies the atomic audit format declaration from R10: V-47 demonstrated that the Row 5
  cell can carry the full column spec and Evidence-column semantics inline, eliminating the
  two-step pointer read (preamble row -> separate subsection). V-47 scored 340/340 with the
  column spec embedded directly in the Row 5 Requirement cell rather than pointed to a
  downstream subsection. The R10 lesson: C-34 satisfied via pointer indirection creates a skip
  surface; C-35 requires the declaration to be atomic at the declaration site, so no pointer
  need be followed.
