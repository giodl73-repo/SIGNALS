...
**Header:** `20 criteria, 200 points max` → `21 criteria, 210 points max`

**New criterion — C-21: Sparse-coverage synthesis protection** *(R5 Pattern — V-05)*

This makes the V-05 sparse-breadth clause non-inert on the rubric. The clause was an invisible quality improvement in v5 (V-03 and V-05 scored 200/200 and 190/190 identically to their paired variants). C-21 turns it into a scored criterion with a distinct PARTIAL/PASS gate:

- **PARTIAL** — three-question mandate present (C-18 PASS) but no explicit sparse-coverage guidance; synthesis may be vacated when coverage is thin
- **PASS** — explicit clause in STORY execution note maintains synthesis under narrow signal breadth; "brief is permitted" bounded by "must state direction"

**Boundary relationship clarified:** C-18 tests structural presence; C-21 tests structural durability at the input boundary. They are independent axes.

**R5 general principles (documented in Notes, not new criteria):**
Patterns 1 and 2 from the R5 `new_patterns` JSON generalize C-19 and C-20 respectively — "enforcement loop closure" and "explicit unit specification." Both are captured as prose principles in the Notes section. They have no new independent test axes yet; if a second scored instance of either emerges in a later round, a dedicated criterion would be warranted.

**R6 investigation candidate (not yet a criterion):**
COMPRESSED format × C-16 timestamp survival — flagged in R5 Design Guidance for investigation before confirming as a v7 criterion.
the threshold counts blocking gaps (entries), with per-entry line count orthogonal.

This is distinct from C-15 (ceiling mechanism exists); C-20 requires the mechanism counts the right unit.

---

**C-21 — Sparse-coverage synthesis protection** *(R5 Pattern — V-05)*

When signal breadth is narrow (found signals concentrated in one or two namespaces), the STORY block must not collapse to a non-finding. A prompt that satisfies C-18 at typical breadth can silently fail at sparse breadth — the three-question structure is nominally present but question 1 becomes "insufficient data to synthesize" rather than "partial evidence points toward X." The sparse case is the high-uncertainty case: it is precisely when synthesis most needs to commit to a direction. PARTIAL = three-question mandate present (C-18 PASS) but no explicit guidance for sparse-coverage behavior; synthesis may be omitted on grounds of thin data. PASS = an explicit sparse-coverage clause in the STORY execution note maintains the synthesis mandate at narrow signal breadth — "the structure cannot be omitted on the grounds of sparse data" — and bounds "brief is permitted" with a direction requirement: "must state what direction even partial evidence points."

This is distinct from C-18, which tests that the three-question structure is mandated; C-21 tests that the mandate is robust to extreme-input conditions and cannot be escaped on the grounds of insufficient coverage.
...
**One-criterion protection for sparse synthesis:** C-21 is a robustness guard on C-18 — C-18 requires the three-question structure; C-21 requires the structure cannot be vacated when coverage is thin.
...
**R6 investigation candidates.**
R5 R6 Design Guidance identified one unconfirmed criterion candidate for investigation:
*COMPRESSED format C-16 interaction* — whether per-signal timestamps (C-16) survive when
the COMPRESSED format trigger activates. If timestamps are dropped in COMPRESSED output,
C-16 is satisfied at low gap count but vacated at high gap count — precisely the wrong
behavior. Investigation in R6 will confirm or reject this as a v7 criterion.
