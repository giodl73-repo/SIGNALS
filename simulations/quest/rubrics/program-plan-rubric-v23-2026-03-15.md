...
## Aspirational Criteria (10 pts total — 18 criteria, 0.56 pts each)
...
aspirational_pass = ___ / 18 -> ___ * 0.56 = ___ pts  (of 10)
...
**Aspirational tier expanded: 14 → 18 criteria, scoring adjusted to 0.56 pts each**

| New ID | Pattern | Source signal | Distinction |
|--------|---------|---------------|-------------|
| C-22 | **BAD/GOOD gate contrast as reference table** — the C-10 contrast takes the specific form of a structured table with BAD and GOOD columns, making gate polarity machine-scannable | V-03 C-10 evidence: "GOOD gate table with BAD/GOOD columns" | C-10 = any labeled BAD/GOOD contrast in any form; C-22 = tabular column structure required; a prose example pair passes C-10 but not C-22 |
| C-23 | **Tabular skill catalog** — skill catalog rendered as a structured reference table (namespace × skill or equivalent), parallel in form to arc and gate tables | V-02 C-19 evidence: "skill catalog (tabular)" as the third distinct pre-template table | C-19 = at least 2 pre-template tables on distinct concerns; C-23 = the skill catalog specifically must be tabular; C-19 can pass with arc + gate tables even if skill catalog is a bullet list |
| C-24 | **Layer paragraph headers** — each C-20 layer narration paragraph opens with an explicit bold header naming both layer identity and arc role (e.g., "**Layer 2 (Depth) — artifact scope and handoff contract:**") | V-02 C-20 evidence: explicit headers "**Layer 1 (Breadth) -- artifact scope and handoff contract:**", "**Layer 2 (Depth)...**", "**Layer 3 (Synthesis)...**" | C-20 = three layer paragraphs required; C-24 = each paragraph must begin with a role-labeled header; plain prose paragraphs satisfying C-20 do not pass C-24 |
| C-25 | **Echo paragraph enumerated** — the C-21 echo prose paragraph uses explicitly numbered sub-items (minimum three) for skill-free contract / `auto: true` semantics / arc closure, making each claim individually citable | V-02 C-21 evidence: "three numbered structural elements: (1) No skills… (2) `auto: true`… (3) Arc closure function…" — "most explicitly structured echo paragraph of any variation" | C-21 = dedicated prose paragraph required; C-25 = enumerated structural points required within that paragraph; flowing prose satisfying C-21 does not pass C-25 |

**Dependency chain for new criteria:**
- C-22 requires C-10 PASS (tabular form is a stricter form of the contrast requirement)
- C-23 is independent (but in practice co-occurs with C-19 in high-scoring outputs)
- C-24 requires C-20 PASS (headers are a stricter form of the layer paragraph requirement)
- C-25 requires C-21 PASS (enumeration is a stricter form of the echo paragraph requirement)

**Scoring arithmetic**: 18 criteria × 0.56 pts = 10.08 pts total. The formula `aspirational_pass/18 * 10` yields exact values; 0.56 is the per-criterion reference weight.

**Re-scoring Round 6 at v23:**

| Variation | C-22 | C-23 | C-24 | C-25 | Aspirational | Composite |
|-----------|------|------|------|------|-------------|-----------|
| V-01 | FAIL (no BAD/GOOD table) | FAIL (skill catalog not described as tabular) | FAIL (no headers on layer paragraphs reported) | FAIL (echo paragraph structure not enumerated) | 12/18 | 96.67 |
| V-02 | FAIL (no BAD/GOOD table — C-10 FAIL) | PASS (tabular skill catalog explicit) | PASS (explicit headers per paragraph) | PASS (three numbered structural elements) | 15/18 | 98.33 |
| V-03 | PASS (BAD/GOOD columns in gate table) | requires re-run to verify | requires re-run to verify | requires re-run to verify | TBD | TBD |
