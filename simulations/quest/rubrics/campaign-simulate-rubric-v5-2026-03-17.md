Written to `simulations/quest/rubrics/campaign-simulate-rubric-v5-2026-03-17.md`.

---

**Four new aspirational criteria added (C-22 through C-25):**

| ID | Criterion | Upgrades | Source |
|----|-----------|----------|--------|
| C-22 | Severity budget gate enforces trace-before-flow | C-11 (outcome → gate) | V-01/V-04/V-05 severity budget gate; V-05 exhaustion rationale on shortfall |
| C-23 | Blast Rationale as named table column | C-20 (prose → structure) | V-02/V-04/V-05 explicit Blast Rationale column in 10–12-column table |
| C-24 | Cross-Skill Synthesis Gate extracts named patterns | C-05 (integrated → gated) | V-03 Axis C isolation; "CROSS-SKILL SYNTHESIS GATE is a synthesis step" |
| C-25 | Severity drawn from closed {CRITICAL/HIGH/MEDIUM/LOW} | completes C-14/C-19 triple | V-05 "verification gate includes Severity from four-value set check" |

**Three structural pairs now in the rubric:**
- **C-11/C-22** — trace budget met vs. trace budget *enforced by gate*
- **C-20/C-23** — rationale exists vs. rationale *in named column*
- **C-05/C-24** — integrated report vs. synthesis *gated on pattern extraction*

**Closed-vocabulary triple complete:** C-14 (System Boundary) + C-19 (Finding Type) + C-25 (Severity) — all three hallucination-susceptible fields now mechanical.

Aspirational pool: 13 → 17 criteria, still capped at 10 pts. Max score 100 unchanged. V-04 and V-05 from R5 score 100/100 on v5.
e gate converts C-11 from a
  post-hoc observation to a pre-flow enforcement step.
- **C-23** upgrades C-20 from a prose requirement to a structural one. C-20 passes when
  rationale sentences exist somewhere in the output for top-tier findings; C-23 passes
  only when those rationale sentences occupy a dedicated named column in the findings
  table, visible by column scan. Same prose-to-structure upgrade relation as C-21 has
  to C-06.
- **C-24** upgrades C-05 from an integration requirement to a synthesis gate requirement.
  C-05 passes when one unified report exists instead of five separate outputs; C-24
  passes only when a named cross-skill synthesis gate runs as an explicit step, extracts
  at least one cross-cutting pattern, and documents it before the report is finalized.
  C-17 gates on blank cells; C-24 gates on pattern extraction.
- **C-25** completes the closed-vocabulary triple with C-14 and C-19. C-14 closes System
  Boundary to 5 fixed labels; C-19 closes Finding Type to a named vocabulary; C-25
  closes Severity to the four-value set CRITICAL / HIGH / MEDIUM / LOW, enforced by a
  verification gate. Together all three fields -- the two most hallucination-prone
  (C-14, C-19) and the severity tier (C-25) -- become mechanical transcription rather
  than model judgment.
- V-04 and V-05 from Round 5 score 100/100 on v5. All four new criteria were already
  present in their outputs. The rubric caught up to the Round 5 gold standard.
...
of 17 criteria saturates the bucket. Max score = 100.
...
**Aspirational subtotal**: 10 pts max (17 criteria x 1 pt each, capped at 10)
**Max score**: 100
...
- C-09 through C-25 are structural markers worth 1 pt each, capped at 10 pts total.
  Passing all 17 earns 10 pts (same as passing any 10). The cap means the last 7 passed
...
- C-17 is the blank-cell process gate; C-24 is the synthesis process gate; together they
  make both completeness (no blanks) and depth (cross-cutting patterns) into explicit
  named steps whose outcomes appear in the artifact.
...
- Common failure modes: silent sub-skill skipping (C-01), five separate outputs with no
  ...
  clears (C-22), no cross-skill patterns extracted from synthesis gate (C-24).
