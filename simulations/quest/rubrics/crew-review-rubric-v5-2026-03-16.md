Written to `simulations/quest/rubrics/crew-review-rubric-v5-2026-03-16.md`.

---

**What changed v4 -> v5:**

Two new aspirational criteria extracted from R4 scorecard evidence:

| ID | Pattern | Source | Extends |
|----|---------|--------|---------|
| C-20 | Multi-condition gate closure with named states | V-01 R4 | C-17 |
| C-21 | Anti-pattern exclusion in escalation rules | V-02 R4 | C-18, C-19 |

**C-20** — V-01 R4's C-17 evidence showed something beyond what C-17 requires: the gate carries explicit `OPEN`/`CLOSED` states and a 5-condition closure predicate. C-17 is a blocking exit condition; C-20 is a typed state machine where "closed" is enumerable, not asserted.

**C-21** — V-02 R4's C-18 evidence included "Do not embed as a sentence within a finding cell" stated inside the escalation rule. This is C-19's anti-pattern exclusion discipline applied one level deeper — to the escalation rule itself rather than to column constraints. C-18 requires the row; C-21 requires the rule to name what is not a row.

**Scoring:** Aspirational 10 × 2.5 = 25, max 145 (was 140). Golden threshold unchanged at 80.

**R4 score examples:**
- V-01: 60 + 60 + 12.5 = **132.5** (C-11, C-15, C-16, C-17, C-20 pass)
- V-02: 60 + 60 + 10.0 = **130.0** (C-11, C-15, C-16, C-18, C-21 pass)

The relationship chains are now four links deep on both the challenger-phase axis and the null-hypothesis-template axis.
ntence") alongside the required form ("dedicated row"), making the rule
  self-documenting about its own failure mode. C-18 requires the row; C-21 requires the
  rule to name what is not a row.

The relationship chains extend:
...
- Null hypothesis template chain: C-11 (templated) -> C-16 (template enforced by
  escalation) -> C-18 (escalation is a matrix row) -> C-21 (row rule names its
  anti-form).
- Schema quality chain: C-13 (typed column contracts) -> C-19 (contracts name failure
  modes). C-21 is the escalation analog of C-19.
...
through C-16 are from R1-R2. C-17 through C-19 are from R3. C-20 through C-21 are new
from R4.
...
|----------|--------|--------|--------|-----------|---------|
| All pass | 5/5 | 6/6 | 10/10 | 60+60+25=145 | YES |
| All essential + 2 recommended | 5/5 | 2/6 | 0/10 | 60+20+0=80 | YES (at threshold) |
| All essential + all recommended | 5/5 | 6/6 | 0/10 | 60+60+0=120 | YES |
| All essential + 1 recommended | 5/5 | 1/6 | 0/10 | 60+10+0=70 | NO |
| 4 essential + all recommended | 4/5 | 6/6 | 0/10 | 48+60+0=108 | NO (essential gap) |
| All essential + 3 rec + 3 asp | 5/5 | 3/6 | 3/10 | 60+30+7.5=97.5 | YES |
| V-01 R4 equivalent | 5/5 | 6/6 | 5/10 | 60+60+12.5=132.5 | YES |
| V-02 R4 equivalent | 5/5 | 6/6 | 4/10 | 60+60+10=130 | YES |
...
- The full relationship chains:
  ...
  - Each step in each chain makes the prior criterion correct-by-construction rather than
    by instruction.
- V-01 R4 equivalent: E=60, R=60, A=C-11 PASS(2.5) + C-12 FAIL(0) + C-13 FAIL(0) +
  C-15 PASS(2.5) + C-16 PASS(2.5) + C-17 PASS(2.5) + C-18 FAIL(0) + C-19 FAIL(0) +
  C-20 PASS(2.5) + C-21 FAIL(0) = 12.5. Composite = 132.5.
- V-02 R4 equivalent: E=60, R=60, A=C-11 PASS(2.5) + C-12 FAIL(0) + C-13 FAIL(0) +
  C-15 PASS(2.5) + C-16 PASS(2.5) + C-17 FAIL(0) + C-18 PASS(2.5) + C-19 FAIL(0) +
  C-20 FAIL(0) + C-21 PASS(2.5) = 10. Composite = 130.
- Scoring formula in v5: Essential 5 x 12 = 60, Recommended 6 x 10 = 60, Aspirational
  10 x 2.5 = 25. Max is 145. Golden threshold is composite >= 80 with all essential
  passing.
