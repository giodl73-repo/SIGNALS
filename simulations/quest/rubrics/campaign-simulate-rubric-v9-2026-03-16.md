v9 written. Here's what was extracted and why:

---

**C-28 — New Finding Schema Fields Absorbed into an Existing Structural Axis**
Source: V-05 vs V-03 architectural delta. Both score 100/100, but V-03 adds Severity and Lifecycle as floating field additions — they pass C-09 and C-15 but don't update the observational discipline axis row or its OBSERVATIONAL DISCIPLINE enforcement block. V-05 absorbs them into the axis row's Observable Output description, declares their scales before execution, names their structural violations inline, and adds sub-claims to the compliance checklist row for observational discipline. Gap over C-22: C-22 requires the three core observational discipline properties to be unified under one axis row. C-28 requires that any new schema field extending that axis is also absorbed — not added as an unanchored addition. A new field added outside the axis architecture cannot be self-audited from the axis declaration.

**C-29 — Aggregation Block Epistemic Architecture Declared as a Named Default**
Source: V-05 vs V-04 delta. V-04 achieves C-27 PASS by adding individual self-characterization to all five blocks — five independent claims. V-05 goes further: OBSERVATIONAL DISCIPLINE declares a single named rule that lists all five blocks by name and assigns additive status by name, creating an architectural default. A reader no longer needs to locate five individual self-characterization statements to reconstruct the epistemic architecture. Gap over C-27: C-27 requires each block to self-characterize. C-29 requires the architecture to be declared once as a named default in OBSERVATIONAL DISCIPLINE, so per-block statements confirm a stated rule rather than each making an independent claim.

**Scoring update:**
- 19 → 21 aspirational criteria
- Raw max: 118 → 122
- Divisor: 1.18 → 1.22
- Normalized max: 100 (unchanged)
or execution log shows the five skills in order with
...
## Aspirational Criteria (42 points total, 2 pts each)
...
- **Weight**: aspirational
- **Category**: format
- **Text**: When a new field is added to the finding schema (e.g., Severity, Lifecycle),
  it must be integrated into the declaration of an existing structural axis — not
  declared as a standalone addition or a floating quality check. The correct integration
  point is the OBSERVATIONAL DISCIPLINE enforcement block: the new field is declared with
  its named scale, its structural violation for absence or merging, and its sub-claim
  entry in the compliance checklist. A field added outside this architecture satisfies
  the field's own criterion (C-09, C-15) but leaves the structural axis incomplete —
  a reader cannot verify the field from the axis declaration alone. Gap over C-22: C-22
  requires the three observational discipline properties (genre, T-1 threshold, per-scope
  gates) to be unified under one axis row. C-28 requires that new schema fields extending
  that axis are absorbed into the same axis row's Observable Output description and
  OBSERVATIONAL DISCIPLINE enforcement language, rather than floating as independent
  additions. An axis declaration that does not enumerate its own fields cannot self-audit.
- **Pass condition**: Any new finding schema field beyond the C-06/C-08 baseline (Source,
  Location, Impact, Remediation) is declared inside the OBSERVATIONAL DISCIPLINE axis row's
  Observable Output description and enforcement block. The field's scale is declared before
  execution; its structural violation for non-compliance is named inline; its sub-claim
  appears in the compliance checklist row for observational discipline. Fail if a new
  finding schema field is introduced without updating the OBSERVATIONAL DISCIPLINE axis
  declaration. Partial pass (half credit) if the field is declared in OBSERVATIONAL
  DISCIPLINE but its structural violation is not named inline or its compliance checklist
  sub-claim is missing.

### C-29 — Aggregation Block Epistemic Architecture Declared as a Named Default
- **Weight**: aspirational
- **Category**: format
- **Text**: The additive-vs-authoritative distinction for all aggregation blocks must be
  declared once as a structural default in OBSERVATIONAL DISCIPLINE — naming all blocks
  and assigning their epistemic status by name — rather than left for each block to
  assert independently through per-block self-characterization statements. A named
  architectural default means: per-scope execution sections are authoritative by default;
  all named aggregation and reorder sections are additive by default. A reader encountering
  any section knows its epistemic role without locating its self-characterization statement.
  Gap over C-27: C-27 requires each aggregation block to carry a self-characterization
  statement. C-29 requires the epistemic architecture to be declared once as a named rule
  in OBSERVATIONAL DISCIPLINE that lists all blocks by name and assigns additive status
  by name, so the per-block statements confirm a stated default rather than each making
  an independent claim. Per-block self-characterization without a named architectural
  default forces readers to aggregate five independent claims to reconstruct the
  architecture — a task that C-29 makes unnecessary by declaring it once.
- **Pass condition**: The OBSERVATIONAL DISCIPLINE section contains a named rule (or
  equivalent labeled declaration) that: (a) lists all aggregation blocks by name,
  (b) assigns additive or authoritative status to each by name, (c) states the
  architectural default for any unlisted aggregation block. The compliance checklist
  verification row for aggregation self-characterization decomposes to sub-claims by
  block group, not by individual block, confirming the declared default. Fail if epistemic
  status is asserted only per-block with no unified architectural rule in OBSERVATIONAL
  DISCIPLINE. Partial pass (half credit) if a unified rule is present but does not list
  all blocks by name, or if the compliance checklist row uses individual-block
  decomposition rather than block-group decomposition.
