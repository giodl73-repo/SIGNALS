**rhythm-behavior Rubric — v6** written to `simulations/quest/rubrics/rhythm-behavior-rubric-v6-2026-03-17.md`.
...
| | v5 | v6 |
|---|---|---|
| Aspirational denominator | 13 | **14** |
| New criterion | — | **C-21** |

**C-21 — Axis-Labeled Gate Header Encoding** (format, R5)

The R5 scorecard identified one structurally distinct pattern: naming the axis in the gate header (e.g., "Coverage Axis Gate") makes C-19 self-enforcing at the layout level — two headers cannot carry the same axis label, so the one-axis-per-gate constraint becomes visible without independent verification. Pass requires the axis word to appear in the header itself, not only in the verb or body. Positional labels ("Gate 1") fail.

**Why only one new criterion:** The other two items in `new_patterns` are explanatory, not generative — the verb-to-axis-type mapping (add→insertion, rewrite→transformation, fix→recalculation) confirms C-20's mechanism but adds no pass condition C-20 doesn't already capture, and the C-19/C-20 orthogonality observation documents a relationship between existing criteria rather than a new test site.
Ranking Present | correctness |
...
- **Category**: format
- **Origin**: R5
- **Pass condition**: Each closing confirmation gate carries a header that names its axis (e.g., "Coverage Axis Gate," "Format Axis Gate," "Inheritance Axis Gate"). The axis label must appear in the header itself — not only in the repair verb or the body of the correction instruction. Pass on this criterion makes C-19 structurally self-enforcing: two headers cannot share the same axis label, so the one-axis-per-gate constraint is visible at a glance without additional verification. Fail if any gate header omits the axis name or uses a positional label only (e.g., "Gate 1," "Check 2").
