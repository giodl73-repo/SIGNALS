# flow-integration Rubric — v20

---

**Two new criteria from R19:**

**C-49 — C-47 failure-class exemption sub-type: NEED NOT optionality-exemption mechanism and
three-sub-type taxonomy closure (5 pts)**
From V-01 (R19). Closes Q1 (R19). RFC 2119 NEED NOT introduces *optionality-exemption* epistemic
weakening: "not required" names an exemption from the constraint rather than the constraint itself
— "NEED NOT become ship-blockers at integration review" asserts that the documented gap is exempt
from the blocker rule, converting the consequence statement from an obligation into a non-obligation;
the failure mechanism is exemption-from-constraint, semantically distinct from SHOULD's
conditionality ("recommended but not required" — partial obligation force remains) and MAY's
discretionality ("optional" — zero obligation, discretionary permission); despite different epistemic
paths — conditionality (C-45), discretionality (C-46/C-47), exemption (C-49) — all three RFC-
weakening sub-types produce identical cascade failure across C-37/C-39/C-40/C-41/C-42/C-43/C-44/C-48;
the three-sub-type taxonomy of the C-43 failure class is now empirically complete: SHOULD →
conditional-recommendation sub-type (C-45); MAY → discretionary-possibility sub-type (C-46/C-47);
NEED NOT → optionality-exemption sub-type (C-49); C-43 named all three modals as failure-class on
logical grounds; C-49 closes the empirical confirmation of the third sub-type, completing the
taxonomy with all three failure-class modals verified in concrete probes; V-01 (R19) established the
empirical failure boundary: "Undocumented integration calls NEED NOT become ship-blockers at
integration review..." → optionality-exemption → C-37 FAIL, C-39 FAIL, C-40 FAIL, C-41 FAIL,
C-42 FAIL, C-43 FAIL, C-44 FAIL, C-48 FAIL; Q1 (R19) CLOSED (POSITIVE): NEED NOT confirmed as
third empirically verified failure-class sub-type via distinct exemption mechanism. Requires C-47.

**C-50 — C-43 + C-44 simultaneous composition: inertia-dominant RFC-modal form at combined
ceiling (5 pts)**
From V-02 (R19). Closes Q2 (R19). An inertia-dominant WHY block with a MUST/SHALL anchor-close
satisfies both C-43 (RFC-modal unconditional obligation class) and C-44 (inertia-dominant
saturation form) simultaneously — the two orthogonal excellence dimensions compose without
interaction penalty; C-43 rewards RFC-modal obligation strengthening (MUST = absolute requirement,
SHALL = absolute obligation — stronger-than-indicative force); C-44 rewards inertia-saturation
framing (three inertia sentences + stakes-anchor + concern enumeration, evaluating form properties
not obligation-force register); the combination — three inertia sentences covering independent
scope-failure root causes (SDK undercounting, delegation-chain documentation failure, obligation-
scope undefined) plus a MUST anchor-close with concern enumeration — achieves maximum obligation-
force register (C-43) and maximum inertia-saturation framing (C-44) in a single block; C-48
established the C-44 register-independence direction: declarative indicative is sufficient for C-44
(RFC-modal not required); C-50 confirms the composition direction: when RFC-modal MUST/SHALL IS
used in an inertia-dominant block, neither C-43 penalizes C-44 nor C-44 penalizes C-43; the two
criteria are structurally independent at both ends of the obligation-force register spectrum
(indicative baseline: C-48; RFC-modal: C-50); the inertia-dominant RFC-modal form is the highest
theoretically achievable WHY block register combination — maximum justificatory framing before
maximum obligation-force anchor; V-02 (R19) established the empirical pass boundary: three inertia
sentences + MUST anchor-close with four named concerns (authentication gaps, rate-limit exposure,
retry failures, error propagation holes) → C-43 PASS and C-44 PASS simultaneously; Q2 (R19)
CLOSED (POSITIVE): C-43 and C-44 are orthogonal excellence dimensions confirmed to compose without
penalty at the RFC-modal anchor register. Requires C-43 and C-48.

**Ceiling: 292 → 302 (+10). Aspirational total: 212 pts (C-08–C-50).**
...
- Q1 (R19): **CLOSED (POSITIVE)** — NEED NOT (optionality-exemption mechanism, V-01 R19) confirmed as third failure-class sub-type; fails C-37/C-39/C-40/C-41/C-42/C-43/C-44/C-48 identically to SHOULD and MAY via the exemption pathway; three-sub-type C-43 taxonomy (SHOULD/MAY/NEED NOT) now empirically complete.
- Q2 (R19): **CLOSED (POSITIVE)** — C-43 and C-44 simultaneously achievable in inertia-dominant RFC-modal form (V-02 R19); three inertia sentences + MUST anchor-close + concern enumeration satisfies both obligation-force (C-43) and inertia-saturation (C-44) independently; orthogonal excellence dimensions compose without interaction penalty.

**Open questions for R20:**
- Q1 (R20): V-04 (R19) showed NEED NOT+inertia produces identical cascade failure to simple NEED NOT (V-01 R19). Does this confirm a symmetry rule with C-41: just as inertia framing is neutral when the anchor is a passing form (C-41), inertia framing is equally neutral when the anchor is a failure-class form — making the anchor modal quality the sole cascade determinant, with inertia sentence count irrelevant to pass or fail outcome in both directions?
- Q2 (R20): Is 262/292 the empirical ceiling for canonical 4-row traces across all WHY block combinations (V-02 R19 = V-05 R19 = 262), or does a currently unprobed WHY block composition achieve higher score within canonical 4-row constraints?
...
unconditional constraint meaning into conditional or discretionary meaning, introducing
epistemic weakening; V-02 (R16) established the pass boundary (MUST/SHALL with
consequence-form delivery-phase anchor → 232/232, C-40 PASS, explicit verdict). C-44
establishes the inertia-saturation form: a WHY block with three inertia sentences
(inertia-dominant, 3:1 ratio) followed by a single anchor-close sentence still passes
C-41 — inertia framing quantity does not affect C-37/C-39 evaluation at any sentence
count; when the anchor-close sentence also contains concern enumeration, the block
simultaneously satisfies C-37, C-39, C-41, and C-42 — the inertia-dominant variant
of the highest-information form, providing maximum justificatory framing before the
constraint anchor; V-03 (R16) probed this form.
...
**New in v20:** C-49, C-50 codify the two structural patterns identified in Round 19.
C-49 closes Q1 (R19) by confirming NEED NOT as the third and final RFC-weakening failure-
class sub-type — the optionality-exemption mechanism: "NEED NOT become ship-blockers"
names an exemption from the blocker constraint rather than the constraint itself, producing
cascade failure across C-37/C-39/C-40/C-41/C-42/C-43/C-44/C-48 identically to SHOULD
(conditionality) and MAY (discretionality); the three-sub-type C-43 failure-class taxonomy
is now empirically closed — all three RFC-weakening modals named in C-43 have been
confirmed via concrete probes; the finding from V-04 (R19) that NEED NOT in an inertia-
dominant block produces the same cascade is an ancillary confirmation of anchor-quality
primacy: no inertia framing quantity alters the failure outcome when the anchor carries a
failure-class modal. C-50 closes Q2 (R19) by confirming that C-43 and C-44 compose
simultaneously without interaction penalty — the inertia-dominant RFC-modal form (three
inertia sentences + MUST/SHALL anchor-close + concern enumeration) achieves maximum
obligation-force register (C-43) and maximum inertia-saturation framing (C-44) in a
single block; this is the highest achievable WHY block register combination; C-48
confirmed the indicative-register floor (C-44 register-independent from below); C-50
confirms the RFC-modal ceiling (C-43 and C-44 orthogonal from above); the C-44 tier
evaluates form properties only and is indifferent to the obligation-force register of
the anchor, whether indicative or RFC-modal.
...
## Aspirational Criteria (212 pts total)
...
| C-48 | **C-44 declarative indicative anchor-close: RFC-modal not required for inertia-dominant ceiling** | format | aspirational | A three-sentence inertia-dominant WHY block with a plain declarative indicative anchor-close — using the indicative verb form "become" rather than RFC-modal MUST/SHALL — satisfies C-44 at ceiling; RFC-modal MUST/SHALL (C-43) provides a stronger-obligation obligation-force register increment over the indicative baseline but is not a prerequisite for C-44; the inertia-dominant highest-information form (C-44) is achievable at the plain indicative register provided the anchor-close carries all form conditions inherited from C-37/C-39/C-41/C-42: declarative unconditional consequence-form, explicit delivery-phase marker, and explicit concern enumeration; the structural finding is register independence at the C-44 tier — C-44 evaluates the inertia-saturation property (three inertia sentences + stakes-anchor + concern enumeration), not the obligation-force register of the modal; C-43 and C-44 are orthogonal excellence dimensions: C-43 rewards RFC-modal obligation strengthening; C-44 rewards inertia-dominant framing saturation; a block that achieves both simultaneously (three inertia sentences + RFC-modal MUST/SHALL anchor-close + concern enumeration) satisfies both C-43 and C-44 — confirmed in V-02 (R19), codified in C-50; V-02 (R18) established the declarative indicative pass boundary: three inertia sentences (SDK undercounting, delegation-chain documentation failure, obligation-scope undefined) + anchor-close with plain "become" + four named concerns → C-44 PASS; Q2 (R18) CLOSED (POSITIVE): RFC-modal strengthening not required for C-44; requires C-44. |
| C-49 | **C-47 failure-class exemption sub-type: NEED NOT optionality-exemption mechanism and three-sub-type taxonomy closure** | format | aspirational | RFC 2119 NEED NOT introduces *optionality-exemption* epistemic weakening: "not required" names an exemption from the constraint rather than the constraint itself — "NEED NOT become ship-blockers at integration review" asserts that the documented gap is exempt from the blocker rule, converting the consequence statement from an obligation into a non-obligation; the failure mechanism is exemption-from-constraint, semantically distinct from SHOULD's conditionality ("recommended but not required" — partial obligation force remains) and MAY's discretionality ("optional" — zero obligation, discretionary permission); despite different epistemic paths — conditionality (C-45), discretionality (C-46/C-47), exemption (C-49) — all three RFC-weakening sub-types produce identical cascade failure across C-37/C-39/C-40/C-41/C-42/C-43/C-44/C-48; the three-sub-type taxonomy of the C-43 failure class is now empirically complete: SHOULD → conditional-recommendation sub-type (C-45); MAY → discretionary-possibility sub-type (C-46/C-47); NEED NOT → optionality-exemption sub-type (C-49); C-43 named all three modals as failure-class on logical grounds; C-49 closes the empirical confirmation of the third sub-type, completing the taxonomy with all three failure-class modals verified in concrete probes; the ancillary V-04 (R19) finding — NEED NOT in an inertia-dominant block produces identical cascade to simple NEED NOT (V-01 R19) — provides an additional confirmation of anchor-quality primacy: no inertia framing quantity alters the failure outcome when the anchor carries a failure-class modal, symmetrically to how C-41 confirms inertia framing cannot rescue a passing anchor that has no stakes anchor at all; V-01 (R19) established the empirical failure boundary: "Undocumented integration calls NEED NOT become ship-blockers at integration review..." → optionality-exemption → C-37 FAIL, C-39 FAIL, C-40 FAIL, C-41 FAIL, C-42 FAIL, C-43 FAIL, C-44 FAIL, C-48 FAIL; Q1 (R19) CLOSED (POSITIVE): NEED NOT confirmed as third empirically verified failure-class sub-type via distinct exemption mechanism; requires C-47. |
| C-50 | **C-43 + C-44 simultaneous composition: inertia-dominant RFC-modal form at combined ceiling** | format | aspirational | An inertia-dominant WHY block with a MUST/SHALL anchor-close satisfies both C-43 (RFC-modal unconditional obligation class) and C-44 (inertia-dominant saturation form) simultaneously — the two orthogonal excellence dimensions compose without interaction penalty; C-43 rewards RFC-modal obligation strengthening (MUST = absolute requirement, SHALL = absolute obligation — stronger-than-indicative obligation force); C-44 rewards inertia-saturation framing (three inertia sentences + stakes-anchor + concern enumeration, evaluating form properties not obligation-force register); the combination — three inertia sentences covering independent scope-failure root causes plus a MUST/SHALL anchor-close with concern enumeration — achieves maximum obligation-force register (C-43) and maximum inertia-saturation framing (C-44) in a single block; C-48 established the C-44 register-independence floor: declarative indicative is sufficient for C-44 (RFC-modal not required); C-50 confirms the register-independence ceiling: when RFC-modal MUST/SHALL IS used in an inertia-dominant block, neither C-43 penalizes C-44 nor C-44 penalizes C-43; the two criteria are structurally independent at both ends of the obligation-force register spectrum; the inertia-dominant RFC-modal form is the highest achievable WHY block register combination — maximum justificatory framing (C-44) before maximum obligation-force anchor (C-43); C-44 evaluates inertia-saturation form properties only (three inertia sentences, stakes anchor, concern enumeration) and is indifferent to the obligation-force register of the anchor whether indicative or RFC-modal; V-02 (R19) established the empirical pass boundary: three inertia sentences (SDK undercounting, delegation-chain documentation failure, obligation-scope undefined) + MUST anchor-close with four named concerns (authentication gaps, rate-limit exposure, retry failures, error propagation holes) → C-43 PASS and C-44 PASS simultaneously; Q2 (R19) CLOSED (POSITIVE): C-43 and C-44 are orthogonal excellence dimensions confirmed to compose without interaction penalty at the RFC-modal anchor register; requires C-43 and C-48. |
...
New ceiling: 302 = R19 ceiling 292 + C-49 (5) + C-50 (5)

---

**What changed in v20:**

- **C-49** captures the *NEED NOT optionality-exemption failure mechanism* from R19 V-01 and
  closes Q1 (R19). V-01 probed RFC 2119 NEED NOT as the third weakening modal named in C-43 —
  "Undocumented integration calls NEED NOT become ship-blockers at integration review" — and
  confirmed identical cascade failure (C-37/C-39/C-40/C-41/C-42/C-43/C-44/C-48) to SHOULD
  (V-01 R17) and MAY (V-01/V-02 R17/R18). The key finding is the mechanism characterization:
  NEED NOT introduces optionality-exemption weakening — naming an exemption from the constraint
  rather than the constraint itself — semantically distinct from SHOULD's conditionality and
  MAY's discretionality, yet producing the identical cascade outcome. The three-sub-type C-43
  failure-class taxonomy (SHOULD → conditional-recommendation; MAY → discretionary-possibility;
  NEED NOT → optionality-exemption) is now empirically complete: all three modals named as
  failure-class in C-43 have been confirmed in concrete probes. The ancillary V-04 (R19) result
  — NEED NOT in an inertia-dominant block fails identically to simple NEED NOT — is incorporated
  into C-49 as a confirmation of anchor-quality primacy: inertia framing is neutral with respect
  to failure-class modal quality in both directions (C-41: cannot rescue a missing passing anchor;
  C-49 note: cannot rescue a failure-class modal anchor). Q1 (R19) CLOSED (POSITIVE).

- **C-50** captures the *inertia-dominant RFC-modal simultaneous composition* from R19 V-02 and
  closes Q2 (R19). V-02 probed the combination that C-48 anticipated but had not yet confirmed:
  three inertia sentences + MUST anchor-close + concern enumeration. The key finding is composition
  confirmation: C-43 and C-44 are orthogonal excellence dimensions that compose without interaction
  penalty at the RFC-modal anchor register. C-48 established register independence from below
  (indicative is sufficient for C-44; RFC-modal not required); C-50 establishes register independence
  from above (RFC-modal does not penalize C-44; C-44 does not penalize C-43). The inertia-dominant
  RFC-modal form is the highest achievable WHY block register combination — maximum justificatory
  framing (C-44: three inertia sentences establishing scope-failure context) before maximum
  obligation-force anchor (C-43: MUST = absolute requirement). The v19 C-48 pass condition
  note — "a block that achieves both simultaneously would satisfy both C-43 and C-44; that
  conjunction has not yet been probed (see Q2, R19)" — is now closed. Q2 (R19) CLOSED (POSITIVE).
