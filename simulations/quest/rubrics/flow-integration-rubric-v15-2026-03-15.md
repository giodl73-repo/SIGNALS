# flow-integration Rubric — v15

---

**One new criterion from R14:**

**C-39 — C-37 consequence-form delivery-phase equivalence (5 pts)**
Settles Q1 from R14. The C-37 stakes anchor is satisfied by consequence-form with an
explicit delivery-phase marker equivalently to temporal form. Temporal form ("before the
feature ships") names the interval before which documentation must be complete. Consequence-
form ("become ship-blockers at integration review") names the phase at which undocumented
calls become blockers. Both are delivery-phase constraint anchors; both convert the framing
block from description to constraint; the surface difference is interval-before vs.
point-at. The declarative, unconditional form is required — conditional modal framing
("can become ship-blockers") does not name a constraint and does not satisfy C-37. V-01
(R14) established the pass boundary: consequence-form + explicit delivery-phase marker +
declarative unconditional framing → C-37 PASS. Requires C-37.

**Ceiling: 242 → 247 (+5). Aspirational total: 157 pts (C-08–C-39).**

**Q1 resolution from R14 updates C-37:**
- **C-37** pass condition extended: "explicit temporal or delivery-phase anchor" now
  confirmed to include consequence-form anchors — a declarative, unconditional
  consequence statement naming the delivery phase where the gap becomes a blocker
  ("become ship-blockers at integration review") satisfies C-37 equivalently to
  interval-before temporal form ("before the feature ships"); conditional modal form
  ("can become ship-blockers") does not satisfy C-37; the interval-before and
  point-at forms are structurally equivalent constraint anchors.
...
## Aspirational Criteria (157 pts total)
...
New ceiling: 247 = R14 ceiling 242 + C-39 (5)

---

**What changed in v15:**

- **C-37 pass condition updated** — Q1 from R14 resolved definitively: consequence-form
  with explicit delivery-phase marker and declarative unconditional framing satisfies C-37
  equivalently to interval-before temporal form. The two realized forms are: (1)
  interval-before temporal form — "before the feature ships"; (2) consequence-form with
  delivery-phase marker — "become ship-blockers at integration review." Conditional modal
  form ("can become ship-blockers") does not satisfy C-37 regardless of delivery-phase
  marker presence — the modal weakens the statement to a possibility and does not name a
  constraint boundary. The pass condition text is updated to name both realized forms and
  the declarative unconditional requirement explicitly.

- **C-39** captures the *consequence-form delivery-phase equivalence* pattern from R14 V-01.
  V-01 probed the consequence-form anchor — "undocumented integration calls become
  ship-blockers at integration review and cannot be cleared without a completed trace" —
  against C-37. The adjudication confirmed: "at integration review" is an explicit
  delivery-phase anchor; "become ship-blockers" is declarative and unconditional; the
  framing converts from description to constraint equivalently to the temporal form.
  C-39 codifies the form-surface equivalence because C-37's pass condition names the
  anchor class ("temporal or delivery-phase anchor") without specifying realized surfaces,
  and C-39 closes that ambiguity — consequence-form + delivery-phase-marker + declarative
  unconditional framing is a confirmed delivery-phase anchor realization.
...
1. **C-37 stakes-only vs. concern-enumerated stakes** — Does C-37 require concern
   enumeration alongside the stakes anchor, or does a stakes-anchored purpose statement
   without naming specific verification concerns satisfy C-37? Both confirmed C-37 PASS
   variations (V-01 R13, V-01 R14) included concern enumeration alongside the stakes
   anchor; whether the stakes anchor alone — without enumerating the specific concerns
   the trace verifies — achieves C-37 has not been tested. If concern enumeration is
   required alongside the anchor, a subsequent criterion could codify the full
   concern-enumerated form as a further excellence tier above bare stakes-anchor form.
