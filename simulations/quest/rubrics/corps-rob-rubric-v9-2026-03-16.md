v9 is written. Here's a summary of what was extracted and added:

---

**Two new criteria from R8:**

**C-30 -- Run-Level Preamble Schema with Named Post-Stage Reference** (format / aspirational)
- Signal: V-05 (and V-02) declare the audit schema(s) in a run-level preamble before stages begin; each post-stage section references the preamble by name ("preamble declaration applies") rather than restating inline.
- Distinction from C-28: C-28 allows the schema co-located with the audit section (V-01 inside RULE AUDIT-SUITE, V-04 in section header). C-30 requires structural separation -- declaration before stages, named reference at application site.
- Dependency: C-28 must pass.

**C-31 -- Rule Citation Anchors in Post-Stage Section Headers** (format / aspirational)
- Signal: V-05 embeds inline rule citations in each section header itself (`[RULE CONDITION-ENUM applies]`, `[RULE BOOKEND-AUDIT: absence = FORMAT VIOLATION]`), making constraints checkable at section-open time without scanning the preamble.
- Distinction from C-26/C-27: Those criteria are satisfied by preamble declarations alone. C-31 requires the rules to be echoed inside the headers they govern.
- Dependency: C-26 or C-27 must pass.

**Scoring impact:**

| | v8 | v9 |
|--|----|----|
| Aspirational pool | 21 x 5 = 105 | 23 x 5 = 115 |
| Max composite | 195 | 205 |

**Retroactive R8 ranks under v9:** V-05 → 170, V-01 → 155, V-04 → 150, V-02 → 145, V-03 → 140.
C-28 must pass for C-30 to be scoreable.

- C-31 vs C-26/C-27: C-26 requires a named audit rule (e.g., RULE AUDIT-SUITE) with anti-pattern declarations. C-27 requires format-error escalation language for absence of gap sections. Both criteria are satisfied by preamble-level declarations. C-31 requires named rules to be echoed as inline citations inside the post-stage section headers they govern -- a rule declared only in the preamble and applied only in the post-stage body does not satisfy C-31. A format spec can pass C-26 and C-27 (rules exist in preamble) without passing C-31 (rules not cited in section headers). At least one named rule (C-26 or C-27) must pass for C-31 to be scoreable.
...
- V-05: 170 (+10 -- gains C-30 and C-31; was 160)
- V-02: 145 (+5 -- gains C-30; schema declared in preamble, post-stage section applies by reference; was 140)
- V-01: 155 (unchanged -- schema declared inside RULE AUDIT-SUITE section, not preamble; C-30 not scoreable)
- V-04: 150 (unchanged -- schema declared in section header, not preamble; no header rule citations; C-30/C-31 fail)
- V-03: 140 (unchanged -- schema declared inline in post-stage section, not preamble; C-30 fails)
...
Each criterion in a chain requires all predecessors to pass before it is scoreable.
...
| | v8 | v9 |
|--|----|----|
| Aspirational pool | 21 criteria x 5 pts = 105 | 23 criteria x 5 pts = 115 |
| Max composite | 195 | 205 |
| Golden threshold | 80 (unchanged) | 80 (unchanged) |
...
- V-05: 170 (+10 -- gains C-30 and C-31; was 160)
- V-01: 155 (unchanged -- schema inside RULE AUDIT-SUITE section, not preamble; no header rule citations)
- V-04: 150 (unchanged -- schema in section header, not preamble; no header rule citations)
- V-02: 145 (+5 -- gains C-30; preamble schema with named reference; no header rule citations)
- V-03: 140 (unchanged -- schema declared inline in post-stage section; no preamble placement)

---

## Example Score Calculations

**Minimal pass (not golden)** -- all 5 essential pass, 1/3 recommended, 0/23 aspirational:
...
**Baseline golden** -- all 5 essential pass, 2/3 recommended, 0/23 aspirational:
...
**Strong** -- all 5 essential pass, 3/3 recommended, 2/23 aspirational:
...
**R8 V-01 equivalent** -- all essential, 2/3 recommended (C-07 + C-08), 15/23 aspirational
(C-12, C-13, C-14, C-16, C-17, C-18, C-19, C-21, C-22, C-23, C-24, C-25, C-26, C-28, C-29):

```
composite = 60 + 20 + (15 * 5)
          = 60 + 20 + 75
          = 155
```
...
**R8 V-02 equivalent (under v9)** -- all essential, 2/3 recommended (C-07 + C-08), 13/23 aspirational
(C-12, C-13, C-14, C-17, C-18, C-19, C-21, C-22, C-23, C-25, C-28, C-29, C-30):

```
composite = 60 + 20 + (13 * 5)
          = 60 + 20 + 65
          = 145
```
...
**R8 V-05 equivalent (under v9)** -- all essential, 2/3 recommended (C-07 + C-08), 18/23 aspirational
(C-12, C-13, C-14, C-16, C-17, C-18, C-19, C-21, C-22, C-23, C-24, C-25, C-26, C-27, C-28, C-29, C-30, C-31):

```
composite = 60 + 20 + (18 * 5)
          = 60 + 20 + 90
          = 170
```
...
**R7 V-01 equivalent** -- all essential, 2/3 recommended (C-07 + C-08), 12/23 aspirational
(C-12, C-13, C-14, C-18, C-19, C-21, C-22, C-23, C-25, C-27, C-28, C-29):

```
composite = 60 + 20 + (12 * 5)
          = 60 + 20 + 60
          = 140
```
...
| v8      | 2026-03-16 | R7 signals -- added C-28 named field-level audit schema, C-29 enumerated condition zero-state (max 195) |
| v9      | 2026-03-16 | R8 signals -- added C-30 preamble schema with named post-stage reference, C-31 rule citation anchors in section headers (max 205) |
