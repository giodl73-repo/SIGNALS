Written to `simulations/quest/rubrics/quest-rubric-rubric-v5-2026-03-14.md`.
...
|----|--------|---------------|
| C-20 | output-contract-missing-scoring-section-fails-c03-c09-c10-jointly | Rubric prompt output contract must explicitly name a Scoring section as a required output element; its absence predicts joint failure on C-03, C-09, and C-10 regardless of criteria quality; no content-level refinement compensates for a structural omission in the output contract |
| C-21 | failure-inventory-satisfies-c11-c14-as-equivalent-design-rationale-block | C-11 pass condition must include explicit "or equivalent block" language naming the failure-inventory route; absent this language, a rubric built with a Failure Analyst phase skeleton receives an ambiguous PARTIAL rather than a clear PASS on C-11 |
| C-22 | construction-time-filter-without-output-slot-fails-c13 | C-13 pass condition must explicitly name construction-time filter activity as insufficient; deletion evidence must appear as a named section in the output document; a prompt that runs a filter during construction but omits the output slot fails C-13 regardless of filter quality |

Also updated:
- **C-11 pass condition** expanded to include "or equivalent block (failure inventory, notes section)" — C-21 self-application
- **C-13 pass condition** updated to explicitly name construction-time filter activity as insufficient — C-22 self-application
- **Self-scoring projection** updated for C-20, C-21, C-22
- **v6 candidates** section updated

**Self-scoring projection (v5 against itself):**
...
- C-20: PASS (v5 contains a Scoring section; the output contract for this rubric's structure explicitly includes the Scoring section as a required element)
- C-21: PASS (C-11 pass condition now includes "or equivalent block (failure inventory, notes section)" language; failure-inventory route produces PASS, not PARTIAL)
- C-22: PASS (C-13 pass condition now explicitly states "construction-time filter activity alone does not satisfy this criterion")
```
...
**C-20** — new criterion, no self-application edit needed (v5 already has a Scoring section from v4).

**C-21** — self-application required editing **C-11's pass condition**. v4's C-11 said only "design rationale contains... this statement appears before the first criteria table." C-14 had the "or equivalent block" language; C-11 did not. v5 adds it: *"Design rationale (or equivalent block: failure inventory, notes section, or named dominant-failure-mode statement)..."*

**C-22** — self-application required editing **C-13's pass condition**. v4's C-13 required evidence in "design rationale or notes section" but didn't explicitly disqualify construction-time-only evidence — producing PARTIAL ambiguity for V-03. v5 adds: *"...a prompt that ran a filter during construction but produced no named rejection log section in the output fails this criterion."*

The v6 candidates section replaces the v4 candidates with the three patterns from R4 analysis that approached but didn't reach criterion status: joint-failure-chain annotation, failure-analyst-skeleton formalization, and output-contract-completeness gate.
 appropriate / clear / complete / thorough / comprehensive / adequate.
...
**Version**: 5. Grows as /quest:golden discovers excellence patterns.
...
- **joint-failure-chain-annotation**: when C-03, C-09, and C-10 fail simultaneously, the
  ...
  reference the chain leave the structural root cause invisible to the rubric author. A new
  criterion requiring joint-failure annotation in the scorecard Notes section would surface this.
- **failure-analyst-skeleton-as-canonical-path**: V-01's failure-analyst route (Phase 1 failure
  ...
  structural winner across R3 and R4; a criterion naming this skeleton as the canonical path to
  Golden would formalize the evolution and give rubric authors a concrete target structure.
- **output-contract-completeness-gate**: a single gate testing whether the output contract names
  all required sections (failure inventory or DR, rejection log, rubric table, scoring section)
  would replace the distributed C-03/C-09/C-10/C-13/C-14 failure pattern with one root-cause
  finding; worth promoting once the required-section list stabilizes across two more rounds.
- **rejection-log-minimum-count-scales-with-aspirational-depth**: rubrics with >= 8 aspirational
  ...
  sufficient depth; observed in v5 (3 entries, 14 aspirational criteria) but no variation test yet.
