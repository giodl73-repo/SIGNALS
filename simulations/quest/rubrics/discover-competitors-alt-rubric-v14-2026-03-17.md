Rubric v14 written to `simulations/quest/rubrics/discover-competitors-alt-rubric-v14-2026-03-17.md`.

---

**Summary of changes from v13 to v14:**

Three new aspirational criteria added from R14 excellence signals:

| ID | Criterion | Category | Key dependency |
|----|-----------|----------|----------------|
| C-50 | Phase 2 COMPLETENESS row as structural tail validation | format | Independent of C-49; extends the final-row pattern to Phase 2 (previously the only production phase without a tail checkpoint) |
| C-51 | OUTPUT CONTRACTS "Validated by" column for full slot lifecycle | behavior | Independent of C-27 and C-34; completes the third lifecycle role (write → validate → consume) in the contract table |
| C-52 | AMEND "Invariants" column for two-sided amendment boundary | format | Strictly stronger than C-38; adds the stable side (what must not change) to the existing delta side (Slots re-filled, Gates re-run) |
...
| Exceptional | >= 280 | >= 295 |
| Strong | >= 253 | >= 266 |
tions named amendment invariant failures; implies C-38 |

**Scoring update (v13 -> v14):**
...
- C-50 is independent of C-49: C-49 applies to Phase 5 (findings synthesis); C-50 applies to Phase 2 (competitor table collection); each makes a distinct production phase self-validating; neither implies the other; together they establish the final-row confirmation pattern in all multi-row production phases
- C-51 extends C-27 (producer column) and C-34 (consumer column) by completing the third lifecycle role: C-27 names the writer; C-34 names the consumers; C-51 names the validator; all three together make the full slot lifecycle (write -> validate -> consume) machine-readable from a single contract row; C-51 is independent of C-47 (C-47 applies bidirectionality to WRITE-TOKEN REGISTRY; C-51 applies lifecycle completeness to OUTPUT CONTRACTS; different tables)
- C-52 is a strictly stronger form of C-38: C-52 satisfaction implies C-38 (Slots re-filled and Gates re-run columns are a subset); C-38 does not imply C-52 (Invariants column may be absent); C-52 closes the last gap in the AMEND boundary: C-38 declares the delta side (what must change); C-52 declares the stable side (what must not change)
...
## Aspirational Criteria (weight = 220 points total, 5 points each)
...
| C-50 | Phase 2 COMPLETENESS row as structural tail validation | format | Phase 2 (competitor table collection) includes a COMPLETENESS row (or equivalent named tail checkpoint) as the final row of its structured output table. The COMPLETENESS row must confirm: C0 is present at row 0; the external competitor row count meets the minimum threshold (>= 3); all non-C0 rows carry a GATE 1 PASS status; all rows carry a GATE 2 PASS status; and Focus-column presence matches the FOCUS-STATE (slot 0) declaration. The COMPLETENESS row must have a named failure state (e.g., "Competitor table completeness failure" or equivalent). Phase 3's prerequisite step in the EXECUTION DEPENDENCY table must reference the Phase 2 COMPLETENESS row specifically -- not merely "Phase 2" -- confirming that the downstream phase depends on a structurally confirmed-complete competitor table. A Phase 2 that ends at the last competitor row without a named tail checkpoint does not satisfy; the completeness validation must be a named, independently reviewable table row. C-50 extends the final-row confirmation pattern established by C-49 (Phase 5 COMPLETENESS row), C-37 (GAP-CONFIRMED row in Phase 3), and C-36/C-45 (THEREFORE row in Phase 4) to Phase 2 -- making every production phase self-validating before downstream execution begins. C-50 is independent of C-49: C-49 applies to Phase 5 (findings synthesis); C-50 applies to Phase 2 (competitor table collection); each makes a distinct production phase self-validating; neither implies the other. |
| C-51 | OUTPUT CONTRACTS "Validated by" column for full slot lifecycle | behavior | The OUTPUT CONTRACTS table includes a dedicated "Validated by" column (or equivalent) that names, for each slot, the gate or step that validates the slot value after it is filled -- making the full slot lifecycle (produced by -> validated by -> required by) machine-readable from a single contract row. A contract table that includes a "Filled by phase" column (C-27) and a "Required by" column (C-34) but omits a "Validated by" column does not satisfy; all three lifecycle roles -- producer, validator, and consumer -- must be declared as dedicated queryable columns within the contract table. Gates that validate slot values must back-reference the OUTPUT CONTRACTS "Validated by" column by label (e.g., "per OUTPUT CONTRACTS 'Validated by'") to close the loop from gate back to contract. C-51 extends C-34 (inter-slot consumer column) and C-27 (producer column) by completing the third lifecycle role: C-27 names the writer; C-34 names the consumers; C-51 names the validator. C-51 is independent of C-47: C-47 applies bidirectionality to the WRITE-TOKEN REGISTRY (consumed by column for tokens); C-51 applies lifecycle completeness to the OUTPUT CONTRACTS table (validated by column for slots); both make different tables more complete; neither implies the other. Neither C-27 nor C-34 individually or jointly implies C-51. |
| C-52 | AMEND "Invariants" column for two-sided amendment boundary | format | The AMEND section's structured table (as required by C-38) includes a dedicated "Invariants" column (or equivalent) that names, for each adjustment row, the set of output elements that must not change when the adjustment is applied -- making the stable set of each amendment machine-readable by column inspection alongside the delta (Slots re-filled, Gates re-run). A violation of the invariants for any adjustment is named an **amendment invariant failure**. An AMEND table satisfying C-38 (Slots re-filled and Gates re-run columns present) but missing the Invariants column does not satisfy C-52; all three boundary columns -- what must change (Slots re-filled), what must re-execute (Gates re-run), and what must not change (Invariants) -- must be present for the amendment boundary to be fully specified as a two-sided machine-checkable specification. C-52 is a strictly stronger form of C-38: C-52 satisfaction implies C-38 (the table has Slots re-filled and Gates re-run columns as a subset of its columns); C-38 satisfaction does not imply C-52 (the Invariants column may be absent). C-52 closes the last gap in the AMEND boundary specification: C-38 declares the delta side (what must change or re-execute); C-52 declares the stable side (what must not change), making amendment correctness checkable by two-sided column inspection rather than one-sided delta tracking alone. |
...
Grade bands rescaled proportionally from v13 (v13 max = 295; v14 max = 310).
...
- C-50 is independent of C-49: C-49 applies to Phase 5 (findings synthesis tail); C-50 applies to Phase 2 (competitor table collection tail); neither implies the other; together they extend the final-row confirmation pattern to all multi-row production phases
- C-50 extends the tail checkpoint pattern established by C-37 (GAP-CONFIRMED, Phase 3), C-36/C-45 (THEREFORE, Phase 4), and C-49 (COMPLETENESS, Phase 5) to Phase 2; C-50 does not imply any of those criteria
- C-51 is independent of C-27 and C-34: all three are dedicated OUTPUT CONTRACTS columns; none implies any other; together all three make the full slot lifecycle (write -> validate -> consume) machine-readable at the contract level
- C-51 is independent of C-47: C-47 adds "Consumed by" to WRITE-TOKEN REGISTRY (token-level); C-51 adds "Validated by" to OUTPUT CONTRACTS (slot-level); different tables; neither implies the other
- C-52 automatically satisfies C-38; C-38 does not imply C-52 (Invariants column may be absent)
- C-52 does not imply C-49 and C-49 does not imply C-52: C-49 makes Phase 5 findings self-validating (tail row); C-52 makes the amendment boundary two-sided (stable-side column); each closes a different production boundary
...
**Why C-50 (Phase 2 COMPLETENESS row as structural tail validation):**
R14 V-01, V-04, and V-05 showed that Phase 2 was the only multi-row production phase without
a structural tail checkpoint. Every other production phase ends with a named confirmation row:
Phase 3 ends with GAP-CONFIRMED (C-37); Phase 4 ends with THEREFORE (C-36/C-45); Phase 5 ends
with COMPLETENESS (C-49). Phase 2 produces the competitor table -- the foundation consumed by
every downstream phase -- but had no analogous row confirming that the table was complete
before Phase 3 began. Without a Phase 2 COMPLETENESS row, the most consequential upstream
failure mode (incomplete or partially gated competitor table) is detectable only by scanning
all competitor rows at Phase 3 entry time. When a COMPLETENESS row is present at Phase 2's
tail, the following invariants are independently checkable as a single named checkpoint: C0 is
present at row 0 (inertia anchor declared first), external competitor count >= 3 (minimum
coverage threshold met), all non-C0 rows GATE 1 PASS (citation gate applied exhaustively), all
rows GATE 2 PASS (anchor gate applied exhaustively), and Focus column presence matches
FOCUS-STATE slot 0 (format integrity confirmed). The COMPLETENESS row also changes the
EXECUTION DEPENDENCY table: Phase 3's prerequisite step becomes "Phase 2 COMPLETENESS row"
rather than "Phase 2," making the downstream dependency conditional on the confirmed-complete
table rather than the phase's mere execution. This is the same DAG-precision insight that
motivates C-42 (explicit prerequisite column) and C-44 (exhaustive slot reads): dependencies
should be declared at the level of specific structural checkpoints, not phase-level granularity.
C-50 is independent of C-49: each applies to a distinct production phase; neither implies the
other. Together, C-49 and C-50 make the two largest multi-row production phases (Phase 2 and
Phase 5) self-validating, completing the final-row confirmation pattern across all major phases.

**Why C-51 (OUTPUT CONTRACTS "Validated by" column for full slot lifecycle):**
R14 V-02, V-04, and V-05 showed that the OUTPUT CONTRACTS table had two of three lifecycle
roles declared as columns -- producer (C-27, "Filled by phase") and consumer (C-34, "Required
by") -- but was missing the validator role. A slot's lifecycle has three distinct parties: the
phase that produces its value (writer), the gate or step that confirms the value conforms to
its declared format (validator), and the downstream slots or steps that consume it (consumers).
When only producer and consumer are declared in the contract table, the validator must be
inferred by reading individual gate descriptions: GATE 1 validates slot 2 (citation values);
GATE 2 validates slot 3 (anchor column values); GATE 4 validates slot 1 (inertia mechanism).
When a "Validated by" column is present, the validation assignment for each slot is queryable
from the contract alone -- a reviewer can determine the complete write-validate-consume chain
for any slot without reading any gate or phase body. V-02, V-04, and V-05 added "Validated by"
with entries such as "GATE 1 (per-row citation check)" for slot 2 and "GATE 4 (inertia write
row)" for slot 1; gates back-reference "per OUTPUT CONTRACTS 'Validated by'" to close the
loop from gate back to contract. This is the same bidirectionality principle applied to the
contract table that C-47 applies to the WRITE-TOKEN REGISTRY: both tables gain a third column
that makes the full artifact lifecycle machine-readable within PREFLIGHT. C-51 is independent
of C-47: they address different tables (OUTPUT CONTRACTS vs. WRITE-TOKEN REGISTRY) and
different artifact types (slots vs. tokens); neither implies the other. Neither C-27 nor C-34
individually or jointly implies C-51: both are present in v13 variations without C-51.

**Why C-52 (AMEND "Invariants" column for two-sided amendment boundary):**
R14 V-03 and V-05 showed that the AMEND table (C-38) declared what must change per adjustment
(Slots re-filled, Gates re-run) but had no column declaring what must not change. The omission
creates an asymmetric amendment specification: a reviewer can verify that the required re-
execution occurred (by checking the Gates re-run column) but cannot verify that no prohibited
changes occurred (because no stable set is declared). V-03 and V-05 added an "Invariants"
column that names, for each adjustment row, the output elements that must remain unchanged --
for example, a focus-dimension shift must not change the C0 row, the INERTIA-REF token value,
or the threat-level assignment for non-focus-related competitors. When invariants are violated,
the failure is named an amendment invariant failure, making the violation a first-class
checkable outcome rather than an undefined error. This closes the last gap in the AMEND
boundary specification: C-38 declared the delta side (what must change or re-execute) and
C-14 declared the proof-rerun requirement for focus shifts; C-52 declares the stable side
(what must not change), making the AMEND table a fully two-sided machine-checkable
specification of each amendment's correctness boundary. The insight is analogous to the one
motivating C-12 (self-negating cross-dimensional finding): a proof of correctness requires
both positive evidence (required elements present) and negative evidence (prohibited changes
absent); C-38 provides the positive side; C-52 provides the negative side. C-52 is a strictly
stronger form of C-38; it satisfies C-38 but not vice versa.
...
**C-50 vs C-49:**
C-50 extends the tail checkpoint pattern to Phase 2 (competitor table); C-49 extends it to
Phase 5 (findings synthesis). Each applies to a distinct production phase; neither implies the
other. Together they make the two largest multi-row production phases self-validating before
downstream execution begins.

**C-50 vs C-37/C-36/C-45:**
C-37 defines the tail confirmation row for Phase 3 (GAP-CONFIRMED); C-36/C-45 define the tail
confirmation row for Phase 4 (THEREFORE); C-50 defines the tail confirmation row for Phase 2
(COMPLETENESS). All three phases share the same pattern -- a named, independently reviewable
final row that confirms the phase's output is complete before the next phase reads it. None
implies any other; each targets a distinct phase boundary.

**C-51 vs C-27 and C-34:**
C-27 adds "Filled by phase" (producer), C-34 adds "Required by" (consumers), and C-51 adds
"Validated by" (validator) -- three orthogonal contract-level columns, each targeting a
different lifecycle role for each slot. None implies either of the others; all three together
make the full slot lifecycle (write -> validate -> consume) machine-readable at the contract
level without reading any gate or phase body.

**C-51 vs C-47:**
C-51 applies lifecycle completeness to the OUTPUT CONTRACTS table (validator role for slots).
C-47 applies bidirectionality to the WRITE-TOKEN REGISTRY (consumer role for tokens). Both
make PREFLIGHT more complete as a specification; they address different tables and different
artifact types. Neither implies the other.

**C-52 vs C-38:**
C-52 is a strictly stronger form of C-38. C-38 requires the AMEND table to have Slots re-filled
and Gates re-run columns (delta side). C-52 requires those columns plus an Invariants column
(stable side). C-52 satisfaction implies C-38 (the required C-38 columns are a subset); C-38
satisfaction does not imply C-52 (Invariants column may be absent).

**C-52 vs C-49:**
C-52 closes the amendment boundary (stable side of each adjustment). C-49 closes the initial
findings production boundary (COMPLETENESS tail row of Phase 5). Each addresses a different
production boundary; neither implies the other. Both together close the full production-and-
amendment lifecycle of Phase 5: C-49 confirms the initial production is complete; C-52 confirms
that any amendment preserves the invariants of the unamended elements.
...
**v13 -> v14:**
- C-50 added -- Phase 2 competitor table must end with a COMPLETENESS row as a named tail checkpoint confirming C0 at row 0, external count >= 3, all GATE 1 + GATE 2 pass, Focus-column-present status; Phase 3 prerequisite step must reference Phase 2 COMPLETENESS row (not merely "Phase 2"); makes Phase 2 self-validating before any downstream phase reads the competitor table; extends the final-row confirmation pattern to Phase 2 (the only production phase previously without a tail checkpoint); independent of C-49 (each targets a distinct phase) (from R14 V-01, V-04, V-05 vs. V-02, V-03 no Phase 2 tail row)
- C-51 added -- OUTPUT CONTRACTS table must include a dedicated "Validated by" column naming the gate or step that validates each slot value after it is filled, completing the full slot lifecycle (write: C-27 "Filled by phase"; validate: C-51 "Validated by"; consume: C-34 "Required by") as three machine-readable columns in a single contract row; gates must back-reference "per OUTPUT CONTRACTS 'Validated by'" to close the loop from gate back to contract; independent of C-47 (C-47 applies bidirectionality to WRITE-TOKEN REGISTRY; C-51 applies lifecycle completeness to OUTPUT CONTRACTS) (from R14 V-02, V-04, V-05 vs. V-01, V-03 no "Validated by" column)
- C-52 added -- AMEND structured table (C-38) must include a dedicated "Invariants" column naming what must not change for each adjustment, completing the amendment boundary as a two-sided specification (delta side: Slots re-filled + Gates re-run; stable side: Invariants); violations named amendment invariant failures; strictly stronger than C-38 (C-52 satisfaction implies C-38); independent of C-49 (amendment boundary vs. initial findings production boundary) (from R14 V-03, V-05 vs. V-01, V-02, V-04 no Invariants column)
- Aspirational tier expands from 41 to 44 criteria; aspirational max moves from 205 to 220
- Max composite moves from 295 to 310
- Golden threshold unchanged: all 5 essential PASS AND composite >= 90
- Grade bands rescaled: Exceptional >= 295, Strong >= 266, Passing >= 90
