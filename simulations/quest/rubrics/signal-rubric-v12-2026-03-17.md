Written to `simulations/quest/rubrics/signal-rubric-v12-2026-03-17.md`.

---

**v12 adds three criteria (C-36, C-37, C-38), denominator 27 → 30:**

| ID | Axis | Source | What it captures |
|----|------|--------|-----------------|
| **C-36** | R | Signal 1: gate-check type symmetry | Prompt must include a named **COMPLIANCE GATE COVERAGE MATRIX** (check types x modes). Same named-table elevation C-15 applied to widths and C-32 applied to domain nouns. Makes completeness a first-class artifact, not an implicit property of individual gate sections. |
| **C-37** | S | Signal 2: format check mirrors the spec | FULL gate Check 4 must **embed** SECTION FORMAT sub-elements verbatim inline (header pattern, `->` separator, footer pattern) — not reference the spec by name. A name-reference requires cross-prompt recall; verbatim embed is self-contained. Same inline-data principle as C-31 (labeled vs anonymous counts). |
| **C-38** | T | Signal 3: positional group boundaries for headerless output | BARE gate Check 3 must use **positional line-range notation** ("first 8 lines are scout-* names, next 4 are draft-* names...") for all 12 namespace groups. Name-list notation for headerless output requires correlating command names to namespaces at gate time; line-range notation requires only positional counting. Complements C-33 (FULL uses names because headers anchor identity; BARE uses positions because there are no headers). |

**R11 retroactive under v12:** V-04/V-05 score 99.33 — C-37 and C-38 already satisfied by their C-34/C-35 implementations; C-36 absent from all R11 prompts. **R12 sole new target: C-36 (coverage matrix).** If R+S+T independence holds, R12 V-06 saturates v12 at 100.00.
 x check-type gaps). R12
targets the *how*: (1) make coverage explicit as a prompt artifact, (2) embed the spec at
the enforcement point rather than referencing it by name, (3) use the only self-verifiable
order notation for headerless output (line-range bounds) rather than name lists that
require a cross-section mapping.

**R11 retroactive under v12:** V-04/V-05 implementations of C-34 and C-35 already satisfy
C-37 and C-38 (their format check embeds sub-elements verbatim; their order check uses
line-range groupings). C-36 is not present in any R11 variation -- it is the sole new
target for R12 isolation.

| V | C-36 | C-37 | C-38 | C-34 | C-35 | Asp (30) | Composite |
|---|------|------|------|------|------|----------|-----------|
| **V-04** | FAIL | PASS | PASS | PASS | PASS | 28/30 -> 9.33 | **99.33** |
| **V-05** | FAIL | PASS | PASS | PASS | PASS | 28/30 -> 9.33 | **99.33** |
| **V-01** | FAIL | PASS | FAIL | PASS | FAIL | 27/30 -> 9.00 | **99.00** |
| **V-02** | FAIL | FAIL | PASS | FAIL | PASS | 27/30 -> 9.00 | **99.00** |
| **V-03** | FAIL | FAIL | FAIL | FAIL | FAIL | 25/30 -> 8.33 | **98.33** |

R12 must run three-axis isolation (V-01=R only, V-02=S only, V-03=T only) to confirm each
contributes exactly +0.33 independently. Then dual-axis (V-04=R+S, V-05=R+T) and
convergence (V-06=R+S+T). If independence holds, R12 V-06 saturates v12 at 100.00.
...
- Aspirational tier: 10 points max (0.33 pts each; 1/30 x 10 = 0.333)
