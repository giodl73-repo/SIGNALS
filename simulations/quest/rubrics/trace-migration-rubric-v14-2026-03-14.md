Reading the scorecard carefully for R15 excellence signals before writing the updated rubric.

Two new patterns emerge:

1. **V-01 Three-Artifact Alignment Check** — the scorecard explicitly flags that B2 contains a named binding statement: *"The substrate condition must be the same infrastructure state named in COST LEDGER row (a) and analyzed in Q1."* C-40 and C-41 test the two individual artifact conditions independently; no criterion yet tests whether the prompt contains an explicit cross-artifact binding constraint that makes the three-artifact coherence structurally enforced rather than coincidentally achieved.

2. **V-02 C-41 evidence** — "includes conditional return-to-parse instruction." V-01's C-41 passes on negative-constraint language alone; V-02 adds a failure-triggered return path. No criterion yet distinguishes a soft categorical prohibition from a hard parse-return gate on row (a) category violation.

These become **C-42** and **C-43**.

---

```markdown
# trace-migration Rubric — v14

---

**What changed: v13 → v14**

Two new aspirational criteria, both sourced from R15 excellence signals:

| New | Source | Distinction from prior |
|-----|--------|------------------------|
| **C-42** Three-Artifact Alignment Binding Statement as Structural Cross-Enforcement | V-01 axis: a named statement in the prompt explicitly requires that the substrate condition named in COST LEDGER row (a) be the same infrastructure state analyzed in Phase A Q1, and that B2's cross-role causal chain cite that same condition as its shared failure substrate. The statement names all three artifacts — COST LEDGER row (a), Phase A Q1, B2 chain substrate — and explicitly requires them to name the same condition class. C-40 and C-41 test whether each artifact individually meets its criterion; C-42 tests whether a binding statement exists that structurally mandates the three-artifact coherence. | C-40 tests whether Phase A Q1 is the Operations/infrastructure role. C-41 tests whether COST LEDGER row (a) names a schema/infrastructure condition. C-42 tests whether the prompt contains an explicit named constraint that BINDS these two artifacts to B2's chain substrate — a statement that makes the three-artifact alignment a structural mandate rather than a happy coincidence of three independent criterion passes. An output where Phase A is Operations-first (PASSING C-40) and COST LEDGER row (a) names a schema condition (PASSING C-41) but contains no explicit binding statement tying both artifacts to B2's chain substrate PASSES C-40 and PASSES C-41 and FAILS C-42: each artifact is individually compliant, but the cross-artifact coherence is asserted by the evaluator rather than enforced by the prompt. |
| **C-43** Conditional Return-to-Parse Gate on COST LEDGER Row (a) Category Violation | V-02 axis: the COST LEDGER row (a) category constraint (C-41) is reinforced by a conditional return-to-parse instruction — if row (a) names a revenue metric, Commerce process disruption, or Finance outcome rather than a schema state or infrastructure condition, the prompt explicitly directs the model to return to parse phase before proceeding to Q1. The return-to-parse path elevates the row (a) category constraint from a structural preference to a hard parse gate, equivalent in enforcement weight to the binary gate mechanism (C-21, C-34): a category violation blocks Phase A entry rather than producing a misaligned substrate that propagates silently through the analysis. | C-38 tests three-row completeness — a category violation is invisible to row-count verification. C-41 tests whether row (a) names the correct category — a strong negative constraint satisfies C-41 even without a return path. C-43 tests whether the prompt specifies a conditional return-to-parse instruction triggered by a C-41 violation. An output that enforces C-41 with a strong negative prohibition ("must not name a revenue metric, a Commerce process disruption, or a Finance outcome") PASSES C-41 and FAILS C-43 if no parse-return path is specified: the constraint is present but the enforcement mechanism treats a category violation as a style deviation rather than a gate failure. When C-43 passes, a wrong-category row (a) triggers a parse restart, ensuring B2's chain substrate and Phase A Q1 analysis can never be built on a misaligned COST LEDGER foundation. |

**Scoring:** 255 → 265 pts · 41 → 43 criteria · 33 → 35 aspirational · Golden 206 → 206 (78%).
...
|-----|-----------------|---------------|
| **C-42** | Cross-artifact binding | Whether a named statement explicitly requires COST LEDGER row (a), Phase A Q1, and B2 chain substrate to name the same infrastructure condition — making three-artifact coherence a prompt-enforced mandate rather than an evaluator inference |
| **C-43** | Parse-gate enforcement | Whether a C-41 category violation triggers a conditional return-to-parse instruction, elevating the row (a) category constraint to the same enforcement level as the binary gate mechanism |
