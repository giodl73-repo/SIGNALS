...
## Aspirational Criteria (10 pts total — 12 criteria, 0.83 pts each)
...
aspirational_pass = ___ / 12 -> ___ * 0.83 = ___ pts  (of 10)
...
- **C-10**: The contrast can appear in instructions, comments, or as an annotated example pair. A checklist item saying "avoid 'stage complete'" is not sufficient — needs a labeled good/bad juxtaposition.
- **C-11**: Checklist must be binary (checkbox or yes/no) and property-verifying, not a "did you remember to think about X" process prompt. Minimum 4 items. A single block comment with rules does not qualify.
...
**Aspirational tier expanded: 10 → 12 criteria, scoring adjusted to 0.83 pts each**

| New ID | Pattern | Source | Distinction |
|--------|---------|--------|-------------|
| C-18 | **YAML-embedded dependency map** — layer divider comments include inline artifact-flow sub-labels annotating producers and consumers | V-02 C-14 scorer note ("`# draft:* produced here | review:* requires draft-spec | flow:*/trace:* require review:*`" as Layer 2 comment) | C-14 = layer labels present; C-17 = prose narration anywhere; C-18 = dependency map structurally embedded inside template YAML comments |
| C-19 | **Pre-template reference tables** — at least two structured tables (arc, gates, or skills) appear before the YAML template body | V-03 compact table format (Evidence Arc table + Gate Criteria table + Skill Catalog table before template) | C-08 = arc phases identifiable in any form; C-14 = arc labeled inside template; C-19 = tabular format, pre-template placement, at least 2 distinct tables |

**Dependency chain for new criteria:**
- C-18 requires C-14 PASS (layer dividers must exist as the base before inline annotations are meaningful)
- C-19 is independent — can pass without C-08, C-14, or any other aspirational criterion

**Scoring arithmetic**: 12 criteria × 0.83 pts = ~10 pts total. A perfect aspirational score (12/12) yields 10.0 pts. Re-scoring V-01 (9/10 at v20 = 9.0 pts) at v21: if C-18 PASS and C-19 FAIL, score is 10/12 × 10 = 8.33 pts.
