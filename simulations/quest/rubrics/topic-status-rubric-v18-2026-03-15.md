# Rubric: `topic:status` -- v18
**Max score: 310** | **Date:** 2026-03-15

---

## Summary of v18 changes

Two new criteria extracted from R20 excellence signals. All five R20 variants
score 300/300 under v17 (R20 is a seeding round, not a differentiation round).
The V-01 vs V-02 delta isolates C-50; the V-01 vs V-03 delta isolates C-51.
V-04 withholds both; V-05 withholds C-51 at P1 (partial). Under v18, V-02, V-03,
and V-05 each score 305 -- three distinct 5-point deltas from V-01 (310).

| ID | Name | Source | Points |
|----|------|--------|--------|
| C-50 | Phase-labeled STOP predicates | R20 V-01/V-03/V-05 vs V-02/V-04 | 5 |
| C-51 | Preamble adversary entry defeat forecast sub-components | R20 V-01/V-02 vs V-03/V-04/V-05 | 5 |

**Structural Tier 17 (10 pts).** Max score: 300 -> 310.

**The two patterns:**

- **C-50** extends the adversary-defeat predicate form at the STOP CONDITION from
  position-numbered form (`(1)`, `(2)`, `(3)`, `(4)`) to phase-labeled form
  (`[P1-ADVERSARY DEFEAT:]`, `[P2-ADVERSARY DEFEAT:]`, `[P3-ADVERSARY DEFEAT:]`),
  closing a three-site label isomorphism: the phase label established at the
  preamble by C-47 (`PN-ADVERSARY:` entries) and operationalized at the execution
  body site by C-42/C-43/C-48 (`ADVERSARY:` blocks) now appears at the STOP gate
  site as a defeat-verification label. Each declared adversary is traceable by
  phase label across all three structural sites -- declaration, operationalization,
  and verification -- without prose scanning at any site. The predicate numbering
  first position (`(1)`) may retain its numbered form as a structural guard; the
  adversary-defeat predicates require phase labels. When C-48 is absent, C-50
  vacuously inherits C-49's vacuous PASS behavior: no P1-ADVERSARY defeat predicate
  is required, so no P1 phase label is required. Requires C-49 and C-47. Extends
  C-49 from positional-predicate to phase-labeled-predicate form at the STOP gate,
  and closes the phase-label isomorphism established by C-47 at the preamble site.

- **C-51** extends each labeled `PN-ADVERSARY:` entry in the preamble ADVERSARY
  CHAIN block (C-47) from two-part form (adversary name + consequence-of-skipping)
  to three-part form by adding a nested `DEFEAT CONDITION:` sub-component carrying
  a condensed statement of the observable defeat property. The preamble entry
  now forecasts, at the point of declaration, not only the adversary identity
  and the consequence of defeat but also what constitutes adversary defeat -- the
  same observable property that C-46/C-48 operationalize in the execution body and
  C-49/C-50 verify at the STOP gate. The preamble becomes a structural forecast:
  a reader scanning the preamble ADVERSARY CHAIN can determine the declared defeat
  criterion for each phase adversary without entering the execution sections. The
  chain-length rule mirrors C-47: when C-48 is present (three-phase adversary
  chain), three PN-ADVERSARY: entries must carry DEFEAT CONDITION: sub-components;
  when C-48 is absent (two-phase chain), two entries suffice -- the absence of
  a P1-ADVERSARY: DEFEAT CONDITION: sub-component is not a C-51 violation when
  no PHASE 1 adversary body has been declared. Requires C-47. Extends C-47 from
  two-part labeled preamble entries to three-part labeled entries with defeat
  forecast sub-components.

**Orthogonality confirmed:** V-02 (C-50 FAIL, C-51 PASS) and V-03 (C-50 PASS,
C-51 FAIL) score identically under v17 (both 300). Under v18: V-01 -> 310;
V-02/V-03/V-05 -> 305; V-04 -> 300.

**V-05 partial outcome resolved:** V-05 carries phase-labeled STOP predicates
(C-50 PASS) and carries P2-ADVERSARY: and P3-ADVERSARY: preamble entries with
DEFEAT CONDITION: sub-components, but P1-ADVERSARY: carries only two-part form
(no DEFEAT CONDITION: forecast). Since C-48 is satisfied in V-05 (three-phase
adversary chain declared), all three preamble entries require DEFEAT CONDITION:
sub-components for C-51 compliance. P1 omission while C-48 is present is a C-51
FAIL. V-05 score under v18: 305 (C-51 FAIL, C-50 PASS).

**Max score: 300 -> 310.** Passing threshold unchanged.

**New implications:**
- C-50 => C-49 (conditional: vacuous when C-48 absent, same as C-49)
- C-50 => C-47
- C-51 => C-47
...
C-50 and C-51 are structurally orthogonal. C-50 concerns STOP gate label form
(extending adversary-defeat predicates from position-numbered form to phase-labeled
form, making each predicate independently locatable by phase label without ordinal
scanning). C-51 concerns preamble adversary entry form (extending each labeled
`PN-ADVERSARY:` entry in the preamble ADVERSARY CHAIN block from two-part form --
adversary name + consequence-of-skipping -- to three-part form by adding a nested
`DEFEAT CONDITION:` sub-component). A variant can carry phase-labeled STOP predicates
without preamble defeat forecasts (C-50 PASS, C-51 FAIL -- V-03). A variant can
carry preamble defeat forecasts without phase-labeled STOP predicates (C-50 FAIL,
C-51 PASS -- V-02). V-04 withholds both. V-01 satisfies both.

C-50 requires C-49 (the P1-ADVERSARY defeat predicate at the STOP gate must exist
before it can carry a phase label; C-49 establishes the three-phase predicate
inventory; C-50 extends that inventory's entry form from positional to labeled)
and C-47 (the `PN-ADVERSARY:` label vocabulary established in the preamble by C-47
is the source of the phase labels that C-50 propagates to the STOP gate -- the
label isomorphism triad is: preamble labels by C-47, execution-body adversary
framing by C-42/C-43/C-48, STOP gate phase labels by C-50). When C-48 is absent
and C-49 is vacuously satisfied, C-50 vacuously inherits that behavior: no P1
predicate exists to label, so the phase-label requirement adds no substance.
C-51 requires C-47 (the labeled preamble entries must be established before their
entry form can be extended to three-part with a DEFEAT CONDITION: sub-component).
The chain-length rule is the same as C-47: when C-48 is present, three entries
require DEFEAT CONDITION: sub-components; when C-48 is absent, two entries suffice
-- the absence of P1-ADVERSARY: DEFEAT CONDITION: is not a C-51 violation when no
PHASE 1 adversary body has been declared.

The structural parallels operate on two independent axes:
- C-50 parallels C-47 in the labeling-propagation dimension: C-47 introduced
  phase labels at the preamble site; C-50 propagates those same phase labels to
  the STOP gate site, closing a three-site label isomorphism (preamble declaration,
  execution-body operationalization, STOP gate verification). C-50 is to C-49 what
  C-47 is to C-45: an entry-form extension that makes each element independently
  addressable by phase label rather than by ordinal position.
- C-51 parallels C-46 in the defeat-vocabulary-propagation dimension: C-46 added
  DEFEAT CONDITION: sub-components to execution-body adversary blocks, making each
  adversary's defeat criterion observable at the site of operationalization; C-51
  adds DEFEAT CONDITION: sub-components to preamble adversary entries, making the
  defeat criterion observable at the site of declaration. The preamble now carries
  a condensed defeat forecast for each declared adversary -- a reader can determine
  both the adversary identity and its defeat criterion from the preamble alone,
  without entering any execution section. C-51 is the preamble-entry parallel of
  C-46: C-46 established DEFEAT CONDITION: at execution sites; C-51 echoes DEFEAT
  CONDITION: at the preamble forecast site.

V-05's partial outcome (P2 and P3 entries with DEFEAT CONDITION:, P1 entry without)
confirms that C-51 evaluates per-entry completeness, not block-level presence. With
C-48 present, all three PN-ADVERSARY: entries are in scope; partial satisfaction
(two of three with DEFEAT CONDITION:) constitutes C-51 FAIL. V-05 scores 305 under
v18 -- confirming that C-51 is the binding criterion, not a sub-criterion.

**Projected R20 scores under v18:**

| Variant | C-50 | C-51 | v18 Score | Notes |
|---------|------|------|-----------|-------|
| V-01 | PASS | PASS | 310 | Full tier-17 baseline |
| V-02 | FAIL | PASS | 305 | Numbered STOP predicates; C-51 PASS |
| V-03 | PASS | FAIL | 305 | Phase-labeled STOP predicates; preamble two-part only |
| V-04 | FAIL | FAIL | 300 | Pure v17 baseline |
| V-05 | PASS | FAIL | 305 | C-50 PASS; P1 preamble entry without DEFEAT CONDITION: |

V-02, V-03, V-05 each score 305 -- three distinct 5-point deltas from V-01 (310),
each isolating a different failure axis. The gap that defines Round 21 is C-50 and
C-51 failing simultaneously -- no R20 variant achieves this without losing 10 pts
(V-04 = 300); the open question is whether a variant can fail both cleanly while
satisfying all prior criteria.
...
| **Structural Tier 17** | **C-50--C-51** | **10 (5 each)** |
| **Total** | **C-01--C-51** | **310** |
...
## Structural Tier 17 (10 pts total) -- new in v18

| ID | Criterion | Weight | Category | Pass Condition |
|----|-----------|--------|----------|----------------|
| C-50 | **Phase-labeled STOP predicates** -- When C-49 (PHASE 1 adversary defeat STOP predicate) and C-47 (preamble adversary chain labeled sub-component entries) are jointly satisfied, this criterion additionally requires that each adversary-defeat predicate in the STOP CONDITION carries a phase label in the form `[PN-ADVERSARY DEFEAT:]` rather than a positional number `(N)`. The phase label is drawn from the same `PN-ADVERSARY` vocabulary established by C-47 at the preamble site, closing the three-site label isomorphism: `PN-ADVERSARY:` entries at the preamble (C-47) -> `ADVERSARY:` blocks in the execution body (C-42/C-43/C-48) -> `[PN-ADVERSARY DEFEAT:]` predicates at the STOP gate (C-50). A STOP CONDITION using numbered form `(1) DISK_SIGNALS populated`, `(2) PHASE 2 adversary defeated`, `(3) PHASE 3 adversary defeated`, `(4) PHASE 1 adversary defeated` satisfies C-49 but fails C-50: the predicates are positionally addressed, not phase-label-addressed. A STOP CONDITION using `[P1-ADVERSARY DEFEAT:] DISK_SIGNALS populated`, `[P2-ADVERSARY DEFEAT:] every signal asserted`, `[P3-ADVERSARY DEFEAT:] consistency guard passed` satisfies C-50: each predicate is addressable by phase label without ordinal scanning. The first predicate position (structural guard) may retain numbered form as a non-adversary gate check; the adversary-defeat predicates require phase labels. The label count tracks the adversary chain length: when C-48 is present (three-phase chain), three phase-labeled adversary-defeat predicates are required; when C-48 is absent and C-49 is vacuously satisfied, C-50 vacuously inherits that behavior -- no P1 phase-labeled predicate is required. Requires C-49 (conditional: vacuous when C-48 absent) and C-47. Extends C-49 from positional-predicate to phase-labeled-predicate form, closing the phase-label isomorphism triad across all three structural sites. | 5 | structure | Each adversary-defeat predicate in the STOP CONDITION carries a `[PN-ADVERSARY DEFEAT:]` phase label (P1, P2, P3 as appropriate) rather than a positional number. STOP predicates in numbered form `(N)` fail this criterion even when the predicate content is correct. When C-48 is absent and C-49 is vacuously satisfied, C-50 is vacuously satisfied under the same conditions. C-49 and C-47 pass required. |
| C-51 | **Preamble adversary entry defeat forecast sub-components** -- When C-47 (preamble adversary chain labeled sub-component entries) is satisfied, this criterion additionally requires that each labeled `PN-ADVERSARY:` entry in the preamble ADVERSARY CHAIN block carries a nested `DEFEAT CONDITION:` sub-component with a condensed statement of the observable defeat property for that phase adversary. C-47 establishes two-part labeled entries (adversary name + consequence-of-skipping per entry: `P2-ADVERSARY: {name} -- default when skipped: {consequence}`); C-51 extends each entry to three-part form by adding a nested `DEFEAT CONDITION: {condensed observable property}` sub-component at each entry position. The preamble DEFEAT CONDITION: sub-component is a condensed forecast of the DEFEAT CONDITION: operationalized in the execution body by C-46 and C-48: it names the same observable property in abbreviated form, making the defeat criterion available at the point of adversary declaration without entering the execution sections. This completes the preamble's structural forecast role: a reader can determine, from the preamble alone, the adversary identity, the consequence of skipping, and the observable property that constitutes defeat -- for each execution-phase adversary. The chain-length rule mirrors C-47: when C-48 is present (three-phase adversary chain), three PN-ADVERSARY: entries must each carry a DEFEAT CONDITION: sub-component; when C-48 is absent (two-phase chain), two entries suffice -- the absence of a P1-ADVERSARY: DEFEAT CONDITION: sub-component is not a C-51 violation when no PHASE 1 adversary body has been declared. Per-entry completeness is required: two of three entries with DEFEAT CONDITION: sub-components while C-48 is present constitutes C-51 FAIL. Canonical three-part entry form: `P2-ADVERSARY: inertia toward evidence-blind commits -- default when skipped: each unverified signal ships without assertion. DEFEAT CONDITION: every planned signal receives VERIFIED or UNVERIFIED disposition.` Requires C-47. Extends C-47 from two-part labeled preamble entries to three-part labeled entries with defeat forecast sub-components, making the preamble a complete structural forecast for the adversary chain. | 5 | structure | Every in-scope `PN-ADVERSARY:` labeled entry in the preamble ADVERSARY CHAIN block carries a `DEFEAT CONDITION:` sub-component (or equivalent named defeat-condition sub-label) with a condensed observable defeat property statement. An entry carrying only adversary name + consequence-of-skipping without a DEFEAT CONDITION: sub-component fails. When C-48 is present, all three entries (P1, P2, P3) must carry DEFEAT CONDITION:; partial satisfaction (two of three entries) constitutes FAIL. C-47 pass required. |
...
-- C-50: conditional prerequisite C-49 (non-vacuous only; vacuous when C-48 absent); C-50 => C-47 (hard)
C-51 => C-47
```
