Written to `simulations/quest/rubrics/discover-feasibility-alt-rubric-v3-2026-03-18.md`.
...
laration and C-12 semantic back-reference are by-construction reachable only when the template body places STRATEGY before ARCHITECT. A preamble-template conflict (C-13 FAIL) degrades C-10 from by-construction to probabilistic, cascading to C-11 and C-12: forward-declarations have no structural guarantee of preceding ARCHITECT rows, and back-references have no guaranteed target to reference. A cascade-safe template embeds ordering in the body such that C-10, C-11, and C-12 each fail or pass independently. Distinct from C-13 (which checks agreement) and C-14 (which checks body independence) -- C-15 checks the transitivity of the guarantee chain.
...
- **N/A handling**: C-05, C-07, and C-09 pass as N/A when no focus is active. C-10, C-11, C-12, C-14, and C-15 pass N/A if the output contains no STRATEGY section. C-13 passes N/A if no ordering directive appears in the preamble.
