`listen-adoption-rubric-v17-2026-03-15.md` written. Three new criteria extracted from R16 excellence signals:

---

**C-47 -- PART 4 champion network rows use SLOT-KEY: prefix at row level** (V-02 Axis B)
Fourth application of the SLOT-KEY: re-assertion pattern (TABLE 3 -> C-38, TABLE 1 -> C-43,
PART 3 -> C-46, PART 4 -> C-47). PART 4 header declares a typed row-key contract for champion
entries; each champion row carries `SLOT-KEY: [Role] -- champion entry` prefix, re-asserting the
double-anchor type contract (archetype bridge + inertia ID) at row generation time independently
of the header. Gate I audit language references "SLOT-KEY: champion entries" explicitly.
Dependency: C-04, C-14.

**C-48 -- PART 2 expansion subsections carry displacement state field per slot** (V-01 Axis A)
Extends C-45's phase-close displacement ledger into expansion slot depth. Each CHASM-A EXPANSION
/ CHASM-B EXPANSION / CHASM-C EXPANSION subsection carries a mandatory `Incumbent position at
this chasm element: [explicit displacement status at this element]` field. C-45 accounts for
incumbent displacement status at phase-close level (six footers per document); C-48 accounts for
it at the expansion-slot level (three fields per PART 2 expansion), making the displacement arc
traceable within the chasm itself, not only across phase boundaries. Dependency: C-40, C-45.

**C-49 -- SECTION A carries DOWNSTREAM CITATION CONTRACT mapping each inertia ID to required
citation sinks** (V-03 Axis C)
After SECTION A's inertia table, a DOWNSTREAM CITATION CONTRACT block explicitly maps each I-0X
to the downstream structural locations where that ID must appear: phase ledgers (C-45), PART 2
expansion slots (C-42/C-48), PART 3 churn rows (C-46), PART 4 champion rows (C-47), PART 5
intervention rows (C-44). Gate H audits against the declared contract rows rather than performing
a free-form scan, making the audit condition a named contract violation rather than a coverage
judgement. The contract makes C-13's downstream backbone requirement verifiable at source: the
model sees the required citation sinks at SECTION A generation time and cannot claim completion
without satisfying the declared row for each I-0X. Dependency: C-11, C-13.

**Max: 280 -> 295. Golden threshold (80%): 236 pts. Paradox ceiling: 290/295.**

Note: V-03 Axis C deferred in R15 is now fully resolved. All three R16 axes extracted as
C-47 (Axis B), C-48 (Axis A), C-49 (Axis C). No further deferrals pending.
...
**Max composite: 280 -> 295. Golden threshold (80%): 236 pts.**
