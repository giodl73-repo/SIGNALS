Written to `simulations/quest/rubrics/review-design-rubric-v12-2026-03-14.md`.

---

**Five new criteria extracted from R12 scorecard:**

**C-36 | BLOCK 2 Section Column Leftmost Placement for Enforcement Salience**
- Pattern class: structural salience via column position (orthogonal to C-10 presence and C-35 halt)
- V-02 called out twice: "section-first column order makes F-27 enforcement maximally salient" (C-07) and "leftmost-column position makes empty Section cell impossible to miss visually" (C-35)

**C-37 | Named Halt Conditions Include Inline Corrective Action Specification**
- Pattern class: halt completeness (trigger + resolution) — extends C-14 beyond trigger declaration
- V-04 called out across 5 criteria (C-09, C-17, C-24, C-34, C-35): every F-ID specifies what specifically to do to resolve it, not just what fires it

**C-38 | BLOCK 0 Amendment Cost Column**
- Pattern class: additive depth column converting BLOCK 0 from signal inventory to prioritized risk register
- V-05 at C-21: "BLOCK 0 extended to 4 columns with Amendment Cost; F-13/F-18 gates still present"

**C-39 | BLOCK 5 Priority Rank Column for Amendment Sequencing**
- Pattern class: additive ordering column beyond C-20's 4-column base schema
- V-03/V-05 at C-20: "5-column table (Priority Rank added); column count non-binding when base schema intact"

**C-40 | BLOCK 5 Inertia Framing as Named Design Constraint**
- Pattern class: reactive halt (F-05) elevated to proactive design principle — same elevation C-14→C-15 applied to roster commitment
- V-05 at C-06: "inertia framing explicitly bars replacement instructions"

**Formula:** denominator 28 → 33. Each aspirational criterion ~0.303 pts. Ceiling 100 preserved.
dments block others; some sections interact).
C-19/F-12 ensures all P1s are present; C-39 adds explicit ordering. Confirmed by V-03 and V-05
in C-20 ("5-column table (Priority Rank added); additive extension satisfies structural table
form; column count non-binding when base schema is intact").

**C-40 | BLOCK 5 Inertia Framing as Named Design Constraint**
A scope constraint elevation: converts F-05's reactive halt (fires when full re-review is
instructed) into a proactive design principle (named constraint declares preservation before
generation begins). F-05 fires defensively; C-40 requires BLOCK 5 to state an explicit inertia
principle -- only identified sections change, all others are preserved -- before amend rows are
generated. Confirmed by V-05 in C-06 ("inertia framing explicitly bars replacement instructions").

**Formula update:** denominator 28 -> 33. Each aspirational criterion ~0.303 pts. Ceiling 100
preserved.
...
Note: aspirational denominator changed from 28 to 33 in v12 (added C-36, C-37, C-38, C-39,
C-40). Each aspirational criterion contributes ~0.303 points at full pass.
