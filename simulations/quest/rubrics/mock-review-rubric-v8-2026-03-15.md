Reading the scorecard's excellence signals and the `new_patterns` JSON carefully before writing the new criteria.

Two new patterns from the JSON:
1. `role-sequence-gate-as-contamination-control` — Architect-first ordering with a named cross-step guard that fires on a specific verdict value
2. `triad-DO-NOT-coverage` — all three auto-rules carry DO NOT statements as an enumerable set, enabling mechanical completeness verification

Patterns 3 and 4 from the prose list (entity-naming SQ grammar, artifact state bridge) are already captured in C-15 and C-14/C-24. Not duplicating.

Aspirational denominator: 17 → 19.

---

```markdown
# Quest Rubric — /quest:mock-review
**Version**: v8 | **Date**: 2026-03-15
...
| C-14 SQ answer citation in verdict traceability | | |
...
- **C-26** — Role-sequence gate as contamination control. Distinct from C-17 (which guards auto-flagged namespaces at phase entry): C-26 requires a named inter-role guard that fires on a specific architect verdict value and suppresses the PM step. The distinguishing structural signal is the verdict-value trigger (`architect-verdict = CONTRADICTS-ARCHITECTURE`), not just role ordering or phase separation.

- **C-27** — Triad DO NOT coverage. Distinct from C-11 (which requires forbidden-output phrasing to be present): C-27 requires per-rule enumeration — three individually labeled DO NOT statements — so the set can be mechanically verified for completeness. A blanket prohibition passes C-11 but fails C-27.

- **Scoring formula denominator**: 17 → 19.

- **Scoring table**: C-26 and C-27 rows added.
