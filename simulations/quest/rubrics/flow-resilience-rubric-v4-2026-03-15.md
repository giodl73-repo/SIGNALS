Done. The v4 rubric is at `simulations/quest/rubrics/flow-resilience-rubric-v4-2026-03-15.md`.

**Three new aspirational criteria extracted from Round 3:**

**C-16 — Named Gate-State Vocabulary** (from V-02 + V-03)
Fixed disposition vocabulary (Include / BARRED / Argued-Impossible); explicit OPEN/CLOSED binary on every gate section visible without reading prose. "Flagged is not a disposition." A gate section with no named state is always OPEN by definition.

**C-17 — Permanently Barred Entries as Named Coverage Gaps** (from V-02)
BARRED entries that reach triage-gate close with an unresolved confidence basis must appear as named coverage gaps in the accountability register. Silently dropping them makes "analyzed, excluded" and "not analyzed" indistinguishable.

**C-18 — Explicit Phase Entry and Exit Conditions** (from V-03)
Each phase states (1) entry condition — which prior gate IDs must be CLOSED — and (2) exit condition — what makes this gate CLOSED. Silence does not mean CLOSED; an explicit CLOSED confirmation is required. Prose-position ordering is unverifiable.

**C-15 also tightened:** pass condition now requires a named `DS Primitive cited:` template field with inline VALID/INVALID counter-examples, closing the gap that let V-01 earn only PARTIAL by citing an "architectural constraint" that wasn't a named DS primitive.

**Aspirational band:** cap stays at 10; uncapped max rises from 14 to 20 (10 criteria × PASS=2).
e, or inconsistent;
  (4) recovery path -- how the system returns to a healthy state.
...
Uncapped maximum across 10 criteria is 20; the cap rewards breadth while holding the
...
- **Text**: Discovery entries that remain BARRED after the confidence triage gate closes --
  ...
  (Signal: V-02 C-11 -- "barred entries either resolves to Include or remains barred and is
...
| Aspirational | 10% | C-09 through C-18 | PASS=2, PARTIAL=1, FAIL=0; capped at 10 |
...
- **C-15 strengthened** -- Pass condition now explicitly requires a named "DS Primitive
  ...
  signal from Round 3.
- **Aspirational cap unchanged** at 10; uncapped maximum rises from 14 to 20 with three new
