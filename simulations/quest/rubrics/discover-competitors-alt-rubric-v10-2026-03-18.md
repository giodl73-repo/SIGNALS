The file on disk is a different rubric. I'll work from the v9 content supplied in the prompt. Let me extract the new excellence patterns from the scorecard.

**New patterns from Round 9:**

- **V-01** ("REQUIRED OUTPUTS blocks replace completion conditions"): Completion conditions are *fully removed* — REQUIRED OUTPUTS is the exclusive output spec per gate. C-26 only requires a dedicated block; V-01 goes further by eliminating the completion condition entirely. → **C-27**

- **V-02** ("REQUIRED OUTPUTS as native markdown tables"): The REQUIRED OUTPUTS block is a native pipe-and-hyphen table, not a labeled list or prose block — paralleling the standard C-24 established for the gate manifest. → **C-28**

- **V-02 TOKEN column**: The REQUIRED OUTPUTS table has an explicit `TOKEN required?` column per gate, making TOKEN's required status machine-verifiable from the block alone without reading body prose. → **C-29**

Dependency chain: C-27 ← C-26; C-28 ← C-26; C-29 ← C-28.
Aspirational pool: 18 → 21; each worth `10/21 ≈ 0.476 pts`.

---

```markdown
# discover-competitors-alt Rubric — v10
...
## Aspirational Criteria (weight = 10 points total, 0.476 points each)
...
| Aspirational | C-09 -- C-29 | 0.476 | 10 |
| **Max composite** | | | **100** |
...
| ID | Source variation | Pattern |
|----|-----------------|---------|
| C-27 | V-01 ("blocks replace completion conditions") | Completion conditions fully removed — REQUIRED OUTPUTS is the sole output spec per gate, no redundancy permitted |
| C-28 | V-02 ("REQUIRED OUTPUTS as native markdown tables") | REQUIRED OUTPUTS block rendered as a native pipe table per gate, not a labeled list; parallels C-24 for the manifest |
| C-29 | V-02 (`TOKEN required?` column evidence) | REQUIRED OUTPUTS table has an explicit TOKEN-required column per gate, making TOKEN's required status machine-verifiable from the block alone |

Dependency chain: C-27 ← C-26; C-28 ← C-26; C-29 ← C-28. Aspirational pool: 18 → 21, each worth `10/21 ≈ 0.476 pts`.
