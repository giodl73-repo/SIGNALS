```markdown
# Rubric: trace-deployment (v7)

**Skill**: `trace:deployment`
**Version**: 7 (updated from R7 scorecard — 2 excellence patterns promoted to C-23/C-24; ceiling updated to 150/170)
**Date**: 2026-03-14
**Criteria**: 24 across 4 tiers | **Total**: 170 pts
...
## Aspirational Criteria (80 pts — structural excellence)
...
C-23 and C-24 promoted from R7 excellence patterns.
...
| C-24 | **C-21 and C-22 jointly satisfied in a single variation** | behavior | C-21 and C-22 both pass in the same variation — colloquial register AND bare field labels — and the composite reaches 150/170, confirming that the two criteria are orthogonal template-path axes whose joint satisfaction is additive. The distinguishing condition: satisfying C-21 alone (colloquial + non-bare) or C-22 alone (formal + bare) each add 5 pts to the C-14/C-16/C-18 base; joint satisfaction adds 10 pts. Variations that satisfy only one of the two axes do not fail C-24, but also do not pass — simultaneous satisfaction under a single variation is the requirement. *(Pattern: c21-c22-orthogonal-template-path-ceiling-150)* |
...
| Aspirational | C-09–C-24 | 80 |
| **Total** | **24** | **170** |

**Hard ceiling architecture (updated for v7):**

- **Prose path**: C-15/C-17/C-19/C-20/C-23 reachable; C-14/C-16/C-18/C-21/C-22/C-24 blocked. Ceiling: 135/170.
- **Template path (formal, non-bare)**: C-14/C-16/C-18/C-21 reachable; C-15/C-17/C-19/C-20/C-22/C-23/C-24 blocked. Ceiling: 145/170.
- **Template path (formal, bare)**: adds C-22. Ceiling: 150/170.
- **Template path (colloquial, non-bare)**: adds C-21. Ceiling: 150/170.
- **Template path (colloquial, bare)**: adds C-21 + C-22 + C-24. Ceiling: **150/170** — absolute maximum under current rubric.

Note: C-23 is reachable on the prose path (V-05 confirmed) and on any template variation that places inertia framing in the role block with a domain-vocabulary disqualifier. C-24 requires template path (colloquial + bare) — not reachable on prose path.
```
...
**Point recount**: The 16 aspirational criteria (C-09–C-24) at 5 pts each = 80 pts. Prior v6 had 14 aspirational criteria at 5 pts each = 70 pts. Total shifts from 160 to 170.

**Ceiling correction**: The R7 scorecard reported 140/160 as the ceiling. Under v7 scoring, V-04 (the colloquial + bare variation) now maps to 150/170 — same variation, same criteria passing, just restated against the new denominator. The ceiling note in C-24 reflects this.
