...
**C-25** captures the double-guard pattern from Signal 1. C-23 already requires a FORBIDDEN at each boundary; C-25 tightens the requirement to *two orthogonal* FORBIDDENs — one outbound at the gate, one inbound at the consuming phase. The distinction is that a prompt satisfying C-23 with a single well-placed FORBIDDEN still fails C-25.

**C-26** captures the structural unification pattern from Signal 2. It is not enough to have record blocks, gate contracts, and ordering guards coexist in a prompt — C-26 requires they be unified in one construct so that each property is derivable from the record block definition alone, without cross-referencing. This is testable: can a reader who sees only the `=== RECORD ===` block deduce the gate schema, the ordering anchor, and the materialization checkpoint?

Denominator: 16 → 18. Formula denominator updated accordingly.
