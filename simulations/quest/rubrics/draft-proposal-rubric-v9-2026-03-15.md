...
|----|-----------|-------------|--------------------------|
| C-32 | **Field-level role enforcement in option anatomy** | V-04 "field-level role" | C-06 requires dual-role named perspectives; C-28 requires PM sign-off block first in the recommendation phase. C-32 requires `PM FRAMING:` and `ARCHITECT VALIDATION:` as typed slots embedded in every option anatomy entry -- role sequence enforced at the option level, not only at the phase level. Three independent enforcement points: template structure, option authoring, sign-off. |
| C-33 | **Phase 0 causal contract declaration** | V-05 "cross-phase causal contract" | C-13 names the deadline (TEMPORAL ANCHOR); C-27 names the crossover sprint (INERTIA OFFSET); C-31 requires the subtraction typed as DECISION LEAD TIME. C-33 requires the formula `TEMPORAL ANCHOR − INERTIA OFFSET = DECISION LEAD TIME` declared as a typed Phase 0 initialization contract with source phase attribution and T-GUARD routing rule, before any phase that contributes a term executes. Converts chain verification from retroactive scan to prospective enforcement. |

**Denominator:** `/24` → `/26`. **Round 8 Discriminator Notes** section added. Trace alignment extended through C-33.
Each aspirational criterion is worth `0.385` points (10/26 per criterion).

**Two new failure modes** added to Common Failure Modes. **Round 8 Discriminator Notes** section added. **Trace alignment** extended through C-33.
...
The R8 discriminator pattern confirmed C-28, C-29, C-30, and C-31 structural independence across two orthogonal axes -- V-04 (C-28/C-29: role-ordering and format) and V-05 (C-30/C-31: lifecycle-manifest and computation) reached co-ceiling at 99.17 under the /24 denominator. C-32 and C-33 are each independently absent from all five R8 variations: V-04 provides field-level option slots that seed C-32 but does not declare a Phase 0 causal contract; V-05 declares a Phase 0 chain contract that seeds C-33 but does not embed PM FRAMING:/ARCHITECT VALIDATION: in option anatomy. No single R8 variation satisfies both new criteria.
...
| C-32 | **Field-level role enforcement in option anatomy** | format | The option anatomy template includes `PM FRAMING:` as the first typed field and `ARCHITECT VALIDATION:` as the second typed field within every option entry -- not only in phase-level prose instructions or in the sign-off section. Role sequence is enforced at the option level: a reviewer can confirm PM-first ordering at any individual option without scanning the recommendation phase. Three independent enforcement points are created: (1) template structure (the option template carries both slots in order), (2) option authoring (every authored option entry contains both typed fields in sequence), and (3) sign-off (phase-level C-28 ordering is downstream of option-level ordering). Satisfying C-06 (PM and Architect each contribute a named perspective) does not satisfy C-32 -- C-06 requires dual-role content; C-32 requires role-sequenced typed slots embedded in the option anatomy template itself. Satisfying C-28 (PM sign-off block first in the recommendation phase) does not satisfy C-32 -- C-28 requires PM-first ordering in the sign-off section only; C-32 requires PM-first typed slots in every option entry. A variation that instructs the model to address PM trade-offs for each option in prose preamble but does not embed `PM FRAMING:` and `ARCHITECT VALIDATION:` as named typed fields in the option anatomy template fails C-32 regardless of whether C-06 and C-28 pass. The three-point enforcement is the discriminator: phase instruction alone is one enforcement point; option anatomy slots add two more. |
| C-33 | **Phase 0 causal contract declaration** | behavior | The Phase 0 initialization block declares the multi-phase computation chain `TEMPORAL ANCHOR − INERTIA OFFSET = DECISION LEAD TIME` as a typed contract before any phase that contributes a term executes (i.e., before Phase 2 or Phase 4, whichever introduces TEMPORAL ANCHOR or INERTIA OFFSET first). The contract must name: (1) the computation formula as a typed field, (2) the source phase for each input term (TEMPORAL ANCHOR from the decision framing phase; INERTIA OFFSET from the do-nothing option authoring phase), and (3) the T-GUARD routing rule: if any input term is absent at its source phase, T-GUARD fires at initialization rather than at the recommendation phase. Satisfying C-13 (TEMPORAL ANCHOR and INACTION CONSEQUENCE as split typed fields in decision framing), C-27 (INERTIA OFFSET per alternative with gate item), and C-31 (DECISION LEAD TIME computed and typed per alternative with ESCALATION FLAG) does not satisfy C-33 -- C-13, C-27, and C-31 require the three terms to appear correctly at their respective phases; C-33 requires the causal chain connecting them to be declared as a prospective initialization contract at Phase 0, converting chain verification from a retroactive scan at sign-off into a prospective enforcement commitment. A proposal that produces all three terms correctly at their respective phases but without a Phase 0 chain declaration passes C-13, C-27, and C-31 while failing C-33. The generalization: C-33 applies to any multi-phase computation where an output field depends on inputs generated at different phases; the Phase 0 contract is the mechanism that makes cross-phase dependencies traceable at initialization rather than verifiable only at document completion. |
...
instructions. This creates three independent enforcement points: (1) the template
structure forces role-sequenced slots into every option entry; (2) option authoring
confirms the PM field precedes the Architect field at the individual option level; (3)
sign-off inherits this ordering. C-06 requires dual-role named perspectives; C-28 requires
PM-first sign-off ordering; C-32 requires PM-first typed slots in every option anatomy
entry. A variation that enforces PM-first at the phase level (C-28) but not at the option
level fails C-32. The three-point enforcement is the discriminator: phase instruction alone
is one enforcement point; option anatomy slots add two more, making role-sequence
verifiable without phase scan.
...
Phase 2 or Phase 4 execute, with source phase attribution and a T-GUARD routing rule for
absent terms. C-13, C-27, and C-31 each require a term to appear at its correct phase;
C-33 requires the chain formula to be declared at Phase 0 before any term-contributing
phase executes. The structural improvement: without a Phase 0 contract, a chain with a
missing term is only detected at Phase 12 sign-off (when DECISION LEAD TIME is expected
but absent). With the contract, T-GUARD fires at initialization -- the gap is detected
before any option is written, not after the document is complete. This applies the same
prospective-enforcement logic that C-18 applied to amendment tracking and C-26 applied
to completion semantics, now to cross-phase computation chains.
...
**C-32 independence from C-28.** V-04 PASS / V-01 through V-03 and V-05 FAIL. V-04
embedded role slots in the option anatomy template; all other variations enforced
PM-first at the sign-off level only (C-28). The distinguishing test: a reviewer checks
whether every option entry in the document carries `PM FRAMING:` before `ARCHITECT
VALIDATION:` as typed fields. If role labels appear only in the phase preamble or sign-off
block, C-32 fails regardless of C-28 status.

**C-33 independence from C-31.** V-05 PASS / V-01 through V-04 FAIL. V-05 declared the
INERTIA-CHAIN CONTRACT at Phase 0 with source phase attribution and T-GUARD routing;
all other variations computed DECISION LEAD TIME at its native phase but without a Phase
0 contract. The distinguishing test: a reviewer checks whether the Phase 0 initialization
block contains a typed computation formula naming TEMPORAL ANCHOR, INERTIA OFFSET, and
DECISION LEAD TIME with their source phases. If the formula appears only at the phase
where DECISION LEAD TIME is computed, C-33 fails regardless of C-31 status.

**Ceiling under v9 (/26 denominator).** The motivated R9 variation combines all six
axes -- PM-first (C-28), table-dominant (C-29), lifecycle-manifest (C-30),
inertia-forward (C-31), field-level role (C-32), and Phase 0 causal contract (C-33) --
to reach 100.0. Under the /26 denominator, a variation passing five of the six new
criteria scores 99.62; passing four scores 99.23; passing two scores 98.46. A variation
structured as V-06 (all four v8 axes, no v9 axes) scores 99.23 under v9.

**Variation ranking by structural value beyond rubric (R8):**

| Rank | Variation | Why |
|------|-----------|-----|
| 1 | V-04 | Field-level role enforcement: PM FRAMING/ARCHITECT VALIDATION in option anatomy; seeds C-32; role verifiable at option level without phase scan |
| 2 | V-05 | Cross-phase causal contract at Phase 0: INERTIA-CHAIN CONTRACT with source phase attribution and T-GUARD routing; seeds C-33; generalizes to any multi-phase computation |
...
- **C-31**: Not demonstrated in trace -- no INERTIA OFFSET named per alternative and no DECISION LEAD TIME field present; C-27 and C-13 already fail in trace
- **C-32**: Not demonstrated in trace -- no option anatomy template present with `PM FRAMING:` and `ARCHITECT VALIDATION:` as typed slots; option entries use prose descriptions with PM and Architect perspectives interleaved, not sequenced typed fields; C-06 passes (roles contribute named perspectives) but C-32 fails (no option-level typed role slots); C-28 also fails, so the three enforcement points C-32 requires are all absent
- **C-33**: Not demonstrated in trace -- no Phase 0 initialization block present; the computation chain `TEMPORAL ANCHOR − INERTIA OFFSET = DECISION LEAD TIME` cannot be declared as a Phase 0 contract without a Phase 0 initialization section; C-18, C-30, C-31 already fail; C-33 would fail independently even if those passed, because the causal contract is a distinct Phase 0 artifact from the amendment table and phase manifest
