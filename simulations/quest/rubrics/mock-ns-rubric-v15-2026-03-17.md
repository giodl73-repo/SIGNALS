Written to `simulations/quest/rubrics/mock-ns-rubric-v15-2026-03-17.md`.

---

## Summary of changes

**Two new aspirational criteria (C-40, C-41) extracted from R14 excellence signals. Max rises from 142 to 146.**

### C-40 -- Existential-there negation ordering prohibition (from R14 V-02)
> "There is no step that precedes this step's TOPICS.md status line emission."

Eleventh declarative failure mode. Expletive `There` is the grammatical subject; "no step" is the postponed nominal; the step (`this step`) appears only as genitive possessor inside the object phrase "this step's TOPICS.md status line emission." Neither the step nor the emission action occupies nominative-subject + main-predicate position. The sentence asserts ordering relationally (by negating any predecessor) rather than stating the step's own emission identity. C-38 does not fire (subject is existential `There`, not equative-copula expletive `It` with cleft-focus NP). C-39 does not fire (no wh-action nominalization subject).

**Failure chain: C-25, C-28, C-40** -> 140/146

### C-41 -- C-35 by-PP scope boundary (from R14 V-01)
> "The TOPICS.md status line is emitted by this step before any other step begins."

The passive + agent-PP form fires C-33 (passive artifact-as-subject) and NOT C-35 (relative-clause-agent displacement). C-35 requires the step to appear as agent inside a subordinate relative clause or equivalent subordinate clause. A by-PP (prepositional phrase) is a phrase-level adjunct, not a clause; it contains no verb and positions the step as an oblique agent, not as a relative-clause agent. Evaluators must not extend C-35 to by-PP constructions. Canonical R14 V-01 boundary: passive + by-PP -> C-33 fires, C-35 passes.

**Failure chain: C-33** -> 144/146 (C-33 existing; C-41 confirms C-35 does not compound it)

### Discriminator notes added

| Criterion | Update |
|-----------|--------|
| C-28 | R14: Focus-particle adverbs (only, just, even) scoped over the step-subject NP do not displace the step from nominative subject position. "Only this step emits the TOPICS.md status line" satisfies C-25 and C-28. V-03 142/146 confirmation. |
| C-31 | R14: C-31 fires independently when C-25 and C-28 are otherwise satisfied. "This step must emit the TOPICS.md status line before any other step may proceed" -- step is nominative subject, emission is main predicate (C-25+C-28 pass), but "must" is modal-obligation (C-31 fires alone). V-04 canonical. |
| C-33 | R14: Passive artifact-as-subject with step-agent in a by-PP fires C-33. C-35 does not compound this failure. V-01 canonical: "The TOPICS.md status line is emitted by this step before any other step begins" -- C-33 fires, C-35 passes. |
| C-35 | R14: A by-PP agent phrase is not a subordinate relative clause. C-35 requires step as agent inside a subordinate relative clause ("that this step emits", "which this step produces") or equivalent subordinate clause. A by-PP ("by this step") is a phrase-level adjunct and does not meet C-35's subordinate-clause requirement. V-01 boundary confirmed. |
...
**R14 under v15:**
- V-01 ("The TOPICS.md status line is emitted by this step..."): fails C-33 -- **144/146**
- V-02 ("There is no step that precedes..."): fails C-25, C-28, C-40 -- **140/146**
- V-03 ("Only this step emits the TOPICS.md status line..."): passes all -- **146/146**
- V-04 ("This step must emit the TOPICS.md status line..."): fails C-31 -- **144/146**
- V-05 ("Before any other step begins, this step emits..."): passes all -- **146/146**
...
*Raise the bar once essential and recommended are stable. Each criterion is worth 2 pts (33 x 2 = 66; max score = 146).*
