Done. `draft-proposal-rubric-v7-2026-03-14.md` is written (45,400 bytes).

---

**What changed from v6:**

**Three new aspirational criteria (C-25, C-26, C-27):**

| ID | Criterion | Parent | What it enforces |
|----|-----------|--------|-----------------|
| C-25 | **Sentinel-first trigger ordering** | C-23 | T-GUARD must be position 1 in the trigger table -- not the last backstop but the first route. Verifiable by reading a single cell. |
| C-26 | **COMPLETION STATUS as Phase 0 typed initialization slot** | C-24 | Completion declaration must exist as a live `PENDING` slot from document creation, not a Phase 13 prose instruction that can be silently skipped. Mirrors how C-18 improved C-09. |
| C-27 | **INERTIA COST / INERTIA OFFSET quantification** | C-16/C-20 | Do-nothing option carries a numeric per-sprint P*I cost (INERTIA COST); each alternative names the sprint crossover point where implementation cost equals inertia cost (INERTIA OFFSET); PREREQUISITE GATE extended with inertia-axis binary item. Converts the R4 "evaluative lens" into a machine-checkable structural requirement. |

**Denominator:** `/17` → `/20`. Each new criterion is worth `0.5` points (10/20 = 0.5 per criterion).

**Three new failure modes** added to Common Failure Modes. **Round 6 Discriminator Notes** section added. **Trace alignment** extended through C-27.
iewer confirms or falsifies it by reading the first trigger entry -- no table scan required.
