Done. Here's what was extracted and why:

---

**C-23 — Pre-Barrier Block Dual-Dimension Completeness** (Signal 1)

The ceiling in R5 required *both* C-21 and C-22 simultaneously — neither substitutes for the other. V-02 had the labeled pair (C-22) but a neutral container name (fails C-21). V-01 had the positional name (C-21) but removed the labels (fails C-22). C-23 captures the combinatorial requirement: the structural unit is complete only when both dimensions are verifiable from the block alone. Pass: C-21 AND C-22 from the same block.

**C-24 — Cross-Generation Labeled-Pair Coverage** (Signal 2)

The R5 scorecard explicitly noted that `Independence:` / `Scope extension:` labels earn 6 points across two rubric generations (C-19 from R4, C-22 from R5). C-24 makes that cross-generation coverage an explicit criterion: the same structural choice — labeled co-located directives in the pre-barrier position — must satisfy both. V-01's label removal cascades as a two-generation failure; V-02 retains labels and can pass C-24 despite failing C-21. Pass: C-19 AND C-22 both satisfied.

**Signals 3 and 4 were not extracted** — register orthogonality (V-04/V-05) and section-order invariance (V-03) confirm existing criteria are register/sequence-agnostic, but they don't reveal new capturable structure. The notes section acknowledges both confirmations without creating spurious criteria.

**Updated score table:** 136 → 142, max band 6 pts for Aspirational v6.
h C-19 and C-22. A labeled
pair appearing after evaluative output satisfies C-19 but not C-22 and fails C-24.
Removing labels fails both C-19 and C-22 and fails C-24. Pre-barrier placement with
distinct labels is the only path.

---

**Scoring implications** (no point changes to existing criteria):

- C-23 requires both C-21 and C-22. V-01 (C-22 fail) and V-02 (C-21 fail) cannot reach
  C-23 regardless of other structural choices.
- C-24 requires both C-19 and C-22. Removing labels (V-01 pattern) causes C-19 regression
  that cancels any C-21 gain and also blocks C-24. A neutral container name (V-02 pattern)
  does not affect C-24 — C-21 is not a dependency.
- Register and section ordering are structurally inert for C-23 and C-24. Formal, technical,
  or conversational phrasing reaches the same outcome when container naming and label
  presence requirements are met. Analysis section reordering does not affect scoring when
  the pre-barrier block is structurally anchored as the first element of Round 2.
...
## Aspirational Criteria — v6 (weight: 6)

### C-23 — Pre-Barrier Block Dual-Dimension Completeness
- **Category**: structure
- **Weight**: aspirational (3)
- **Text**: The pre-barrier block achieves both orthogonal structural properties
  simultaneously in the same section: the container name encodes positional role (C-21)
  AND the block carries a labeled independence + scope extension pair (C-22). Neither
  property substitutes for the other — C-21 makes the block's position verifiable from
  its name; C-22 makes its mechanisms independently verifiable from labels. The structural
  unit is complete only when both dimensions are present.
- **Pass condition**: Passes C-21 AND C-22 from the same pre-barrier block. Passing C-21
  without C-22, or C-22 without C-21, does not satisfy this criterion. Both axes required.

### C-24 — Cross-Generation Labeled-Pair Coverage
- **Category**: structure
- **Weight**: aspirational (3)
- **Text**: The labeled pair (`Independence:` / `Scope extension:`) simultaneously
  satisfies both the R4 co-location criterion (C-19: distinct labels in a shared block)
  and the R5 pre-barrier criterion (C-22: labeled pair before evaluative output). One
  structural choice — adding distinct named labels to co-located directives in the pre-
  barrier position — earns points across two rubric generations.
- **Pass condition**: Passes both C-19 and C-22. A labeled pair appearing after evaluative
  output satisfies C-19 but not C-22 and fails C-24. Removing labels fails both C-19 and
  C-22 and fails C-24. Pre-barrier placement with distinct labels is the only path.
