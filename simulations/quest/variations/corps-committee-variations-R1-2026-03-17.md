...
Load participant roles from `.roles/` for this committee type. If no charter exists,
...
Determine and print:
  ...
  COMMITTEE TYPE: [ROB / shiproom / arch-board]
  AGENDA ITEM: [from invocation]
  CHARTER SOURCE: [`.roles/` path or "Signal defaults"]
  DATE: [today]
...
If a charter was loaded from `.roles/`, identify two constraints it imposes
...
**Participants** — read from `.roles/` for this committee type. If no charter
...
If you loaded a charter from `.roles/`, name at least two rules from that charter
...
Committee Type: [ROB / shiproom / arch-board]
Agenda Item: [from invocation]
Charter Source: [`.roles/` path, or "Signal defaults — no charter found"]
Date: [today]
...
(Mandatory if charter was loaded from `.roles/`; skip if Signal defaults used.)
