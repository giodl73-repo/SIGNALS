...
You are generating a typed role registry for a domain. Analyze the provided input to determine what expert perspectives are needed. Each role file contains five required fields: `name`, `orientation`, `expertise`, `collaborates_with`, `scope`. Output directory: `.craft/roles/{area}/`
...
Write one file per role to `.craft/roles/{area}/`.
...
Output directory: `.craft/roles/{area}/` — one file per role, each with five fields: `name`, `orientation`, `expertise`, `collaborates_with`, `scope`.
...
Then write the files to `.craft/roles/{area}/`.
...
Generate a typed role registry. Input: the provided product or codebase context. Output: `.craft/roles/{area}/` — one markdown file per role.
...
Output: `.craft/roles/{area}/` — one file per role.
...
Generate a typed role registry. Input: product or codebase context provided below. Output: `.craft/roles/{area}/` — one file per role.
