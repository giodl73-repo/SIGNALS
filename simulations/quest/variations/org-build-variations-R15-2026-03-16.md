...
2. `.craft/roles/{area}/{role}.md` — one file per role, five required fields each
...
### Task Steps
1. For every role enumerated in Phase 3, write `.craft/roles/{area}/{role-name}.md`.
...
- MUST write role files under `.craft/roles/{area}/`. FORBIDDEN: placing all files flat with
...
|----|-----------|---------|
| G-01 | All five role fields present in every .craft/roles/ file | MUST |
...
| Step | Action |
|------|--------|
| 1 | Write `.craft/roles/{area}/{role-name}.md` for every role |
...
| Role files under `.craft/roles/{area}/` | MUST |
...
PRIMARY OUTPUTS:
  ...
  (2) .craft/roles/{area}/{role}.md — one file per role, five required fields
...
**T4.1** For every role in the Phase 3 enumeration, write:
  Path: `.craft/roles/{area-name}/{role-name}.md`
...
### ── Constraints ─────────────────────────────────────
  ...
  MUST organize files under .craft/roles/{area}/.
  FORBIDDEN: placing all files flat under .craft/roles/ with no area subdirectory.
...
| Step | Action |
|------|--------|
| 1 | Write `.craft/roles/{area}/{role-name}.md` for every enumerated role |
...
| Files organized under .craft/roles/{area}/ | MUST |
...
MUST place all role files under `.craft/roles/{area}/`.
FORBIDDEN: placing all files flat under `.craft/roles/` with no area subdirectory.
