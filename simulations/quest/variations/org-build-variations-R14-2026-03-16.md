...
typed `.craft/roles/{area}/{role}.md` files for every role.
...
#### 5B — Generate .craft/roles/{area}/{role}.md files

For every role enumerated in Phase 2, generate a role file at
`.craft/roles/{area}/{role}.md` with exactly these five fields:
...
- MUST place the file at `.craft/roles/{area}/inertia-advocate.md`

#### 5C — File layout

MUST group role files into area subdirectories under `.craft/roles/`.
...
#### 4B — Generate .craft/roles/{area}/{role}.md files
...
#### 4C — Generate .craft/roles/{area}/{role}.md files

For every role in the Role Registry table, write a role file at
`.craft/roles/{area}/{role}.md`. Use this table-format template:
...
2. Generate `.craft/roles/{area}/{role}.md` for every enumerated role.
...
3. MUST include the inertia-advocate.
   ...
   - Place at `.craft/roles/{primary-area}/inertia-advocate.md`
...
Generate `.craft/roles/{area}/{role}.md` for every enumerated role.
