...
Look for role definitions in `.craft/roles/`. If you find role files there, list every role you found in a `ROLES-LOADED:` block, one per line, formatted as `- [role name] — [one-line description from the role file]`.
