...
2. `.craft/roles/{area}/{role}.md` — one typed file per role
...
| T3-FILE-COUNT | int | total .craft/roles/ files written |
...
Every `.craft/roles/{area}/{role}.md` MUST contain all five fields:
```yaml
...
MUST create `.craft/roles/{area}/inertia-advocate.md`.
...
FORBIDDEN: placing all roles flat in `.craft/roles/` with no subdirectory structure.
...
MUST produce two artifact sets:
1. `org-chart.md`
2. `.craft/roles/{area}/{role}.md` — one file per role
...
**Inertia-advocate:**
MUST write `.craft/roles/{area}/inertia-advocate.md` first, before any other role.
...
MUST write `.craft/roles/{area}/inertia-advocate.md` before any other role.
