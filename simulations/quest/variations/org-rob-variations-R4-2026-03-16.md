...
Construct inline role definitions if a required role has no `.roles/` file.
...
Role: VP/executive archetype; construct if absent from `.roles/`.
...
**pm**
- Role: load `.roles/signal/pm.md`. Cite `lens.verify` explicitly.
...
Role: load `.roles/signal/pm.md`. Cite `lens.verify` items explicitly.
...
Role: load `.roles/signal/pm.md`. Cite `lens.verify` items.
