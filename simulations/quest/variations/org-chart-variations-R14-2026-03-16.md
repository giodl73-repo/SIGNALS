...
If no files are found, produce `ROLES-NOTE: No .roles/ files found. Using inferred roles:` with one inferred entry per role.
...
DO check `.roles/` before writing anything.
Produce `ROLES-LOADED:` block: `- [role name] — [description]` per role.
If no files: `ROLES-NOTE: No .roles/ files found. Using inferred roles:` with inferred entries.
...
─── GATE 0 DO / DO NOT ───────────────────────
DO:
  - Check `.roles/` before writing any content
...
DO NOT:
  - Invent role names if .roles/ files exist
