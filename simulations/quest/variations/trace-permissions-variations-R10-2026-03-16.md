Producing V-01 and V-02 now. V-03 through V-05 in the next piece.

---

## V-01 — Single Axis: Role Sequence (SE-first)

**Axis:** Role sequence — SE executes before CS; SE populates TABLE_1-5 without CS operational assumptions
**Hypothesis:** SE-first eliminates framing dependency between CS expectations and SE analysis. C-31/C-32/C-33 already present from baseline. C-34/C-35/C-36 absent. Expected: 25/28.
...
Four explicitly labeled phases. SE-first sequence — SE builds TABLE_1-5 before CS responds.
Phase labels MUST appear as section headers. Each phase completes fully before the next begins.
...
Executes second in privilege-tier descent order. SE-0 (admin-tier inventory and TABLE_4 escalation
...
differential. After TABLE_1 and TABLE_3, SE populates CS-3 SE Cross-Reference and SE Confirmation.
After TABLE_4, SE populates CS-2 SE Cross-Reference and SE Confirmation. Issues handoff to PHASE 2.

**PHASE 2 — CS (Customer Success):**
Executes third. Opens with CS-0 sub-section (Schema Registry acknowledgment: CS echoes Registry
schema IDs for CS-2 and CS-3 by exact Schema ID label, lists SE-updatable columns for each, and
confirms SE has already populated those columns). Then reviews SE-populated CS-2 and CS-3.
CS-1 provides qualitative baseline. Issues handoff to PHASE 3.
...
fails C-24. CA-1.8 verifies CS-0 acknowledgment precedes CS-1.
...
Note: TABLE_4 executes at SE-0 before SE-1 — the privilege ceiling is established first.
...
- SHALL-G: At the opening of SE-2, SE-3, and SE-4, closure MUST appear as a two-part labeled block — Line 1: `MANUAL GAP [<exact CONTEXT label>]:` where the label is character-for-character as written in the CONTEXT section; Line 2: description of what manual audits miss; Line 3: `STRUCTURED CLOSE:`; Line 4: which table and mechanism closes it. Paraphrase, single-line annotation, blockquote, or TABLE_5 consolidation all fail SHALL-G. CA-1.7 verifies.

*Handing to PHASE 1 — SE.*

---

## PHASE 1 — SE: Security Analysis (Privilege-Tier Descent Order)
...
CS-3 SE Cross-Reference and SE Confirmation. Note confirmations and CONTRADICTED rows (trigger
CS-EXPECTATION-VIOLATED per SHALL-F).

**Cross-role differential analysis:** Compare at least one Admin-tier role against one Custom-tier
role for the same entity and operation. Cite SE-0 TABLE_4 data for differentials attributable to
admin-tier escalation vectors.
...
role-only scope due to team membership; flag CS-EXPECTATION-VIOLATED in TABLE_5 if this was
not anticipated in CS-2/CS-3.
...
SE has already populated SE Cross-Reference and SE Confirmation columns in CS-2 and CS-3.*

### CS-0 — Schema Registry Acknowledgment (CS-authored, Phase 2)

CS echoes the Schema Registry entries for CS-2 and CS-3 from Phase 0 before reviewing
expectation rows. This sub-section confirms the handoff from CA's Registry to CS's review task.
...
SE has populated these columns per SE-update protocol. CS reviews SE Confirmation values.
...
**CS-2 — Sharing rule review** *(Schema Registry CS-2; SE has populated SE Cross-Reference and
SE Confirmation)*:

| Rule Name | Trigger | Expected Access Granted | Intended for CS Role? | Potential Overreach? | SE Cross-Reference | SE Confirmation |
|-----------|---------|------------------------|-----------------------|---------------------|---------------------|-----------------|

For each SE Confirmation = CONTRADICTED: verify CS-EXPECTATION-VIOLATED exists in TABLE_5.

**CS-3 — Cross-role access differential review** *(Schema Registry CS-3; SE has populated SE
Cross-Reference and SE Confirmation)*:
...
- Registry anchor — Schema Registry declared CS-2 as a named schema entry with SE-updatable columns SE Cross-Reference and SE Confirmation and SE-update protocol co-located. Schema Registry declared CS-3 same. PHASE 2 mandate requires CS-0 sub-section to acknowledge both schema IDs and list SE-updatable columns before CS-1/CS-2/CS-3.
- Preamble anchor — CS-2 and CS-3 are Schema Registry entries alongside TABLE_1-5. CS-0 acknowledgment is a PHASE 2 structural requirement: CS cites both schema IDs and SE-updatable columns before reviewing expectation rows. CA-1.8 verifies.
- Verification — CS-2 in Schema Registry with SE-updatable column declarations? CS-3 same? CS-0 present before CS-1? CS-0 cites CS-2 schema ID by exact label? CS-0 cites CS-3 schema ID by exact label? -> PASS / FAIL
...
## V-02 — Single Axis: Output Format (Structural Axis Non-Interference Declaration)

**Axis:** Output format — Phase 0 Step 0.2 adds a STRUCTURAL AXIS NON-INTERFERENCE DECLARATION table naming C-31/C-32/C-33 as three orthogonal dimensions with binding non-interference contract
**Hypothesis:** The declaration converts the empirical composability of C-31/C-32/C-33 into a structural contract. C-34 is satisfied. C-35 and C-36 absent. Expected: 26/28.
...
columns on TABLE_1 and TABLE_3; CS-2 and CS-3 with SE-updatable columns and SE-update protocols
— seven schemas total) and the Criterion Enforcement Matrix preamble (Step 0.2: SHALL-A through
SHALL-G, M4 row IDs CA-1.1 through CA-1.8 pre-assigned, STRUCTURAL AXIS NON-INTERFERENCE
DECLARATION declaring C-31/C-32/C-33 as three orthogonal dimensions). CA issues handoff to PHASE 1.
...
Three structural dimensions enforced independently in this output. Each dimension operates on a
distinct structural layer. No dimension's compliance status affects any other dimension's compliance
status. A failure in one axis does not cascade to the other two axes.

| Axis | Dimension | Criterion | Output-Layer Mechanism | Non-Interference Boundary |
|------|-----------|-----------|----------------------|--------------------------|
| A-1 | Execution order | C-31 | SE-0 section header precedes SE-1 section header in output body; TABLE_4 rows appear before TABLE_1 rows | A-1 compliance is solely whether TABLE_4 precedes TABLE_1 in the output. It is independent of A-2 (closure-note format) and A-3 (CS self-reference). |
| A-2 | Closure-note format | C-32 | MANUAL GAP / STRUCTURED CLOSE two-part blocks at SE-2, SE-3, SE-4 openings with character-for-character CONTEXT labels | A-2 compliance is solely whether the exact-label two-part block format appears at each SE section opening. It is independent of A-1 (execution order) and A-3 (CS self-reference). |
| A-3 | CS self-reference | C-33 | CS-0 cites Schema Registry IDs for CS-2 and CS-3 by exact Schema ID label before CS-1 content begins | A-3 compliance is solely whether CS-0 contains the exact Schema ID citations before CS-1. It is independent of A-1 (execution order) and A-2 (closure-note format). |

**Non-interference statement:** The three dimensions (A-1/A-2/A-3) are orthogonal. The output
satisfies or fails each dimension independently. CA-1.4 verifies A-1 (SE-0 ordering) only.
CA-1.7 verifies A-2 (MANUAL GAP exact-label format) only. CA-1.8 verifies A-3 (CS-0 Registry
citation) only. No single CA-1 row's verdict affects another axis's verdict.
...
Every TABLE_1 role MUST appear, ordered by Tier.
...
**CA-1.4 — C-04 verification (A-1: execution order)** *(Phase 0 M4 pre-assignment CA-1.4)*:
- Registry anchor — Schema ID TABLE_4: [Vector, Checked?, Finding, Severity] — all four vectors, Checked? = YES. Registry note: TABLE_4 fills at SE-0 before TABLE_1.
- Preamble anchor — C-04 as authored by CA in Phase 0: TABLE_4 / SE-0 / SHALL-D / CA-1.4. Axis A-1 (execution order): CA-1.4 scope is exclusively SE-0 ordering and four-vector presence; non-interference with A-2 and A-3 declared in preamble — verifying...
- Verification — TABLE_4 at SE-0 (before SE-1)? All four vectors? Checked? = YES? Findings or specific rule-outs? -> PASS / FAIL
...
**CA-1.7 — SHALL-G compliance verification (A-2: closure-note format)** *(exact-label two-part closure blocks)*:
...
- Preamble anchor — SHALL-G as authored by CA in Phase 0: Line 1 MUST be `MANUAL GAP [<exact CONTEXT label>]:`; exact = character-for-character. Axis A-2 (closure-note format): CA-1.7 scope is exclusively exact-label match at SE-2/SE-3/SE-4; non-interference with A-1 and A-3 declared in preamble — verifying...
- Verification — SE-2 opens with `MANUAL GAP [Blind spot 1 — Post-incident FLS gap]:`? SE-3 opens with `MANUAL GAP [Blind spot 2 — Cumulative privilege blind spot]:`? SE-4 opens with `MANUAL GAP [Blind spot 3 — OWD-sharing-rule override gap]:`? Each followed by `STRUCTURED CLOSE:`? -> PASS / FAIL per section.

**CA-1.8 — CS-0 Registry acknowledgment precedes CS-1 (A-3: CS self-reference)** *(C-29 + C-33)*:
- Registry anchor — Schema Registry declared CS-2 and CS-3 with Schema IDs, each with SE-updatable column declarations co-located.
- Preamble anchor — C-33 requires CS to open with CS-0 citing Schema Registry IDs for CS-2 and CS-3 by exact label before CS-1 content. Axis A-3 (CS self-reference): CA-1.8 scope is exclusively CS-0 presence and exact Schema ID citations; non-interference with A-1 and A-2 declared in preamble — verifying...
- Verification — CS-2 in Schema Registry? CS-3 same? CS-0 present before CS-1? CS-0 cites CS-2 schema ID by exact label? CS-0 cites CS-3 schema ID by exact label? CS-0 lists SE-updatable columns for both? -> PASS / FAIL

**CA Format Compliance Verdict — Phase 0 citation:**
Citing: CA-authored Schema Registry (Step 0.1, Phase 0), CA-authored preamble (Step 0.2, Phase 0), STRUCTURAL AXIS NON-INTERFERENCE DECLARATION (Step 0.2, Phase 0).
[C-01 through C-05 with CA-1.1 through CA-1.5 results. SHALL-F (CA-1.6). SHALL-G exact-label two-part block compliance — Axis A-2 (CA-1.7). C-29+C-33 CS-0 acknowledgment — Axis A-3 (CA-1.8). Axis A-1 execution order (CA-1.4). Three-axis non-interference: each axis verified independently per STRUCTURAL AXIS NON-INTERFERENCE DECLARATION. Overall: COMPLIANT / NON-COMPLIANT. Open items attributed to responsible role.]
```

---

V-03 through V-05 in the next piece.
