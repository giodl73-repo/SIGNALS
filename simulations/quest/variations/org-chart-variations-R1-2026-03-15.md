...
| V-03 | role-sequence | `.roles/` read is Step 1; produces a ROLES-LOADED list that all subsequent sections must reference by name | Forces C-03 from the start rather than as a retrofit; if roles exist they anchor the design |
...
- **C-03 context-sensitivity** — V-03 makes the `.roles/` read explicit and produces an artifact (ROLES-LOADED table) that is visible in the output, making it easy for the evaluator to check the run context. The `.roles/signal/` directory has `pm.md`, `architect.md`, and `inertia-advocate.md` — these should appear by name in any run against the current repo.
