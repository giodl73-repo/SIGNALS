Written to `simulations/quest/variations/signal-check-variations-R13-2026-03-18.md`.

---

## R13 Variations Summary

| Var | C-39 | C-40 | Est. Asp. | Est. Composite |
|-----|------|------|-----------|----------------|
| V-01 | **FAIL** | PASS | 31/32 | 99.69 |
| V-02 | PASS | **FAIL** | 31/32 | 99.69 |
| V-03 | **FAIL** (boundary) | **FAIL** (boundary) | 30/32 | 99.38 |
| V-04 | **FAIL** | **FAIL** | 30/32 | 99.38 |
| V-05 | PASS | PASS | 32/32 | 100.00 |

---

**Structural changes across variations -- only two locations differ:**

**ENFORCEMENT STACK NOTE** (C-39 axis):
- FAIL (V-01, V-03, V-04): retains `"prevents silent omissions: ..."` / `"prevents reference ambiguity: ..."` / `"prevents constraint scope gaps: ..."` prose alongside forward-refs
- FAIL boundary (V-03): retains only one "prevents..." sentence (Rule 1), Rules 2 and 3 have bare forward-refs -- still fails because C-39 requires *exclusively* the interdependency statement
- PASS (V-02, V-05): collapsed to `"Each addresses an independent failure mode -- failure class encoded in each rule's heading above. No layer substitutes for another; the three rules address independent failure modes. All three must pass."`

**CONSULTING DIRECTORY** (C-40 axis):
- FAIL (V-02, V-04): no directory present at block entrance
- FAIL boundary (V-03): directory present but routes only 2 of 3 reader roles (Rule 2 omitted) -- fails because C-40 requires *all three routes together*
- PASS (V-01, V-05): named `Consulting Directory:` block at block entrance listing all three roles by name, rule, and decision act

**Potential R14 signals from V-05:**
1. Directory entries use short parenthetical decision-acts -- candidate for whether each entry should also name the specific output section (Finding fields, action fields) the reader will land on
2. NOTE phrase "failure class encoded in each rule's heading above" references the heading collection as a unit -- candidate for whether NOTE should carry a numbered heading index, making it an explicit taxonomy pointer rather than only an interdependency declaration
3. Consulting directory is not itself subject to a Rule 3 `Applies-to` annotation -- candidate for whether a block-level declaration in the STANDING RULES context should carry its own scope declaration
directory (C-40 fail) and
ENFORCEMENT STACK NOTE retains "prevents..." prose alongside forward-refs (C-39 fail). Scores
30/32. Confirms each criterion contributes exactly one point and both gaps are isolable.

**V-05** (full ceiling): NOTE collapsed to pure interdependency statement (C-39 pass) and
consulting directory added at block entrance naming all three reader routes by role, rule, and
decision act (C-40 pass). Scores 32/32.

---

## V-01: C-39 FAIL -- Note retains "prevents..." prose; consulting directory present

**Variation axis**: Consulting directory (C-40) without note residual cleanup
**Hypothesis**: A STANDING RULES block where a named consulting directory at the block
entrance routes all three reader types to the correct rule by name and decision act
(C-40 passes) -- but the ENFORCEMENT STACK NOTE still carries explanatory failure
descriptions ("prevents silent omissions", "prevents reference ambiguity", "prevents
constraint scope gaps") alongside the C-37 forward-reference pointers -- fails C-39
only. The consulting directory and the note's residual prose are at separate structural
locations: directory at block entrance, note after Rule 3. Adding the directory does
not clean up the note. C-40 PASS, C-39 FAIL -- independently testable. Scores 31/32.
...
## V-02: C-40 FAIL -- Note pure interdependency; no consulting directory

**Variation axis**: Note residual cleanup (C-39) without consulting directory
**Hypothesis**: A STANDING RULES block where the ENFORCEMENT STACK NOTE is collapsed
to a pure interdependency statement -- no "prevents..." prose, no failure descriptions,
only the stack coordination claim, the no-substitution rule, and the all-three-must-pass
requirement (C-39 passes) -- but no consulting directory is added at the block entrance
(C-40 fails). Per-rule reader positions exist per-rule in each rule body (C-38 satisfied)
but they are not consolidated into a named routing structure the reader can scan before
entering any rule body. C-39 PASS, C-40 FAIL. Confirms C-40 is not discharged by
C-38 alone: embedding reader positions per-rule satisfies C-38 but the consulting
directory is a block-level structural addition that C-38 does not imply. Scores 31/32.
...
## V-03: Boundary -- partial directory (2 of 3 routes); note retains 1 explanatory sentence

**Variation axis**: Combined boundary: C-40 partial directory + C-39 partial cleanup
**Hypothesis**: A STANDING RULES block where (a) the consulting directory is present
but names only two of three reader routes -- Rule 1 and Rule 3 routes declared, Rule 2
route omitted -- and (b) the ENFORCEMENT STACK NOTE has forward-reference pointers
(C-37 satisfied) but retains one explanatory sentence for Rule 1 only ("prevents silent
omissions: outputs that appear complete but contain no evidence for missing dimensions")
while Rules 2 and 3 carry only forward-refs and no prose -- fails both C-39 and C-40.
C-39 requires "exclusively" the interdependency statement: any retained explanatory
prose about what a rule prevents fails the criterion, even if only one of three rules
retains it. C-40 requires "all three reader routes together": routing two of three does
not satisfy the universal quantifier. Scores 30/32. Confirms strict reading of both
criteria: "exclusively" means exclusively; "all three" means all three.
...
ENFORCEMENT STACK NOTE:
  ...
  independent failure mode -- failure class encoded in each rule's heading above:
    Rule 1 -- failure class: see Rule 1 heading above -- prevents silent omissions:
      outputs that appear complete but contain no evidence for missing dimensions.
    Rule 2 -- failure class: see Rule 2 heading above.
    Rule 3 -- failure class: see Rule 3 heading above.
  No layer substitutes for another; the three rules address independent failure modes.
  All three must pass for the output to be a valid health report.
...
## V-04: Combined regression -- R12 V-05 verbatim (C-39 FAIL, C-40 FAIL)

**Variation axis**: Regression baseline (R12 V-05 verbatim)
**Hypothesis**: R12 V-05 satisfies all 30 criteria from the R12 rubric, including C-37
(forward-refs in note) and C-38 (per-rule reader positions). Under the R13 rubric, it
scores 30/32 because C-39 fails (ENFORCEMENT STACK NOTE retains "prevents..." prose
alongside the forward-refs) and C-40 fails (no consulting directory at the block
entrance). This variation establishes the delta: V-01 (+C-40) and V-02 (+C-39) each
add exactly one point; V-05 adds both. The regression score confirms both criteria
are load-bearing and non-overlapping with all prior criteria.
...
## V-05: Full ceiling -- passes all 32/32 under v13

**Variation axis**: Full ceiling (C-39 + C-40)
**Hypothesis**: Adding (a) a named consulting directory at the STANDING RULES block
entrance routing all three reader types to the correct rule by role name and decision
act (C-40 passes) and (b) collapsing the ENFORCEMENT STACK NOTE to a pure
interdependency statement -- no "prevents..." prose, no failure descriptions, only
the stack coordination claim, the failure-class-in-headings pointer, and the
no-substitution rule (C-39 passes) -- yields a 32/32 sweep. The consulting directory
and the note cleanup address different structural locations (block entrance vs. post-
Rule-3 note) and different reader needs (entry-point routing vs. note residual
deduplication); they are independently testable and their combination is additive.
Scores 32/32.

**Potential R14 signals from V-05:** (1) The consulting directory entries currently
use a short parenthetical decision-act description -- a candidate for whether each
entry should also name the specific output section the reader will land on (e.g.,
"Finding fields in preflight items"), making the directory actionable at a finer
granularity than rule-level routing; (2) the pure interdependency NOTE's phrase
"failure class encoded in each rule's heading above" references the heading collection
as a unit without listing them -- a candidate for whether the NOTE should carry a
numbered heading index ("Rule 1 -- Absence Drift; Rule 2 -- Reference Ambiguity;
Rule 3 -- Constraint Scope Gaps") serving as a scannable taxonomy pointer, making
the NOTE itself an index to the heading structure rather than only an interdependency
declaration; (3) the consulting directory is not itself subject to a Rule 3 Applies-to
annotation -- a candidate for whether the directory, as a block-level declaration
in the STANDING RULES context, should carry its own scope declaration, making it
subject to the same existence-condition enforcement that governs all rules in the block.
