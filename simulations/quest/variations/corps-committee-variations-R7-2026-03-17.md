...
Read participant charters from `.craft/roles/`. AMEND: add attendees, change scope.
...
| Field | Value |
|-------|-------|
| Role | [role name from .craft/roles/] |
...
| Quorum Threshold | [n required] | `.craft/roles/[charter]` |
...
Read charters from `.craft/roles/`. Honor every constraint you find there.
...
From the roster (read `.craft/roles/`), designate one participant as the Inertia Challenger:
...
If no qualifying participant exists in `.craft/roles/`, inject a provisional Inertia-Advocate:
...
**Charter Constraints** (from `.craft/roles/`):
...
Read charters from `.craft/roles/`. AMEND: add attendees or change scope.
...
```
[ ] Designated Inertia Challenger: [role name from .craft/roles/]
...
Simulate a committee meeting before the real one.
Types: ROB (product/service review) | Shiproom (go/no-go) | Arch-board (architecture decision review)
Charters: `.craft/roles/`. AMEND: add attendees or change scope.
...
| Field | Value |
|-------|-------|
| Designated Inertia Challenger | [role from `.craft/roles/`] |
