...
The path `.roles/{area}/inertia-advocate.md` is checked for existence.
...
- **ABSENT:** CONTRACT: INERTIA-ADVOCATE-TEMPLATE governs companion generation. Content is produced with all `{area}` slots substituted. The file is held in working memory, not written yet.
  ...
  -> **Gate 2b:** Companion file path is confirmed as `.roles/{area}/inertia-advocate.md`. PASS / FAIL
...
YAML frontmatter is generated for `.roles/{area}/{subrole}.md`. CONTRACT: FIELD-REGISTER governs all 12 required fields.
...
1. If companion was generated in Phase 2: `.roles/{area}/inertia-advocate.md`
...
Check `.roles/{area}/inertia-advocate.md`. If ABSENT: apply CONTRACT: INERTIA-ADVOCATE-TEMPLATE; substitute all `{area}` slots; hold in working memory. If PRESENT: continue.

| Gate | [ID] | Criterion | Verdict |
|------|------|-----------|---------|
| 2a | [H] | INERTIA-ADVOCATE-TEMPLATE applied; companion content complete; no literal `{area}` remaining | PASS / FAIL |
| 2b | -- | Companion path confirmed as `.roles/{area}/inertia-advocate.md` | PASS / FAIL |
...
Apply CONTRACT: FIELD-REGISTER. Generate all 12 required frontmatter fields for `.roles/{area}/{subrole}.md`.
