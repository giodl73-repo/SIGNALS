...
- `.roles/` absent: `ERROR: role registry missing — cannot proceed`
- `.roles/` present but empty: `ERROR: role registry empty — no reviewers available`
...
Read every file in `.roles/`. Build the reviewer pool from what you find.

If `.roles/` is absent: output `ERROR: .roles/ not found — cannot proceed.` Stop.
If `.roles/` is empty: output `ERROR: .roles/ is empty — no reviewers available.` Stop.
...
**L1** Open `.roles/`. Attempt to read the directory.

**L2** Apply error cascade — halt at the first condition that fires:

| State | Condition | Halt message |
|-------|-----------|-------------|
| MISSING_DIR | `.roles/` does not exist or is unreadable | `ERROR [MISSING_DIR]: .roles/ not found. Resolve before running.` |
| EMPTY_POOL | Directory exists but contains zero readable files | `ERROR [EMPTY_POOL]: .roles/ has no role files. Add roles before running.` |
...
|--------|------|-----------------|
| Role | string | Filename stem from `.roles/` — never invented |
...
Read `.roles/`. Role pool = all files found.

Error halts:
- Missing directory: `ERROR: .roles/ not found.`
- Empty directory: `ERROR: .roles/ empty.`
...
Open `.roles/`. Check in order:

| Check | Fail condition | Halt message |
|-------|---------------|-------------|
| Directory exists | `.roles/` absent or unreadable | `ERROR [NO_REGISTRY]: .roles/ not found. Execution halted.` |
| Files present | Directory is empty | `ERROR [EMPTY_REGISTRY]: .roles/ contains no role files. Execution halted.` |
