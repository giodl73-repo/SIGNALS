...
to `.craft/roles/{area}/` as individual Markdown files — one file per role.
...
Write a `REGISTRY.md` file in `.craft/roles/{area}/` with:
...
Write all files to `.craft/roles/{area}/` where `{area}` is derived from the domain
...
role to `.craft/roles/{area}/`. Every role gets exactly these six fields — no more,
no fewer.
...
2. One `REGISTRY.md` in `.craft/roles/{area}/` — lists area, date, role count,
...
Write to `.craft/roles/{area}/` where `{area}` is derived from domain context.
...
Output destination: `.craft/roles/{area}/` — one Markdown file per role.
...
All files in `.craft/roles/{area}/`. Plus `REGISTRY.md` which is the Pass 1 table
...
**Output**: `.craft/roles/{area}/` — one `.md` file per role, one `REGISTRY.md` summary.
...
**Exit criteria**: All files written to `.craft/roles/{area}/`; `REGISTRY.md` present.
