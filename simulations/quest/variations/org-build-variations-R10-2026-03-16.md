...
You are running `org-build`. Produce two outputs: (1) `org-chart.md` at repo root, (2) `.roles/{area}/{role}.md` for every role identified.
...
MUST produce a `ROLES-LOADED:` block with one entry per role: `- [role name] — [one-line description]`. If no role files exist, produce `ROLES-NOTE: No .roles/ files found. Using inferred roles:` instead.
...
Write one `.roles/{area}/{role}.md` per role in T1-ROLE-LIST. MUST group files by area subdir.
...
You are running `org-build`. Produce two outputs: (1) `org-chart.md`, (2) `.roles/{area}/{role}.md` for every role.
...
Write `.roles/{area}/{role}.md` for every role in T1-ROLE-LIST. MUST group by area subdir. Each file uses this field-value format:
...
You are running `org-build`. Produce: (1) `org-chart.md`, (2) `.roles/{area}/{role}.md` for every role.
...
Write `.roles/{area}/{role}.md` per role in T1-ROLE-LIST. MUST group by area subdir. Each file MUST contain all five fields: `orientation`, `lens`, `expertise`, `scope`, `collaborates_with`. Missing any field in any file is a hard failure.
...
You are running `org-build`. Produce: (1) `org-chart.md`, (2) `.roles/{area}/{role}.md`.
...
Write `.roles/{area}/{role}.md` per role. Group by area subdir. Every file MUST contain all five fields: `orientation`, `lens`, `expertise`, `scope`, `collaborates_with`. Missing any field is a hard failure.
...
Write `.roles/{area}/{role}.md` per role. Group by area subdir. Every file — all five fields required:
