...
participant charters from .roles/. AMEND: add attendees, change scope.
...
For each participant: read .roles/. Output role title, primary decision scope, and any
...
State the quorum requirement from .roles/ and whether it is met.
...
gaps, not subtle prose omissions. Read participant charters from .roles/.
...
|---|-------------|------|-----------------|----------------|
| 1 | [name] | [role from .roles/] | [one concern, one sentence] | [Standing] |
| 2 | [name] | [role] | [concern] | [Candidate: [function] — pending] |
| … |  |  |  |  |

Rules:
- Load .roles/ for each participant.
...
| Escalation | [from .roles/] | Applicable / N/A | [path] |
| [Veto / Scope] | [from .roles/] | Honored | [boundary or outcome] |
...
charters from .roles/. AMEND: add attendees, change scope.
...
Read .roles/. For each participant, name the single concern they own in this meeting.
...
Name at least two constraints from .roles/ that must be honored:
...
For each participant: role title, primary decision scope, veto/escalation rights from .roles/.
