...
Read `.craft/roles/` for this committee type's charter. Load every named role. If no charter exists, fall back to Signal defaults (PM, Engineering Lead, Design Lead, Inertia-Advocate) and state the fallback explicitly.
...
Read `.craft/roles/` for this committee type's charter. Load all named participants with their documented orientations. Disclose if falling back to Signal defaults (PM, Engineering Lead, Design Lead, Inertia-Advocate).
...
Next, load the participants. Read `.craft/roles/` to find the charter for this committee type. List every named participant role with a one-phrase description of their role orientation. If there's no charter file, say so explicitly and use Signal's default committee roles: PM, Engineering Lead, Design Lead, and Inertia-Advocate.
...
```
Committee: [TYPE: ROB / shiproom / arch-board / other] — [agenda item title]
Charter: [path to .craft/roles/ file, or "Signal defaults — no charter found"]
Participants:
- [Role Name] — [charter-defined orientation in one phrase]
- [add one line per loaded role]
```

Read `.craft/roles/` for the committee charter. Load every named role. Disclose fallback to Signal defaults (PM, Engineering Lead, Design Lead, Inertia-Advocate) if no charter file exists.
