The file is complete (2,060 lines). Here is the Round 5 output:

---

# trace-permissions Variate — Round 5 (R13)
**Date:** 2026-03-16
**Rubric:** v12 (C-01 through C-36)
**Target:** 28/28 — add C-34 + C-35 + C-36 to R12-V-05 base

**State entering Round 5:**

| Variation | v12 score | Notes |
|-----------|-----------|-------|
| R12-V-05 | 98.9 (25/28) | passes C-31+C-32+C-33; fails C-34 (no preamble axis declaration), C-35 (no SE-4 STRUCTURED CLOSE TABLE_4 row cross-ref + no CA-1.9), C-36 (no tri-dimensional CA verdict attestation) |

All three dependency prerequisites satisfied by R12-V-05: C-31+C-32+C-33+C-17 for C-34; C-31+C-32 for C-35; C-22+C-32+C-33 for C-36. No dependency gaps.
...
|-----------|------|------------|
| V-01 | Single-axis C-34: preamble R12 Structural Axis Declaration — adds block after SHALL-G naming C-31/C-32/C-33 as orthogonal dimensions (execution order / closure-note format / CS self-reference) with explicit non-interference statement. All other mechanisms from R12-V-05 preserved. | 26/28 (adds C-34; fails C-35 no CA-1.9, fails C-36 no tri-dimensional verdict). |
| V-02 | Single-axis C-35: SE-4 STRUCTURED CLOSE TABLE_4 SE-0 row cross-reference + CA-1.9 — Schema Registry TABLE_4 entry declares SE-0 row IDs T4-1 through T4-4; SE-4 STRUCTURED CLOSE names those rows; CA-1.9 verifies as distinct from CA-1.4 (SE-0 ordering) and CA-1.7 (MANUAL GAP labels). No C-34. No C-36. | 26/28 (adds C-35; fails C-34 no axis declaration, fails C-36 no tri-dimensional verdict). |
| V-03 | Single-axis C-36: CA terminal verdict tri-dimensional self-evidence attestation — adds R12 Tri-Dimensional Attestation block to CA verdict naming each R12 dimension with specific output-body evidence source. No C-34. No C-35. | 26/28 (adds C-36; fails C-34 no axis declaration, fails C-35 no CA-1.9). |
| V-04 | Combined C-34+C-35: preamble axis declaration + SE-4 STRUCTURED CLOSE TABLE_4 row cross-reference + CA-1.9. No tri-dimensional verdict (no C-36). | 27/28 (adds C-34+C-35; fails C-36). |
| V-05 | Full combination C-34+C-35+C-36: all three R13 axes on R12-V-05 base. | 28/28 — the 100.0 candidate. |

---

## V-01 — Single-Axis C-34: Preamble Structural Axis Declaration

**Axis:** Adds R12 Structural Axis Declaration block to Step 0.2 after SHALL-G. Names C-31 (execution order), C-32 (closure-note format), C-33 (CS self-reference) as orthogonal non-interfering dimensions with explicit non-interference statement. Converts R12-V-05's empirical composability into a binding preamble contract. No SE-4 STRUCTURED CLOSE modification. No CA-1.9. No tri-dimensional verdict.
**Hypothesis:** 26/28.
...
SHALL-G plus R12 Structural Axis Declaration, M4 row IDs CA-1.1 through CA-1.8 pre-assigned).
...
- Preamble anchor — SHALL-G as authored by CA in Phase 0: Line 1 MUST be `MANUAL GAP [<exact CONTEXT label>]:`; exact = character-for-character; paraphrase fails; single-line annotation fails; TABLE_5 consolidation fails.
...
- Registry anchor — Schema Registry declared CS-2 and CS-3 as named schema entries with SE-updatable columns and SE-update protocols. PHASE 1 mandate requires CS-0 to acknowledge both schema IDs and list SE-updatable columns before CS-1/CS-2/CS-3.
- Preamble anchor — CS-0 acknowledgment is a PHASE 1 structural requirement: CS cites both schema IDs and SE-updatable columns before authoring expectation rows.
- Verification — CS-2 in Schema Registry with SE-updatable column declarations? CS-3 same? CS-0 present before CS-1? CS-0 cites CS-2 and CS-3 schema IDs by exact label? CS-0 lists SE-updatable columns for both? SE populated SE-updatable columns per protocol? -> PASS / FAIL
...
## V-02 — Single-Axis C-35: SE-4 STRUCTURED CLOSE TABLE_4 SE-0 Row Cross-Reference + CA-1.9

**Axis:** Adds SE-0 row ID declaration to Schema Registry TABLE_4 entry (T4-1 through T4-4); modifies SE-4 STRUCTURED CLOSE to name TABLE_4 rows by SE-0 row ID; adds CA-1.9 as a distinct verification row separate from CA-1.4 and CA-1.7. No preamble axis declaration (no C-34). No tri-dimensional verdict (no C-36).
**Hypothesis:** 26/28.

**Diff over V-01 base (R12-V-05 + C-34 removed):**

Three targeted changes:

1. **Schema Registry TABLE_4 entry** gains SE-0 row ID declaration:
```
SE-0 row ID assignment (for SE-4 STRUCTURED CLOSE cross-reference):
T4-1 = Team inheritance | T4-2 = Sharing rules | T4-3 = Impersonation (Act on Behalf Of) | T4-4 = Admin / environment role override.
SE-4 STRUCTURED CLOSE SHALL reference TABLE_4 SE-0 rows by these IDs. CA-1.9 verifies.
```

2. **SE-4 STRUCTURED CLOSE** (replacing generic SE-0 citation with row-ID cross-reference):
```
STRUCTURED CLOSE:
TABLE_4 (filled at SE-0) evaluated all four escalation vectors. Cross-tier differential: for the
most privileged Admin-tier role and the most restricted Restricted-tier role, cite the specific
TABLE_4 SE-0 row IDs (T4-1 through T4-4) where escalation was confirmed or ruled out —
e.g., "T4-2 (Sharing rules): [finding] established the admin-tier ceiling for Case read access."
State the exact T4-N row ID that established the privilege ceiling differential; generic SE-0
TABLE_4 citations without row IDs fail C-35.
```

3. **CA-1.9** added after CA-1.8:
```
...
*(Full prompt body is R12-V-05 with these three changes applied. Role Sequencing Mandate updated to note CA-1.9 in PHASE 3 description and M4 pre-assignment range extended to CA-1.1 through CA-1.9.)*

---

## V-03 — Single-Axis C-36: CA Terminal Verdict Tri-Dimensional Self-Evidence Attestation

**Axis:** Adds R12 Tri-Dimensional Self-Evidence Attestation block to CA Format Compliance Verdict, naming each R12 dimension (C-31/execution order, C-32/closure-note format, C-33/CS self-reference) with specific output-body evidence source. No preamble axis declaration (no C-34). No CA-1.9 (no C-35).
**Hypothesis:** 26/28.

**Diff over R12-V-05 base:**

**CA Format Compliance Verdict** gains the following block appended after the verdict bracket:
...
- Dimension 1 — Execution order (C-31): Output-body evidence source = "SE-0 — Admin-Tier
  ...
  header; TABLE_4 rows appear under SE-0 sub-section before any SE-1 content; TABLE_1 Tier
  column populated with Admin / Custom / Restricted values; SE-4 STRUCTURED CLOSE cites SE-0
  TABLE_4 data in cross-tier differential. Body-verifiable without prompt inspection.
- Dimension 2 — Closure-note format (C-32): Output-body evidence source = SE-2 section opens
  ...
  exact label. Body-verifiable without prompt inspection.
- Dimension 3 — CS self-reference (C-33): Output-body evidence source = CS-0 sub-section
  ...
  both. CA-1.8 verified. Body-verifiable without prompt inspection.

All three R12 dimensions are simultaneously self-evident from the output body without reference
to prompt instructions.
```

*(Full prompt body is R12-V-05 with this block appended to the CA verdict.)*

---

## V-04 — Combined C-34+C-35: Preamble Axis Declaration + SE-4 TABLE_4 Row Cross-Reference

**Axis:** V-01's R12 Structural Axis Declaration in Step 0.2 (C-34) combined with V-02's SE-4 STRUCTURED CLOSE TABLE_4 SE-0 row cross-reference and CA-1.9 (C-35). No tri-dimensional verdict attestation (no C-36).
**Hypothesis:** 27/28.

**Diff over R12-V-05 base:**

Applies V-01's C-34 addition (full axis declaration block after SHALL-G) AND V-02's C-35 additions (TABLE_4 SE-0 row ID schema annotation + SE-4 STRUCTURED CLOSE row-ID citation + CA-1.9). Role Sequencing Mandate PHASE 0 description updated to name both additions:

```
**PHASE 0 — CA (Compliance Auditor):**
CA executes first. Authors the Schema Registry (Step 0.1: TABLE_1 through TABLE_5 with Tier
columns on TABLE_1 and TABLE_3; CS-2 and CS-3 with SE-updatable columns and SE-update protocols;
TABLE_4 entry declares SE-0 row IDs T4-1 through T4-4 — seven schemas total) and the Criterion
Enforcement Matrix preamble (Step 0.2: SHALL-A through SHALL-G plus R12 Structural Axis
Declaration, M4 row IDs CA-1.1 through CA-1.9 pre-assigned). CA issues handoff to PHASE 1.
```

*(All other sections identical to R12-V-05 except the two V-01 and V-02 changes applied simultaneously.)*

---

## V-05 — Full Combination C-34+C-35+C-36: The 28/28 Candidate

**Axis:** All three R13 axes on R12-V-05 base: preamble R12 Structural Axis Declaration (C-34) + SE-4 STRUCTURED CLOSE TABLE_4 SE-0 row cross-reference with CA-1.9 (C-35) + CA terminal verdict R12 Tri-Dimensional Self-Evidence Attestation (C-36).
...
[identical to V-01/V-02/V-03/V-04]
...
**PHASE 2 — SE (Security Engineer / Dataverse Security Expert):**
Executes third in privilege-tier descent order. SE-0 (admin-tier inventory and TABLE_4 escalation
vectors with SE-0 row IDs T4-1 through T4-4) runs before SE-1 (role-operation matrix), establishing
the privilege ceiling before populating operation cells. TABLE_1 and TABLE_3 include Tier column.
SE-2, SE-3, SE-4 each open with two-part exact-label closure blocks per SHALL-G. SE-4 STRUCTURED
CLOSE cites TABLE_4 SE-0 row IDs (T4-1 through T4-4) in cross-tier differential. Issues handoff
to PHASE 3.
...
CLOSE TABLE_4 SE-0 row cross-reference as a distinct audit target from CA-1.4 and CA-1.7.
...
**SHALL obligations (authored by CA, Phase 0):**
[SHALL-A through SHALL-G identical to V-01]
...
- Axis 1 — Execution order (C-31): SE-0 runs before SE-1; TABLE_4 is filled at SE-0 with SE-0
  row IDs T4-1 through T4-4; TABLE_1 and TABLE_3 include a Tier column; SE-4 STRUCTURED CLOSE
  cites SE-0 TABLE_4 row IDs in cross-tier differential. Governs SE section execution order and
  Tier column presence only.
- Axis 2 — Closure-note format (C-32): SE-2, SE-3, and SE-4 each open with a two-part labeled
  block beginning `MANUAL GAP [<exact CONTEXT label>]:` followed by `STRUCTURED CLOSE:`. Governs
  SE sub-section opening block format only.
- Axis 3 — CS self-reference (C-33): PHASE 1 opens with CS-0, which acknowledges CS-2 and CS-3
...
(with SE-0 row IDs T4-1 through T4-4) before SE-1. TABLE_1 and TABLE_3 include Tier column.
SE-2/SE-3/SE-4 use exact-label two-part closure blocks per SHALL-G.*

### SE-0 — Admin-Tier Inventory and Escalation Vectors (executes before TABLE_1)

[identical to V-01 / R12-V-05, TABLE_4 rows implicitly assigned T4-1 through T4-4]
...
STRUCTURED CLOSE:
TABLE_4 (filled at SE-0) evaluated all four escalation vectors. Cross-tier differential: for the
most privileged Admin-tier role and the most restricted Restricted-tier role, identify the
specific enforcement layer where access diverges by citing the TABLE_4 SE-0 row IDs established
in Phase 0 — e.g., "T4-2 (Sharing rules): [finding] established the privilege ceiling for
[entity] [operation] access; T4-4 (Admin/environment role override): [finding]." Cite specific
T4-N row IDs. State whether each divergence is expected given the role tiers and what
sharing-rule interaction (if any) was confirmed or ruled out in TABLE_4 SE-0 rows T4-1 through
T4-4. Generic "SE-0 TABLE_4 data" citations without SE-0 row IDs fail C-35.
...
[CA-1.1 through CA-1.7 identical to V-01 / R12-V-05]
...
**CA Format Compliance Verdict — Phase 0 citation:**
Citing: CA-authored Schema Registry (Step 0.1, Phase 0), CA-authored preamble (Step 0.2, Phase 0), Phase 0 by label.
[C-01 through C-05 with CA-1.1 through CA-1.5 results. SHALL-F (CA-1.6). SHALL-G exact-label two-part block compliance (CA-1.7). C-29 + C-33 CS-0 acknowledgment (CA-1.8). C-35 SE-4 STRUCTURED CLOSE TABLE_4 SE-0 row cross-reference (CA-1.9). Overall: COMPLIANT / NON-COMPLIANT. Open items attributed to responsible role.]
...
- Dimension 1 — Execution order (C-31): Output-body evidence source = "SE-0 — Admin-Tier
  ...
  Body-verifiable without prompt inspection.
- Dimension 2 — Closure-note format (C-32): Output-body evidence source = SE-2 section opens
  with two-part block `MANUAL GAP [Blind spot 1 — Post-incident FLS gap]:` followed by
  `STRUCTURED CLOSE:`; SE-3 opens with `MANUAL GAP [Blind spot 2 — Cumulative privilege blind
  spot]:` followed by `STRUCTURED CLOSE:`; SE-4 opens with `MANUAL GAP [Blind spot 3 —
...
**Full output on disk:** `simulations/quest/golden/trace-permissions-variate-R13-2026-03-16.md` (2,060 lines)

**Projected scores under v12:**

| Variation | Criteria added | Expected score |
|-----------|---------------|----------------|
| V-01 | C-34 | 26/28 (93.2) |
| V-02 | C-35 | 26/28 (93.2) |
| V-03 | C-36 | 26/28 (93.2) |
| V-04 | C-34+C-35 | 27/28 (96.1) |
