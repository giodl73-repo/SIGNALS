...
are needed. Produce one markdown role file per role in `.craft/roles/{area}/`. The
...
Write one markdown file per role. File path: `.craft/roles/{area}/{role-name}.md`
...
markdown role file per role in `.craft/roles/{area}/`. The inertia-advocate is
...
role in `.craft/roles/{area}/`. Inertia-advocate always present. Each file:
...
`.craft/roles/{area}/{role-name}.md`. The inertia-advocate role is mandatory.
...
Output: one markdown file per role at `.craft/roles/{area}/{role-name}.md`.
