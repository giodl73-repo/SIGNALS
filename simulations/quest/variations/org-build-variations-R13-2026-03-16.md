...
#### Phase 4 — Produce .craft/roles/ Files
...
1. Create `.craft/roles/{area}/{role}.md` for every role enumerated in Phase 1.
...
- MUST group files under `.craft/roles/{area}/` subdirectories. FORBIDDEN: placing all role files flat with no area grouping.
...
- FORBIDDEN: overwriting any `.craft/roles/` file that already exists at the target path.
...
- `.craft/roles/{area}/{role}.md` — one per enumerated role
...
Generate a complete organizational structure from scan results or a repository. Outputs: `org-chart.md` (ASCII diagram, headcount table, operating rhythm, inertia assessment, anti-pattern watch, org evolution roadmap) and typed `.craft/roles/{area}/{role}.md` files for every role.
...
1. Create `.craft/roles/{area}/{role}.md` for every role from Phase 1.
...
| P4-C5 | Group files under `.craft/roles/{area}/` subdirectories | MUST | file structure |
...
| P4-C11 | Overwrite any `.craft/roles/` file already existing at target path | FORBIDDEN | all files |
...
Generate a complete organizational structure from scan results or a repository. The inertia-advocate leads: the FLAT-CASE-PRESSURE verdict is the first output, and the entire org is built in response to it. Outputs: `org-chart.md` and typed `.craft/roles/{area}/{role}.md` files.
...
1. Create `.craft/roles/{area}/{role}.md` for every role enumerated in Phase 2.
...
1. For every role enumerated in Phase 1, produce a file at `.craft/roles/{area}/{role}.md`.
...
- MUST include all five fields in every file. FORBIDDEN: omitting any field from any role file.
- MUST organize files under area subdirectories matching T1-AREAS. FORBIDDEN: organizing all files flat under `.craft/roles/` without area subdirectories.
- MUST produce the `inertia-advocate` file. FORBIDDEN: omitting the inertia-advocate role.
...
#### Phase 4 — Produce .craft/roles/ Files (Skeleton → Fill)
...
1. Emit the role file skeleton once before producing any files:
   ...
   ```
2. For every role in the Phase 1 inventory table, produce `.craft/roles/{area}/{role}.md` by filling the skeleton.
