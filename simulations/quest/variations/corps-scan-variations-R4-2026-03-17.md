...
PURPOSE: walk this repo and produce a draft org.yaml — the org configuration contract —
  for human review and confirmation.
SCOPE: draft org.yaml only. No `.roles/` files. No role file content of any kind.
...
`.roles/` files. No role produces role file content or role file markdown. The only
