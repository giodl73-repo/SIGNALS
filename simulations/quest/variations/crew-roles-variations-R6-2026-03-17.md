...
Write each role as a markdown file at `.craft/roles/{area}/{role-name}.md`.
...
Write the inertia-advocate role file at `.craft/roles/{area}/inertia-advocate.md`.
...
Write each additional role at `.craft/roles/{area}/{role-name}.md`. Required fields in every role file:
...
Roles go in `.craft/roles/{area}/`. Every role file needs five things: `name`, `orientation` (which has `frame`, `verify`, and `simplify` subfields), `expertise` (with `relevance` and `depth`), `scope`, and `collaborates_with`. The inertia-advocate is always included — it's the role that asks whether the current approach actually needs replacing.
...
Write each role to `.craft/roles/{area}/{role-name}.md`.
...
For each role, write to `.craft/roles/{area}/{role-name}.md`.
...
For every `collaborates_with` value in every role file: verify the name resolves to a file in `.craft/roles/{area}/`.
