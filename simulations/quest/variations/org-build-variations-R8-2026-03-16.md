...
### .craft/roles/{area}/{role}.md skeleton
...
**MUST include** an `inertia-advocate` role in `.craft/roles/leadership/inertia-advocate.md`. At this stage, write a placeholder scope using the template for MODERATE pressure. You MUST amend this in Phase 3 once T2-PRESSURE is known.
...
**MUST group role files under `.craft/roles/{area}/` subdirectories.**
FORBIDDEN: placing all role files flat under `.craft/roles/`.
...
Overwrite the placeholder scope in `.craft/roles/leadership/inertia-advocate.md` with the verbatim text from the table above.
...
- `T5-ROLE-FILE-COUNT` — total .craft/roles/ files written
...
**inertia-advocate role — MUST be present.** File: `.craft/roles/leadership/inertia-advocate.md`
...
| Variable | Type | Value |
|----------|------|-------|
| T4-ROLE-COUNT | integer | *(fill — total .craft/roles/ files written)* |
...
MUST group under `.craft/roles/{area}/` subdirectories.
FORBIDDEN: all roles flat under `.craft/roles/`.

**inertia-advocate — MUST be present** at `.craft/roles/leadership/inertia-advocate.md`.
...
Fill skeleton slots from gate records. Write `org-chart.md` and all `.craft/roles/` files.
...
Every `.craft/roles/{area}/{role}.md` file MUST contain exactly these five fields:
```
---
orientation: <string>
lens: <string>
...
FORBIDDEN: flat placement under `.craft/roles/`.

### Inertia-Advocate Role

MUST be present at `.craft/roles/leadership/inertia-advocate.md`.
...
Write all `.craft/roles/{area}/{role}.md` files.
...
## Skeleton — .craft/roles/{area}/{role}.md
...
For `.craft/roles/leadership/inertia-advocate.md` specifically:
```
---
...
Fill every skeleton slot from the gate records. Emit `org-chart.md` and all `.craft/roles/` files.
