...
Write one markdown file per role to: .roles/{area}/
...
.roles/{area}/   where {area} reflects the actual input domain.
```
...
Output: one markdown file per role at .roles/{area}/
