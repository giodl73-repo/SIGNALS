...
Write each role from the ROLE PLAN TABLE to `.craft/roles/{area}/`. Required fields per role:
...
Write the inertia-advocate to `.craft/roles/{area}/`. Required fields:
...
For each remaining role in the plan, write to `.craft/roles/{area}/`. Each role must:
...
Write each role to `.craft/roles/{area}/`. Required fields:
...
Write each role to `.craft/roles/{area}/`. Here's the format each file needs:
...
**Exit artifact**: role files at `.craft/roles/{area}/`
