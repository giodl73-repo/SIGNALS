...
**Hypothesis:** Making the `inertia-advocate` (from `.roles/signal/inertia-advocate.md`) the first voice in every stage forces each role to answer "why is change better than doing nothing?" before writing findings. Produces role-specific findings grounded in Signal's unique philosophy (C-02) rather than generic governance output.
