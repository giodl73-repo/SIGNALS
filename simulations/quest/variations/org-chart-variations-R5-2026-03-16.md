...
Read all files in .roles/. Extract every role name and any available metadata.
...
```
ROLES-NOTE: No .roles/ found. Proceeding with roles declared by user input.
```
...
Read all files in .roles/. Extract every role name and available metadata.
...
Read all files in .roles/. Extract every role name.
