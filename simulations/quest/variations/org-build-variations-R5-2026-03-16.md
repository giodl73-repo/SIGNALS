...
Generate the inertia-advocate role file at `.craft/roles/governance/inertia-advocate.md`:
...
Identify primary expertise areas from the source material. Each area becomes a subdirectory under `.craft/roles/`.
...
### Skeleton B — Role File (.craft/roles/{area}/{role}.md)
...
**MUST: all roles under `.craft/roles/{{AREA-NAME}}/{{ROLE-NAME}}.md` paths.**  
...
Generate `.craft/roles/governance/inertia-advocate.md`:
...
For every role, generate `.craft/roles/{area}/{role}.md` with all five required fields:
...
**MUST generate `.craft/roles/governance/inertia-advocate.md` before generating any other role file.**
...
**MUST generate all role files at `.craft/roles/{area}/{role}.md`.**
...
Generate `.craft/roles/governance/inertia-advocate.md` using Skeleton B. MUST fill `{{IA-SCOPE-VERBATIM}}` with verbatim template text. MUST fill `{{IA-COLLABORATES}}` with at minimum 2 area lead roles.
...
MUST generate all role files using Skeleton C. MUST place each file at `.craft/roles/{{AREA-NAME}}/{{ROLE-NAME}}.md` where AREA-NAME is drawn from T3-AREAS. FORBIDDEN: role files outside area subdirectories.
