Written to `simulations/quest/variations/validate-inertia-variations-R20-2026-03-17.md`.

---

## R20 Variations -- V-96 through V-100

| # | Label | C-45 | C-46 | C-47 | C-48 | C-49 | C-50 | Predicted |
|---|-------|------|------|------|------|------|------|-----------|
| **V-96** | Ceiling: both F1 and F2 as distinct disqualifiers | PASS | PASS | PASS | PASS | PASS | PASS | **500** |
| **V-97** | C-50 PARTIAL: F1 only (F2 absent) | PASS | PASS | PASS | PASS | PASS | PARTIAL | **495** |
| **V-98** | C-50 PARTIAL: F2 only (F1 absent) | PASS | PASS | PASS | PASS | PASS | PARTIAL | **495** |
| **V-99** | C-50 PARTIAL: merged "or" sentence | PASS | PASS | PASS | PASS | PASS | PARTIAL | **495** |
| **V-100** | Gap: no function names anywhere | FAIL | PASS | FAIL | PASS | PASS | FAIL | **470** |

**Single locus of change:** V-96--V-99 vary only the C-50 paragraph appended after C-49. V-100 rewrites the gate closure block to remove all function names — "self-documenting label function" and "self-enforcing block body function" replaced by "two roles" / "two purposes" language — which simultaneously fails C-45 (no affirmative duality declaration) and C-47 (no FAIL disqualifier can be written without the function names), while C-46/C-48/C-49 remain satisfiable without those specific labels.

**Key discriminations:**
- V-97 vs V-98 — orthogonal 495 paths; tests whether rubric correctly identifies which F-shape is present
- V-96 vs V-97/V-98/V-99 — 10 pts isolates C-50 PASS vs PARTIAL
- V-100 vs V-95(R19) — both C-45 FAIL + C-46 PASS; V-95 has C-47 PASS (480), V-100 has C-47 FAIL (470); predicted 10-pt delta confirms C-47 independent of C-45
 delta: 480 vs 470.
- V-100 vs V-95(R19) -- confirms C-47 is independently scorable from C-45

**Score derivation:**

- Baseline (C-01--C-44 all PASS): 440 pts
- V-96: +10(C-45)+10(C-46)+10(C-47)+10(C-48)+10(C-49)+10(C-50) = **500**
- V-97: +10(C-45)+10(C-46)+10(C-47)+10(C-48)+10(C-49)+5(C-50) = **495**
- V-98: +10(C-45)+10(C-46)+10(C-47)+10(C-48)+10(C-49)+5(C-50) = **495**
- V-99: +10(C-45)+10(C-46)+10(C-47)+10(C-48)+10(C-49)+5(C-50) = **495**
- V-100: +0(C-45)+10(C-46)+0(C-47)+10(C-48)+10(C-49)+0(C-50) = **470**

---

### V-96: Ceiling -- C-50 PASS (500/500)
...
disqualifier sentences (C-48 PASS), (5) enumerates both C-48 PARTIAL shapes S1 and S2 as distinct
named disqualifier cases (C-49 PASS), and (6) enumerates both C-45 FAIL shapes F1
(zero-function-named) and F2 (negative-framing-only) as distinct named disqualifiers (C-50 PASS).
**Hypothesis:** C-01--C-49 = 490 pts + C-50 PASS (10) = **500/500**.
**Change from V-91:** C-50 paragraph appended after the C-49 paragraph. C-50 paragraph names F1
and F2 as distinct C-45 FAIL disqualifiers, each labeled and in its own sentence. All phases, CCV,
and AMEND identical to V-91.
...
The C-45 FAIL condition -- a block body that lacks an affirmative duality declaration of both
closure functions -- has two structurally distinct shapes. Shape F1 is the zero-function-named
shape: neither the self-documenting label function nor the self-enforcing block body function is
named anywhere in the block body -- not in any positive declaration and not in any disqualifier
sentence. F1 is a disqualifier: a block body that contains no closure function names fails C-45
entirely. Shape F2 is the negative-framing-only shape: both function names appear in the block
body, but exclusively in disqualifier-context sentences -- for example, in the C-47 FAIL sentence
("a block body naming neither function fails") or in C-48 PARTIAL sentences ("a block naming one
function without the other earns partial credit") -- with no affirmative duality declaration
present. F2 is a disqualifier: function names appearing only in negative-framing sentences,
without a positive duality assertion, fail C-45 just as F1 does. Both shapes are distinct C-45
FAIL disqualifiers. Naming F1 without F2 earns partial credit. Naming F2 without F1 earns
partial credit. A merged description of both shapes earns partial credit.
...
**Axis:** All of V-96 except C-50 paragraph names only Shape F1 (zero-function-named: neither
closure function appears anywhere in block body). Shape F2 (negative-framing-only: both function
names appear exclusively in disqualifier-context sentences) absent from enumeration (C-50 PARTIAL).
**Hypothesis:** C-01--C-49 = 490 pts + C-50 PARTIAL (5) = **495/500**.
**Change from V-96:** C-50 paragraph reduced to F1 only. F2 sentence removed. All other closure
text, all phases, CCV, and AMEND identical to V-96.
...
The C-45 FAIL condition -- a block body that lacks an affirmative duality declaration of both
closure functions -- has a structurally critical shape that must be named as an explicit
disqualifier. Shape F1 is the zero-function-named shape: neither the self-documenting label
function nor the self-enforcing block body function is named anywhere in the block body -- not
in any positive declaration and not in any disqualifier sentence. F1 is a disqualifier: a block
body that contains no closure function names fails C-45 entirely. A block body that names F1
without also enumerating the complementary FAIL shape earns partial credit on this enumeration
requirement.
...
**Axis:** All of V-96 except C-50 paragraph names only Shape F2 (negative-framing-only: both
function names appear exclusively in disqualifier-context sentences, no affirmative duality
declaration). Shape F1 (zero-function-named: neither function appears anywhere in block body)
absent from enumeration (C-50 PARTIAL). Mirror of V-97.
**Hypothesis:** C-01--C-49 = 490 pts + C-50 PARTIAL (5) = **495/500**.
**Change from V-96:** C-50 paragraph reduced to F2 only. F1 sentence removed. All other closure
text, all phases, CCV, and AMEND identical to V-96.
...
The C-45 FAIL condition -- a block body that lacks an affirmative duality declaration of both
closure functions -- has a structurally critical shape that must be named as an explicit
disqualifier. Shape F2 is the negative-framing-only shape: both function names appear in the
block body, but exclusively in disqualifier-context sentences -- for example, in the C-47 FAIL
sentence ("a block body naming neither function fails") or in C-48 PARTIAL sentences ("a block
naming one function without the other earns partial credit") -- with no affirmative duality
declaration present. F2 is a disqualifier: function names appearing only in negative-framing
sentences, without a positive duality assertion, fail C-45 entirely. A block body that names F2
without also enumerating the complementary FAIL shape earns partial credit on this enumeration
requirement.
...
**Axis:** All of V-96 except C-50 paragraph merges F1 and F2 into a single sentence using "or"
-- neither shape is labeled as a distinct named disqualifier. A block body that names neither
closure function anywhere or names both functions only in disqualifier-context sentences
constitutes the merged C-50 disqualifier description (C-50 PARTIAL).
**Hypothesis:** C-01--C-49 = 490 pts + C-50 PARTIAL (5) = **495/500**.
**Change from V-96:** C-50 paragraph replaced by a single merged sentence. Neither F1 nor F2
is labeled distinctly. All other closure text, all phases, CCV, and AMEND identical to V-96.
...
The C-45 FAIL condition covers a block body that names neither closure function anywhere in the
block body or names both functions only in disqualifier-context sentences without an affirmative
duality declaration -- either shape fails C-45 entirely and is a disqualifier.
...
### V-100: Gap -- Zero-Function-Named Body (predicted: 470)

**Axis:** Block body omits all closure function names -- neither "self-documenting label
function" nor "self-enforcing block body function" appears anywhere in the block body, including
not in C-47 FAIL disqualifier sentences and not in C-48 PARTIAL disqualifier sentences. No
affirmative duality declaration (C-45 FAIL). No FAIL disqualifier for naming neither function
(C-47 FAIL). Non-propagation between C-43 and C-44 declared without naming the functions
(C-46 PASS). C-48 PARTIAL paths described functionally without specific function labels
(C-48 PASS). C-49 S1/S2 taxonomy present (C-49 PASS). No C-50 paragraph (C-50 FAIL).
**Hypothesis:** 440 (C-01--C-44) +0(C-45)+10(C-46)+0(C-47)+10(C-48)+10(C-49)+0(C-50) = **470**.
**Structural target:** Tests whether C-47 FAIL is independently scorable from C-45 FAIL.
V-95(R19) scored 480: C-45 FAIL + C-46/C-47/C-48/C-49 all PASS (function names present in
disqualifier sentences). V-100 predicts 470: C-45 FAIL + C-47 FAIL + C-46/C-48/C-49 PASS (no
function names anywhere). Predicted 10-pt delta isolates C-47 from C-45.
**Change from V-91:** Gate closure paragraph rewritten to remove all function names. C-47
disqualifier sentence absent (cannot write it without the specific function labels). C-48 PARTIAL
paths described using "closure roles" rather than the specific function names. No C-50 paragraph.
...
The gate architecture closure has two roles serving structurally distinct purposes. The produced
architecture label (the "CITATION ARCHITECTURE DECLARED" label required below) certifies that
the block instruction body declared the gate constraints before Phase 1. The block instruction
body enforces compliance by making the gate prohibitions present and visible before Phase 1 is
executed. These are structurally distinct purposes -- the produced label certifies what the block
body declared; the block body enforces compliance before execution. A block instruction body that
enumerates only one of these two roles without the other is a disqualifier earning partial
credit -- naming one purpose omits one structural contribution and does not satisfy the duality
requirement. A block instruction body that describes both purposes without declaring them as
structurally distinct is a further separate disqualifier earning partial credit -- both purposes
present without structural differentiation fails the distinctness requirement. Both roles must be
present and declared structurally distinct to satisfy this requirement.

These two closure criteria are genuinely independent. Satisfying the produced-label
certification requirement (C-43: the produced architecture label cites the block instruction body
as the explicit source of both gate components -- axis enumeration and impossibility statement)
does not discharge the split-gate prohibition requirement (C-44: the block instruction body names
both split-gate failure modes as explicit disqualifiers in the block body). Satisfying the
...
disqualifiers) does not discharge the produced-label certification requirement (C-43: produced
architecture label cites the block body as the source of both gate components). Both closure
criteria must be independently satisfied. A block body that describes both structural purposes
without explicitly declaring this non-propagation relationship earns partial credit: describing
the duality does not substitute for declaring the independence between the two closure criteria.

The C-48 PARTIAL taxonomy -- enumerating both C-45 PARTIAL paths as separate disqualifiers --
has two structurally distinct shapes. Shape S1 is a merged "or" condition: both PARTIAL paths
combined into a single sentence using "or" -- for example, "A block instruction body that
enumerates only one closure role without the other or enumerates both roles without structural
distinctness earns partial credit." Shape S1 is a disqualifier: merging both PARTIAL paths into
one sentence fails the enumeration requirement and earns partial credit rather than full passage.
Shape S2 is a separate-sentence form using outcome language without explicit "disqualifier"
framing: the two PARTIAL paths appear in separate sentences, each using language such as "earns
partial credit," without labeling either sentence a disqualifier -- for example, "A block
instruction body that enumerates only one closure role without the other earns partial credit.
A block instruction body that enumerates both closure roles without declaring them structurally
distinct also earns partial credit." Shape S2 is a disqualifier: outcome language without
disqualifier framing fails the enumeration requirement even when paths appear in separate
sentences. Both shapes are distinct partial-credit disqualifiers. Both shapes must be named as
distinct cases to satisfy this enumeration requirement. A merged description of S1 and S2 that
does not name each shape as a distinct case is itself a partial-credit form and does not satisfy
this requirement.
