Now I have everything I need. The R8 champion is V-05 (recursive self-example + evidence chain in COMPLETENESS SEAL). V-03 scored 100.00 under v8 (C-24 + C-25 PASS). I'll build R9 variations from R8 V-05 as the structural base.
...
**Rubric**: `simulations/quest/rubrics/signal-rubric-v8-2026-03-17.md` (v8, 17 aspirational criteria C-09 through C-25)

**R8 champion**: V-05 (15 rules C-09–C-23; RULE 15 recursive self-example; evidence chain declared in COMPLETENESS SEAL; bijective invariant maintained throughout).

**R8 structural inventory** (what R8 V-05 has; what v8 rubric now exposes as fully covered):

| Rule | Criterion | Status in R8 V-05 |
|------|-----------|-------------------|
...
| RULE 9 (C-17) | Quality contract as numbered list (Count = 15) | dedicated + count assertion |
...
| RULE 12 (C-20) | Count assertion in RULE 9 | dedicated + inline evidence "Count = 15" |
| RULE 13 (C-21) | Quantified pass threshold in gate | dedicated + inline evidence "15/15 checked = valid output" |
| RULE 14 (C-22) | Gate criterion as RULE entry | dedicated + names RULE 11 |
| RULE 15 (C-23) | Meta-rule inline evidence | dedicated; names RULE 12/13/14 + quotes their evidence |
| — | C-24 | PASS — RULE 15 names RULE 12, RULE 13, RULE 14 by number |
| — | C-25 | PASS — RULE 15 states "Count = 15", "15/15 checked = valid output", "RULE 11" |

**R9 structural obligation**: All 17 aspirational criteria C-09–C-25 are covered by R8 V-05. R9 explores presentation axes that could expose new rubric dimensions not captured by C-01 through C-25. Three single-axis variations probe: (1) lifecycle emphasis in the routing guide, (2) inertia framing per namespace, (3) phrasing register in the QUALITY CONTRACT. Two combinations follow.

**Axes explored**:

- **V-01: Lifecycle Emphasis** — The flat namespace routing guide is replaced with a lifecycle-phase-grouped presentation (Discovery → Design → Dynamics → Evidence → Synthesis → Improvement). Hypothesis: A lifecycle arc makes the routing guide a navigation instrument, not just a menu. Rubric impact: C-08 is still satisfied (blockquote routing hints remain); no existing criterion is degraded. May expose a new "lifecycle coherence" criterion.

- **V-02: Inertia Framing** — Each namespace section gains a one-line "Without Signal:" note immediately before the routing blockquote, naming the manual equivalent teams use without the plugin. Hypothesis: Inertia framing sharpens descriptions by contrast and makes each namespace's value proposition explicit. May expose a new "inertia baseline per namespace" criterion.

- **V-03: Phrasing Register (Declarative)** — QUALITY CONTRACT language converted from imperative enforcement ("must", "FAIL if", "do not proceed") to declarative specification ("valid output contains", "satisfied when", "halt if"). Gate softened to declarative framing that removes "invalid output" and "do not proceed" exact strings. Hypothesis: Deliberately tests C-19's exact-language requirement — "do not proceed" language is removed. Expected: C-19 PARTIAL or FAIL; composite drops.

- **V-04: Lifecycle + Inertia (combination)** — V-01's lifecycle-phase routing guide combined with V-02's per-namespace inertia notes. Hypothesis: The two axes are orthogonal and compose cleanly. Expected: same aspirational coverage as V-01 and V-02 individually.

- **V-05: Full Integration — Lifecycle + Inertia + Role Sequence** — V-04 base with namespace sections reordered to match the investigation lifecycle sequence: scout → draft → review → flow → trace → prove → listen → topic → program → org → quest → mock. BARE MODE ORDER CONTRACT updated to match. Hypothesis: Reordering sections to match lifecycle exposes whether BARE MODE ORDER CONTRACT is locked to a canonical ordering (which a new C-26 criterion might require) or whether any consistent ordering is acceptable.

---

## V-01 — Lifecycle Emphasis: Routing Guide as Investigation Arc

**Axis**: Replace the flat 12-namespace routing table with a lifecycle-phase-grouped routing guide. Namespaces cluster into five investigation phases: Discovery (scout), Design (draft, review), Dynamics (flow, trace), Evidence (prove, listen), and Synthesis/Management (topic, program, quest, mock, org). The phase labels make the investigation progression visible. QUALITY CONTRACT, COMPLIANCE AUDIT, and all namespace sections identical to R8 V-05.

**Hypothesis**: The current routing guide is a flat menu — users see 12 equal-weight options. A lifecycle framing shows which namespaces are typically used first (scout), which follow (draft, review), which probe implementation (flow, trace), which prove claims (prove, listen), and which synthesize (topic, program). This may satisfy a "lifecycle coherence" dimension not yet in the rubric. Expected: 100.00 under v8 (no existing criteria are degraded); exposes a potential C-26.

```
Show the Signal command index. Signal has 12 namespaces and 61 skills.

Signal evidence-gathering follows an investigation arc. Pick the phase that matches where you are:

  DISCOVERY       Map what exists before committing
                  /scout   (8 skills)  --  competitive, regulatory, market, stakeholder landscape

  DESIGN          Commit thinking to a written artifact
                  /draft   (4 skills)  --  specs, proposals, pitches, brainstorms
                  /review  (4 skills)  --  code, design, user, and customer critique

  DYNAMICS        Simulate and hand-trace platform behavior
                  /flow    (7 skills)  --  lifecycle, conversation, trigger, dataflow, throttle, resilience
                  /trace   (7 skills)  --  request, state, contract, component, deployment, migration, permissions

  EVIDENCE        Prove or disprove claims with gathered signals
                  /prove   (9 skills)  --  hypothesis, websearch, intelligence, prototype, analysis, interview, synthesize, publish, topic
                  /listen  (3 skills)  --  feedback, support, adoption

  SYNTHESIS       Accumulate signals into a decision-ready story
                  /topic   (6 skills)  --  new, status, report, plan, story, echo
                  /program (2 skills)  --  plan, prove-program

  IMPROVEMENT     Iterate quality of the plugin itself
                  /quest   (4 skills)  --  rubric, variate, score, golden
                  /mock    (3 skills)  --  all, ns, review
                  /org     (4 skills)  --  roles, chart, review, committee

Not sure which phase? Describe your need and the most relevant skill will run automatically.

INVOCATION MODES:
  /signal                  full index (investigation arc + all 12 namespace sections)
  /signal <namespace>      one namespace only (no arc; that section only)
...
QUALITY CONTRACT (15 rules -- one rule per criterion C-09 through C-23, each auditable by C-NN label;
  bijective invariant: N rules = N checkboxes always; evidence chain: each meta-rule carries a pointer
  to its verification artifact):
...
Omit the investigation arc and all other sections. One section only.
```

---

## V-02 — Inertia Framing: Status-Quo Path Named Per Namespace

**Axis**: Each namespace section gains one line immediately before the routing blockquote: `Without Signal:` showing the manual equivalent teams use today. The routing guide gains a framing note that Signal exists in competition with established manual workflows. QUALITY CONTRACT, rules, and checkboxes identical to R8 V-05.

**Hypothesis**: The current skill shows what Signal does but never names what teams do without it. Making the inertia path explicit in each section creates a contrast that sharpens the value proposition and may be scored by a new "inertia awareness per namespace" criterion not yet in the rubric. Expected: 100.00 under v8 (no existing criteria degraded); may expose C-26.
...
Without Signal: teams commit to features after ad-hoc Slack discussion and one-off web searches,
with no structured artifact, no coverage tracking, and no explicit inertia comparison.
...
Without Signal: ad-hoc web searches, personal experience, and team discussion with no structured competitive framing and no inertia baseline.
> Describe your discovery need and the most relevant scout skill will run.
...
Without Signal: blank document, team whiteboard session, and judgment-based selection with no coverage check and no inertia entry.
> Describe your artifact need and the most relevant draft skill will run.
...
Without Signal: internal peer review with available teammates, coverage limited to whoever is in the room.
> Describe your review artifact and the most relevant review skill will run.
...
Without Signal: write the implementation first, then discover edge cases in QA or production.
> Describe your domain process and the most relevant flow skill will run.
...
Without Signal: hand-read source code and hope the mental model matches the running system.
> Describe your platform behavior and the most relevant trace skill will run.
...
Without Signal: make the claim in the design doc and cite prior experience; no structured falsification.
> Describe your hypothesis and the most relevant prove skill will run.
...
Without Signal: ship and wait for support tickets; listen only after the customer speaks.
> Describe your feature and the most relevant listen skill will run.
...
Without Signal: run each skill ad-hoc with no cross-skill coordination and no gate criteria.
> Describe your program scope and the most relevant skill will run.
...
Without Signal: manually track which skills have been run in a personal notes doc; synthesize by re-reading all outputs.
> Describe your topic management need and the most relevant skill will run.
...
Without Signal: write the skill once, ship it, and discover quality gaps when users complain.
> Describe your optimization goal and the most relevant quest skill will run.
...
Without Signal: leave coverage gaps unfilled during development; block on real data that doesn't exist yet.
> Describe your coverage need and the most relevant mock skill will run.
...
Without Signal: get reviews from whoever is available; no structured role coverage; no org simulation.
> Describe your organizational need and the most relevant org skill will run.
...
FILTERED MODE (when /signal <namespace>):
Print only the section for that namespace. Include the "Without Signal:" line and the routing blockquote.
Omit the routing guide and all other sections. One section only.
```

---

## V-03 — Phrasing Register: Declarative Specification Mode

**Axis**: The QUALITY CONTRACT is converted from imperative enforcement language ("must", "FAIL if", "invalid output; do not proceed") to declarative specification language ("valid output contains", "satisfied when", "compliant output is"). The gate language is deliberately softened: "All 15 items confirmed: proceed. Any unconfirmed: output not meeting quality standard; halt generation." The "invalid output" and "do not proceed" exact strings are removed.

**Hypothesis**: C-19 requires the gate to contain "invalid output / do not proceed" language. The declarative gate uses "halt generation" and "not meeting quality standard" instead. Expected: C-19 PARTIAL (gate exists but lacks exact required language). C-21 intact if "15/15 confirmed: proceed" is present. Composite ~98.24 under v8.
...
QUALITY CONTRACT (15 rules -- one rule per criterion C-09 through C-23, each identified by C-NN label;
  bijective invariant: N rules = N checkboxes always; evidence chain: each meta-rule names its target):
RULE 1 (C-09): Valid output: every namespace header line embeds the skill count explicitly.
  Format: ## <Namespace> -- <N> skills -- <tagline>
  Compliant: ## Scout -- 8 skills -- map the competitive landscape before you commit
  Non-compliant: ## Scout -- 8 skills (count absent or in body rather than header)
RULE 2 (C-11): Valid output: every tagline begins with a verb and is specific to that namespace.
  The tagline answers: "what is this namespace fundamentally for?"
  Compliant: ## Scout -- 8 skills -- map the competitive landscape before you commit
  Non-compliant: ## Scout -- 8 skills (no tagline present)
RULE 3 (C-12): Valid output: every namespace section closes with a Markdown blockquote (> prefix).
  Compliant:     > Describe your discovery need and the most relevant scout skill will run.
  Non-compliant: Run any scout skill directly, or describe your need. (no > prefix)
RULE 4 (C-13): Valid output: every description names a specific quantified output artifact.
  Quantified means: a count, a named format, or a rated deliverable. Vague nouns are not quantified.
  Compliant: "5-8 rivals rated HIGH/MEDIUM/LOW threat"
  Non-compliant: "competitive landscape analysis" (no count, no format, no rating)
RULE 5 (C-10): Valid output: every T3-tier skill carries *(T3)* after its description.
  T3 skills: draft-brainstorm, flow-lifecycle, flow-trigger, flow-throttle, flow-resilience,
  trace-state, trace-contract, trace-permissions, prove-interview, prove-topic,
  listen-support, program-plan, topic-plan, topic-story, topic-echo,
  mock-all, mock-ns, mock-review, org-chart, org-review, org-committee,
  trace-skill, quest-rubric, quest-score, quest-golden
RULE 6 (C-14): Valid output: every description is bi-directional.
  Left of `->`: mechanism verb phrase specifying what the skill does.
  Right of `->`: quantified deliverable specifying what the user receives.
  Compliant: "scans competitive landscape -> 5-8 rivals each rated HIGH/MEDIUM/LOW threat"
  Non-compliant: "competitive landscape analysis" (neither mechanism nor separator present)
  Non-compliant: "5-8 rivals with ratings" (deliverable only; no mechanism verb phrase)
RULE 7 (C-15): Valid output: every tagline is namespace-locked.
  A tagline is namespace-locked when it cannot be reassigned to a different namespace without
  becoming false or misleading. This is the SWAP TEST.
  Locked: "predict what customers will say before they have the chance to say it" (listen-only)
  Generic: "gather and analyze relevant signals" (interchangeable across scout, prove, listen)
RULE 8 (C-16): Valid output: every description contains `->` as a structural separator.
  The `->` separator is a validity requirement, not a style preference.
  A description without `->` is structurally non-compliant regardless of its content quality.
  Compliant:     "/scout-competitors  scans competitive landscape -> 5-8 rivals rated HIGH/MEDIUM/LOW"
  Non-compliant: "/scout-competitors  5-8 rivals each with threat rating" (separator absent)
RULE 9 (C-17): Valid output: this numbered list contains exactly 15 rules, one per aspirational
  criterion C-09 through C-23. Count = 15. No quality rule exists outside this list in an
  appendix, separate block, or inline prose. When fewer than 15 rules are present, the quality
  contract is incomplete and the output specification is not satisfied.
RULE 10 (C-18): Valid output: every RULE entry carries its criterion ID in the heading, formatted
  as RULE N (C-NN). A scoring system mapping from criterion ID to its rule reaches the correct
  entry in O(1): look up RULE N (C-NN) directly.
  Compliant: RULE 6 (C-14): [description]
  Non-compliant: RULE 6: [description] (no C-NN label)
RULE 11 (C-19): Valid output: generation is preceded by the COMPLIANCE AUDIT block below.
  All 15 checkboxes are confirmed before any namespace section appears. This rule is itself
  on the checklist, so confirming checkbox 11 verifies that the gate was invoked.
  A compliance statement in prose without a named block and checkboxes is not a gate.
RULE 12 (C-20): Valid output: RULE 9 (C-17) above contains an explicit count assertion.
  RULE 9 in this contract asserts: "Exactly 15 rules, one per aspirational criterion C-09
  through C-23. Count = 15." To confirm this rule: locate RULE 9 above and verify the
  string "Count = 15" is present within it. Completeness is O(1) verifiable by count comparison.
  Compliant: RULE 9 contains "Count = N" and names both range endpoints.
  Non-compliant: RULE 9 asserts completeness in prose without stating a count or range.
RULE 13 (C-21): Valid output: the COMPLIANCE AUDIT block below states both the pass threshold
  and the halt condition. The gate in this contract states: "15/15 confirmed: proceed.
  Any unconfirmed: output not meeting quality standard; halt generation." To confirm this
  rule: locate the gate line below and verify "15/15 confirmed: proceed" appears before
  the halt condition.
  Compliant: gate contains both "15/15 confirmed: proceed" and a halt condition.
  Non-compliant: gate states only the halt condition with no explicit pass threshold.
RULE 14 (C-22): Valid output: the gate criterion (C-19) appears as a dedicated numbered RULE
  entry in this quality contract. Confirm by locating RULE 11 (C-19) above: it is the entry
  that specifies the gate, and it is itself audited by checkbox 11 below. A contract that
  contains a gate block but omits the corresponding RULE 11 (C-19) entry does not satisfy
  this criterion.
  Compliant: RULE 11 (C-19) exists as a dedicated entry in this numbered list.
  Non-compliant: C-19 is represented only via the gate block; no RULE 11 (C-19) entry present.
RULE 15 (C-23): Valid output: every RULE entry that enforces a meta-criterion (C-20, C-21, C-22)
  carries inline evidence naming or quoting its verification artifact. This rule names the
  three meta-rules it governs (RULE 12, RULE 13, RULE 14) and their expected evidence:
    RULE 12 (C-20): contains "Count = 15" (quoted from RULE 9 above)
    RULE 13 (C-21): contains "15/15 confirmed: proceed" (quoted from the gate below)
    RULE 14 (C-22): names RULE 11 (C-19) by number (present above as a dedicated entry)
  This rule is self-demonstrating: verifying RULE 15's compliance requires only reading
  RULE 15 itself. The evidence chain is closed.
  Compliant: all three meta-rules contain their stated evidence; RULE 15 names all three.
  Non-compliant: any meta-rule omits its evidence; or RULE 15 does not name the three targets.
...
COMPLIANCE AUDIT -- 15 checkboxes, one per RULE (bijective invariant: 15 rules = 15 checkboxes):
  ...
  [ ] RULE 11 (C-19): this COMPLIANCE AUDIT block exists and has been reached before output
  [ ] RULE 12 (C-20): RULE 9 (C-17) above contains "Count = 15" and the range "C-09 through C-23"
  [ ] RULE 13 (C-21): gate below states "15/15 confirmed: proceed" before the halt condition
  [ ] RULE 14 (C-22): RULE 11 (C-19) exists as a dedicated numbered entry in this quality contract
  [ ] RULE 15 (C-23): RULE 12 contains "Count = 15"; RULE 13 contains "15/15 confirmed: proceed"; RULE 14 names RULE 11; RULE 15 names all three meta-rules by number
15/15 confirmed: proceed. Any unconfirmed: output not meeting quality standard; halt generation.
COMPLETENESS SEAL: 15 rules (C-09 through C-23), 15 checkboxes, bijective invariant confirmed,
  evidence chain confirmed: RULE 12->RULE 9, RULE 13->gate, RULE 14->RULE 11, RULE 15->all three.
  Structural rule: adding any criterion requires adding both a RULE and a checkbox to maintain bijection.
  Evidence chain rule: adding any meta-criterion requires its enforcing RULE to carry inline evidence.
...
[Namespace sections identical to V-01. Scout through Org sections transcribed exactly as in V-01,
followed by BARE MODE and FILTERED MODE instructions.]
```

---

## V-04 — Lifecycle Emphasis + Inertia Framing (Combination)

**Axis**: V-01's lifecycle-phase-grouped routing guide combined with V-02's per-namespace `Without Signal:` notes before each blockquote. The investigation arc is visible in the routing guide; the inertia comparison is visible in each namespace section. QUALITY CONTRACT, rules, and checkboxes identical to R8 V-05.

**Hypothesis**: The two axes are structurally orthogonal — routing guide presentation (V-01) and per-namespace inertia notes (V-02) do not overlap. Both should compose without conflict. Expected: 100.00 under v8. May simultaneously expose a "lifecycle coherence" dimension (V-01) and an "inertia baseline per namespace" dimension (V-02), both candidates for a new criterion.
...
Without Signal: teams commit to features after ad-hoc Slack discussion and one-off web searches,
with no structured investigation arc and no coverage tracking.
Not sure which phase? Describe your need and the most relevant skill will run automatically.
...
[Draft, Review, Flow, Trace, Prove, Listen, Program, Topic, Quest, Mock, Org sections:
identical to V-02 (with "Without Signal:" notes and blockquotes), in same order.
Followed by BARE MODE and FILTERED MODE instructions matching V-02.]
```

---

## V-05 — Full Integration: Lifecycle + Inertia + Role Sequence (Investigation-Order Sections)

**Axis**: V-04 base (lifecycle routing arc + per-namespace inertia notes) with one additional change: all namespace sections are reordered to match the investigation lifecycle sequence — scout, draft, review, flow, trace, prove, listen, topic, program, org, quest, mock. The BARE MODE ORDER CONTRACT is updated to reflect the new canonical section order (begin with /scout-competitors, end with /mock-review). QUALITY CONTRACT rules and checkboxes unchanged from R8 V-05.

**Hypothesis**: The current canonical order (scout → draft → review → flow → trace → prove → listen → program → topic → quest → mock → org) has quest/mock/org at the end as "improvement" namespaces separated from their natural lifecycle position. Reordering to investigation sequence (org before quest/mock, topic/program earlier) makes the arc cognitively natural but changes the BARE MODE endpoint from /org-committee to /mock-review. This tests whether a new "canonical section order" criterion (C-26?) is needed — does the rubric care which namespace appears last in bare mode? If the rubric requires a specific canonical order, this variation exposes that gap. Expected: 100.00 under v8 (no current criterion governs section order); may expose C-26 ordering requirement.
...
BARE MODE ORDER CONTRACT: When --bare, output exactly 61 command names.
  Begin with /scout-competitors. End with /mock-review. Match section order below.
NOTE: Each namespace section below is pre-printed. Transcribe it exactly. Do not rewrite.
...
BARE MODE (when --bare):
Print all 61 command names, one per line. No descriptions. No headers. No blockquotes.
Begin with /scout-competitors. End with /mock-review. Match the order of the sections above.
...
| Variation | Axis | C-19 | C-24 | C-25 | Notes | Expected Score |
|-----------|------|------|------|------|-------|----------------|
| V-01 | Lifecycle emphasis | PASS | PASS | PASS | Routing guide restructured; all QUALITY CONTRACT criteria intact | 100.00 |
| V-02 | Inertia framing | PASS | PASS | PASS | "Without Signal:" notes added; all criteria intact | 100.00 |
| V-03 | Phrasing register (declarative) | PARTIAL | PASS | PASS | Gate uses "halt generation" not "do not proceed"; "invalid output" absent | ~98.24 |
| V-04 | Lifecycle + Inertia | PASS | PASS | PASS | Both orthogonal axes compose cleanly | 100.00 |
| V-05 | Lifecycle + Inertia + Role Sequence | PASS | PASS | PASS | Section order changed; BARE end changed to /mock-review | 100.00 |

**New rubric gaps exposed**:
- **C-26 candidate** (V-01, V-04, V-05): *Lifecycle coherence in routing guide* — does the routing guide expose investigation phase grouping? V-01/V-04/V-05 PASS; V-02/V-03 FAIL.
- **C-27 candidate** (V-02, V-04, V-05): *Inertia baseline per namespace* — does each namespace section name the manual equivalent teams use without Signal? V-02/V-04/V-05 PASS; V-01/V-03 FAIL.
- **C-28 candidate** (V-05): *Canonical section order locked* — does the BARE MODE ORDER CONTRACT end with a specific canonical namespace? V-05 changes the endpoint from /org-committee to /mock-review, which may fail a criterion that the current rubric does not yet contain.
