Written to `simulations/quest/variations/validate-inertia-variations-R17-2026-03-17.md`.

---

## R17 Variations -- V-81 through V-85

| # | Label | C-45 | C-46 | Predicted |
|---|-------|------|------|-----------|
| **V-81** | Ceiling: duality declared + both non-propagation directions | PASS | PASS | **460** |
| **V-82** | Mirror non-propagation direction omitted | PASS | PARTIAL | **455** |
| **V-83** | Both functions named but not declared structurally distinct | PARTIAL | PASS | **455** |
| **V-84** | Duality declared, no non-propagation | PASS | FAIL | **450** |
| **V-85** | One function named, both non-propagation directions | FAIL | PASS | **450** |

---

**Structural manipulation:** All 5 variations are single-locus changes in the AUDIT ARCHITECTURE DECLARED block body, appended after the split-gate disqualifiers (C-44 PASS throughout). C-43 PASS and C-01--C-42 are held constant.

**Dimension 1 (C-45):** V-81/V-82/V-84 name both closure functions with explicit "structurally distinct roles" framing (PASS). V-83 names both but drops the distinctness declaration (PARTIAL). V-85 names only the self-documenting label function, omitting the self-enforcing block body function (FAIL).

**Dimension 2 (C-46):** V-81/V-83/V-85 declare both non-propagation directions (PASS). V-82 declares only "C-43 PASS does not discharge C-44" without the mirror (PARTIAL). V-84 has no non-propagation language at all (FAIL).

**Key tests:**
- **V-84** is the critical proof that C-45 PASS does not imply C-46 PASS -- naming the duality doesn't discharge the independence requirement.
- **V-85** is the complementary inversion -- C-46 PASS achievable without C-45, by declaring both non-propagation directions using criterion descriptions directly.
- **V-82/V-83** form a 455-pair with orthogonal failure axes (C-46 PARTIAL vs C-45 PARTIAL), same score by different structural paths.
the mirror (PARTIAL).
V-84 declares neither direction (FAIL -- duality named in C-45 but no independence language).

**Key discriminators:**
- V-82 and V-83 both score 455 but fail on different axes -- C-46 PARTIAL vs C-45 PARTIAL.
- V-84 and V-85 both score 450 as complementary inversions: C-45 PASS + C-46 FAIL vs
  C-45 FAIL + C-46 PASS. These are the primary discrimination tests for the two new criteria.
- V-84 is the critical test for C-45 not implying C-46: naming the duality as structurally
  distinct roles does NOT discharge the requirement to declare the non-propagation relationship.
- V-85 demonstrates C-46 can pass independently of C-45: both non-propagation directions can
  be declared even when only one closure function is named as a distinct role.

**All five variations hold constant:** C-43 PASS, C-44 PASS, C-01--C-42 identical to V-76.
Only the C-45/C-46 paragraph appended after the split-gate disqualifiers changes.

---

### V-81: Ceiling -- C-45 PASS + C-46 PASS (460/460)

**Axis:** Block body (1) places both gate components in the block body (C-41/C-42 PASS), (2)
names both split-gate failure modes as explicit disqualifiers (C-44 PASS), (3) names the gate
closure as a duality of self-documenting label function and self-enforcing block body function
declared as structurally distinct roles (C-45 PASS), and (4) declares both directions of
non-propagation between C-43 and C-44 explicitly (C-46 PASS). Produced architecture label
identical to V-76 (C-43 PASS).
**Hypothesis:** C-41 PASS (10) + C-42 PASS (10) + C-43 PASS (10) + C-44 PASS (10) +
C-45 PASS (10) + C-46 PASS (10) + C-01--C-40 = 400 pts. Ceiling: 460/460.
**Change from V-76:** Block body gate check section extended after the split-gate disqualifiers
with (a) a duality paragraph naming both closure functions as structurally distinct roles and
(b) a non-propagation paragraph declaring both directions of C-43/C-44 independence. Produced
architecture label, all phases, CCV, and AMEND identical to V-76.
...
### V-82: C-46 PARTIAL -- Mirror Non-Propagation Direction Omitted

**Axis:** Block body names both closure functions as structurally distinct roles (C-45 PASS)
and names both split-gate failure modes as explicit disqualifiers (C-44 PASS). The
non-propagation paragraph declares only one direction: "C-43 PASS does not discharge C-44"
without its mirror "C-44 PASS does not discharge C-43". C-46 PARTIAL because one direction
of non-propagation is declared and one is absent. Produced architecture label identical to V-76
(C-43 PASS).
**Hypothesis:** C-41 PASS (10) + C-42 PASS (10) + C-43 PASS (10) + C-44 PASS (10) +
C-45 PASS (10) + C-46 PARTIAL (5) + C-01--C-40 = 400 pts. Total predicted: 455/460.
**Change from V-81:** Non-propagation paragraph retains "C-43 PASS does not discharge C-44"
but removes the mirror sentence "C-44 PASS does not discharge C-43". C-46 rubric: declaring
one direction without its mirror earns PARTIAL. C-45 duality paragraph identical to V-81.
All phases, CCV, and AMEND identical to V-76.
...
as the explicit source of both gate components -- axis enumeration and impossibility statement)
does not discharge the split-gate prohibition requirement (C-44: the block instruction body names
both split-gate failure modes as explicit disqualifiers in the block body). Both closure criteria
must be independently satisfied. A block body that names both closure functions (satisfying the
duality requirement above) without explicitly declaring this non-propagation relationship earns
partial credit: naming the duality does not substitute for declaring the independence between
the two closure criteria.
...
### V-83: C-45 PARTIAL -- Both Functions Named, Not Declared Structurally Distinct

**Axis:** Block body names both closure functions (self-documenting and self-enforcing) but
does not declare them as structurally distinct roles -- the duality paragraph names both
functions without the "structurally distinct" framing required for C-45 PASS. C-45 PARTIAL.
Block body declares both directions of non-propagation (C-46 PASS). Produced architecture
label identical to V-76 (C-43 PASS). Both split-gate disqualifiers present (C-44 PASS).
**Hypothesis:** C-41 PASS (10) + C-42 PASS (10) + C-43 PASS (10) + C-44 PASS (10) +
C-45 PARTIAL (5) + C-46 PASS (10) + C-01--C-40 = 400 pts. Total predicted: 455/460.
**Change from V-81:** Duality paragraph names both functions ("self-documenting" and
"self-enforcing") but replaces the structural distinctness declaration with a descriptive
list -- no "structurally distinct roles" framing, no statement that one cannot discharge the
other. C-45 rubric: naming both functions without declaring them as structurally distinct roles
earns PARTIAL. Non-propagation paragraph identical to V-81 (both directions, C-46 PASS).
All phases, CCV, and AMEND identical to V-76.
...
The gate architecture closure has two functions. The produced architecture label (the
"CITATION ARCHITECTURE DECLARED" label required below) serves a self-documenting function:
it certifies that the block instruction body named all three axes by label and declared partial
axis coverage a gate failure. The block instruction body serves a self-enforcing function: it
declares the gate prohibitions before any production phase begins. A block body that names only
the self-documenting label function without the self-enforcing block body function earns partial
credit. A block body that names only the self-enforcing block body function without the
self-documenting label function also earns partial credit.
...
architecture label cites the block body as the source of both gate components). Both closure
criteria must be independently satisfied. A block body that names both closure functions
without explicitly declaring this non-propagation relationship earns partial credit: naming
the functions does not substitute for declaring the independence between the two closure criteria.
...
### V-84: C-45 PASS + C-46 FAIL -- Duality Declared, No Non-Propagation

**Axis:** Block body names both closure functions as structurally distinct roles (C-45 PASS)
and names both split-gate failure modes as explicit disqualifiers (C-44 PASS). No
non-propagation language of any kind -- neither direction declared (C-46 FAIL). This is the
critical test that C-45 PASS does not imply C-46 PASS: naming the gate closure as a duality
of structurally distinct roles does not discharge the requirement to explicitly declare the
independence between the two closure criteria.
**Hypothesis:** C-41 PASS (10) + C-42 PASS (10) + C-43 PASS (10) + C-44 PASS (10) +
C-45 PASS (10) + C-46 FAIL (0) + C-01--C-40 = 400 pts. Total predicted: 450/460.
**Change from V-81:** Non-propagation paragraph removed entirely. C-45 duality paragraph
identical to V-81. Produced architecture label, all phases, CCV, and AMEND identical to V-76.
...
### V-85: C-45 FAIL + C-46 PASS -- One Function Named, Both Non-Propagation Directions

**Axis:** Block body names only the self-documenting label function -- the self-enforcing
block body function is not named as a distinct structural role (C-45 FAIL). Block body
declares both directions of non-propagation between C-43 and C-44 explicitly (C-46 PASS),
using the functional descriptions of each criterion directly rather than the duality names.
Complementary inversion of V-84: same total score (450), different failure axis.
**Hypothesis:** C-41 PASS (10) + C-42 PASS (10) + C-43 PASS (10) + C-44 PASS (10) +
C-45 FAIL (0) + C-46 PASS (10) + C-01--C-40 = 400 pts. Total predicted: 450/460.
**Change from V-81:** Duality paragraph retains only the self-documenting label function;
self-enforcing block body function is not named. C-45 rubric: naming only one of the two
structural roles earns FAIL (not PARTIAL -- only one function named, not both named without
distinction). Non-propagation paragraph declares both directions using criterion descriptions
rather than duality labels, satisfying C-46. Produced architecture label, all phases,
CCV, and AMEND identical to V-76.
...
Satisfying the produced-label certification requirement (the produced architecture label cites
the block instruction body as the explicit source of both gate components -- axis enumeration
and impossibility statement) does not discharge the split-gate prohibition requirement (the block
instruction body names both split-gate failure modes as explicit disqualifiers in the block body).
Satisfying the split-gate prohibition requirement (the block instruction body names both
split-gate disqualifiers) does not discharge the produced-label certification requirement (the
produced architecture label cites the block body as the source of both gate components). Both
closure criteria must be independently satisfied. Neither criterion's satisfaction implies the
