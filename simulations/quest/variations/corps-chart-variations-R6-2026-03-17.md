...
INPUT SOURCES:
- Read all role files from `.roles/` (if present). These are your ROLES-LOADED set.
- If `.roles/` is absent or empty, ROLES-LOADED is empty; note this as ROLES-NOTE.
...
Read all files in `.roles/`. These become ROLES-LOADED. If absent, note ROLES-NOTE and continue.
...
Read all files in `.roles/`. These are ROLES-LOADED. If absent, note ROLES-NOTE.
...
Read `.roles/` and collect all role files as ROLES-LOADED. If absent, note ROLES-NOTE.
...
Read `.roles/` to collect ROLES-LOADED. If absent, note ROLES-NOTE and continue.
