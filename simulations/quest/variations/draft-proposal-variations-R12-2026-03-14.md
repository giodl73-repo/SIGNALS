I have the complete Round 12 variate content from the existing file. Here are the 5 variations:

---

# draft-proposal Variate — Round 12

**Rubric version:** v12 · Formula: /28 · New criteria: C-34 (Phase 9b domain-indexed labels), C-35 (Phase 9b back-fill arrow notation)

**Single-axis variations first (V-01..V-03), then combinations (V-04..V-05).**

**Axes explored:**
- **V-01**: Role sequence (Architect-first, no register linkage) — designed fail: C-23 cascade → C-26, C-31, C-32, C-33, C-34, C-35 (seven fails)
- **V-02**: Output format (compact tables, PM-first) — designed fail: C-22 (C-34 + C-35 PASS in this variation)
- **V-03**: Lifecycle emphasis (stale trigger count: 31 rows) — designed fail: C-24 + C-32 + C-33 + C-34 + C-35
- **V-04**: Phrasing register (conversational) + C-35 isolation — designed fail: C-25 + C-33 + C-35 (C-34 PASS via ordinal "(1)"/"(2)" prefix form)
- **V-05**: Inertia framing + aggregate Phase 9b — designed fail: C-32 + C-34 + C-35 (C-33 PASS)

**Predicted scores:** V-02 → 99.64 · V-04 → 98.93 · V-05 → 98.93 · V-03 → 98.21 · V-01 → 97.50

---

## V-01 — Architect-First

**Axis:** Role Sequence
**Hypothesis:** An Architect-led variation that assesses risk inline per option using P×I notation, without a separate register-linkage phase, will fail C-23 (no R-NN in RISK fields). Because no R-NN usage exists anywhere in the document, no Phase 9b is authored (C-26 fail), no forward-reference placeholder pattern is possible (C-31 fail by cascade), no Phase 9b exists to enumerate citation sites (C-32 fail by cascade), no domain-index labels are possible (C-34 fail by cascade), no per-site arrow notation is possible (C-35 fail by cascade), and no Phase 2 template has placeholder syntax adjacent to a prohibition instruction (C-33 fail by cascade). Seven open triggers: T-23, T-26, T-31, T-32, T-33, T-34, T-35. Score: 21/28 → 97.50.
...
## V-02 — PM-First, Compact Tables

**Axis:** Output Format
**Hypothesis:** A compact-table variation with PM leading will omit the CLOSED BY column from the coverage table (treating it as metadata overhead in a compact format), failing C-22. All register-linkage, propagation, prohibition, dual-domain enumeration, domain-index labeling, and per-site arrow notation behavior is intact: C-33 PASS, C-32 PASS, C-34 PASS (Phase 9b uses "Domain 1 —" and "Domain 2 —" numeric index prefixes), C-35 PASS (shows "[R-NN pending] → [applicable R-NN entries, P×I scores]" per citation site), C-31 PASS. One open trigger: T-22. Score: 27/28 → 99.64.
...
## V-03 — Lifecycle-Heavy

**Axis:** Lifecycle Emphasis
**Hypothesis:** A variation emphasizing explicit lifecycle gate checks at every phase transition will build its amendment table from the v10 rubric criteria list (C-01..C-31, producing 31 rows) rather than the current v12 count of 35. T-24 fires immediately at PRE-DOCUMENT (31 != 35). Phase 9b enumerates "each citation site visited by option label" without naming the comparison matrix risk column as a second structural domain, and without domain-index prefixes or per-site arrow notation (C-32 FAIL, C-34 FAIL, C-35 FAIL). Phase 2 RISK carries "[R-NN pending] — placeholder" with no adjacent prohibition instruction (C-33 FAIL). T-32 through T-35 absent from 31-row table; behavioral fails not captured as trigger findings. One finding: Finding 1: T-24. Score: 23/28 → 98.21.
...
## V-04 — Conversational Register + C-35 Isolation

**Axis:** Phrasing Register
**Hypothesis:** A conversational-register variation will perform finalization as continuous prose narrative, explicitly avoiding numbered steps (C-25 FAIL). Phase 9b uses "(1) Option RISK fields by option label" and "(2) Comparison matrix risk column" — two named structural domains with ordinal-indexed prefixes (C-32 PASS, C-34 PASS via "(1)"/"(2)" form). Phase 9b states only the outcome at each option site without showing the [R-NN pending] → [R-NN IDs] state transition (C-35 FAIL). Phase 2 RISK carries placeholder but no adjacent prohibition (C-33 FAIL). Three open triggers: T-25, T-33, T-35. Score: 25/28 → 98.93.
...
## V-05 — Inertia-Forward + Aggregate Phase 9b

**Axis:** Inertia Framing
**Hypothesis:** A variation that foregrounds the status-quo option and uses back-fill mode (C-31 PASS: placeholder at Phase 2; C-33 PASS: prohibition instruction adjacent to placeholder directive) will fail C-32, C-34, and C-35 when Phase 9b confirms back-fill with an aggregate statement ("all [R-NN pending] placeholders resolved; back-fill complete") rather than a structural-domain enumeration. The aggregate form names neither option RISK fields by label nor the comparison matrix risk column as a named structural domain (C-32 FAIL). Because no domains are enumerated, no domain-index prefixes can be present (C-34 FAIL by necessity). Because the aggregate form has no per-site entries, no site can show the [R-NN pending] → [R-NN IDs] transition notation (C-35 FAIL). Three open triggers: T-32, T-34, T-35. Score: 25/28 → 98.93.
...
| Variation | Axis | v12 Fails | Score | Composite |
|-----------|------|-----------|-------|-----------|
...
**Key isolation evidence from R12:**
- **V-01 cascade (7 fails):** No R-NN usage (C-23) → no Phase 9b (C-26) → no placeholder (C-31) → all Phase 9b-dependent criteria fail by cascade (C-32, C-33, C-34, C-35)
- **V-02 as C-34+C-35 exemplar:** The only variation where both C-34 PASS and C-35 PASS simultaneously, with arrow notation per domain site and numeric-indexed domain headers
- **V-04 C-35 isolation:** Domain-indexed "(1)"/"(2)" satisfies C-34; outcome-only per-site output ("the R-NN IDs applied to its RISK field") fails C-35 — cleanest isolation of the pair
- **V-05 aggregate collapse:** Single aggregate "back-fill complete" causes C-32 + C-34 + C-35 to co-fail — confirms all three require structural enumeration, not summary confirmation
