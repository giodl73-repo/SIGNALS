...
**Category A — Standard roles**
Path: `.roles/{area-slug}/{role-slug}.md`. Frontmatter: `role-type: standard`, `area: {area-slug}`.
...
After all teams: shared-group to `.roles/shared/`, exec-office to `.roles/exec-office/`.
...
INVARIANT-4 (directory fidelity): `.roles/` subdirectory names match `org.yaml` area
  slugs one-to-one.
...
**Category A — Standard roles**
`.roles/{area-slug}/{role-slug}.md`. Frontmatter: `role-type: standard`, `area: {area-slug}`.
...
`.roles/{area-slug}/inertia-advocate.md`. Frontmatter: `role-type: inertia-advocate`.
...
**Category A — Standard roles**
`.roles/{area-slug}/{role-slug}.md`. `role-type: standard`, `area: {area-slug}`.
All F-1..F-5 with substantive body text.
...
**Category C — Inertia Advocate** (INVARIANT-3: mandatory per team)
`.roles/{area-slug}/inertia-advocate.md`. `role-type: inertia-advocate`.
...
COVERAGE-AUDIT:
  ...
  [B] IA per team vs INVARIANT-3
      [team]: [PRESENT at .roles/{area}/inertia-advocate.md | MISSING]
      Status:   [PASS | FAIL]
