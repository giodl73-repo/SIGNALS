...
Scan .craft/roles/ for all role definition files.

  ROLES-LOADED: [role name, role name, ...]

If directory absent or empty:

  ROLES-NOTE: No .craft/roles/ directory found. Using domain-inferred roles:
  [list].
...
Generate an org structure for a product or domain. Read .craft/roles/ first.
...
Scan .craft/roles/ for all role definition files. Collect every role name.

  ROLES-LOADED: [role name, role name, ...]

If directory absent:

  ROLES-NOTE: No .craft/roles/ found. Using: [list].
