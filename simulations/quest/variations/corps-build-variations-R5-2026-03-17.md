...
by group/area, and level distribution; (2) a .craft/roles/
...
INV-1  PATH FIDELITY: Every role file is written to
       .craft/roles/{area-slug}/{role-slug}.md where
...
For each team in write-order (INV-2):
  ...
  a. Write all STANDARD roles first.
     ...
     Each file: .craft/roles/{area-slug}/{role-slug}.md
...
.craft/roles/{area}/{role}.md for every role.
...
Process teams in org.yaml order. For each team:
  ...
  Path: .craft/roles/{area-slug}/{role-slug}.md (INV-A)
  role-type: standard
...
typed role file per role at .craft/roles/{area}/{role}.md.
...
INV-1  PATH PROHIBITION: NEVER write a file to any path
       ...
       .craft/roles/{area-slug}/{role-slug}.md. Validate
...
Two outputs:
  ...
  OUTPUT-2: .craft/roles/{area}/{role}.md — one typed file
...
--- STANDARD PHASE ---
Write one file per standard role.
Path: .craft/roles/{area-slug}/{role-slug}.md (INV-1)
...
INV-S2  PATH DERIVATION: All file paths derived from
        org.yaml slugs. Pattern: .craft/roles/{area-slug}/
...
Path Prefix column: .craft/roles/{area-slug}/ for each team.
