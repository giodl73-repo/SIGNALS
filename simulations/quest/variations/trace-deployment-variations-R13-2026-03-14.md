...
**Single-axis** (V-01–V-03):
- **Phrasing register**: question-form disqualifier on prose path (C-30 axis)
- **Output format**: declarative disqualifier at compressed density (C-29 + C-17 joint)
- **Lifecycle emphasis + register**: template-path ceiling via interrogative headers (C-26) + rollback triple (C-27) + colloquial bare labels (C-21/C-22/C-24) + C-14/C-16/C-18

**Combination** (V-04–V-05):
- Inertia framing + question-form disqualifier (C-23 ∩ C-30 orthogonality test)
- Compressed density + question-form disqualifier (C-17 ∩ C-30 joint)
...
**Hypothesis**: Question-form disqualifiers ("a check that asks 'is the environment ready?' names no specific condition") pass C-19, C-20, C-30. C-15 passes via prose. C-17 fails (standard density). C-29 fails (question-form source, not declarative).

**Predicted**: 140/200 — C-01–C-13, C-15, C-19, C-20, C-25, C-30
...
## V-02: Declarative domain-contextual disqualifier — compressed prose (C-17 + C-29 joint)

**Axis**: Output format — all phase instructions collapsed to a single flowing paragraph; disqualifier is declarative
**Hypothesis**: A single compressed instruction paragraph with declarative domain-contextual disqualifier ("'keep an eye on error rates' names no probe or threshold and does not qualify") passes C-15, C-17, C-19, C-20, C-29 simultaneously. C-30 fails (declarative source). C-17 passes because the paragraph is below single-paragraph-per-phase density.

**Predicted**: 145/200 — C-01–C-13, C-15, C-17, C-19, C-20, C-25, C-29
...
**Axis**: Lifecycle emphasis + phrasing register — targets the 165/200 template-path ceiling via C-26/C-27/C-28 (interrogative + rollback triple joint) plus C-21/C-22/C-24 (colloquial bare labels joint) plus C-14/C-16/C-18 (front-loaded gate-free skeleton)
**Hypothesis**: A colloquial bare-label template with interrogative phase headers and exactly three rollback fields satisfies C-21, C-22, C-24, C-26, C-27, C-28 simultaneously — the template-path ceiling. All prose-path criteria (C-15/C-17/C-19/C-20/C-23/C-29/C-30) fail because template apparatus is present.

**Predicted**: 165/200 — C-01–C-14, C-16, C-18, C-21, C-22, C-24, C-25, C-26, C-27, C-28
...
**Axis**: Inertia framing + phrasing register — role block contains incident narrative (C-23 axis); disqualifier in prose is question-form (C-30 axis); tests orthogonality
**Hypothesis**: Role-block inertia narrative ("After the catalog sync outage we learned...") plus question-form disqualifier outside the role block passes C-20 (question-form carries no incident narrative), C-23 (C-20 passes and role block has inertia), and C-30 (question-form expression) simultaneously. C-25 fails (role block contains incident narrative). C-29 fails (question-form disqualifier). Orthogonality of C-23 and C-30 confirmed.

**Predicted**: 140/200 — C-01–C-13, C-15, C-19, C-20, C-23, C-30
...
**Axis**: Output format + phrasing register — single compressed instruction block (C-17) with question-form disqualifiers (C-30); tests whether question-form expression compresses more naturally than declarative form
**Hypothesis**: Question-form disqualifiers ("a check that asks 'is everything ready?' names no condition") are naturally shorter than declarative equivalents and achieve all three C-15 structural requirements at below-single-paragraph-per-phase density. C-17 and C-30 jointly pass. C-29 fails (question-form source). C-25 passes (no incident narrative anywhere). V-05 vs V-02 at equal predicted scores isolates whether question-form has a compression advantage.

**Predicted**: 145/200 — C-01–C-13, C-15, C-17, C-19, C-20, C-25, C-30
...
| Rank | Variation | Essential | Rec | Aspirational | Score | Discriminating patterns |
|------|-----------|-----------|-----|--------------|-------|------------------------|
| 1 | V-03 Template ceiling | 5/5 | 3/3 | 15/22 | **165** | C-24+C-28 joint |
| 2 | V-02 Declarative compressed | 5/5 | 3/3 | 11/22 | **145** | C-17+C-29 joint |
| 2 | V-05 Question-form compressed | 5/5 | 3/3 | 11/22 | **145** | C-17+C-30 joint |
...
**Key discriminators:**
- V-02 vs V-05 (equal predicted score): declarative vs question-form disqualifier at compressed density — isolates whether question-form gives a natural compression advantage for C-17
- V-01 vs V-04 (equal predicted score): question-form alone vs inertia role-block + question-form — C-23 requires C-25 to fail; trades one criterion for another
- V-03 holds the template ceiling at 165 — confirms C-28 (interrogative + rollback triple) and C-24 (colloquial + bare) are jointly achievable in one variation
- C-29 and C-30 remain mutually exclusive within any single variation (declarative vs question-form disqualifier source)
