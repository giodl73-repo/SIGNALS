...
Roles are written to `.craft/roles/{area}/`, one file per role.
...
Write each role file to `.craft/roles/{area}/{role-slug}.md`.
...
Write `.craft/roles/{area}/inertia-advocate.md` now, before planning any
other role.
...
Write each planned role to `.craft/roles/{area}/{role-slug}.md`.
...
Write `.craft/roles/{area}/{role-slug}.md` for each row in Table 5.
...
```
Build a typed role registry for this domain. Write files to `.craft/roles/{area}/`.
...
Write `.craft/roles/{area}/{slug}.md` for each role.
...
GATE 4 — WRITE INERTIA-ADVOCATE
  ...
  Work: write `.craft/roles/{area}/inertia-advocate.md` before planning
  any other role.
...
GATE 6 — WRITE REMAINING ROLES
  ...
  Each `.craft/roles/{area}/{slug}.md` must contain all five fields:
...
GATE 7 — VERIFICATION GATE
  Input: all role files in `.craft/roles/{area}/`.
