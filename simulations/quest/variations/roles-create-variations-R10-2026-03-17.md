...
Before generating the requested role, the companion check ensures the area's status-quo voice is present. Check whether `.roles/{area}/inertia-advocate.md` exists.
...
With area, subrole, and companion status resolved, generate the role's YAML frontmatter. Apply CONTRACT: FIELD-REGISTER. Generate all 12 required fields for `.roles/{area}/{subrole}.md`. If any gate fails, fix the failing field and re-check the gate before advancing to Phase 4.
