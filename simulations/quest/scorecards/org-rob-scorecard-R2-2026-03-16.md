...
Evidence: Each stage loads `.roles/` and explicitly extracts `orientation.frame` and `lens.verify`. The `Orientation: {one sentence}` field in the Entry block is stage-level, not synthesis-level. Role lens shapes findings, but not with `++` systematic forcing (e.g., no lens.verify checklist row requirement).
