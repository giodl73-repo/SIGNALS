...
| C-04 | PASS | Path pattern `.craft/roles/{area-slug}/{role-slug}.md` + example files make directory fidelity concrete. |
...
| C-04 | PASS | INVARIANT-4 declared; "INVARIANT-4 enforcement: before writing the first file for any team, confirm `.craft/roles/{area-slug}/` is correct path derived from `org.yaml`." Pre-write path validation. |
...
| C-04 | PASS | Path `.craft/roles/{area-slug}/{role-slug}.md` explicit; after-teams paths for shared/exec-office stated. |
