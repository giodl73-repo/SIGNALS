...
| C-02 | PASS | `Charter Source: ___` + `Participants:` slots in skeleton; fill rule reads `.craft/roles/` with explicit Signal-defaults fallback. |
...
### Essential: 60/60 — all pass (skeleton Phase 0 declares committee type; `.craft/roles/` fallback; role-lens voices; three Phase 5 sections; challenge requirement)
