...
| C-02 | PASS | "Read the charter files from `.craft/roles/`. Load every participant named in the charter." Graceful fallback with disclosure. |
...
| C-02 | PASS | "Read `.craft/roles/`... Load every named participant. If no charter exists, fall back to default Signal roles and disclose the fallback." |
...
| C-02 | PASS | "open `.craft/roles/` and load the charter... If the charter doesn't exist, use Signal's default roles and say so." |
