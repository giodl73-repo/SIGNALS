...
**Run context:** `.craft/roles/signal/` is populated with `pm.md`, `architect.md`, and `inertia-advocate.md`. All C-03 evaluations assume roles exist.
...
| C-03 | Roles input honored | PASS | ROLES INPUT section checks `.craft/roles/`, emits ROLES-LOADED list, or writes ROLES-NOTE if absent. Both paths handled. |
...
| C-03 | Roles input honored | PASS | ROLES INPUT PROTOCOL at end requires checking `.craft/roles/`, listing in ROLES-LOADED field in Section 2, and roles must appear in at least one output section. |
...
| C-03 | Roles input honored | PASS | "DO check .craft/roles/ before writing anything. DO list every role found in ROLES-LOADED. DO NOT invent role names." |
