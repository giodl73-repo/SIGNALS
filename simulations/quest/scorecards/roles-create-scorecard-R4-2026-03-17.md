...
**C-01** — All variations write to `.roles/{area}/{subrole}.md` in the final write phase. Phase 6 in every variation explicitly states both the inertia-advocate path and the main role path. Audit item [A] in every variation confirms path before write.
...
**C-05** — Phase 2 in all variations checks whether `.roles/{area}/inertia-advocate.md` exists, and if absent, generates a complete companion file with full frontmatter and body table. This exceeds the minimum (suggestion); satisfies C-05 by doing more.
