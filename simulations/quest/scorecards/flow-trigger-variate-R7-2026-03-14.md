...
**Context**: R6 confirmed V-04 and V-05 at 100; V-01 structurally capped at 99 (C-10 ordering conflict); V-02 and V-03 capped at 99 (C-14 missing named roles). R7 probes two unexplored axes — phrasing register and inertia framing — and their combinations.

## Axis Selection

| Axis | Rationale |
|------|-----------|
| **Phrasing register** | Unexplored; tests whether compliance is structural (any register achieves 100) or stylistic (formal imperative required) |
| **Inertia framing** | Unexplored; hypothesized to strengthen C-08 specificity without affecting other criteria |
| **Role sequence** | Already fully characterized across R1–R6; used only in combination variations |

Single-axis variations: **V-01** (conversational imperative), **V-02** (descriptive third-person), **V-03** (inertia framing)
...
**Axis**: Phrasing register — second-person "here's what to do" conversational imperative throughout
**Hypothesis**: Conversational register achieves 100 when all structural requirements (typed catalog, role accountability, budget ordering, verdict-first) are explicitly stated; informal phrasing is orthogonal to rubric compliance
...
3. `Condition` — cite the TC-1 entry: "TC-1: [gate expression]". Don't paraphrase the condition; reference it.
...
For every DETECTED or INDETERMINATE: add a Remediation block. It needs three things:
...
**Axis**: Phrasing register — descriptive third-person throughout ("the analyst will...", "the specialist produces...", "the output includes...")
...
**Axis**: Inertia framing — each phase opens with a "what manual review misses" contrast; remediation section explicitly contrasts named PA constructs against manual alternatives; structural backbone is 4-role
**Hypothesis**: Inertia framing strengthens C-08 remediation specificity (named PA constructs become more precise when contrasted against what manual review cannot prescribe) without degrading any other criterion; architecture achieves 100 with potential above-baseline signal on C-08
...
*What manual review misses: an engineer reviewing flows one at a time has no view of the complete trigger registry, cannot enumerate cascade paths across multiple flows, and discovers side-effect scope only by reading each flow's action list. The Threat Analyst catalogs the entire threat surface before a single table row is written.*
...
*What manual review misses: a reviewer inspecting individual flows has no mechanism to see registered-but-not-firing automations alongside firing ones. A "what ran" view without a "what is registered" view makes incident diagnosis incomplete. The Registry Analyst closes that gap with one unified table.*
...
*What manual review misses: there is no manual process that estimates aggregate API request budgets and run durations before a change reaches production. Trigger storms surface as throttling errors or unexpected license charges. The Budget Analyst provides this computation before pathology detection — not after, and not conditionally on confirming a storm.*
...
When required, produce a Budget section here — before Phase 4. This section is structural; it does not wait for a confirmed storm verdict.
...
- Name the exact Power Automate or Copilot Studio construct — the one that a manual review narrative cannot prescribe at this precision. Examples: "Enable concurrency control with degree 1 in Flow Settings > Concurrency Control", "Insert a Do Until loop with an environment variable cycle-break flag initialized to false before the trigger condition evaluates", "Add a Condition action gating on the Modified By field before the cascade step to break the initiating user loop". Generic guidance ("fix the concurrency") fails this requirement.
- State the expected outcome after remediation and why this construct addresses the root cause rather than the symptom.

**Owns: pathology detection, remediation**

---

## V-04 — Conversational Register + 4-Role (C-10 + C-14 + C-16 robustness)
...
**Hypothesis**: The 4-role 100-scoring architecture from R6 is robust to conversational register — informal phrasing of the same structural requirements (typed catalog, role accountability, budget ordering, verdict-first) achieves 100; register change alone does not break any criterion
...
List every registered automation in scope. For each: name it, give its trigger type (record-change / scheduled / manual / child flow), and write the exact gate expression — field, operator, value. Don't paraphrase. The downstream phases cite TC-1 by name.
...
2. **`Fires?` — YES or NO only.** This is the enforcement rule: YES or NO only — no row may omit this column. No blank. No "maybe". No "conditional".
3. **`#`** — integer in firing order for YES rows; `--` for NO rows.
4. **`Condition`** — cite TC-1 by type-number: "TC-1: [gate expression]". Don't rewrite the condition; reference it.
...
8. If a trigger "always fires", justify it in one sentence in the Condition cell. Don't just write "always fires" without grounding.
...
These are named variables. The Budget Analyst and Pathology Analyst read them by name — not by counting rows in the table.
...
Read M and N from REGISTRY SUMMARY. Check: is M >= 3? Does any YES trigger call a connector action? If yes to either: you're running. If no to both: write `Budget Gate: skipped — M < 3 and no connector actions` and stop.
...
## V-05 — Inertia Framing + 5-Role with Narrator (C-08 uplift hypothesis)
...
*What manual review misses*: Individual flow inspection has no mechanism to enumerate the complete threat surface before table construction begins. Cascade paths and side-effect scope are discovered inline, which means high-risk paths are found at the same time as they are assessed — too late to scope them correctly. The Threat Analyst closes this gap by cataloging everything before Role 2 writes a single row.
...
**TC-1 — Candidate Conditions**: Every registered automation in scope, its trigger type (record-change / scheduled / manual / child flow), and the exact gate expression (field, operator, value). This is the canonical registry; Roles 2 through 5 cite TC-1 entries by type-number designation.

**TC-2 — Cascade Paths**: For each TC-1 automation, trace the downstream invocation chain — child flows, HTTP connector targets, Copilot Studio topic triggers, Power Apps callbacks. Write chains as A → B → C. Flag potential loops as cascade candidates. TC-2 is the primary evidence source for trigger storm and circular trigger pathology detection.
...
These are downstream-referenceable variables. Roles 3 and 4 read M and N by name from this block.
...
*What manual review misses*: No manual review process estimates aggregate API request budgets or run durations before the change reaches production. Trigger storms appear as platform throttling errors. The Budget Analyst provides this computation before pathology detection — not after, not conditionally.
...
When required, produce a Budget section here — before Role 4 begins. This section is structural; it does not wait for a storm verdict.
...
*What manual review misses*: No engineer reliably detects all three pathology types — storm, missing, circular — against a complete registry in a single review pass. The verdict-first format makes the detection disposition unambiguous before evidence is read. The construct-level remediation prescribes the exact configuration path that a narrative review cannot reach.
...
### Trigger Storm
DETECTED | NOT DETECTED | INDETERMINATE
[Evidence citing TC-2 cascade pairs and budget section totals. If DETECTED: name automations involved, storm depth, full cascade chain.]
...
Format rule: verdict keyword is the first content element. Do not prefix with "Verdict:", "Finding:", "Assessment:", or any label. Do not build toward the verdict in prose. State it, then support it.

For every DETECTED or INDETERMINATE: produce a Remediation block.
- Specific action to take.
- Exact PA or Copilot Studio construct — the configuration path or action type that a manual review narrative cannot prescribe at this precision (e.g., "Enable Concurrency Control with degree 1 under Flow Settings > Concurrency Control", "Add a Do Until action with an `IsProcessed` environment variable flag initialized to false, set to true inside the loop body, with exit condition `IsProcessed = true`", "Insert a Condition action before step 2 of the cascade checking `triggerOutputs()?['body/modifiedby']` against the system integration user ID"). Generic framing ("reduce concurrency") fails this requirement.
...
*What manual review misses*: A manual review produces findings in technical language for flow developers. Stakeholders without Power Automate expertise receive no cross-phase synthesis. The Narrator produces the one artifact that a manual review debrief typically lacks.
