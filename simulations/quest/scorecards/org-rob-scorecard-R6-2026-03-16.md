...
| **C-02** Role-loaded perspective | ++ | `.roles/` loaded; `orientation.frame` extracted; lens items cited per-finding via Via: |
...
| **C-02** Role-loaded perspective | ++ | Roles loaded from `.roles/`; Frame: and lens items extracted; findings cite role-specific lens items |
...
| **C-18** Role orientation frame citation | ++ | `Frame: orientation.frame = "{extracted value from .roles/}"` in every stage ROLE: block; "Appears in every stage ROLE: block" |
...
| **C-02** Role-loaded perspective | ++ | `.roles/` loaded; orientation.frame extracted; Via: citations tie every finding to a specific role lens item |
...
| **C-18** Role orientation frame citation | ++ | `Frame: orientation.frame = "{extracted value — must match .roles/ field}"` in ROLE: block; "Every stage ROLE: block includes the Frame: line. No exceptions." |
