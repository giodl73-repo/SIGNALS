...
| C-02 | PASS | `LOAD: charter from .craft/roles/`; fallback to Signal defaults with explicit disclosure |
...
| C-02 | PASS | `LOAD: charter from .craft/roles/`; fallback declared |
...
| C-30 | **PARTIAL** | Fill rules blend imperatives (ENFORCE, PRINT at commit/gate blocks) with descriptive prose: "Load the charter file from `.craft/roles/`...Pull every role...If no charter exists, declare that and fall back..." — Phase 0 and Phase 1 descriptions are narrative rather than verb-first imperatives. |
| C-31 | PASS | PHASE-N-COMMIT in every phase |
