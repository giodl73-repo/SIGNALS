...
**V-02 evidence (thin phases):** Phase 3 body: `Generate YAML frontmatter for .craft/roles/{area}/{subrole}.md. All 12 fields required. Apply CONTRACT: FIELD-REGISTER.` — exactly one CONTRACT citation, no rule restatement. Phase 4 adds "Minimum 3 rows" which is a row-count spec absent from CONTRACT: COLUMN-HEADER (not a restatement of any CONTRACT rule). C-19 = P.
