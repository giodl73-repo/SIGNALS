...
| ID | Verdict | Evidence |
|----|---------|----------|
| C-01 | PASS | `.craft/roles/{area-slug}/{role-slug}.md` paths, ROLES-COMPLETE tracking vs org.yaml slot count |
...
| C-13 | **FAIL** | Pass condition: "headcount table in org-chart.md is written prior to the first role file." In V-05, WRITE-ROLES (Phase 7) executes before WRITE-CHART (Phase 8). The CONTRACT artifact exists before any file write, but org-chart.md is written *after* .craft/roles/ files, inverting the required order. |
