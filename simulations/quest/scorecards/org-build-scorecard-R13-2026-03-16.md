...
| ID | Result | Evidence |
|----|--------|----------|
| C-06 | **PASS** | Phase 4 Step 4 + Constraints: MUST group under `.roles/{area}/`, FORBIDDEN: flat placement |
...
| C-29 | **PASS** | Full scan: no "do not X", "never X", "avoid X" forms found in any constraint context. All prohibitions use `FORBIDDEN:` including the overwrite guard in Phase 4: "FORBIDDEN: overwriting any `.roles/` file that already exists at the target path." |
