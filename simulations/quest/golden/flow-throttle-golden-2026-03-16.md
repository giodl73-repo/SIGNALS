...
date: 2026-03-15
rounds: 20
rubric_final: flow-throttle-rubric-v21-2026-03-15.md
score: 325
status: GOLDEN
---

# flow-throttle — Golden Prompt

**Prompt body:** simplified (77% reduction — PASS; body is simplified V-01 with R20 additions)
**Winning variation:** V-05 (Round 20, top-ranked by implementation richness)
**Rubric:** v21 (C-01–C-56; N_essential=5, N_recommended=3, N_aspirational=48)
**Score:** 325/325 (all 5 essential PASS; all 3 recommended PASS; 40/40 aspirational C-09–C-48; C-53/C-54/C-55 PASS)
...
Three patterns distinguish the R20 golden from the R19 baseline (310/325). Each addresses a
criterion that all prior rounds failed across all variants.

**Pattern 1 — Per-step consequence clauses (C-53)**

R19 argued non-collapsibility globally: "no single gesture satisfies all components
simultaneously." R20 moves the argument to each sub-step. Every G-step carries a co-located
`Consequence of skipping:` field that names the specific structural failure if that step is
collapsed. G-1 names bypass-detection invisibility; G-2 names silent upstream-signal failure;
G-3 names count-verification loss; G-4 names implicit-signal ambiguity. The consequence is
header-scannable from the sub-step, not inferred from a preamble.

V-05 (winning variation) elevated this further: each consequence clause names the specific
*executor inertia path* the sub-step defeats — not just the structural failure, but the
rational-seeming shortcut that produces it (e.g., `` `GATE-A complete = bypass detected` ``).
This is formalized as C-56 in v21.

**Pattern 2 — Joint multi-artifact audit row (C-54)**

R19's Field 5 audit table had 8 rows, each auditing a single artifact. Bypass-detection
co-presence required cross-row synthesis — an inference task, not a scan task. R20 adds a 9th
row that simultaneously cites the bypass register Attempt type cell, the G-1 gate fill value,
and the SIG-01 Handoff cell in a single row. A reader confirms bypass-detection co-presence
by scanning one row, not by correlating three rows.

**Pattern 3 — Contracted blockquote for non-conflation (C-55)**

R19 quoted the SIG-01 "Does NOT confirm" cell inline within prose. The quote was evidential —
it appeared as supporting text. R20 wraps it in a blockquote with the label
`SHALL-authoritative cell value:`, converting the citation's epistemic status from evidential
to contractual. String equality is declared as the verification method. A reader who locates
the blockquote can confirm the non-conflation claim without prose interpretation.

**Pattern 4 — Single-responsibility gate split**

V-05 made the gate architecture explicit by naming GATE-A's single responsibility (signal
distinction + bypass detection, defeating the "SIG-01 present = analysis ready" inertia path)
separately from GATE-B's single responsibility (count verification, defeating the "7 rows
visibly present" visual-inspection path). Each gate's Responsibility column in the header table
names the inertia path it defeats — header-scannable without entering the gate body.

---

## Final Rubric Criteria Summary (v21)

**Rubric version:** v21 | **Max composite:** 330 | **Golden threshold:** 325/330

### Essential criteria (12 pts each, N=5)

| ID | Criterion |
|----|-----------|
| C-01 | TABLE A present with all required columns |
| C-02 | TABLE B present with hop chain |
| C-03 | TABLE C present with error codes |
| C-04 | TABLE E present with typed risks |
| C-05 | TABLE F present with named parameters |

### Recommended criteria (10 pts each, N=3)

| ID | Criterion |
|----|-----------|
| C-06 | Load-shape classification per tier |
| C-07 | Cascade scenario names 3+ tiers |
| C-08 | Retry-After assessment per caller |

### Aspirational criteria (5 pts each, N=48)

Selected highlights relevant to the R20 additions:

| ID | Criterion |
|----|-----------|
| C-13 | Tier-ID threading — T-NN appears verbatim in all downstream sections |
| C-16 | Load-shape verdict assigned at registration (Step 1B), not deferred |
| C-17 | Registry gap prohibition stated and enforced at each Step 2 section |
| C-27 | Violation-type annotations present at every precision site |
| C-40 | Annotation dropout: all 7 Annot-IDs present at anchor sites |
| C-41 | SIG-02 = INVENTORY VERIFIED precedes Step 1A |
| C-42 | HANDOFF SIGNAL declared before Role 2 opens |
| C-44 | SIG-01 presence confirmed in Role 1 output |
| C-45 | Signal distinction table with "Does NOT confirm" column |
| C-46 | Role activation chain declared at prompt header |
| C-47 | Bypass-attempt rejection register with 3 named columns |
| C-48 | Bypass gate field filled before count sub-steps |
| C-50 | Count verification atomized into G-1–G-4 with per-step SHALL instructions |
| C-51 | `SHALL NOT activate if` bypass-condition column in header table |
| C-52 | Verbatim SIG-01 "Does NOT confirm" cell quoted in non-conflation statement |
| **C-53** | **Per-step consequence clause naming structural failure (R20 addition)** |
| **C-54** | **Joint multi-artifact audit row for bypass-detection co-presence (R20 addition)** |
| **C-55** | **Contracted blockquote with obligation-class label for non-conflation (R20 addition)** |

### v21 addition (5 pts)

| ID | Criterion |
|----|-----------|
| **C-56** | **Named executor inertia path per consequence clause — names the rational-executor shortcut that generates the structural failure (ES-01 from R20, formalized in v21)** |

**Score breakdown at 325/330:**

| Band | Points |
|------|--------|
| C-01–C-05 essential (5 × 12) | 60 |
| C-06–C-08 recommended (3 × 10) | 30 |
| C-09–C-52 aspirational (44 × 5) | 220 |
| C-53 (5) | 5 |
| C-54 (5) | 5 |
| C-55 (5) | 5 |
| **Total** | **325** |
| C-56 (5 — v21, not yet in body) | +5 |
| **Max composite** | **330** |
