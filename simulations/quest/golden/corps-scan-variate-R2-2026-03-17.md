...
This is a **draft-only** skill. Do NOT write `.roles/` files. Do NOT instruct the user to
...
**Scope constraint, stated unconditionally**: corps-scan does not write `.roles/` files.
...
SCOPE CONSTRAINT: draft-only — NO .roles/ files, NO role file content, NO role file
...
PRECONDITION: draft-only constraint active — no .roles/ output permitted
...
- [ ] No `.roles/` content in this phase
...
**Non-negotiable scope**: This skill does not write `.roles/` files. It does not describe
...
`.roles/` files. No role includes role file content. No role instructs the user to
...
**Draft-only constraint**: Active. No `.roles/` files. No role file content. org.yaml
only.
