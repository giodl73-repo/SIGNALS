## V-05 — Full Combination: V-01 + V-02 + V-03

**Variation axis**: All three axes combined — inertia-dominant framing (V-01: STATUS QUO
VIABILITY ASSESSMENT before claims, STATUS QUO VERDICT at closing), descriptive register
(V-02: narrative instructions throughout), and lifecycle emphasis (V-03: mandatory
COMMITMENT-READINESS MATRIX in LIFECYCLE section).

**Hypothesis**: Each axis independently improves a different failure mode: V-01 prevents
generic challenge claims, V-02 prevents mechanical-but-hollow field fills, V-03 prevents
vague program verdicts. Combined, the three axes cover the three most common ways the review
fails to surface real risk. The hypothesis is that the full combination produces the highest
average output quality across diverse artifact types, even if some individual axes might
trade off against each other in specific cases.
...
Read `.craft/roles/signal/`. Choose the inertia-advocate as the challenger — the role that
most forcefully argues against building, names switching costs and workaround viability, and
owns both bracket sections. Choose a domain role whose lens.verify entries best match this
artifact's technical surface. Choose a lifecycle/program role focused on commitment readiness.

| Slot | Archetype | Role | Selection Rationale |
|------|-----------|------|---------------------|
| CHALLENGER (bracket open + close -- fixed) | inertia-advocate | [ROLE_NAME] | Write one sentence explaining why this inertia-advocate is right for this artifact type. |
...
|--------|------|------------------|------------------------|----------------------|------------------|
| AM-01 | [DOMAIN ROLE] | Write the full text of the first lens.verify entry for this role. | Name the artifact domain this entry covers. | HIGH, MEDIUM, or LOW. | One sentence explaining the rating based on this artifact's content. |
...
Before any challenge is issued, establish why the current state is a viable default. For each
status quo dimension, describe what the team actually does today and identify the concrete
advantage that makes it acceptable. This assessment is not a concession — it is the evidence
base from which challenge claims are drawn. Each claim below should reference at least one
dimension from this table. A challenge claim that cannot be tied to a named SQ advantage is
weak; this table prevents that.

| SQ-DIM | Current State Description | Why Current State Is Acceptable | Viability |
|--------|--------------------------|--------------------------------|-----------|
| Name the first status quo dimension — a specific area where current practice works. | Describe what the team actually does today on this dimension. | State the concrete advantage this approach provides — what would be lost by switching. | HIGH if the SQ is robust here. MEDIUM if adequate but imperfect. LOW if strained. |
| Second dimension. | Current practice. | Advantage. | Viability. |
| Third dimension. | Current practice. | Advantage. | Viability. |

**Overall SQ Viability**: Write VIABLE if the team can continue without this artifact
indefinitely, MARGINAL if bearable short-term, or STRETCHED if already showing strain.

---

**Null hypothesis**: State in one sentence what the team currently does instead of building
this artifact. Be concrete — name the specific workaround, tool, or process in use.

**Alternatives comparison table** *(score 0-10 per dimension; at least three dimensions; distinct from §17 below)*:

| Dimension | Status Quo (A) | Build (B) | Best Non-Build Alt (C) | Notes |
|-----------|---------------|-----------|----------------------|-------|
| Name the first dimension. | Score 0-10. | Score 0-10. | Score 0-10. | Optional note. |
...
|-----------|---------------------|--------------------------|------------------------------|----------|----------|------------|
| Name a must-clear dimension and mark *. | Score 0-10 for status quo. | Score 0-10 for proposed build. | Score 0-10 for best non-build alt. | Compute B-A. | Compute B-C. | FAVORS-BUILD if D1>0 and D2>0; CONDITIONAL if D1>0 and D2<=0; OPPOSES-BUILD if D1<=0. |
| Second must-clear dimension. * | Score. | Score. | Score. | D1. | D2. | Verdict. |
| Third dimension. | Score. | Score. | Score. | D1. | D2. | Verdict. |
...
Note: DEFEATED requires BOTH D1 AND D2 positive majority -- build must beat both status quo
      AND best alternative. Positive D1 alone (beats status quo but not best alt) is CONDITIONAL.
```
**g_null(initial)** *(§9 Stage 1)*: Write HOLDS, CONDITIONAL, or DEFEATED.

**NULL HYPOTHESIS DERIVATION RULE** *(re-applied by BRACKET CLOSING to the revised table)*:
  DEFEATED requires count(FAVORS-BUILD) majority, no MUST-CLEAR OPPOSES-BUILD, and BOTH D1 AND D2
  positive across the majority of rows.

**Challenge claims** *(each claim should reference a named SQ-DIM; focus on switching costs,
workaround viability, or adoption friction; assign sequential CH-IDs)*:

| CH-ID | Challenge Claim | SQ-DIM Referenced | Severity |
|-------|----------------|-------------------|---------|
| CH-001 | Write the first challenge claim in one sentence. Ground it in a specific SQ advantage from the viability table. Name the concrete inertia vector being defended. | Name the SQ-DIM this claim is grounded in. | HIGH if it would block commitment. MEDIUM if it conditions commitment. LOW if advisory. |
| CH-002 | Second claim, similarly grounded. | SQ-DIM. | Severity. |
| CH-003 | Optional third claim, or omit this row. | SQ-DIM or N/A. | Severity. |

**Verify Question**: Write one question from this role's lens.verify that is most critical for this artifact.
...
**GOpen Reason**: One sentence. State which g_null result drove this verdict.
...
|---------|------|---------|-------|
| BRACKET OPENING | GOpen | Write verdict. | Write class per §6. |
...
You are the domain-expert reviewer. Begin by responding to each challenge claim from your
technical perspective — assess whether the concern is valid given the artifact's actual design.
Then re-score the alternatives table with your own analysis. Report your NH dimension scores
for each §17 dimension (the challenger will average these at bracket closing per the NH TABLE
AGGREGATION RULE). Then produce findings from your lens.verify entries, citing §1 surfaces
for each. Conclude with a lens coverage table.

**CH-ID Response Table**:

| CH-ID | Challenge Claim (copy verbatim) | Your Technical Assessment | Status |
|-------|----------------------|-------------------------------|--------|
| CH-001 | Copy the claim from BRACKET OPENING. | Assess this claim technically. Does the artifact's design address or mitigate this concern? Name specific sections or design elements. | ADDRESSED if fully resolved. PARTIAL if partially addressed — state what remains open in one sentence. OPEN if outside your technical scope. |
| CH-002 | Copy. | Technical assessment. | Status. |
...
|-----------|---------------|-----------|----------------------|-----------|
| Copy dimension from BRACKET OPENING. | Your score. | Your score. | Your score. | One sentence noting where your view differs from the challenger's, if at all. |
| Second dimension. | Score. | Score. | Score. | |
| Third dimension. | Score. | Score. | Score. | |

**NH Dimension Scores** *(BRACKET CLOSING averages these per NH TABLE AGGREGATION RULE)*:

| Dimension | Your Proposed-Build Score (B) | Your Best-Non-Build-Alt Score (C) |
|-----------|-------------------------------|-----------------------------------|
| Copy §17 dimension name. | Your 0-10 score for the proposed build on this dimension. | Your 0-10 score for the best non-build alternative on this dimension. |
...
|---|-----------|---------|---------|
| F-1 | Write [§1:S-n] citing the relevant surface by its number. | Describe the finding in one to two sentences. Ground it in your lens.verify entries or expertise.depth. | HIGH, MEDIUM, or LOW per §2. |
| F-2 | [§1:S-n]. | Second finding. | Severity. |
| F-3 | [§1:S-n] — optional. | Third finding. | Severity. |

**Finding Severity Aggregation (per §14)** — apply mechanically:
```
worst(F-1=[_], F-2=[_], ...) = [SECTION_SEVERITY]
```
**Section Severity**: Write the mechanical result.

**Lens Coverage Table** *(cite AM row and tier exactly as assigned in ROLE MANIFEST)*:

| # | lens.verify Entry | Applicability (from ROLE MANIFEST) | Status | Finding Reference |
|---|------------------|-----------------------------------|--------|-------------------|
| Q-1 | Copy the full text of lens.verify entry 1 for this role. | Write "AM-01: HIGH" or MEDIUM or LOW. Do not change this value from the ROLE MANIFEST. | ADDRESSED if a finding addresses this. OPEN if not. | F-n if addressed. If LOW and OPEN: "AM-01: LOW -- [basis from matrix]". |
...
You are the lifecycle/program reviewer. Your role is to assess whether this artifact is ready
for commitment from a decision, resource, and program perspective — not whether the technical
design is correct. Begin with CH-ID responses from your commitment frame. Then complete the
COMMITMENT-READINESS MATRIX to assess four program dimensions explicitly. Then write a
decision-readiness assessment and produce findings.

**CH-ID Response Table**:

| CH-ID | Challenge Claim (copy) | Your Commitment-Frame Response | Status |
|-------|----------------------|--------------------------------------|--------|
| CH-001 | Copy claim from BRACKET OPENING. | Assess this claim from a program and commitment perspective. Does your program knowledge support or undermine this concern? | ADDRESSED / PARTIAL / OPEN. |
...
Assess each of the four program dimensions. For each, describe the current state, what is
required for commitment to proceed safely, and the gap between them. A HIGH-severity gap on
any dimension prevents G_lifecycle from being PASS. A MEDIUM-severity gap conditions it.

| CRM Dimension | Current State | Required State for Commitment | Gap | Severity |
|---------------|--------------|-------------------------------|-----|---------|
| Timeline | Describe the current delivery timeline or milestone state for this artifact. | Describe what timeline certainty or milestone state is required before committing. | Describe the gap, or write "None" if current state meets the requirement. | HIGH if this gap blocks commitment. MEDIUM if it conditions it. LOW if advisory. |
| Resources | Describe the current team and budget allocation for this work. | Describe what resource commitment is needed to proceed. | Gap or "None". | Severity. |
| Rollback | Describe the current state of rollback or exit planning for this artifact. | Describe what rollback capability is required before commitment. | Gap or "None". | Severity. |
| Stakeholder Alignment | Describe the current state of stakeholder buy-in and alignment. | Describe the alignment level required for safe commitment. | Gap or "None". | Severity. |

**CRM Severity Aggregation**:
```
worst(Timeline=[_], Resources=[_], Rollback=[_], Stakeholder=[_]) = [CRM_SEVERITY]
```
**CRM Signal**: Write HIGH, MEDIUM, or LOW.
*(HIGH -> G_lifecycle cannot be PASS unless waived. MEDIUM -> G_lifecycle cannot be PASS without
a named resolution condition. LOW -> advisory; G_lifecycle may be PASS with caveats noted.)*

**Decision-readiness assessment**: Write one paragraph. Reference GOpen, G_domain Aggregate,
g_null(post-domain), and CRM Signal by name. Assess whether the decision is ready, noting what
is clear and what remains uncertain from a program and commitment perspective.

**Null hypothesis status**: One sentence. State whether g_null(post-domain) has been defeated,
remains conditional, or holds, referencing the value explicitly.

**Additional Findings** *(HIGH/MEDIUM CRM gaps should appear as findings; cite §1 surfaces)*:

| # | §1 Surface | Finding | Severity |
|---|-----------|---------|---------|
| F-1 | [§1:S-n] | Commitment, program, or CRM concern derived from the artifact. | HIGH / MEDIUM / LOW. |
| F-2 | [§1:S-n] | Second finding. | Severity. |
...
| # | lens.verify Entry | Applicability (from ROLE MANIFEST) | Status | Finding Reference |
|---|------------------|-----------------------------------|--------|-------------------|
| Q-1 | Copy full text of lifecycle lens.verify entry 1. | AM-04: tier from ROLE MANIFEST. | ADDRESSED / OPEN. | F-n or AM cite. |
...
*(CRM Signal=HIGH -> FAIL unless waived with named justification. CRM Signal=MEDIUM -> CONDITIONAL
unless the gap is resolved with a named condition.)*
...
**V-01 AXIS: STATUS QUO VERDICT** *(revisit STATUS QUO VIABILITY ASSESSMENT from opening)*

Having received domain and lifecycle testimony, re-evaluate each SQ dimension. For each
dimension, note whether any received finding changed your view of the status quo's viability.
Then assess the overall trajectory — did evidence strengthen or weaken the inertia case?

| SQ-DIM | Opening Viability | Evidence That Changed This | Revised Viability |
|--------|------------------|---------------------------|-------------------|
| Copy SQ-DIM-1 from BRACKET OPENING. | Copy the opening viability rating. | Name the specific domain or lifecycle finding that changed your view, or write "none" if no finding affected this dimension. | VIABLE, MARGINAL, or STRETCHED after evidence. |
| SQ-DIM-2. | Opening. | Finding ref or "none". | Revised. |
| SQ-DIM-3. | Opening. | Finding ref or "none". | Revised. |

**SQ Viability Trajectory**: REINFORCED if evidence strengthened the status quo case.
UNCHANGED if no finding altered the assessment. WEAKENED if domain or lifecycle evidence
undermined one or more SQ advantages named at opening.
...
**Revised NULL HYPOTHESIS DIMENSION TABLE** *(per §17 + NH TABLE AGGREGATION RULE -- derive all values mechanically)*:

| Dimension | Status-Quo Score (A) | Proposed-Build Score (B) | Best-Non-Build-Alt Score (C) | D1=(B-A) | D2=(B-C) | NH Verdict |
|-----------|---------------------|--------------------------|------------------------------|----------|----------|------------|
| DIM-A*. | A: carry from opening unchanged (control arm). | B: average of all domain reviewers' B scores for this dimension. | C: average of all domain reviewers' C scores for this dimension. | Compute D1=B-A. | Compute D2=B-C. | Recompute verdict. |
...
Column B averages per dimension:
  [DIM-A]: avg([list each domain reviewer's B score]) = [result]
  [DIM-B]: avg([scores]) = [result]
  [DIM-C]: avg([scores]) = [result]
Column C averages per dimension:
  [DIM-A]: avg([list each domain reviewer's C score]) = [result]
  [DIM-B]: avg([scores]) = [result]
  [DIM-C]: avg([scores]) = [result]
```
...
DEFEATED condition (count(FAVORS-BUILD) > count(OPPOSES-BUILD)+count(CONDITIONAL) AND no MUST-CLEAR OPPOSES): [Y/N]
...
|-------|--------------|-----------------|-----------------|---------------------|
| CH-001 | Copy from DOMAIN. | Copy from LIFECYCLE. | FULLY SATURATED if both ADDRESSED; UNSATURATED otherwise. | WAIVER text or N/A. |
...
**Remaining gaps**: All defeated, or list what holds.
**GClose Verdict**: PASS if g_null=DEFEATED and CH-IDs satisfied. CONDITIONAL or FAIL otherwise.
**Override invoked**: YES or NO.
**g_null Override**: N/A -- formula applied, or write justification in one sentence.
**GClose Rationale**: Two to three sentences. Reference g_null(final), CH-ID saturation,
SQ Viability Trajectory, CRM Signal from LIFECYCLE, and overall trajectory.
...
|-----------------|-------------|---------------------|----------|
| Copy §1 surface 1. | Reviewer name or "none". | Finding refs. | COVERED or GAP. |
...
*(Group §1 surfaces by their [DOMAIN: label] annotations. Check APPLICABILITY MATRIX for HIGH
rows covering each domain.)*

| Artifact Domain | HIGH-Applicability MATRIX Rows | Coverage |
|----------------|-------------------------------|----------|
| Domain from §1 annotations. | List AM rows with HIGH applicability for this domain, or "none". | COVERED or ADVISORY-GAP. |
| Second domain. | AM rows or "none". | Coverage. |

**§11 ADVISORY-ORPHAN check**: List findings that lack §1:S-n citations, or "None detected."
...
**Conflicts**: Identify any two reviewers who gave incompatible assessments on the same claim
or surface. If none, write "None detected."

**Convergence**: Identify any concern raised independently by two or more reviewers. Name it
and explain why convergence makes it the highest-confidence signal in this review.
...
Trajectory:         MONOTONIC-PASS if each stage >= prior (PASS scale: DEFEATED > CONDITIONAL > HOLDS)
                    MONOTONIC-FAIL if each stage <= prior
                    DOMAIN-DEGRADED if g_null(post-domain) < g_null(initial)
                    LIFECYCLE-DEGRADED if g_null(final) < g_null(post-domain)
                    DOMAIN-RECOVERED if post-domain improved from initial
                    LIFECYCLE-RECOVERED if final improved from post-domain
```
...
```
GClose=FAIL? -> BLOCKED (challenger holds; null hypothesis not defeated by received testimony)
...
**Applying §16 rule**: State the rule number and the specific reason it fired.
**Primary Driver**: BRACKET CLOSING, DOMAIN, LIFECYCLE, BRACKET OPENING, or N/A.

**Conditions** *(only if CONDITIONAL)*:
1. State the condition from the first CONDITIONAL gate.
2. State additional conditions if more than one gate is CONDITIONAL.

**Blocker** *(only if BLOCKED)*: State the specific finding from the FAIL gate. If GClose=FAIL:
write "Challenger final verdict HOLDS -- [one sentence summarizing GClose Rationale]."

**RESOLUTION REGISTRY** *(only if CONDITIONAL)*:
| Gate | Condition Summary | Resolution Owner | Resolution Criterion | Status |
|------|------------------|-----------------|---------------------|--------|
| Gate name. | One sentence summarizing the condition. | Role name. | A falsifiable test that is observable from outside the review. | OPEN |
...
Copy all local gate ledger rows verbatim. Then add all advisory items as described in §6.

| Source | CH-ID / Finding | Item Description | Disposition Class | Owner Role | Resolution Criterion |
|--------|----------------|-----------------|------------------|------------|---------------------|
| Gate ledger copy. | -- | Exact text: Section / Gate / Verdict / Class. | Class. | Role. | Criterion. |
| CH-ID item. | CH-00n | Describe what must be resolved for any PARTIAL or HOLDS status. | Class. | Role. | Criterion. |
| Lens HIGH. | AM-0n / Q-n | ADVISORY-OPEN-LENS-REQUIRED: [lens entry text] not addressed; HIGH applicability. | ADVISORY. | Role. | Produce a finding addressing this lens entry for the artifact. |
| Lens MEDIUM. | AM-0n / Q-n | ADVISORY-OPEN-LENS: [lens entry text] not addressed; MEDIUM applicability. | ADVISORY. | Role. | Address the lens entry or provide justification for non-applicability. |
| Scope gap. | -- | ADVISORY-GAP: §1 surface [name] not covered by any reviewer. | ADVISORY. | Role. | Assign a reviewer with relevant lens coverage. |
| Domain gap. | -- | ADVISORY-GAP (DOMAIN COVERAGE): [domain] has no HIGH-applicability reviewer in APPLICABILITY MATRIX. | ADVISORY. | Role. | Add a reviewer with HIGH applicability for this domain, or justify waiver. |
| Orphan. | -- | ADVISORY-ORPHAN: finding [F-n] from [SECTION] lacks §1:S-n citation. | ADVISORY. | Role. | Re-assign to a valid §1 surface or remove the finding. |
