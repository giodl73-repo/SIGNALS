...
Every role name in the output corresponds to a file that exists in `.roles/` at
execution time. No role is invented, recalled from a prior run, or substituted.
- If `.roles/` is absent: output exactly `ERROR: .roles/ not found.` Stop.
- If `.roles/` is present but empty: output exactly `ERROR: .roles/ is empty.` Stop.
...
>   (a) all roles in `.roles/` with `archetype: challenger` have been processed,
...
- **Role**: filename stem from `.roles/`; no invented names
...
|--------|--------------|-----------------------------------|
| Role | string -- filename stem from `.roles/` at execution time | NOT: invented names; names from memory or prior runs; any name not present in the registry at this execution |
