...
Read every file in `.craft/roles/`. Build a list of all available roles.
Do not fabricate roles not found in that directory. If `.craft/roles/` is absent or empty,
halt and output: `ERROR: .craft/roles/ not found. Cannot proceed.`
...
- **Role**: name from `.craft/roles/` file
...
If directory is missing: output `ERROR: .craft/roles/ not found.` and stop.
...
Go to `.craft/roles/`. Read every file. That directory is your complete reviewer pool —
...
Read every file in `.craft/roles/`. Extract each role's `archetype` field.
...
If `.craft/roles/` is missing: output `ERROR: .craft/roles/ not found.` and stop.
...
**L1** Open `.craft/roles/`. Read every file in the directory.
**L2** For each file, extract: role name, archetype, `orientation.frame`, `expertise.relevance`.
**L3** If `.craft/roles/` is missing or empty: output `ERROR: role registry unavailable.` and halt.
...
|------|----------------|
| Role | Name matches `.craft/roles/` file — not invented |
