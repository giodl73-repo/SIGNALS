...
- **Standard** (default): Select one role per archetype slot -- CHALLENGER (inertia-advocate),
  ...
  `.roles/signal/` whose `expertise.relevance` matches this artifact type. State each
...
{{reviewer_set}}  -- reviewer configuration; "auto" to select by relevance, or a comma-separated
                     list of role names from .roles/signal/ to run exactly
```
...
3. If `{{reviewer_set}}` = "auto" and `{{depth}}` = "deep": include all roles in
   `.roles/signal/`. State total count.
...
organizational reviewers drawn from `.roles/signal/`. Each reviewer applies their
