...
**EXAMPLE role file: .roles/core/lead-engineer.md**
...
**EXAMPLE Inertia Advocate file: .roles/core/inertia-advocate.md**
...
to `.roles/exec-office/` if declared in `org.yaml`. Same five fields required.
...
INVARIANT-1 (file completeness): The count of files in `.roles/` must equal
...
INVARIANT-3 (Inertia Advocate per team): Every team node in `org.yaml` must have a
  `.roles/{area-slug}/inertia-advocate.md` file. Not one global IA — one per team.

INVARIANT-4 (directory fidelity): The subdirectory structure under `.roles/`
...
INVARIANT-5 (five typed fields): Every `.roles/` file must contain five fields —
...
`.roles/{area-slug}/` is the correct path derived from `org.yaml`. Do not create
subdirectories not present in the parse manifest.

After all teams:
- Shared-group roles: `.roles/shared/`, `role-type: shared-group`
- Exec-office roles: `.roles/exec-office/`, `role-type: exec-office`
...
Write every role file declared in `org.yaml`. Path: `.roles/{area-slug}/{role-slug}.md`.
...
After all teams: write shared-group roles to `.roles/shared/` and exec-office roles
to `.roles/exec-office/` if declared. Same five fields. Skip if not in `org.yaml`.
...
INVARIANT-1 (file completeness): Count of `.roles/` files = total role slots in `org.yaml`.
INVARIANT-2 (ASCII hierarchy): `org-chart.md` contains `+--`/`|` ASCII tree; all area names
  verbatim; at least two nesting levels.
INVARIANT-3 (IA per team): Every team node has `.roles/{area-slug}/inertia-advocate.md`.
INVARIANT-4 (directory fidelity): `.roles/` subdirectories mirror `org.yaml` area names
...
**Example role file: .roles/pipelines/data-engineer.md**
...
**Example Inertia Advocate: .roles/pipelines/inertia-advocate.md**
...
All role files at `.roles/{area-slug}/{role-slug}.md`.
...
After all teams: write shared-group roles to `.roles/shared/`, exec-office roles to
`.roles/exec-office/`, if declared. Same five fields required.
...
INVARIANT-1: Count of `.roles/` files = total role slots declared in `org.yaml`.
...
INVARIANT-4: `.roles/` subdirectory names match `org.yaml` area slugs one-to-one.
...
After all teams:
- Shared-group roles: `.roles/shared/`, `role-type: shared-group`; F-4 must reference
...
COVERAGE-AUDIT:
  ...
  [A] File count vs INVARIANT-1
      ...
      Written:  [count of .roles/ files, excluding index/README]
  ...
  [C] Directory fidelity vs INVARIANT-4
      [area slug]: [EXISTS as .roles/{area-slug}/ | MISSING]
      (one line per area)
