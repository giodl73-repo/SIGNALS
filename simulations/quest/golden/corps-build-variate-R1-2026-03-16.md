...
Path: `.roles/{group-slug}/{role-slug}.md` (no team subdirectory)
...
1. **Regenerate area**: `--area "[area name]"` — rewrites all .roles/ files for the
...
3. **Change group structure**: `--restructure "[team] > [new-parent]"` — moves a team to a
   new parent group, renames the .roles/ subdirectory, and redraws the org diagram.
...
- .roles/ files written (exclude README or index files)
- Teams in org.yaml
- IA files written
...
count. Role file generation must produce exactly [n] .roles/ files.`
...
Write all .roles/ files. Process in headcount table row order — left to right, top to
bottom. For each area row:

a. Write standard role files: `.roles/{area-slug}/{role-slug}.md`
b. Write specialized role files: `.roles/{area-slug}/{role-slug}.md`
c. Write Inertia Advocate: `.roles/{area-slug}/inertia-advocate.md`

Also write:
- Shared group roles: `.roles/{group-slug}/{role-slug}.md`
- Exec office roles: `.roles/exec-office/{role-slug}.md`
...
- Count .roles/ files generated (exclude README or index files)
...
3. **Change group structure**: `--restructure "[team] > [new-parent]"` — moves a team to
   a new parent group, renames the .roles/ subdirectory, and redraws the ASCII diagram.
...
1. **Regenerate area**: `--area "[name]"` — rewrites all .roles/ files for one area
...
3. **Change group structure**: `--restructure "[team] > [new-parent]"` — moves a team node,
   renames its .roles/ subdirectory, and redraws the ASCII diagram.
...
Write all .roles/ files. Process in this order:
...
- Standard/specialized: `.roles/{area-slug}/{role-slug}.md`
- Shared group: `.roles/{group-slug}/{role-slug}.md`
- Exec office: `.roles/exec-office/{role-slug}.md`
- Inertia Advocate: `.roles/{area-slug}/inertia-advocate.md`
...
3. **Change group structure**: `--restructure "[team] > [new-parent]"` — moves a team to a
   new parent group, renames .roles/ subdirectory, and redraws the diagram.
...
Emit: `PARSE-GATE PASS: [n] role files will be written to .roles/. Proceeding to PHASE 2.`
...
3. **Change group structure**: `--restructure "[team] > [new-parent]"` — moves a team to a
   new parent group, renames its .roles/ path, and redraws the ASCII diagram.
...
CROSS-REF CHECKS:
  ...
  C1 — headcount table total == .roles/ file count
  ...
  C3 — every area in headcount table has a .roles/{area-slug}/ directory
       table areas: [list]
...
CORPS-BUILD COMPLETE — {date}
  org-chart.md:        written
  .roles/ files: [n] written / [n] declared
