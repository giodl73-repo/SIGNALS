...
Read `.roles/` for this committee type's charter and load every named role. If no charter exists, fall back to Signal defaults (PM, Architect, Inertia-Advocate) and state the fallback explicitly.
...
**Load participants.** Read `.roles/` for this committee type's charter. Load every named role with their documented orientation. If no charter exists, fall back to Signal defaults (PM, Architect, Inertia-Advocate) and state the fallback explicitly.
...
**Load participants.** Read `.roles/` for this committee type's charter. Additionally, check `.roles/` for `inertia-advocate.md` — if it exists and is not already in the charter, add the Inertia-Advocate as a mandatory first speaker. If no charter exists, fall back to Signal defaults (PM, Architect, Inertia-Advocate) and state them.
...
Read `.roles/` for this committee type's charter. Load every named role. If no charter exists, fall back to Signal defaults (PM, Architect, Inertia-Advocate) and state the fallback.
...
Committee: [TYPE: ROB / shiproom / arch-board / other] — [agenda item title]
Charter loaded from: [path to .roles/ file, or "Signal defaults — no charter found: [list fallback roles]"]
Participants:
