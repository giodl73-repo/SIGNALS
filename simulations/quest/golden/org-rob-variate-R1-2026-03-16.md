...
.craft/roles/. Each role reviews from their perspective. Output: stage document in ROB format.
...
Glob `.craft/roles/` to discover all available roles. Load each role file and extract:
...
Role: load `.craft/roles/` team-level role(s). If no team role exists, construct from
...
```
## Stage: teams
Role: [role name from .craft/roles/ or constructed]
...
Role: load `.craft/roles/signal/pm.md`.
...
Read `.craft/roles/` to load all role files. For each role, extract orientation, lens, and
expertise. If a required role has no file in `.craft/roles/`, construct a minimal role inline
...
**leadership**
- Role loaded from `.craft/roles/` executive or VP archetype; construct if absent.
...
Read `.craft/roles/` and load all available role files. Required: load
`.craft/roles/signal/inertia-advocate.md` before any stage begins. The inertia-advocate
...
```
## Stage: {stage-name}
Role: {role-name} — loaded from .craft/roles/{path}
Orientation: {one sentence from orientation.frame}

### Inertia Challenge

**Inertia-Advocate** (`.craft/roles/signal/inertia-advocate.md`):
...
**leadership**
Role: VP/executive archetype. Load from `.craft/roles/` if available; construct if not.
...
**teams**
Role: team-level archetype. Load from `.craft/roles/` or construct.
...
**pm**
Role: load `.craft/roles/signal/pm.md` — use `lens.verify` verbatim as the review checklist.
...
**arch-board**
Role: load `.craft/roles/signal/architect.md` — use `lens.verify` as the review checklist.
...
Read `.craft/roles/` and load all role files. For each role, capture orientation, lens,
...
- **Role** — who conducts this stage and from which `.craft/roles/` file
...
Role: VP/executive archetype from .craft/roles/ (construct if absent)
...
Role: team-level archetype from .craft/roles/ (construct if absent)
...
Before running any stage, read every file in `.craft/roles/`. For each role file, you will
...
If a role is not in `.craft/roles/`, construct the voice from the stage description and
...
### {Role Name} Speaking
*Role loaded from: .craft/roles/{path} | Archetype: {archetype}*
...
**leadership**
Load from `.craft/roles/` executive archetype (construct if absent as "VP of Engineering/Product").
...
**teams**
Load from `.craft/roles/` team archetype (construct if absent as "Engineering Lead / Team Representative").
...
**pm**
Load `.craft/roles/signal/pm.md`. Voice this role using its documented lens items directly.
...
**arch-board**
Load `.craft/roles/signal/architect.md`. Voice using `lens.verify` and `lens.simplify` directly.
