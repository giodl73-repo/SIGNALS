...
2. Read `.craft/roles/` -- load orientation.frame and lens for each assigned role.
...
Via: must name a specific orientation item from the loaded .craft/roles/ file.
...
.craft/roles/ file) -- not just the role's name. Knowing the orientation frame makes
