...
Load the charter file from `.roles/` that matches this committee type. Pull every role with its documented orientation. If no charter exists, say so explicitly and fall back to Signal defaults (PM, Architect, Inertia-Advocate). Fill Committee Type, Agenda Item, Charter Source, and the Participants list.
