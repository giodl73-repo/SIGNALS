...
If `.roles/` is absent or empty, produce instead:
...
Check `.roles/` for role files relevant to {topic} or the domain. Produce a ROLES-LOADED or ROLES-NOTE block before writing anything else.
...
Read `.roles/` for role files relevant to {topic} or the domain.
...
Source: inferred (no .roles/ files found for this domain)
