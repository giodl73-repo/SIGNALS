...
- **Standard** (default): Fill three archetype slots from `.roles/signal/`: CHALLENGER
...
Read `.roles/signal/`. Fill each archetype slot with a role from that directory.
...
- **Standard** (default): Select one role per archetype slot: CHALLENGER, DOMAIN, LIFECYCLE.
  Map each to a role in `.roles/signal/` whose `expertise.relevance` matches this artifact
...
Read `.roles/signal/`. Assign each slot to a role from that directory.
