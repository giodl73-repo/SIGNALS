...
If `.roles/` is absent or empty: output `ERROR: .roles/ not found. Halting.`
...
- **Role**: exact name from `.roles/` file stem
...
Read `.roles/`. Build role pool from files found. No invented roles.
If directory missing: `ERROR: .roles/ not found.` Stop.
...
4. If directory missing or empty: halt. Output: `ERROR: .roles/ unavailable.`
...
| Column | Constraint |
|--------|-----------|
| Role | SHALL be the filename stem of a role file in `.roles/`. MAY NOT be invented. |
...
Read `.roles/`. Build the available role pool from the files present.
The pool is strictly limited to discovered files. If `.roles/` is absent:
output `ERROR: .roles/ not found.` Execution halts.
...
**P1.1** Open `.roles/`. Read every file.
...
If `.roles/` missing: output `ERROR: registry unavailable.` Halt.
...
- Role name matches a `.roles/` file
