...
- **Roles**: full stock list from the spec; any custom roles in `.roles/signal/`
...
- Which roles will run? (stock roles in the spec; custom roles in `.roles/signal/`
  if present)
...
- **Roles**: stock list from spec; custom roles in `.roles/signal/` if present
