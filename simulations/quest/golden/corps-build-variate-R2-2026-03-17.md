...
The skeleton is now the contract. The directory structure of `.craft/roles/` must mirror
...
to `.craft/roles/exec-office/` with `role-type: shared-group` and `role-type: exec-office`
...
After all teams: write shared-group roles (`.craft/roles/shared/`) and exec-office roles
(`.craft/roles/exec-office/`) if declared in `org.yaml`.
...
COVERAGE-AUDIT:
  ...
  [A] Role file count
      ...
      Actual:   [count of .craft/roles/ files written, excluding index/README]
  ...
  [C] Directory fidelity
      For each area in org.yaml:
        [area name]: [directory exists as .craft/roles/{area-slug}/ | MISSING]
...
Every `.craft/roles/` file must contain all five schema fields. Reference them as F-1..F-5.
...
[ ] [team name] — [n] role files expected at .craft/roles/{area-slug}/
[ ] [team name] — [n] role files expected at .craft/roles/{area-slug}/
...
...
Per team sequence:
1. Standard roles (path: `.craft/roles/{area-slug}/{role-slug}.md`, `role-type: standard`)
...
After all teams: shared-group roles to `.craft/roles/shared/`, exec-office to
`.craft/roles/exec-office/`. Skip if not declared.

Mark each team's checklist entry as completed once files are written:
`[x] [team name] — [n] files written at .craft/roles/{area-slug}/`
...
COVERAGE-AUDIT:
  ...
  [A] File count
      ...
      Actual written:    [count of .craft/roles/ files, excluding index/README]
      Status:   [PASS | FAIL — list any team with wrong count]

  [B] Inertia Advocate per team
      [team name]: [PRESENT | MISSING] — .craft/roles/{area}/inertia-advocate.md
      (repeat for each team in skeleton checklist)
      Status:   [PASS | FAIL]

  [C] Directory-to-skeleton fidelity
      For each area in skeleton:
        [area slug]: [EXISTS | MISSING as .craft/roles/{area-slug}/]
...
```
TEAM CHECKLIST:
[ ] [team name] — [n] files expected at .craft/roles/{area-slug}/
[ ] ...
...
After all teams:
- Shared-group roles: `.craft/roles/shared/`, `role-type: shared-group`, populate F-1..F-5
- Exec-office roles: `.craft/roles/exec-office/`, `role-type: exec-office`, populate F-1..F-5
