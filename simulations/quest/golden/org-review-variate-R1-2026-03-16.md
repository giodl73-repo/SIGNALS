...
- **`--depth deep`**: Include every role found in `.roles/signal/`. State the total role count
  before proceeding.
...
- **Standard** (default): Select 2–4 roles whose `expertise.relevance` matches this artifact type.
  ...
  included.
- **`--depth deep`**: Include all roles in `.roles/signal/`. State the count.
...
Read `.roles/signal/`. Enumerate available roles.
...
organizational reviewers drawn from the role registry at `.roles/signal/`. Each reviewer
...
No role may be invented. Every reviewer must have a corresponding file in `.roles/signal/`.
...
3a. Load role file from `.roles/signal/{role}.md`.
...
1. Read the role file from `.roles/signal/{role}.md`.
