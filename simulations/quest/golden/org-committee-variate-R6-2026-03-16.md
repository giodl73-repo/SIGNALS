...
**Load participants.** Read `.craft/roles/` for this committee type's charter. Load every named role. If no charter exists, use Signal defaults (PM, Architect, Inertia-Advocate) and state them explicitly.
...
Load participants from `.craft/roles/` for this committee type. State every loaded role. Fallback: Signal defaults (PM, Architect, Inertia-Advocate) — state them explicitly.
...
Load participants from `.craft/roles/` for this committee type. Additionally, check `.craft/roles/` for an `inertia-advocate.md` file — if it exists and is not already in the charter, add it as a mandatory first speaker. Fallback: Signal defaults (PM, Architect, Inertia-Advocate) — state them.
