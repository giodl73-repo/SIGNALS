...
Path: `.roles/shared/{role-slug}.md`
...
against the actual `.roles/` file count, directly satisfying C-10. The ASCII diagram
...
Each role file path: `.roles/{area-slug}/{role-slug}.md`
...
to `.roles/exec-office/`. Skip if none declared.
...
The Total must equal the exact number of `.roles/` files written in this run
...
from it — two outputs: a set of typed role files in `.roles/` and an `org-chart.md`
...
For each team in the org, generate one file per role in `.roles/{area-slug}/`.
...
If `org.yaml` declares shared-group roles, write them in `.roles/shared/`. If it
declares exec-office roles, write them in `.roles/exec-office/` — not inside any
...
After all teams, write shared-group roles to `.roles/shared/` and exec-office
roles to `.roles/exec-office/` (only if declared in `org.yaml`).
