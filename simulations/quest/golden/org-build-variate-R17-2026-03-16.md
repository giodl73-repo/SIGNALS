...
| 7 | Write role files per area subdir; inertia-advocate scope verbatim from SCOPE-TEMPLATE[T1-PRESSURE] | .roles/ |
...
Sub-step AREA: Map roles to area subdirectories under `.roles/`.
...
table, and inertia assessment; (2) `.roles/{area}/{role}.md` for every role.
...
2. Write `.roles/{area}/{role}.md` for every role: five fields, grouped by area subdir.
