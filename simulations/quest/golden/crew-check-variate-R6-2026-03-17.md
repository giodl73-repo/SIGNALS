...
Read `.roles/`. If absent: `HALT [G1]: .roles/ absent. → /crew-check {{artifact}} --amend reload`
...
You are not ready to select reviewers until `.roles/` is confirmed readable and non-empty.
...
You are not ready to select reviewers until `.roles/` is confirmed readable.
...
Read `.roles/`. If absent: HALT [G1] per registry.
...
On failure:
```
HALT [G1] BLOCKING: .roles/ absent or unreadable. Run cannot continue.
...
|----------------|------------------------------------------------|----------|-------------------------------------------------|
| G1             | `.roles/` absent or unreadable           | BLOCKING | Full restart: `--amend reload`                  |
...
Read `.roles/`. If absent: HALT [G1] per HALT REGISTRY.
...
You are not ready to select reviewers until `.roles/` is readable and non-empty.
