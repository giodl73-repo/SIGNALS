...
date: 2026-03-15
rounds: 20
rubric_final: program-plan-rubric-v20-2026-03-15.md
score: 350
status: GOLDEN
---

# program-plan -- Golden Prompt

Round 20. Source: V-05 (BOUNDED BLOCK PROTOCOL, C-57+C-58+C-59), QU5-simplified (20% reduction, all essential criteria preserved).
...
**1. Universal per-step active verification (C-58)**
Every construction step with constraints carries a SELF-VERIFY NOT/IS block. Prior variates placed active verification only at the Component 1 declaration site (C-55). V-05 extends the same audit discipline to Steps 1-10, making constraint compliance a continuous protocol rather than a one-time check at the BAD PLAN.

**2. Template field saturation (C-59)**
Every constrained YAML field in the Step 8 schema carries an inline echo of its mandate and NOT list (`strategy:`, `name:`, `intent:`, `skills:`, `gate_fail:`, `gate:`). Prior variates enforced co-location only at the BAD PLAN's field lines (C-26/C-37/C-39). V-05 extends the same co-location discipline to the authoritative template itself, making every authoring decision locally self-documenting at the point of production.

**3. Dual-site active verification echo (C-57)**
The Component 1 header carries both the static mandate echo (C-56: `# Format: 4-column pipe table...`) AND the active NOT/IS checklist (C-57: `# Verify before finalizing...`). The Component 1 declaration site (BOUNDED BLOCK PROTOCOL section) carries the same checklist independently. This closes the document-skip gap: a reader who jumps directly to the BAD PLAN encounters the active trigger without having read the governing declaration.

**4. Named protocol first (C-51) with prescriptive column schema (C-52/C-53/C-54)**
The BOUNDED BLOCK PROTOCOL section appears before the construction steps, making it a governing declaration rather than an annotation attached to the example. The column schema uses MUST/SHALL language and explicitly names all three forbidden formats by category. This structural positioning prevents the protocol from being treated as commentary.

**5. Three-anchor reinforcement for every essential criterion**
Each of the seven essential/recommended criteria has independent coverage at: (a) the CORRECTION TABLE (wrong-to-correct pairs), (b) the BAD PLAN block (inline `# WRONG C-NN` tags), and (c) the YAML template scaffold (inline `# Mandate` echoes). No essential criterion depends on a single teaching mechanism.

---

## Final Rubric Criteria Summary

**Rubric**: v20 | **Total criteria**: 59 (4 essential + 3 recommended + 52 aspirational) | **Max score**: 350

| Tier | Count | Points | V-05 score |
|------|-------|--------|------------|
| Essential (C-01--C-04) | 4 | 60 | 60/60 |
| Recommended (C-05--C-07) | 3 | 30 | 30/30 |
| Aspirational (C-08--C-59) | 52 | 260 | 260/260 |
| **Composite** | **59** | **350** | **350/350** |

**New criteria unlocked in R20:**

| Criterion | Pattern | Prerequisite |
|-----------|---------|--------------|
| C-57 | Active NOT/IS checklist echoed at Component 1 header (dual-site) | C-55 + C-56 |
| C-58 | Universal per-step active verification at every constraint-bearing construction step | C-55 |
| C-59 | Per-field mandate echo saturation across all constrained YAML template fields | C-56 |

**Criterion chain**: C-53 -> C-54 -> C-55 -> C-58 (universal step audit); C-55 + C-56 -> C-57 (dual-site trigger); C-56 -> C-59 (template saturation). C-57 sits at the intersection of both C-55 and C-56 sub-chains.

```json
{"skill_target": "program-plan", "rounds": 20, "score": 350, "max": 350, "essential_pass": "4/4", "recommended_pass": "3/3", "aspirational_pass": "52/52", "source_variate": "V-05", "qu5_simplified": true, "word_count": 2754, "golden_criteria": ["C-57", "C-58", "C-59"]}
```
