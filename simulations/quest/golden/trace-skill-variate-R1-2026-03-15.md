...
- **ROLE-SELECTION**: Stock roles active; any custom roles from `.roles/signal/`
...
List the stock roles the skill spec defines. If custom roles exist in `.roles/signal/`,
...
- Roles: stock list from spec; custom roles from `.roles/signal/` if present
...
- which roles will run? (stock roles from the spec; any custom roles in `.roles/signal/`)
