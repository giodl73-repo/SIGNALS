...
3. Read `.roles/` — load orientation and lens for each role you will select.
...
Orientation: [one phrase from .roles/ describing their lens]
...
Open `.roles/`. For each role you'll run: read their orientation — what they're paid
...
Let the role's orientation from `.roles/` shape what they find. If a finding reads
...
Read `.roles/` for each role in the routing table. Load orientation and lens.
...
3. Read `.roles/` — orientation and lens for each role you will use.
...
Orientation: [one phrase from .roles/ describing this role's lens]
