...
# mock-accept Variate — Round 9

**Date:** 2026-03-17
**Skill:** mock-accept
**Rubric:** v9 (C-01–C-05 essential, C-06–C-08 recommended, C-09–C-33 aspirational;
             C-33 new this round; denominator 25)
**Baseline:** R8 V-01 (sole golden, 25/25 against v9: 3-column COST REGISTER Namespace|Phase|Cost-of-MOCK;
             C-33 PASS via Phase column enabling column-inspection cross-verification against INERTIA LEDGER)
**Round:** R9 — 3 single-axis passes (V-01 through V-03), 2 combinations (V-04, V-05)

---

## Axis-Assignment Plan

R8 V-01 is the sole golden at 25/25 against v9. C-33 requires the Phase column; R8 V-01 satisfies it
with `Namespace | Phase | Cost-of-MOCK`. R9 probes three structural questions that C-33's cross-
verification capability opens up but does not mandate:

1. Does C-33's "column inspection" guarantee hold equally when Phase is the first column (grouping
   Architect rows together and Strategy rows together) vs the second column? Or does the current
   Namespace-first ordering already provide optimal scanability?

2. Does C-33's cross-verifiability require only that the Phase column be present (satisfied by R8 V-01),
   or should GATE E-COST explicitly verify the per-phase row split against the INERTIA LEDGER tallies
   from GATE C and GATE D? R8 V-01 provides structural possibility; V-02 tests whether a mandatory
   verification assertion adds structural completeness.

3. Does DEFAULT DECISION POSITION benefit from a numeric scoreboard format (explicit counters
   REAL-REQUIRED: 9, MOCK-ACCEPTED: 0) that updates at each gate, vs the current prose-form
   inertia declaration? The prose form satisfies C-30; the scoreboard tests whether numeric tracking
   from the start changes the evaluation frame.

| Plan | Axis | Target | What changes from R8 V-01 | Predicted |
|------|------|--------|---------------------------|-----------|
| V-01 | output-format (Phase-first columns) | C-33 column ordering | Phase column moves to position 1: Phase\|Namespace\|Cost-of-MOCK | 25/25 |
| V-02 | lifecycle-emphasis (GATE E-COST phase split) | C-33 cross-verification | GATE E-COST adds per-phase count assertions tied to GATE C / GATE D INERTIA LEDGER | 25/25 |
| V-03 | inertia-framing (numeric scoreboard) | C-30 + C-31 expressiveness | DEFAULT DECISION POSITION uses DECISION SCOREBOARD with explicit counters, updated at each gate | 25/25 |
| V-04 | combined: Phase-first + GATE E-COST phase-split | V-01 + V-02 | Phase-first columns + per-phase count assertion in GATE E-COST | 25/25 |
| V-05 | combined: Phase-first + scoreboard + GATE E-COST phase-split | V-01 + V-02 + V-03 | All three single-axis improvements combined | 25/25 |

---

## V-01 — Output Format: Phase-First COST REGISTER Columns

**axis:** output-format (Phase column position)
**hypothesis:** R8 V-01 presents the MOCK COST REGISTER with columns ordered Namespace | Phase |
Cost-of-MOCK. C-33 requires an explicit Phase column enabling cross-verification against the INERTIA
LEDGER by column inspection — but does not specify column ordering. Moving Phase to position 1
(Phase | Namespace | Cost-of-MOCK) groups Architect rows together and Strategy rows together,
making the per-phase count verifiable by reading consecutive rows rather than filtering a middle
column. The cross-verification property is preserved; the grouping improves scanability. All other
steps identical to R8 V-01. Predicted score against v9: 25/25.
...
## V-02 — Lifecycle Emphasis: GATE E-COST Phase-Split Verification

**axis:** lifecycle-emphasis (GATE E-COST phase-split assertion)
**hypothesis:** R8 V-01's GATE E-COST asserts only total row count. C-33 requires the Phase
column to enable cross-verification against per-phase INERTIA LEDGER tallies — but R8 V-01 makes
this cross-verification possible without requiring it. V-02 adds explicit per-phase count
assertions inside GATE E-COST: Architect rows must equal GATE C INERTIA LEDGER's "earned
MOCK-ACCEPTED" count; Strategy rows must equal GATE D INERTIA LEDGER's count. This converts
C-33's structural possibility into a mandatory verification step — the register's Phase column
must be consistent with the phase-gate ledger counts, not just present. All other steps
identical to R8 V-01 (Namespace|Phase|Cost column order preserved). Predicted score against
v9: 25/25.
...
CONSTRAINT: Each Phase cell must be exactly "Architect" or "Strategy".
HALT. Delete any Phase cell that is not exactly "Architect" or "Strategy". Rewrite.
...
## V-03 — Inertia Framing: Numeric DECISION SCOREBOARD

**axis:** inertia-framing (DEFAULT DECISION POSITION as numeric scoreboard)
**hypothesis:** R8 V-01's DEFAULT DECISION POSITION is prose-form ("Every namespace begins as
REAL-REQUIRED…"). C-30 requires a named block before STEP 1 declaring REAL-REQUIRED as the
default. V-03 replaces the prose with a structured DECISION SCOREBOARD that names explicit
numeric counters (REAL-REQUIRED: 9, MOCK-ACCEPTED: 0) and announces when those counters update
(at GATE C and GATE D). This makes inertia numerically visible — the evaluator tracks movement
from the scoreboard rather than inferring the default from prose. The scoreboard satisfies
C-30 (named block before STEP 1 with REAL-REQUIRED as default) and reinforces C-31 (INERTIA
LEDGER as the gate-exit expression of the scoreboard's movement). All other steps identical
to R8 V-01. Predicted score against v9: 25/25.
...
## V-04 — Combined: Phase-First Columns + GATE E-COST Phase-Split Verification

**axis:** combined (V-01 Phase-first + V-02 GATE E-COST phase-split)
**hypothesis:** V-01 moves Phase to column 1, grouping Architect rows above Strategy rows.
V-02 adds explicit per-phase count assertions in GATE E-COST. Together: Phase-first grouping
makes the Architect/Strategy row split countable by visual scan; the GATE E-COST assertions
verify that scan matches the INERTIA LEDGER tallies. The combination tightens C-33's cross-
verification path from "possible by column inspection" to "required and verified at GATE E-COST".
All other steps identical to R8 V-01. Predicted score against v9: 25/25.
...
Collect all Cost-of-MOCK entries from STEP 3 and STEP 4 into a named registry.
One entry per namespace that received MOCK-ACCEPTED in either evaluation phase.
Order rows by Phase: all Architect rows first, then all Strategy rows.
...
CONSTRAINT: Each Phase cell must be exactly "Architect" or "Strategy".
  Architect rows must appear before Strategy rows.
HALT. Delete any Phase cell that is not exactly "Architect" or "Strategy". Rewrite.
HALT. If Strategy rows precede Architect rows, reorder the table before advancing.
...
## V-05 — Full Integration: Phase-First + Numeric Scoreboard + GATE E-COST Phase-Split

**axis:** combined (V-01 Phase-first + V-03 numeric scoreboard + V-02 GATE E-COST phase-split)
**hypothesis:** Full integration of all three single-axis improvements. Phase-first column
ordering groups the register by phase. The DECISION SCOREBOARD makes inertia numerically
explicit from the start with running updates at GATE C and GATE D. GATE E-COST verifies the
per-phase row split against the INERTIA LEDGER tallies. Together these three changes form a
complete audit chain: inertia is declared numerically before evaluation (scoreboard), tracked
incrementally at phase gates (INERTIA LEDGER + scoreboard update), aggregated by phase in the
register (Phase-first table), and verified at GATE E-COST against both INERTIA LEDGER values.
C-33 cross-verification is not only possible by inspection but required by gate assertion.
Predicted score against v9: 25/25.
...
This scoreboard governs STEP 3 (Architect phase) and STEP 4 (Strategy phase).
GATE C records the Architect phase movement. GATE D records the Strategy phase movement.
GATE E-COST verifies the final register split matches the phase-gate totals.
...
GATE E-COST — COST REGISTER COMPLETE
  ...
  Strategy rows: ___. Expected: ___ (= GATE D INERTIA LEDGER "earned MOCK-ACCEPTED" count).
  CONSTRAINT: All three counts must match. Per-phase split must equal the corresponding
    INERTIA LEDGER tally at GATE C and GATE D respectively. Also matches DECISION SCOREBOARD
    FINAL values.
  HALT. If any count mismatches, identify the discrepancy against the source INERTIA LEDGER
    and correct the register before proceeding.
  Do not proceed to STEP 6 until GATE E-COST is written with all three counts confirmed.
