...
ROLE: [name loaded from .roles/] — [their orientation in one phrase]
...
ROLE: [name] | ORIENTATION: [lens from .roles/ — one phrase]
...
2. Read `.roles/` — persona orientation and lens for each assigned role.
...
ROLE: [name from .roles/] | LENS: [role orientation — one phrase]
