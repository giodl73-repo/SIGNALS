...
Read `.craft/roles/`. Role locator is valid if the file exists and `name:` is non-empty.

**Locator self-correction gate**: If locator valid → include. If locator invalid → rewrite to the most specific anchor that exists in `.craft/roles/`, then include.
...
Read `.craft/roles/`. A locator is valid if the file exists and `name:` is non-empty.

**Locator self-correction gate**: For each role needed in this review:
- If locator valid → include.
- If locator invalid → rewrite to the most specific anchor that exists in `.craft/roles/`, then include.
...
6. All role locators either resolved to valid `.craft/roles/` files or were self-corrected before inclusion.
