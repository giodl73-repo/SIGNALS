...
- `.roles/` — role orientation and lens for each reviewer
...
3. Read `.roles/` — orientation and lens for each role you will select.
...
3. Read `.roles/` — orientation and lens for each selected role.
...
- `.roles/`: for every reviewer you'll run — how they read code, what failure
...
Read `.roles/` for each reviewer. Their orientation tells you what failure patterns
...
Load `.roles/` for each role in the routing table. Extract orientation and lens.
