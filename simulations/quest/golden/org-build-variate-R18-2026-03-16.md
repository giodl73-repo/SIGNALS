...
Outputs produced:
  ...
  2. .roles/{area}/{role}.md -- one file per role, all five fields present.
...
Sub-step B -- AREA: Group enumerated roles into area subdirectories.
  MUST produce a minimum of 2 area subdirectories under .roles/.

- FORBIDDEN: beginning Sub-step C (role file generation) before completing Sub-step B
  (AREA grouping).

Sub-step C -- GENERATE: Write .roles/{area}/{role}.md for every enumerated role.
...
.roles/{area}/inertia-advocate.md
  orientation:      ...
  lens:             ...
  expertise:        ...
...
Outputs:
  1. org-chart.md  2. .roles/{area}/{role}.md (all five fields, all roles)
...
Sub-step B -- AREA: Assign each enumerated role to an area subdirectory.
  MUST produce minimum 2 area subdirectories under .roles/.

- FORBIDDEN: beginning Sub-step C (file generation) before completing Sub-step B (AREA --
  area assignments recorded).

Sub-step C -- GENERATE: Write .roles/{area}/{role}.md for each role.
...
Outputs:
  ...
  2. .roles/{area}/{role}.md (every role, all five fields)
...
MUST generate: (1) org-chart.md (2) .roles/{area}/{role}.md for every role.
...
Sub-step C -- GENERATE:
MUST: write .roles/{area}/{role}.md for every enumerated role.
...
Outputs:
  ...
  2. .roles/{area}/{role}.md -- verdicted role files, including inertia-advocate
...
Sub-step C -- GENERATE: Write all role files.
  Every .roles/{area}/{role}.md MUST contain:
...
2. Confirm T4-CHART-COMPLETE = YES.
3. Emit artifact manifest: org-chart.md path + all .roles/{area}/{role}.md paths.
