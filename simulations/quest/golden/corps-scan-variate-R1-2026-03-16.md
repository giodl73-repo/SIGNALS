...
corps-scan is a *draft-only* skill. You will not write `.roles/` files. You will not
...
This is a **draft-only** skill. At no point will you write `.roles/` files, instruct
...
**Draft-only constraint**: This phase produces `org.yaml` only. No `.roles/` files.
...
- [ ] No `.roles/` content anywhere in this output
...
**Non-negotiable constraint**: This skill does not write `.roles/` files. It does not
