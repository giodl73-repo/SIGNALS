...
This section governs all tables and blocks in Roles 3–8, opening with a Schema Production
Registry then schema body entries.
...
- BASELINE: connector API call volume at burst onset at the Role 3 BURST PATH TABLE endpoint,
  SCHEMA-A BASELINE SOURCE column — before any mitigation. Routes through the BURST PATH
  TABLE by name and column; does not independently cite a Finding ID.
...
- **CELL ANCHOR violation:** cell missing the "BP-xx:" prefix entirely. Format violation,
  scan-time detectable.
- **UNRESOLVED REFERENCE violation:** cell has a syntactically valid "BP-xx:" prefix but
  the named Finding ID does not exist in Role 3's BURST PATH TABLE. Detectable by
  cross-referencing the prefix against the BURST PATH TABLE's BP ID column.
...
- **SCHEMA-A BASELINE SOURCE VIOLATION: a BASELINE cell cites a Finding ID directly
  ...
  (e.g., "BURST PATH TABLE / BP-01 / SCHEMA-A BASELINE SOURCE: 600 req/min").
  Detectable at scan time by checking whether the BASELINE cell names the BURST PATH
  TABLE or names a Finding ID directly.**
...
- VOLUME RANGE: derived from threshold in Verdict Claim (a).
- Delta: quantified improvement (relative labels without a value are SCHEMA-V CONTENT violations).
...
schema body entries present. SCHEMA-A BASELINE routes through BURST PATH TABLE by name and
column. SCHEMA-A CELL ANCHOR, both violation classes, and SCHEMA-A BASELINE SOURCE
VIOLATION rejection clause declared. No analysis has begun.
...
- ENDPOINT NAME: canonical name used in all downstream references.
- SCHEMA-A BASELINE SOURCE: baseline measurement; authoritative source for Format Contract
  SCHEMA-A BASELINE definitions. Column name identifies the schema it feeds (backward);
  Format Contract SCHEMA-A definition names this table and column (forward).
- All downstream BP-xx prefixes must reference a BP ID in this table's BP ID column.
...
open with "BP-xx: [value]" where BP-xx is the BURST PATH TABLE BP ID. Example cells:
...
Three independent traceability planes. Failure in one does not affect the others.

**Plane 1 — Schema Production Registry Completeness (e.1–e.3) [C-47]:**

- e.1: Count registry rows. Count table-producing roles. If row count ≠ producing role
       count: **REGISTRY GAP** violation — report registry row count, producing role count,
       missing role(s).
- e.2: For each registry row, confirm GATE CONDITION is filled. If not: **REGISTRY GATE GAP**
       (report schema name).
- e.3: For each schema body entry, confirm it has a matching registry row. If not:
       **ORPHANED SCHEMA** violation (report schema name). Both classes must appear under
       separate named labels in the findings output.
...
- e.4: Audit BURST PATH TABLE and Format Contract SCHEMA-A BASELINE definition:
  - Forward check: does SCHEMA-A BASELINE definition name the BURST PATH TABLE and
    SCHEMA-A BASELINE SOURCE column? If not: FORWARD TRACEABILITY ABSENT.
  - Backward check: does BURST PATH TABLE have a column named SCHEMA-A BASELINE SOURCE?
    If not: BACKWARD TRACEABILITY ABSENT.
  - Cell routing check: for each SCHEMA-A BASELINE cell in Roles 7 and 8a, does the cell
    name the BURST PATH TABLE (routing) or cite a Finding ID directly? Direct citation:
    **SCHEMA-A BASELINE SOURCE VIOLATION**. Report cell location and direct citation found.
...
- e.5: Scan every data cell in every SCHEMA-A table (Roles 7 and 8a). If cell does not open
       with "BP-xx:" prefix: **CELL ANCHOR** violation (scan-time detectable; report location
       and column).
- e.6: For each cell with "BP-xx:" prefix, verify the named Finding ID exists in Role 3's
       BURST PATH TABLE BP ID column. If not found: **UNRESOLVED REFERENCE** violation
       (report cell location, claimed prefix, and gap).

  Violation class distinction: CELL ANCHOR and UNRESOLVED REFERENCE must appear as separate
  named classes. **The prohibited merged form is "missing or invalid prefix" — any findings
  report that uses this phrase, or an equivalent combined label, to describe both classes in
