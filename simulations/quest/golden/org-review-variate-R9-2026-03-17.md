...
- `{{reviewer_set}}` -- domain reviewer roles from `.craft/roles/` (auto-selected
...
Read `.craft/roles/{{reviewer_set}}/`. Assign each slot.
...
- `{{depth}}` -- review depth: quick | standard | deep (default: standard)
- `{{artifact}}` -- the artifact under review (injected at end of prompt)
- `{{reviewer_set}}` -- domain reviewer roles from `.craft/roles/`
