# listen-support — QU5 Simplification Pass (R20 V-05)

**Date:** 2026-03-15
**Source:** listen-support-variate-R20-2026-03-15.md V-05
**Rubric target:** v20 (350 pts ceiling — C-01 through C-60)
...
| Item | Removed | Words |
|------|---------|-------|
| Step 1 forward-reference note | "This target will be referenced at Step 5..." | ~22 |
| Step 2C cross-reference | "Applied in: Steps 5, 5B, and Pass 2 INFERENCE AUDIT" | ~9 |
| Step 2C gate reminder | "Do not proceed until filled with your own-words paraphrase." | ~10 |
| Step 3A forward-reference note | "This table is re-verified at Step 5B Constraint 1B..." | ~18 |
| Phase 1 body tail | "No third-person. Every old-tool reference: product name from Step 1b-ii." | ~10 |
| Phase 2 body tail | "No third-person. Product name for old tool." | ~8 |
| Phase 3 body tail | "No third-person. Product name for old tool." | ~8 |
| Step 8 Entry-specific guidance block | Full block (label + entries 1, 2, 3) | ~45 |
| Part B item 2 cell-failure explanation | "A NO in any cell is a concreteness failure..." | ~17 |
| Paraphrase consistency verbose qualifier | "triple-clause verbatim (do not paraphrase, do not summarize, copy it verbatim word-for-word)" → "verbatim" | ~12 |
| State final probe completion lines | C-58/C-59/C-60 probe: complete (×3 lines), updated "C-57" to "C-60" | ~25 |

**What was NOT removed:** forward-declaration tables, all constraint gates, correction instructions ("If any MISMATCH: correct..."), all Prohibited sentinels, inference rule verbatim enforcement language, "Confirmation: this rule applies to every severity assignment", all verification state blocks.

**Rationale for each removal:** All removed items are either (a) cross-references whose content is enforced at the referenced location, (b) reminders already covered by Step 4 Performance Mode Declaration, (c) labels for template content already present in the template, or (d) redundant prose explanations of structural facts (a NO in a table cell is self-evident).
