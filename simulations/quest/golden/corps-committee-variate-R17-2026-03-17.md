...
| ID   | Axis                                                                                          | Hypothesis |
|------|-----------------------------------------------------------------------------------------------|------------|
| V-01 | C-46 only -- FAILS template extended with C-44/C-45 self-referential entries                 | Adding CORRECT examples for C-44 and C-45 to the FAILS SYNTAX TEMPLATE -- alongside the existing C-42/C-43 entries -- satisfies C-46 (recursive template currency) without altering BEFORE YOU START. The test: does a reviewer encountering a template-currency violation or a missing three-leg preamble row have a canonical FAILS entry to cite? With V-01, yes. C-47 is not satisfied: BEFORE YOU START still carries no pre-flight halt for FAILS template currency. C-48 is not satisfied: BEFORE YOU START still carries no pre-flight halt for CO-DEPENDENCY PREAMBLE completeness. |
| V-02 | C-47 only -- BEFORE YOU START pre-flight halt for FAILS template currency                     | Adding an imperative pre-flight line to BEFORE YOU START that halts if the FAILS SYNTAX TEMPLATE omits self-referential entries for criteria added after C-39 satisfies C-47 at session-open. The test: does template-currency enforcement appear at the earliest possible execution site, independently of the template itself? With V-02, yes. C-46 is not satisfied: the FAILS template still carries only C-42/C-43 self-referential entries -- no entries for C-44/C-45. C-48 is not satisfied: no CO-DEPENDENCY PREAMBLE pre-flight in BEFORE YOU START. |
| V-03 | C-48 only -- BEFORE YOU START pre-flight halt for CO-DEPENDENCY PREAMBLE completeness        | Adding an imperative pre-flight line to BEFORE YOU START that halts if the CO-DEPENDENCY PREAMBLE omits the C-43/C-41 three-leg row satisfies C-48. The test: does preamble-completeness enforcement appear at session-open, independently of the preamble's static content? With V-03, yes. C-46 is not satisfied: the FAILS template carries no C-44/C-45 self-referential entries. C-47 is not satisfied: no FAILS template currency pre-flight in BEFORE YOU START. |
| V-04 | Combination: C-46 + C-47                                                                      | Pairing FAILS template extension with C-44/C-45 entries (C-46) and a BEFORE YOU START pre-flight halt for FAILS template currency (C-47) closes two of the three R17 gaps simultaneously. The two changes operate at independent sites -- FAILS SYNTAX TEMPLATE vs BEFORE YOU START -- with no interaction effects. C-48 is not satisfied: BEFORE YOU START still carries no CO-DEPENDENCY PREAMBLE completeness pre-flight. The combination is the minimum change from R16 V-04 that satisfies C-46 + C-47 without adding the C-48 preamble detection layer. |
| V-05 | Full integration: C-46 + C-47 + C-48 -- defense in depth across all three new axes           | All R17 structural additions on the R16 V-04 baseline, plus: FAILS template extended with C-44/C-45 self-referential entries (C-46); BEFORE YOU START gains C-44 pre-flight halt (C-47); BEFORE YOU START gains C-45 pre-flight halt (C-48). This is the only variation where: a template-currency violation for C-44/C-45 produces a FAILS template citation (C-46); a stale FAILS template produces a BEFORE YOU START halt before any skeleton is printed (C-47); a missing three-leg preamble row produces a BEFORE YOU START halt at session-open (C-48). In V-05 the C-47 pre-flight text also updates its criteria list to include C-44/C-45 as aspirational pairs now requiring template entries, tagging any omission as a C-46 miss. Remove any one mechanism and a gap reopens. |

---

## V-01 -- FAILS Template Recursive Currency Extension (C-46, single axis)

**Axis:** Extend FAILS SYNTAX TEMPLATE with self-referential entries for C-44 and C-45
**Hypothesis:** Adding CORRECT examples for C-44 and C-45 to the FAILS SYNTAX TEMPLATE --
alongside the existing C-42/C-43 entries -- satisfies C-46 (the recursive application of the
C-40 currency pattern to the C-44/C-45 pair itself). BEFORE YOU START is unchanged from the
R16 V-04 baseline: no pre-flight halts for C-44 or C-45 currency. C-47 and C-48 are not
satisfied.
...
```
=== org:committee SIMULATION -- SKELETON (V-01: FAILS Template Recursive Currency) ===
...
## V-02 -- BEFORE YOU START C-44 Pre-Flight Halt (C-47, single axis)

**Axis:** Add C-44 pre-flight halt to BEFORE YOU START
**Hypothesis:** An imperative pre-flight line in BEFORE YOU START that halts execution if the
FAILS SYNTAX TEMPLATE omits self-referential entries for criteria added after C-39 satisfies
C-47 (the C-44 detection layer at session-open). The halt fires before any skeleton is printed,
making template staleness visible at the earliest possible execution site. C-46 is NOT satisfied:
the FAILS template still carries only C-42/C-43 self-referential entries -- no C-44/C-45 entries.
C-48 is NOT satisfied: no CO-DEPENDENCY PREAMBLE pre-flight in BEFORE YOU START.

The single axis: the new check appears in BEFORE YOU START, not in the FAILS template.
BEFORE YOU START, skeleton (STEP 1), all phase fill rules, CO-DEPENDENCY PREAMBLE, and AMEND
fill rules are identical to V-01. Only BEFORE YOU START differs.
...
[Identical to V-01 FAILS SYNTAX TEMPLATE -- C-42/C-43 CORRECT entries present; C-44/C-45
 CORRECT entries absent. C-46 not satisfied.]
...
## V-03 -- BEFORE YOU START C-45 Pre-Flight Halt (C-48, single axis)

**Axis:** Add C-45 pre-flight halt to BEFORE YOU START
**Hypothesis:** An imperative pre-flight line in BEFORE YOU START that halts execution if the
CO-DEPENDENCY PREAMBLE omits the C-43/C-41 three-leg row satisfies C-48 (the C-45 detection
layer at session-open). This is parallel to C-47 on the C-45 axis: a skill can have the
three-leg row in the preamble (C-45 satisfied) without BEFORE YOU START checking for its
presence (C-48 not satisfied). The halt makes preamble incompleteness visible before any phase
executes. C-46 is NOT satisfied: the FAILS template still carries only C-42/C-43 entries.
C-47 is NOT satisfied: no FAILS template currency pre-flight in BEFORE YOU START.
...
**Axis:** FAILS template extended with C-44/C-45 entries (C-46) and BEFORE YOU START gains
C-44 pre-flight halt (C-47)
**Hypothesis:** Pairing FAILS template recursive currency extension (C-46) with the BEFORE YOU
START C-44 pre-flight halt (C-47) closes two of the three R17 gaps simultaneously. The two
changes operate at independent sites -- FAILS SYNTAX TEMPLATE vs BEFORE YOU START -- with no
interaction effects. This is the minimum change from the R16 V-04 baseline that satisfies
both C-46 and C-47 without adding the C-48 preamble detection layer. C-48 is NOT satisfied:
BEFORE YOU START still carries no CO-DEPENDENCY PREAMBLE completeness pre-flight.
...
**Axis:** All R17 structural additions on the R16 V-04 baseline
**Hypothesis:** All three R17 additions applied simultaneously: FAILS template extended with
C-44/C-45 self-referential entries (C-46); BEFORE YOU START gains C-44 pre-flight halt (C-47);
BEFORE YOU START gains C-45 pre-flight halt (C-48). This is the only variation where:
- A C-44 or C-45 violation produces a FAILS template citation (C-46 CORRECT entries present)
- A stale FAILS template halts at session-open before the skeleton is printed (C-47)
- A missing preamble three-leg row halts at session-open independently of C-47 (C-48)
- In V-05, the C-47 pre-flight text also lists C-44/C-45 as aspirational pairs requiring entries,
  so a template omitting those entries triggers a C-46 miss -- not merely a C-44 miss

The test for V-05: remove any one mechanism and a gap reopens. All three are load-bearing and
satisfy independent criteria; none substitutes for either of the others.
...
```
=== org:committee SIMULATION -- SKELETON (V-05: Full R17 Integration) ===
...
## Structural Axis Map (R17)

| Axis                                                  | V-01        | V-02        | V-03        | V-04        | V-05        |
|-------------------------------------------------------|-------------|-------------|-------------|-------------|-------------|
| FAILS template: C-44/C-45 self-referential entries    | **Present** | Absent      | Absent      | **Present** | **Present** |
| BEFORE YOU START: C-44 pre-flight halt [C-47]         | Absent      | **Present** | Absent      | **Present** | **Present** |
| BEFORE YOU START: C-45 pre-flight halt [C-48]         | Absent      | Absent      | **Present** | Absent      | **Present** |
| C-47 pre-flight names C-44/C-45 as aspirational pairs | Absent      | Absent      | Absent      | Absent      | **Present** |
| C-46 satisfied                                        | **Yes**     | No          | No          | **Yes**     | **Yes**     |
| C-47 satisfied                                        | No          | **Yes**     | No          | **Yes**     | **Yes**     |
| C-48 satisfied                                        | No          | No          | **Yes**     | No          | **Yes**     |

**Key:** V-01 is single-axis for C-46 (template extension). V-02 is single-axis for C-47
(BEFORE YOU START C-44 detection). V-03 is single-axis for C-48 (BEFORE YOU START C-45
detection). V-04 combines C-46 + C-47 at minimum change. V-05 applies full R17 integration:
all three criteria satisfied with independent detection mechanisms that cannot substitute
for each other. In V-05 only, the C-47 pre-flight text is updated to name C-44/C-45 in the
aspirational pair list and cites C-46 miss rather than C-44 miss, reflecting that the template
must now carry entries not only for C-42/C-43 but also for C-44/C-45 themselves.
