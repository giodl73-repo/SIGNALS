...
**Date:** 2026-03-15
**Rubric:** v17 (A-30 + A-31 introduced)
**Target criteria:** A-30 (derivation-phase protocol), A-31 (CI failure mode at point of constraint)

---

## Round 19 Variation Map

| Variation | Axis | A-29 | A-30 | A-31 | Notes |
|-----------|------|------|------|------|-------|
| V-01 | Output Format | PASS (format) | FAIL | FAIL | Two-part labeled CI block; inspection-time only |
| V-02 | Role Sequence | PARTIAL | FAIL | FAIL | Grouped personas + A-25 rationale co-located in ordering |
| V-03 | Lifecycle Emphasis | PASS (derivation) | PASS | FAIL | Explicit D.1–D.4 phase; no failure mode co-location |
...
1. **Current approach:** What does this persona do today without the spec's feature? Name their existing workflow or status-quo tool. If no equivalent workflow exists, state that explicitly.
...
4. **Feedback** (severity-first within list: blocking → major → minor → cosmetic):
...
- Theme: short label for the recurring issue or positive signal
- Peak Severity: highest severity level raised under this theme
- Severity Pattern: e.g., "blocking in C-05, major in C-01 and C-08"
...
Synthesize usability and accessibility signals from the 12 persona cards. 3 sentences covering: dominant usability risk, accessibility finding, and what UX is working well.

UX Read precedes PM Read in this section.

**PM Read (synthesize second):**
Synthesize adoption, ROI, and purchase-intent signals from the 12 persona cards. 3 sentences covering: primary adoption barrier, purchase intent drivers, and go-to-market objection.
...
Dominant Detractor objection: [single most common blocking or major objection raised by Detractor-tier personas, with count of Detractors citing it]
Threshold: 7.0
...
### Step 6 — Revision Directive (conditional)
...
For each persona scoring below 7.0, state the single highest-priority spec change. Reference a named spec element.

*Pass 2 — Cross-persona prioritized summary:*
Top 3 revisions ranked by (personas affected × maximum severity). For each: issue, personas affected, implementation cost tier (low-cost = copy/label/wording changes; high-cost = architecture or workflow changes), and minimal spec change.
...
**Axis:** Role sequence
**Hypothesis:** Grouping personas by functional affinity (Group A: Technical, B: End User, C: Business, D: Operational) and stating the UX-before-PM ordering rationale within the ordering instruction itself (not separately) satisfies A-25 while producing a cleaner signal chain from domain concern to synthesis finding. CI enforcement is lighter (inline formula in the aggregate section, no derivation phase).
...
Use this numbered field manifest for every card. Omitting any numbered field constitutes an incomplete card.
...
Identify themes that appear in 2+ personas. At minimum one theme required.

| Theme | Personas | Peak Severity | Severity Pattern | Action |
|-------|----------|---------------|-----------------|--------|

Order themes descending by peak severity.
...
**Hypothesis:** An explicit four-step derivation phase (D.1 state formula → D.2 collect scores → D.3 compute SD → D.4 apply and report) enforces the CI formula requirement at generation time. The formula is a required input to computation, not a label added to output after the fact. Satisfies A-30; does not yet co-locate the failure mode at the point of constraint (A-31 still open).
...
Minimum one theme. Order descending by peak severity. Every row must include a severity annotation.
...
**UX Read:**
Synthesize usability and accessibility signals. 3 sentences: dominant usability risk, accessibility finding, what is working well.
...
**Convergence:**
One sentence: where do UX and PM reads agree? Where do they diverge?
...
The formula is an input to the computation in D.3–D.4. It is not a label appended to the output after the fact. D.2 cannot be entered until D.1 is stated.
...
Threshold: 7.0
Verdict: **PASS** — spec ready for development | **FAIL** — spec needs revision before proceeding
...
*Pass 1 — Per-persona targeted changes:*
For each persona scoring below 7.0, state the single highest-priority spec change referencing a named spec element.

*Pass 2 — Cross-persona prioritized summary:*
Top 3 revisions by (personas affected × severity). For each: issue, personas, cost tier (low-cost / high-cost), minimal spec change.
...
**Hypothesis:** Dual enforcement — derivation-phase protocol (generation-time) and labeled output blocks (inspection-time) — creates two independent verification points for CI formula compliance. An evaluator cannot satisfy the output format without the derivation phase already complete; an auditor cannot confirm compliance without checking both. Satisfies A-30 and A-29 at both levels; A-31 still open (no failure mode co-located at point of constraint).
...
**Convergence:**
One sentence identifying where UX and PM reads agree and where they diverge.
...
Margin: 1.96 × [SD] / √12 = 1.96 × [SD] / 3.4641 = [value to 4 dp]
...
*Pass 1 — Per-persona targeted changes:*
For each persona scoring below 7.0, state the highest-priority spec change referencing a named spec element.

*Pass 2 — Cross-persona prioritized summary:*
Top 3 revisions ranked by (personas affected × maximum severity). For each: issue, personas, cost tier (low-cost / high-cost), minimal spec change.
...
**Hypothesis:** Naming the CI formula failure mode within derivation step D.1 itself — co-located with the formula constraint instruction, not in a separate rationale section — ensures a reader following the instruction encounters both the rule and its epistemic grounding in a single pass. This directly satisfies A-31. The pattern parallels A-25: just as the UX-before-PM rationale must be stated within the ordering instruction, the CI failure mode must be stated within the formula constraint. Full compliance: A-30 (derivation phase), A-31 (failure mode at point of constraint), A-27 (structural constraint annotated with failure mode).

---

You are simulating customer reactions to a spec before it ships. Twelve customer personas will evaluate the spec through their functional lens. This protocol uses a derivation-phase approach to aggregate NPS computation, with structural constraints accompanied by co-located failure mode annotations. All sections are required.
...
**PM Read (runs second):**
Synthesize adoption, ROI, and purchase-intent signals from the 12 persona cards. 3 sentences: primary adoption barrier, purchase intent driver, go-to-market objection.
