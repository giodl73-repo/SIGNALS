...
date: 2026-03-17
rounds: 20
rubric_final: corps-pr-rubric-v18-2026-03-17.md
score: 100
status: GOLDEN
---

# Golden Prompt — corps-pr

## Winning Variation: V-05 (Full Combination)

**Axes**: Role Sequence (security/compliance first) + Output Format (Phase D ranked table) + Phrasing Register (formal declarative passive throughout)
...
Phase A -- Route
  Entry:    org.yaml + PR diff + .roles/ are loaded
...
Five patterns distinguish V-05 from the baseline and from every prior round:

**1. Three orthogonal axes confirmed independent**
Role sequence (Phase C), output format (Phase D), and phrasing register (surface prose) are
non-interfering structural regions. V-05 combines all three and scores 100 -- proving that
saturating one axis does not degrade compliance on any other. Future variations can stack
axes freely without risk of cross-contamination.

**2. Formal declarative passive makes compliance surface-readable without instruction verbs**
Replacing imperative "produce the routing table" with declarative "the routing table is produced
as follows" removes ambiguity about who acts and when. The output template drives execution;
the phrasing declares what the pipeline produces, not what the model must do. This register
survived all four annotation invariants unchanged -- labels, header strings, and schema slots
are phrasing-invariant.

**3. Phase D ranked table co-locates ban-to-fix reference with the Mechanism column declaration**
The table format (V-02/V-04/V-05) embeds the ban-to-fix table directly adjacent to the
Mechanism column declaration. Compliance is O(1) per cell: scan the column header, read the
ban list, verify no match. This is the pattern that generated C-55 in rubric v18.

**4. Phase D named fields extend the pipeline-wide named-block convention to synthesis**
`IA ledger:` and `IA position:` named fields in Phase D mirror the READ RECEIPT, C2 RESULT,
and PRE-COMMITMENT named blocks in Phase C. The Phase D summary is now independently auditable
by field label without reconstructing Phase B or Phase C content. This generated C-56 in v18.

**5. Four annotation invariants carried through all axes unchanged**
C-51 (`Status-quo champion [C-11]:`), C-52 (`[C-26]` on C1/C2 headers), C-53 (`<- B3 line N [C-25]`
on Budget verdict lines), and C-54 (field (e) dual-form disjunct) are embedded in schema slots
that are structurally immune to sequence reordering, format substitution, and register change.
All four survived V-05 without modification.
...
| Aspirational | C-09 -- C-56 (48 total) | ~0.208 | 10 |
| **Max** | | | **100** |
...
|---|-----------|-------------|
| C-01 | Phase A routing complete | Every file has a row; exact scope pattern cited; `COVERAGE GAP:` format string declared |
| C-02 | Phase C reviews present | Per-role output sequence (security/compliance first) declared |
| C-03 | Phase D synthesis | Consensus covers Agreement/Divergence/Critical across all roles |
| C-04 | Phase E decision | Exactly one GO / NO-GO / GO WITH CONDITIONS; "Delegated decisions fail. Hedged decisions fail." |
| C-05 | Minimum 2 findings per role | F-01 IA-RESPONSE + at least 1 DOMAIN; `[minimum 2]` enforced |

### Recommended (strong signal)

| # | Criterion | Requirement |
|---|-----------|-------------|
| C-06 | Coverage gaps | `COVERAGE GAP: [file] -- no org.yaml scope pattern covers this path.` |
| C-07 | Domain-lens gate | Two-step test per finding; Step A (YES -> include) / Step B (revise or drop) |
| C-08 | Post-commitment delta | `Pre-committed:` / `After findings:` / `HELD or CHANGED` fields per role |

### Key Aspirational (new in v18)

| # | Criterion | Requirement |
|---|-----------|-------------|
| C-51 | IA adversarial designation | `Status-quo champion [C-11]:` label on Phase B section header |
| C-52 | Sub-condition criterion annotations | `[C-26]` on both C1 and C2 sub-condition headers |
| C-53 | Budget verdict governing criterion | `<- B3 line N [C-25]` on all three Budget verdict clause lines |
| C-54 | Field (e) dual-form disjunct | Names both valid evidence forms: `inline value OR cross-reference "see PRE-COMMITMENT block: [row]"` |
| C-55 | Phase D ban-to-fix co-located | Ban-to-fix reference table adjacent to Mechanism column declaration (table format only) |
| C-56 | Phase D IA ledger named fields | `IA ledger:` and `IA position:` as named field labels in Phase D preamble |
