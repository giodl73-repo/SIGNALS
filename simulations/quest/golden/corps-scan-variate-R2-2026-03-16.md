...
> write `.craft/roles/` files, does not instruct the user to create role files, and does not
...
State that this output is a draft org.yaml for human review and that no `.craft/roles/` files
...
Do not omit any of the four items. Do not write `.craft/roles/` files. Do not include role
file content.
...
> `.craft/roles/` files. It does not instruct the user to create role files. It does not include
...
> All team names match inventory. No .craft/roles/ content produced."
