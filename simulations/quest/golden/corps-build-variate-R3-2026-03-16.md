...
`.roles/`. What follows describes what good output looks like at each stage.
...
### What .roles/inertia-advocate.md files contain
...
**6a. Standard role files**: `.roles/{area-slug}/{role-slug}.md`, `scope: standard -- present in all teams`
**6b. Specialized role files**: `.roles/{area-slug}/{role-slug}.md`, `scope: specialized -- [team name]`
**6c. Shared group roles** (if declared): `.roles/{group-slug}/{role-slug}.md`, `scope: shared -- group: [name]`
**6d. Exec office roles** (if declared): `.roles/exec-office/{role-slug}.md`, `scope: exec office`
...
**Entry**: VALIDATE-PASS.
**This phase produces the first artifact -- the headcount table -- before any .roles/ file.**
...
File generation must produce exactly [n] .roles/ files.`
