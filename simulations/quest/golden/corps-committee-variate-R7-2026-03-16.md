...
# corps-committee -- Prompt Variations R7

## Variation Summary

| ID   | Axis                                                              | Hypothesis |
|------|-------------------------------------------------------------------|------------|
| V-01 | Action-Attribution Linkage (lifecycle emphasis -- Origin field on each ACTION ITEM linking to Phase 3 voice) | The ACTION ITEMS section currently requires three fields: Owner Role, specific action, deadline. No field links each action to the Phase 3 voice that raised the concern prompting it. A missing-origin failure is structurally silent -- a model can produce plausible action items with no traceability to participant stances. Adding a mandatory `Origin:` field (naming the Phase 3 participant whose concern prompted the action) converts Phase 3->5 attribution from prose inference to a structural requirement. The new field is validated against the Phase 3 STANCE MANIFEST: an Origin naming a participant not in the STANCE MANIFEST is a distinct structural defect from a missing field. |
| V-02 | Inertia Resolution Summary (inertia framing -- Phase 5 block mapping all four INERTIA-FINDING-* labels to resolution status) | DISSENT INERTIA LINKAGE (C-23) closes the dissent traceability gap: at least one dissenting opinion must cite an INERTIA-FINDING-* label. But it only requires ONE finding to be cited -- the other three can remain unaddressed. A `## INERTIA RESOLUTION SUMMARY` block in Phase 5, placed after DISSENTING OPINIONS and before PHASE-5-COMMIT, requires each of the four INERTIA-FINDING-* labels to declare one of: `Addressed by: [action item]` or `No action needed: [justification]`. This converts finding coverage from a partial requirement (one cited in dissent) to a complete Phase 1->5 accountability loop. |
| V-03 | Committee-Type-Aware Outcome Vocabulary (output format -- OUTCOME tokens keyed to committee type declared at Phase 0) | The TALLY OUTCOME vocabulary is currently fixed: APPROVED / APPROVED WITH CONDITIONS / BLOCKED / DEFERRED. This vocabulary is semantically correct for ROB meetings but wrong for shiproom (GO / NO-GO / HOLD) and arch-board (ACCEPTED / ACCEPTED WITH REVISIONS / REJECTED / DEFERRED). The mismatch is currently silent -- a shiproom simulation that outputs APPROVED passes all structural checks despite using wrong vocabulary. Declaring OUTCOME-VOCABULARY in Phase 0 as a COMMITTEE TYPE GATE checkbox makes vocabulary a Phase 0 structural commitment. Phase 4 and Phase 5 consume the declared vocabulary, and the MINUTES INTEGRITY CHECK box 1 validates against the Phase 0 declaration rather than a fixed list. |
| V-04 | Action-Attribution + Inertia Resolution (V-01 + V-03 axes combined) | V-01 targets Phase 3->5 action traceability; V-02 targets Phase 1->5 inertia finding accountability. No structural interaction -- action origin validation and finding resolution summary are independent checks on independent outputs. Co-applying both closes both gaps simultaneously: a well-executed simulation must attribute every action to a voice AND account for every inertia finding. |
| V-05 | Full R7 Integration (V-01 + V-02 + V-03 on R6 V-05 foundation) | All three R7 axes applied to the complete R6 V-05 foundation (Voice Completeness Check + Phase-tagged Constraints Table + Dissent Inertia Linkage). V-01 extends Phase 5 action traceability. V-02 extends Phase 5 inertia finding coverage. V-03 makes outcome vocabulary a Phase 0 structural decision. Each axis targets a distinct structural gap; none interacts mechanically with the others. |

---

## V-01 -- Action-Attribution Linkage

**Axis:** Lifecycle emphasis -- mandatory `Origin:` field on each ACTION ITEM linking to Phase 3 voice
**Hypothesis:** The ACTION ITEMS section requires Owner Role / specific action / deadline but no field traces each action to the Phase 3 participant whose concern prompted it. Adding a validated `Origin:` field makes Phase 3->5 attribution a structural requirement rather than a silent omission. Origin is validated against the STANCE MANIFEST: a name not in the STANCE MANIFEST is a distinct defect class.
...
PRINT: action items -- four fields required per item:
  [Owner Role] -- [specific action] -- [deadline] -- Origin: [Phase 3 participant]
  VALIDATE Origin: the named participant must appear in the Phase 3 STANCE MANIFEST.
    If Origin names a participant not in STANCE MANIFEST: halt, identify error, correct.
    Generic placeholders ("a participant", "the team") fail the same as a missing field.
...
## V-02 -- Inertia Resolution Summary

**Axis:** Inertia framing -- `## INERTIA RESOLUTION SUMMARY` block in Phase 5 mapping all four INERTIA-FINDING-* labels to resolution status
**Hypothesis:** C-23 DISSENT INERTIA LINKAGE requires at least one dissenting opinion to cite an INERTIA-FINDING-* label -- but only one. The other three findings can remain unaccounted for in the minutes. A `## INERTIA RESOLUTION SUMMARY` block placed after DISSENTING OPINIONS, before PHASE-5-COMMIT, requires each of the four INERTIA-FINDING-* labels to declare: `Addressed by: [action item]` OR `No action needed: [justification]`. An incomplete summary halts Phase 5 via a 7th MINUTES INTEGRITY CHECK checkbox. This converts Phase 1 finding coverage from a partial requirement to a complete accountability loop.
...
| STANCE MANIFEST              | Phase 3->4      | Entry missing or format not [Role Name] STANCE-TOKEN                     |
| TALLY LEDGER PROTOCOL        | Phase 4         | Count mismatch or TALLY RESULT undeclared                                |
| MINUTES INTEGRITY CHECK      | Phase 4->5      | Unticked checkbox (incl. DISSENT INERTIA LINKAGE and INERTIA RESOLUTION  |
|                              |                 | SUMMARY) before Phase 5                                                  |
| DISSENT INERTIA LINKAGE      | Phase 5         | No INERTIA-FINDING-* token cited in dissent; no linkage justification    |
...
### INERTIA RESOLUTION SUMMARY
  ...
  RESOLUTION RESULT: ___ [COMPLETE -- all four findings accounted for /
                           INCOMPLETE -- halt, identify missing entry]
...
Before writing any Phase 5 content, complete the MINUTES INTEGRITY CHECK block:
  ...
  Tick box 6 -- DISSENT INERTIA LINKAGE:
    ...
    IF no dissent: write `No dissent -- [reason] -- No inertia linkage required --
      [justification]`.
  Tick box 7 -- INERTIA RESOLUTION SUMMARY:
    Confirm ## INERTIA RESOLUTION SUMMARY block is present in the skeleton.
    Confirm entries for all four INERTIA-FINDING-* labels are planned before writing
      Phase 5 content. Any label with no planned entry halts Phase 5.
  All seven checkboxes must be ticked. Any unticked box halts Phase 5.
...
COMPLETE ## INERTIA RESOLUTION SUMMARY before writing PHASE-5-COMMIT:
  For each label INERTIA-FINDING-A through INERTIA-FINDING-D:
    INSPECT ACTION ITEMS: does any action item address this finding's content?
      If YES: write `Addressed by: [Owner Role] -- [action description]`.
      If NO: write `No action needed: [justification for why no action is required]`.
  DECLARE: `RESOLUTION RESULT: COMPLETE` when all four entries filled.
    If any entry blank: `RESOLUTION RESULT: INCOMPLETE -- halt`; identify missing entry; fill it.
  PHASE-5-COMMIT may not print until RESOLUTION RESULT is COMPLETE.

PRINT: PHASE-5-COMMIT -- include "DISSENT INERTIA LINKAGE ticked. INERTIA RESOLUTION SUMMARY
  complete, RESOLUTION RESULT declared."
```
