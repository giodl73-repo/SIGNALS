...
Paths:
- `.roles/{area-slug}/{role-slug}.md` — standard and specialized
- `.roles/{group-slug}/{role-slug}.md` — shared group roles
...
VERIFY:
  ...
  [C3] every area in headcount table has a .roles/{area}/ directory
...
Before writing org-chart.md or any .roles/ file, emit the headcount table.
...
The .roles/ write phase must produce exactly [n] files.`
...
3. **Change group structure**: `--restructure "[team] > [new-parent]"` — moves a team to
   a new parent group, renames the .roles/ path, and redraws the ASCII diagram.
...
Write .roles/ files using the headcount table as the work list — process rows top to
bottom. For each area row:

**Order within each area**: Inertia Advocate first, then standard roles, then specialized.

Paths:
- `.roles/{area-slug}/inertia-advocate.md` — IA
- `.roles/{area-slug}/{role-slug}.md` — standard and specialized
...
`.roles/`. What follows describes what good output looks like for each.
...
3. `--restructure "[team] > [new-parent]"` — move a team to a new parent, rename its
   `.roles/` subdirectory, redraw the diagram.
   The current parent-child relationships are listed.

---

### What .roles/ files contain
...
Role file generation must produce exactly [n] .roles/ files.`
...
the Total row is the authoritative .roles/ file count for this run.
...
Write all .roles/ files in this sequence:
...
Path: `.roles/{group-slug}/{role-slug}.md`, `scope: shared — group: [name]`

**4e — Exec office roles** (if declared)

Path: `.roles/exec-office/{role-slug}.md`, `scope: exec office`
...
```
CROSS-REF:
  C1 — TABLE-CLOSED contract vs .roles/ file count
