...
- Role name is in `.roles/` -- not invented ✓
...
| Column | Type -- valid | Anti-pattern excluded (NOT valid) |
|--------|--------------|-----------------------------------|
| Role | string -- filename stem from `.roles/` at execution time | NOT: invented names; names from memory or prior runs; any name not in the registry |
