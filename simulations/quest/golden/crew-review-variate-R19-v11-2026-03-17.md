...
{{reviewer_set}}), every `lens.verify` entry from the role's .craft/roles/
...
- `deep`: all roles from .craft/roles/ invoked.
...
Step 9 (LENS COVERAGE TABLE): For each active reviewer role from .craft/roles/,
...
**LENS COVERAGE TABLE (step 9)**: For each active reviewer role from .craft/roles/,
