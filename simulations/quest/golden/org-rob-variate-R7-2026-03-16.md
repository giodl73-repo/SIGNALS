...
3. Read `.roles/` — extract `orientation.frame` and lens items for each assigned role.
...
2. Read `org.yaml`. Read `.roles/` for each stage's role. Fallback: teams=Team Leads, pm=Senior PM, arch-board=Principal Architect, leadership=VP Product, tpm=TPM Lead, spire=Chief of Staff.
...
loaded `.roles/` file is INCOMPLETE. Without the orientation frame explicitly
...
2. Read `org.yaml`. Read `.roles/` for each stage. Extract `orientation.frame` and
...
**Step 3 — Load roles:** Read `org.yaml`. Read `.roles/`. Extract `orientation.frame`.
