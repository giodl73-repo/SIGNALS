...
|--------|--------------|-----------------------------------|
| Role | string -- filename stem from `.roles/` at execution time | NOT: invented role names; names from memory or prior runs; names not present in the registry at this execution |
...
Open `.roles/`. Read every file. Role pool = all files found.
...
- Role cell: filename stem from `.roles/` -- not invented ✓
...
| Column | Type -- valid | Anti-pattern excluded (NOT valid) |
|--------|--------------|-----------------------------------|
| Role | string -- filename stem from `.roles/` at execution time | NOT: invented names; names from memory or prior runs; any name not in the registry at this execution |
