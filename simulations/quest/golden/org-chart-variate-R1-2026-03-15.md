...
| V-03 | role-sequence | .craft/roles/ read step is the first action, roles are named in a ROLES-LOADED list, and subsequent sections must reference at least one loaded role by name | Front-loading the roles step before any writing ensures C-03 is satisfied from context, not as a retrofit; named roles anchor the org design rather than appearing as optional flavor |
...
Check whether `.craft/roles/` contains role files for this domain. If role files exist:
...
If `.craft/roles/` is absent or empty:
- Write a ROLES-NOTE line: "No .craft/roles/ files found — using inferred roles."
...
for each, and any known named roles from `.craft/roles/`:

| Area | Headcount | Key Roles |
|------|-----------|-----------|
| ...  | N or N-M  | role names from .craft/roles/ or inferred |
...
ROLES-LOADED: [comma-separated list of role names from .craft/roles/, or
"none — using inferred roles" if .craft/roles/ is absent]
...
- Use role names from `.craft/roles/` if available; inferred roles otherwise
...
- DRI column should reference a role name from `.craft/roles/` where possible
...
1. Check whether `.craft/roles/` contains role files for {topic} or the domain
2. If found: list roles in the ROLES-LOADED field in Section 2
3. If not found: write "none — using inferred roles" and proceed

Roles found in `.craft/roles/` must appear in at least one of: the org diagram,
...
If `.craft/roles/` is empty or absent, produce instead:

```
ROLES-NOTE: No .craft/roles/ files found for {topic}. Using inferred roles.
...
DO NOT invent role names not found in `.craft/roles/` unless no roles are present.
If no roles are present, DO write: "ROLES-NOTE: No .craft/roles/ files found — using inferred roles."
...
Read `.craft/roles/` for role files relevant to {topic} or the domain. Produce:

```
## Roles Input

Source: .craft/roles/{domain}/ (or "absent — using inferred roles")
...
If `.craft/roles/` is absent: note it explicitly and list inferred roles in the same
