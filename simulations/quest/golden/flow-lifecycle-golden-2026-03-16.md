...
date: 2026-03-16
rounds: 20
rubric_final: flow-lifecycle-rubric-v21-2026-03-16.md
score: 100
status: GOLDEN
---

# flow-lifecycle — Golden Prompt (Round 20, rubric v20, score 58/58)

**Winning variation:** V-05 — Full Combination Floor (R19 V-05 + D-21/D-22 + AC-31/AC-32 formalization)
...
**1. Gate sentence carries parenthetical D-IDs on every inline category (C-57)**

The production gate enumerates each defect category with an immediately-adjacent
parenthetical D-ID -- "an unterminated path (D-01), an uncertified exception handler
(D-02), an unresolvable decision owner (D-03)" -- making the gate-to-taxonomy cross-
reference row-scannable without a category-name lookup. This was already present in
R19 V-05 (HOLD/PASS) and carries forward unchanged. The baseline V-01 in R20 also
holds this property; V-04 was the TARGET variation that required explicit remediation.

**2. Escape code column headers use code-token + parenthetical binding (C-58)**

Both the LT-ID Trace column (Step 1) and Entry Trigger column (Step 2) bind the typed
escape code and its eligibility clause as a single typographic unit --
"SECONDARY (use only when...)" rather than "write SECONDARY -- but only when..." --
eliminating the intervening instruction verb. This was already present in R19 V-05
(HOLD/PASS). V-04 was the only R20 variation requiring remediation; V-05 confirms the
full-floor form is structurally stable across three combined axes.

**3. Inertia-first SQ Declaration Column Contract (C-52 + C-53 + C-54 combined)**

V-05 uniquely combines: (a) inertia-first field ordering (Inertia Driver before Incumbent
Process), (b) Column Contract format with three-column definition layer, (c) GATE SQ as a
labeled gate node with "Blocked by" field. No other variation carries all three. This
combination means the organizational context (why the incumbent persists) is established
before the technical failure modes are named -- a structural sequencing choice that
anchors FM-IDs in real-world friction rather than abstract process description.

**4. D-21/D-22 taxonomy entries + AC-31/AC-32 scan rows close the C-57/C-58 verification gap**

R19 V-05 already satisfied C-57 and C-58 in artifact form but had no named defect types
or coverage scan rows for them. V-05 R20 formalizes both: D-21 names the D-ID-absent gate
defect; D-22 names the non-parenthetical-binding defect; AC-31 and AC-32 make verification
mandatory and non-overlapping. The criterion-taxonomy co-registration pattern (C-59) is
directly instantiated here -- each new criterion immediately produces a named D-ID entry.

**5. Three-axis structural combination confirms criteria are axis-independent**

V-05 combines inertia-first framing (R14 axis), Column Contract output format (R10 axis),
and pre-declaration role registry (R13 axis). That all 58 criteria -- including C-57 and
C-58 -- hold at PASS across this maximal structural combination is the key stability signal:
none of the criteria are format-sensitive or axis-entangled. An LLM that produces a
flow-lifecycle artifact from a signal input can satisfy all criteria regardless of which
structural register it uses.

---

## Final Rubric Criteria Summary (v21)

**Denominator: 60 | Aspirational: 52 | All essential must PASS**

| Group | Criteria | Count |
|-------|----------|-------|
| Essential | C-01 -- C-05 | 5 |
| Recommended | C-06 -- C-08 | 3 |
| Aspirational | C-09 -- C-60 | 52 |

**R20 probe criteria (v20 additions):**

| C-ID | Name | Type | Status in V-05 |
|------|------|------|----------------|
| C-57 | Gate Defect D-ID Cross-Reference | Aspirational | HOLD/PASS |
| C-58 | Parenthetical Eligibility Binding Format | Aspirational | HOLD/PASS |

**v21 extracted criteria (from R20 signals, denominator 58 → 60):**

| C-ID | Name | Type | Rule |
|------|------|------|------|
| C-59 | Criterion-Taxonomy Co-Registration | Aspirational (clarity) | Each new clarity criterion must produce at least one D-ID entry naming its primary violation mode -- taxonomy classification power must scale with criterion count |
| C-60 | Group B Coverage Scan Criterion Proportionality | Aspirational (structure) | Group B must carry exactly one AC item per gate-backed aspirational criterion -- criterion-to-AC cardinality is the structural enforcement mechanism for scan completeness |

**Key defect types added in R20:**

| D-ID | Defect Type | Criterion |
|------|-------------|-----------|
| D-21 | Gate-defect-D-ID-absent | C-57 / AC-31 |
| D-22 | Escape-code-non-parenthetical-binding | C-58 / AC-32 |
