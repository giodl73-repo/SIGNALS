...
in `for:`. Produce one markdown file per role at `.craft/roles/{domain}/`. Inertia-advocate
...
Write role files to `.craft/roles/{domain}/`. Follow this sequence:
  ...
  Emit: INERTIA-ADVOCATE WRITTEN: .craft/roles/{domain}/{filename}
...
moving to the next.
  ...
  CHECK 7 — OUTPUT PATH
  Confirm all files are at `.craft/roles/{domain}/` and no file is at a different path.
...
markdown file per role at `.craft/roles/{domain}/`. You always include a role whose job
...
"Inertia advocate written: .craft/roles/{domain}/{filename}"
...
next one.
  ...
  7. Output path correct: confirm all files are at `.craft/roles/{domain}/`. Write
...
per role at `.craft/roles/{domain}/`. Every role gets: name, orientation (frame +
...
CHECK 7 -- All files at `.craft/roles/{domain}/`.
...
you identify, write a markdown file at `.craft/roles/{domain}/` with these fields: name,
...
.craft/roles/{domain}/{filename}". This confirms you haven't skipped ahead.
...
Work through these in order. Write each result before running the next.
  ...
  7. Output path: are all files at `.craft/roles/{domain}/`?
