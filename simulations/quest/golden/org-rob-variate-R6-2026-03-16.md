...
`.roles/`. If a role is constructed inline, the `orientation.frame` value is
...
If a required role has no file in `.roles/`, construct a minimal role inline. State
...
Role: VP/executive archetype from `.roles/`; construct if absent as orientation
...
Role: team-level archetype from `.roles/`; construct if absent with orientation
...
Role: load `.roles/signal/pm.md`. Extract Frame: value. Work through `lens.verify`
...
Role: load `.roles/signal/architect.md`. Extract Frame: value.
...
Role: load `.roles/signal/pm.md`. Extract Frame: value.
...
Role: load `.roles/signal/pm.md`. Frame: MUST cite `orientation.frame` value.
...
Role: load `.roles/signal/architect.md`. Frame: MUST cite architecture orientation.
...
**ROLE:** {role-name}
**Frame:** orientation.frame = "{extracted value from .roles/}"
...
**ROLE:** {role-name}
**Frame:** orientation.frame = "{extracted value -- must match .roles/ field}"
**Loaded from:** {source path or "(constructed) -- orientation.frame stated as: {value}"}
...
3. The value in quotes must be traceable to `.roles/`; constructed roles must state
...
Role: VP/executive archetype from `.roles/`; construct if absent with explicit
...
Role: load `.roles/signal/pm.md`. Extract Frame: value from role file.
