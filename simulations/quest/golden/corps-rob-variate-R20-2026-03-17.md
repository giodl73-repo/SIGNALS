...
not loaded from the role's `lens.primary` in `.roles/`. AP-1 enforces structural position;
...
RULE VIA-POSITION
  ...
  Anti-pattern-2: Via: in second column position but cell content blank, containing a
    placeholder (TBD / N/A / "--" / empty), or containing a lens name not loaded from the
    role's lens.primary in .roles/. The position requirement (AP-1) and the content
