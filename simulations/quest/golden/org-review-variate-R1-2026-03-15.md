...
Read `.roles/signal/` to identify available reviewers. You will see role files including `pm.md`, `architect.md`, and `inertia-advocate.md`. Do not invent roles — all reviewers must come from this directory.
...
- **`--depth deep`**: Include every role found in `.roles/signal/`. State the total count.
...
1. Read the role's `lens.verify`, `lens.simplify`, and `expertise` fields from its `.roles/signal/` file.
...
- **Standard** (default): Select reviewers from `.roles/signal/` whose expertise is most relevant to this artifact type. Name the selected roles and your selection rationale in one sentence each.
- **`--depth deep`**: Include every role found in `.roles/signal/`. State the total count before the table.

All roles must come from `.roles/signal/`. Do not invent roles.
...
- **Role**: the role name exactly as it appears in the `.roles/signal/` filename (e.g., `pm`, `architect`, `inertia-advocate`).
...
Read `.roles/signal/` to identify available reviewers.
...
- **`--depth deep`**: Use all roles in `.roles/signal/`. State the total count.

All roles must come from `.roles/signal/`.
...
You are executing the `org-review` skill. This skill distributes an artifact to a set of organizational reviewers drawn from the role registry at `.roles/signal/`. Each reviewer applies their designated lens — verify, simplify, expertise — and returns a structured finding. The output is a review matrix suitable for decision-making.

**Phase 1: Reviewer Selection**

Input: `.roles/signal/` directory
Depth parameter: `{{depth}}` (values: `standard` | `deep`)

Procedure:
1. Enumerate all role files in `.roles/signal/`.
...
Constraint: No role may be invented. Every reviewer must have a corresponding file in `.roles/signal/`. If a desired reviewer does not exist in the registry, that reviewer is omitted.
...
2a. Load the role file from `.roles/signal/{role}.md`.
...
- **`--depth deep`**: Include all roles in `.roles/signal/`. State the count. On a deep run, every desk gets the artifact.
