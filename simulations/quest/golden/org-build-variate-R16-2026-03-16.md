...
| MUST create min 2 area subdirs under .craft/roles/ | FORBIDDEN: beginning ARTIFACT-GEN before this record block is emitted |
...
| 7 | Write T2-ROLE-COUNT role files; inertia-advocate scope = verbatim SCOPE-TEMPLATE[T1-PRESSURE] | .craft/roles/ |
...
2. Write role files: `.craft/roles/{area}/{role}.md`.
...
Output: `org-chart.md` + `.craft/roles/{area}/{role}.md`.
Phase sequence: INPUT -> INERTIA-VERDICT -> ROLE-ENUM -> ARTIFACT-GEN.
...
2. Write role files: `.craft/roles/{area}/{role}.md`. Five fields each. inertia-advocate
...
1. Bind count floor to T0-DEPTH-FLAG:
   T0-DEPTH-FLAG = standard -> MUST enumerate 20-30 roles.
   T0-DEPTH-FLAG = deep -> MUST enumerate 50+ roles.
2. Map roles to area subdirectories. MUST create min 2 area subdirs under `.craft/roles/`.
...
2. Write `.craft/roles/{area}/{role}.md` for all T2-ROLE-COUNT roles.
