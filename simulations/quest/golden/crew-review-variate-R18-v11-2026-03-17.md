...
- `Reviewer Role`: name from .craft/roles/.
...
reviewer role, every `lens.verify` entry from the role's .craft/roles/ definition must
...
active reviewer role from .craft/roles/, every `lens.verify` entry in the role definition
