...
Read every file in `.roles/`. Build the role pool from only what you find there.
Do not add roles not present in that directory.

For each role, record: name, archetype, `orientation.frame`, `expertise.relevance`.

If `.roles/` is absent or empty: output `ERROR: .roles/ not found. Cannot proceed.`
Stop. You are not ready.

Gate 1 passed when: pool contains at least one role sourced exclusively from `.roles/`.
...
| Gate | Halt message |
|------|--------------|
| G1 registry | `HALT [G1]: .roles/ not found or empty. No role pool. Cannot proceed.` |
...
If `.roles/` is absent or empty: output `HALT [G1]` message exactly. Stop.
...
1. Does `.roles/` exist and contain at least one file?
   - No → Output: `ERROR: .roles/ not found.` Stop here.
   - Yes → Continue.

2. What roles are present? For each file, record: name, archetype, `orientation.frame`,
   `expertise.relevance`.

3. Are there any roles in the pool that were not sourced from `.roles/`?
...
| Gate | Halt message |
|------|--------------|
| G1 registry | `HALT [G1]: .roles/ not found or empty. Not ready to select reviewers.` |
...
**You are not ready to proceed until: role pool is built exclusively from `.roles/`.**

Read every file in `.roles/`. Record: name, archetype, `orientation.frame`,
...
| Gate | Halt message |
|------|--------------|
| G1 registry | `HALT [G1]: .roles/ absent or empty. Not ready to proceed.` |
...
If `.roles/` absent or empty: output `HALT [G1]`. Stop.
