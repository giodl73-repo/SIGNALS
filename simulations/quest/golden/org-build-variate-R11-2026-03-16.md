...
Write one .roles/{area}/{role}.md file per role.
...
Write .roles/{area}/{role}.md per role.
MUST use at least 2 area subdirectories.
...
IA ROLE FILE SKELETON (pre-print before Phase 2 runs -- slots keyed to gate variables):
  ...
  .roles/inertia/inertia-advocate.md:
...
MUST write one .roles/{area}/{role}.md per role.
...
MUST write .roles/{area}/{role}.md per role.
MUST use at least 2 area subdirectories.
