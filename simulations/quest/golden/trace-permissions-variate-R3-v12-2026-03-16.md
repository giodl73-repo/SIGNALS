...
**Session note:** R12-V-05 scored 25/28 under v12 — fails C-34, C-35, C-36 only. All three
hang off prerequisites already satisfied by V-05 (C-31+C-32+C-33+C-17+C-22 all PASS in R12-V-05).

**State entering Round 3:**

| Variation | v12 score | Notes |
|-----------|-----------|-------|
| R12-V-05 | 98.9 (25/28) | Passes C-31+C-32+C-33; fails C-34 (no axis declaration), C-35 (no SE-4 STRUCTURED CLOSE TABLE_4 SE-0 cross-ref + no CA-1.9), C-36 (no tri-dimensional terminal verdict) |
| R12-V-02 | 98.6 (24/28) | Passes C-31+C-33; missing C-32 blocks all three new criteria |
| R12-V-01 | 98.2 (23/28) | Passes C-32 only of R12 axes; missing C-31+C-33 blocks C-34/C-35/C-36 |

**Path to 28/28:**
- C-34: Add STRUCTURAL AXIS NON-INTERFERENCE DECLARATION to Step 0.2 preamble naming C-31/C-32/C-33 as orthogonal dimensions (execution order / closure-note format / CS self-reference) with explicit non-interference rule
- C-35: Add TABLE_4 row cross-reference at SE-0 position to SE-4's STRUCTURED CLOSE block; pre-assign CA-1.9 in Phase 0 mandate; add CA-1.9 verification row in Phase 3 (distinct from CA-1.4 SE-0 ordering and CA-1.7 MANUAL GAP labels)
- C-36: Add explicit tri-dimensional self-evidence attestation to CA terminal verdict naming each R12 dimension and its specific output-body evidence source
...
|-----------|------|------------|
| V-01 | Single-axis C-34: STRUCTURAL AXIS NON-INTERFERENCE DECLARATION in Step 0.2 — names C-31/C-32/C-33 as orthogonal (execution order / closure-note format / CS self-reference) with non-interference rule; all other R12-V-05 mechanisms preserved | R12-V-05 has floor (C-31+C-32+C-33+C-17 all PASS). Only the preamble axis declaration is absent. Adding it satisfies C-34. Expected: 26/28 (adds C-34; fails C-35 no CA-1.9, C-36 no tri-dimensional verdict). |
| V-02 | Single-axis C-35: SE-4 STRUCTURED CLOSE block gains explicit TABLE_4 row cross-reference at SE-0 position; CA-1.9 pre-assigned in Phase 0 mandate; CA-1.9 verification row added in Phase 3 verifying this as distinct from CA-1.4 and CA-1.7 | R12-V-05 has floor (C-31+C-32 both PASS). SE-4's STRUCTURED CLOSE already references SE-0 data in passing prose; V-02 makes the TABLE_4 row cross-reference explicit and structural with CA-1.9 as its dedicated verifier. Expected: 26/28 (adds C-35; fails C-34 no axis declaration, C-36 no tri-dimensional verdict). |
| V-03 | Single-axis C-36: CA terminal verdict gains explicit tri-dimensional self-evidence attestation naming R12 Dimension 1 (execution order: SE-0 header / Phase 2 mandate / TABLE_4 before TABLE_1), Dimension 2 (closure-note format: MANUAL GAP blocks at SE-2/SE-3/SE-4), Dimension 3 (CS self-reference: CS-0 cites Registry schema IDs); all other R12-V-05 mechanisms preserved | R12-V-05 has floor (C-22+C-32+C-33 all PASS). The terminal verdict already cites Phase 0 by label per C-22; V-03 extends that attestation to all three R12 dimensions simultaneously. Expected: 26/28 (adds C-36; fails C-34 no axis declaration, C-35 no CA-1.9). |
| V-04 | Combined C-34+C-35: STRUCTURAL AXIS NON-INTERFERENCE DECLARATION (V-01) + SE-4 STRUCTURED CLOSE TABLE_4 SE-0 cross-reference + CA-1.9 (V-02) on R12-V-05 base | Two independent axes. Expected: 27/28 (adds C-34+C-35; fails C-36 only — no tri-dimensional verdict). |
| V-05 | Full combination C-34+C-35+C-36: axis declaration + SE-4 TABLE_4 SE-0 cross-reference + CA-1.9 + tri-dimensional terminal verdict on R12-V-05 base | All three new axes simultaneously. Expected: 28/28 (100.0). |

---

## V-01 — Single-Axis C-34: STRUCTURAL AXIS NON-INTERFERENCE DECLARATION

**Axis:** STRUCTURAL AXIS NON-INTERFERENCE DECLARATION added to Step 0.2 of Phase 0. Names C-31
(execution order), C-32 (closure-note format), C-33 (CS self-reference) as three orthogonal
dimensions with an explicit non-interference rule and per-axis non-interference statements.
R12-V-05 base preserved in all other respects: SE-0 ordering (C-31), exact-label two-part
MANUAL GAP / STRUCTURED CLOSE blocks (C-32), CS-0 acknowledgment (C-33), CA-1.8 extended.
**Hypothesis:** 26/28 — adds C-34; still fails C-35 (no SE-4 STRUCTURED CLOSE TABLE_4 SE-0
cross-ref, no CA-1.9) and C-36 (no tri-dimensional terminal verdict attestation).
...
DECLARATION with axes A-1/A-2/A-3). CA issues handoff to PHASE 1.
...
was independently satisfiable in prior rounds and is composable without structural conflict. The
three axes target disjoint structural elements: phase-sequence/table-columns (A-1),
