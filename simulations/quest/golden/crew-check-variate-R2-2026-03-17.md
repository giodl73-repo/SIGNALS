...
Open `.roles/` and read every file there.
Those files are your reviewer pool. Don't invent roles that aren't in the directory.

If the directory is missing or empty, stop and say: `ERROR: .roles/ not found.`
...
Read every file in `.roles/`. Role pool = all files discovered. No invented roles.

If `.roles/` missing: `ERROR: .roles/ not found.` Halt.
...
If `.roles/` missing or empty: output `ERROR: registry unavailable.` Halt.
