...
|--------|------|------------|
| Role | string | Must match a filename stem in `.craft/roles/`. Zero fabricated names. If no match exists, the role may not appear. |
...
- Directory absent: `ERROR: .craft/roles/ not found.` and stop.
- Directory empty: `ERROR: .craft/roles/ is empty.` and stop.
...
- If the directory doesn't exist: stop, output `ERROR: .craft/roles/ not found.`
- If the directory is empty: stop, output `ERROR: .craft/roles/ is empty.`
...
| Column | Type | Constraint |
|--------|------|------------|
| Role | string | Must match a `.craft/roles/` filename stem. No invented names. |
...
- `.craft/roles/` absent: `ERROR: .craft/roles/ not found.` Halt.
- `.craft/roles/` empty: `ERROR: .craft/roles/ is empty.` Halt.
- Any file missing `lens.verify` or `expertise.relevance`:
  `ERROR: [role-name] missing required fields. Registry malformed.` Halt.
