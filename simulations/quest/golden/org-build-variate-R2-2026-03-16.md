...
Check .roles/ for existing role files.
...
Write one .roles/{area}/{role}.md file per role declared in ROLES-LOADED or
ROLES-NOTE.

Standard depth: 20-30 role files. Deep depth (--depth deep): 50+ role files.
Role files MUST be grouped under area subdirectories: .roles/{area}/{role}.md
...
Glob .roles/. MUST produce ROLES-LOADED:
  ROLES-LOADED:
  - [role name] -- [one-line description from role file]
...
MUST write one .roles/{area}/{role}.md file per role in ROLES-LOADED or ROLES-NOTE.
...
MUST group role files under area subdirectories: .roles/{area}/{role}.md
...
Glob .roles/. If files found, produce ROLES-LOADED:
  ROLES-LOADED:
  - [role name] -- [one-line description from role file]
...
Write one .roles/{area}/{role}.md per role. Five fields required in every file:
...
Group under area subdirectories: .roles/{area}/{role}.md (minimum 2 subdirs).
...
=== [PHASE 3 GATE: ROLE FILES COMPLETE]
  ...
  Output produced: [N] role files in .roles/
...
Glob .roles/. Produce ROLES-LOADED or ROLES-NOTE.
...
Write .roles/{area}/{role}.md for every role:
...
Glob .roles/. MUST produce ROLES-LOADED if files found:
  ROLES-LOADED:
  - [role name] -- [one-line description from role file]
...
MUST write one .roles/{area}/{role}.md per role in ROLES-LOADED or ROLES-NOTE.
...
MUST group under area subdirs: .roles/{area}/{role}.md
REQUIRED: minimum 2 area subdirectories.
FORBIDDEN: all roles in a flat .roles/ directory with no area grouping.
