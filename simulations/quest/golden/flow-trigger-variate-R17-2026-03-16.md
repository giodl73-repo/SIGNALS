...
# flow-trigger — Round 17 (Rubric v14) Variations

**New criterion this round:**
- **C-41** — Register-uniform embedded structural sub-blocks: the C-40 NOTE block adopts
  assertion-register framing consistent with the surrounding CLOSURE CHECK counter assertions,
  eliminating the annotation/obligation register boundary that C-40's physical inseparability
  was designed to close. C-41 requires that every line inside the NOTE block use the same
  `PROPERTY: VALUE [must be CONSTRAINT]` format as the surrounding counters — making the NOTE
  block a peer assertion set, not an appended annotation.

**Round 16 gap analysis:**
- V-01 R16: C-39 FAIL (no consequence statement), C-40 N/A. C-41 FAIL (vacuous — C-40 not satisfied).
- V-02 R16: C-39 WEAK PASS (structural inventory only), C-40 WEAK PASS (inline sentence at end of
  prose counter block, no named note block title). C-41 FAIL (vacuous — C-40 not a full pass).
- V-03 R16: C-39 PASS (standalone section before CLOSURE CHECK), C-40 FAIL (section precedes CLOSURE
  CHECK block, not inside it). C-41 FAIL (vacuous — C-40 FAIL).
- V-04 R16: C-39 PASS (via INERTIA CONTRAST entry), C-40 WEAK PASS (INERTIA CONTRAST section outside
  CLOSURE CHECK). C-41 FAIL (vacuous — C-40 not a full pass).
- V-05 R16: C-39 PASS, C-40 PASS (named NOTE block inside CLOSURE CHECK code fence). C-41 PASS
  retroactively under v14: NOTE block's combined axis (phrasing register + inseparability) makes
  it read as part of the detection record; however, the NOTE content uses narrative/prose register
  rather than the strict `PROPERTY: VALUE [must be VALUE]` format of the surrounding counters.

**Round 17 ladder (C-41):**
R16 V-05 established C-40 PASS with a NOTE block in prose register. R17 probes the register axis
inside the NOTE block — the C-41 ladder:
V-01 (C-41 FAIL vacuous: no NOTE block, C-40 FAIL) →
V-02 (C-41 FAIL vacuous: NOTE adjacent, not inside fence, C-40 PARTIAL) →
V-03 (C-41 FAIL: NOTE inside fence, pure prose register) →
V-04 (C-41 PARTIAL: NOTE inside fence, mixed register — assertion lines + prose consequence) →
V-05 (C-41 PASS: NOTE inside fence, pure assertion register throughout).

**Variation axes selected:**
- Single-axis: Role sequence (V-01), Output format (V-02), Lifecycle emphasis (V-03)
- Combined: Inertia framing + phrasing register (V-04), Phrasing register + assertion-register NOTE (V-05)

---

## V-01

**Variation axis:** Role sequence — Domain Expert obligations appear before Auditor obligations in
the PHASE 0 OBLIGATION REGISTRY row ordering; no embedded NOTE block in the CLOSURE CHECK; the
post-fence attribution note names both layers and both artifacts but states no consequence for
removing either layer.
...
Domain Expert obligations establish the enumeration infrastructure; Auditor obligations guard it.
The CLOSURE CHECK carries per-counter inline role attribution (C-37 PASS) and a post-fence
italicized note reading: "Role attribution operates at two structural time points: (1) declaration
time — the ARTIFACT MANIFEST names the producing role and phase before enumeration begins;
(2) detection time — each CLOSURE CHECK counter names the producing role and phase inline. Both
attribution points are present. Both artifacts are named." C-39 FAIL: structural inventory without
consequence statement. C-40 FAIL: note is post-fence. C-41 FAIL (vacuous path): C-40 is not
satisfied; the NOTE block register question is moot.
...
rather than a code fence; the attribution chain NOTE content appears as a labeled prose paragraph
immediately after the prose counter block, outside any fenced structure; the PHASE 0 OBLIGATION
REGISTRY obligation text carries role + SHALL + artifact but omits temporal constraint.

**Hypothesis:** The PHASE 0 OBLIGATION REGISTRY obligation text reads "The [Role] SHALL produce
the [ARTIFACT]" — naming the role, the modal verb, and the artifact, but omitting the temporal
constraint ("before Phase N begins" / "during Phase 0"). C-38 WEAK PASS: three of four required
components are present; a reader consulting only the Obligation column cannot determine when
the obligation must be fulfilled. The CLOSURE CHECK is reformatted as a prose block with pipe
delimiters — no code fence — so no fence boundary exists to contain any NOTE block. The
attribution chain content appears as a prose paragraph labelled "**Attribution Note:**"
immediately after the last counter line. C-39 PASS: both layers named (ARTIFACT MANIFEST,
CLOSURE CHECK counters), both artifacts named, consequence statement present ("removing either
layer breaks remediation self-sufficiency, not merely reduces coverage"). C-40 PARTIAL: the
attribution note is co-located adjacent to the CLOSURE CHECK section but is not inside a code
fence — no fence exists. C-41 FAIL (vacuous path): C-40 is PARTIAL; the NOTE block register
question requires a fully embedded NOTE block (C-40 PASS) before C-41 can be evaluated.
...
points. Declaration time: the ARTIFACT MANIFEST (ART-06) names the producing role and phase
for each artifact before enumeration begins — accountability is auditable before any gap forms.
...
complete four-component obligation text; a named NOTE block is embedded inside the CLOSURE CHECK
code fence, but its content uses pure prose/narrative register rather than the assertion-register
format of the surrounding counters.

**Hypothesis:** Lifecycle emphasis variation adds a phase-rationale paragraph to every phase
body, explaining what structural gap the phase closes before the next begins. The PHASE 0
OBLIGATION REGISTRY carries complete four-component obligation text per row (C-38 PASS). The
CLOSURE CHECK carries per-counter inline role attribution (C-37 PASS). A named NOTE block is
embedded as the last element inside the CLOSURE CHECK code fence — C-40 PASS: removing the
NOTE requires editing the code fence itself. However, the NOTE's content uses narrative prose
register: "Role attribution for artifact production exists at two structural time points..." —
sentences, not `PROPERTY: VALUE [must be VALUE]` declarations. C-39 PASS: both layers named,
both artifacts named, consequence statement present ("removing either layer breaks remediation
self-sufficiency"). C-41 FAIL: the NOTE block is physically embedded (C-40 PASS) but uses
annotation/prose register rather than the assertion-register format (`PROPERTY: VALUE [must be
CONSTRAINT]`) shared by the surrounding counters. A reader scanning the CLOSURE CHECK can
distinguish the NOTE's prose sentences from the counter entries by register alone — the
register boundary that C-40's physical inseparability was designed to close remains open.
...
attribution means role identity is only discoverable after a gap is detected. Removing
detection-time attribution means an ABSENT counter is anonymous; the remediation target
is only determinable via secondary navigation, and if that navigation fails, the target
is unidentified. Removing either layer is understood to break remediation self-sufficiency,
not merely reduce coverage.
---- END NOTE ----------------------------------------------------------------
```
...
CLOSURE CHECK code fence using mixed register: assertion-format lines for each attribution
layer followed by a prose consequence sentence.

**Hypothesis:** The PHASE 0 OBLIGATION REGISTRY carries complete four-component obligation
text per row (C-38 PASS). The INERTIA CONTRAST section adds a seventh mechanism entry for
the dual-time attribution chain (identical to V-04 R16). C-39 PASS via INERTIA CONTRAST
mechanism path. The CLOSURE CHECK carries per-counter inline role attribution (C-37 PASS)
and a named NOTE block embedded inside the code fence (C-40 PASS). The NOTE block opens
with assertion-register lines per attribution layer:
...
These two lines use `PROPERTY: VALUE [must be VALUE]` format co-equal with the surrounding
counters. However, the NOTE block then continues with a prose consequence sentence: "Both
layers are required for remediation self-sufficiency; removing either layer breaks
self-sufficiency, not merely reduces coverage." This prose sentence switches from assertion
register to annotation/consequence register. C-41 PARTIAL: the NOTE block partially adopts
assertion-register format (two assertion-register lines) but appends a prose sentence that
breaks register uniformity. A reader scanning the CLOSURE CHECK can distinguish the prose
sentence from the counter entries by register alone — the register boundary is not fully
eliminated.
...
DECLARATION TIME ATTRIBUTION (ART-06):        present [must be present]
DETECTION TIME ATTRIBUTION (ART-NN counters): present [must be present]
Both layers are required for remediation self-sufficiency; removing either layer breaks
self-sufficiency, not merely reduces coverage. The ARTIFACT MANIFEST attribution and
CLOSURE CHECK counter attribution SHALL be declared and maintained together.
---- END NOTE ---------------------------------------------------------------
```
...
+ assertion-register NOTE block (all NOTE content uses `PROPERTY: VALUE [must be CONSTRAINT]`
format, co-equal with surrounding counters, with no prose sentence inside the NOTE block).

**Hypothesis:** The PHASE 0 OBLIGATION REGISTRY carries complete four-component obligation
text per row (C-38 PASS). The INERTIA CONTRAST section covers all seven mechanism entries
(six Phase 0 mechanisms plus DUAL-TIME ATTRIBUTION CHAIN — identical to V-04). C-39 PASS
via INERTIA CONTRAST mechanism path. The CLOSURE CHECK carries per-counter inline role
attribution (C-37 PASS) and a named NOTE block embedded inside the code fence (C-40 PASS).

The NOTE block differs from V-04 in one structural property: every line inside the NOTE block
uses the same `PROPERTY: VALUE [must be CONSTRAINT]` assertion register as the surrounding
counters. No prose sentence appears inside the NOTE block. The three NOTE lines are:
...
C-41 PASS: the NOTE block adopts assertion-register framing consistent with the surrounding
CLOSURE CHECK counter assertions. A reader scanning the CLOSURE CHECK cannot distinguish the
NOTE's assertion lines from the counter entries by register alone — the NOTE block is a peer
assertion set, not an appended annotation. The annotation/obligation register boundary that
C-40's physical inseparability was designed to close is eliminated by register uniformity.
The combined-axis signal (phrasing register + inseparability) is non-trivial: C-40 provides
structural containment; C-41 provides semantic co-domain. Both are required for the NOTE block
to read as part of the detection record rather than as an embedded annotation.
