...
- Existing `.roles/` subdirectories (if any)
...
**Inertia-advocate**: path `.roles/{area-slug}/inertia-advocate.md`
...
**Standard roles**: path `.roles/{area-slug}/{role-slug}.md`

Five fields — orientation, lens, expertise, scope: `standard`, collaborates_with.

**Specialized roles**: path `.roles/{area-slug}/{role-slug}.md`
...
1. **Regenerate area**: `--area "{area-slug}"` — rewrites all .roles/ files for the
