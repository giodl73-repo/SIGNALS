...
Write one `.md` file per role to `.craft/roles/{area}/`. Each file must contain all six required fields:
...
After all role files are written, emit a `REGISTRY.md` file to `.craft/roles/{area}/REGISTRY.md`:
...
Output area: .craft/roles/{derived-area-name}/
```
...
Write one `.md` file per role to `.craft/roles/{area}/`. Use this exact template for every file. Fill in every slot. Do not omit any field or sub-field.
...
Write `.craft/roles/{area}/REGISTRY.md` with: area, total role count, stock role names, domain expert names and their derivation source, and any coverage gaps detected.
...
Write one `.md` file per role to `.craft/roles/{area}/`. Write domain-expert roles first (from Phase 1), then stock roles (from Phase 2).
...
Write `.craft/roles/{area}/REGISTRY.md` using this template:
...
**Purpose:** Emit one `.md` file per role to `.craft/roles/{area}/`. Write domain experts first (Phase 1), then stock roles (Phase 2).
...
**Completion condition:** Every role file exists at `.craft/roles/{area}/` AND every file contains all six top-level fields with all required sub-fields filled (not empty strings or placeholder text).
...
DO write `.craft/roles/{area}/REGISTRY.md` containing all five required fields:
