...
Check `.craft/roles/` for role files relevant to {topic}.
...
```
ROLES-NOTE: No .craft/roles/ files found — using inferred roles: [list]
```
...
"none — using inferred roles: [list]" if .craft/roles/ is absent]
...
1. Check `.craft/roles/` for files relevant to {topic} or the domain
...
If `.craft/roles/` is absent or contains no relevant files, mark Source as
...
DO NOT invent role names that contradict roles already defined in `.craft/roles/`.
If no role files exist: DO write "ROLES-NOTE: No .craft/roles/ files found — using inferred roles: [list]."
...
If `.craft/roles/` is absent: mark Source as "absent — using inferred roles" and
