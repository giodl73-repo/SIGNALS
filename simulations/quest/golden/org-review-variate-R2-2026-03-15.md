...
- **`--depth deep`**: Include all roles in `.roles/signal/`. State the total count.
...
- **`--depth deep`**: Include all roles from `.roles/signal/`. Assign each to their gate. State total count and gate distribution.
...
- **`--depth deep`**: Include all non-challenger roles from `.roles/signal/`, plus the inertia-advocate (last). State total count.
...
- **`--depth deep`**: Include all roles from `.roles/signal/`. The inertia-advocate bracket remains in place; all other roles run between the two bracket positions. State total count.
...
- **Standard** (default): Select reviewers from `.roles/signal/` whose `expertise.relevance` matches this artifact type. Minimum 2 reviewers; the inertia-advocate is always included. State which roles were selected and why.
- **`--depth deep`**: Include all roles in `.roles/signal/`. State total count. Every reviewer issues a verdict card.
