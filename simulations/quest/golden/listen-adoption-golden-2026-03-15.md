...
date: 2026-03-14
rounds: 20
rubric_final: listen-adoption-rubric-v21-2026-03-14.md
score: 330
status: GOLDEN
---

# listen-adoption — Golden Prompt

## What Made It Golden

Five patterns distinguish the final V-05 body from the Round 1 baseline:

1. **Named Inertia IDs as answer-form Q-anchors (C-29).** Every structural position that could silently omit an inertia citation is instead posed as an answer-form question: `Q-BARRIER`, `Q-CHAMPION`, `Q-TRIGGER`, `Q-INTERVENTION`. The question syntax makes the citation *the only coherent answer*, collapsing the omission space from open-ended fill to forced-choice. No other structural mechanism achieves zero-omission for non-primary inertia IDs.

2. **Phase-entry Protected behavior census (Axis A / C-58).** Before each phase body is written, five items enumerate every I-0X as `active` or `displaced`. This closes the gap that Q-BARRIER alone leaves: Q-BARRIER enforces the *primary* inertia per phase; the census enforces *all five* at every boundary. Non-primary inertia silently drifting through phases becomes structurally conspicuous.

3. **Arc integrity propagation with Protected behavior at boundary (Axes B+C / C-59, C-60).** SECTION K carries a five-row displacement arc table with a `Protected behavior at boundary` column. When a MISMATCH fires, the ARC CORRECTION BLOCK label is self-describing at displacement-stake level: `ARC CORRECTION BLOCK -- PHASE N close -> PHASE N+1 open MISMATCH -- Protected behavior at boundary: [I-0X: named behavior]`. This extends the C-55 label self-sufficiency standard from Gate H citation failures into the arc repair pathway.

4. **Downstream Citation Contract locks generation-time vs. audit-time fill.** The SECTION A contract table separates `Protected behavior: fill at generation time` from `Gate H Status: fill at audit time`. This two-phase discipline prevents audit fields from being pre-populated during generation and generation fields from being deferred to audit — each fill obligation is isolated to its correct stage.

5. **PART 6 and PART 7 removed — zero criterion loss.** Sensitivity Analysis and Signal Readiness Table carried no audited criteria (C-11 through C-60) and were referenced by no gate. Their removal reduces prompt length by ~15% without any rubric impact, confirming the prompt had reached structural saturation in Rounds 1–19 and residual mass was prose scaffolding.

---

## Final Rubric Criteria Summary (v21 — 350 pts)

| Range | Criteria | Theme |
|-------|----------|-------|
| C-11 | SECTION A Named Inertia table, 5 rows I-01..I-05 | Named Inertia existence |
| C-13 | Downstream Citation Contract — four mandatory sink locations per I-0X | Citation completeness |
| C-14 | PART 4 SLOT-KEY champion entries with archetype-bridge rationale | Champion network |
| C-15 | PART 3 concrete-action retention intervention column | Churn risk register |
| C-19 | Inline CORRECTION BLOCK with BEFORE/AFTER fields *(Paradox ceiling — always FAIL on well-formed generation)* | Self-repair completeness |
| C-24 | Terminal Invariant: BEFORE and AFTER fields mandatory per correction | Terminal audit integrity |
| C-25/C-45 | Phase entry state declaration + phase-close displacement ledger in every phase | Phase boundary tracking |
| C-26 | PART 5 Incumbent Impact column | Intervention ranking |
| C-27 | PART 3 retention-intervention column (concrete action, not surveillance flag) | Churn mitigation quality |
| C-28/C-36/C-38 | TABLE 3 SLOT-KEY typed transition pairs | Spread mechanism structure |
| C-29 | Q-BARRIER / Q-CHAMPION / Q-TRIGGER / Q-INTERVENTION embedded at structural positions | Answer-form citation enforcement |
| C-30/C-51 | SECTION K gate execution stamp table | Terminal gate record |
| C-32/C-34/C-35 | STRUCTURAL CONTRACT both requirements present + MECHANISM STATE blockquotes in H/I/J/pre-verification | Correction block mechanism |
| C-37/C-41/C-43 | TABLE 1 Inertia ID column + Incumbent Dependency column + SLOT-KEY prefix | Role archetype map completeness |
| C-39 | PHASE 3 CHASM-A / CHASM-B / CHASM-C named subsections | Chasm structure |
| C-40/C-42/C-48 | PART 2 Named Inertia + Incumbent position per CHASM slot | Chasm expansion depth |
| C-44 | PART 5 Targets inertia column (Q-INTERVENTION) | Intervention-inertia linkage |
| C-49/C-50/C-54 | Downstream Citation Contract Protected behavior + Gate H Status columns | Citation audit trail |
| C-53 | SECTION K DISPLACEMENT ARC INTEGRITY CHECK 5-row table | Arc existence |
| C-55 | Gate H correction block label carries I-0X + Protected behavior | Citation repair label quality |
| C-56 | SECTION K Citation Contract Completion Record carries Protected behavior column | Citation completion record |
| C-57 | Arc MISMATCH triggers ARC CORRECTION BLOCK | Arc repair obligation |
| C-58 (new) | Phase-entry Protected behavior census enumerates all I-0X as active or displaced | Per-behavior phase tracking (Axis A) |
| C-59 (new) | ARC CORRECTION BLOCK label carries Protected behavior at boundary | Arc repair label self-sufficiency (Axis B) |
| C-60 (new) | SECTION K arc table carries Protected behavior at boundary column | Arc table displacement-stake record (Axis C) |

**Scoring:** 350 pts maximum. Golden threshold (80%): 280 pts. Paradox ceiling: 345/350 (C-19 structural FAIL persists on all well-formed generations — answer-form architecture prevents gate failure from firing, which is the correct outcome).
...
MECHANISM STATE in SECTIONS H, I, J and before VERIFICATION MODE (C-34). C-55: Gate H labels carry Protected behavior. C-56: SECTION K citation record carries Protected behavior column. C-57: arc MISMATCH triggers ARC CORRECTION BLOCK. Axis A: phase entry Protected behavior census. Axis B: ARC CORRECTION BLOCK label carries Protected behavior at boundary. Axis C: SECTION K arc table carries Protected behavior at boundary column.
