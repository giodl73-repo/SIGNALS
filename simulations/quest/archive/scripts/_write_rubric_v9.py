rubric = open('C:/src/sim/simulations/quest/rubrics/topic-roadmap-rubric-v8-2026-03-17.md').read()

# Strip the v8 header block (lines 1-14 before the first ---)
# and replace with v9 header, then patch formula and add C-21/22/23

v9 = """# topic-roadmap rubric -- v9
...
| Version | 8 -> 9 |
| Formula | `/ 24 * 10` -> `/ 30 * 10` |
| Aspirational range | C-09-C-20 -> C-09-C-23 |
| **C-21 added** | Dual-site INERTIA-GATE enforcement -- from V-03/V-05: gate labeled at Phase 5 entry AND Phase 6 entry independently; single-site labeling allows Phase 6 to open without passing through Phase 5 |
| **C-22 added** | Restart isolation clause in Phase 1 read-barrier -- from V-05: explicit instruction covers the restart-without-context scenario; a barrier without restart handling is only half-isolated |
| **C-23 added** | Dedicated verdict vocabulary block before Phase 5 -- from V-05: standalone DEFEATED/HOLDS definition block with forward-path statements appears before Phase 5 begins; V-02/V-04 achieve C-13 via distributed framing, V-05 achieves it via a self-contained block that is independently verifiable |

The three new criteria are orthogonal to one another and to existing criteria:
- C-21 tests whether gate enforcement covers two structural transition points.
- C-22 tests whether the pre-read barrier survives a model restart.
- C-23 tests whether verdict semantics are encapsulated in a standalone block rather than embedded across phase instructions.
A skill can pass any subset independently. V-05 achieves all three; V-03 achieves C-21 only; V-02/V-04 achieve neither C-21 nor C-22 nor C-23 in the V-05 form.
...
Composite = (essential_pass / 5 * 60)
          ...
          + (aspirational_raw / 30 * 10)
```

Where `aspirational_raw` = sum of per-criterion scores across C-09-C-23
(full pass = 2, partial = 1, fail = 0; max = 30).
...
| Change | Detail |
|--------|--------|
| Version bump | version: 8 -> version: 9 |
| Formula denominator | / 24 -> / 30 (15 aspirational criteria x 2 max each) |
| Aspirational range | C-09-C-20 -> C-09-C-23 |
| C-21 added | Dual-site INERTIA-GATE enforcement -- from V-03/V-05 Round 8 pattern: gate labeled at Phase 5 entry AND Phase 6 entry independently; C-11 tests existence of the gate, C-21 tests whether both transition points carry independent enforcement |
| C-22 added | Restart isolation clause in Phase 1 read-barrier -- from V-05 Round 8 pattern: explicit restart-without-context clause added to Phase 1 barrier; C-09 tests pre-read isolation on first run, C-22 tests whether restart paths are also guarded |
| C-23 added | Dedicated verdict vocabulary block before Phase 5 -- from V-05 Round 8 pattern: standalone DEFEATED/HOLDS definition block with forward-path statements; C-13 tests whether semantics are defined (distributed framing passes), C-23 tests whether they are encapsulated in a self-contained block verifiable without reading all phase instructions |

**Why three new criteria rather than one or two:** Each tests an orthogonal structural property.
A skill can label INERTIA-GATE at both sites (C-21) while leaving the restart path open (C-22 fail).
It can handle restarts (C-22) while distributing verdict semantics across phases (C-23 fail).
It can have a dedicated verdict vocabulary block (C-23) while labeling INERTIA-GATE at only one site (C-21 fail).
V-05 achieves all three; V-03 achieves C-21 only; V-02 and V-04 achieve neither in the V-05 form.
"""

out = 'C:/src/sim/simulations/quest/rubrics/topic-roadmap-rubric-v9-2026-03-17.md'
