path = 'C:/src/sim/simulations/quest/rubrics/topic-echo-rubric-v7-2026-03-16.md'
...
C-19 requires the Chain Integrity Audit to emit CHAIN-COMPLETE or CHAIN-FLAG tokens per entry.
C-22 requires the CHAIN-FLAG token to function as a hard gate: any CHAIN-FLAG blocks further
production until the flag is resolved. Without a blocking condition, a reviewer can acknowledge
CHAIN-FLAG tokens and proceed to completion anyway -- the audit is visible but advisory. With a
gate, chain integrity failure is a production stop, not an observation. First appeared in Round 6
V-01's PHASE 3B specification ("CHAIN-FLAG blocks progression"); all five Round 6 variations
retain it.

**C-23 -- Distillation phase blocking condition with named gate**
(behavior, gate: C-20)

C-20 requires a RULES-ANCHORED traceability check confirming tier alignment. C-23 requires that
check to name an explicit blocking condition: production cannot proceed to the next phase until
all rules show ALIGNED. Without it, the traceability check is advisory -- a reviewer can note
misalignment and the artifact can still be completed. With a named gate, the distillation layer
enforces the same production-stop semantics as C-16 and C-18's per-entry attestation gates. First
appeared in Round 6 V-02's PHASE 4B specification ("Do not proceed to Phase 5 until all rules
show ALIGNED"); V-03, V-04, V-05 retain it.

**C-24 -- Enforcement mechanism section at artifact head position**
(behavior, gate: C-21)

C-21 requires the enforcement mechanism declaration to be in an independently navigable section.
C-24 requires that section to be placed at position 1 of the artifact -- before PBI, before Handle
Ledger, before any entries. C-21 ensures a reviewer can find the mechanism classification via
section headings; C-24 ensures a reviewer who opens the artifact encounters it before any content.
Without head positioning the reviewer must navigate to the section; with it, orientation precedes
content, making the enforcement frame the first interpretive lens rather than a findable annotation.
V-03 and V-05 achieve this (dedicated `== ENFORCEMENT MECHANISM ==` section, position 1); V-01,
...
- New relationships: C-19/C-22 (blocking gate extends chain audit), C-20/C-23 (named gate
  extends distillation check), C-21/C-24 (head position extends navigability)
