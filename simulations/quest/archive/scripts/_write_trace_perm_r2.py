...
Three new aspirational criteria added from Round 1 excellence signals (V-01 through V-04):

- **C-10** (real-time gap accumulation): V-01 required gaps logged at discovery; V-03 proved the failure mode -- inline signals not structurally accumulated are lost. Addresses gap collapse, a pattern absent from v1.
- **C-11** (explicit non-escalation documentation): V-01 rule-out format ("Checked [Role]: ... No escalation because [specific reason]") is a higher discipline than C-05 (which only requires finding escalation paths). Negative-case discipline distinguishes thorough traces from cherry-picked positives.
- **C-12** (phase-gate completeness checkpoint): V-04 inventory-as-gated-phase axis introduced explicit self-audit before advancing. Prevents early-phase gaps (missed entities, omitted fields) from propagating silently into analysis.

The aspirational tier now has 5 criteria (N=5); point allocation per criterion decreases from 5 to 2, preserving the 10-point tier total.
...
Aspirational pass count (N=5): ___  ->  ___ / 5 * 10 = ___
...
with open(r"C:\\src\\sim\\simulations\\quest\\rubrics\\trace-permissions-rubric-v2-2026-03-15.md", "w", encoding="utf-8") as f:
    f.write(content)
