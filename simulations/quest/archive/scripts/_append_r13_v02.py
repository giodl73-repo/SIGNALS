...
**Axis:** C-35 -- SE-4 STRUCTURED CLOSE block gains an explicit row-level TABLE_4 cross-reference
at SE-0 position using the format "TABLE_4 (filled at SE-0) row [ID] -- [role] privilege ceiling:
[pattern]". CA-1.9 added as a verification row targeting SE-4 STRUCTURED CLOSE content
specifically, distinct from CA-1.4 (SE-0 ordering) and CA-1.7 (MANUAL GAP exact labels).
PHASE 3 description updated to mention CA-1.9. All other mechanisms from R12-V-05 preserved.
**Hypothesis:** 26/28 (adds C-35; fails C-34 -- no axis declaration; fails C-36 -- no tri-dimensional attestation).
...
M4 row IDs CA-1.1 through CA-1.9 pre-assigned; CA-1.9 reserved for C-35 SE-4 STRUCTURED CLOSE
verification). CA issues handoff to PHASE 1.
...
distinct from CA-1.4 (SE-0 ordering) and CA-1.7 (MANUAL GAP exact labels).
...
**Note:** CA-1.6 verifies SHALL-F (CS-EXPECTATION-VIOLATED structure). CA-1.7 verifies SHALL-G
(exact-label two-part blocks). CA-1.8 verifies C-29+C-33 (CS-2/CS-3 Registry registration +
CS-0 acknowledgment). CA-1.9 verifies C-35 (SE-4 STRUCTURED CLOSE TABLE_4 row cross-reference
at SE-0 position -- pre-assigned as a Phase 0 contract obligation).
...
column. SE-2/SE-3/SE-4 use exact-label two-part closure blocks per SHALL-G. SE-4 STRUCTURED CLOSE
MUST include TABLE_4 row cross-reference at SE-0 position per SHALL-D extension.*
...
Configured), surfacing pre-event what the security roles UI conceals. All Not Configured sensitive
fields are forwarded to TABLE_5 as MISSING-FLS gaps with Tier column identifying highest-privilege
tier with access.
...
STRUCTURED CLOSE:
TABLE_3 maps effective scope for every role by Tier, recording OWD baseline, scope modifier from
team or role depth, and resulting effective scope as a single traceable row.
...
Note: the TABLE_4 row cross-reference above MUST appear here in the STRUCTURED CLOSE content;
CA-1.9 verifies this location specifically.
...
- Preamble anchor -- SHALL-D extension as authored by CA in Phase 0: SE-4 STRUCTURED CLOSE MUST include "TABLE_4 (filled at SE-0) row [ID] -- [role] privilege ceiling: [pattern]". Cross-reference MUST appear in the STRUCTURED CLOSE content, not only in analysis prose above the block. CA-1.9 is the pre-assigned verification row for this requirement (Phase 0 contract obligation).
- Verification -- SE-4 STRUCTURED CLOSE content contains TABLE_4 row cross-reference? Format matches "TABLE_4 (filled at SE-0) row [ID]"? SE-0 position explicitly named? Row-level identifier used (not generic "SE-0 data")? -> PASS / FAIL
...
[C-01 through C-05 with CA-1.1 through CA-1.5 results. SHALL-F (CA-1.6). SHALL-G exact-label two-part compliance (CA-1.7). C-29+C-33 CS-0 acknowledgment (CA-1.8). C-35 SE-4 STRUCTURED CLOSE TABLE_4 row cross-reference (CA-1.9). Overall: COMPLIANT / NON-COMPLIANT. Open items attributed.]
