...
The output must acknowledge whether `.roles/` contains role definitions for the
domain. Silence about roles is not acceptable.

**Pass condition:** One of two conditions is met:
(a) A ROLES-LOADED line or block lists role names sourced from `.roles/`, or
(b) An explicit ROLES-NOTE states that no `.roles/` files were found and that
...
Role names sourced from `.roles/` (or from the inferred roles list when no role
