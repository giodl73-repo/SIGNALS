...
From V-01/V-04/V-05 C-14 mechanism analysis. C-14 verifies that body text contains phase-appropriate vocabulary (outcome check). C-16 verifies the structural guarantee that produced it: a per-ticket commitment table recorded before body writing, binding each ticket to its opening phrase in advance. An output can pass C-14 (bodies happen to contain discovery phrases) while failing C-16 (vocabulary appeared by instruction-following, not pre-commitment). An output can also fail C-14 while passing C-16 if the committed phrases are not phase-appropriate -- the two criteria are orthogonal.

Pass condition: A per-ticket vocabulary commitment table is present in the output with one row per ticket showing ticket ID and committed opening phrase. Each ticket body begins with its committed phrase, verifiable by matching body openings to table entries.

**C-17 -- Role-Phase Vocabulary Matrix** (depth)
From V-03/V-05 C-14 mechanism analysis. C-14 is a 1D phase constraint: discovery vocabulary in early-phase bodies, operational vocabulary in late-phase bodies. C-17 is a 2D role-x-phase constraint: the vocabulary register is both role-appropriate AND phase-appropriate. An SRE body in Phase-1 uses different language than an end-user body in Phase-1 (different register, not just different content), and both differ from an SRE body in Phase-3. An output can pass C-14 (has discovery phrases in early bodies) while failing C-17 (same generic discovery phrases across all roles, no role differentiation). The matrix makes vocabulary expectations jointly determined by two axes.

Pass condition: A role-phase vocabulary matrix or equivalent register table appears in the output covering at least two distinct roles and two distinct phases (minimum 4 cells). Each cell specifies a vocabulary register, example phrase, or sentence scaffold distinct from adjacent cells. Ticket bodies use vocabulary consistent with their assigned role-phase cell when cross-referenced against the matrix.
...
with open("C:/src/sim/simulations/quest/rubrics/listen-support-rubric-v4-2026-03-15.md", "w", encoding="utf-8") as f:
    f.write(content)
