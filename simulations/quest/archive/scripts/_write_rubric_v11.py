path = 'simulations/quest/rubrics/mock-accept-rubric-v11-2026-03-17.md'
content = r"""# mock-accept-rubric-v11-2026-03-17.md
...
| What | v10 | v11 |
|------|-----|-----|
| New criteria | C-34, C-35, C-36 (from R9) | C-37, C-38, C-39 (from R10) |
| Denominator | 28 | 31 |
| C-09 absent denom | 23 | 26 |
| New failure modes | MA-32, MA-33, MA-34 | MA-35, MA-36, MA-37 |
| Golden | R9 V-05 sole golden (28/28 = 100.0) | R10 V-05 sole golden (31/31 = 100.0) |

---

### Three new criteria extracted from R10

**C-37 -- GATE F SECTION FORMAT QUALIFIERS**

R10 V-01, V-04, and V-05 extended the GATE F section list from stable IDs alone to stable IDs with format qualifiers: "Coverage: 4 columns / Structural records: 9 sentences / Risk: Urgency column / Next Steps: ordered list". A format qualifier names the structural element expected in each section -- column count, sentence count, required column name, or list form. R10 V-02 and V-03 name sections by stable IDs only (satisfies C-24) but carry no format qualifier per section. Distinct from C-24 (section presence by name) because C-37 governs structural verification: the gate assertion becomes a per-section structure check rather than a presence check, making GATE F independently verifiable without parsing the artifact body.

**C-38 -- GATE B ENUMERATED PARTITION OUTPUT**

R10 V-02, V-04, and V-05 replaced the count-only GATE B assertion with an enumerated partition output: each namespace is named under its partition bucket (auto-flagged / Architect-phase / Strategy-phase) so the gate output is auditable by namespace inspection, not only by numeric count. R10 V-01 and V-03 use a count-only partition assertion (satisfies C-11) but do not enumerate namespace membership. Distinct from C-11 (count assertion at gate) because C-38 requires each namespace to be identified by name in the gate output, making partition membership independently verifiable without cross-referencing the evaluation records.

**C-39 -- ROLE HANDOFF BLOCK**

R10 V-03 and V-05 inserted a named ROLE HANDOFF block between GATE C and STEP 4 (Strategy evaluation phase). The ROLE HANDOFF explicitly transfers running state from the Architect-phase role to the Strategy-phase role: current DECISION SCOREBOARD values (from GATE C UPDATE), current INERTIA LEDGER state (from GATE C), and the list of namespaces remaining at REAL-REQUIRED default entering the Strategy phase. R10 V-01, V-02, and V-04 omit the ROLE HANDOFF; state transfer occurs implicitly across the phase boundary. Distinct from C-31 (INERTIA LEDGER membership records inside gates) and C-36 (DECISION SCOREBOARD numeric tracking) because C-39 governs the explicit transfer act: a named structural block that makes the between-phase state transition observable as an artifact boundary, not merely derivable from gate records.

---

### R10 ceiling re-scored against v11

| Variation | C-37 | C-38 | C-39 | Asp passes | Asp score | Total |
|-----------|------|------|------|------------|-----------|-------|
| V-05 (GATE F qualifiers + GATE B enum + ROLE HANDOFF) | PASS | PASS | PASS | 31/31 | 10.0 | **100.0** |
| V-04 (GATE F qualifiers + GATE B enum; no ROLE HANDOFF) | PASS | PASS | FAIL | 30/31 | 9.7 | **99.7** |
| V-01 (GATE F qualifiers only) | PASS | FAIL | FAIL | 29/31 | 9.4 | **99.4** |
| V-02 (GATE B enum only) | FAIL | PASS | FAIL | 29/31 | 9.4 | **99.4** |
| V-03 (ROLE HANDOFF only) | FAIL | FAIL | PASS | 29/31 | 9.4 | **99.4** |
| R9 V-05 (previous golden) | FAIL | FAIL | FAIL | 28/31 | 9.0 | **99.0** |

R10 V-05 is the sole golden. R10 V-04 is silver at 99.7. R9 V-05 is demoted to 99.0.
...
### Aspirational -- C-09 to C-39

**Scoring:** (passes / 31) x 10

**C-09 N/A cascade:** C-09 -> C-15 -> C-20 -> C-23 -> C-26; denominator when C-09 absent = 26 (31 - 5). C-24, C-25, C-27, C-28, C-29, C-30, C-31, C-32, C-33, C-34, C-35, C-36, C-37, C-38, and C-39 are independent of C-09 status.
...
**C-09 N/A cascade:** if C-09 is absent -> C-15 N/A -> C-20 N/A -> C-23 N/A -> C-26 N/A; active denominator becomes 26 (31 - 5). C-24, C-25, C-27, C-28, C-29, C-30, C-31, C-32, C-33, C-34, C-35, C-36, C-37, C-38, and C-39 are independent of C-09 status.
"""
