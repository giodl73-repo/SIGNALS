v04 = r"""
## V-04 -- Combined C-34+C-35: Axis Declaration + SE-4 STRUCTURED CLOSE Row Cross-Reference

**Axis:** V-01 axis (C-34 structural axis non-interference declaration) + V-02 axis (C-35 SE-4
STRUCTURED CLOSE TABLE_4 row cross-reference + CA-1.9) on the R12-V-05 base. The preamble
gains the STRUCTURAL AXIS NON-INTERFERENCE DECLARATION block naming C-31/C-32/C-33 as orthogonal
dimensions. SE-4 STRUCTURED CLOSE gains the explicit TABLE_4 row cross-reference at SE-0
position. CA-1.9 added as a distinct verification row. No tri-dimensional terminal attestation.
**Hypothesis:** 27/28 (adds C-34+C-35; fails C-36 -- no tri-dimensional terminal attestation).
...
with Tier columns on TABLE_1 and TABLE_3; CS-2 and CS-3 with SE-updatable columns and SE-update
protocols) and the Criterion Enforcement Matrix preamble (Step 0.2: SHALL-A through SHALL-G,
M4 row IDs CA-1.1 through CA-1.9 pre-assigned; Structural Axis Non-Interference Declaration
naming C-31/C-32/C-33 as orthogonal dimensions). CA issues handoff to PHASE 1.
...
open with exact-label two-part closure blocks per SHALL-G. SE-4 STRUCTURED CLOSE MUST include
TABLE_4 row cross-reference at SE-0 position per SHALL-D extension. Handoff to PHASE 3.
...
CA-1.9 verifies SE-4 STRUCTURED CLOSE contains TABLE_4 row cross-reference at SE-0 position --
...
SE-updatable columns: SE Cross-Reference; SE Confirmation.
SE update protocol: SE populates after TABLE_4 analysis at SE-0.
...
SE-updatable columns: SE Cross-Reference; SE Confirmation.
SE update protocol: SE populates after TABLE_1/TABLE_3 analysis.

### Step 0.2 -- Criterion Enforcement Matrix Preamble (CA-authored, Phase 0)

**Declared rule: M1, M2, M3 simultaneously active. M4 pre-assigned (CA-1.1 through CA-1.9).**
...
- SHALL-D: TABLE_4 at SE-0 (before SE-1), all four vectors, Checked? = YES. Rule-outs name specific mechanism and reason. Extension for C-35: SE-4 STRUCTURED CLOSE block MUST include a TABLE_4 row cross-reference at the SE-0 position using the format "TABLE_4 (filled at SE-0) row [ID] -- [role] privilege ceiling: [pattern]". This cross-reference MUST appear in the STRUCTURED CLOSE content, not only in SE-4 analysis prose. CA-1.9 verifies.
...
- SHALL-G: SE-2, SE-3, SE-4 each open with two-part labeled block -- Line 1: `MANUAL GAP [<exact CONTEXT label>]:`; Line 2: what manual audits miss; Line 3: `STRUCTURED CLOSE:`; Line 4: closing mechanism. CA-1.7 verifies.

**STRUCTURAL AXIS NON-INTERFERENCE DECLARATION (CA-authored, Phase 0):**

The three Round 12 aspirational structural axes operate on orthogonal dimensions with zero
mechanism overlap. This declaration is separate from the C-01-C-05 mechanism mapping above.

| Axis ID | Criterion | Structural Dimension | Non-Interference Statement |
|---------|-----------|---------------------|---------------------------|
| A-1 | C-31 | SE execution order (privilege-tier descent) | C-31 governs when SE-0 executes relative to SE-1 and when TABLE_4 is populated relative to TABLE_1. Activating C-32 does not alter SE execution order. Activating C-33 does not alter SE execution order. |
| A-2 | C-32 | Closure-note format (exact-label two-part blocks) | C-32 governs the structural format of MANUAL GAP / STRUCTURED CLOSE blocks at SE-2, SE-3, SE-4. Activating C-31 does not alter closure-note format. Activating C-33 does not alter closure-note format. |
| A-3 | C-33 | CS self-reference (Registry acknowledgment at execution time) | C-33 governs whether CS-0 cites Schema Registry entries by exact Schema ID before authoring expectation rows. Activating C-31 does not alter CS-0 acknowledgment structure. Activating C-32 does not alter CS-0 acknowledgment structure. |

All three axes are simultaneously active in this output. A model executing this prompt MUST
satisfy all three axes concurrently. Satisfying one axis does not justify relaxing another.
...
admin-equivalent roles with record scope and capabilities. Note admin capabilities that bypass
lower-tier controls. This establishes the privilege ceiling for all lower-tier roles.
...
STRUCTURED CLOSE:
TABLE_2 enumerates every sensitive field with explicit FLS status (Configured / Not Configured),
surfacing pre-event what the security roles UI conceals. All Not Configured sensitive fields are
...
cross-referenced against OWD settings. Cite: TABLE_4 (filled at SE-0) row [ID] -- [role]
privilege ceiling: [pattern that established the admin-tier ceiling]. Cross-tier differential
summary: for the most privileged Admin-tier role and the most restricted Restricted-tier role,
identify the specific enforcement layer where access diverges and state whether the divergence
is expected. State what sharing-rule interaction was confirmed or ruled out in SE-0 TABLE_4.
Note: the TABLE_4 row cross-reference MUST appear here in the STRUCTURED CLOSE content;
CA-1.9 verifies this location as a distinct audit target from CA-1.4 and CA-1.7.
...
elements. Verification line follows. Inline concatenation fails C-24.*
...
- Verification -- SE-2 `MANUAL GAP [Blind spot 1 -- Post-incident FLS gap]:`? SE-3 `MANUAL GAP [Blind spot 2 -- Cumulative privilege blind spot]:`? SE-4 `MANUAL GAP [Blind spot 3 -- OWD-sharing-rule override gap]:`? Each followed by `STRUCTURED CLOSE:`? -> PASS / FAIL per section.
...
- Registry anchor -- Schema ID TABLE_4: [Vector, Checked?, Finding, Severity] -- rows populated at SE-0 before SE-1. SE-4 STRUCTURED CLOSE block is the second part of the SHALL-G two-part block at SE-4. This row verifies that STRUCTURED CLOSE content includes a TABLE_4 row cross-reference at SE-0 position at row-level identifier granularity. Distinct from CA-1.4 (SE-0 ordering) and CA-1.7 (MANUAL GAP exact labels).
- Preamble anchor -- SHALL-D extension as authored by CA in Phase 0: SE-4 STRUCTURED CLOSE MUST include "TABLE_4 (filled at SE-0) row [ID] -- [role] privilege ceiling: [pattern]". Cross-reference MUST appear in STRUCTURED CLOSE content. CA-1.9 is the pre-assigned verification row for this requirement.
- Verification -- SE-4 STRUCTURED CLOSE content contains TABLE_4 row cross-reference? Format "TABLE_4 (filled at SE-0) row [ID]"? SE-0 position named? Row-level identifier used? -> PASS / FAIL
...
[C-01 through C-05 with CA-1.1 through CA-1.5 results. SHALL-F (CA-1.6). SHALL-G (CA-1.7). C-29+C-33 CS-0 acknowledgment (CA-1.8). C-35 SE-4 STRUCTURED CLOSE row cross-reference (CA-1.9). Overall: COMPLIANT / NON-COMPLIANT. Open items attributed.]
...
**Axis:** All three new R13 axes on the R12-V-05 base:
- C-34: STRUCTURAL AXIS NON-INTERFERENCE DECLARATION block in preamble naming C-31/C-32/C-33 as three orthogonal dimensions (V-01 axis).
- C-35: SE-4 STRUCTURED CLOSE TABLE_4 row cross-reference at SE-0 position + CA-1.9 verification row (V-02 axis).
- C-36: TRI-DIMENSIONAL SELF-EVIDENCE ATTESTATION in CA Format Compliance Verdict naming per-dimension output-body evidence sources (V-03 axis).
**Hypothesis:** 28/28.
...
protocols) and the Criterion Enforcement Matrix preamble (Step 0.2: SHALL-A through SHALL-G,
M4 row IDs CA-1.1 through CA-1.9 pre-assigned; Structural Axis Non-Interference Declaration
naming C-31/C-32/C-33 as orthogonal dimensions with per-axis non-interference statements).
CA issues handoff to PHASE 1.

**PHASE 1 -- CS (Customer Success):**
Executes second. Opens with CS-0 sub-section (Schema Registry acknowledgment: cites Registry
schema IDs for CS-2 and CS-3 by exact Schema ID label, lists SE-updatable columns for each,
confirms expectation format before authoring any expectation rows). Then produces CS-1, CS-2,
...
with two-part exact-label closure blocks per SHALL-G. SE-4 STRUCTURED CLOSE MUST include TABLE_4
row cross-reference at SE-0 position per SHALL-D extension. Issues handoff to PHASE 3.

**PHASE 3 -- CA-1 (CA Verification Pass):**
CA returns. Each CA-1 row performs double-anchor reaffirmation as structurally distinct labeled
elements. Verification line follows. Inline concatenation fails C-24. CA-1.8 verifies CS-0
acknowledgment precedes CS-1. CA-1.9 verifies SE-4 STRUCTURED CLOSE contains TABLE_4 row
cross-reference at SE-0 position -- distinct from CA-1.4 and CA-1.7. CA Format Compliance
Verdict includes tri-dimensional self-evidence attestation naming per-dimension body evidence
sources for execution order (C-31), closure-note format (C-32), and CS self-reference (C-33).
...
- SHALL-B: TABLE_2 lists all PII, Financial, and Audit-Compliance fields with explicit FLS status. Null case explicit. Gap? YES rows in TABLE_5.
- SHALL-C: Every TABLE_1 role in TABLE_3 with Tier and Effective Scope filled. Ambiguous scope in TABLE_5.
- SHALL-D: TABLE_4 at SE-0 (before SE-1), all four vectors, Checked? = YES. Rule-outs name specific mechanism and reason. Extension for C-35: SE-4 STRUCTURED CLOSE block MUST include a TABLE_4 row cross-reference at the SE-0 position using the row-level identifier format "TABLE_4 (filled at SE-0) row [ID] -- [role] privilege ceiling: [pattern]". This cross-reference MUST appear in the STRUCTURED CLOSE content, not only in SE-4 analysis prose outside the block. CA-1.9 verifies this as a distinct audit target from CA-1.4 and CA-1.7.
- SHALL-E: TABLE_5 has at least one named gap. Zero-gap case requires explicit evidence rows.
...
The three Round 12 aspirational structural axes operate on orthogonal dimensions with zero
mechanism overlap. This declaration is separate from the C-01-C-05 mechanism mapping above.
Activating any one axis does not constrain or alter either of the other two.

| Axis ID | Criterion | Structural Dimension | Non-Interference Statement |
|---------|-----------|---------------------|---------------------------|
| A-1 | C-31 | SE execution order (privilege-tier descent) | C-31 governs when SE-0 executes relative to SE-1 and when TABLE_4 is populated relative to TABLE_1. Activating C-32 (closure-note format) does not alter SE execution order. Activating C-33 (CS self-reference) does not alter SE execution order. |
| A-2 | C-32 | Closure-note format (exact-label two-part blocks) | C-32 governs the structural format of MANUAL GAP / STRUCTURED CLOSE blocks at SE-2, SE-3, SE-4. Activating C-31 (SE-0 ordering) does not alter closure-note format. Activating C-33 (CS-0 acknowledgment) does not alter closure-note format. |
| A-3 | C-33 | CS self-reference (Registry acknowledgment at execution time) | C-33 governs whether CS-0 cites Schema Registry entries by exact Schema ID before authoring expectation rows. Activating C-31 (SE-0 ordering) does not alter CS-0 acknowledgment structure. Activating C-32 (closure-note format) does not alter CS-0 acknowledgment structure. |
...
two-part closure blocks per SHALL-G. SE-4 STRUCTURED CLOSE MUST include TABLE_4 row cross-
reference at SE-0 position per SHALL-D extension.*
...
is expected. State what sharing-rule interaction was confirmed or ruled out in SE-0 TABLE_4.
Note: the TABLE_4 row cross-reference MUST appear here in the STRUCTURED CLOSE content;
CA-1.9 verifies this location specifically as a distinct audit target from CA-1.4 and CA-1.7.
...
**CA-1.9 -- C-35 verification** *(SE-4 STRUCTURED CLOSE TABLE_4 row cross-reference at SE-0 position)*:
- Registry anchor -- Schema ID TABLE_4: [Vector, Checked?, Finding, Severity] -- rows populated at SE-0 before SE-1. SE-4 STRUCTURED CLOSE block is the second part of the SHALL-G two-part block at the SE-4 sub-section. This row verifies that the STRUCTURED CLOSE content includes a TABLE_4 row cross-reference naming the SE-0 position at row-level identifier granularity. Distinct from CA-1.4 (which verifies SE-0 ordering) and CA-1.7 (which verifies MANUAL GAP exact label carry-through).
- Preamble anchor -- SHALL-D extension as authored by CA in Phase 0: SE-4 STRUCTURED CLOSE MUST include "TABLE_4 (filled at SE-0) row [ID] -- [role] privilege ceiling: [pattern]". Cross-reference MUST appear in the STRUCTURED CLOSE content, not only in analysis prose outside the block. CA-1.9 is the pre-assigned verification row for this requirement (Phase 0 contract obligation).
- Verification -- SE-4 STRUCTURED CLOSE content contains TABLE_4 row cross-reference? Format matches "TABLE_4 (filled at SE-0) row [ID]"? SE-0 position explicitly named? Row-level identifier used (not generic "SE-0 data" reference)? -> PASS / FAIL

**CA Format Compliance Verdict -- Phase 0 citation:**
Citing: CA-authored Schema Registry (Step 0.1, Phase 0), CA-authored preamble (Step 0.2, Phase 0), Phase 0 by label.
[C-01 through C-05 with CA-1.1 through CA-1.5 results. SHALL-F (CA-1.6). SHALL-G exact-label two-part block compliance (CA-1.7). C-29+C-33 CS-0 acknowledgment (CA-1.8). C-35 SE-4 STRUCTURED CLOSE TABLE_4 row cross-reference (CA-1.9).]
...
alone, without consulting the prompt. Each dimension's specific output-body evidence source is
named below.

| Dimension | Criterion | Output-Body Evidence Source | Body-Verifiable? |
|-----------|-----------|----------------------------|-----------------|
| Execution order | C-31 | PHASE 2 section header "SE-0 -- Admin-Tier Inventory and Escalation Vectors (executes before TABLE_1)" precedes SE-1; TABLE_4 is populated at SE-0 before TABLE_1 at SE-1 -- the ordering is readable from the section header sequence and table placement in the output body alone, without prompt inspection | CONFIRMED |
| Closure-note format | C-32 | MANUAL GAP [exact label] / STRUCTURED CLOSE two-part structure appears at SE-2, SE-3, SE-4 -- the bracket pattern and two-line format are body-readable by structure alone; an evaluator can identify each anchor by reading the block structure without evaluating prompt instructions | CONFIRMED |
