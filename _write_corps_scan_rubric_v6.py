...
Five new aspirational criteria extracted from Round 5 excellence signals (V-01 through V-04,
all 220/220).

**C-23 -- Inline detected-from: traceability field embedded in YAML team entries** (V-01)
V-01 C-02 evidence: "detected-from: field traces every team to ROLE 2 inventory".
C-09 requires a pre-YAML inventory; C-16 requires a repair path when traceability fails;
C-23 additionally requires the YAML output itself to carry inline traceability: each team entry
includes a detected-from: annotation field naming the specific inventory signal that justified
the team. The YAML becomes self-documenting -- a reviewer can verify C-02 grounding without
consulting the pre-YAML inventory separately. Requires C-02 and C-09.

**C-24 -- Pivot mode candidates enumerated before selection** (V-01)
V-01 C-06 evidence: "ROLE 2: pivot mode candidates + selected pivot with Signal citation".
C-06 requires a rationale for the selected mode; C-24 additionally requires the pivot phase to
enumerate all modes considered before declaring the selected one -- e.g., "directory (selected),
concept (rejected: no abstraction layer), service (rejected: directories already service-named)."
Directly declaring the selected mode without enumerating rejected alternatives does not pass.
The candidate list makes the selection auditable against alternatives without re-reading the
repo. Requires C-06 and C-12.

**C-25 -- Each IVR triple carries a phase-scoped structural label** (V-01, V-02, V-03)
V-01 C-22 evidence: "13 labeled IVR triples across all roles; no prose-only constraints".
V-02 C-22 evidence: "12 SPECs cover every phase" (labels SPEC-01 through SPEC-12).
V-03 C-22 evidence: "11 REQ blocks each with INVARIANT/VIOLATION/REPAIR" (labels REQ-1.1
through REQ-4.1). C-22 requires all structural constraints to use IVR triples; C-25 additionally
requires each triple to carry a structural label that scopes it to its phase and enumerates it
within that phase -- e.g., IVR-1A, IVR-2B, IVR-3E; or SPEC-07, SPEC-12; or REQ-3.4. The label
enables: (a) cross-reference from other constraints ("see IVR-2A"); (b) counting to verify
...
with open("C:/src/sim/simulations/quest/rubrics/corps-scan-rubric-v6-2026-03-17.md", "w") as f:
    f.write(rubric)
print("written")
