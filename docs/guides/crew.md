...
relevant roles. Zero config required -- stock roles activate by default when no .craft/roles/
directory exists.
...
- You do not have .craft/roles/ configured but want expert review now.
...
When `.craft/roles/` does not exist, crew uses a set of stock roles that cover the most
...
When `.craft/roles/` exists, crew reads it and selects relevant roles based on the artifact
...
|-------|-------------|----------------|
| `crew-roles` | Generates a typed role registry for a domain. Analyzes product/codebase to determine expert perspectives needed. Each role gets orientation, lens, expertise, scope, collaborates_with. Inertia-advocate always included. Outputs .craft/roles/{area}/ directory. | Before switching from crew to corp. When you want to customize the role definitions. |
| `crew-review` | Runs any artifact through the full org (or stock roles if no .craft/roles/ exists). Per-role findings with severity. Review matrix: role, findings, severity, recommendation. --depth deep runs all roles; standard runs relevant roles only. | Anytime you have an artifact worth reviewing. No config required. |
...
No .craft/roles/ detected. Stock roles activate. Seven roles review the spec.
...
Analyzes the repo and the existing spec. Generates `.craft/roles/billing/` with:
...
All roles in .craft/roles/ review the artifact, not just the auto-selected relevant ones.
...
crew-roles writes to `.craft/roles/`:
