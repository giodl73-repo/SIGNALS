...
.craft/roles/{area}/        role definition files (one per role per team)
```

`org.yaml` declares your org structure: divisions, tribes, pillars, teams, and the exec office.
It does not describe what each person does -- that is what `.craft/roles/` is for. The roles
in `.craft/roles/` are markdown files with each role's orientation, lens, expertise, scope,
and collaboration pattern.

When corp-rob or corp-pr runs, it reads `org.yaml` to select relevant participants, then
loads each participant's role file from `.craft/roles/` to instantiate their perspective.
...
|-------|-------------|----------------|
| `corp-scan` | Walks the repo and produces a draft org.yaml. Pivot modes: directory, concept, service, domain. Does NOT write .craft/roles/ -- outputs draft for human review first. | First-time setup. When onboarding a new codebase to corp. |
| `corp-build` | Reads confirmed org.yaml and generates org-chart.md and all .craft/roles/ files. Supports arbitrary group nesting. Inertia-advocate always included per team. | After confirming org.yaml. Regenerate when org structure changes. |
| `corp-rob` | Runs Review of Business stages. Stages: leadership, teams, pm, tpm, arch-board, exec-office. --stage all for full ROB. --scope {group} for one division. Reads org.yaml + .craft/roles/. | Before planning cycles, feature commits, or major releases. |
...
| `corp-chart` | Generates org structure for a product or domain from existing .craft/roles/. Produces org-chart.md with ASCII box diagram, headcount by area, committee charters, operating rhythm. | When you need a visual org representation. After corp-build. |
...
Generates org-chart.md and `.craft/roles/` directory tree with a role file for every
