...
5. **Custom roles unlock depth** — some skills produce better results with custom expert roles in `.roles/`. Any skill that uses personas surfaces on first run: "To add personas for your domain, create .roles/signal/{name}/ROLE.md." Never required, always beneficial.
6. **Artifacts land in the repo** — user chooses where (e.g., `design/simulations/`), organized by technique
7. **Metadata in `.roles/`** — custom roles, config, persona definitions
...
.roles/                            <- lightweight, personalized
...
|-------|-----------------|
| `/prove:review` | Industry-specific expert reviewers (create in `.roles/simulate/`) |
...
| `panel/.roles/panel-reviewer/` | 51 academic reviewer personas (OLE format) | 51 |
...
Writes `simulate.yaml` to `.roles/`. All subsequent runs use this config.

### Custom roles (when prompted by a skill)

```
/prove:review says: "For deeper reviews, create custom expert roles in .roles/simulate/"
```
