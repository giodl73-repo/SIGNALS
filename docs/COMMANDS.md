...
**60 skills** across 14 namespaces. Type `/signal` in Claude Code to see this index interactively.
...
| `/behavior` | `/rhythm-behavior` | rhythm | Run the full technical simulation campaign. Orchestrates: flow-li |
| `/brainstorm` | `/discover-brainstorm` | discover | Generate a pool of ideas for any topic -- more than you need, so ✓ |
| `/brief` | `/rhythm-brief` | rhythm | Run the full topic narrative campaign. Orchestrates: topic-new (r ✓ |
| `/build` | `/roles-build` | roles | Read a confirmed org.yaml and generate the full org. Outputs: (1) ✓ |
| `/causal` | `/discover-causal` | discover | Test whether the causal mechanism linking your feature to the cla |
| `/chart` | `/roles-chart` | roles | Generate org structure for a product or domain: areas, divisions, ✓ |
| `/check` | `/roles-check` | roles | Run any artifact through the full org. Reads .craft/roles/ and se ✓ |
...
| `/committee` | `/roles-committee` | roles | Simulate a committee meeting before the real one. Types: ROB (pro ✓ |
...
| `/competitors-alt` | `/discover-competitors-alt` | discover | UNIFIED VERSION (A/B test against discover-competitors). Single p |
...
| `/create` | `/roles-create` | roles | Create a new role or sub-role in .craft/roles/. Three modes: (1) |
| `/customers` | `/validate-customers` | validate | 12 customer personas (C-01 through C-12) evaluate a feature for u ✓ |
| `/decide` | `/rhythm-decide` | rhythm | Run the full pre-commitment decision campaign. Orchestrates: scou ✓ |
...
| `/feasibility-alt` | `/discover-feasibility-alt` | discover | UNIFIED VERSION (A/B test against discover-feasibility). Single p |
| `/feedback` | `/validate-feedback` | validate | Simulate customer reactions to a spec or design before it ships ✓ |
| `/generate` | `/roles-generate` | roles | Generate a typed role registry for a domain. Analyzes product/cod ✓ |
...
| `/permissions` | `/simulate-permissions` | simulate | Trace who can do what through RBAC, security roles, teams, and fi ✓ |
| `/pitch` | `/specify-pitch` | specify | Author a pitch deck narrative in exec, developer, and maker versi ✓ |
| `/preview` | `/tools-preview` | tools | Generate a mock artifact for a single namespace. Same annotation ✓ |
| `/product-review` | `/roles-product-review` | roles | Run Review of Business (ROB) stages. Stages: leadership (exec bri ✓ |
| `/proposal` | `/specify-proposal` | specify | Author a proposal or ADR with options and trade-offs. Three optio ✓ |
| `/pull-request` | `/roles-pull-request` | roles | Run a PR through the full org. Selects reviewers based on files c ✓ |
| `/qa` | `/rhythm-qa` | rhythm | Run the full design validation campaign. Orchestrates: review-des ✓ |
...
| `/rubric` | `/quest-rubric` | quest | Define what good output looks like for a skill. Given a skill spe ✓ |
| `/scan` | `/roles-scan` | roles | Walk a repo and produce a draft org.yaml -- the org configuration ✓ |
| `/score` | `/quest-score` | quest | Score N skill outputs against a rubric. Per-criterion PASS/PARTIA ✓ |
| `/setup` | `/signal-setup` | topic | Configure Signal in your workspace after installation. Checks for ✓ |
...
| `/status` | `/rhythm-status` | rhythm | Show topic coverage and commitment readiness. DISPLAY ONLY. Use - ✓ |
| `/story` | `/rhythm-story` | rhythm | Synthesize all signals into a coherent narrative. NOT a summary o ✓ |
...
| `/users` | `/validate-users` | validate | 5 persona advocates walk through a design from their user perspec ✓ |
| `/variate` | `/quest-variate` | quest | Generate N distinct prompt variations of a skill body. Each varia ✓ |
| `/websearch` | `/discover-websearch` | discover | Search the public web for evidence supporting or refuting the hyp ✓ |
...
### topic
*Narrative — status, story, achievements, index*

| Command | Bare | Description |
|---|---|---|
| `/signal` | `/signal` | Show the Signal command index. Lists all available skills organized by |
| `/signal-layout` | `/layout` | Show or change your Signal navigation layout. Five variants: A) groupe |
| `/signal-setup` | `/setup` | Configure Signal in your workspace after installation. Checks for exis |

### quest
*Quest loop — rubric, variate, score, golden prompt*

| Command | Bare | Description |
|---|---|---|
| `/quest-golden` | `/golden` | Find the golden prompt through systematic variation and scoring. Full |
| `/quest-rubric` | `/rubric` | Define what good output looks like for a skill. Given a skill spec and |
| `/quest-score` | `/score` | Score N skill outputs against a rubric. Per-criterion PASS/PARTIAL/FAI |
| `/quest-variate` | `/variate` | Generate N distinct prompt variations of a skill body. Each variation |
