...
Reads `.craft/roles/` and selects reviewers relevant to the artifact type. Each role reviews from their perspective. Produces a review matrix: role, findings, severity, recommendation. No roles configured? Crew uses a set of stock roles automatically.
