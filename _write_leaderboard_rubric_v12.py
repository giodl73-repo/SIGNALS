content = """\
## corps-leaderboard Rubric v12 -- 2026-03-17

---

### What changed: v11 -> v12

Two new aspirational criteria extracted from R14 scorecard excellence signals.

**C-32 -- Milestone Proximity Ladder** (from Seed N -- V-04, V-05)
C-03 tests whether all three team milestones are present with status and evidence. C-21 and C-23
test nearest-miss reporting at the achievement level. C-32 tests a different dimension -- whether
each NOT YET milestone row carries a per-precondition decomposition of the remaining deficit,
breaking the aggregate gap into individual per-topic or per-contributor action targets. An
instruction that reports "shared coverage NOT YET -- 3 topics need a second contributor" satisfies
C-03's evidence requirement but fails C-32 unless it identifies which 3 topics and how many
contributors each requires. A proximity ladder that covers only one milestone fails -- the
decomposition must be required for every NOT YET milestone. The Skeptic gate must also
explicitly include milestone proximity ladders in its list of known prior context, raising the
novelty bar so that the Team Insight cannot satisfy C-25 merely by restating a per-milestone gap
that the ladder already exposes.
- **Derived from**: R14 excellence signal -- V-04, V-05. **Passes**: V-04, V-05.

**C-33 -- Flag/Achievement Logical Consistency Check (Echo Detection)** (from Seed S -- V-05)
C-27 tests whether the health and inertia phases are structurally separated. C-30 tests whether
the gate polices cross-phase contamination. C-33 tests a third, orthogonal dimension -- whether
the output includes a dedicated post-health consistency scan that enforces logical coexistence
rules between inertia flag assignments and achievement claims before the action-writing phase
begins. The target violations are impossibility conditions not detectable by a data-level
discrepancy check: `healthy-momentum` flag raised on a topic with `Solo Pioneer EARNED`
(healthy-momentum requires >=2 contributors; Solo Pioneer requires exactly 1 -- logically
incompatible); any inertia flag raised on a topic with `First Signal LOCKED` (flags require at
least one file to evaluate; LOCKED First Signal means 0 files). The echo check produces an echo
table with explicit retraction statements for each violation, and the Strategist pre-write check
must explicitly exclude echo-retracted flags from `resolves:` targeting. An instruction that
defines a data-level discrepancy table but no impossibility scan fails C-33 even if C-27 and
C-30 are satisfied. An instruction that mentions flag consistency in prose without a structured
...
with open('C:/src/sim/simulations/quest/rubrics/corps-leaderboard-rubric-v12-2026-03-17.md', 'w') as f:
    f.write(content)
