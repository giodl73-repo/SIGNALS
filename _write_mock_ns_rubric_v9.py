content = r"""# mock-ns Rubric v9

## Changelog

### v9 (2026-03-16) -- R8 patterns promoted to aspirational criteria

**Two new criteria (C-29, C-30) from R8.** Two new failure traps. C-27 content-agnostic confirmation appended. Scoring max rises from 120 to **124** (22 aspirational x 2 = 44).

| ID | Text | Source |
|----|------|--------|
| C-29 | The declarative sentence must have the step as direct nominative subject -- possessive-nominal constructions ("This step's [noun phrase] is to...") fail because the step is demoted to genitive modifier and a nominalization derived from the step becomes the grammatical subject | R8 V-02 possessive-nominal boundary |
| C-30 | The declarative sentence must not promote an artifact to nominative subject position in active voice -- "The TOPICS.md status line is the first output of this step" fails because the artifact is subject and the step appears only as oblique possessive modifier "of this step" | R8 V-01 artifact-as-subject boundary |

**Extracted patterns:**

- **C-29** (from V-02 vs V-04): V-02's declarative is "This step's defining action is to emit first." The grammatical subject is "defining action" -- a nominalization. "This step" is in genitive/possessive modifier position. The step is not the agent; an abstraction derived from the step's role is the agent. C-28 and C-29 are independent: C-28 prohibits output-primacy predicates; C-29 prohibits nominalized-abstraction subjects. A sentence can use step-role language in the predicate while demoting the step to possessive modifier in the subject and thereby fail C-29 while appearing to satisfy C-28. Test: substitute "It" for the subject noun phrase -- if "It" refers to an abstraction ("the defining action") rather than to the step itself, the step is not the grammatical subject.

- **C-30** (from V-01 vs V-04): V-01's declarative is "The TOPICS.md status line is the first output of this step." The grammatical subject is the artifact "The TOPICS.md status line." The step appears only as "of this step" -- an oblique possessive modifier in the predicate. This is a third distinct failure mode: neither output-sequence-subject (C-28 canonical form) nor possessive-nominal-subject (C-29) -- but artifact-as-subject in active voice. The intent is to describe the step's emit behavior, but the syntactic structure fails: the artifact is the agent of the identity assertion, not the step.

- **C-27 content-agnostic confirmation** (from V-03): R8 V-03's third sentence is "No other step may begin before this emission completes." -- an enforcement sentence with operational value, not mere elaboration. It names the prohibited action and restates the gate. The hypothesis tested whether enforcement value exempts a sentence from the count. It does not. C-27 is confirmed content-agnostic: three sentences in the gate = FAIL regardless of sentence purpose, type, or utility.

- **Compound predicate positive signal** (from V-04): V-04's declarative is "This step emits first and resolves the tier variable." The step is nominative subject; the compound predicate merges emit primacy with tier resolution. C-28, C-29, and C-30 are all satisfied because the step remains the direct nominative agent of both predicates. Multiple pipeline obligations can be encoded in the declarative via compound predicate without displacing the step from subject position. This is a permissive note added to C-29 and C-30.

**Two new failure traps:**
- "Possessive-nominal declarative" (V-02): "This step's defining action is to emit first" -- grammatical subject is a nominalization ("defining action"); step is genitive modifier. Fails C-29 even when the predicate correctly captures the step's role and C-27/C-28 structural requirements appear satisfied. The intent is correct; the syntax demotes the step.
- "Artifact-as-subject active declarative" (V-01): "The TOPICS.md status line is the first output of this step" -- grammatical subject is the output artifact; step appears only as oblique modifier "of this step." Fails C-30 even when the semantic content describes the step's output and C-27/C-28/C-29 appear satisfied. Distinct from C-28's output-sequence failure mode ("The first observable output of this step is...") in that the artifact is directly in subject position rather than via output-position predicate.

**Two new discriminator notes** added to C-28: "C-28 passing does not imply C-29 passing" and "C-28 passing does not imply C-30 passing."

**R8 variate scores under v9 (max 124):**

| Variate | C-27 | C-28 | C-29 | C-30 | Score |
|---------|------|------|------|------|-------|
| V-04 (Compound-Predicate Declarative) | PASS | PASS | PASS | PASS | 124/124 |
| V-05 (Zero-Lexeme Synonym Depth) | PASS | PASS | PASS | PASS | 124/124 |
| V-01 (Artifact-as-Subject Active) | PASS | PASS | PASS | **FAIL** | 122/124 |
| V-02 (Possessive-Nominal Declarative) | PASS | PASS | **FAIL** | PASS | 122/124 |
| V-03 (Enforcement Three-Sentence Gate) | **FAIL** | PASS | PASS | PASS | 122/124 |

V-04 and V-05 are the only variates to achieve 124/124. V-01, V-02, and V-03 each fail one criterion at -2, each from a distinct failure mode, cleanly separating C-27, C-29, and C-30 as independent discriminators.
...
*Raise the bar once essential and recommended are stable. Each criterion is worth 2 pts (22 x 2 = 44; max score = 124).*
