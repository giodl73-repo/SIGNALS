The full 22-finding competitive brief is already in the golden. Here it is in full — the canonical output for `plugin-competitors-2026-03-15.md`:
...
- Teams that have shipped without research before will do so again.
...
| Technical moat | Acquired by Cognition (Devin's parent) for ~$250M, Dec 2025; #1 LogRocket AI Dev Tool Power Rankings (Feb 2026) |
...
**F-09:** Linear's AI workflow for 2026 is explicitly "from drafting PRDs to pushing PRs" (linear.app/ai) — and its MCP server integration with Cursor and Claude means it is moving toward the handoff point Signal targets. Linear does not gather evidence; it structures and tracks work. But its PRD-to-issue bridge is the closest named workflow to Signal's scout-to-draft pipeline. If Linear adds a `/research` namespace that produces structured findings, Signal's PM-facing namespaces face a real challenger.
...
| Threat | LOW-MODERATE for Signal directly; HIGH for Signal's PM audience if Signal expands into scout/listen |

**F-22:** Productboard, Jira PD, and Mixpanel are the 2026 consolidation anchors for PM-side discovery. None produce structured technical evidence artifacts — no feasibility assessments, architecture traces, or compliance checks. Signal's `/scout:`, `/draft:`, and `/trace:` namespaces are the gap these tools explicitly leave open. The cleaner positioning: Signal is downstream of PM discovery, feeding the technical evidence layer that PM tools cannot produce. In enterprise deals, Signal should integrate with Productboard/Jira PD as the "technical depth" layer rather than competing for the PM budget.
...
A team commits to building Feature X on intuition. They spend 6 weeks building. On day 43, they learn:
...
- Three of the twelve customer personas will never use it because the UX assumes a workflow they don't have (missed by `/review:customers`)
...
- Add `--scope plugin-ecosystem` to focus exclusively on Claude Code plugins and MCP servers. Removes SaaS tools. Tightens whitespace to "what no MCP server currently owns." Primary competitor shifts from "inertia" to "generic prompting."

**AMEND-02: Add internal competitor analysis (build vs. buy)**
- Add `--include internal` with the organization's existing tooling inventory. Reframes the matrix from "Signal vs. market" to "Signal vs. what we already pay for." Produces the highest-value output for teams evaluating adoption alongside existing tooling.

**AMEND-03: Add time-to-value benchmarks per skill vs. manual equivalent**
- Add `--mode benchmarks` to add a "time-to-value" column. Quantifies the inertia argument: "A manual `/scout:competitors` equivalent takes 4-8 hours of senior PM time; Signal produces it in under 30 minutes of async interaction."
...
**22 findings across 13 named competitors + 3 structural threats** (inertia, MCP marketplace noise, vibe coding culture). The new findings vs. the 2026-03-14 version:

| New finding | Competitor | Why it matters |
|-------------|------------|----------------|
| F-16 | Windsurf | Cognition acquisition; persistent memory inches toward decision tracking |
| F-17 | GitHub Copilot Workspace | Plan agent + CLAUDE.md support = highest-threat named competitor |
| F-18 | OpenAI Codex CLI | Open-source o3 CLI = zero-cost informal Signal substitute |
| F-19 | Sourcegraph Cody | Enterprise compliance baseline Signal must match or exceed |
| F-20 | MCP ecosystem | 18K+ servers = discoverability is the GTM problem |
| F-21 | Vibe coding | Cultural counter-narrative; position Signal as the remedy, not the prevention |
| F-22 | PM discovery tools | Productboard/Jira PD are upstream allies, not competitors |
